...
import { getLiveDir, safeSessionId } from './lib/pictorial-paths.mjs';
 ...
 * <div data-pictorial-carbonize> so the slot keeps a single root node.
 */
function buildCarbonizeReplacement({
  ...
  const pushCarbonizeBody = (bodyIndent) => {
    ...
    lines.push(bodyIndent + commentSyntax.open + ' pictorial-carbonize-start ' + id + ' ' + commentSyntax.close);
    lines.push(bodyIndent + '<style data-pictorial-css="' + id + '">' + (isJsx ? '{`' : ''));
    ...
    if (paramValues && Object.keys(paramValues).length > 0) {
      lines.push(
        bodyIndent + commentSyntax.open + ' pictorial-param-values ' + id + ': ' + JSON.stringify(paramValues) + ' ' + commentSyntax.close,
      );
    }
    lines.push(bodyIndent + commentSyntax.open + ' pictorial-carbonize-end ' + id + ' ' + commentSyntax.close);
    lines.push(bodyIndent + '<div data-pictorial-variant="' + variantNum + '" ' + variantStyleAttr + '>');
  ...
  if (isJsx) {
    ...
    lines.push(indent + '<div data-pictorial-carbonize="' + id + '" ' + wrapperStyle + '>');
...
function buildAcceptedWrappedSource(id, variantNum, lines, targetFile, paramValues) {
  ...
  const hasHelperAttrs = variantText.includes('data-pictorial-variant');
...
function readSourceShadowPreviewMeta(content, id) {
  ...
  const wrapperRe = new RegExp('<[^>]+data-pictorial-variants=(["\'])' + escaped + '\\1[^>]*>');
  const match = String(content || '').match(wrapperRe);
  if (!match) return null;
  const tag = match[0];
  if (readHtmlAttr(tag, 'data-pictorial-preview') !== 'source-shadow') return null;
  const sourceFile = readHtmlAttr(tag, 'data-pictorial-source-file');
  const sourceStartLine = Number(readHtmlAttr(tag, 'data-pictorial-source-start'));
  const sourceEndLine = Number(readHtmlAttr(tag, 'data-pictorial-source-end'));
...
function findMarkerBlock(id, lines) {
  let start = -1;
  let end = -1;
  const startPattern = 'pictorial-variants-start ' + id;
  const endPattern = 'pictorial-variants-end ' + id;
 ...
 * the `<div data-pictorial-variants="ID">` outer wrapper so the picked
 ...
 * (with `data-pictorial-variants="ID"`) and its matching `</div>`. We
...
function expandReplaceRange(block, lines, isJsx) {
  ...
  // Walk back for the wrapper `<div data-pictorial-variants="..."` opener.
...
function isVariantEndMarkerLine(line, id) {
  return new RegExp('pictorial-variants-end\\s+' + escapeRegExp(id) + '(?:\\s|--|\\*/|$)').test(line);
}

function hasVariantWrapperAttr(line, id) {
  const escaped = escapeRegExp(id);
  return new RegExp(`data-pictorial-variants\\s*=\\s*(?:"${escaped}"|'${escaped}'|\\{["']${escaped}["']\\})`).test(line);
}
 ...
 *   - CSS `@scope ([data-pictorial-variant="N"])` strings that look like the
...
function extractOriginal(lines, block) {
  ...
  const inner = extractInnerByAttr(text, 'data-pictorial-variant="original"');
...
function extractVariant(lines, block, variantNum) {
  const text = stripStyleAndJoin(lines, block);
  const inner = extractInnerByAttr(text, 'data-pictorial-variant="' + variantNum + '"');
 ...
 *
 * Handles three shapes of `<style data-pictorial-css="ID" ...>`:
...
function extractCss(lines, block, id) {
  const styleAttr = 'data-pictorial-css="' + id + '"';
  let inStyle = false;
  const content = [];
...
function findSessionFile(id, cwd) {
  const result = findSourceFile({
    query: 'pictorial-variants-start ' + id,
    cwd,
