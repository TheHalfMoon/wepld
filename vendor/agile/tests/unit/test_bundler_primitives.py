...
from agile_cli.bundler import BundlerError
...
from agile_cli.bundler.services.primitives import (
...
def test_offline_workflow_allows_bundled(tmp_path: Path, monkeypatch):
    # A workflow that ships with Agile must install even with --offline.
    import agile_cli
    import agile_cli._assets as assets

    monkeypatch.setattr(
        assets, "_locate_bundled_workflow", lambda wid: tmp_path / "wf"
    )
    calls: list[str] = []
    monkeypatch.setattr(agile_cli, "workflow_add", lambda wid: calls.append(wid))
...
def test_assert_pinned_version_matches_passes():
    from agile_cli.bundler.services.primitives import _assert_pinned_version
...
def test_preset_install_preserves_explicit_zero_priority(tmp_path: Path, monkeypatch):
    ...
    class _FakeManager:
        def install_from_directory(self, directory, agile_version, priority):
...
def _plan(manifest):
    from agile_cli.bundler.services.installer import InstallPlan
    from agile_cli.bundler.models.manifest import ComponentRef as CR
