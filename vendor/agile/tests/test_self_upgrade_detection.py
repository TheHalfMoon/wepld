...
class TestDetectionUvTool:
    ...
    def test_posix_uv_tool_prefix_matches(self, uv_tool_argv0):
        ...
        assert "uv/tools/wepld-agile" in signals.matched_prefix.replace("\\", "/")
    ...
    def test_no_argv_match_falls_through_to_unsupported(self, unsupported_argv0):
        with patch("agile_cli._version.shutil.which", return_value=None), patch(
            "agile_cli._version._editable_marker_seen", return_value=False
        ):
    ...
    def test_bare_argv0_is_resolved_via_path_lookup(self, monkeypatch, tmp_path):
        if os.name == "nt":
            ...
            fake_dir = tmp_path / "uv" / "tools" / "wepld-agile" / "bin"
        else:
            monkeypatch.setenv("HOME", str(tmp_path))
            fake_dir = (
                tmp_path / ".local" / "share" / "uv" / "tools" / "wepld-agile" / "bin"
            )
        fake_dir.mkdir(parents=True)
        fake_specify = fake_dir / "agile"
        fake_specify.write_text("#!/usr/bin/env python\n")
        monkeypatch.setattr("sys.argv", ["agile"])
        with patch(
            "agile_cli._version.shutil.which",
            side_effect=lambda name: str(fake_specify) if name == "agile" else None,
        ):
            method = _detect_install_method()
        assert method == _InstallMethod.UV_TOOL
    ...
    def test_prefix_match_does_not_accept_sibling_directory(self, monkeypatch, tmp_path):
        monkeypatch.setenv("HOME", str(tmp_path))
        fake_dir = tmp_path / ".local" / "share" / "uv" / "tools" / "wepld-agile2" / "bin"
        fake_dir.mkdir(parents=True)
        fake_specify = fake_dir / "agile"
        fake_specify.write_text("#!/usr/bin/env python\n")
    ...
    def test_tier3_uv_tool_when_registry_lists_exact_name(
        ...
        monkeypatch.setattr("sys.argv", [str(tmp_path / "missing" / "agile")])
        ...
        with patch("agile_cli._version.shutil.which", side_effect=fake_which), patch(
            "agile_cli._version.subprocess.run", side_effect=fake_run
        ), patch("agile_cli._version._editable_marker_seen", return_value=False):
            method, signals = _detect_install_method(include_signals=True)
        assert method == _InstallMethod.UV_TOOL
    ...
    def test_bare_argv0_missing_path_resolution_allows_tier3_registry_detection(
        ...
        missing_specify = tmp_path / "missing" / "agile"
        monkeypatch.setattr("sys.argv", ["agile"])
    ...
    def test_missing_relative_argv0_falls_back_to_entrypoint_name_lookup(
        ...
        def fake_which(name):
            return str(fake_specify) if name == "agile" else None

        with patch("agile_cli._version.shutil.which", side_effect=fake_which):
            method = _detect_install_method()
        ...
        def fake_run(argv, *args, **kwargs):
            if argv[:3] == ["uv", "tool", "list"]:
                return subprocess.CompletedProcess(
                    args=argv,
                    returncode=0,
                    stdout="my-wepld-agile-helper v0.1.0\n",
                    stderr="",
                )
            return subprocess.CompletedProcess(
                args=argv, returncode=1, stdout="", stderr=""
...
class TestPrefixExpansion:
    ...
    def test_literal_dollar_without_variable_name_is_preserved(self, tmp_path):
        prefix_path = tmp_path / "specify-$-cache" / "tools" / "wepld-agile"
        prefix = str(prefix_path)

        expanded = agile_cli._version._expand_prefix(prefix)
    ...
    def test_absolute_prefix_resolve_oserror_is_rejected(self, tmp_path):
        ...
        with patch("pathlib.Path.resolve", side_effect=OSError("bad path")):
            assert agile_cli._version._expand_prefix(prefix) is None
...
class TestArgv0Resolution:
    ...
    def test_absolute_argv0_resolve_oserror_returns_original_path(self, tmp_path):
        argv0 = tmp_path / "agile"

        with patch("pathlib.Path.resolve", side_effect=OSError("bad path")):
            assert agile_cli._version._resolved_argv0_path(str(argv0)) == argv0

    def test_path_lookup_resolve_oserror_returns_unresolved_lookup_path(self):
        with patch(
            "agile_cli._version.shutil.which", return_value="/broken/specify"
        ), patch("pathlib.Path.resolve", side_effect=OSError("bad path")):
            result = agile_cli._version._resolved_argv0_path("agile")
...
class TestArgvAssemblyUvTool:
    ...
    def test_stable_tag_produces_expected_argv(self):
        with patch("agile_cli._version.shutil.which", return_value="uv"):
...
class TestBareUpgradeUvTool:
    ...
    def test_happy_path_end_to_end(self, uv_tool_argv0, clean_environ):
        with patch("agile_cli.authentication.http.urllib.request.urlopen") as mock_urlopen, patch(
            ...
            "agile_cli._version._get_installed_version", return_value="0.7.5"
        ):
        ...
        assert "Upgrading wepld-agile 0.7.5 → v0.7.6 via uv tool:" in out
        assert "Upgraded wepld-agile: 0.7.5 → 0.7.6" in out
...
class TestAlreadyLatestUvTool:
    ...
    def test_already_latest_exits_zero_no_subprocess(
        ...
        with patch("agile_cli.authentication.http.urllib.request.urlopen") as mock_urlopen, patch(
            "agile_cli._version.subprocess.run"
        ) as mock_run, patch(
            "agile_cli._version.shutil.which", return_value="uv"
        ), patch("agile_cli._version._get_installed_version", return_value="0.7.6"):
            mock_urlopen.return_value = mock_urlopen_response({"tag_name": "v0.7.6"})
            result = runner.invoke(app, ["self", "upgrade"])
    ...
    def test_trailing_zero_equivalent_version_reports_latest_not_newer(
        ...
        with patch("agile_cli.authentication.http.urllib.request.urlopen") as mock_urlopen, patch(
            ...
            "agile_cli._version.shutil.which", return_value="uv"
        ), patch("agile_cli._version._get_installed_version", return_value="1.0"):
    ...
    def test_dev_build_ahead_of_release_reports_newer_noop(
        ...
        with patch("agile_cli.authentication.http.urllib.request.urlopen") as mock_urlopen, patch(
            ...
            "agile_cli._version.shutil.which", return_value="uv"
        ), patch("agile_cli._version._get_installed_version", return_value="0.7.7.dev0"):
            mock_urlopen.return_value = mock_urlopen_response({"tag_name": "v0.7.6"})
            result = runner.invoke(app, ["self", "upgrade"])
    ...
    def test_unparseable_current_version_does_not_false_noop(
        ...
        with patch("agile_cli.authentication.http.urllib.request.urlopen") as mock_urlopen, patch(
            ...
            "agile_cli._version.shutil.which", return_value="uv"
        ), patch("agile_cli._version._get_installed_version", return_value="release-main"):
            mock_urlopen.return_value = mock_urlopen_response({"tag_name": "v0.7.6"})
            mock_run.side_effect = [
                _completed_process(0),
                _completed_process(0, stdout="specify 0.7.6\n"),
        ...
        assert "Upgrading wepld-agile release-main → v0.7.6 via uv tool:" in out
        assert mock_run.call_count == 2
    ...
    def test_pinned_older_tag_still_runs_installer(
        ...
        with patch("agile_cli._version.shutil.which", return_value="uv"), patch(
            "agile_cli._version.subprocess.run"
        ) as mock_run, patch(
            "agile_cli._version._get_installed_version", return_value="0.7.6"
        ):
            mock_run.side_effect = [
                _completed_process(0),
        ...
        assert "Downgrading wepld-agile 0.7.6 → v0.7.5 via uv tool:" in out
        assert "Upgrading wepld-agile" not in out
        assert mock_run.call_count == 2
    ...
    def test_pinned_rc_tag_uses_canonical_version_equality_for_noop(
        ...
        with patch("agile_cli._version.shutil.which", return_value="uv"), patch(
            "agile_cli._version._get_installed_version", return_value="1.0.0rc1"
        ):
...
class TestDryRunUvTool:
    ...
    def test_dry_run_without_tag_resolves_network_but_no_subprocess(
        ...
        with patch("agile_cli.authentication.http.urllib.request.urlopen") as mock_urlopen, patch(
            ...
            "agile_cli._version.shutil.which", return_value="uv"
        ), patch("agile_cli._version._get_installed_version", return_value="0.7.5"):
            mock_urlopen.return_value = mock_urlopen_response({"tag_name": "v0.7.6"})
    ...
    def test_dry_run_with_tag_skips_network(self, uv_tool_argv0, clean_environ):
        ...
        with patch("agile_cli.authentication.http.urllib.request.urlopen") as mock_urlopen, patch(
            "agile_cli._version.subprocess.run"
        ), patch("agile_cli._version.shutil.which", return_value="uv"), patch(
            "agile_cli._version._get_installed_version", return_value="0.7.5"
        ):
    ...
    def test_dry_run_with_missing_uv_flags_unresolved_installer(
        ...
        with patch("agile_cli.authentication.http.urllib.request.urlopen") as mock_urlopen, patch(
            ...
            "agile_cli._version.shutil.which", return_value=None
        ), patch("agile_cli._version._get_installed_version", return_value="0.7.5"):
            mock_urlopen.return_value = mock_urlopen_response({"tag_name": "v0.7.6"})
            result = runner.invoke(app, ["self", "upgrade", "--dry-run"])
        ...
        def fake_run(argv, *args, **kwargs):
            if argv[:3] == ["pipx", "list", "--json"]:
                return subprocess.CompletedProcess(
                    args=argv,
                    returncode=0,
                    stdout="not json but mentions wepld-agile",
                    stderr="",
                )
            return subprocess.CompletedProcess(
                args=argv, returncode=1, stdout="", stderr=""
...
class TestEditableInstallMetadata:
    ...
    def test_editable_marker_false_when_metadata_is_invalid(self):
        ...
        with patch(
            ...
            assert agile_cli._version._editable_marker_seen() is False
            assert agile_cli._version._source_checkout_path() is None

    def test_direct_url_editable_install_marks_source_checkout(self, tmp_path):
        project_root = tmp_path / "agile"
        project_root.mkdir()
        ...
        with patch("importlib.metadata.distribution", return_value=FakeDist()):
            assert agile_cli._version._editable_marker_seen() is True
            assert agile_cli._version._source_checkout_path() == project_root.resolve()

    def test_editable_marker_false_without_explicit_editable_metadata(self, tmp_path):
        repo_root = tmp_path / "repo"
        repo_root.mkdir()
        (repo_root / ".git").mkdir()
        venv_file = repo_root / ".venv" / "lib" / "python3.13" / "site-packages" / "agile_cli.py"
...
class TestArgvAssemblyPipx:
    ...
    def test_pipx_argv_uses_install_force_positional_not_upgrade(self):
        with patch("agile_cli._version.shutil.which", return_value="pipx"):
...
class TestBareUpgradePipx:
    ...
    def test_happy_path(self, pipx_argv0, clean_environ):
        with patch("agile_cli.authentication.http.urllib.request.urlopen") as mock_urlopen, patch(
            "agile_cli._version.shutil.which", return_value="pipx"
        ), patch("agile_cli._version.subprocess.run") as mock_run, patch(
            "agile_cli._version._get_installed_version", return_value="0.7.5"
        ):
            mock_urlopen.return_value = mock_urlopen_response({"tag_name": "v0.7.6"})
...
class TestDetectionShortCircuit:
    ...
    def test_pipx_argv0_prefix_short_circuits_before_registry_checks(
        ...
        with patch("agile_cli._version.shutil.which", return_value="/usr/bin/X"), patch(
            "agile_cli._version.subprocess.run"
        ) as mock_run:
            method = _detect_install_method()
        assert method == _InstallMethod.PIPX
