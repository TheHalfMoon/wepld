...
describe('framework registry — fixture detection', () => {
  ...
  it('adapters ask for the gitignore patterns their generated paths need', () => {
    ...
    assert.deepEqual(frameworkIgnorePatterns(resolvedNuxt), ['plugins/pictorial-live.client.ts']);
    ...
    assert.deepEqual(
      ...
      ['src/pictorial/PictorialLiveRoot.tsx'],
    );
  ...
  it('describes adapter artifacts as created files plus patched anchors', () => {
    ...
    assert.deepEqual(artifacts.map((a) => [a.kind, a.path]), [
      ['created', 'src/pictorial/PictorialLiveRoot.tsx'],
...
const NUXT_PLUGIN_BODY = '/* pictorial-live-nuxt-plugin */\nexport default defineNuxtPlugin(() => {});\n';

describe('inject journal — crash recovery', () => {
  let tmp;
  beforeEach(() => { tmp = mkdtempSync(join(tmpdir(), 'pictorial-journal-')); });
  ...
  function stageKilledSession({ withPlugin = true, withTag = true } = {}) {
    ...
    if (withPlugin) {
      ...
      writeFileSync(join(tmp, 'app', 'plugins', 'pictorial-live.client.ts'), NUXT_PLUGIN_BODY);
      artifacts.push({
        kind: 'created',
    ...
    if (withTag) {
      ...
      artifacts.push({
        ...
        markers: ['pictorial-live-start', 'data-pictorial-csp-original'],
      });
    }
  ...
  it('writes the journal under .pictorial/live/ so the ignore block covers it', () => {
    stageKilledSession();
    assert.equal(injectJournalPath(tmp), join(tmp, '.pictorial', 'live', 'inject-journal.json'));
  ...
  it('heals a SIGKILLed session back to a clean tree', () => {
    ...
    assert.deepEqual(
      ...
      [['app/plugins/pictorial-live.client.ts', 'removed'], ['index.html', 'unpatched']].sort(),
    );
    assert.equal(
  ...
  it('keeps artifacts the current run is about to rewrite', () => {
    ...
    const { healed, kept } = healInjectJournal(tmp, {
      keep: ['app/plugins/pictorial-live.client.ts'],
    });

    assert.deepEqual(healed.map((h) => h.path), ['index.html']);
    assert.deepEqual(kept.map((k) => k.path), ['app/plugins/pictorial-live.client.ts']);
    assert.ok(
      existsSync(join(tmp, 'app', 'plugins', 'pictorial-live.client.ts')),
      'a kept artifact is left in place so re-injection stays byte-idempotent',
    );
    const journal = readInjectJournal(tmp);
    assert.deepEqual(journal.artifacts.map((a) => a.path), ['app/plugins/pictorial-live.client.ts']);
  });
  ...
  it('tolerates artifacts whose files are already gone', () => {
    stageKilledSession({ withTag: false });
    rmSync(join(tmp, 'app', 'plugins', 'pictorial-live.client.ts'));
...
`;
  ...
  it('stamps the import with a token-derived revision and swaps it on rotation', () => {
    ...
    assert.ok(patchedA.includes(`PictorialLiveRoot.svelte?pictorial-live=${revA}'`), 'import carries the revision');
    ...
    const importCount = (patchedB.match(/import PictorialLiveRoot/g) || []).length;
    assert.equal(importCount, 1, 'rotation must not stack imports');
    assert.ok(patchedB.includes(`?pictorial-live=${revB}'`));
    assert.ok(!patchedB.includes(`?pictorial-live=${revA}'`));
    assert.match(patchedB, /\n  import PictorialLiveRoot/, 'replacement keeps the original indentation');
  });
  ...
  it('removal of a created-from-scratch layout leaves no script husk', () => {
    ...
    const restored = adapter.unpatchSvelteLayout(patched);
    assert.doesNotMatch(restored, /PictorialLiveRoot|pictorial-live-svelte/);
