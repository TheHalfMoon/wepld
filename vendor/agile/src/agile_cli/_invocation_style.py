...
Agents that scaffold skills (``agile-<name>/SKILL.md``) use different
...
and ``agile init``'s next-steps output stay consistent.
"""

from __future__ import annotations

# Agents that render $agile-<name> (chat invocation) when in skills mode.
DOLLAR_SKILLS_AGENTS: frozenset[str] = frozenset({"codex", "zcode", "command-code"})

# Agents that always render /agile-<name>, regardless of ai_skills.
ALWAYS_SLASH_AGENTS: frozenset[str] = frozenset(
    {"devin", "droid", "grok", "qodercli", "trae", "zed"}
)

# Agents that render /agile-<name> only when ai_skills is enabled.
...
def is_dollar_skills_agent(selected_ai: str | None, ai_skills_enabled: bool) -> bool:
    """Return ``True`` if *selected_ai* uses ``$agile-<name>`` invocations.

    Agents in `DOLLAR_SKILLS_AGENTS` (e.g. ``codex``, ``zcode``) render
    ``$agile-<name>`` chat invocations when installed in skills mode.
    """
