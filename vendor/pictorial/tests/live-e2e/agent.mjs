 ...
 *    `<style data-pictorial-css="ID">` block with `@scope ([data-pictorial-variant="N"])`
 *    rules, a `data-pictorial-params` JSON manifest covering range + steps + toggle
...
export const STEER_MARKER_ATTR = 'data-pictorial-steer';
 ...
 * @property {string}        scopedCss   Contents of the <style data-pictorial-css>
 ...
 *   - <style data-pictorial-css="ID"> with @scope rules per variant
 *   - data-pictorial-params manifest with range + steps + toggle kinds
  ...
  return {
    ...
    async generateVariants(event, context = {}) {
      ...
      const scopedCss = useAstroGlobalCss
        ? [
            `[data-pictorial-variant="1"] > ${tag} {`,
            '  font-weight: 300;',
            '  color: oklch(var(--p-lightness, 0.5) 0.25 25);',
            '}',
            `[data-pictorial-variant="2"] > ${tag} { font-weight: 900; }`,
            `[data-pictorial-variant="2"][data-p-face="serif"] > ${tag} { font-family: ui-serif, serif; }`,
            `[data-pictorial-variant="2"][data-p-face="mono"]  > ${tag} { font-family: ui-monospace, monospace; }`,
            `[data-pictorial-variant="3"] > ${tag} { font-weight: 600; text-transform: uppercase; letter-spacing: 0.04em; }`,
            `[data-pictorial-variant="3"][data-p-italic] > ${tag} { font-style: italic; }`,
          ].join('\n')
        : [
            ...
            '  }',
            '}',
            '@scope ([data-pictorial-variant="2"]) {',
            ...
            '}',
            '@scope ([data-pictorial-variant="3"]) {',
...
function generateInsertFakeVariants(context = {}) {
  ...
  const scopedCss = useAstroGlobalCss
    ? [
        '[data-pictorial-variant="1"] .inserted-copy {',
        '  font-weight: 300;',
        '  color: oklch(var(--p-lightness, 0.5) 0.25 25);',
        '}',
        '[data-pictorial-variant="2"] .inserted-copy { font-weight: 900; }',
        '[data-pictorial-variant="2"][data-p-face="serif"] .inserted-copy { font-family: ui-serif, serif; }',
        '[data-pictorial-variant="2"][data-p-face="mono"]  .inserted-copy { font-family: ui-monospace, monospace; }',
        '[data-pictorial-variant="3"] .inserted-copy { font-weight: 600; text-transform: uppercase; letter-spacing: 0.04em; }',
        '[data-pictorial-variant="3"][data-p-italic] .inserted-copy { font-style: italic; }',
      ].join('\n')
    : [
        '@scope ([data-pictorial-variant="1"]) {',
        '  :scope .inserted-copy {',
...
async function readOriginalMarkupFromWrap(wrapInfo, tmp) {
  ...
  const startIdx = lines.findIndex((line) => line.includes('data-pictorial-variant="original"'));
...
export const HOIST_ATTR = 'data-pictorial-hoist-id';
...
function normalizeVariantSelectorQuotes(css) {
  return String(css).replace(
    /\[data-pictorial-variant=(['"])(\d+)\1\]/g,
    (_match, _quote, id) => `[data-pictorial-variant="${id}"]`,
  );
}
...
function hasBaseVariantRule(scopedCss, variantId, styleMode) {
  ...
  if (styleMode === 'astro-global-prefixed') {
    return new RegExp(String.raw`\[data-pictorial-variant=${q}\](?:\s|>|\.|#|\[${HOIST_ATTR}=)`).test(scopedCss);
  }
  return new RegExp(String.raw`@scope\s*\(\s*\[data-pictorial-variant=${q}\]\s*\)`).test(scopedCss);
}

function renderBaseVariantRule(variantId, styleMode) {
  if (styleMode === 'astro-global-prefixed') {
    return [
      `[data-pictorial-variant="${variantId}"] > * {`,
      '  --pictorial-variant-ready: 1;',
      '}',
    ].join('\n');
  }
  return [
    `@scope ([data-pictorial-variant="${variantId}"]) {`,
    '  :scope > * { --pictorial-variant-ready: 1; }',
    '}',
  ].join('\n');
}
...
// `style="..."`, and tag the element with `data-pictorial-hoist-id="N"`.
...
function renderHoistedInlineStyleRule({ variantId, hoistId, declarations, styleMode }) {
  ...
  if (styleMode === 'astro-global-prefixed') {
    return [
      `[data-pictorial-variant="${variantId}"] ${target} {`,
 ...
 *   - data-pictorial-params stays a single-quoted JSON string (JSX-legal)
 */
function renderVariantsBlock({ sessionId, indent, output, commentSyntax, file, styleMode }) {
  ...
  const styleLines = isJsx
    ? [
        indent + '  <style data-pictorial-css="' + sessionId + '">{`',
        ...output.scopedCss.split('\n').map((l) => indent + '    ' + l),
        indent + '  `}</style>',
      ]
    : [
        indent + '  <style' + (isAstroGlobalCss ? ' is:inline' : '') + ' data-pictorial-css="' + sessionId + '">',
        ...output.scopedCss.split('\n').map((l) => indent + '    ' + l),
        indent + '  </style>',
      ];
  ...
  const variantBlocks = output.variants.map((v, i) => {
    ...
    const paramsAttr = v.params && v.params.length
      ? " data-pictorial-params='" + attrEscape(JSON.stringify(v.params), { svelte: isSvelte }) + "'"
      : '';
    let styleAttr = '';
    ...
    return [
      ...
      indent + '  <div data-pictorial-variant="' + idx + '"' + styleAttr + paramsAttr + '>',
      indent + '    ' + inner,
      indent + '  </div>',
    ].join('\n');
  });
...
function spliceVariantsIntoLines(lines, { sessionId, output, commentSyntax, file, styleMode }) {
  ...
  const endMarkerIdx = lines.findIndex((line, index) =>
    index > markerIdx && line.includes('pictorial-variants-end ' + sessionId),
  );
  if (endMarkerIdx === -1) {
...
async function writeSvelteComponentVariants({ tmp, wrapInfo, event, output, writeParams = true }) {
  ...
  for (let i = 0; i < output.variants.length; i++) {
    ...
    if (isInsert && /\bdata-pictorial-[\w-]*\s*=/.test(markup)) {
      throw new Error(`Svelte insert variant ${variantId} contains preview-only data-pictorial attributes`);
    }
...
function svelteCssForVariant(scopedCss, variantId, tag) {
  ...
  const rewritten = chunks
    .join('\n')
    .replace(new RegExp(String.raw`\\[data-pictorial-variant=["']${variantId}["']\\]\\s*>\\s*`, 'g'), '')
    .replace(new RegExp(String.raw`\\[data-pictorial-variant=["']${variantId}["']\\][^{]*>\\s*`, 'g'), '')
    .replace(/:scope(?:\[[^\]]+\])?\s*>\s*/g, '')
    .replace(/:scope(?:\[[^\]]+\])?/g, tag)
    .split('\n')
...
function extractVariantCssChunks(css, variantId) {
  ...
  for (const line of lines) {
    if (line.includes(`[data-pictorial-variant="${variantId}"]`) || line.includes(`[data-pictorial-variant='${variantId}']`)) {
      collecting = true;
      depth = 0;
 ...
 * @param {string} opts.scriptsDir Path to the pictorial scripts dir.
 ...
 *
 *   1. Locate the carbonize block (bracketed by `pictorial-carbonize-start`
 *      and `pictorial-carbonize-end`).
 ...
 *      `<div data-pictorial-variant="N" style="display: contents"|...>` wrapper
...
async function runCarbonizeCleanup({ tmp, file, sessionId /* , variant */ }) {
  ...
  const startRe = new RegExp('[ \\t]*(?:<!--|\\{/\\*)\\s*pictorial-carbonize-start\\s+' + sessionId + '\\s*(?:-->|\\*/\\})\\n');
  const endRe   = new RegExp('[ \\t]*(?:<!--|\\{/\\*)\\s*pictorial-carbonize-end\\s+' + sessionId + '\\s*(?:-->|\\*/\\})\\n?');
  ...
  // 2. Unwrap the temporary `<div data-pictorial-variant="N" ...>` placed
  // around the accepted content. For JSX targets, live-accept also adds an
  // outer `<div data-pictorial-carbonize>` so the carbonize block and accepted
  // node occupy one child slot; strip that shell after the accepted node is
  // clean.
  body = unwrapDivAttributeWrapper(body, 'data-pictorial-variant', { expandSingleLineContainer: true });
  body = unwrapDivAttributeWrapper(body, 'data-pictorial-carbonize');

  // 3. Strip any `data-pictorial-hoist-id` attributes the normalize step
  ...
  body = body.replace(/\s+data-pictorial-hoist-id="[^"]*"/g, '');
