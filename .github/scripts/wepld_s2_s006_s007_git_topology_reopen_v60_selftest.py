#!/usr/bin/env python3
"""Self-tests for the v60 S2-S006/S2-S007 git_topology_v1.rs evidence reopen authority."""
...
import wepld_s2_s006_s007_git_topology_reopen_v60_integrity as p
    ...
    def read_bytes(self, path: str, max_bytes: int) -> bytes:
        ...
        if path in self._replacements:
            ...
            if len(data) > max_bytes:
                base.fail(f"v60 self-test overlay exceeds read bound: {path}")
            return data
...
def _expect_failure(label: str, action: Any, expected: str) -> None:
    try:
        ...
        if expected not in str(exc):
            base.fail(f"v60 self-test rejection came from wrong cause: {label}: {exc}")
        return
    base.fail(f"v60 self-test expected fail-closed rejection: {label}")


def _check_authority_markers() -> None:
    if p.AUTH != "S2_S006_S007_GIT_TOPOLOGY_REOPEN_ONLY":
        base.fail("v60 authority marker drift")
    if p.S2_IMPLEMENTATION_AUTHORITY != "TEST_OR_EVIDENCE_REOPEN_ONLY":
        base.fail("v60 S2 implementation boundary drift")
    if p.S2_S006_S007_GIT_TOPOLOGY_REOPEN_AUTHORITY != (
        "SINGLE_USE_TEST_ONLY_REOPEN_OF_GIT_TOPOLOGY_V1_RS_FOR_S2_S006_S007"
    ):
        base.fail("v60 must grant exactly the single-use S2-S006/S2-S007 git_topology_v1.rs reopen authority")
    if p.NEXT_AUTHORITY_GATE != "S2-ACCEPTANCE":
        base.fail("v60 next authority gate drift")
    inherited_unchanged = (
    ...
    for name in inherited_unchanged:
        if getattr(p, name) != getattr(p.q, name):
            base.fail(f"v60 widened or changed inherited authority: {name}")
    for name, value in (
        ...
        if value != "NONE":
            base.fail(f"v60 must not grant {name}")
...
def _check_reopen_contract() -> None:
    for invariant in (
        ...
        if invariant not in p.S2_S006_S007_GIT_TOPOLOGY_REOPEN_CONTRACT:
            base.fail(f"v60 reopen contract lost invariant: {invariant}")


def _check_reopen_scope() -> None:
    if p.REOPEN_FILES != frozenset({"crates/core/tests/git_topology_v1.rs"}):
        base.fail("v60 reopen path set drift")
    if p.CORE_MANIFEST in p.REOPEN_FILES or p.ROOT_CARGO_LOCK in p.REOPEN_FILES:
        base.fail("v60 reopen must not authorize dependency mutation")
    if p.ROOT_CARGO in p.REOPEN_FILES:
        base.fail("v60 reopen must not authorize workspace manifest mutation")
    if p.GIT_TOPOLOGY_MODULE in p.REOPEN_FILES or p.PROJECT_MODULE in p.REOPEN_FILES:
        base.fail("v60 reopen must not authorize source module mutation")
    if p.CORE_EXPORT in p.REOPEN_FILES:
        base.fail("v60 reopen must not touch the shared Core export")
    for path in p.REOPEN_FILES:
        if not path.startswith("crates/core/tests/"):
            base.fail(f"v60 reopen escaped crates/core/tests: {path}")
    if len(p.REOPEN_FILES) != 1:
        base.fail("v60 reopen must be a single exact path")
    if p.REOPEN_TASKS != frozenset({"S2-S006", "S2-S007"}):
        base.fail("v60 reopen task allowlist must claim exactly S2-S006 and S2-S007")
    if any(t.startswith("S2-AUTH") for t in p.REOPEN_TASKS):
        base.fail("v60 reopen task allowlist must not claim authority tasks")


def _check_predecessor_exact() -> None:
    p.req_v59(p.raw_root)
    if p.V59_P_BLOB != "5184b8ca796e51cace5bd26353633cfe8ce7998f":
        base.fail("v60 frozen v59 integrity identity drift")
    if p.V59_T_BLOB != "381667a670a134f05f6b20ac425a7e6dfeafd231":
        base.fail("v60 frozen v59 self-test identity drift")


def _check_workflow_projection() -> None:
    projected = p._project_for_v59(p.raw_root)
    for path in (p.FW, p.AW):
        data = projected.read_bytes(path, base.MAX_POLICY_FILE_BYTES)
        if p.V25.sha(data) != p.Q_WF[path]:
            base.fail(f"v60 workflow projection does not reverse to exact v59: {path}")
        if p._V60_ENTRYPOINT in data:
            base.fail(f"v60 workflow projection left the v60 entrypoint: {path}")
...
def _check_bootstrap_scope() -> None:
    if p.BOOT != frozenset({p.P, p.T, p.FW, p.AW}):
        base.fail("v60 bootstrap path set drift")
    if p.CONTROLLED_FILES != p.POLICY_FILES:
        base.fail("v60 controlled-file set must equal policy-file set")
    p.delta(p.raw_root, _boot_base())

    smuggled = OverlayView(
        p.raw_root,
        {"docs/canonical/UNAUTHORIZED_V60_BOOTSTRAP.md": b"# smuggled\n"},
    )
    _expect_failure(
        "v60 bootstrap mixed with fifth path",
        lambda: p.delta(smuggled, _boot_base()),
        "bootstrap delta must be exactly two v60 policy files plus two integrity workflows",
    )
...
def _check_reopen_delta_shape() -> None:
    ...
    if not p._reopen_available(p.raw_root):
        _expect_failure(
            "v60 reopen already consumed on real checkout",
    ...
    _expect_failure(
        "v60 reopen mixed with non-reopen path",
    ...
    _expect_failure(
        "v60 reopen must not mutate Cargo.lock",
    ...
    _expect_failure(
        "v60 reopen must not mutate the core manifest",
    ...
    _expect_failure(
        "v60 reopen must not mutate the project source module",
    ...
    _expect_failure(
        "v60 reopen must not add an unauthorized extra test file",
    ...
    _expect_failure(
        "v60 reopen must not mutate a workflow beyond entrypoint migration",
...
def _check_reopen_single_use() -> None:
    ...
    _expect_failure(
        "v60 reopen grant is single-use",
...
def run() -> None:
    ...
    p.install()
    p.overlay()
    print("wepld v60 S2-S006/S2-S007 git_topology_v1.rs evidence reopen authority self-tests: PASS")
