...
# ///
"""
Agile CLI - Setup tool for Specify projects

Usage:
    uvx wepld-agile.py init <project-name>
    uvx wepld-agile.py init .
    uvx wepld-agile.py init --here

Or install globally:
    uv tool install --from wepld-agile.py wepld-agile
...
from ._assets import (
    ...
    _locate_core_pack,
    _repo_root,
    get_agile_version as get_agile_version,
)
from ._utils import (
...
def _version_callback(value: bool):
    if value:
        console.print(f"specify {get_agile_version()}")
        raise typer.Exit()
...
def _install_shared_infra(
    ...
    Copies ``.agile/scripts/<variant>/`` and ``.agile/templates/`` from
    ...
    fallback. Tracks all installed files in ``agile.manifest.json``.

    Shared scripts and page templates are processed to resolve
    ``__AGILE_COMMAND_<NAME>__`` placeholders using *invoke_separator*
    ...
    * ``refresh_managed=True`` — overwrite only files whose on-disk hash
      ...
      files installed by agile). Files with diverging hashes are
...
def ensure_executable_scripts(project_path: Path, tracker: StepTracker | None = None) -> None:
    """Ensure POSIX .sh scripts under .agile/scripts and .agile/extensions (recursively) have execute bits (no-op on Windows)."""
    if os.name == "nt":
        return  # Windows: skip silently
    scan_roots = [
        project_path / ".agile" / "scripts",
        project_path / ".agile" / "extensions",
    ]
    failures: list[str] = []
    updated = 0
...
def resolve_active_skills_dir(project_root: Path) -> Path | None:
    ...
    Reads ``.agile/init-options.json`` to determine whether skills are
...
SKILL_DESCRIPTIONS = {
    "agile": "Create or update feature specifications from natural language descriptions.",
...
def check():
    ...
    console.print("\n[bold green]Agile CLI is ready to use![/bold green]")
...
def version(
    ...
    if features:
        ...
        console.print(f"Agile CLI: {cli_version}")
        console.print()
    ...
    panel = Panel(
        info_table,
        title="[bold cyan]Agile CLI Information[/bold cyan]",
        border_style="cyan",
        padding=(1, 2)
    )
...
def _require_specify_project() -> Path:
    ...
    rest that operate on an existing ``.agile/`` project — so the override
...
# Bundler subcommand group (agile bundle ...) — see commands/bundle/.
...
# handlers via ``from agile_cli import workflow_*`` (and tests monkeypatch
# ``agile_cli.workflow_add``). Keep these names resolvable from the root.
