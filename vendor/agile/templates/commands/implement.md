...
2. **Check checklists status** (if FEATURE_DIR/checklists/ exists):
   ...
   - `checklists/requirements.md` is the built-in spec-quality checklist maintained by `__AGILE_COMMAND_SPECIFY__` and `__AGILE_COMMAND_CLARIFY__`; custom checklists generated by `__AGILE_COMMAND_CHECKLIST__` are reviewer-owned requirements-quality review artifacts
...
Note: This command assumes a complete task breakdown exists in tasks.md. If tasks are incomplete or missing, suggest running `__AGILE_COMMAND_TASKS__` first to regenerate the task list.
