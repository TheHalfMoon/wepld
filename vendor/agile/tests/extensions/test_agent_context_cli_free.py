"""Static guard: the Agile CLI source must contain no agent-context lifecycle code.

The ``agent-context`` extension is a full opt-in and owns its own lifecycle. The
Python codebase (``src/agile_cli/**``) must therefore not reference any of the
...
SRC_ROOT = PROJECT_ROOT / "src" / "agile_cli"
