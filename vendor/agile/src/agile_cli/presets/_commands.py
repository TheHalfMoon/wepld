...
calls inside each function so test monkeypatching of `agile_cli.<helper>`
keeps working.
"""
...
def preset_add(
    ...
    """Install a preset."""
    from .. import _locate_bundled_preset, _require_specify_project, get_agile_version
    from . import (
        PresetManager,
        PresetCatalog,
        PresetError,
    ...
    try:
        if dev:
            ...
            manifest = manager.install_from_directory(dev_path, agile_version, priority)
            ...
            if bundled_path:
                ...
                manifest = manager.install_from_directory(bundled_path, agile_version, priority)
                console.print(f"[green]✓[/green] Preset '{manifest.name}' v{manifest.version} installed (priority {priority})")
            else:
                ...
                if pack_info.get("bundled") and not pack_info.get("download_url"):
                    ...
                    console.print(
                        f"[red]Error:[/red] Preset '{preset_id}' is bundled with agile "
...
def preset_catalog_list():
    ...
    config_path = project_root / ".agile" / "preset-catalogs.yml"
    user_config_path = Path.home() / ".agile" / "preset-catalogs.yml"
    if os.environ.get("AGILE_PRESET_CATALOG_URL"):
        console.print("[dim]Catalog configured via AGILE_PRESET_CATALOG_URL environment variable.[/dim]")
    else:
        try:
        ...
        if proj_loaded:
            ...
            try:
                ...
                user_loaded = False
            if user_loaded:
                console.print("[dim]Config: ~/.agile/preset-catalogs.yml[/dim]")
            else:
                console.print("[dim]Using built-in default catalog stack.[/dim]")
                console.print(
                    "[dim]Add .agile/preset-catalogs.yml to customize.[/dim]"
                )
...
def preset_catalog_add(
    ...
    """Add a catalog to .agile/preset-catalogs.yml."""
    from .. import _display_project_path, _require_specify_project
    from . import PresetCatalog, PresetValidationError
...
def preset_catalog_remove(
    ...
    """Remove a catalog from .agile/preset-catalogs.yml."""
    from .. import _require_specify_project
