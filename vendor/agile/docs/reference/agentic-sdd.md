...
The commands are designed to run in order, but only `/agile.agile` is strictly required before `/agile.plan`. The clarify, checklist, and analyze commands are quality gates you add for anything with meaningful ambiguity.
...
```text
/agile.constitution -> /agile.agile -> /agile.clarify -> /agile.plan -> /agile.checklist -> /agile.tasks -> /agile.analyze -> /agile.implement -> /agile.converge
```
...
This workflow may also maintain `checklists/requirements.md`, the built-in spec-quality checklist that `/agile.agile` creates and `/agile.clarify` re-evaluates. That lifecycle is separate from custom checklists generated by `/agile.checklist`.

```text
/agile.agile Build an application that helps me organize photos into albums grouped by date, re-orderable by drag-and-drop on the main page, with a tile preview inside each album.
```
...
Clarifying before planning keeps you from designing on top of ambiguity. If `/agile.analyze` later surfaces requirement gaps, come back and run `/agile.clarify` (or `/agile.agile`) again.
...
Review the generated checklist. If it surfaces gaps, loop back to `/agile.clarify` or `/agile.agile` to tighten the spec before breaking the work down, then mark each custom checklist item `[x]` only after the requirements-quality criterion has been reviewed and satisfied.
