...
`crates/core/tests/git_topology_v1.rs` malformed `.git` *directory* test
evidence gap.

v65 is an append-only policy successor over canonical v64 at main
8a1f488e67443ef30de15eb7cd9b3cd165095ab3. Founder standing authorization for
...
test path so a real fixture for the S2-S006 malformed `.git` *directory* angle
can be recorded -- a `.git` that IS a real directory (not a gitfile pointer)
but is structurally broken: empty, a garbage `HEAD`, a missing `HEAD`, an
unparseable `config`. Each case is passed to the qualified system Git adapter
via `discover_system_git` + `observe_git_topology`, proving the adapter yields
a bounded, typed `GitTopologyError` from the closed failure-classification set
(`NotGitRepository` / `GitProcessFailed` / `UntrustedRepositoryRefusedByGit`)
-- never an `Ok` topology, never an uncaught panic, and never any action on
the broken repository in the adapter's own logic. This complements the
already-covered malicious-gitfile (v61, PR #313), malformed machine-output,
and hook-execution (v60, PR #310) angles of S2-S006.

`crates/core/tests/git_topology_v1.rs` was reopened before by v52 (S2-I006 /
S2-I007 adversarial fixtures, PR #287), v53 (S2-S005 no-auto-bypass + bare
repo, PR #292), v60 (S2-S006 / S2-S007 no-hook, PR #310), and v61 (S2-S006
malicious gitfile, PR #313); all four grants are consumed, so the path is
frozen again by the same v45 Git-topology product-tranche freeze mechanism
those successors themselves reopened against. A fresh single-use reopen is
required. It is not a wholly new file.

v65 grants exactly one thing and nothing else:
...
`crates/core/src/project.rs`, `crates/core/src/identity.rs`,
...
read-only Git-topology adapter through its existing public functions
(`discover_system_git`, `observe_git_topology`), using only ordinary
filesystem fixture setup under a Cargo tmpdir. `GIT_EXECUTION_AUTHORITY`
stays `READ_ONLY_TOPOLOGY_OBSERVATION_ONLY` (inherited, not widened); no new
process, network, model, or S3+ authority. No `unsafe` code (the file keeps
...
against the inherited v64 chain.

Package-load / resting-view note: v65 follows the v45..v64 discipline. It
owns a fresh `LocalRepositoryView` of the exact checked-out head, imports
frozen v64 under an exact v65->v64 workflow-entrypoint reversal, and inherits
every v64 hook by reference, plus v60's, v61's and v64's own reopen-authority
markers carried forward in the inherited-authority list. The one path this
successor reopens is subject to v45's Git-topology product-tranche freeze
inside the inherited cascade's `delta()`, which this successor's `delta()`
intercepts directly before any predecessor delegation, exactly the way
v45/v52/v53/v54/v60/v61/v64 intercept their own reopened paths - so no
further predecessor-facing projection is required (v45's freeze is
delta()-only; its files() check on the path is structural, and
`git_topology_v1.rs` is not in v36's canonical-frontier blob set, so no
v56-style frontier projection is needed). Unlike v55/v56/v57/v58's
identity-store reopen, this grant needs no pre-reopen-bytes token-scan
projection: the new test adds no `std::process` use of its own and no other
token the inherited v28 scan forbids, so the reopened content is scanned
exactly as written, exactly like v52's, v53's, v60's and v61's own
git_topology_v1.rs grants.
"""
...
# Do not inherit a predecessor module's resting/projection view. v65 bases
...
def _v64_workflow_bytes(data: bytes, path: str) -> bytes:
    count = data.count(_V65_ENTRYPOINT)
    if count != _WORKFLOW_ENTRYPOINT_COUNTS[path]:
        base.fail(
            "v65 workflow entrypoint count drifted before v64 projection: "
            f"{path} expected={_WORKFLOW_ENTRYPOINT_COUNTS[path]} actual={count}"
        )
    return data.replace(_V65_ENTRYPOINT, _V64_ENTRYPOINT)


def _import_v64_under_workflow_projection() -> Any:
    """Import frozen v64 while it observes exact v64 workflow bytes.

    v64 (hence v57..v45..v36) reads workflow bytes while its module is
    imported, and the v64->v65 entrypoint migration ships in this same
    candidate, so v64 must not observe its own successor's bytes. Only
    ...
    def _v64_import_read_bytes(local_view: Any, relative: str, limit: int) -> bytes:
        data = original_read_bytes(local_view, relative, limit)
        if relative in (FW, AW):
            data = _v64_workflow_bytes(data, relative)
            if len(data) > limit:
                base.fail(
                    f"v65 v64-import workflow projection exceeds read bound: {relative}"
                )
        return data

    base.LocalRepositoryView.read_bytes = _v64_import_read_bytes
    try:
        return importlib.import_module(
            "wepld_s2_s004_project_v1_case_identity_reopen_v64_integrity"
        )
    finally:
        base.LocalRepositoryView.read_bytes = original_read_bytes


q = _import_v64_under_workflow_projection()
...
# --- S2-S006 git_topology_v1.rs test-evidence reopen allowlist ---
...
# main 8a1f488e67443ef30de15eb7cd9b3cd165095ab3 (git_topology_v1.rs as it
# stands after the S2-S006 gitfile-redirect fixture landed via the v61 reopen,
# PR #313; the same v45 Git-topology product-tranche freeze re-closed the path
# when that content became canonical).
...
# consumed.
EXACT_FROZEN_BLOB = "f55e9578a4c46205fc93bb69cd9900a242bd5d1e"

AUTH = "S2_S006_GITDIR_REOPEN_ONLY"
...
# or grant any new runtime behaviour merely because the reopened test invokes
# existing `git_topology.rs` functions (`discover_system_git`,
# `observe_git_topology`) against a fixture whose `.git` is a real directory but
# structurally broken (empty, a garbage `HEAD`, a missing `HEAD`, an unparseable
# `config`), proving the adapter returns a bounded typed `GitTopologyError` from
# the closed failure-classification set -- never an `Ok` topology, never a panic.
S2_S006_GITDIR_REOPEN_AUTHORITY = (
    "SINGLE_USE_TEST_ONLY_REOPEN_OF_GIT_TOPOLOGY_V1_RS_FOR_S2_S006_MALFORMED_GITDIR"
)

S2_S006_GITDIR_REOPEN_CONTRACT = (
    "TEST_FILE_ONLY",
    "SINGLE_PATH_SCOPE",
    "SINGLE_USE_REOPEN",
    "NO_SOURCE_CHANGE",
...
for _path, _expected in ((q.P, V64_P_BLOB), (q.T, V64_T_BLOB), (T, T_BLOB)):
    _actual = V25.blob(raw_root.read_bytes(_path, base.MAX_POLICY_FILE_BYTES))
    if _actual != _expected:
        base.fail(
            f"frozen v65 package input drifted: {_path}: "
...
def _workflow_replacements(view: Any) -> dict[str, bytes]:
    ...
    for path in (FW, AW):
        ...
        predecessor = _v64_workflow_bytes(data, path)
        actual = V25.sha(predecessor)
        if actual != Q_WF[path]:
            base.fail(
                "v65 workflow does not reverse to exact canonical v64 predecessor: "
...
def req_v64(view: Any) -> None:
    for path, expected in ((q.P, V64_P_BLOB), (q.T, V64_T_BLOB)):
        if path not in V25.ps(view):
            base.fail(f"v65 candidate/base is missing frozen v64 predecessor: {path}")
        actual = V25.blob(view.read_bytes(path, base.MAX_POLICY_FILE_BYTES))
        if actual != expected:
            base.fail(
                f"frozen v64 predecessor drifted: {path}: "
                f"expected={expected} actual={actual}"
            )


def _project_for_v64(view: Any) -> Any:
...
def _v64_views(candidate: Any, policy_base: Any) -> tuple[Any, Any]:
    """Project the candidate to v64's view always; project the policy base
    only when it is a real post-v65 base. A pre-v65 bootstrap base predates
    the v64->v65 workflow migration and carries no v65 policy files, so it
    must reach v64's frozen hooks unprojected."""
    projected_candidate = _project_for_v64(candidate)
    if bootbase(policy_base):
        return projected_candidate, policy_base
    return projected_candidate, _project_for_v64(policy_base)


def run_predecessor_selftests() -> None:
    """Run frozen v64's own self-tests once, under a v65->v64 workflow
    reversal.

    v64's corrected hooks are inherited by reference. Only ``read_bytes`` is
    wrapped here for the v65->v64 workflow reversal; the wrap is restored in
    ...
    def _v64_selftest_read_bytes(local_view: Any, relative: str, limit: int) -> bytes:
        if relative in workflow_reversal:
            data = workflow_reversal[relative]
            if len(data) > limit:
                base.fail(
                    f"v65 v64-selftest workflow projection exceeds read bound: {relative}"
                )
            return data
        return original_read_bytes(local_view, relative, limit)

    base.LocalRepositoryView.read_bytes = _v64_selftest_read_bytes
    try:
        _call("v64 self-tests under v65->v64 workflow reversal", q.selftest)
    finally:
        base.LocalRepositoryView.read_bytes = original_read_bytes
...
def _reopen_available(view: Any) -> bool:
    if REOPEN_TEST not in V25.ps(view):
        base.fail(f"v65 reopen base is missing the frozen reopened path: {REOPEN_TEST}")
...
def _require_reopen_base(view: Any) -> None:
    if not _reopen_available(view):
        base.fail(
            "v65 S2-S006 git_topology_v1.rs evidence reopen already consumed; "
...
def _verify_text_reopen_file(view: Any, path: str) -> None:
    if path not in V25.ps(view):
        base.fail(f"v65 reopen path missing: {path}")
    if V25.mode(view, path) != "100644":
        base.fail(f"v65 reopen path mode invalid: {path}")
    data = view.read_bytes(path, base.MAX_POLICY_FILE_BYTES)
    if not data:
        base.fail(f"v65 reopen path must not be empty: {path}")
    if len(data) > MAX_REOPEN_FILE_BYTES:
        base.fail(f"v65 reopen path exceeds bounded size: {path}")
    try:
        data.decode("utf-8", errors="strict")
    except UnicodeError as exc:
        base.fail(f"v65 reopen path must be UTF-8: {path}: {exc}")
...
def _verify_reopen_candidate(candidate: Any, policy_base: Any) -> None:
    ...
    for path in (
        ...
        if candidate.read_bytes(path, base.MAX_POLICY_FILE_BYTES) != policy_base.read_bytes(
            ...
            base.fail(f"v65 S2-S006 git_topology_v1.rs evidence reopen must not change frozen path: {path}")
    for relative in sorted(V25.FROZEN_STATE_PATHS):
        if candidate.read_bytes(relative, V25.MAX_S1_STATE_BYTES) != policy_base.read_bytes(
            ...
            base.fail(f"v65 candidate changed frozen S1 state: {relative}")
...
def delta(candidate: Any, policy_base: Any) -> None:
    ...
    if bootbase(policy_base):
        if paths != BOOT:
            if paths & BOOT:
                base.fail(
                    "v65 bootstrap delta must be exactly two v65 policy files plus two integrity workflows"
                )
            base.fail("v65 bootstrap base authorizes only exact S2-S006 git_topology_v1.rs reopen policy activation")
        req_v64(candidate)
        req_v64(policy_base)
        return

    if paths & CONTROLLED_FILES:
        base.fail("canonical v65 policy files are frozen after activation")

    reopen_changed = frozenset(paths & REOPEN_FILES)
    if reopen_changed:
        if paths != REOPEN_FILES:
            base.fail("v65 S2-S006 git_topology_v1.rs evidence reopen must not mix with non-reopen paths")
        _verify_reopen_candidate(candidate, policy_base)
        return

    q.delta(_project_for_v64(candidate), _project_for_v64(policy_base))


def basectrl(candidate: Any, policy_base: Any) -> None:
    if not bootbase(policy_base):
        q.basectrl(*_v64_views(candidate, policy_base))
        return
    for path in sorted(base.BASE_CONTROLLED_PATHS):
        ...
        if path in (FW, AW):
            if V25.sha(candidate_bytes) != WF[path] or V25.sha(base_bytes) != Q_WF[path]:
                base.fail(f"v65 bootstrap workflow drifted: {path}")
...
def ext(candidate: Any, policy_base: Any, safe: Any) -> None:
    ...
    for path in sorted(CONTROLLED_FILES & safe_paths):
        if path not in V25.ps(candidate):
            base.fail(f"v65 controlled file missing: {path}")
        if bootbase(policy_base):
            if path in V25.ps(policy_base):
                base.fail(f"v65 controlled file unexpectedly exists in bootstrap base: {path}")
        elif path not in V25.ps(policy_base) or candidate.read_bytes(
            path, base.MAX_POLICY_FILE_BYTES
        ) != policy_base.read_bytes(path, base.MAX_POLICY_FILE_BYTES):
            base.fail(f"v65 steady-state controlled file drifted: {path}")
    ...
    if rest:
        projected_candidate, projected_base = _v64_views(candidate, policy_base)
...
def files(view: Any) -> None:
    ...
    for path in sorted(CONTROLLED_FILES):
        if path not in V25.ps(view):
            base.fail(f"v65 controlled file missing: {path}")
        if V25.mode(view, path) != "100644":
            base.fail(f"v65 controlled file mode invalid: {path}")
        if view.read_bytes(path, base.MAX_POLICY_FILE_BYTES) != approved[path]:
            base.fail(f"v65 controlled file content drifted: {path}")
    if not _reopen_available(view):
...
def verify_component_base(
    ...
    *,
    ...
    if _PREDECESSOR_COMPONENT_BASE is None:
        base.fail("v65 predecessor component-base hook unavailable")
    path_set = set(paths)
    if path_set & REOPEN_FILES:
        remaining = path_set - REOPEN_FILES
        _call(
            "v65 projected predecessor component-base verifier",
            _PREDECESSOR_COMPONENT_BASE,
            _project_for_v64(view),
            remaining - CONTROLLED_FILES,
            allow_core_main_change=False,
        )
        return
    _call(
        "v65 predecessor component-base verifier",
        _PREDECESSOR_COMPONENT_BASE,
        _project_for_v64(view),
...
def freeze_s1_007_state(candidate: Any, policy_base: Any) -> None:
    if _PREDECESSOR_FREEZE_S1 is None:
        base.fail("v65 predecessor S1 freeze hook unavailable")
    paths = V25.changed(V25.v24.v23, candidate, policy_base)
    if paths == REOPEN_FILES:
        for relative in sorted(V25.FROZEN_STATE_PATHS):
            if candidate.read_bytes(relative, V25.MAX_S1_STATE_BYTES) != policy_base.read_bytes(
    ...
    projected_candidate, projected_base = _v64_views(candidate, policy_base)
    _call(
        "v65 predecessor S1 state freeze",
...
def printer(stage: str, mode_: str) -> None:
    ...
    print("wepld_policy_successor_v65=S2_S006_GITDIR_REOPEN_ONLY")
    print(f"v65_authority={AUTH}")
    print(f"s2_implementation_authority_v65={S2_IMPLEMENTATION_AUTHORITY}")
    print(
        "s2_s006_gitdir_reopen_authority_v65="
        f"{S2_S006_GITDIR_REOPEN_AUTHORITY}"
    )
    print(f"reopen_available_v65={_reopen_available(raw_root)}")
    print(f"doctor_cli_authority_v65={DOCTOR_CLI_AUTHORITY}")
    print(f"general_shell_authority_v65={GENERAL_SHELL_AUTHORITY}")
    print(f"arbitrary_process_authority_v65={ARBITRARY_PROCESS_AUTHORITY}")
    print(f"package_install_authority_v65={PACKAGE_INSTALL_AUTHORITY}")
    print(f"project_native_command_execution_v65={PROJECT_NATIVE_COMMAND_EXECUTION}")
    print(f"git_mutation_authority_v65={GIT_MUTATION_AUTHORITY}")
    print(f"safe_directory_mutation_authority_v65={SAFE_DIRECTORY_MUTATION_AUTHORITY}")
    print(f"remediation_execution_authority_v65={REMEDIATION_EXECUTION_AUTHORITY}")
    print(f"git_process_admission_v65={GIT_PROCESS_ADMISSION}")
    print(f"git_execution_authority_v65={GIT_EXECUTION_AUTHORITY}")
    print(f"external_process_authority_v65={EXTERNAL_PROCESS_AUTHORITY}")
    print(f"network_authority_v65={NETWORK_AUTHORITY}")
    print(f"model_provider_execution_v65={MODEL_PROVIDER_EXECUTION}")
    print(f"source_admission_v65={SOURCE_ADMISSION}")
    print(f"dependency_admission_v65={DEPENDENCY_ADMISSION}")
    print(f"s3_plus_authority_v65={S3_PLUS_AUTHORITY}")
    print(f"test_child_process_authority_v65={TEST_CHILD_PROCESS_AUTHORITY}")
    print(f"next_authority_gate_v65={NEXT_AUTHORITY_GATE}")
...
def prepare_q() -> None:
    for module in _chain():
        ...
        if current not in (Q_WF, dict(WF)):
            base.fail(f"v65 predecessor workflow identity map drifted: actual={current}")
    for module in _chain():
        module.WF = dict(WF)
...
def overlay() -> None:
    ...
    if any(actual is not expected for actual, expected in pairs):
        base.fail("v65 installed overlay drifted")
    if any(dict(module.WF) != dict(WF) for module in _chain()):
        base.fail("v65 workflow identity projection drifted")
    for name in _INHERITED_AUTHORITY_NAMES:
        if getattr(q, name) != globals()[name]:
            base.fail(f"v65 inherited authority drifted: {name}")
...
def install() -> None:
    ...
    shell, routing, _, desktop, execution = V25.topo()
    pairs = (
        (_attr(routing, "IMPL_REQUIRE_EXACT_DELTA", "v64 routing hook"), q.delta),
        (base.compare_base_controlled, q.basectrl),
        (_attr(desktop, "verify_extension_controlled_paths", "v64 desktop hook"), q.dext),
        (_attr(execution, "verify_extension_controlled_paths", "v64 execution hook"), q.eext),
        (_attr(shell, "validate_allowed_paths", "v64 allowed hook"), q.allowed),
        (_attr(shell, "verify_policy_files", "v64 files hook"), q.files),
        (_attr(shell, "print_success", "v64 printer"), q.printer),
    )
    if any(actual is not expected for actual, expected in pairs):
        base.fail("v65 predecessor hook drifted")
    ...
    _bind(desktop, "EXTENSION_CONTROLLED_PATHS", desktop_extensions, "v65 desktop registration")
    _bind(execution, "EXTENSION_CONTROLLED_PATHS", execution_extensions, "v65 execution registration")
    _bind(routing, "IMPL_REQUIRE_EXACT_DELTA", delta, "v65 routing hook")
    base.compare_base_controlled = basectrl
    _bind(desktop, "verify_extension_controlled_paths", dext, "v65 desktop hook")
    _bind(execution, "verify_extension_controlled_paths", eext, "v65 execution hook")
    _bind(shell, "validate_allowed_paths", allowed, "v65 allowed hook")
    _bind(shell, "verify_policy_files", files, "v65 files hook")
    _bind(shell, "print_success", printer, "v65 printer hook")
    _bind(execution, "_verify_component_base", verify_component_base, "v65 component-base hook")
    _bind(execution, "freeze_s1_007_state", freeze_s1_007_state, "v65 S1 state freeze hook")
    _INST = True
    overlay()


def selftest() -> None:
    from wepld_s2_s006_gitdir_reopen_v65_selftest import run
