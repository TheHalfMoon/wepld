 ...
 * PICTORIAL_E2E_LLM_PROVIDER=deepseek. Override the model via { model } when
 * constructing, or via PICTORIAL_E2E_LLM_MODEL at the call site.
 *
...
export const VARIANT_SYSTEM_INSTRUCTIONS = [
  'You are an automated subagent inside Pictorial\'s live-mode test harness.',
  ...
  '- Insert mode variants must contain visible inserted content. Do not return empty roots, placeholder-only roots, inline style= attributes, or test hooks like <div data-pictorial-e2e-variant="1"></div>.',
  ...
  '- Replace mode: for non-bare elements where the existing children must stay in place, add a harmless root attribute such as data-pictorial-e2e-variant="1" or another non-copy styling hook so the markup is materially changed without changing visible text.',
  ...
  '- Do NOT emit the wrapping <div data-pictorial-variant="N">. The orchestrator wraps your content.',
  '- Do NOT emit the outer <style data-pictorial-css> tag. Only its contents go in scopedCss.',
...
export const MANUAL_EDIT_SYSTEM_INSTRUCTIONS = [
  '<role>',
  'You are an automated source-edit planner for one Pictorial live manual_edit_apply batch or chunk.',
  '</role>',
  '',
  '<workflow>',
  ...
  '- Never copy live runtime scaffolding into sourceEdits: no contenteditable, data-pictorial-* attributes, variant wrappers, live markers, <style>, <script>, comments, or generated browser attributes.',
  '</source_edit_rules>',
  '',
  'OUTPUT CONTRACT — return ONLY a JSON object with this exact shape. No prose, no code fences, no commentary:',
  '',
...
const STEER_SYSTEM_INSTRUCTIONS = [
  ...
  '- Never edit temporary preview or scratch paths such as node_modules/.pictorial-live; Steer edits must land in the real app source file.',
 ...
 * @property {'anthropic' | 'deepseek'=} provider Override PICTORIAL_E2E_LLM_PROVIDER.
...
export function resolveLlmAgentConfig(opts = {}, env = process.env) {
  ...
  if (provider === 'openai') {
    return {
      provider,
      model: opts.model || env.PICTORIAL_E2E_LLM_MODEL || DEFAULT_OPENAI_MODEL,
      apiKey: opts.apiKey || env.OPENAI_API_KEY,
      requiredEnv: 'OPENAI_API_KEY',
      baseURL: opts.baseURL || env.OPENAI_BASE_URL,
      reasoningEffort: opts.reasoningEffort || env.PICTORIAL_E2E_LLM_EFFORT || DEFAULT_OPENAI_REASONING_EFFORT,
    };
  }
  ...
  if (provider === 'anthropic') {
    return {
      provider,
      model: opts.model || env.PICTORIAL_E2E_LLM_MODEL || DEFAULT_ANTHROPIC_MODEL,
  ...
  if (provider === 'deepseek') {
    return {
      provider,
      model: opts.model || env.PICTORIAL_E2E_LLM_MODEL || DEFAULT_DEEPSEEK_MODEL,
  ...
  throw new Error(`Unsupported PICTORIAL_E2E_LLM_PROVIDER: ${provider}`);
}

function resolveProvider(opts, env) {
  const explicit = opts.provider || env.PICTORIAL_E2E_LLM_PROVIDER;
      ...
      for (let attempt = 0; attempt < MANUAL_EDIT_RESPONSE_MAX_ATTEMPTS; attempt += 1) {
        ...
        if (isInsert) {
          userMessage = [
            ...
            'Do not use data-pictorial-* attributes or empty test-hook-only roots.',
  ...
  if (event.progressive?.phase === 'remaining') {
    return [
      ...
      ...(event.progressive.omitFirstVariantCss ? [
        '- Variant 1 CSS is already published and immutable. Do not repeat or modify any scopedCss rule for data-pictorial-variant="1"; return scopedCss rules for variants 2+ only.',
      ] : []),
...
function validateVariantInnerHtml(html) {
  ...
  if (/\bdata-pictorial-variants?\s*=/.test(html)) return 'must not include Pictorial wrapper attributes';
...
export function validateInsertVariantOutput(parsed, event = {}) {
  for (const [i, variant] of parsed.variants.entries()) {
    ...
    if (/\bdata-pictorial-[\w-]*\s*=/.test(html)) {
      return `insert variant ${i} contains preview-only data-pictorial attributes`;
    }
...
export function validateProgressiveVariantOutput(parsed, event = {}) {
  ...
  if (event.progressive?.phase === 'remaining' && event.progressive.firstVariant?.innerHtml) {
    ...
    if (event.progressive.omitFirstVariantCss && /\[data-pictorial-variant\s*=\s*["']1["'][^\]]*\]/.test(parsed.scopedCss || '')) {
