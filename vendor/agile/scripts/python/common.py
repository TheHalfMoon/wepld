"""Shared helpers for Agile Python scripts."""
...
def get_repo_root(script_file: Path | None = None) -> Path:
    ...
    if script_file is not None:
        ...
        # Installed scripts live at .agile/scripts/python/<script>.py.
...
def read_feature_json_feature_directory(repo_root: Path) -> str:
    feature_json = repo_root / ".agile" / "feature.json"
...
def get_feature_paths(
    *, no_persist: bool = False, script_file: Path | None = None
    ...
    elif (repo_root / ".agile" / "feature.json").is_file():
...
def resolve_template(template_name: str, repo_root: Path) -> Path | None:
    ...
    Order (mirrors resolve_template in scripts/bash/common.sh):
      ...
      2. .agile/presets/<preset-id>/templates/ (sorted by .registry priority)
      3. .agile/extensions/<ext-id>/templates/ (hidden directories skipped)
      4. .agile/templates/ (core)
    """
    if not _is_safe_component(template_name):
        return None

    base = repo_root / ".agile" / "templates"

    override = base / "overrides" / f"{template_name}.md"
    if override.is_file():
        return override

    presets_dir = repo_root / ".agile" / "presets"
    ...
    ext_dir = repo_root / ".agile" / "extensions"
    if ext_dir.is_dir():
...
# Mirror the canonical PresetManifest contract (see src/agile_cli/presets)
...
def resolve_template_content(template_name: str, repo_root: Path) -> str | None:
    ...
    extensions_dir = repo_root / ".agile" / "extensions"
    ...
    core = repo_root / ".agile" / "templates" / f"{template_name}.md"
    if core.is_file():
...
def get_invoke_separator(repo_root: Path) -> str:
    integration_json = repo_root / ".agile" / "integration.json"
...
def format_agile_command(command_name: str, repo_root: Path) -> str:
