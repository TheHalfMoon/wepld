...
from agile_cli.integrations import INTEGRATION_REGISTRY, get_integration
from agile_cli.integrations.base import SkillsIntegration
...
class TestBobIntegrationRegistration:
    ...
    def test_invoke_separator_is_hyphen(self):
        ...
        generates correct /agile-<name> refs without calling effective_invoke_separator."""
        bob = get_integration("bob")
        assert bob.invoke_separator == "-"
...
class TestBobIsSkillsModeHook:
    ...
    def test_existing_commands_layout_preserved_on_use(self, tmp_path):
        ...
        ``.bob/commands/agile.*.md`` on disk, no stored ``legacy_commands``)
        ...
        (cmds / "agile.plan.md").write_text("# plan", encoding="utf-8")
    ...
    def test_existing_skills_layout_stays_skills_on_use(self, tmp_path):
        """A project with managed ``agile-*`` skills resolves to skills mode."""
        bob = get_integration("bob")
        (tmp_path / ".bob" / "skills" / "agile-plan").mkdir(parents=True)
    ...
    def test_managed_commands_with_unrelated_skills_dir_stays_legacy(
        ...
        """Regression (review #3415, 4723246468): a legacy Agile install
        (managed ``.bob/commands/agile.*.md``) that *also* carries unrelated
        Bob 2 skills (a ``.bob/skills/`` dir with no managed ``agile-*``
        skills) must stay in command mode — the mere presence of a skills
        directory is not evidence that Agile is skills-based.
        """
        bob = get_integration("bob")
        cmds = tmp_path / ".bob" / "commands"
        cmds.mkdir(parents=True)
        (cmds / "agile.plan.md").write_text("# plan", encoding="utf-8")
        # An unrelated (non-Agile) skill the user authored.
    ...
    def test_managed_skills_win_when_both_layouts_present(self, tmp_path):
        """When managed Agile skills exist, skills mode wins even if a stale
...
class TestBobDefaultSkillsMode:
    """Default mode: .bob/skills/agile-<name>/SKILL.md layout."""
    ...
    def test_skill_directory_structure(self, tmp_path):
        ...
        actual_commands = {f.parent.name.removeprefix("agile-") for f in created}
    ...
    def test_command_refs_use_hyphen_separator(self, tmp_path):
        """Default skills layout must use /agile-<name>, not /agile.<name>."""
        bob = get_integration("bob")
        m = IntegrationManifest("bob", tmp_path)
        created = bob.setup(tmp_path, m)
        for f in created:
...
class TestBobLegacyCommandsMode:
    """Legacy opt-in mode: .bob/commands/agile.<name>.md layout."""

    def test_setup_legacy_creates_markdown_files(self, tmp_path):
        from agile_cli.integrations.bob import BobIntegration
        bob = BobIntegration()
        m = IntegrationManifest("bob", tmp_path)
    ...
    def test_setup_legacy_templates_are_processed(self, tmp_path):
        ...
        for md_file in commands_dir.glob("agile.*.md"):
...
class TestBobInitFlowDefault:
    ...
    def test_init_default_creates_skills(self, tmp_path):
        ...
        assert (target / ".bob" / "skills" / "agile-plan" / "SKILL.md").exists()
    ...
    def test_init_default_complete_file_inventory_sh(self, tmp_path):
        ...
        for cmd in commands:
            assert (project / ".bob" / "skills" / f"agile-{cmd}" / "SKILL.md").exists(), (
                f"Missing .bob/skills/agile-{cmd}/SKILL.md"
            )
...
class TestBobInitFlowLegacy:
    ...
    def test_init_legacy_creates_commands(self, tmp_path):
        ...
        assert (target / ".bob" / "commands" / "agile.plan.md").exists()
...
class TestBobRegistrarConfig:
    ...
    def test_skills_mode_project_extension_registration_skipped(self, tmp_path):
        ...
        results = registrar.register_commands_for_all_agents(
            commands=[{"name": "agile.test-cmd", "file": "test.md"}],
            source_id="test",
            source_dir=tmp_path,
    ...
    def test_legacy_mode_project_extension_registration_runs(self, tmp_path):
        ...
        registered_file = commands_dir / "agile.test-cmd.md"
    ...
    def test_legacy_extension_command_refs_use_dot_separator(self, tmp_path):
        ...
        render Bob 1.x ``/agile.<cmd>`` refs, not the skills-layout ``/agile-<cmd>``.
...
class TestBobUseFlowPreservesLegacyLayout:
    ...
    def test_update_init_options_preserves_legacy_commands_project(self, tmp_path):
        ...
        """
        from agile_cli.integrations._helpers import (
    ...
    def test_with_integration_setting_stores_dot_separator_for_legacy(self, tmp_path):
        ...
        command references get rewritten to ``/agile-*``.
        """
        from agile_cli.integration_runtime import with_integration_setting
        ...
        assert settings["bob"]["invoke_separator"] == ".", (
            ...
            "shared templates render Bob 1.x /agile.<cmd> references"
        )
    ...
    def test_use_force_keeps_legacy_command_refs_in_shared_templates(self, tmp_path):
        ...
        with ``/agile.<cmd>`` (dot), not ``/agile-<cmd>``.
        """
        import json
        from typer.testing import CliRunner
        from agile_cli import app
        ...
        template = target / ".agile" / "templates" / "plan-template.md"
        ...
        integ_json = target / ".agile" / "integration.json"
        ...
        assert "/agile.plan" in rendered, (
            "legacy Bob project must keep /agile.plan (dot) after refresh"
        )
        assert "/agile-plan" not in rendered, (
            "shared templates must not be rewritten to the skills /agile-plan"
        )
...
class TestBobCommandRefScopedToActiveAgent:
    ...
    ``/agile.<cmd>``) rather than inheriting Copilot's ``ai_skills=True`` and
    rendering ``/agile-<cmd>``.
    """
    ...
    def _write_command_ref_ext(self, source_dir):
        ...
        cmd.write_text(
            "---\ndescription: Run\n---\n\nUse __AGILE_COMMAND_PLAN__ first.\n",
            encoding="utf-8",
        )
        return [{"name": "agile.ext.run", "file": "run.md"}]

    def test_legacy_bob_ref_not_rewritten_when_other_agent_active_in_skills(
        self, tmp_path
    ):
        from agile_cli._init_options import save_init_options
        from agile_cli.agents import CommandRegistrar
        ...
        assert "agile.ext.run" in registered
    ...
    def test_active_bob_skills_command_output_uses_dot(self, tmp_path):
        ...
        command-layout artifact and must render Bob 1.x ``/agile.<cmd>``.
        Rendering ``/agile-<cmd>`` into a command file (as the old
    ...
    def test_inactive_bob_command_output_uses_dot_even_with_skills_dir(
        ...
        Even when a ``.bob/skills/`` directory (with managed ``agile-*``
        skills) coexists, command-layout files must keep ``/agile.<cmd>``.
        """
        from agile_cli._init_options import save_init_options
        from agile_cli.agents import CommandRegistrar
...
class TestBobSetupPreservesLegacyOnUpgrade:
    ...
    ``.bob/commands/agile.*.md`` on disk, no stored options) preserves the
...
class TestBobPostProcessSkillContent:
    ...
    def test_post_process_is_noop_no_hook_note_injected(self):
        ...
        sample = (
            "---\nname: agile-plan\n---\n\n"
            "Run /agile.plan then /agile.tasks.\n"
        )
        assert bob.post_process_skill_content(sample) == sample

    def test_post_process_matches_skills_helper(self):
        from agile_cli.integrations.bob import _BobSkillsHelper

        bob = get_integration("bob")
        sample = "---\nname: agile-analyze\n---\n\nSome body with /agile.plan.\n"
        assert (
