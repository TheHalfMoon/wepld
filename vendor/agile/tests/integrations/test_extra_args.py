"""Tests for the per-integration `AGILE_INTEGRATION_<KEY>_EXTRA_ARGS` and
`AGILE_INTEGRATION_<KEY>_EXECUTABLE` env-var hooks.
...
from agile_cli.integrations.base import (
    MarkdownIntegration,
    SkillsIntegration,
    TomlIntegration,
)
...
def _clean_extra_args_env(monkeypatch):
    """Strip any leaked AGILE_INTEGRATION_*_EXTRA_ARGS and
    AGILE_INTEGRATION_*_EXECUTABLE vars from the test env so a
    developer's shell setting doesn't pollute results."""
    for key in list(os.environ):
        if key.startswith("AGILE_INTEGRATION_") and (
...
def test_env_var_set_flag_inserted_before_model_and_output_format(
    ...
    monkeypatch.setenv(
        "AGILE_INTEGRATION_CLAUDE_EXTRA_ARGS", "--dangerously-skip-permissions"
    )
...
def test_env_var_multi_token_parsed_via_shlex(monkeypatch):
    monkeypatch.setenv(
        "AGILE_INTEGRATION_CLAUDE_EXTRA_ARGS",
...
def test_malformed_quoting_raises_actionable_value_error(monkeypatch):
    ...
    assert "AGILE_INTEGRATION_CLAUDE_EXTRA_ARGS" in msg
...
def test_env_var_empty_or_whitespace_is_noop(monkeypatch):
    ...
    monkeypatch.setenv("AGILE_INTEGRATION_CLAUDE_EXTRA_ARGS", "   ")
    args = _ClaudeStub().build_exec_args("p")
    assert args == ["claude", "-p", "p", "--output-format", "json"]


def test_other_integration_env_var_ignored(monkeypatch):
    """`AGILE_INTEGRATION_GEMINI_EXTRA_ARGS` set must NOT leak into
    Claude's argv (per-integration scoping)."""
    monkeypatch.setenv("AGILE_INTEGRATION_GEMINI_EXTRA_ARGS", "--gemini-only-flag")
    args = _ClaudeStub().build_exec_args("p")
    assert args == ["claude", "-p", "p", "--output-format", "json"]


def test_key_normalization_hyphen_to_underscore_uppercase(monkeypatch):
    """`kiro-cli` key looks up `AGILE_INTEGRATION_KIRO_CLI_EXTRA_ARGS`
    (hyphens replaced with underscores, then uppercased)."""
    monkeypatch.setenv(
        "AGILE_INTEGRATION_KIRO_CLI_EXTRA_ARGS", "--some-kiro-flag"
    )
...
def test_requires_cli_false_returns_none(monkeypatch):
    ...
    monkeypatch.setenv("AGILE_INTEGRATION_NO_CLI_EXTRA_ARGS", "--should-not-appear")
...
def test_markdown_integration_base_honours_extra_args(monkeypatch):
    ...
    monkeypatch.setenv(
        "AGILE_INTEGRATION_MD_AGENT_EXTRA_ARGS", "--debug --max-tokens 100"
    )
...
def test_toml_integration_base_honours_extra_args(monkeypatch):
    ...
    monkeypatch.setenv(
        "AGILE_INTEGRATION_TOML_AGENT_EXTRA_ARGS", "--yolo"
    )
...
def test_codex_integration_honours_extra_args(monkeypatch):
    ...
    monkeypatch.setenv("AGILE_INTEGRATION_CODEX_EXTRA_ARGS", "--sandbox read-only")
...
def test_devin_integration_honours_extra_args(monkeypatch):
    ...
    monkeypatch.setenv("AGILE_INTEGRATION_DEVIN_EXTRA_ARGS", "--no-confirm")
...
def test_opencode_integration_honours_extra_args(monkeypatch):
    ...
    monkeypatch.setenv("AGILE_INTEGRATION_OPENCODE_EXTRA_ARGS", "--quiet")
...
def test_opencode_extra_args_cannot_clobber_prompt_derived_command(
    ...
    ``--command <X>`` so that Agile's command selection wins under
    repeated-flag CLI semantics (last value typically takes precedence).

    Locks against the regression where an operator setting
    ``AGILE_INTEGRATION_OPENCODE_EXTRA_ARGS="--command malicious"`` could redirect
    a slash-prefixed prompt to a different command.
    """
    from agile_cli.integrations.opencode import OpencodeIntegration

    monkeypatch.setenv(
        "AGILE_INTEGRATION_OPENCODE_EXTRA_ARGS", "--command operator-override"
    )
    args = OpencodeIntegration().build_exec_args("/agile body text")
    # Prompt-derived "--command agile" appears AFTER the
    # operator-injected one, so a CLI that resolves repeated flags
    # last-wins will honour Agile's choice.
    assert args == [
        "opencode",
        "run",
        "--command",
...
def test_copilot_integration_honours_extra_args(monkeypatch):
    from agile_cli.integrations.copilot import (
        CopilotIntegration,
        _copilot_executable,
    )
    ...
    monkeypatch.setenv(
        "AGILE_INTEGRATION_COPILOT_EXTRA_ARGS", "--allow-tool 'shell(echo)'"
    )
...
def test_copilot_commands_dispatch_includes_extra_args(monkeypatch):
    ...
    must honour `AGILE_INTEGRATION_COPILOT_EXTRA_ARGS`, not just `build_exec_args`.
    """
    import subprocess
    ...
    integration.dispatch_command("agile.plan", args="body", stream=False)
...
def test_codex_dispatch_command_includes_extra_args(monkeypatch):
    ...
    CodexIntegration().dispatch_command(
        "agile.plan", args="body", stream=False
    )
...
# ---------------------------------------------------------------------------
# AGILE_INTEGRATION_<KEY>_EXECUTABLE tests
#
# The `_resolve_executable()` method on `IntegrationBase` checks
# `AGILE_INTEGRATION_<KEY>_EXECUTABLE` and, when set, substitutes that
...
def test_executable_env_var_replaces_first_argv_token(monkeypatch):
    ...
    monkeypatch.setenv("AGILE_INTEGRATION_CLAUDE_EXECUTABLE", "/opt/claude/bin/claude")
...
def test_executable_env_var_whitespace_only_falls_back_to_key(monkeypatch):
    ...
    monkeypatch.setenv("AGILE_INTEGRATION_CLAUDE_EXECUTABLE", "   ")
    args = _ClaudeStub().build_exec_args("p")
    assert args[0] == "claude"


def test_executable_env_var_key_normalization_hyphen_to_underscore(monkeypatch):
    """`kiro-cli` key maps to `AGILE_INTEGRATION_KIRO_CLI_EXECUTABLE`."""
    monkeypatch.setenv("AGILE_INTEGRATION_KIRO_CLI_EXECUTABLE", "/usr/local/bin/kiro-cli")
    args = _KiroCliStub().build_exec_args("p")
    assert args[0] == "/usr/local/bin/kiro-cli"


def test_executable_env_var_other_integration_ignored(monkeypatch):
    """`AGILE_INTEGRATION_GEMINI_EXECUTABLE` must NOT affect Claude."""
    monkeypatch.setenv("AGILE_INTEGRATION_GEMINI_EXECUTABLE", "/custom/gemini")
    args = _ClaudeStub().build_exec_args("p")
    assert args[0] == "claude"
...
def test_executable_env_var_markdown_integration(monkeypatch):
    ...
    monkeypatch.setenv("AGILE_INTEGRATION_MD_AGENT_EXECUTABLE", "/custom/md-agent")
    args = _MarkdownAgentStub().build_exec_args("p")
...
def test_executable_env_var_toml_integration(monkeypatch):
    ...
    monkeypatch.setenv("AGILE_INTEGRATION_TOML_AGENT_EXECUTABLE", "/custom/toml-agent")
...
def test_executable_env_var_requires_cli_false_returns_none(monkeypatch):
    ...
    monkeypatch.setenv("AGILE_INTEGRATION_NO_CLI_EXECUTABLE", "/custom/no-cli")
    assert _NoCliStub().build_exec_args("p") is None
...
def test_executable_env_var_codex_integration(monkeypatch):
    ...
    monkeypatch.setenv("AGILE_INTEGRATION_CODEX_EXECUTABLE", "/opt/codex")
...
def test_executable_env_var_devin_integration(monkeypatch):
    ...
    monkeypatch.setenv("AGILE_INTEGRATION_DEVIN_EXECUTABLE", "/opt/devin")
    args = DevinIntegration().build_exec_args("p")
...
def test_goose_extra_args_precede_canonical_flags(monkeypatch):
    """Extra args are applied before Agile's canonical flags, matching the
    ...
    # Agile itself must never emit a duplicate single-value flag.
...
def test_executable_env_var_goose_integration(monkeypatch):
    ...
    monkeypatch.setenv("AGILE_INTEGRATION_GOOSE_EXECUTABLE", "/opt/goose")
...
def test_executable_env_var_opencode_integration(monkeypatch):
    ...
    monkeypatch.setenv("AGILE_INTEGRATION_OPENCODE_EXECUTABLE", "/opt/opencode")
    args = OpencodeIntegration().build_exec_args("p")
...
def test_executable_env_var_copilot_integration(monkeypatch):
    ...
    monkeypatch.setenv("AGILE_INTEGRATION_COPILOT_EXECUTABLE", "/opt/copilot")
    monkeypatch.setenv("AGILE_COPILOT_ALLOW_ALL_TOOLS", "0")
    args = CopilotIntegration().build_exec_args("p")
...
def test_executable_env_var_copilot_unset_uses_platform_default(monkeypatch):
    """When `AGILE_INTEGRATION_COPILOT_EXECUTABLE` is unset, Copilot
    falls back to the platform-specific default from `_copilot_executable()`."""
    from agile_cli.integrations.copilot import CopilotIntegration, _copilot_executable
...
def test_executable_env_var_copilot_dispatch_command(monkeypatch):
    ...
    CopilotIntegration().dispatch_command("agile.plan", args="body", stream=False)
