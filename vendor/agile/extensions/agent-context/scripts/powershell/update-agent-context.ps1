...
$DefaultStart = '<!-- AGILE START -->'
$DefaultEnd   = '<!-- AGILE END -->'
$ProjectRoot  = (Get-Location).Path
$ExtConfig    = Join-Path $ProjectRoot '.agile/extensions/agent-context/agent-context-config.yml'
...
if ($null -eq $Options) {
    ...
    $pythonCandidates = @()
    if ($env:AGILE_PYTHON) {
        $pythonCandidates += $env:AGILE_PYTHON
    }
...
if ($ContextFiles.Count -eq 0) {
    ...
    # (agent-context-defaults.json). Independent of the Agile CLI by design.
    $initOptionsPath = Join-Path $ProjectRoot '.agile/init-options.json'
    if (Test-Path -LiteralPath $initOptionsPath) {
        try {
            ...
            if ($integrationKey) {
                $defaultsPath = Join-Path $ProjectRoot '.agile/extensions/agent-context/agent-context-defaults.json'
...
if (-not $PlanPath) {
    ...
    $FeatureJson = Join-Path $ProjectRoot '.agile/feature.json'
