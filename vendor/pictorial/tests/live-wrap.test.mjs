...
describe('findElement', () => {
  ...
  it('skips lines that contain data-pictorial-variant', () => {
    const lines = [
      '<div class="hero" data-pictorial-variant="original">Old</div>',
...
describe('wrapCli integration', () => {
  ...
  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), 'pictorial-wrap-test-'));
...
<body>
  <div class="hero-section">
    ...
    assert.ok(modified.includes('data-pictorial-variants="test123"'));
    assert.ok(modified.includes('data-pictorial-variant-count="3"'));
    assert.ok(modified.includes('data-pictorial-variant="original"'));
    assert.ok(modified.includes('display: contents'));
    assert.ok(modified.includes('pictorial-variants-start test123'));
    assert.ok(modified.includes('pictorial-variants-end test123'));
    // Original should NOT be hidden (stays visible until variants arrive)
    assert.ok(!modified.includes('data-pictorial-variant="original" style="display: none"'));
  });
...
<body>
  <div class="hero-section">
    ...
    assert.ok(result.wrapperBlock.includes('data-pictorial-variants="defer1"'));
    assert.ok(result.wrapperBlock.includes('Variants: insert below this line'));
    assert.ok(result.wrapperBlock.includes('pictorial-variants-end defer1'));
  ...
  return (
    ...
    assert.ok(modified.includes('{/* pictorial-variants-start jsx123'));
    assert.ok(modified.includes('data-pictorial-variant-count="2"'));
  });
...
<div id="pricing">
  ...
  <p>Plans start at $10/mo.</p>
    ...
    assert.ok(modified.includes('data-pictorial-variants="id123"'));
...
<div class="after">After</div>`;
    ...
    assert.ok(modified.includes('data-pictorial-variants="pres123"'));
  });
  ...
  <h1>Plain title</h1>
    ...
    assert.equal(result.cssAuthoring.styleTag, '<style data-pictorial-css="SESSION_ID">');
...
<section class="hero-shell">
  <h1>{title}</h1>
    ...
    assert.deepEqual(result.cssSelectorPrefixExamples, [
      '[data-pictorial-variant="1"]',
      '[data-pictorial-variant="2"]',
      '[data-pictorial-variant="3"]',
    ]);
    assert.equal(result.cssAuthoring.mode, 'astro-global-prefixed');
    assert.equal(result.cssAuthoring.strategy, 'global-prefixed');
    assert.equal(result.cssAuthoring.styleTag, '<style is:inline data-pictorial-css="SESSION_ID">');
    assert.match(result.cssAuthoring.rulePattern, /^\[data-pictorial-variant="N"\]/);
    assert.ok(
...
// .pictorial/live/pending-manual-edits.json from local dev tripped the
// fail-loud check in live-wrap. Clear the buffer around each test.
function clearManualEditsBuffer() {
  try {
    const p = join(process.cwd(), '.pictorial/live/pending-manual-edits.json');
    rmSync(p, { force: true });
  } catch {}
}
...
describe('live-wrap — JSX / TSX correctness', () => {
  let tmp;
  beforeEach(() => { tmp = mkdtempSync(join(tmpdir(), 'pictorial-wrap-jsx-')); clearManualEditsBuffer(); });
  ...
  return (
    ...
    assert.ok(modified.includes('pictorial-variants-start wrapA'), 'wrapper was created');
    ...
    const originalMatch = modified.match(/data-pictorial-variant="original"[^>]*>([\s\S]*?)\s*<\/div>/);
...
export default function App() {
  ...
  return (
    ...
    // The outer wrapper <div data-pictorial-variants="..."> appears BEFORE
    // both marker comments — markers are tucked inside.
    const wrapperIdx = modified.indexOf('data-pictorial-variants="frag1"');
    const startMarkerIdx = modified.indexOf('pictorial-variants-start frag1');
    const endMarkerIdx = modified.indexOf('pictorial-variants-end frag1');
  ...
  it('HTML wrapper keeps marker comments OUTSIDE the wrapper <div> (existing layout)', () => {
    ...
    const modified = readFileSync(join(tmp, 'page.html'), 'utf-8');
    const wrapperIdx = modified.indexOf('data-pictorial-variants="htmlFrag"');
    const startMarkerIdx = modified.indexOf('pictorial-variants-start htmlFrag');
  ...
  return (
    ...
    const modified = readFileSync(join(tmp, 'Page.tsx'), 'utf-8');
    const originalMatch = modified.match(/data-pictorial-variant="original"[\s\S]*?<\/div>/);
  ...
  return (
    ...
    assert.ok(modified.includes('data-pictorial-variants="short1"'),
  ...
  <section class="multiline-target">
    ...
    // line (the pictorial-variants-end marker for HTML).
    assert.match(lines[result.endLine - 1], /pictorial-variants-end ml1/,
  ...
  return (
    ...
    assert.ok(modified.includes('data-pictorial-variants="dyn1"'), 'wrapped (first-match fallback)');
  });
...
<main>
  ...
  <article class="result-card"><h2>{results[1].title}</h2></article>
    ...
    assert.doesNotMatch(readFileSync(file, 'utf-8'), /pictorial-variants-start/);
  });
