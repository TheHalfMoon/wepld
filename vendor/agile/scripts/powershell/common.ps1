...
function Find-SpecifyRoot {
    ...
    while ($true) {
        if (Test-Path -LiteralPath (Join-Path $current ".agile") -PathType Container) {
            return $current
        }
...
function Save-FeatureJson {
    ...
    $fjPath = Join-Path (Join-Path $RepoRoot '.agile') 'feature.json'
    ...
    $specifyDir = Join-Path $RepoRoot '.agile'
...
function Get-FeaturePathsEnv {
    ...
    # so pure path resolution never writes .agile/feature.json, which would
...
function Get-InvokeSeparator {
    ...
    if ($null -eq $script:AgileInvokeSeparatorCache) {
        $script:AgileInvokeSeparatorCache = @{}
    }
    if ($script:AgileInvokeSeparatorCache.ContainsKey($RepoRoot)) {
        return $script:AgileInvokeSeparatorCache[$RepoRoot]
    }

    $separator = '.'
    $integrationJson = Join-Path $RepoRoot '.agile/integration.json'
    ...
    $script:AgileInvokeSeparatorCache[$RepoRoot] = $separator
    return $separator
}

function Format-AgileCommand {
    param(
    ...
    } elseif ($name.StartsWith('agile-')) {
        $name = $name.Substring(8)
    }
    $name = $name -replace '\.', $separator

    return "/agile$separator$name"
}
...
function Resolve-Template {
    ...
    $base = Join-Path $RepoRoot '.agile/templates'
    ...
    $presetsDir = Join-Path $RepoRoot '.agile/presets'
    ...
    $extDir = Join-Path $RepoRoot '.agile/extensions'
    if (Test-Path $extDir) {
