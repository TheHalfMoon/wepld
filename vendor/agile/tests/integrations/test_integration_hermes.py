...
class TestHermesIntegration(SkillsIntegrationTests):
    ...
    def test_setup_writes_to_global_skills_dir(self, tmp_path, monkeypatch):
        ...
        for f in skill_files:
            # Every skill file should be under ~/.hermes/skills/agile-*/
    ...
    def test_plan_skill_has_no_context_placeholder(self, tmp_path, monkeypatch):
        ...
        plan_file = home / ".hermes" / "skills" / "agile-plan" / "SKILL.md"
    ...
    def test_pre_existing_skills_not_removed(self, tmp_path, monkeypatch):
        """Pre-existing non-agile global skills should survive Hermes uninstall."""
        home = _fake_home(tmp_path)
        monkeypatch.setattr(Path, "home", lambda: home)
    ...
    def test_hook_sections_explain_dotted_command_conversion(self, tmp_path, monkeypatch):
        ...
        m = IntegrationManifest(self.KEY, tmp_path)
        i.setup(tmp_path, m)
        specify_skill = home / ".hermes" / "skills" / "agile-specify" / "SKILL.md"
    ...
    def test_complete_file_inventory_sh(self, tmp_path, monkeypatch):
        ...
        # Ensure no core .hermes/skills/agile-*/SKILL.md in project dir
        # (extension-installed skills like agent-context-update may appear)
        hermes_skill_files = [
            f for f in actual
            if f.startswith(".hermes/skills/agile-")
...
class TestHermesInitFlow:
    ...
    def test_integration_hermes_creates_global_skills(self, tmp_path, monkeypatch):
        ...
        assert (home / ".hermes" / "skills" / "agile-plan" / "SKILL.md").exists()
...
class TestHermesBuildExecArgs:
    ...
    def test_build_exec_args_default_shape(self):
        ...
        assert i.build_exec_args("/agile-plan hi", output_json=True) == [
            "hermes", "chat", "-Q", "--json", "-s", "agile-plan", "-q", "hi",
        ]

    def test_build_exec_args_honors_extra_args(self, monkeypatch):
        """AGILE_INTEGRATION_HERMES_EXTRA_ARGS is injected before the
        ...
        """
        monkeypatch.setenv(
            "AGILE_INTEGRATION_HERMES_EXTRA_ARGS", "--temperature 0.2"
        )
        i = get_integration("hermes")
        args = i.build_exec_args("/agile-plan hi", output_json=True)
        assert args == [
            "hermes", "chat", "-Q", "--temperature", "0.2",
            "--json", "-s", "agile-plan", "-q", "hi",
        ]
    ...
    def test_build_exec_args_honors_executable_override(self, monkeypatch):
        monkeypatch.setenv(
            "AGILE_INTEGRATION_HERMES_EXECUTABLE", "/custom/hermes"
        )
        i = get_integration("hermes")
        assert i.build_exec_args("/agile-plan hi")[0] == "/custom/hermes"
