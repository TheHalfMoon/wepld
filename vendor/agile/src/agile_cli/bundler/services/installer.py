...
class PrimitiveInstaller(Protocol):
    """Adapter over the existing Agile primitive install/remove machinery."""
...
def install_bundle(
    ...
    When *refresh* is True (used by ``agile bundle update``), components that
    ...
    refresh; running ``agile bundle update`` re-applies every owned component
