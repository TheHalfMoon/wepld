"""Shared project-resolution helpers for the Agile CLI."""
...
def _resolve_init_dir_override() -> Path | None:
    ...
    the project root — the directory *containing* ``.agile/`` — and is strict.
    Relative paths resolve against the current directory; the path must exist and
    contain ``.agile/``, otherwise this hard-errors with no fallback to cwd
