...
Agile CLI uses **opt-in authentication** for HTTP requests to catalog
sources, extension downloads, and release checks.  No credentials are
sent unless you explicitly configure them.

## Configuration

Create `~/.agile/auth.json` to enable authentication:
...
```bash
mkdir -p ~/.agile
# Copy the JSON above into ~/.agile/auth.json
chmod 600 ~/.agile/auth.json
```
