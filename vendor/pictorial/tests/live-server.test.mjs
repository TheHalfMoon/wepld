...
import {
  ...
  getLiveServerPath,
  getLiveSessionsDir,
} from '../skill/scripts/lib/pictorial-paths.mjs';
import {
...
function startServer(port = 8499, { cwd = REPO_ROOT, env = {} } = {}) {
  return new Promise((resolve, reject) => {
    const proc = spawn('node', [SERVER_SCRIPT, '--port=' + port], {
      ...
      env: { ...process.env, PICTORIAL_LIVE_COPY_AGENT: 'off', ...env },
    });
    let output = '';
...
it('gitignores local Pictorial runtime artifacts', () => {
  const ignored = execFileSync('git', [
    'check-ignore',
    '.pictorial/live/manual-edit-apply-transaction.json',
    '.pictorial/live/manual-edit-evidence/example.json',
  ...
  assert.match(ignored, /\.pictorial\/live\/manual-edit-apply-transaction\.json/);
  assert.match(ignored, /\.pictorial\/live\/manual-edit-evidence\/example\.json/);
  assert.match(ignored, /\.pictorial\/hook\.cache\.json/);
  assert.match(ignored, /\.pictorial\/hook\.pending\.json/);
  assert.match(ignored, /\.pictorial\/config\.local\.json/);
  assert.match(ignored, /\.pictorial\/live\/deferred-svelte-component-accepts\.json/);
});
...
describe('live-server integration', () => {
  ...
  before(async () => {
    ...
    // never land in the real repo's `.pictorial/live/sessions/`. Those would
    // otherwise be replayed into the poll queue on the next real `live` run.
    serverCwd = mkdtempSync(join(tmpdir(), 'pictorial-live-server-'));
    ...
    writeFileSync(join(serverCwd, 'package.json'), JSON.stringify({ name: 'pictorial' }));
  ...
  it('/live.js injects the canonical command vocabulary', async () => {
    // live-browser.js builds its action picker from window.__PICTORIAL_VOCAB__
    ...
    assert.match(body, /window\.__PICTORIAL_VOCAB__\s*=/);
    const injected = JSON.parse(body.match(/window\.__PICTORIAL_VOCAB__\s*=\s*(\[.*?\]);/s)[1]);
  ...
  it('/live.js injects the canonical Live UI surface inventory', async () => {
    ...
    // module's list. Node consumers (including the pictorial-site Live UI lab)
    ...
    const body = await (await fetch(`http://localhost:${server.port}/live.js?token=${server.token}`)).text();
    const injected = JSON.parse(body.match(/window\.__PICTORIAL_LIVE_UI_SURFACES__\s*=\s*(\[.*?\]);\n/s)[1]);
  ...
  it('/live.js serves script with token injected', async () => {
    ...
    assert.ok(text.includes('__PICTORIAL_TOKEN__'));
    assert.ok(text.includes(server.token));
    assert.ok(text.includes('__PICTORIAL_PORT__'));
    const preludeIndex = text.indexOf('window.__PICTORIAL_VOCAB__');
    const sessionPartIndex = text.indexOf('pictorial live script part: session-state (live-browser-session.js)');
    const domPartIndex = text.indexOf('pictorial live script part: dom-helpers (live-browser-dom.js)');
    const browserPartIndex = text.indexOf('pictorial live script part: browser-ui (live-browser.js)');
    const sessionHelperIndex = text.indexOf('__PICTORIAL_LIVE_SESSION__');
    const domHelperIndex = text.indexOf('__PICTORIAL_LIVE_DOM__');
    const browserInitIndex = text.indexOf('__PICTORIAL_LIVE_INIT__');
  ...
  it('/live.js returns 401 without the token and 200 with it', async () => {
    ...
    assert.ok(body.includes('__PICTORIAL_LIVE_INIT__'), 'authorized /live.js returns the assembled bundle');
  });
  ...
  it('/design-system.json reads DESIGN.md plus .pictorial/design.json', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'pictorial-design-system-'));
    let designServer;
    try {
...
`);
  ...
  it('/design-system.json falls back to legacy root DESIGN.json', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'pictorial-design-system-legacy-'));
    let designServer;
    try {
      writeFileSync(join(tmp, 'DESIGN.md'), `---
name: Legacy System
...
`);
  ...
  it('/manual-edit-commit runs the batched AI apply path and clears successful entries', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'pictorial-manual-commit-server-'));
    let commitServer;
    try {
      ...
      commitServer = await startServer(8522, {
        cwd: tmp,
        env: {
          PICTORIAL_LIVE_COPY_AGENT: 'mock',
          PICTORIAL_LIVE_COPY_AGENT_MOCK_DELAY_MS: '400',
          PICTORIAL_LIVE_COPY_AGENT_MOCK_RESULT: JSON.stringify({
            status: 'done',
  ...
  it('/manual-edit-commit async mode returns immediately and reports completion through status/SSE activity', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'pictorial-manual-commit-server-async-'));
    let asyncServer;
    try {
      mkdirSync(join(tmp, 'src'), { recursive: true });
      const sourcePath = join(tmp, 'src', 'page.html');
      ...
      asyncServer = await startServer(8546, {
        ...
        env: {
          PICTORIAL_LIVE_COPY_AGENT: 'mock',
          PICTORIAL_LIVE_COPY_AGENT_MOCK_DELAY_MS: '300',
          PICTORIAL_LIVE_COPY_AGENT_MOCK_RESULT: JSON.stringify({
            status: 'done',
  ...
  it('/manual-edit-commit routes through the chat agent poll loop when configured', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'pictorial-manual-commit-chat-'));
    let chatServer;
    try {
      mkdirSync(join(tmp, 'src'), { recursive: true });
      const sourcePath = join(tmp, 'src', 'page.html');
      ...
      chatServer = await startServer(8524, {
        cwd: tmp,
        env: { PICTORIAL_LIVE_COPY_AGENT: 'chat' },
      });
  ...
  it('/manual-edit-commit includes compact source candidates in chat Apply events', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'pictorial-manual-commit-chat-candidates-'));
    let candidateServer;
    try {
      mkdirSync(join(tmp, 'site/scripts/components'), { recursive: true });
      writeFileSync(join(tmp, 'site/scripts/data.js'), [
        "export const skillFocusAreas = { pictorial: [",
  ...
  it('/manual-edit-commit rejects malformed chat Apply results without rolling back before retry', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'pictorial-manual-commit-chat-invalid-result-'));
    let chatServer;
    try {
      mkdirSync(join(tmp, 'src'), { recursive: true });
      const sourcePath = join(tmp, 'src', 'page.html');
  ...
  it('/manual-edit-commit chunks chat Apply events by op count and aggregates replies', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'pictorial-manual-commit-chat-chunks-'));
    let chunkServer;
    try {
      mkdirSync(join(tmp, 'src'), { recursive: true });
      const sourcePath = join(tmp, 'src', 'page.html');
      ...
      chunkServer = await startServer(8528, {
        cwd: tmp,
        env: {
          PICTORIAL_LIVE_COPY_AGENT: 'chat',
          PICTORIAL_LIVE_MANUAL_EDIT_CHUNK_SIZE: '3',
        },
      });
  ...
  it('/manual-edit-commit keeps fitting multi-op entries together across chat chunks', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'pictorial-manual-commit-chat-entry-chunks-'));
    let chunkServer;
    try {
      mkdirSync(join(tmp, 'src'), { recursive: true });
      const sourcePath = join(tmp, 'src', 'page.html');
  ...
  it('/manual-edit-commit splits one multi-op entry and clears it only after every chunk applies', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'pictorial-manual-commit-chat-split-entry-'));
    let splitServer;
    try {
      mkdirSync(join(tmp, 'src'), { recursive: true });
      const sourcePath = join(tmp, 'src', 'page.html');
  ...
  it('/manual-edit-commit rolls back a split entry when a later chat chunk fails', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'pictorial-manual-commit-chat-chunk-fail-'));
    let failServer;
    try {
      mkdirSync(join(tmp, 'src'), { recursive: true });
      const sourcePath = join(tmp, 'src', 'page.html');
  ...
  it('/manual-edit-commit keeps entries staged when the chat agent does not ack', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'pictorial-manual-commit-timeout-'));
    let timeoutServer;
    try {
      mkdirSync(join(tmp, 'src'), { recursive: true });
      const sourcePath = join(tmp, 'src', 'page.html');
      ...
      timeoutServer = await startServer(8525, {
        ...
        env: {
          PICTORIAL_LIVE_COPY_AGENT: 'chat',
          PICTORIAL_LIVE_APPLY_EVENT_HARD_TIMEOUT_MS: '300',
          PICTORIAL_LIVE_APPLY_EVENT_SOFT_DEADLINE_MS: '250',
        },
      });
  ...
  it('/manual-edit-commit repairs post-apply validation failures instead of rolling back', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'pictorial-manual-repair-success-'));
    let repairServer;
    try {
      mkdirSync(join(tmp, 'src'), { recursive: true });
  ...
  it('/manual-edit-commit asks for a decision after repeated repair failures', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'pictorial-manual-repair-decision-'));
    let decisionServer;
    try {
      mkdirSync(join(tmp, 'src'), { recursive: true });
      writeFileSync(join(tmp, 'package.json'), '{"type":"module"}\n');
  ...
  it('/manual-edit-discard cancels leased chat Apply events instead of redelivering them', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'pictorial-manual-discard-apply-'));
  ...
  it('/manual-edit-commit rolls back abandoned chunk transactions after server restart', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'pictorial-manual-abandoned-transaction-'));
    let abandonedServer;
    let restarted;
    try {
      mkdirSync(join(tmp, 'src'), { recursive: true });
  ...
  it('/manual-edit-discard only cancels in-flight Apply events for the discarded page', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'pictorial-manual-discard-page-scope-'));
    let pageScopeServer;
    try {
      mkdirSync(join(tmp, 'src'), { recursive: true });
  ...
  it('/manual-edit-discard returns discarded entries so the browser can restore visible text', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'pictorial-manual-discard-server-'));
    let discardServer;
    try {
  ...
  it('/manual-edit-stash rejects a corrupt pending buffer instead of overwriting it', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'pictorial-manual-stash-corrupt-'));
    let stashServer;
    try {
  ...
  it('streams Svelte component checkpoints as progressive preview updates', async () => {
    ...
    const res = await fetch(`http://localhost:${server.port}/events`, {
      ...
      body: JSON.stringify({
        ...
        previewFile: 'node_modules/.pictorial-live/a1b2c3de/manifest.json',
  ...
  it('redelivers an unacknowledged browser event after helper server restart', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'pictorial-server-restart-'));
    let firstServer;
    let restarted;
    try {
  ...
  it('/source reads project files with valid token', async () => {
    ...
    assert.equal(res.status, 200);
    const text = await res.text();
    assert.ok(text.includes('"pictorial"'));
  });
  ...
  it('journals variant_mounted without handing it to the agent', async () => {
    ...
    const id = 'bb11cc22';
    await postEvent({
      type: 'generate', id, action: 'pictorial', count: 2, pageUrl: '/',
  ...
  it('rebroadcasts done over SSE so the browser re-runs its injection', async () => {
    ...
    const id = 'dd55ee66';
    await postEvent({
      type: 'generate', id, action: 'pictorial', count: 1, pageUrl: '/',
      element: { outerHTML: '<section>Hero</section>', tagName: 'SECTION' },
    });
    const leased = await (await fetch(
      `http://localhost:${server.port}/poll?token=${server.token}&timeout=500&leaseMs=60000`,
  ...
  it('reports the browser summary fields a storage-less page needs to rehydrate', async () => {
    ...
    const id = 'ee77ff88';
    await postEvent({
      type: 'generate', id, action: 'pictorial', count: 3, pageUrl: '/pricing',
...
describe('Svelte component preview tree cleanup', () => {
  ...
  function seed(sessionIds) {
    const root = join(tmp, 'node_modules', '.pictorial-live');
  ...
  before(() => {
    tmp = realpathSync(mkdtempSync(join(tmpdir(), 'pictorial-sweep-')));
  });
  ...
  it('removeAllSvelteComponentSessions takes the runtime shim and the parent dir with it', () => {
    ...
    assert.equal(existsSync(root), false, 'the .pictorial-live parent dir must not survive a full sweep');
  });
  ...
  it('sweepInactiveSvelteComponentSessions clears the whole tree when nothing is active', () => {
    ...
    assert.equal(existsSync(root), false, 'the .pictorial-live parent dir must be gone');
  });

  it('both sweeps are no-ops when the tree was never created', () => {
    const clean = realpathSync(mkdtempSync(join(tmpdir(), 'pictorial-sweep-empty-')));
    try {
