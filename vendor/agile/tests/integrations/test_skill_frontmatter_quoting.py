...
from agile_cli.integrations.base import yaml_quote
...
class TestYamlQuote:
    def test_simple_value_keeps_plain_double_quoted_form(self):
        assert yaml_quote("agile-plan") == '"agile-plan"'
...
class TestSkillFrontmatterEmbeddedDashes:
    ...
    def test_dashed_description_is_preserved(self, tmp_path, monkeypatch):
        ...
        # (or drops it entirely, falling back to "Agile: plan workflow").
...
class TestKimiGeneratedSkillDetection:
    """``_is_agile_generated_skill`` must survive a ``---`` in a value.

    Teardown only removes a legacy skill directory it recognizes as
    Agile-generated via the frontmatter ``metadata`` block. A substring split
    ...
    def test_detects_skill_with_dashes_in_description(self, tmp_path):
        from agile_cli.integrations.kimi import _is_agile_generated_skill

        skill_dir = tmp_path / "agile-plan"
        self._write_skill(skill_dir, "Separate sections with --- markers")
        assert _is_agile_generated_skill(skill_dir) is True
