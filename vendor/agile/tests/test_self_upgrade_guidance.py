...
class TestSourceCheckout:
    ...
    def test_source_checkout_prints_git_pull_guidance(
        ...
        with patch("agile_cli._version._editable_marker_seen", return_value=True), patch(
            "agile_cli._version._source_checkout_path", return_value=fake_tree
        ), patch("agile_cli.authentication.http.urllib.request.urlopen") as mock_urlopen, patch(
            "agile_cli._version.subprocess.run"
        ) as mock_run:
            mock_urlopen.return_value = mock_urlopen_response({"tag_name": "v0.7.6"})
            result = runner.invoke(app, ["self", "upgrade"])
    ...
    def test_source_checkout_without_path_mentions_checkout_directory(
        ...
        with patch("agile_cli._version._editable_marker_seen", return_value=True), patch(
            "agile_cli._version._source_checkout_path", return_value=None
        ), patch("agile_cli.authentication.http.urllib.request.urlopen") as mock_urlopen, patch(
            "agile_cli._version.subprocess.run"
        ) as mock_run:
            mock_urlopen.return_value = mock_urlopen_response({"tag_name": "v0.7.6"})
...
class TestUnsupported:
    ...
    def test_unsupported_prints_both_reinstall_commands(
        ...
        with patch("agile_cli._version._editable_marker_seen", return_value=False), patch(
        ...
        assert (
            ...
            "git+https://github.com/TheHalfMoon/wepld.git@vX.Y.Z"
        ) in out
        assert (
...
class TestDryRunNonUpgradablePaths:
    ...
    def test_dry_run_on_uvx_ephemeral_emits_guidance_not_preview(
        ...
        with patch("agile_cli.authentication.http.urllib.request.urlopen") as mock_urlopen:
            mock_urlopen.return_value = mock_urlopen_response({"tag_name": "v0.7.6"})
    ...
    def test_dry_run_on_unsupported_emits_manual_commands(
        ...
        with patch("agile_cli._version._editable_marker_seen", return_value=False), patch(
            "agile_cli._version.shutil.which", return_value=None
        ), patch("agile_cli.authentication.http.urllib.request.urlopen") as mock_urlopen:
            mock_urlopen.return_value = mock_urlopen_response({"tag_name": "v0.7.6"})
            result = runner.invoke(app, ["self", "upgrade", "--dry-run"])
        assert result.exit_code == 0
