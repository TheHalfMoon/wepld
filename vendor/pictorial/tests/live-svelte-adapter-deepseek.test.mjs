  ...
  it('covers live chrome, replace, edit, annotations, insert, refresh recovery, accept/discard, and exit', async (t) => {
    ...
    try {
      ...
      const afterExit = await page.evaluate(() => ({
        ...
        hasGlobalBar: Boolean(window.__pictorialLiveQuery?.('#pictorial-live-global-bar')),
      }));
...
function loadConsolidatedFixture() {
  ...
  fixture.runtime = {
    ...
    stateProbe: {
      ...
      windowProperty: '__pictorialStatefulMounts',
    ...
    steer: {
      ...
      expectSelector: 'h1.hero-title[data-pictorial-steer="e2e"]',
      expectSourceContains: 'data-pictorial-steer="e2e"',
    },
    probe: {
      expectLiveInit: true,
...
async function runReplaceDiscardRecoveryFlow({ page, tmp, evidence }) {
  ...
  assert.match(session.previewFile, /^node_modules\/\.pictorial-live\/[^/]+\/manifest\.json$/);
...
async function runEditCopyFlow({ page, tmp, live, evidence }) {
  ...
  assert.doesNotMatch(sourceAfter, /contenteditable|data-pictorial-editable|data-pictorial-original-text|pictorial-variants/, 'manual Apply does not leak live scaffolding');
}
...
async function runAcceptReplaceFlow({ page, tmp, evidence }) {
  ...
  assert.doesNotMatch(source, /data-pictorial-variants|pictorial-variants-start|data-pictorial-variant=/, 'accepted replace source is clean');
...
async function runInsertFlowWithRecovery({ page, tmp, evidence }) {
  ...
  assert.doesNotMatch(source, /data-pictorial-variants|pictorial-variants-start|data-pictorial-variant=/, 'accepted insert source is clean');
...
async function assertSvelteAdapterInjection(tmp) {
  ...
  assert.doesNotMatch(appHtml, /live\.js|pictorial-live-start/, 'Svelte adapter leaves src/app.html untouched');
  assert.match(layout, /PictorialLiveRoot|pictorial-live-svelte-start/, 'Svelte adapter patches layout with live root');
}

async function assertShadowChrome(page) {
  const result = await page.evaluate(() => {
    const host = document.getElementById('pictorial-live-root');
    const root = host?.shadowRoot;
    return {
      hasHost: Boolean(host),
      hasShadowRoot: Boolean(root),
      hasGlobalBar: Boolean(root?.getElementById('pictorial-live-global-bar')),
      documentHasGlobalBar: Boolean(document.querySelector('#pictorial-live-global-bar')),
    };
  });
...
async function assertHostileCssIsolation(page) {
  ...
  const hostileStyle = await page.addStyleTag({
    content: `
      button, div, input, svg, #pictorial-live-root, pictorial-live-root * {
...
async function waitForGlobalBarStable(page) {
  ...
  await page.waitForFunction(
    () => {
      const bar = window.__pictorialLiveQuery?.('#pictorial-live-global-bar');
      if (!bar) return false;
...
async function assertSelectedElementChrome(page) {
  ...
  const result = await page.evaluate(() => {
    ...
    const bar = q('#pictorial-live-bar');
    const picker = q('#pictorial-live-picker');
    const badge = q('#pictorial-live-edit-badge');
    const highlight = q('#pictorial-live-highlight');
    const input = q('#pictorial-live-input');
    return {
      hasBar: Boolean(bar),
...
async function selectAction(page, label) {
  ...
  await page.waitForFunction(
    (expected) => {
      const picker = window.__pictorialLiveQuery('#pictorial-live-picker');
...
async function clickEditBadgeAction(page, label) {
  const clicked = await page.evaluate((expected) => {
    const badge = window.__pictorialLiveQuery('#pictorial-live-edit-badge');
...
async function clickPendingTrash(page) {
  ...
  const clicked = await page.evaluate(() => {
    const dock = window.__pictorialLiveQuery('#pictorial-live-pending-dock');
...
async function waitForSvelteAcceptComplete(tmp, session, { timeout }) {
  ...
  while (Date.now() < deadline) {
    ...
    if (
      ...
      && !/data-pictorial-variants|pictorial-variants-start|data-pictorial-variant=/.test(source)
    ) {
      return;
    }
...
function assertCompletedRealSource(tmp, id) {
  ...
  assert.doesNotMatch(snapshot.sourceFile || '', /node_modules\/\.pictorial-live/);
  assert.match(snapshot.previewFile || '', /node_modules\/\.pictorial-live\/[^/]+\/manifest\.json/);
}
...
function readSessionSnapshot(tmp, id) {
  const file = join(tmp, '.pictorial/live/sessions', `${id}.snapshot.json`);
...
async function assertNoCyclingZero(page) {
  const text = await page.evaluate(() => window.__pictorialLiveQuery('#pictorial-live-bar')?.textContent || '');
...
async function readStateProbe(page) {
  return page.evaluate(() => ({
    ...
    mounts: window.__pictorialStatefulMounts,
  }));
}
...
function assertFinalSourceClean(tmp) {
  const source = readFileSync(join(tmp, ROUTE_FILE), 'utf-8');
  assert.doesNotMatch(source, /node_modules\/\.pictorial-live|data-pictorial-variants|pictorial-variants-start|data-pictorial-variant=/);
}
...
function readJournalEvents(tmp) {
  const dir = join(tmp, '.pictorial/live/sessions');
...
function createEvidenceWriter({ page, tmp, live, consoleErrors }) {
  ...
  return {
    async capture(name) {
      ...
      try {
        const dom = await page.evaluate(() => {
          ...
          return {
            ...
            liveState: window.__PICTORIAL_LIVE_CHROME_CORE__?.debugState?.() || null,
            localSession: localStorage.getItem('pictorial-live-session'),
          };
        });
      ...
      copyDirIfExists(join(tmp, '.pictorial/live/sessions'), join(dir, 'sessions'));
...
function createArtifactRoot() {
  const explicit = process.env.PICTORIAL_SVELTE_DEEPSEEK_ARTIFACT_DIR;
