...
/**
 * Pictorial design hook — PostToolUse + Stop entry point.
 *
...
async function main() {
  ...
  process.env.PICTORIAL_HOOK_DEPTH = process.env.PICTORIAL_HOOK_DEPTH || '1';
...
main().catch((err) => {
  ...
  if (process.env.PICTORIAL_HOOK_DEBUG) {
    process.stderr.write(`[pictorial-hook] ${err}\n`);
  }
  process.exit(0);
});
