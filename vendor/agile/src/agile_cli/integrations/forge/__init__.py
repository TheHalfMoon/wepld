...
- Uses a hyphenated frontmatter `name` value (e.g., `agile-foo-bar`) for shell compatibility, especially with ZSH
"""
...
def format_forge_command_name(cmd_name: str) -> str:
    ...
    Examples:
        ...
        'agile-plan'
        >>> format_forge_command_name("agile.plan")
        'agile-plan'
        >>> format_forge_command_name("agile-plan")
        'agile-plan'
        >>> format_forge_command_name("agile.my-extension.example")
        'agile-my-extension-example'
        >>> format_forge_command_name("agile-my-extension-example")
        'agile-my-extension-example'
        >>> format_forge_command_name("agile.jira.sync-status")
        'agile-jira-sync-status'

    Args:
        cmd_name: Command name in dot notation (agile.foo.bar),
                  hyphenated format (agile-foo-bar), or plain name (foo)

    Returns:
        Hyphenated command name with 'agile-' prefix
    """
    # Already in hyphenated format - return as-is (idempotent)
    if cmd_name.startswith("agile-"):
        return cmd_name

    # Strip 'agile.' prefix if present
    short_name = cmd_name
...
class ForgeIntegration(MarkdownIntegration):
    ...
    def build_command_invocation(self, command_name: str, args: str = "") -> str:
        ...
        builds the dotted ``/agile.<name>``, which references a command Forge
