...
from agile_cli.integrations.cursor_agent import CursorAgentIntegration
from agile_cli.integrations.opencode import OpencodeIntegration
...
class TestResolveEvents:
    ...
    def test_layer1_disabled_returns_empty(self, tmp_path):
        ...
        result = resolve_events(
            "claude",
            {"events": {"post_tool_use": {"command": "agile.tdd.validate"}}},
            tmp_path,
            {"events": "false"},
        )
        assert result == {}
    ...
    def test_layer3_extension_events_appended(self, tmp_path):
        ...
        assert result["session_start"] == [{"command": "agile.my-ext.boot"}]
    ...
    def test_layer3_multiple_extensions_same_event_accumulate(self, tmp_path):
        ...
        for ext_id, cmd in (("my-ext", "agile.my-ext.boot"), ("other-ext", "agile.other.boot")):
            ext_dir = tmp_path / ".agile" / "extensions" / ext_id
            ext_dir.mkdir(parents=True)
    ...
    def test_layer2_yaml_override_replaces(self, tmp_path):
        ...
        assert result == {"stop": [{"command": "agile.override.stop"}]}
    ...
    def test_unreadable_yaml_override_keeps_prior_layers(self, tmp_path):
        ...
        assert result == {
            "post_tool_use": [{"command": "agile.tdd.validate"}]
        }
...
class TestCollectExtensionEvents:
    ...
    def test_events_collected(self, tmp_path):
        ...
        ext_dir.mkdir(parents=True)
        (ext_dir / "extension.yml").write_text(
            "events:\n  pre_tool_use:\n    command: agile.my-ext.check\n",
            encoding="utf-8",
        )
        result = collect_extension_events(tmp_path)
        assert result == {"pre_tool_use": [{"command": "agile.my-ext.check"}]}
    ...
    def test_event_command_ref_canonicalized_via_manifest(self, tmp_path):
        ...
        (agile.my-ext.boot) the same way hook refs are at install."""
        ...
        # the ref to agile.my-ext.boot (C11).
        (ext_dir / "extension.yml").write_text(
            _yaml.dump({
                "schema_version": "1.0",
                "extension": {
                ...
                "requires": {"agile_version": ">=0.1"},
                "provides": {
                    "commands": [
                        {"name": "agile.my-ext.boot", "file": "commands/boot.md"}
                    ]
                },
        ...
        ExtensionRegistry(tmp_path / ".agile" / "extensions").add(
            "my-ext", {"enabled": True}
        )

        result = collect_extension_events(tmp_path)
        # The ref was canonicalized to agile.my-ext.boot by the validated
...
class TestCanonicalEventMapping:
    ...
    def test_tabnine_mapping_includes_before_agent(self):
        ...
        from agile_cli.integrations.tabnine import TabnineIntegration
...
class TestEventCapableOptionsComposition:
    ...
    def test_devin_declares_events(self):
        ...
        # is declared alongside --skills.
        from agile_cli.integrations.devin import DevinIntegration
...
class TestValidateEvents:
    ...
    def test_unknown_event_rejected(self):
        from agile_cli.extensions import ValidationError
        data = {"events": {"unknown_event": {"command": "agile.tdd.validate"}}}
        with pytest.raises(ValidationError) as exc:
            validate_events(data)
        assert "Unknown event" in str(exc.value)

    def test_known_event_accepted(self):
        data = {"events": {"pre_tool_use": {"command": "agile.tdd.validate"}}}
        validate_events(data)  # no raise

    def test_all_canonical_events_accepted(self):
        data = {
            "events": {
                name: {"command": "agile.test"}
...
class TestClaudeJsonMerging:
    ...
    def test_merge_into_empty_file(self, tmp_path):
        ...
        events = {
            "pre_tool_use": [{"command": "agile.tdd.validate", "matcher": "Edit|Write"}],
        }
        ...
        assert "agile.tdd.validate" in inner["command"]
        ...
        assert '"${CLAUDE_PROJECT_DIR}/.agile/events.py"' in inner["command"]
    ...
    def test_claude_emits_all_handlers_for_same_event(self, tmp_path):
        ...
        events = {
            "pre_tool_use": [
                {"command": "agile.tdd.validate"},
                {"command": "agile.other.check"},
            ],
        }
        install_integration_events(integration, tmp_path, manifest, events)
        ...
        assert any("agile.tdd.validate" in c for c in commands)
        assert any("agile.other.check" in c for c in commands)
    ...
    def test_remove_preserves_user_hooks(self, tmp_path):
        ...
        events = {
            "pre_tool_use": [{"command": "agile.tdd.validate"}],
        }
        install_integration_events(integration, tmp_path, manifest, events)
...
class TestCopilotJsonWriting:
    """Test Copilot dedicated .github/hooks/agile.json generation."""
    ...
    def test_copilot_json_generation(self, tmp_path):
        ...
        events = {
            "session_start": [{"command": "agile.agent-context.update", "timeout": 60}],
        }
        install_integration_events(integration, tmp_path, manifest, events)

        config_path = tmp_path / ".github/hooks/agile.json"
        assert config_path.is_file()
        data = json.loads(config_path.read_text())
        ...
        assert "agile.agent-context.update" in entry["bash"]
        ...
        assert "agile.agent-context.update" in entry["powershell"]
...
class TestCursorJsonWriting:
    ...
    def test_cursor_json_includes_version(self, tmp_path):
        ...
        install_integration_events(
            ...
            {"session_start": [{"command": "agile.boot"}]},
        )
    ...
    def test_nested_matcher_grouping_per_distinct_matcher(self, tmp_path):
        ...
        events = {
            "pre_tool_use": [
                {"command": "agile.first", "matcher": "Edit"},
                {"command": "agile.second", "matcher": "Bash"},
            ],
        }
        install_integration_events(integration, tmp_path, manifest, events)
        data = json.loads((tmp_path / ".claude/settings.json").read_text())
        ...
        assert "agile.first" in by_matcher["Edit"][0]["command"]
        assert len(by_matcher["Bash"]) == 1
        assert "agile.second" in by_matcher["Bash"][0]["command"]
    ...
    def test_nested_shared_matcher_stays_one_group(self, tmp_path):
        ...
        events = {
            "pre_tool_use": [
                {"command": "agile.first", "matcher": "Edit"},
                {"command": "agile.second", "matcher": "Edit"},
            ],
        }
        install_integration_events(integration, tmp_path, manifest, events)
        data = json.loads((tmp_path / ".claude/settings.json").read_text())
...
class TestGeminiTimeoutUnit:
    ...
    def test_gemini_timeout_converted_to_ms(self, tmp_path):
        from agile_cli.integrations.gemini import GeminiIntegration
        from agile_cli.events import _native_timeout
    ...
    def test_qwen_timeout_converted_to_ms(self):
        ...
        from agile_cli.integrations.qwen import QwenIntegration
        from agile_cli.events import _native_timeout
...
class TestContextInjectionEnvelopes:
    ...
    def test_emit_event_stdout_wrapping(self, capsys):
        from agile_cli.events import _emit_event_stdout
    ...
    def test_envelope_resolution_and_command_formatting(self):
        from agile_cli.events import _dispatcher_command, _context_envelope_for
        from agile_cli.integrations.gemini import GeminiIntegration
        from agile_cli.integrations.qwen import QwenIntegration
        from agile_cli.integrations.copilot import CopilotIntegration
        from agile_cli.integrations.cursor_agent import CursorAgentIntegration
        ...
        # alignment (R3).
        cmd_gemini_start = _dispatcher_command(gemini, Path("/proj"), "agile.boot", "session_start")
        assert cmd_gemini_start.endswith(" 60 hookSpecificOutput SessionStart")

        cmd_gemini_prompt = _dispatcher_command(gemini, Path("/proj"), "agile.prompt", "user_prompt_submit")
        assert cmd_gemini_prompt.endswith(" 60 hookSpecificOutput BeforeAgent")

        cmd_gemini_tool = _dispatcher_command(gemini, Path("/proj"), "agile.guard", "pre_tool_use")
        ...
        cmd_qwen_start = _dispatcher_command(qwen, Path("/proj"), "agile.boot", "session_start")
        assert cmd_qwen_start.endswith(" 60 hookSpecificOutput SessionStart")
        cmd_qwen_prompt = _dispatcher_command(qwen, Path("/proj"), "agile.prompt", "user_prompt_submit")
        ...
        cmd_copilot_start = _dispatcher_command(copilot, Path("/proj"), "agile.boot", "session_start")
        assert cmd_copilot_start.endswith(" 60 additionalContext")
        cmd_copilot_prompt = _dispatcher_command(copilot, Path("/proj"), "agile.prompt", "user_prompt_submit")
        ...
        cmd_cursor_start = _dispatcher_command(cursor, Path("/proj"), "agile.boot", "session_start")
...
class TestDispatcherCommandQuoting:
    ...
    def test_command_metacharacters_are_quoted_posix(self, tmp_path):
        from agile_cli.events import _dispatcher_command

        cmd = _dispatcher_command(
            ClaudeIntegration(), tmp_path, "agile.x; rm -rf /", "pre_tool_use",
            target_os="posix",
        )
        # The metacharacter-bearing command is single-quoted as one argument.
        assert "'agile.x; rm -rf /'" in cmd
    ...
    def test_interpreter_with_space_is_quoted_posix(self, tmp_path):
        ...
        proj = tmp_path
        cmd = _dispatcher_command(
            ClaudeIntegration(), proj, "agile.x.y", "stop", target_os="host",
        )
    ...
    def test_claude_dispatcher_double_quoted_for_spaces(self, tmp_path):
        ...
        from agile_cli.events import _dispatcher_command
        cmd = _dispatcher_command(
            ClaudeIntegration(), tmp_path, "agile.x.y", "stop", target_os="host",
        )
        assert '"${CLAUDE_PROJECT_DIR}/.agile/events.py"' in cmd
...
class TestTomlMatcherEscaping:
    ...
    def test_matcher_with_quote_stays_valid_toml(self, tmp_path):
        ...
        install_integration_events(
            integration, tmp_path, manifest,
            {"pre_tool_use": [{"command": "agile.x.y", "matcher": 'Ba"sh'}]},
        )
...
class TestOpencodePluginMerging:
    ...
    def test_opencode_ts_plugin_generation(self, tmp_path):
        ...
        events = {
            "pre_tool_use": [{"command": "agile.tdd.validate", "matcher": "Edit"}],
            "session_start": [{"command": "agile.agent-context.update"}],
            "session_end": [{"command": "agile.agent-context.teardown"}],
        }
        install_integration_events(integration, tmp_path, manifest, events)

        plugin_path = tmp_path / ".opencode/plugin/agile-events.ts"
        ...
        assert "agile.tdd.validate" in content
        assert "agile.agent-context.update" in content
    ...
    def test_opencode_ts_plugin_chat_message_part_injection(self, tmp_path):
        ...
        events = {
            "user_prompt_submit": [{"command": "agile.discover"}],
        }
        install_integration_events(integration, tmp_path, manifest, events)
    ...
    def test_opencode_ts_plugin_resolves_interpreter_and_directory_at_load(self, tmp_path):
        ...
        events = {"session_start": [{"command": "agile.boot"}]}
        install_integration_events(integration, tmp_path, manifest, events)
        content = (tmp_path / ".opencode/plugin/agile-events.ts").read_text()
        ...
        # R2: venv interpreter is probed for agile_cli importability before
    ...
    def test_opencode_ts_plugin_emits_all_handlers(self, tmp_path):
        ...
        events = {
            "session_start": [
                {"command": "agile.first.boot"},
                {"command": "agile.second.boot"},
            ],
        }
        install_integration_events(integration, tmp_path, manifest, events)
        content = (tmp_path / ".opencode/plugin/agile-events.ts").read_text()
        assert "agile.first.boot" in content
        assert "agile.second.boot" in content
    ...
    def test_opencode_ts_plugin_forwards_output(self, tmp_path):
        ...
        events = {
            "pre_tool_use": [{"command": "agile.tdd.validate", "matcher": "Edit"}],
            "post_tool_use": [{"command": "agile.tdd.after"}],
        }
        install_integration_events(integration, tmp_path, manifest, events)
        content = (tmp_path / ".opencode/plugin/agile-events.ts").read_text()
        ...
        assert 'runEvent("agile.tdd.validate", "pre_tool_use", input, output, 60)' in content
        assert 'runEvent("agile.tdd.after", "post_tool_use", input, output, 60)' in content
    ...
    def test_opencode_ts_plugin_escapes_metacharacters(self, tmp_path):
        ...
        events = {
            "pre_tool_use": [{"command": "agile.x'y`code", "matcher": "Ed'it"}],
        }
        install_integration_events(integration, tmp_path, manifest, events)
        content = (tmp_path / ".opencode/plugin/agile-events.ts").read_text()
        ...
        assert json.dumps("agile.x'y`code") in content
        # The dangerous single-quoted form (runEvent('agile.x'y...')) —
        # where the embedded quote would terminate the literal — is absent.
        assert "runEvent('agile.x" not in content
...
class TestCommandRunner:
    ...
    def test_extension_command_resolves_when_file_stem_differs(self, tmp_path):
        ...
        from agile_cli.events import _find_command_template
        from agile_cli.extensions import ExtensionRegistry
        ...
        # Command name is agile.selftest.extension but the file is selftest.md.
        (ext_dir / "extension.yml").write_text(
            "schema_version: '1.0'\n"
            "extension:\n"
            "  id: selftest\n"
        ...
        template, resolved_ext = _find_command_template(
            "agile.selftest.extension", tmp_path
        )
    ...
    def test_disabled_extension_command_not_resolved(self, tmp_path):
        ...
        (ext_dir / "extension.yml").write_text(
            _yaml.dump({
                ...
                "extension": {"id": "my-ext", "name": "My Ext", "version": "1.0.0",
                              "description": "test"},
                "requires": {"agile_version": ">=0.1"},
                "provides": {"commands": [{"name": "agile.my-ext.boot",
                                           "file": "commands/boot.md"}]},
                "events": {"session_start": {"command": "agile.my-ext.boot"}},
            }),
            encoding="utf-8",
        )
        ...
        template, _ = _find_command_template("agile.my-ext.boot", tmp_path)
        ...
        (cmds_dir / "agile.my-ext.boot.md").write_text("---\ndescription: \"x\"\n---\nBody\n", encoding="utf-8")
        template, _ = _find_command_template("agile.my-ext.boot", tmp_path)
    ...
    def test_run_command_resolves_and_executes(self, tmp_path):
        ...
        cmd_dir = tmp_path / ".agile" / "templates" / "commands"
        ...
        script_dir = tmp_path / ".agile" / "scripts"
        ...
        code = resolve_and_run_event_command("agile.test", "session_start", "{}", tmp_path)
        assert code == 0

    def test_py_variant_anchored_under_specify(self, tmp_path):
        """S2: the py variant resolves scripts/... under .agile/, not the
        project root, and prepends the resolved interpreter."""
        from agile_cli.events import _resolve_event_command_argv
        ...
        py_dir = tmp_path / ".agile" / "scripts" / "python"
        py_dir.mkdir(parents=True)
        (py_dir / "boot.py").write_text("import sys; sys.exit(0)\n", encoding="utf-8")

        argv = _resolve_event_command_argv(cmd_dir / "boot.md", tmp_path, None)
        assert argv is not None
        # Interpreter first, then the .agile-anchored script path. Compare in
        # POSIX form so the assertion holds on Windows (backslash paths) too.
        assert len(argv) >= 2
        assert PurePath(argv[1]).as_posix().endswith(".agile/scripts/python/boot.py")
        assert ".agile" in argv[1]
    ...
    def test_ps_variant_prefixed_with_powershell_launcher(self, tmp_path):
        ...
        ps_dir = tmp_path / ".agile" / "scripts" / "powershell"
        ps_dir.mkdir(parents=True)
        (ps_dir / "boot.ps1").write_text("exit 0\n", encoding="utf-8")

        argv = _resolve_event_command_argv(cmd_dir / "boot.md", tmp_path, None)
        assert argv is not None
        # Launcher (pwsh or powershell), -File, then the .agile-anchored
        ...
        assert argv[1] == "-File"
        assert PurePath(argv[2]).as_posix().endswith(".agile/scripts/powershell/boot.ps1")
    ...
    def test_run_command_executes_with_project_root_cwd(self, tmp_path):
        ...
        try:
            _os.chdir(subdir)
            code = resolve_and_run_event_command("agile.cwd", "session_start", "{}", tmp_path)
        finally:
            _os.chdir(prev)
        assert code == 0
    ...
    def test_dispatcher_is_self_contained(self, tmp_path):
        """R1: the generated dispatcher prefers `import agile_cli` (durable
        install) and falls back to an inline stdlib resolver so it works
        without a persistent `agile` executable (e.g. one-time uvx)."""
        integration = ClaudeIntegration()
        manifest = MagicMock(spec=IntegrationManifest)
        manifest.files = {}
        manifest.record_file = MagicMock()
        ...
        # Delegates to agile_cli when importable and confinement is present.
        assert "EVENT_SCRIPT_PATH_CONFINEMENT" in content
        assert "from agile_cli.events import" in content
        ...
        # No dependency on a persistent `agile` executable.
        assert '["agile"]' not in content

    def test_dispatcher_inline_fallback_runs_script(self, tmp_path):
        """R1: with agile_cli.events NOT importable, the inline resolver
        ...
        # Force the inline fallback: shadow `agile_cli` with an empty package
        # (no `events` submodule) so `from agile_cli.events import ...` raises
        ...
        (fake_dir / "agile_cli").mkdir(parents=True)
        (fake_dir / "agile_cli" / "__init__.py").write_text("", encoding="utf-8")
        env = dict(os.environ)
        env["PYTHONPATH"] = str(fake_dir)
        result = _sp.run(
            [_sys.executable, str(dispatcher), "agile.boot", "session_start", "60"],
            input='{"tool_name":"x"}',
            capture_output=True,
            text=True,
            env=env,
    ...
    def test_dispatcher_ignores_stale_agile_cli_without_confinement(self, tmp_path):
        """A generated dispatcher must not delegate to an older agile_cli
    ...
    def test_dispatcher_threads_per_handler_timeout(self, tmp_path):
        ...
        install_integration_events(
            integration, tmp_path, manifest,
            {"pre_tool_use": [{"command": "agile.tdd.validate", "timeout": 300}]},
        )
        content = (tmp_path / EVENTS_DISPATCHER_REL).read_text()
    ...
    def test_native_command_carries_resolved_timeout(self, tmp_path):
        ...
        from agile_cli.events import _dispatcher_command
        cmd = _dispatcher_command(
            ClaudeIntegration(), tmp_path, "agile.x.y", "stop",
            timeout_seconds=300,
        )
    ...
    def test_dispatcher_timeout_not_unit_converted_for_ms_adapters(self, tmp_path):
        ...
        from agile_cli.integrations.gemini import GeminiIntegration
        cmd = _dispatcher_command(
            GeminiIntegration(), tmp_path, "agile.x.y", "pre_tool_use",
            timeout_seconds=60,
        )
    ...
    def test_sh_variant_uses_launcher_on_windows(self, tmp_path):
        ...
        sh_dir = tmp_path / ".agile" / "scripts" / "bash"
        ...
        if platform.system().lower().startswith("win"):
            ...
            assert PurePath(argv[1]).as_posix().endswith(".agile/scripts/bash/boot.sh")
        else:
            assert PurePath(argv[0]).as_posix().endswith(".agile/scripts/bash/boot.sh")
    ...
    def test_extension_dotdot_to_core_scripts_resolves(self, tmp_path):
        ...
        ext_id = "my-ext"
        cmd_dir = tmp_path / ".agile" / "extensions" / ext_id / "commands"
        cmd_dir.mkdir(parents=True)
        (cmd_dir / "boot.md").write_text(
            "---\n"
            "description: \"Boot\"\n"
        ...
        helper_dir = tmp_path / ".agile" / "scripts" / "bash"
        ...
        assert PurePath(script_arg).as_posix().endswith(".agile/scripts/bash/helper.sh")
    ...
    def test_dispatcher_template_confines_script_token(self):
        ...
        from agile_cli.events import _EVENTS_DISPATCHER_TEMPLATE
...
class TestMergeIdempotency:
    ...
    def test_upgrade_does_not_duplicate_nested_hooks(self, tmp_path):
        ...
        events = {"pre_tool_use": [{"command": "agile.tdd.validate"}]}
        for _ in range(2):
            manifest = _claude_manifest(tmp_path)
            install_integration_events(integration, tmp_path, manifest, events)
        ...
        assert len(inner) == 1
        assert "agile.tdd.validate" in inner[0]["command"]
...
class TestTeardownDataSafety:
    """#14/#22/#23: preserve user content, delete Agile-created empties."""

    def test_remove_deletes_agile_created_config(self, tmp_path):
        """#14: a config Agile created from scratch is deleted (not left as
...
class TestCopilotMergeTeardown:
    ...
    def test_copilot_merge_preserves_user_hooks(self, tmp_path):
        ...
        assert any("agile.boot" in c for c in bash_cmds)
    ...
    def test_copilot_teardown_removes_only_owned_entries(self, tmp_path):
        ...
        data = json.loads(config_path.read_text())
        # User hook preserved; Agile entry gone.
        assert data["hooks"]["sessionStart"][0]["bash"] == "user-hook"

    def test_copilot_teardown_deletes_agile_only_file(self, tmp_path):
        """#8/#14: when the file held only Agile entries, teardown deletes it."""
        integration = CopilotIntegration()
        manifest = _claude_manifest(tmp_path)
        install_integration_events(
            integration, tmp_path, manifest,
...
class TestSharedDispatcherRefcount:
    """#10: the shared .agile/events.py dispatcher is not deleted while
    ...
    def test_dispatcher_kept_when_other_integration_references_it(self, tmp_path):
        ...
        install_integration_events(
            ...
            {"pre_tool_use": [{"command": "agile.codex.check"}]},
        )
        ...
        state_path = tmp_path / ".agile" / "integration.json"
...
class TestSafeWriteDestination:
    ...
    def test_plugin_remover_rejects_symlinked_plugin(self, tmp_path):
        ...
        manifest.files[".opencode/plugin/agile-events.ts"] = "hash"
        with pytest.raises(ValueError, match="(?i)symlink|escapes|outside"):
            remove_integration_events(integration, tmp_path, manifest)
...
class TestRefreshIntegrationEvents:
    ...
    def test_refresh_strips_removed_extension_events(self, tmp_path):
        from agile_cli.events import refresh_integration_events
        from agile_cli.integrations.manifest import IntegrationManifest
        ...
        install_integration_events(
            integration, tmp_path, manifest,
            {"pre_tool_use": [{"command": "agile.my-ext.check"}]},
        )
        manifest.save()
        config_path = tmp_path / ".claude/settings.json"
        assert config_path.is_file()
...
class TestMatcherValidation:
    ...
    def test_non_string_matcher_rejected_in_manifest(self):
        from agile_cli.extensions import ValidationError
        data = {"events": {"pre_tool_use": {"command": "agile.x.y", "matcher": []}}}
...
class TestEventCommandRefCanonicalization:
    ...
    def test_event_command_ref_lifted_to_canonical(self, tmp_path):
        ...
        assert manifest.data["events"]["session_start"]["command"] == "agile.my-ext.boot"
        assert any(
...
class TestDispatcherManifestClaimDroppedOnRetain:
    ...
    def test_empty_map_upgrade_deletes_dispatcher_for_last_integration(self, tmp_path):
        ...
        install_integration_events(claude, tmp_path, cm, {"pre_tool_use": [{"command": "agile.x"}]})
        cm.save()
        xm = IntegrationManifest(codex.key, tmp_path, version="test")
        install_integration_events(codex, tmp_path, xm, {"pre_tool_use": [{"command": "agile.y"}]})
        xm.save()
        state_path = tmp_path / ".agile" / "integration.json"
        state_path.parent.mkdir(parents=True, exist_ok=True)
        state_path.write_text(json.dumps({
    ...
    def test_fresh_manifest_upgrade_deletes_dispatcher_when_last(self, tmp_path):
        ...
        install_integration_events(claude, tmp_path, old, {"pre_tool_use": [{"command": "agile.x"}]})
        old.save()
        state_path = tmp_path / ".agile" / "integration.json"
        state_path.parent.mkdir(parents=True, exist_ok=True)
        state_path.write_text(json.dumps({
...
class TestDispatcherStaleExclusion:
    ...
    def test_dispatcher_in_stale_exclusions(self):
        from agile_cli.events import events_stale_exclusions
...
class TestCursorVersionOnlyStubDeletion:
    """C5: a Agile-created Cursor file retaining only {"version": 1} after
    ...
    def test_version_only_cursor_file_deleted_on_teardown(self, tmp_path):
        ...
        remove_integration_events(integration, tmp_path, manifest)
        # No user content remained (only the Agile-managed version field) →
...
class TestNonDestructiveRefresh:
    ...
    def test_refresh_failure_preserves_existing_config(self, tmp_path):
        ...
        from agile_cli.events import EventRefreshError
        with patch(
            "agile_cli.events.install_integration_events",
