...
//   New installs write project config at .pictorial/live/config.json instead.
...
// bundled engine. `cli/engine/cli/main.mjs` imports `../../lib/pictorial-config.mjs`,
// which in the source CLI resolves to `cli/lib/pictorial-config.mjs`. The detector
...
// `scripts/lib/pictorial-config.mjs`. Copy the dependency there or the bundled
// detector fails at import time with "Cannot find module .../lib/pictorial-config.mjs".
const DETECTOR_EXTERNAL_DEPS = [
  { src: 'cli/lib/pictorial-config.mjs', dest: 'lib/pictorial-config.mjs' },
];
...
/**
 * Read and parse the pictorial skill source.
 ...
 * through to a compiled harness dir (`.agents/skills/pictorial`) instead.
 */
export function readSourceFiles(rootDir) {
  ...
  skills.push({
    name: frontmatter.name || 'pictorial',
...
const EXCLUDED_FROM_SUGGESTIONS = new Set([
  'pictorial',               // foundational skill, not a steering command
  'teach-pictorial',         // deprecated shim
  'frontend-design',          // deprecated shim
]);

// Sub-commands of /pictorial that should appear in {{available_commands}}.
// These are the commands that audit/critique/etc. reference when suggesting next steps.
const PICTORIAL_SUB_COMMANDS = [
...
export function replacePlaceholders(content, provider, commandNames = [], allSkillNames = []) {
  ...
  // After the v3.0 consolidation, commands are sub-commands of /pictorial.
  // If there's only one user-invocable skill (pictorial), generate sub-command references.
  ...
  if (nonExcluded.length === 0) {
    // Single-skill architecture: list sub-commands as /pictorial <sub>
    commandList = PICTORIAL_SUB_COMMANDS
      .map(n => `${cmdPrefix}pictorial ${n}`)
      .join(', ');
  } else {
  ...
  // outside a path or URL so `.github/hooks/pictorial.json` and
  // `.codex/skills/pictorial` remain untouched.
...
export function replaceScriptProviderMarker(content, provider, buildProvider = provider) {
  ...
  const prefixMarker = "export const PICTORIAL_COMMAND_PREFIX = '/'; // @pictorial-provider-command-prefix";
  const providerMarker = "export const PICTORIAL_PROVIDER_ID = 'source'; // @pictorial-provider-id";
  return content
    .replace(prefixMarker, `export const PICTORIAL_COMMAND_PREFIX = ${JSON.stringify(commandPrefix)};`)
    .replace(providerMarker, `export const PICTORIAL_PROVIDER_ID = ${JSON.stringify(buildProvider)};`);
}
