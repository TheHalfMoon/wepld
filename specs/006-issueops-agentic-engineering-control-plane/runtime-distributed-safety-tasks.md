...
- [ ] `006-RT-S3-004` Add negative fixtures for replayed events, duplicate delivery, out-of-order terminal events, conflicting runtime histories, absent/forged/stale event authenticity, edited payload/kind/scope, revoked producer incarnations and attempted promotion of inert observations; invalid consequential events produce zero state transition. Cover non-trigger ownership/execution/result/authority-adjacent events with missing logical identity, separate delivery IDs for the same transition, restart/incarnation changes, identity/payload conflicts and expired dedupe history; prove zero duplicate transition/effect and explicit quarantine when identity cannot be established.
