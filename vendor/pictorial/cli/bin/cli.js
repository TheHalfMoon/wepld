...
async function main() {
  ...
  if (!command || command === '--help' || command === '-h') {
    console.log(`Usage: pictorial <command> [options]

Commands:
  detect [file-or-dir-or-url...]   Scan for UI anti-patterns and design quality issues
  ignores                          Manage detector ignore rules, files, and values
  help                             List all available skills and commands
  install                          Install pictorial skills into your project or global harness
...
Compatibility:
  pictorial skills <command>       Legacy namespace; still supported.`);
    process.exit(0);
  }
...
main().catch(error => {
  if (error?.code === 'PICTORIAL_PROMPT_ABORT') {
    console.log('\nAborted.');
    process.exit(130);
  }
