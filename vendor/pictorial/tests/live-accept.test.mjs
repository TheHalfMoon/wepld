...
// publication stages `.pictorial/live/artifacts/<id>-r<n>.<source-ext>`, which
...
describe('live-accept — marker search must ignore Pictorial state', () => {
  let tmp;
  beforeEach(() => { tmp = mkdtempSync(join(tmpdir(), 'pictorial-accept-decoy-')); });
  afterEach(() => rmSync(tmp, { recursive: true, force: true }));

  const SOURCE = [
    '<main>',
    '<!-- pictorial-variants-start ab12cd34 -->',
    '<div data-pictorial-variant="original">ORIGINAL</div>',
    '<div data-pictorial-variant="1">VARIANT ONE</div>',
    '<!-- pictorial-variants-end ab12cd34 -->',
    '</main>',
    '',
  ].join('\n');
  ...
  function seed({ revisions = 3 } = {}) {
    ...
    mkdirSync(join(tmp, '.pictorial', 'live', 'artifacts'), { recursive: true });
    writeFileSync(join(tmp, 'site', 'pages', 'index.astro'), SOURCE);
    for (let r = 1; r <= revisions; r += 1) {
      writeFileSync(join(tmp, '.pictorial', 'live', 'artifacts', `ab12cd34-r${r}.astro`), SOURCE);
    }
  }
  ...
  it('accepts into real source when a staged artifact carries the same marker', () => {
    ...
    assert.equal(
      ...
      'accept must resolve the project file, not the .pictorial artifact decoy',
    );
    const source = readFileSync(join(tmp, 'site', 'pages', 'index.astro'), 'utf-8');
    assert.match(source, /VARIANT ONE/);
    assert.doesNotMatch(source, /pictorial-variants-start/, 'the wrapper must be gone from real source');
  });
...
describe('live-accept — session id validation', () => {
  let tmp;
  beforeEach(() => { tmp = mkdtempSync(join(tmpdir(), 'pictorial-accept-id-')); });
  afterEach(() => rmSync(tmp, { recursive: true, force: true }));
  ...
  for (const id of ['../../../../etc/evil', 'a/b', '..', 'a\\b', '']) {
    it(`refuses --id ${JSON.stringify(id)} without writing a receipt`, () => {
      ...
      assert.equal(existsSync(join(tmp, '.pictorial', 'live', 'accept-receipts')), false);
    });
  }
  ...
  for (const variant of ['.*', '[12]', 'original', '1e2', '']) {
    it(`refuses --variant ${JSON.stringify(variant)} rather than matching by regex`, () => {
      writeFileSync(join(tmp, 'page.html'), [
        '<!-- pictorial-variants-start ab12cd34 -->',
        '<div data-pictorial-variant="original">ORIGINAL CONTENT</div>',
        '<div data-pictorial-variant="1">VARIANT ONE</div>',
        '<!-- pictorial-variants-end ab12cd34 -->',
        '',
      ].join('\n'));
...
describe('live-accept — plain wrapper under source-lock contention', () => {
  let tmp;
  beforeEach(() => { tmp = mkdtempSync(join(tmpdir(), 'pictorial-accept-lock-')); });
  afterEach(() => rmSync(tmp, { recursive: true, force: true }));
  ...
  for (const [label, args] of [['accept', ['--variant', '1']], ['discard', ['--discard']]]) {
    it(`reports a blocked ${label} as mode:error rather than a manual handoff`, () => {
      ...
      assert.equal(existsSync(join(tmp, '.pictorial', 'live', 'accept-receipts')), false, 'no receipt for a failed op');
    });
  }
...
describe('live-accept — style-element edge cases', () => {
  let tmp;
  beforeEach(() => { tmp = mkdtempSync(join(tmpdir(), 'pictorial-accept-test-')); });
  afterEach(() => { rmSync(tmp, { recursive: true, force: true }); });
  ...
  // separate closer, so it got stuck forever and missed data-pictorial-variant
  // divs that came after.
  it('finds the accepted variant after a JSX self-closing <style /> block', () => {
    const html = `<body>
  <!-- pictorial-variants-start SELFC -->
  <div data-pictorial-variants="SELFC" data-pictorial-variant-count="3" style="display: contents">
    <div data-pictorial-variant="original">
      <p class="hook">original text</p>
    </div>
    <style data-pictorial-css="SELFC" dangerouslySetInnerHTML={{ __html: '@scope ([data-pictorial-variant="1"]) { .hook { color: red; } }' }} />
    <div data-pictorial-variant="1">
      <p class="hook">variant one</p>
    </div>
    <div data-pictorial-variant="2" style="display: none">
      <p class="hook">variant two</p>
    </div>
    <div data-pictorial-variant="3" style="display: none">
      <p class="hook">variant three</p>
    </div>
  </div>
  <!-- pictorial-variants-end SELFC -->
</body>`;
    ...
    // no carbonize block, no data-pictorial-variant wrapper (it would serve no purpose).
    assert.ok(!after.includes('pictorial-carbonize-start'), 'no carbonize block (self-closing style has no body)');
    assert.ok(!after.includes('pictorial-variants-start'), 'variant markers removed');
  ...
  it('replays a durable receipt when Accept is retried after source was already written', () => {
    const html = `<body>
  <!-- pictorial-variants-start RECEIPT1 -->
  <div data-pictorial-variants="RECEIPT1" data-pictorial-variant-count="2" style="display: contents">
    <div data-pictorial-variant="original"><p>original</p></div>
    <style data-pictorial-css="RECEIPT1" />
    <div data-pictorial-variant="1"><p>accepted once</p></div>
    <div data-pictorial-variant="2" style="display: none"><p>other</p></div>
  </div>
  <!-- pictorial-variants-end RECEIPT1 -->
</body>`;
    writeFileSync(join(tmp, 'page.html'), html);
  ...
  it('finds the accepted variant after a single-line <style>…</style> block', () => {
    const html = `<body>
  <!-- pictorial-variants-start ONELINE -->
  <div data-pictorial-variants="ONELINE" data-pictorial-variant-count="3" style="display: contents">
    <div data-pictorial-variant="original"><p class="hook">original</p></div>
    <style data-pictorial-css="ONELINE">@scope ([data-pictorial-variant="1"]) { .hook { color: red; } }</style>
    <div data-pictorial-variant="1"><p class="hook">variant one</p></div>
    <div data-pictorial-variant="2" style="display: none"><p class="hook">variant two</p></div>
    <div data-pictorial-variant="3" style="display: none"><p class="hook">variant three</p></div>
  </div>
  <!-- pictorial-variants-end ONELINE -->
</body>`;
    writeFileSync(join(tmp, 'page.html'), html);
    ...
    const after = readFileSync(join(tmp, 'page.html'), 'utf-8');
    assert.ok(after.includes('data-pictorial-variant="3"'), 'accepted wrapper for variant 3 present');
  ...
  it('finds the accepted variant after a multi-line <style>…</style> block (regression baseline)', () => {
    const html = `<body>
  <!-- pictorial-variants-start MULTI -->
  <div data-pictorial-variants="MULTI" data-pictorial-variant-count="3" style="display: contents">
    <div data-pictorial-variant="original"><p class="hook">original</p></div>
    <style data-pictorial-css="MULTI">
      @scope ([data-pictorial-variant="1"]) { .hook { color: red; } }
      @scope ([data-pictorial-variant="2"]) { .hook { color: green; } }
    </style>
    <div data-pictorial-variant="1"><p class="hook">variant one</p></div>
    <div data-pictorial-variant="2" style="display: none"><p class="hook">variant two</p></div>
  </div>
  <!-- pictorial-variants-end MULTI -->
</body>`;
    writeFileSync(join(tmp, 'page.html'), html);
    ...
    const after = readFileSync(join(tmp, 'page.html'), 'utf-8');
    assert.ok(after.includes('data-pictorial-variant="1"'), 'accepted wrapper for variant 1 present');
  ...
  it('carbonize does not double-wrap when the variants block uses JSX template literals on their own lines', () => {
    const tsx = `export default function App() {\n` +
      ...
      `    <main>\n` +
      `      <>\n` +
      `        {/* pictorial-variants-start TPL */}\n` +
      `        <div data-pictorial-variants="TPL" data-pictorial-variant-count="2" style={{ display: 'contents' }}>\n` +
      `          <div data-pictorial-variant="original"><p className="hook">orig</p></div>\n` +
      `          <style data-pictorial-css="TPL">\n` +
      "            {`\n" +
      `              @scope ([data-pictorial-variant="1"]) { .hook { color: red; } }\n` +
      `              @scope ([data-pictorial-variant="2"]) { .hook { color: green; } }\n` +
      "            `}\n" +
      `          </style>\n` +
      `          <div data-pictorial-variant="1"><p className="hook">variant one</p></div>\n` +
      `          <div data-pictorial-variant="2" style={{ display: 'none' }}><p className="hook">variant two</p></div>\n` +
      `        </div>\n` +
      `        {/* pictorial-variants-end TPL */}\n` +
      `      </>\n` +
      `    </main>\n` +
      `  );\n` +
      `}\n`;
    ...
    const carbonStyleMatch = after.match(/<style data-pictorial-css="TPL">([\s\S]*?)<\/style>/);
    ...
    assert.ok(inner.includes('@scope ([data-pictorial-variant="1"])'), 'variant-1 scope kept');
    assert.match(
      after,
      /\n          <div data-pictorial-variant="1" style=\{\{ display: 'contents' \}\}>\n            <p className="hook">variant one<\/p>\n          <\/div>/,
  ...
  it('carbonize does not double-wrap when JSX template-literal punctuation hugs the CSS lines', () => {
    const tsx = `export default function App() {\n` +
      ...
      `    <main>\n` +
      `      <>\n` +
      `        {/* pictorial-variants-start INLINE */}\n` +
      `        <div data-pictorial-variants="INLINE" data-pictorial-variant-count="2" style={{ display: 'contents' }}>\n` +
      `          <div data-pictorial-variant="original"><p className="hook">orig</p></div>\n` +
      `          <style data-pictorial-css="INLINE">\n` +
      "            {`@scope ([data-pictorial-variant=\"1\"]) { .hook { color: red; } }\n" +
      "             @scope ([data-pictorial-variant=\"2\"]) { .hook { color: green; } }`}\n" +
      `          </style>\n` +
      `          <div data-pictorial-variant="1"><p className="hook">variant one</p></div>\n` +
    ...
    const after = readFileSync(join(tmp, 'App.tsx'), 'utf-8');
    const inner = after.match(/<style data-pictorial-css="INLINE">([\s\S]*?)<\/style>/)[1];
    const openCount = (inner.match(/\{`/g) || []).length;
    const closeCount = (inner.match(/`\}/g) || []).length;
  ...
  it('carbonize preserves nested JSX indentation when the wrapper starts at column 0', () => {
    const tsx = `<div data-pictorial-variants="ROOTIND" data-pictorial-variant-count="2" style={{ display: 'contents' }}>
  {/* pictorial-variants-start ROOTIND */}
  <div data-pictorial-variant="original">
    <section className="hook">
      <span>original</span>
    </section>
  </div>
  <style data-pictorial-css="ROOTIND">{\`@scope ([data-pictorial-variant="1"]) { .hook { color: red; } }\`}</style>
  <div data-pictorial-variant="1">
    <section className="hook">
  ...
  {/* pictorial-variants-end ROOTIND */}
</div>
`;
  ...
  return (
    ...
    // the outer wrapper, and no pictorial markers/data attributes.
    assert.ok(!after.includes('data-pictorial-variants'),
      `outer wrapper div must be fully removed; got:\n${after}`);
    assert.ok(!after.includes('data-pictorial-variant'),
      `original-div wrapper must be fully removed; got:\n${after}`);
    assert.ok(!after.includes('pictorial-variants-start'),
  ...
  return (
    <main>
      <div
        className="pictorial-preview-shell"
${extraAttrs}
        data-pictorial-variants="LONGOPEN"
        data-pictorial-variant-count="2"
        style={{ display: 'contents' }}
      >
        {/* pictorial-variants-start LONGOPEN */}
        {/* Original */}
        <div data-pictorial-variant="original">
          <aside className="card">
            <h1>Original</h1>
        ...
        <div data-pictorial-variant="1"><aside className="card"><h1>Variant</h1></aside></div>
        {/* pictorial-variants-end LONGOPEN */}
      </div>
      <div className="next-card">After</div>
    </main>
  );
    ...
    const after = readFileSync(join(tmp, 'App.tsx'), 'utf-8');
    assert.doesNotMatch(after, /data-pictorial-variants/);
    assert.doesNotMatch(after, /pictorial-variants-start/);
  ...
  return (
    <main>
      <div data-pictorial-variants="ACTIVE" data-pictorial-variant-count="2" style={{ display: 'contents' }}>
        <div className="historical-marker-note">
          {/* pictorial-variants-end OLD */}
        </div>
        {/* pictorial-variants-start ACTIVE */}
        {/* Original */}
        <div data-pictorial-variant="original">
          <aside className="card">
            <h1>Original</h1>
    ...
    const after = readFileSync(join(tmp, 'App.tsx'), 'utf-8');
    assert.doesNotMatch(after, /data-pictorial-variants/);
    assert.doesNotMatch(after, /pictorial-variants-start ACTIVE/);
    assert.doesNotMatch(after, /pictorial-variants-end OLD/);
    assert.match(after, /<aside className="card">\s*<h1>Original<\/h1>\s*<\/aside>/m);
    assert.ok(after.includes('<div className="next-card">After</div>'));
  });
  ...
  return (
    <main>
      <div data-pictorial-variants="INDENTACC" data-pictorial-variant-count="3" style={{ display: "contents" }}>
        {/* pictorial-variants-start INDENTACC */}
        {/* Original */}
        <div data-pictorial-variant="original">
          <aside className="card">
            <h1 className="hero-title">Hero</h1>
        ...
        {/* Variants: insert below this line */}
        <div data-pictorial-variant="1"><aside className="card variant-one"><h1 className="hero-title">Hero</h1></aside></div>
        {/* pictorial-variants-end INDENTACC */}
      </div>
    </main>
  );
}`;
  ...
  it('discard restores the original element after a JSX self-closing <style />', () => {
    const html = `<body>
  <!-- pictorial-variants-start DISC -->
  <div data-pictorial-variants="DISC" data-pictorial-variant-count="2" style="display: contents">
    <div data-pictorial-variant="original"><p class="hook">ORIGINAL CONTENT</p></div>
    <style data-pictorial-css="DISC" dangerouslySetInnerHTML={{ __html: '@scope ([data-pictorial-variant="1"]) { .hook { color: red; } }' }} />
    <div data-pictorial-variant="1"><p class="hook">variant one</p></div>
    <div data-pictorial-variant="2" style="display: none"><p class="hook">variant two</p></div>
  </div>
  <!-- pictorial-variants-end DISC -->
</body>`;
    writeFileSync(join(tmp, 'page.html'), html);
    ...
    assert.ok(!after.includes('pictorial-variants-start'), 'wrapper markers gone');
...
describe('live-accept — insert sessions', () => {
  let tmp;
  beforeEach(() => { tmp = mkdtempSync(join(tmpdir(), 'pictorial-accept-insert-')); });
  afterEach(() => { rmSync(tmp, { recursive: true, force: true }); });

  const insertHtml = (id) => `<main>
  <section class="hero">Hero block</section>
  <!-- pictorial-variants-start ${id} -->
  <div data-pictorial-variants="${id}" data-pictorial-mode="insert" data-pictorial-variant-count="2" style="display: contents">
    <!-- Variants: insert below this line -->
    <style data-pictorial-css="${id}">@scope ([data-pictorial-variant="1"]) { .cta { color: red; } }</style>
    <div data-pictorial-variant="1"><p class="cta">Variant one</p></div>
    <div data-pictorial-variant="2" style="display: none"><p class="cta">Variant two</p></div>
  </div>
  <!-- pictorial-variants-end ${id} -->
...
describe('live-accept — Elixir templates under lib/', () => {
  let tmp;
  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), 'pictorial-accept-elixir-'));
  ...
  def nav(assigns) do
    ~H"""
    <!-- pictorial-variants-start ${id} -->
    <div data-pictorial-variants="${id}" data-pictorial-variant-count="2" style="display: contents">
      <div data-pictorial-variant="original">
...
`;
    ...
    assert.ok(!after.includes('pictorial-variants-start'));
    assert.ok(!after.includes('data-pictorial-variant="original"'));
  });
});
