"""Init step — bootstrap a Agile project from within a workflow.

Runs the same scaffolding as ``agile init`` so a workflow can create
...
from agile_cli._agent_config import (
    SCRIPT_TYPE_CHOICES,
    resolve_default_init_integration,
)
from agile_cli.workflows.base import StepBase, StepContext, StepResult, StepStatus
...
_ENGINE_OWNED_DIRS = {".agile"}


class InitStep(StepBase):
    """Bootstrap a project, equivalent to running ``agile init``.

    The step runs the bundled ``agile init`` command non-interactively,
    ...
    ``integration``
        ...
        (``AGILE_INTEGRATION_DEFAULT`` env var, else ``copilot``).
    ``integration_options``
    ...
    def execute(self, config: dict[str, Any], context: StepContext) -> StepResult:
        ...
        # Apply the same default that agile init uses in non-interactive mode
        ...
        # ``agile init`` prompts for confirmation if the directory is not
        ...
        # .agile/workflows/runs/), treat it as implicitly empty and auto-add
        ...
        if exit_code != 0:
            return StepResult(
                ...
                error=(
                    stderr.strip()
                    or stdout.strip()
                    or f"agile init exited with code {exit_code}."
                ),
            )
    ...
    def _run_init(
        ...
        """Invoke ``agile init`` in-process and capture exit code/output.
