...
describe('tanstack-adapter — detection', () => {
  let tmp;
  beforeEach(() => { tmp = mkdtempSync(join(tmpdir(), 'pictorial-tanstack-')); });
  ...
  it('detects a TanStack Start project from package + root route', () => {
    ...
    assert.equal(project.componentFile, 'src/pictorial/PictorialLiveRoot.tsx');
    assert.equal(project.componentImport, '../pictorial/PictorialLiveRoot');
  });
  ...
  it('mirrors the root-route extension for the mount component (jsx)', () => {
    ...
    assert.equal(project.componentFile, 'src/pictorial/PictorialLiveRoot.jsx');
  });
...
describe('tanstack-adapter — patch/unpatch round-trip', () => {
  it('inserts the import + mount component before <Scripts />', () => {
    const patched = patchTanStackRoot(ROOT_TSX, '../pictorial/PictorialLiveRoot');
    assert.match(patched, /import PictorialLiveRoot from '\.\.\/pictorial\/PictorialLiveRoot';/);
    assert.match(patched, /\{\/\* pictorial-live-tanstack-start \*\/\}/);
    assert.match(patched, /<PictorialLiveRoot \/>/);
    // component renders before <Scripts />
    assert.ok(patched.indexOf('<PictorialLiveRoot />') < patched.indexOf('<Scripts />'));
  });
  ...
  it('is idempotent (double patch adds one import + one mount)', () => {
    const once = patchTanStackRoot(ROOT_TSX, '../pictorial/PictorialLiveRoot');
    const twice = patchTanStackRoot(once, '../pictorial/PictorialLiveRoot');
    assert.equal(twice, once);
    assert.equal((twice.match(/<PictorialLiveRoot \/>/g) || []).length, 1);
    assert.equal((twice.match(/^import PictorialLiveRoot/gm) || []).length, 1);
  });

  it('falls back to </body> when <Scripts /> is absent', () => {
    const noScripts = ROOT_TSX.replace(/\s*<Scripts \/>/, '');
    const patched = patchTanStackRoot(noScripts, '../pictorial/PictorialLiveRoot');
    assert.match(patched, /<PictorialLiveRoot \/>/);
    assert.ok(patched.indexOf('<PictorialLiveRoot />') < patched.indexOf('</body>'));
  ...
  it('builds a client-only mount component carrying the token', () => {
    ...
    assert.match(body, /data-pictorial-live-tanstack/);
  });
});
...
describe('tanstack-adapter — apply/remove on disk', () => {
  ...
  it('apply writes the component + patches root, remove restores byte-for-byte', () => {
    ...
    assert.ok(existsSync(join(tmp, 'src/pictorial/PictorialLiveRoot.tsx')));
    assert.match(readFileSync(join(tmp, 'src/routes/__root.tsx'), 'utf-8'), /PictorialLiveRoot/);
    assert.match(
      readFileSync(join(tmp, 'src/pictorial/PictorialLiveRoot.tsx'), 'utf-8'),
      /localhost:9100\/live\.js\?token=T1/,
    );

    const removed = removeTanStackLiveAdapter({ cwd: tmp });
    assert.equal(removed.removed, true);
    assert.equal(existsSync(join(tmp, 'src/pictorial/PictorialLiveRoot.tsx')), false);
    assert.equal(existsSync(join(tmp, 'src/pictorial')), false, 'empty managed dir pruned');
  ...
  it('refuses to clobber an unmanaged file at the component path', () => {
    scaffold(tmp);
    mkdirSync(join(tmp, 'src/pictorial'), { recursive: true });
    writeFileSync(join(tmp, 'src/pictorial/PictorialLiveRoot.tsx'), 'export const mine = 1;\n');
