 ...
 *
 * The managed component lives OUTSIDE `src/routes/` (in `src/pictorial/`) so
...
export const TANSTACK_MARKER_OPEN = '{/* pictorial-live-tanstack-start */}';
export const TANSTACK_MARKER_CLOSE = '{/* pictorial-live-tanstack-end */}';
export const TANSTACK_COMPONENT_DIR = 'src/pictorial';
export const TANSTACK_COMPONENT_BASENAME = 'PictorialLiveRoot';
...
export function applyTanStackLiveAdapter({ cwd = process.cwd(), port, token, project = detectTanStackStartProject(cwd) } = {}) {
  ...
  if (componentExisted && !isManagedComponent(fs.readFileSync(componentAbs, 'utf-8'))) {
    // A non-Pictorial file already sits at our managed path — refuse to clobber.
    return {
      file: project.componentFile,
      error: 'tanstack_component_conflict',
      hint: `${project.componentFile} already exists and is not managed by Pictorial Live`,
    };
  }
...
export function patchTanStackRoot(content, componentImport) {
  ...
  const importStatement = `import PictorialLiveRoot from '${componentImport}';`;
...
export function unpatchTanStackRoot(content) {
  ...
  out = out.replace(
    new RegExp("^import PictorialLiveRoot from '[^']*';[ \\t]*\\r?\\n", 'gm'),
    '',
  );
  return out;
}
...
export function buildTanStackLiveRootComponent(port, token) {
  ...
  return `/* pictorial-live-tanstack-start */
import { useEffect } from 'react';

const LIVE_SRC = '${liveSrc}';
const LIVE_SELECTOR = 'script[data-pictorial-live-tanstack]';

// Dev-only mount for Pictorial Live. TanStack Start server-renders the root
...
export default function PictorialLiveRoot() {
  useEffect(() => {
    ...
    script.src = LIVE_SRC;
    script.async = true;
    script.setAttribute('data-pictorial-live-tanstack', '');
    script.setAttribute('data-pictorial-live-script', 'true');
...
// The managed mount component carries the `pictorial-live-tanstack` marker in
// its leading comment and its script data-attribute; user files never do.
function isManagedComponent(content) {
  return String(content || '').includes('pictorial-live-tanstack');
}
