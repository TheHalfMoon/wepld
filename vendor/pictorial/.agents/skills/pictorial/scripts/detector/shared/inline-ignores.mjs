 ...
 * an ignore in `.pictorial/config.json`.
 *
 ...
 * line, so the same marker works across every comment style pictorial scans —
 * `//`, `/* *\/`, `<!-- -->`, `#`, `{/* *\/}`, `{# #}`. Trailing comment closers
 * are stripped before the rule list is parsed.
 *
 * Syntax (reason optional; eslint `--` or biome `:` separator):
 *
 *   pictorial-disable <rule>[, <rule>...] [-- reason]   whole file
 *   pictorial-disable-line <rule>...      [-- reason]   the same line
 *   pictorial-disable-next-line <rule>... [-- reason]   the following line
 *   pictorial-disable                                   bare / `*` = every rule
 *
 * Examples:
 *
 *   <!-- pictorial-disable overused-font -- exported brand doc, font is first-party -->
 *   .brand { font-family: Inter; } /* pictorial-disable-line overused-font *\/
 *   // pictorial-disable-next-line bounce-easing: intentional playful affordance
 *
...
const DIRECTIVE_RE = /pictorial-(disable-next-line|disable-line|disable)\b[ \t]*([^\n\r]*)/gi;
...
function parseInlineIgnores(content) {
  ...
  // Case-insensitive to match DIRECTIVE_RE's `i` flag (e.g. `Pictorial-Disable`).
  if (!/pictorial-disable/i.test(text)) return result;
