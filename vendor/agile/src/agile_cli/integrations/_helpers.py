"""agile integration helpers — internal utilities shared across command modules."""
...
def _get_agile_version() -> str:
    """Return the current Agile version.

    Resolved lazily through ``_commands.get_agile_version`` so that tests
    that monkeypatch ``agile_cli.integrations._commands.get_agile_version``
    ...
    return _commands.get_agile_version()
...
def _read_integration_json(project_root: Path) -> dict[str, Any]:
    """Load ``.agile/integration.json``. Returns normalized state when present.
    ...
    if error.kind == "decode":
        ...
        console.print("Please upgrade Agile before modifying integrations.")
    raise typer.Exit(1)
...
def _write_integration_json(
    ...
    """Write ``.agile/integration.json`` with legacy-compatible state."""
    _write_integration_json_file(
        project_root,
        version=_get_agile_version(),
...
def _refresh_init_options_agile_version(project_root: Path) -> None:
    """Refresh only the Agile version recorded in init-options.json."""
    from .. import load_init_options, save_init_options
    opts = load_init_options(project_root)
    if not isinstance(opts, dict) or not opts:
        return
    opts["agile_version"] = _get_agile_version()
...
def _remove_integration_json(project_root: Path) -> None:
    """Remove ``.agile/integration.json`` if it exists."""
    path = project_root / INTEGRATION_JSON
...
def _resolve_script_type(project_root: Path, script_type: str | None) -> str:
    ...
    if isinstance(saved, str) and saved.strip():
        return _normalize_script_type(saved, ".agile/init-options.json")
