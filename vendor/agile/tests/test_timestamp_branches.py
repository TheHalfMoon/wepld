...
def git_repo(tmp_path: Path) -> Path:
    """Create a temp git repo with scripts and .agile dir."""
...
def ext_git_repo(tmp_path: Path) -> Path:
    ...
    # Extension script needs common.sh at .agile/scripts/bash/
    specify_scripts = tmp_path / ".agile" / "scripts" / "bash"
    ...
    ext_dir = tmp_path / ".agile" / "extensions" / "git" / "scripts" / "bash"
    ...
    (tmp_path / ".agile" / "templates").mkdir(parents=True, exist_ok=True)
...
def ext_ps_git_repo(tmp_path: Path) -> Path:
    ...
    # Also install at .agile/scripts/powershell/ for extension resolution
    specify_ps = tmp_path / ".agile" / "scripts" / "powershell"
...
class TestSequentialBranchPowerShell:
    ...
    def test_missing_spec_template_warns_matching_bash(self, ps_git_repo: Path):
        ...
        (ps_git_repo / ".agile" / "templates" / "spec-template.md").unlink()
        script = ps_git_repo / "scripts" / "powershell" / "create-new-feature.ps1"
        result = subprocess.run(
            ["pwsh", "-NoProfile", "-File", str(script),
...
class TestGitBranchNameOverrideBash:
    ...
    def _run_ext(self, ext_git_repo: Path, env_extras: dict, *extra_args: str):
        script = ext_git_repo / ".agile" / "extensions" / "git" / "scripts" / "bash" / "create-new-feature-branch.sh"
...
class TestGitBranchNameOverridePowerShell:
    ...
    def _run_ext(self, ext_ps_git_repo: Path, env_extras: dict):
        script = ext_ps_git_repo / ".agile" / "extensions" / "git" / "scripts" / "powershell" / "create-new-feature-branch.ps1"
        return subprocess.run(
...
class TestFeatureDirectoryResolution:
    ...
    def test_feature_json_overrides_branch_lookup(self, git_repo: Path):
        ...
        feature_json = git_repo / ".agile" / "feature.json"
...
class TestDescriptionQuoting:
    ...
    Regression tests for https://github.com/TheHalfMoon/wepld/issues/2339
    """
