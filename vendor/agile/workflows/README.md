...
Workflows are multi-step, resumable automation pipelines defined in YAML. They orchestrate Agile commands across integrations, evaluate control flow, and pause at human review gates — enabling end-to-end Spec-Driven Development cycles without manual step-by-step invocation.
...
specify workflow run agile --input spec="Build a user authentication system with OAuth support"
...
specify workflow info agile

# Remove a workflow
specify workflow remove agile
```
...
Invoke an installed Agile command by name via the integration CLI:
...
Bootstrap a project the same way `agile init` does — scaffolding
...
- id: check-result
  ...
  then:
    ...
    - id: recover
      ...
      then:
        - id: rerun
          command: agile.recovery
  else:
    - id: next-thing
      command: agile.next-thing
```
...
|----------|-------------|
| `context.run_id` | The current workflow run id (the same value Agile prints as `Run ID:` at the end of `workflow run`). Auto-generated runs are 8-character hex from `uuid4`; operator-supplied ids may be any alphanumeric string with hyphens or underscores. Empty string outside a run context. |
| `context.workflow_dir` | The resolved absolute path to the directory containing the workflow source file. For file-loaded workflows this is the parent directory of the YAML file; for installed-by-ID workflows it is the absolute path to the installation directory (e.g. `<project>/.agile/workflows/<id>/`); for string-loaded workflows it is an empty string. On resume the original source directory is preserved from the first execution. |
...
Every workflow run persists state to `.agile/workflows/runs/<run_id>/`:
...
Workflows are discovered through catalogs. By default, Agile uses the official and community catalogs:

> [!NOTE]
> Community workflows are independently created and maintained by their respective authors. GitHub and the Agile maintainers may review pull requests that add entries to the community catalog for formatting and structure, but they do **not review, audit, endorse, or support the workflow definitions themselves**. Review workflow source before installation and use at your own discretion.
...
| Variable | Description |
|----------|-------------|
| `AGILE_WORKFLOW_CATALOG_URL` | Override the catalog URL (replaces all defaults) |
| `AGILE_WORKFLOW_DIR` | Set automatically for shell steps; contains the resolved absolute path to the workflow source directory (same value as `{{ context.workflow_dir }}`). Not set when the workflow has no source path (string-loaded workflows). |

## Configuration Files

| File | Scope | Description |
|------|-------|-------------|
| `.agile/workflow-catalogs.yml` | Project | Custom catalog stack for this project |
| `~/.agile/workflow-catalogs.yml` | User | Custom catalog stack for all projects |
...
└── agile/                                # Built-in SDD cycle workflow
    └── workflow.yml
```
