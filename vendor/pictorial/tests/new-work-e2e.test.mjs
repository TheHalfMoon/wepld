...
function run(args, cwd) {
  return new Promise((resolve) => {
    const child = spawn(process.execPath, [SERVE, ...args], {
      cwd,
      env: { ...process.env, PICTORIAL_QUESTION_FORCE: '1', PICTORIAL_CATALOG_DIR: CATALOG_DIR },
...
function spawnSyncGen(prompt, out, size = null) {
  ...
  return spawnSync(process.execPath, args, {
    env: { ...process.env, PICTORIAL_IMAGE_GEN_FAKE: '1' },
    encoding: 'buffer',
  });
}
...
describe('new-work-e2e: serve-question decision page', () => {
  ...
  it('(b) re-roll with steer keeps the server alive; --update re-deals; the next pick is terminal', async () => {
    ...
    try {
      ...
      assert.ok(existsSync(path.join(cwd, '.pictorial', 'questions', `${key}.state.json`)),
    ...
    const payload = {
      ...
      options: [
        ...
        },
        {
          ...
          comp: path.join(cwd, '.pictorial', 'mocks', 'decision', 'plain-body-missing.webp'),
        },
        {
          id: 'board-only', label: 'Board Fallback',
          board,
          comp: path.join(cwd, '.pictorial', 'mocks', 'decision', 'board-missing.webp'),
        },
      ],
      reroll: true,
      steer: true,
    ...
    const payload = {
      ...
      options: [
        ...
        // the reorder, so payload order cannot promote a demoted world.
        {
          ...
          sketch: '.pictorial/sketches/challenger-deepsea.webp',
        },
  ...
  it('(h) the build-path toggle: a code-to-comp flip shimmers the slots, surfaces through --wait, and rides the answer', async () => {
      ...
      options: [
        {
          ...
          comp: '.pictorial/mocks/decision/assigned.webp',
    ...
    try {
      ...
      mkdirSync(path.join(cwd, '.pictorial', 'mocks', 'decision'), { recursive: true });
      makeFakeImage(path.join(cwd, '.pictorial', 'mocks', 'decision'), 'ledger spine comp', 'assigned.webp');
  ...
  it('(g) a wireframe card draws its schematic in the media slot and keeps its full read on the front', async () => {
    ...
    await startDaemon(cwd, payload, key);
    const state = JSON.parse(readFileSync(path.join(cwd, '.pictorial', 'questions', `${key}.state.json`), 'utf8'));
    try {
      const context = await browser.newContext();
      const page = await context.newPage();
