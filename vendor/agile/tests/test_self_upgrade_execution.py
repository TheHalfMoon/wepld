...
class TestInstallerMissing:
    ...
    def test_uv_missing_exits_3(self, uv_tool_argv0, clean_environ):
        which_results = {"agile": "/usr/local/bin/specify"}
        with patch("agile_cli.authentication.http.urllib.request.urlopen") as mock_urlopen, patch(
            "agile_cli._version.shutil.which", side_effect=lambda n: which_results.get(n)
    ...
    def test_absolute_installer_path_does_not_require_path_lookup(
        ...
        with patch("agile_cli.authentication.http.urllib.request.urlopen") as mock_urlopen, patch(
            ...
            "agile_cli._version._verify_upgrade", return_value="0.7.6"
        ), patch(
            "agile_cli._version._assemble_installer_argv",
            return_value=[
                str(fake_uv),
                "tool",
                "install",
    ...
    def test_relative_installer_path_missing_gets_path_specific_message(
        ...
        with patch("agile_cli.authentication.http.urllib.request.urlopen") as mock_urlopen, patch(
            "agile_cli._version.shutil.which", side_effect=lambda name: None
        ), patch("agile_cli._version._get_installed_version", return_value="0.7.5"), patch(
            "agile_cli._version._assemble_installer_argv",
            return_value=[
                "./uv",
                "tool",
    ...
    def test_resolved_absolute_installer_removed_before_exec_gets_missing_path_message(
        ...
        ), patch("agile_cli._version.subprocess.run", side_effect=fake_run), patch(
            "agile_cli._version._get_installed_version", return_value="0.7.5"
        ):
            mock_urlopen.return_value = mock_urlopen_response({"tag_name": "v0.7.6"})
            result = runner.invoke(app, ["self", "upgrade"])
    ...
    def test_absolute_installer_path_not_executable_gets_specific_message(
        ...
        with patch("agile_cli.authentication.http.urllib.request.urlopen") as mock_urlopen, patch(
            "agile_cli._version.shutil.which", side_effect=lambda name: None
        ), patch("agile_cli._version.os.access", return_value=False), patch(
            "agile_cli._version._get_installed_version", return_value="0.7.5"
        ), patch(
            "agile_cli._version._assemble_installer_argv",
            return_value=[
    ...
    def test_exec_oserror_errno_is_treated_as_invalid_installer(
        ...
        ), patch("agile_cli._version.subprocess.run", side_effect=invalid_error):
            mock_urlopen.return_value = mock_urlopen_response({"tag_name": "v0.7.6"})
            result = runner.invoke(app, ["self", "upgrade"])
        assert result.exit_code == 3
        out = strip_ansi(result.output)
    ...
    def test_transient_exec_oserror_is_not_treated_as_invalid_installer(
        ...
        ), patch("agile_cli._version.subprocess.run", side_effect=transient_error):
            mock_urlopen.return_value = mock_urlopen_response({"tag_name": "v0.7.6"})
            result = runner.invoke(app, ["self", "upgrade"])
...
class TestInstallerFailed:
    ...
    def test_installer_exit_2_propagates(self, uv_tool_argv0, clean_environ):
        ...
        assert "git+https://github.com/TheHalfMoon/wepld.git@v0.7.6" in out
        assert (
            "To pin back to the previous version: "
            "uv tool install wepld-agile --force --from "
            "git+https://github.com/TheHalfMoon/wepld.git@v0.7.5"
        ) in out
    ...
    def test_rollback_hint_accepts_normalizable_stable_snapshot(
        ...
        with patch("agile_cli.authentication.http.urllib.request.urlopen") as mock_urlopen, patch(
            ...
            "agile_cli._version._get_installed_version", return_value="v0.7.5"
        ):
            mock_urlopen.return_value = mock_urlopen_response({"tag_name": "v0.7.6"})
            mock_run.side_effect = [_completed_process(2)]
            result = runner.invoke(app, ["self", "upgrade"])
        ...
        out = strip_ansi(result.output)
        assert (
            "To pin back to the previous version: uv tool install wepld-agile --force "
            "--from git+https://github.com/TheHalfMoon/wepld.git@v0.7.5"
        ) in out
    ...
    def test_prerelease_failure_degrades_rollback_hint_to_releases_page(
        ...
        assert "git+https://github.com/TheHalfMoon/wepld.git@v1.0.0rc1" not in out
