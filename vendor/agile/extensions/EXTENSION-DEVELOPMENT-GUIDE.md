...
A guide for creating Agile extensions.
...
extension:
  ...
  author: "Your Name"
  repository: "https://github.com/you/agile-my-ext"
  license: "MIT"

requires:
  agile_version: ">=0.1.0"            # Minimum agile version
...
provides:
  commands:
    - name: "agile.my-ext.hello"      # Must follow pattern: agile.{ext-id}.{cmd}
      file: "commands/hello.md"
      description: "Say hello"
      aliases: ["agile.my-ext.hi"]    # Optional aliases, same pattern
...
Load extension config from `.agile/extensions/my-ext/my-ext-config.yml`.

### 5. Test Locally

```bash
cd /path/to/agile-project
...
```bash
claude
> /agile.my-ext.hello world
```

The command will be available in `.claude/commands/agile.my-ext.hello.md`.
...
- `agile_version`: Semantic version specifier (e.g., ">=0.1.0,<2.0.0")

**Optional sub-fields**:

- `tools`: External tools required (array of tool objects)
- `commands`: Core agile commands needed (array of command names)
...
- `name`: Command name (must match `agile.{ext-id}.{command}`)
- `file`: Path to command file (relative to extension root)
- `description`: Command description (optional)
- `aliases`: Alternative command names (optional, array; each must match `agile.{ext-id}.{command}`)
...
- `__AGILE_COMMAND_<NAME>__`: Replaced with the invocation of another command, rendered using the active integration's separator (see [Referencing other commands](#referencing-other-commands))
...
A command body is a *template* that Agile renders once per agent. Different agents invoke commands with different surface syntax — for example `/agile.plan` (dot separator) or `/agile-plan` (hyphen separator). Some agents also use different prefixes in skills mode (e.g. Kimi `/skill:agile-plan`, Codex/ZCode `$agile-plan`). So when you reference a sibling command from a body, **do not hard-code a literal invocation** like `/agile.my-ext.prepare`. A literal is correct for exactly one agent and breaks on the rest.

Instead use the agent-neutral token `__AGILE_COMMAND_<NAME>__`. Agile resolves it to a `/agile<separator>...` invocation using the active integration's `invoke_separator` (and integrations may post-process that further in skills output).

Encode the command name in upper case, dropping the `agile.` prefix and turning each dotted segment separator into an underscore:

| Command file | Token |
| --- | --- |
| `agile.plan.md` | `__AGILE_COMMAND_PLAN__` |
| `agile.bug.fix.md` | `__AGILE_COMMAND_BUG_FIX__` |
| `agile.git.commit.md` | `__AGILE_COMMAND_GIT_COMMIT__` |
...
```markdown
Once the assessment exists, the next step is `__AGILE_COMMAND_BUG_FIX__ slug=<slug>`.
```

This renders as `/agile.bug.fix slug=<slug>` for a slash-based agent, `/agile-bug-fix slug=<slug>` for a skills-based agent, and so on — the author writes it once and it stays portable. The first-party `bug` and `git` extensions use this token exclusively; see `extensions/bug/commands/` for working examples.
...
> `resolve_command_refs`, so a `__AGILE_COMMAND_<NAME>__` token reaches
...
```yaml
scripts:
  sh: .agile/scripts/bash/helper.sh
```

This allows scripts to reference core agile scripts.
...
2. Project config (`.agile/extensions/my-ext/my-ext-config.yml`)
3. Local overrides (`.agile/extensions/my-ext/my-ext-config.local.yml` - gitignored)
4. Environment variables (`AGILE_MY_EXT_*`)

**Example loading script**:

```bash
#!/usr/bin/env bash
EXT_DIR=".agile/extensions/my-ext"

# Load and merge config
config=$(yq eval '.' "$EXT_DIR/my-ext-config.yml" -o=json)

# Apply env overrides
if [ -n "${AGILE_MY_EXT_API_KEY:-}" ]; then
  config=$(echo "$config" | jq ".api.api_key = \"$AGILE_MY_EXT_API_KEY\"")
fi
...
Extension authors can create a `.extensionignore` file in the extension root to exclude files and folders from being copied when a user installs the extension with `agile extension add`. This is useful for keeping development-only files (tests, CI configs, docs source, etc.) out of the installed copy.
...
- **Pattern**: `^agile\.[a-z0-9-]+\.[a-z0-9-]+$`
- **Valid**: `agile.my-ext.hello`, `agile.tool.cmd`
- **Invalid**: `my-ext.hello` (missing prefix), `agile.hello` (no extension namespace)
...
5. **Check command registration**:
   ...
   ```bash
   ls .claude/commands/agile.my-ext.*
   ```

6. **Remove extension**:

   ```bash
   agile extension remove my-ext
   ```
...
1. **Create repository**: `agile-my-ext`
2. **Add files**:
...
4. **Install from repo**:
   ...
   ```bash
   git clone https://github.com/you/agile-my-ext
   agile extension add --dev agile-my-ext/
   ```
...
```bash
zip -r agile-my-ext-1.0.0.zip extension.yml commands/ scripts/ docs/
```

Users install with:

```bash
agile extension add <extension-name> --from https://github.com/.../agile-my-ext-1.0.0.zip
```
...
2. **File an issue** using the [Extension Submission](https://github.com/TheHalfMoon/wepld/issues/new?template=extension_submission.yml) template
3. **After review**, a maintainer updates the catalog and your extension becomes available:
   ...
   - Users install with: `agile extension add my-ext` (from their catalog)
...
Load config:
```bash
config_file=".agile/extensions/tool/tool-config.yml"
...
hooks:
  after_plan:
    - command: "agile.my-ext.verify"
      priority: 5
      optional: false
      description: "Verify the plan"
    - command: "agile.my-ext.report"
      priority: 10
      optional: true
...
**Error**: `Extension requires agile >=0.2.0`

- **Fix**: Upgrade Agile using the [Upgrade Guide](../docs/upgrade.md). `uv tool install wepld-agile --force --from git+https://github.com/TheHalfMoon/wepld.git` remains available as a source-install fallback. If you installed from PyPI and want to stay on that route, follow the [PyPI upgrade guidance](../docs/install/pypi.md#upgrade).
...
1. Config file exists: `.agile/extensions/{ext-id}/{ext-id}-config.yml`
...
- **Examples**: See `agile-jira` for full-featured example (Phase B)
