...
def _resolve(tmp_path: Path, script: str | None, monkeypatch) -> str:
    ...
    monkeypatch.setattr(
        "agile_cli.integrations.base.IntegrationBase._interpreter_runs",
...
def test_py_variant_prefixes_interpreter(tmp_path, monkeypatch):
    ...
    assert "python3 .agile/scripts/python/setup_plan.py --json" in body
    assert "{SCRIPT}" not in body


def test_sh_variant_is_not_prefixed(tmp_path, monkeypatch):
    body = _resolve(tmp_path, "sh", monkeypatch)
    assert ".agile/scripts/bash/setup-plan.sh --json" in body
...
def test_py_interpreter_with_spaces_uses_powershell_call_operator(
    ...
    monkeypatch.setattr(
        "agile_cli.integrations.base.os", SimpleNamespace(name="nt")
    )
...
def test_py_install_includes_python_and_fallback_scripts(tmp_path, monkeypatch):
    ...
    assert (tmp_path / ".agile/scripts/python/setup_plan.py").is_file()
    assert (tmp_path / ".agile/scripts/python/setup_tasks.py").is_file()
    ...
    fallback = (
        ...
        else ".agile/scripts/powershell/check-prerequisites.ps1"
    )
...
def test_py_rejects_one_sided_opposite_platform_fallback(
    ...
    from agile_cli import _install_shared_infra
    from agile_cli import shared_infra
    ...
    assert not (
        tmp_path / ".agile/scripts/bash/check-prerequisites.sh"
    ).exists()
