...
async function captureVisualContrastCandidate(page, candidate, viewport) {
  ...
  const token = `pictorial-contrast-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  const applied = await page.evaluate(({ selector, token, backgroundClipText }) => {
    ...
    if (!el) return false;
    let style = document.getElementById('pictorial-visual-contrast-hide-style');
    if (!style) {
      style = document.createElement('style');
      style.id = 'pictorial-visual-contrast-hide-style';
      style.textContent = [
        '[data-pictorial-visual-contrast-target] {',
        '  color: transparent !important;',
        '  -webkit-text-fill-color: transparent !important;',
        '  text-shadow: none !important;',
        '}',
        '[data-pictorial-visual-contrast-target][data-pictorial-bgclip-text="true"] {',
        '  background-image: none !important;',
        '}',
      ].join('\n');
      document.head.appendChild(style);
    }
    el.setAttribute('data-pictorial-visual-contrast-target', token);
    if (backgroundClipText) el.setAttribute('data-pictorial-bgclip-text', 'true');
    return true;
  }, {
  ...
  try {
    ...
    await page.evaluate(({ selector }) => {
      try {
        ...
        if (el) {
          el.removeAttribute('data-pictorial-visual-contrast-target');
          el.removeAttribute('data-pictorial-bgclip-text');
        }
      } catch {
