...
adapted for the ``agile-<name>/SKILL.md`` skills layout.
"""
...
class SkillsIntegrationTests:
    ...
    def test_setup_writes_to_correct_directory(self, tmp_path):
        ...
        for f in skill_files:
            # Each SKILL.md is in agile-<name>/ under the skills directory
    ...
    def test_skill_directory_structure(self, tmp_path):
        ...
        actual_commands = set()
        for f in skill_files:
            skill_dir_name = f.parent.name  # e.g. "agile-plan"
    ...
    def test_command_refs_use_hyphen_separator(self, tmp_path):
        ...
        for f in skill_files:
            content = f.read_text(encoding="utf-8")
            # Skills agents must use /agile-<name>, not /agile.<name>
            assert "/agile." not in content, (
                f"{f.name} contains dot-notation /agile. reference; "
                f"skills agents must use /agile-<name>"
            )
    ...
    def test_hook_sections_explain_dotted_command_conversion(self, tmp_path):
        ...
        m = IntegrationManifest(self.KEY, tmp_path)
        i.setup(tmp_path, m)
        specify_skill = i.skills_dest(tmp_path) / "agile-specify" / "SKILL.md"
        assert specify_skill.exists()
        content = specify_skill.read_text(encoding="utf-8")
        assert "replace dots" in content, (
            "agile-specify should explain dotted hook command conversion"
        )
    ...
    def test_hook_note_injected_for_each_instruction_independently(self):
        ...
        content = (
            ...
            "For example, `agile.git.commit` → `/agile-git-commit`.\n"
    ...
    def test_plan_skill_has_no_context_placeholder(self, tmp_path):
        ...
        m = IntegrationManifest(self.KEY, tmp_path)
        i.setup(tmp_path, m)
        plan_file = i.skills_dest(tmp_path) / "agile-plan" / "SKILL.md"
    ...
    def test_pre_existing_skills_not_removed(self, tmp_path):
        """Pre-existing non-agile skills should be left untouched."""
        i = get_integration(self.KEY)
    ...
    def _expected_files(self, script_variant: str) -> list[str]:
        ...
        for cmd in self._SKILL_COMMANDS:
            files.append(f"{skills_prefix}/agile-{cmd}/SKILL.md")
        # Integration metadata
        files += [
            ".agile/init-options.json",
            ".agile/integration.json",
            f".agile/integrations/{self.KEY}.manifest.json",
    ...
    def test_complete_file_inventory_sh(self, tmp_path):
        """Every file produced by agile init --integration <key> --script sh."""
        from typer.testing import CliRunner
        from agile_cli import app
    ...
    def test_complete_file_inventory_ps(self, tmp_path):
        """Every file produced by agile init --integration <key> --script ps."""
        from typer.testing import CliRunner
        from agile_cli import app
