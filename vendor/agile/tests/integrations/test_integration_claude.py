...
from agile_cli.integrations.base import IntegrationBase, SkillsIntegration
from agile_cli.integrations.claude import ARGUMENT_HINTS, FORK_CONTEXT_COMMANDS
...
class TestClaudeIntegration:
    ...
    def test_setup_creates_skill_files(self, tmp_path):
        ...
        assert "__AGILE_COMMAND_" not in content, "unprocessed __AGILE_COMMAND_*__"
        assert "/agile." not in content, "skills agent must use /agile-<name> not /agile.<name>"

        parts = content.split("---", 2)
        parsed = yaml.safe_load(parts[1])
        assert parsed["name"] == "agile-plan"
    ...
    def test_integration_flag_creates_skill_files_cli(self, tmp_path):
        ...
        init_options = json.loads(
            (project / ".agile" / "init-options.json").read_text(encoding="utf-8")
        )
    ...
    def test_integration_flag_creates_skill_files(self, tmp_path):
        ...
        assert result.exit_code == 0, result.output
        assert (project / ".claude" / "skills" / "agile-specify" / "SKILL.md").exists()
    ...
    def test_interactive_claude_selection_uses_integration_path(self, tmp_path):
        ...
        try:
            ...
            runner = CliRunner()
            with (
                patch("agile_cli.commands.init._stdin_is_interactive", return_value=True),
                patch("agile_cli.commands.init.select_with_arrows", return_value="claude"),
            ):
                result = runner.invoke(
                    app,
                    [
        ...
        assert result.exit_code == 0, result.output
        assert (project / ".agile" / "integration.json").exists()
        assert (project / ".agile" / "integrations" / "claude.manifest.json").exists()

        skill_file = project / ".claude" / "skills" / "agile-plan" / "SKILL.md"
    ...
    def test_claude_init_remains_usable_when_converter_fails(self, tmp_path):
        ...
        assert (target / ".claude" / "skills" / "agile-specify" / "SKILL.md").exists()
    ...
    def test_claude_preset_creates_new_skill_without_commands_dir(self, tmp_path):
        ...
        assert "agile-research" in metadata.get("registered_skills", {}).get("claude", [])
...
class TestClaudeArgumentHints:
    ...
    def test_inject_argument_hint_survives_folded_description(self):
        ...
        frontmatter = {
            ...
            "compatibility": "Requires agile project structure with .agile/ directory",
        }
...
class TestClaudeDisableModelInvocation:
    ...
    def test_non_claude_agents_lack_disable_model_invocation(self, tmp_path):
        ...
        fm = CommandRegistrar.build_skill_frontmatter(
            "codex", "agile-plan", "desc", "templates/commands/plan.md"
        )
...
class TestClaudeForkContext:
    ...
    def test_analyze_skill_does_not_fork(self, tmp_path):
        """agile-analyze must run in the main session, not a forked subagent (#3185)."""
        i = get_integration("claude")
        m = IntegrationManifest("claude", tmp_path)
        i.setup(tmp_path, m, script_type="sh")
        analyze_skill = tmp_path / ".claude/skills/agile-analyze/SKILL.md"
    ...
    def test_post_process_no_fork_for_skills(self):
        ...
        i = get_integration("claude")
        for name in ("agile-analyze", "agile-plan"):
            content = f'---\nname: "{name}"\ndescription: "x"\n---\n\nBody\n'
    ...
    def test_fork_mechanism_injects_when_configured(self, monkeypatch):
        ...
        import agile_cli.integrations.claude as claude_mod
...
class TestClaudeHookCommandNote:
    ...
    def test_hook_note_injected_in_skills_with_hooks(self, tmp_path):
        ...
        m = IntegrationManifest("claude", tmp_path)
        i.setup(tmp_path, m, script_type="sh")
        specify_skill = tmp_path / ".claude/skills/agile-specify/SKILL.md"
        assert specify_skill.exists()
        content = specify_skill.read_text(encoding="utf-8")
        # specify.md has hook sections
        assert "replace dots" in content, (
            "agile-specify should have dot-to-hyphen hook note"
        )
    ...
    def test_hook_note_fills_missing_repeated_instructions(self, tmp_path):
        ...
        from agile_cli.integrations.base import _HOOK_COMMAND_NOTE
...
class TestAgileManifestRecordsSkippedFiles:
    """Regression test for issue #2107.

    ``install_shared_infra`` must record every shared-infrastructure file
    under ``.agile/`` in ``agile.manifest.json``, including files that
    ...
    against an already-populated ``.agile/`` tree, every file went down the
    ...
    Reproduction (without the fix) using ``install_shared_infra`` directly:
        ...
        (p / ".agile/integrations/agile.manifest.json").unlink()
    ...
    def _read_manifest_files(self, project_path: Path) -> dict:
        manifest_path = (
            project_path / ".agile" / "integrations" / "agile.manifest.json"
        )
        assert manifest_path.exists(), (
            f"agile.manifest.json not written at {manifest_path}"
        )
    ...
    def test_install_shared_infra_records_skipped_files(self, tmp_path):
        """With ``force=False`` and ``.agile/`` already populated, the
        ...
        # Simulate a lost manifest while ``.agile/`` is still on disk
        # (e.g. the manifest was deleted, corrupted, or the layout was
        # extracted out-of-band).
        manifest_path = (
            tmp_path / ".agile" / "integrations" / "agile.manifest.json"
        )
        ...
        assert second_files, (
            "agile.manifest.json files dict is empty after install with "
    ...
    def test_install_shared_infra_handles_directory_at_script_destination(
        ...
        # Pre-create the .agile/scripts/bash tree, then plant a directory
        # where a script file is expected so the skip branch hits a
        # non-regular-file path.
        bash_dir = tmp_path / ".agile" / "scripts" / "bash"
        ...
        assert ".agile/scripts/bash/common.sh" not in files, (
    ...
    def test_install_shared_infra_handles_directory_at_template_destination(
        ...
        templates_dir = tmp_path / ".agile" / "templates"
        ...
        files = self._read_manifest_files(tmp_path)
        template_rel = f".agile/templates/{real_template}"
