"""
Extension Manager for Agile

Handles installation, removal, and management of Agile extensions.
Extensions are modular packages that add commands and functionality to agile
...
EXTENSION_COMMAND_NAME_PATTERN = re.compile(r"^agile\.([a-z0-9-]+)\.([a-z0-9-]+)$")
...
REINSTALL_COMMAND = "uv tool install wepld-agile --force --from git+https://github.com/TheHalfMoon/wepld.git"
...
def _load_core_command_names() -> frozenset[str]:
    ...
    #3014 move of this module from ``agile_cli/extensions.py`` to
    ``agile_cli/extensions/__init__.py`` pushed the file one directory deeper
...
class ExtensionManifest:
    ...
    def _validate(self):
        ...
        # extension made ``agile extension list`` exit 1 with a raw
        ...
        if "agile_version" not in requires:
            raise ValidationError("Missing requires.agile_version")
        ...
        # naming no field. An unquoted ``agile_version: 1.0`` is an easy YAML
        # slip. Mirrors the sibling IntegrationDescriptor, which already requires
        # a non-empty string here.
        if (
            not isinstance(requires["agile_version"], str)
            or not requires["agile_version"].strip()
        ):
            raise ValidationError(
                "Invalid requires.agile_version: expected a non-empty string, "
                f"got {type(requires['agile_version']).__name__}"
            )
        ...
        for cmd in commands:
            ...
            if not EXTENSION_COMMAND_NAME_PATTERN.match(cmd["name"]):
                ...
                if corrected:
                    self.warnings.append(
                        ...
                        f"'agile.{{extension}}.{{command}}'. Registering as '{corrected}'. "
                    ...
                    raise ValidationError(
                        ...
                        "must follow pattern 'agile.{extension}.{command}'"
                    )
            ...
            # (e.g. 'agile.verify' short aliases used by existing extensions).
        ...
        # an alias-form ref (ext.cmd → agile.ext.cmd).  Always emit a warning when
        ...
        for hook_name, hook_data in self.data.get("hooks", {}).items():
            for entry in coerce_hook_entries(hook_data):
                ...
                # Step 2: lift alias-form '{ext_id}.cmd' to canonical 'agile.{ext_id}.cmd'.
                parts = after_rename.split(".")
                if len(parts) == 2 and parts[0] == ext["id"]:
                    final_ref = f"agile.{ext['id']}.{parts[1]}"
                else:
        ...
        # that was auto-corrected (e.g. agile.boot -> agile.<id>.boot)
    ...
    def _try_correct_command_name(name: str, ext_id: str) -> Optional[str]:
        ...
        Handles the two legacy formats used by community extensions:
          - 'agile.command'  → 'agile.{ext_id}.command'
          - '{ext_id}.command' → 'agile.{ext_id}.command'
        ...
        parts = name.split(".")
        if len(parts) == 2:
            if parts[0] == "agile" or parts[0] == ext_id:
                candidate = f"agile.{ext_id}.{parts[1]}"
    ...
    @property
    def requires_agile_version(self) -> str:
        """Get required agile version range."""
        return self.data["requires"]["agile_version"]
...
class ExtensionRegistry:
    ...
    def __init__(self, extensions_dir: Path):
        ...
        Args:
            extensions_dir: Path to .agile/extensions/ directory
        """
...
class ExtensionManager:
    ...
    def __init__(self, project_root: Path):
        ...
        self.extensions_dir = project_root / ".agile" / "extensions"
    ...
    def _collect_manifest_command_names(manifest: ExtensionManifest) -> Dict[str, str]:
        ...
        - primary commands must use the canonical `agile.{extension}.{command}` shape
    ...
    def _skill_name_for_command(command_name: str) -> str:
        ...
        if short_name.startswith("agile."):
            short_name = short_name[len("agile.") :]
        return f"agile-{short_name.replace('.', '-')}"
    ...
    def _register_extension_skills(
        ...
        def _resolve_command_ref_tokens(body: str) -> str:
            ...
            def _replacement(match: re.Match[str]) -> str:
                command_name = "agile." + match.group(1).lower().replace("_", ".")
                if is_dollar_skills_agent(selected_ai, ai_skills_enabled):
                    return "$" + command_name.replace("agile.", "agile-").replace(
                        ".", "-"
                    )
            ...
            return re.sub(
                r"__AGILE_COMMAND_([A-Z][A-Z0-9_]*)__", _replacement, body
            )
        ...
        for cmd_info in manifest.commands:
            ...
            cache_root = extension_dir / ".agile-dev" / "extension-skills"
            ...
            # etc.) to their installed .agile/extensions/<id>/ location
    ...
    def check_compatibility(
        self, manifest: ExtensionManifest, agile_version: str
    ) -> bool:
        """Check if extension is compatible with current agile version.

        Args:
            manifest: Extension manifest
            agile_version: Current agile version
        ...
        """
        required = manifest.requires_agile_version

        # Parse version specifier (e.g., ">=0.1.0,<2.0.0")
        # Defense in depth: the manifest validator now rejects a non-string
        # requires.agile_version, but this method is public and also reachable
        ...
        if not version_satisfies(agile_version, required):
            raise CompatibilityError(
                f"Extension requires agile {required}, "
                f"but {agile_version} is installed.\n"
                f"Upgrade agile with: {REINSTALL_COMMAND}"
            )
    ...
    def install_from_directory(
        ...
        # Check compatibility
        self.check_compatibility(manifest, agile_version)
        ...
        if self.registry.is_installed(manifest.id):
            if not force:
                raise ExtensionError(
                    ...
                    f"Use 'agile extension remove {manifest.id}' first, "
        ...
        # and a documented `.agile/extensions/<id>/scripts/...` invocation would fail
        ...
        # is "make .agile scripts executable" — the same idempotent invariant that
        ...
        *,
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            return self.install_from_directory(
                extension_dir, agile_version, priority=priority, force=force
            )

    def _config_root_is_contained(self, specify_dir: Path) -> bool:
        """Report whether `.agile` is a real directory inside the project.

        Checked component by component so a symlink anywhere on the path is
        rejected before it becomes the containment root. A missing `.agile`
    ...
    def scaffold_config(self, extension_id: str) -> tuple[List[str], List[str], List[str]]:
        ...
        each config template to the project's .agile/ directory. Existing
        ...
        # `.agile/extensions/<id>/<id>-config.yml`, and the bundled scripts
        # and READMEs use the same location. Writing to `.agile/<name>` put
        # the file somewhere nothing ever looks.
        config_dir = self.project_root / ".agile" / "extensions" / extension_id
        ...
        for config_entry in manifest.config:
            ...
            try:
                ...
                # foo/config.yml must land in `failed` when `.agile/foo` is a
...
class CommandRegistrar:
    ...
    def _render_markdown_command(self, frontmatter, body, ext_id):
        ...
        context_note = f"\n<!-- Extension: {ext_id} -->\n<!-- Config: .agile/extensions/{ext_id}/ -->\n"
        return (
    ...
    def register_commands_for_agent(
        ...
        context_note = f"\n<!-- Extension: {manifest.id} -->\n<!-- Config: .agile/extensions/{manifest.id}/ -->\n"
...
class ExtensionCatalog(CatalogStackBase):
    ...
    def __init__(self, project_root: Path):
        ...
        Args:
            project_root: Root directory of the agile project
        """
        self.project_root = project_root
        self.extensions_dir = project_root / ".agile" / "extensions"
    ...
    def _make_request(self, url: str):
        ...
        Delegates to :func:`agile_cli.authentication.http.build_request`.
        """
        from agile_cli.authentication.http import build_request
    ...
    def _open_url(
        ...
        Delegates to :func:`agile_cli.authentication.http.open_url`.
        *redirect_validator*, when provided, is invoked as ``(old_url, new_url)``
        ...
        """
        from agile_cli.authentication.http import open_url
    ...
    def _resolve_github_release_asset_api_url(
        ...
        Delegates to the shared helper in :mod:`agile_cli._github_http`,
        passing the ``github`` provider hosts from ``auth.json`` so GitHub
        Enterprise Server release assets resolve via ``/api/v3``.
        """
        from agile_cli._github_http import resolve_github_release_asset_api_url
        from agile_cli.authentication.http import github_provider_hosts
    ...
    def _validate_catalog_payload(self, catalog_data: Any, url: str) -> None:
        ...
        once-poisoned cache (older agile version, manual edit, upstream
    ...
    def get_active_catalogs(self) -> List[CatalogEntry]:
        ...
        Resolution order:
        1. AGILE_CATALOG_URL env var — single catalog replacing all defaults
        2. Project-level .agile/extension-catalogs.yml
        3. User-level ~/.agile/extension-catalogs.yml
        ...
        # 1. AGILE_CATALOG_URL env var replaces all defaults for backward compat
        if env_value := os.environ.get("AGILE_CATALOG_URL"):
            ...
            return [
                self._entry(
                    ...
                    description="Custom catalog via AGILE_CATALOG_URL",
                )
            ]

        # 2. Project-level config overrides all defaults
        project_config_path = self.project_root / ".agile" / self.CONFIG_FILENAME
        catalogs = self._load_catalog_config(project_config_path)
        if catalogs is not None:
            return catalogs

        # 3. User-level config
        user_config_path = Path.home() / ".agile" / self.CONFIG_FILENAME
    ...
    def _fetch_single_catalog(
        ...
        # poisoned cache (older agile version, manual edit, upstream
        # served a bad payload before the network-side guards were added)
    ...
    def download_extension(
        ...
        if ext_info.get("bundled") and not ext_info.get("download_url"):
            raise ExtensionError(
                f"Extension '{extension_id}' is bundled with agile and has no download URL. "
...
class ConfigManager:
    ...
    2. Project config (.agile/extensions/{ext-id}/{ext-id}-config.yml)
    3. Local config (.agile/extensions/{ext-id}/local-config.yml) - gitignored
    4. Environment variables (AGILE_{EXT_ID}_{KEY})
    """
    ...
    def __init__(self, project_root: Path, extension_id: str):
        ...
        self.extension_dir = project_root / ".agile" / "extensions" / extension_id
    ...
    def _sibling_extension_ids(self) -> list[str]:
        ...
        Sourced from ``ExtensionRegistry`` (``.agile/extensions/.registry``)
        ...
        absorbing its ``AGILE_<sibling>_*`` env vars into no one. The
        ...
        a longer, sibling-owned prefix (e.g. ``AGILE_GIT_HOOKS_URL`` is
        owned by ``git-hooks`` when it is co-installed with ``git``).
        """
        extensions_dir = self.project_root / ".agile" / "extensions"
        try:
    ...
    def _get_env_config(self) -> Dict[str, Any]:
        ...
        AGILE_{EXT_ID}_{SECTION}_{KEY}

        For example:
        - AGILE_JIRA_CONNECTION_URL
        - AGILE_JIRA_PROJECT_KEY
        ...
        prefix = f"AGILE_{ext_id_upper}_"
        ...
        # ``AGILE_GIT_HOOKS_URL`` begins with *both* the ``AGILE_GIT_``
        # prefix of the ``git`` extension and the ``AGILE_GIT_HOOKS_`` prefix
        ...
        for key, value in os.environ.items():
            ...
            # above. Keeps ``AGILE_GIT_HOOKS_URL`` out of the ``git``
            ...
            # malformed name (e.g. ``AGILE_<EXT>_`` with no key, or
            # consecutive underscores ``AGILE_X__Y``) so we never create an
            ...
            # AGILE_X_CONNECTION=a and AGILE_X_CONNECTION_URL=b. Guard the
...
class HookExecutor:
    ...
    def __init__(self, project_root: Path):
        ...
        self.project_root = project_root
        self.extensions_dir = project_root / ".agile" / "extensions"
        self.config_file = project_root / ".agile" / "extensions.yml"
    ...
    def _load_init_options(self) -> Dict[str, Any]:
        ...
        Uses the shared helper from agile_cli and caches values per executor
    ...
    def _skill_name_from_command(command: Any) -> str:
        """Map a command id like agile.plan to agile-plan skill name."""
        if not isinstance(command, str):
            return ""
        command_id = command.strip()
        if not command_id.startswith("agile."):
            return ""
        return f"agile-{command_id[len('agile.') :].replace('.', '-')}"
