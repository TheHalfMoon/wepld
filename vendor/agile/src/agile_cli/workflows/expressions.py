...
def _resolve_dot_path(obj: Any, path: str) -> Any:
    """Resolve a dotted path like ``steps.agile.output.file`` against *obj*.
...
def _build_namespace(context: Any) -> dict[str, Any]:
    ...
    # ad-hoc evaluator usage). The value is the same one Agile
...
def _evaluate_simple_expression(expr: str, namespace: dict[str, Any]) -> Any:
    ...
    Supports:
    - Dot-path access: ``steps.agile.output.file``
    - Comparisons: ``==``, ``!=``, ``>``, ``<``, ``>=``, ``<=``
