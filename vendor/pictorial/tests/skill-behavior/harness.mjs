 ...
 *   2. Symlinks the real .claude/skills/pictorial into the workspace so
...
function loadSkillBody() {
  ...
  // and the scripts the skill references live at .claude/skills/pictorial/
  ...
  md = md
    ...
    .replaceAll('{{scripts_path}}', '.claude/skills/pictorial/scripts')
 ...
 *
 * - `.claude/skills/pictorial` is symlinked at the SOURCE skill dir (not
 *   the built `.claude/skills/pictorial/`) so the test exercises whatever
...
export function prepareWorkspace({ files = {}, skillVersion = null } = {}) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'pictorial-skill-test-'));
  const skillDest = path.join(dir, '.claude', 'skills', 'pictorial');
...
export function makeTools(workspace, extraEnv = {}, simulatedUser = {}) {
  ...
  const tools = {
    bash: tool({
      description:
        'Run a bash command in the workspace root. Use this to invoke skill scripts (e.g. `node .claude/skills/pictorial/scripts/context.mjs`).',
    ...
    ask_user_question: tool({
      description:
        'Ask the user 1-4 structured questions and wait for answers. Use this for required Pictorial init, visual-world selection, and task-concept checkpoints instead of asking in prose.',
      inputSchema: z.object({
