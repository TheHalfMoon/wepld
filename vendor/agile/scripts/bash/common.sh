...
# Find repository root by searching upward for .agile directory
# This is the primary marker for agile projects
find_specify_root() {
    local dir="${1:-$(pwd)}"
...
# Get repository root, prioritizing .agile directory
...
get_repo_root() {
    ...
    # First, look for .agile directory (agile's own marker)
    local specify_root
...
# Safely read .agile/feature.json's "feature_directory" value.
...
read_feature_json_feature_directory() {
    local repo_root="$1"
    local fj="$repo_root/.agile/feature.json"
...
# Persist a feature_directory value to .agile/feature.json.
...
_persist_feature_json() {
    ...
    # Ensure .agile/ directory exists
    mkdir -p "$repo_root/.agile"
...
get_feature_paths() {
    ...
    # --no-persist so pure path resolution never writes .agile/feature.json,
...
get_invoke_separator() {
    ...
    local integration_json="$repo_root/.agile/integration.json"
    local separator="."
    local parsed=0
...
format_agile_command() {
    ...
    command_name="${command_name#agile.}"
    command_name="${command_name#agile-}"
...
_sorted_extension_ids() {
    ...
    if python_spec=$(_python3_command); then
        ...
        py_stderr=$(mktemp)
        if sorted_ids=$(AGILE_EXTENSIONS="$ext_dir" "${python_cmd[@]}" -c "
import json, os, re, sys
from pathlib import Path

root = Path(os.environ['AGILE_EXTENSIONS'])
registered = {}
...
#   1. .agile/templates/overrides/
#   2. .agile/presets/<preset-id>/templates/ (sorted by priority from .registry)
#   3. .agile/extensions/<ext-id>/templates/
#   4. .agile/templates/ (core)
resolve_template() {
    local template_name="$1"
    local repo_root="$2"
    local base="$repo_root/.agile/templates"
    ...
    local presets_dir="$repo_root/.agile/presets"
    if [ -d "$presets_dir" ]; then
        ...
        if [ -f "$registry_file" ] && [ "${#python_cmd[@]}" -gt 0 ]; then
            ...
            if sorted_presets=$(AGILE_REGISTRY="$registry_file" "${python_cmd[@]}" -c "
import json, re, sys, os
try:
    with open(os.environ['AGILE_REGISTRY'], encoding='utf-8') as f:
        data = json.load(f)
...
" 2>/dev/null); then
    ...
    local ext_dir="$repo_root/.agile/extensions"
    if [ -d "$ext_dir" ]; then
        ...
        if [ -n "$sorted_presets" ]; then
            while IFS= read -r preset_id; do
                ...
                if [ -f "$manifest" ]; then
                    ...
                    local parse_status
                    py_stderr=$(mktemp)
                    if result=$(AGILE_MANIFEST="$manifest" AGILE_TMPL="$template_name" "${python_cmd[@]}" -c "
import sys, os
try:
    import yaml
except ImportError:
    ...
    sys.exit(2)
try:
    with open(os.environ['AGILE_MANIFEST'], encoding='utf-8') as f:
    ...
    for t in templates:
        if t.get('name') == os.environ['AGILE_TMPL'] and t.get('type', 'template') == 'template':
