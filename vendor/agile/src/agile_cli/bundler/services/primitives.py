...
existing Agile primitive machinery so a bundle install behaves exactly as a
...
* **presets** / **extensions** — wired through their reusable managers
  ...
  with Agile install fully offline; catalog assets are fetched only when
    ...
    def _do_install(self, component: ComponentRef, *, force: bool) -> None:
        ...
        if bundled is not None:
            ...
            self._manager.install_from_directory(
                bundled, agile_version, priority, **({"force": True} if force else {})
            )
            return
        ...
        try:
            self._manager.install_from_zip(
                zip_path, agile_version, priority, **({"force": True} if force else {})
            )
        finally:
...
class _ExtensionKindManager:
    ...
    def _do_install(self, component: ComponentRef, *, force: bool) -> None:
        ...
        if bundled is not None:
            ...
            self._manager.install_from_directory(
                bundled, agile_version, priority=priority, force=force
            )
            return
        ...
        if not self._allow_network:
            raise BundlerError(
                ...
                f"disabled; re-run without --offline or install it first with "
                f"'agile extension add {component.id}'."
            )
        ...
        try:
            self._manager.install_from_zip(
                zip_path, agile_version, priority=priority, force=force
            )
        finally:
            with contextlib.suppress(Exception):
                if zip_path.exists():
...
class _WorkflowKindManager:
    ...
    def _is_bundled(workflow_id: str) -> bool:
        # A workflow that ships with Agile installs fully offline.
...
class _StepKindManager:
    ...
    def refresh(self, component: ComponentRef) -> None:
        ...
        backup_dir = Path(tempfile.mkdtemp(prefix="agile-step-refresh-")) / component.id
        try:
            if step_dir.exists():
