...
class TestVersionFlag:
    ...
    def test_version_long_flag(self):
        ...
        with patch("agile_cli.get_agile_version", return_value="1.2.3"):
    ...
    def test_version_flag_takes_precedence_over_subcommand(self):
        ...
        with patch("agile_cli.get_agile_version", return_value="0.7.2"):
...
class TestVersionCommand:
    """Test the `agile version` subcommand."""

    def test_version_features_text(self):
        """agile version --features prints local capability flags."""
        with patch("agile_cli.get_agile_version", return_value="1.2.3"):
            result = runner.invoke(app, ["version", "--features"])

        assert result.exit_code == 0
        assert "Agile CLI: 1.2.3" in result.output
    ...
    def test_version_features_json(self):
        """agile version --features --json prints machine-readable capabilities."""
        with patch("agile_cli.get_agile_version", return_value="1.2.3"):
    ...
    def test_version_json_requires_features(self):
        """agile version --json is rejected until a JSON surface exists."""
