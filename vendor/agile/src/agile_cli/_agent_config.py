...
#: ``agile init`` selects in non-interactive sessions.  Follows the existing
#: ``AGILE_INTEGRATION_*`` namespace (see ``AGILE_INTEGRATION_<KEY>_EXECUTABLE``
#: and ``AGILE_INTEGRATION_CATALOG_URL``).
DEFAULT_INIT_INTEGRATION_ENV_VAR = "AGILE_INTEGRATION_DEFAULT"
...
def resolve_default_init_integration() -> str:
    ...
    (``AGILE_INTEGRATION_DEFAULT``).  When it names a registered integration
