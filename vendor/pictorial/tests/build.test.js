...
// `../../lib/pictorial-config.mjs`, a file that lives outside `cli/engine` and
// was never copied into the bundle, so `/pictorial critique` crashed with
// "Cannot find module .../lib/pictorial-config.mjs". This walks every bundled
...
describe('bundled skill scripts are self-contained', () => {
  ...
  test('the detector bundle includes its config dependency', () => {
    expect(names.has('lib/pictorial-config.mjs')).toBe(true);
  });
...
describe('degraded-mode fallback reference generation', () => {
  ...
  const readDegraded = (provider, configDir, role) =>
    fs.readFileSync(
      path.join(DIST, provider, configDir, 'skills', 'pictorial', 'reference', 'degraded', `${role}.md`),
      'utf-8'
    );
  ...
  test('a build emits reference/degraded/<role>.md for every agent, prefix-stripped', () => {
    const dir = path.join(DIST, 'codex', '.codex', 'skills', 'pictorial', 'reference', 'degraded');
  ...
  test('generated fallbacks pass through provider-block compilation (codex keeps its block, others strip it)', () => {
    ...
    const read = (provider, configDir) =>
      fs.readFileSync(
        path.join(synthDist, provider, configDir, 'skills', 'pictorial', 'reference', 'degraded', 'synthetic.md'),
        'utf-8'
      );
...
describe('GitHub Copilot custom agent generation', () => {
  ...
  test('emits .github/agents/<name>.agent.md for every shipped agent', () => {
    ...
    expect(files).toEqual([
      'pictorial-asset-producer.agent.md',
      'pictorial-documenter.agent.md',
      'pictorial-finish-reviewer.agent.md',
      'pictorial-manual-edit-applier.agent.md',
    ]);
  });

  test('frontmatter carries only name and description, description verbatim from the source', () => {
    const source = fs.readFileSync(path.join(ROOT, 'skill', 'agents', 'pictorial-finish-reviewer.md'), 'utf-8');
    const sourceDescription = source.match(/^description:\s*(.+)$/m)[1].trim();

    const content = fs.readFileSync(path.join(AGENTS_DIR, 'pictorial-finish-reviewer.agent.md'), 'utf-8');
    const frontmatter = content.split('---')[1];
    expect(frontmatter).toContain('name: pictorial-finish-reviewer');
  ...
  test('bodies are compiled: placeholders resolved, rule markers stripped', () => {
    ...
    const assetProducer = fs.readFileSync(path.join(AGENTS_DIR, 'pictorial-asset-producer.agent.md'), 'utf-8');
    expect(assetProducer).toContain('.github/skills/pictorial/scripts');
    // A distinctive body phrase proves the agent body itself was inlined.
    const reviewer = fs.readFileSync(path.join(AGENTS_DIR, 'pictorial-finish-reviewer.agent.md'), 'utf-8');
  ...
  test('degraded fallbacks still ship for the github provider alongside the real agents', () => {
    const degradedDir = path.join(DIST, 'github', '.github', 'skills', 'pictorial', 'reference', 'degraded');
...
describe('Cursor subagent generation', () => {
  ...
  test('emits .cursor/agents/<name>.md for every shipped agent', () => {
    const files = fs.readdirSync(AGENTS_DIR).sort();
    expect(files).toEqual([
      'pictorial-asset-producer.md',
      'pictorial-documenter.md',
      'pictorial-finish-reviewer.md',
      'pictorial-manual-edit-applier.md',
    ]);
  });
  ...
  test('frontmatter maps name, description, model inherit, is_background false; readonly only on the reviewer', () => {
    for (const name of fs.readdirSync(AGENTS_DIR)) {
      ...
      if (name === 'pictorial-finish-reviewer.md') {
  ...
  test('bodies are compiled: placeholders resolved, rule markers stripped', () => {
    ...
    const assetProducer = fs.readFileSync(path.join(AGENTS_DIR, 'pictorial-asset-producer.md'), 'utf-8');
    expect(assetProducer).toContain('.cursor/skills/pictorial/scripts');
  });
});
...
describe('agent bodies resolve placeholders on every surface that ships them', () => {
  ...
  const SURFACES = [
    ...
    ['codex/.codex/skills/pictorial/agents/pictorial_asset_producer.toml', '.codex/skills/pictorial/scripts'],
    ['agents/.agents/skills/pictorial/agents/pictorial_asset_producer.toml', '.agents/skills/pictorial/scripts'],
    // Native agent files.
    ['claude-code/.claude/agents/pictorial-asset-producer.md', '.claude/skills/pictorial/scripts'],
    ['github/.github/agents/pictorial-asset-producer.agent.md', '.github/skills/pictorial/scripts'],
    ['grok/.grok/agents/pictorial-asset-producer.md', '.grok/skills/pictorial/scripts'],
    // Degraded fallback reference generated from the same agent definition.
    ['codex/.codex/skills/pictorial/reference/degraded/asset-producer.md', '.codex/skills/pictorial/scripts'],
  ];
  ...
  test('no emitted agent body carries an unresolved placeholder or a rule marker', () => {
    ...
    const emitted = [
      'codex/.codex/skills/pictorial/agents/pictorial_synthetic.toml',
      'codex/.codex/skills/pictorial/reference/degraded/synthetic.md',
      'claude-code/.claude/agents/pictorial-synthetic.md',
    ];
    ...
    const codexToml = fs.readFileSync(
      path.join(synthDist, 'codex/.codex/skills/pictorial/agents/pictorial_synthetic.toml'),
      'utf-8'
    );
    expect(codexToml).toContain('node .codex/skills/pictorial/scripts/embed-prompt.mjs');
