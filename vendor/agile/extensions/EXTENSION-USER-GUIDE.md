...
Complete guide for using Agile extensions to enhance your workflow.
...
Extensions are modular packages that add new commands and functionality to Agile without bloating the core framework. They allow you to:
...
- **Clean Core**: Keeps agile lightweight and focused
...
- Agile version 0.1.0 or higher
- A agile project (directory with `.agile/` folder)
...
`agile extension search` searches **all active catalogs** simultaneously, including the community catalog by default. Results are annotated with their source catalog and install status.
...
agile extension search --tag atlassian
```
...
agile extension search --author "Stats Perform"
```
...
agile extension search --verified
```
...
3. Check compatibility with your agile version
4. Install to `.agile/extensions/jira/`
...
```bash
# From GitHub release
agile extension add <extension-name> --from https://github.com/org/agile-ext/archive/refs/tags/v1.0.0.zip
```
...
agile extension add --dev /path/to/extension
```
...
Provided commands:
  ...
  • agile.jira.discover-fields - Discover Jira custom fields for configuration
  • agile.jira.sync-status - Sync task completion status to Jira

⚠  Configuration may be required
   Check: .agile/extensions/jira/
```
...
ls .agile/extensions/jira/

# 2. Copy template to config
cp .agile/extensions/jira/jira-config.template.yml \
   .agile/extensions/jira/jira-config.yml
...
**Example**: Jira extension hooks into `/agile.tasks`
...
To execute: `/agile.jira.specstoissues`
```
...
agile extension remove jira --keep-config

# Force removal (no confirmation)
agile extension remove jira --force
```
...
**Project config** (`.agile/extensions/jira/jira-config.yml`):
...
**Local override** (`.agile/extensions/jira/jira-config.local.yml`):
...
In addition to extension-specific environment variables (`AGILE_{EXT_ID}_*`), agile supports core environment variables:

| Variable | Description | Default |
|----------|-------------|---------|
| `AGILE_CATALOG_URL`       | Override the full catalog stack with a single URL (backward compat) | Built-in default stack |
...
export AGILE_CATALOG_URL="https://example.com/staging/catalog.json"
```
...
# Search a private catalog added via `agile extension catalog add`
agile extension search jira

# Install from a private catalog
agile extension add jira-sync
```
...
Agile uses a **catalog stack** — an ordered list of catalogs searched simultaneously. By default, two catalogs are active:
...
```text
agile extension catalog --help

 Usage: agile extension catalog [OPTIONS] COMMAND [ARGS]...
...
│ add      Add a catalog to .agile/extension-catalogs.yml.                       │
│ remove   Remove a catalog from .agile/extension-catalogs.yml.                  │
╰──────────────────────────────────────────────────────────────────────────────────╯
```
...
agile extension catalog add \
  --name "partner" \
  --priority 5 \
  https://partner.example.com/agile/catalog.json
```

This creates or updates `.agile/extension-catalogs.yml`.
...
You can also edit `.agile/extension-catalogs.yml` directly:
...
A user-level equivalent lives at `~/.agile/extension-catalogs.yml`. Project-level config takes full precedence when it contains one or more catalog entries. An empty `catalogs: []` list falls back to built-in defaults.
...
```json
{
  ...
  "catalog_url": "https://your-org.com/agile/catalog.json",
  "extensions": {
    "jira": {
      "name": "Jira Integration",
      "id": "jira",
      "description": "Create Jira issues from agile artifacts",
      "author": "Your Organization",
      "version": "2.1.0",
      "download_url": "https://github.com/your-org/agile-jira/archive/refs/tags/v2.1.0.zip",
      "repository": "https://github.com/your-org/agile-jira",
      "license": "MIT",
      "requires": {
        "agile_version": ">=0.1.0",
        "tools": [
    ...
    "internal-tool": {
      ...
      "repository": "https://github.internal.your-org.com/agile-internal",
      "license": "Proprietary",
      "requires": {
        "agile_version": ">=0.1.0"
      },
...
| ------ | ----------- | -------- |
| GitHub Pages | `https://your-org.github.io/agile-catalog/catalog.json` | Public or org-visible |
| Internal web server | `https://internal.company.com/agile/catalog.json` | Corporate network |
...
Add to `.agile/extension-catalogs.yml` in your project:

```yaml
catalogs:
  - name: "my-org"
    url: "https://your-org.com/agile/catalog.json"
    priority: 1
    install_allowed: true
```
...
agile extension catalog add \
  --name "my-org" \
  --install-allowed \
  https://your-org.com/agile/catalog.json
```
...
export AGILE_CATALOG_URL="https://your-org.com/agile/catalog.json"
```
...
| `requires.agile_version` | string | No | Version constraint |
...
```json
{
  "internal-auth": {
    ...
    "download_url": "https://artifactory.company.com/agile/internal-auth-1.0.0.zip",
    "verified": true
  }
}
```
...
Only `jira` and `github` will appear in `agile extension search`.
...
{
  "jira": {
    "download_url": "https://files.internal/agile/jira-2.1.0.zip"
  }
}
```
...
# Point agile to local catalog
export AGILE_CATALOG_URL="http://localhost:8000/catalog.json"

# Test installation
agile extension add my-new-extension
```
...
agile extension add <extension-name> --from https://github.com/someone/agile-ext/archive/v1.0.0.zip
...
1. Check spelling: `agile extension search jira`
2. Refresh catalog: `agile extension search --help`
...
1. Check if extension is installed: `agile extension list`
...
3. Reinstall extension: `agile extension remove jira && agile extension add jira`
...
1. Check extension is enabled: `agile extension list`
2. Restart coding agent (Claude Code)
3. Check command file exists:

   ```bash
   ls .claude/commands/agile.jira.*.md
   ```
...
**Error**: `Extension requires agile >=0.2.0, but you have 0.1.0`
...
1. Upgrade agile:
   ...
   ```bash
   uv tool upgrade wepld-agile
   ```

2. Install older version of extension:

   ```bash
   agile extension add <extension-name> --from https://github.com/org/ext/archive/v1.0.0.zip
   ```
...
4. Check extension requirements: `agile extension info jira`
...
- `.agile/extensions.yml` (project extension config)
- `.agile/extensions/*/jira-config.yml` (project config)

**Don't commit**:

- `.agile/extensions/.cache/` (catalog cache)
- `.agile/extensions/.backup/` (config backups)
- `.agile/extensions/*/*.local.yml` (local overrides)
- `.agile/extensions/.registry` (installation state)

Add to `.gitignore`:

```gitignore
.agile/extensions/.cache/
.agile/extensions/.backup/
.agile/extensions/*/*.local.yml
.agile/extensions/.registry
```
...
- **jira** (v1.0.0) - Jira integration
  - Config: `.agile/extensions/jira/jira-config.yml`
  - Requires: jira-mcp-server

To install: `agile extension add jira`
```
...
```yaml
# .agile/extensions/jira/jira-config.local.yml
project:
...
env:
  AGILE_JIRA_PROJECT_KEY: ${{ secrets.JIRA_PROJECT }}

- name: Create Jira Issues
  run: agile extension add jira && ...
```
...
1. Run `/agile.tasks` to generate tasks
2. Run `/agile.jira.specstoissues` to create Jira issues
3. Run `/agile.jira.sync-status` to update status
```
...
### Q: Do extensions slow down agile?
...
### Q: Can extensions modify agile core?
...
**A**: Extensions use namespaced commands (`agile.{extension}.{command}`), so conflicts are very rare. The extension system will warn you if conflicts occur.

### Q: Can I contribute to existing extensions?

**A**: Yes! Most extensions are open source. Check the repository link in `agile extension info {extension}`.

### Q: How do I report extension bugs?

**A**: Go to the extension's repository (shown in `agile extension info`) and create an issue.
...
**A**: Extension configs are in `.agile/extensions/{extension}/`. Back up this directory or commit configs to git.

---

## Support

- **Extension Issues**: Report to extension repository (see `agile extension info`)
- **Agile Issues**: <https://github.com/statsperform/agile/issues>
- **Extension Catalog**: <https://github.com/statsperform/agile/tree/main/extensions>
