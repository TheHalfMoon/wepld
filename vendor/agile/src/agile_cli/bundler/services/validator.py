...
``agile bundle validate`` reports whether a manifest is well-formed and all
...
def validate_manifest(
    ...
    if manifest.requires.agile_version:
        try:
            parse_constraint(manifest.requires.agile_version)
        except BundlerError as exc:
            report.errors.append(
                f"requires.agile_version '{manifest.requires.agile_version}' "
