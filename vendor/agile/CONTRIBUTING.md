...
Hi there! We're thrilled that you'd like to contribute to Agile. Contributions to this project are [released](https://help.github.com/articles/github-terms-of-service/#6-contributions-under-repository-license) to the public under the [project's open source license](LICENSE).
...
Activate the project virtual environment (see [Testing setup](#testing-setup) below), then install the CLI from your working tree (`uv pip install -e .` after `uv sync --extra test`) or otherwise ensure the shell uses the local `agile` binary before running the manual slash-command tests described below.
...
When working on agile:

1. Test changes with the `agile` CLI commands (`/agile.specify`, `/agile.plan`, `/agile.tasks`) in your coding agent of choice
...
> If another Agile checkout has an editable (`-e`) install registered in a
> shared/global environment, `uv run pytest` can resolve `agile_cli` to that
...
> `agile_cli` to this checkout's `src/`. This matches the gotcha documented in
...
uv pip install -e .
# Ensure the `agile` binary in this environment points at your working tree so the agent runs the branch you're testing.

# Initialize a test project using your local changes
uv run agile init <temp-dir>/agile-test --integration <agent>
cd <temp-dir>/agile-test
...
4. **Run prerequisites first** — commands that depend on earlier commands (e.g., `/agile.tasks` requires `/agile.plan` which requires `/agile.specify`) must be run in order.
...
- src/agile_cli/*.py → CLI commands (`agile init`, `agile check`, `agile extension *`, `agile preset *`); test the affected CLI command and, for init/scaffolding changes, at minimum test /agile.specify
...
- presets/*/* → test preset scaffolding via `agile init` with the preset
- pyproject.toml → packaging/bundling; test `agile init` and verify bundled assets
...
- T1: /agile.command — (reason)
- T2: /agile.command — (reason)
~~~

## AI contributions in Agile

> [!IMPORTANT]
>
> If you are using **any kind of AI assistance** to contribute to Agile,
> it must be disclosed in the pull request or issue.

We welcome and encourage the use of AI tools to help improve Agile! Many valuable contributions have been enhanced with AI assistance for code generation, issue detection, and feature definition.

That being said, if you are using any kind of AI assistance (e.g., agents, ChatGPT) while contributing to Agile,
...
- **Clear rationale** - You can explain why the change is needed and how it fits within Agile's goals
...
- Generic suggestions that don't address specific Agile needs
