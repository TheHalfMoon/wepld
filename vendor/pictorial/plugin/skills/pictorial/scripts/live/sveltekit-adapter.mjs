...
export const SVELTE_LIVE_ROOT_COMPONENT = 'src/lib/pictorial/PictorialLiveRoot.svelte';
export const SVELTE_LAYOUT_MARKER_OPEN = '<!-- pictorial-live-svelte-start -->';
export const SVELTE_LAYOUT_MARKER_CLOSE = '<!-- pictorial-live-svelte-end -->';
export const SVELTE_ROOT_IMPORT = "import PictorialLiveRoot from '$lib/pictorial/PictorialLiveRoot.svelte';";
...
const SVELTE_ROOT_IMPORT_LINE_RE = /^[ \t]*import PictorialLiveRoot from '\$lib\/pictorial\/PictorialLiveRoot\.svelte(?:\?[^']*)?';[ \t]*\r?\n?/gm;
...
export function svelteRootImportLine(rev) {
  ...
  return "import PictorialLiveRoot from '$lib/pictorial/PictorialLiveRoot.svelte?pictorial-live=" + rev + "';";
}
...
export function patchSvelteLayout(content, { rev = null } = {}) {
  ...
  if (!out.includes(importLine)) {
    ...
    // indentation; only a layout with no pictorial import gets an insert.
    let replaced = false;
...
export function buildSvelteLiveRootComponent(port, token) {
  ...
  const HOST_ID = 'pictorial-live-root';

  onMount(() => {
    let host = document.querySelector('pictorial-live-root#' + HOST_ID) || document.getElementById(HOST_ID);
    if (!host) {
      host = document.createElement('pictorial-live-root');
      host.id = HOST_ID;
      document.body.appendChild(host);
    }

    host.dataset.pictorialLiveAdapter = 'sveltekit';
    ...
    if (!root.querySelector('style[data-pictorial-live-reset]')) {
      const reset = document.createElement('style');
      reset.dataset.pictorialLiveReset = 'true';
      reset.textContent = ':host, :host *, * { box-sizing: border-box; }';
      root.appendChild(reset);
    }

    window.__PICTORIAL_LIVE_ADAPTER__ = 'sveltekit';
    window.__PICTORIAL_LIVE_UI_ROOT__ = root;
    window.__PICTORIAL_LIVE_CHROME_MOUNT__ = {
      adapter: 'sveltekit',
      version: 1,
      host,
      root,
    ...
    script.src = LIVE_URL;
    script.async = true;
    script.dataset.pictorialLiveScript = 'true';
    script.onerror = () => console.error(
      '[pictorial] live.js failed to load from ' + LIVE_URL
    ...
    return () => {
      script.remove();
      if (window.__PICTORIAL_LIVE_UI_ROOT__ === root) delete window.__PICTORIAL_LIVE_UI_ROOT__;
      if (window.__PICTORIAL_LIVE_CHROME_MOUNT__?.root === root) delete window.__PICTORIAL_LIVE_CHROME_MOUNT__;
      if (window.__PICTORIAL_LIVE_ADAPTER__ === 'sveltekit') delete window.__PICTORIAL_LIVE_ADAPTER__;
    };
  });
</script>
`;
