...
/**
 * CLI-side reader/writer for the unified `.pictorial` config.
 *
...
export function getConfigPath(root) {
  return join(root, '.pictorial', 'config.json');
}

export function getLocalConfigPath(root) {
  return join(root, '.pictorial', 'config.local.json');
}
...
function applyDetectionConfigSource(config, raw) {
  ...
  // so config round-trips (e.g. `pictorial hooks ignore-value`) preserve it.
...
/**
 * Detector filters shared by `npx @wepld/pictorial detect` and the design hook.
...
const EXCLUDE_OPEN = '# pictorial-config-ignore-start';
const EXCLUDE_CLOSE = '# pictorial-config-ignore-end';
const EXCLUDE_PATTERNS = ['.pictorial/config.local.json'];
