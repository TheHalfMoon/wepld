...
Report and repair drift between this project's Pictorial artifacts and what the installed version reads: PRODUCT.md, DESIGN.md and its `.pictorial/design.json` sidecar, `.pictorial/config.json`, persisted surface briefs, and the design hook.
...
- **Tool version.** The installed skill is older than the published one. `context.mjs` reports that at boot as `UPDATE_AVAILABLE` and `npx @wepld/pictorial update` fixes it. Not this command's job.
- **Schema drift.** An artifact was written by an older Pictorial: fields nothing reads, fields now expected, files in retired locations. Mechanical, and this command repairs most of it.
...
```
node .opencode/skills/pictorial/scripts/doctor.mjs --json
```
...
- **`auto`** carries no decision. Run `node .opencode/skills/pictorial/scripts/doctor.mjs --fix` once to apply these, then report what it moved in one line. Do not ask permission first, and do not ask about them afterward.
