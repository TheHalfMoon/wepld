...
Technical reference for Agile extension system APIs and manifest schema.
...
requires:
  agile_version: string  # Required, version specifier (>=X.Y.Z)
...
provides:
  commands:              # At least one of commands/templates/scripts/hooks/events required
    - name: string       # Required, pattern: ^agile\.[a-z0-9-]+\.[a-z0-9-]+$
...
#### `requires.agile_version`

- **Type**: string
- **Format**: Version specifier
- **Description**: Required agile version range
- **Examples**:
...
- **Format**:  `agile.{extension-id}.{command-name}`
- **Examples**: `agile.jira.specstoissues`, `agile.linear.sync`
- **Invalid**: `jira.specstoissues`, `agile.command`, `agile.jira.CreateIssues`
...
- **Events**: Defined by core agile commands
...
manifest.requires_agile_version  # str: Required agile version
...
CompatibilityError    # Incompatible with current agile version
```
...
```json
{
  ...
  "extensions": {
    "jira": {
      ...
      "enabled": true,
      "registered_commands": ["agile.jira.specstoissues", ...],
...
```python
from agile_cli.extensions import CatalogEntry
...
**Catalog config file** (`.agile/extension-catalogs.yml`):
...
- Extension context automatically injected:
  ...
  <!-- Config: .agile/extensions/{extension-id}/ -->
  ```
...
**File**: `.agile/extensions/{extension-id}/{extension-id}-config.yml`
...
4. **Environment Variables** (`AGILE_{EXTENSION}_*`)

### Environment Variable Pattern

Format: `AGILE_{EXTENSION}_{KEY}`

Examples:

- `AGILE_JIRA_PROJECT_KEY`
- `AGILE_LINEAR_API_KEY`
- `AGILE_GITHUB_TOKEN`
...
**Usage**: `agile extension list [OPTIONS]`
...
**Usage**: `agile extension catalog list`
...
**Usage**: `agile extension catalog add URL [OPTIONS]`
...
Adds a catalog entry to `.agile/extension-catalogs.yml`.

### extension catalog remove

**Usage**: `agile extension catalog remove NAME`

**Arguments**:

- `NAME` - Catalog name to remove

Removes a catalog entry from `.agile/extension-catalogs.yml`.

### extension add

**Usage**: `agile extension add EXTENSION [OPTIONS]`
...
**Usage**: `agile extension remove EXTENSION [OPTIONS]`
...
**Usage**: `agile extension search [QUERY] [OPTIONS]`
...
**Usage**: `agile extension info EXTENSION`
...
**Usage**: `agile extension update [EXTENSION]`
...
**Usage**: `agile extension enable EXTENSION`
...
**Usage**: `agile extension disable EXTENSION`
...
Raised when extension is incompatible with current agile version.

```python
from agile_cli.extensions import CompatibilityError
...
```python
from agile_cli.extensions import ExtensionError
...
```python
from agile_cli.extensions import version_satisfies
...
└── scripts/                    # (existing agile)

.claude/
└── commands/
    └── agile.{ext}.{cmd}.md  # Registered commands
```
