...
export function stageOpenAIPlugin(rootDir, distDir) {
  ...
  const codexSkillSrc = path.join(distDir, 'codex', '.codex', 'skills', 'pictorial');
  ...
  const pluginRoot = path.join(distDir, 'openai', 'pictorial');
  ...
  fs.cpSync(
    codexSkillSrc,
    path.join(pluginRoot, 'skills', 'pictorial'),
    { recursive: true },
  );
