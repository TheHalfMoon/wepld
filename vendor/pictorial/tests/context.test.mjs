 ...
 *   3. PICTORIAL_CONTEXT_DIR env var as a power-user escape hatch (only
...
beforeEach(() => {
  scratch = fs.mkdtempSync(path.join(os.tmpdir(), 'pictorial-loadctx-'));
  savedEnv = process.env.PICTORIAL_CONTEXT_DIR;
  delete process.env.PICTORIAL_CONTEXT_DIR;
});

afterEach(() => {
  if (savedEnv === undefined) delete process.env.PICTORIAL_CONTEXT_DIR;
  else process.env.PICTORIAL_CONTEXT_DIR = savedEnv;
  ...
  if (providerId) {
    ...
    fs.writeFileSync(
      ...
      fs.readFileSync(providerPath, 'utf8')
        .replace("PICTORIAL_PROVIDER_ID = 'source'", `PICTORIAL_PROVIDER_ID = '${providerId}'`),
    );
  }
...
describe('resolveContextDir', () => {
  ...
  it('uses PICTORIAL_CONTEXT_DIR as a fallback when defaults are empty (relative path)', () => {
    write('design/PRODUCT.md');
    process.env.PICTORIAL_CONTEXT_DIR = 'design';
    assert.equal(resolveContextDir(scratch), path.join(scratch, 'design'));
  });

  it('uses PICTORIAL_CONTEXT_DIR as a fallback when defaults are empty (absolute path)', () => {
    const elsewhere = fs.mkdtempSync(path.join(os.tmpdir(), 'pictorial-elsewhere-'));
    try {
      process.env.PICTORIAL_CONTEXT_DIR = elsewhere;
  ...
  it('default paths win over PICTORIAL_CONTEXT_DIR (lazy escape hatch)', () => {
  ...
  it('ignores empty PICTORIAL_CONTEXT_DIR', () => {
    write('PRODUCT.md');
    process.env.PICTORIAL_CONTEXT_DIR = '   ';
    assert.equal(resolveContextDir(scratch), scratch);
  });
...
describe('loadContext (monorepo project context)', () => {
  ...
  it('does not discover dependency or generated directories as workspace candidates', () => {
    ...
    const res = spawnSync(process.execPath, [SCRIPT_PATH], {
      cwd: scratch,
      encoding: 'utf8',
      env: { ...process.env, PICTORIAL_NO_UPDATE_CHECK: '1', PICTORIAL_NO_STALENESS_CHECK: '1' },
    });
...
describe('loadContext (pictorial projectRoots config)', () => {
  function writeSkinsConfig(extra = {}) {
    write('.pictorial/config.json', JSON.stringify({ projectRoots: ['docs/design/skins/*'], ...extra }, null, 2));
    write('PRODUCT.md', '# Root product\n');
    write('DESIGN.md', '# Root design\n');
  }
  ...
  it('extends shared projectRoots with config.local.json', () => {
    write('.pictorial/config.json', JSON.stringify({ projectRoots: ['docs/design/skins/*'] }, null, 2));
    write('.pictorial/config.local.json', JSON.stringify({ projectRoots: ['experiments/*'] }, null, 2));
    write('PRODUCT.md', '# Root product\n');
    write('DESIGN.md', '# Root design\n');
  ...
  it('does not treat an .pictorial config without projectRoots as a monorepo', () => {
    write('.pictorial/config.json', JSON.stringify({ hook: { consent: 'accepted' } }, null, 2));
    write('PRODUCT.md', '# Root product\n');
    write('docs/design/skins/marble/DESIGN.md', '# Marble design\n');
  ...
  // projectRoots pattern (positive or negated) is governed by the pictorial
  ...
  describe('composition with package-manager workspaces', () => {
    ...
    it('lets an pictorial negation exclude a package-manager workspace', () => {
      write('package.json', JSON.stringify({ private: true, workspaces: ['apps/*'] }, null, 2));
      write('.pictorial/config.json', JSON.stringify({ projectRoots: ['!apps/internal'] }, null, 2));
      write('PRODUCT.md', '# Root product\n');
      write('apps/dashboard/PRODUCT.md', '# Dashboard product\n');
    ...
    it('gives a broad pictorial pattern the boundary over a deeper package workspace', () => {
      write('package.json', JSON.stringify({ private: true, workspaces: ['apps/web/packages/ui'] }, null, 2));
      write('.pictorial/config.json', JSON.stringify({ projectRoots: ['apps/*'] }, null, 2));
      write('PRODUCT.md', '# Root product\n');
    ...
    it('falls through to package workspaces for paths pictorial does not match', () => {
      write('package.json', JSON.stringify({ private: true, workspaces: ['apps/*'] }, null, 2));
      write('.pictorial/config.json', JSON.stringify({ projectRoots: ['docs/design/skins/*'] }, null, 2));
      write('PRODUCT.md', '# Root product\n');
      write('apps/dashboard/PRODUCT.md', '# Dashboard product\n');
    ...
    it('resolves other workspaces normally when pictorial config only negates', () => {
      write('package.json', JSON.stringify({ private: true, workspaces: ['apps/*'] }, null, 2));
      write('.pictorial/config.json', JSON.stringify({ projectRoots: ['!apps/internal'] }, null, 2));
      write('PRODUCT.md', '# Root product\n');
      write('apps/dashboard/PRODUCT.md', '# Dashboard product\n');
...
describe('loadContext (PICTORIAL_CONTEXT_DIR escape hatch)', () => {
  ...
  it('reports a missing override directory as no-context, not as a crash', () => {
    process.env.PICTORIAL_CONTEXT_DIR = 'no/such/dir';
    const ctx = loadContext(scratch);
...
describe('context.mjs CLI', () => {
  it('emits NO_PRODUCT_MD directive when no PRODUCT.md is found', async () => {
    const { spawnSync } = await import('node:child_process');
    const res = spawnSync(process.execPath, [SCRIPT_PATH], { cwd: scratch, encoding: 'utf8', env: { ...process.env, PICTORIAL_NO_UPDATE_CHECK: '1' } });
    assert.equal(res.status, 0);
  ...
  describe('BUILD_PATH_DEFAULT', () => {
    ...
    it('reports a recorded preference from the shared config', () => {
      write('PRODUCT.md', '# Acme\n');
      write('.pictorial/config.json', JSON.stringify({ buildPath: 'code' }));
      assert.match(run().stdout, /BUILD_PATH_DEFAULT: code \(from \.pictorial\/config\.json\)/);
    });

    it('lets the gitignored local config win over the committed one', () => {
      write('PRODUCT.md', '# Acme\n');
      write('.pictorial/config.json', JSON.stringify({ buildPath: 'comp' }));
      write('.pictorial/config.local.json', JSON.stringify({ buildPath: 'code' }));
      assert.match(run().stdout, /BUILD_PATH_DEFAULT: code \(from \.pictorial\/config\.local\.json\)/);
    });
    ...
    it('stays silent on a value nothing reads rather than guessing at it', () => {
      write('PRODUCT.md', '# Acme\n');
      write('.pictorial/config.json', JSON.stringify({ buildPath: 'code-first' }));
      assert.equal(run().stdout.includes('BUILD_PATH_DEFAULT'), false);
    });
    ...
    describe('in a monorepo', () => {
      ...
      it('lets the workspace override the repo root, nearest root first', () => {
        writeWorkspace();
        write('.pictorial/config.json', JSON.stringify({ buildPath: 'code' }));
        write('apps/dashboard/.pictorial/config.json', JSON.stringify({ buildPath: 'comp' }));
      ...
      it('does not let the invoking workspace lend its value to the target', () => {
        ...
        write('.pictorial/config.json', JSON.stringify({ buildPath: 'code' }));
        write('apps/marketing/.pictorial/config.json', JSON.stringify({ buildPath: 'comp' }));
  ...
  it('keeps the manual-detector directive out of early context when the current provider hook is active', () => {
    const scripts = path.join(scratch, 'bundle', 'skills', 'pictorial', 'scripts');
    ...
    fs.writeFileSync(path.join(project, '.codex', 'hooks.json'), JSON.stringify({
      hooks: { Stop: [{ hooks: [{ command: 'node .agents/skills/pictorial/scripts/hook.mjs' }] }] },
    }));
    ...
    fs.mkdirSync(path.join(project, '.pictorial'), { recursive: true });
    fs.writeFileSync(path.join(project, '.pictorial', 'config.json'), JSON.stringify({ hook: { enabled: false } }));
  ...
  it('loads the only persisted surface brief as current task context', () => {
    write('PRODUCT.md', '# Acme product\n');
    write('DESIGN.md', '# Acme design\n');
    write('.pictorial/surfaces/src-pages-pricing-astro.md', `---
version: 1
...
`);
    ...
    assert.equal(res.status, 0);
    assert.match(res.stdout, /# SURFACE BRIEF \(\.pictorial\/surfaces\/src-pages-pricing-astro\.md\)/);
...
`);
    write('.pictorial/surfaces/src-pages-home-astro.md', `---
version: 1
...
`);
  ...
  it('lists candidates instead of guessing when several surface briefs exist', () => {
    ...
    for (const [slug, target] of [
      ...
      write(`.pictorial/surfaces/${slug}.md`, `---
version: 1
slug: "${slug}"
...
describe('context.mjs update check', () => {
  ...
  function setup(cacheObj, { disable = false, host } = {}) {
    ...
    const env = {
      ...process.env,
      PICTORIAL_UPDATE_CACHE: cachePath(),
      PICTORIAL_NO_UPDATE_CHECK: disable ? '1' : '',
      ...
      PICTORIAL_NO_STALENESS_CHECK: '1',
      ...(host ? { PICTORIAL_UPDATE_HOST: host } : {}),
    };
  ...
  it('respects PICTORIAL_NO_UPDATE_CHECK', () => {
