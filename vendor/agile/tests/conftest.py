"""Shared test helpers for the Agile test suite."""
...
def _has_working_bash() -> bool:
    ...
    Set AGILE_TEST_BASH=1 to force-enable bash tests regardless.
    """
    if os.environ.get("AGILE_TEST_BASH") == "1":
        return True
...
# ---------------------------------------------------------------------------
# Auth config isolation — prevents tests from reading ~/.agile/auth.json
# ---------------------------------------------------------------------------


@pytest.fixture(autouse=True)
def _isolate_auth_config(monkeypatch):
    """Ensure no test reads the real ~/.agile/auth.json."""
...
def _strip_specify_env(monkeypatch):
    """Drop any inherited AGILE_* vars for every test.

    The Python CLI's project resolver (`_require_specify_project`) now honors
    AGILE_INIT_DIR, and the shell resolvers honor AGILE_FEATURE* — so a
    developer or CI runner with any AGILE_* var exported would silently
    ...
    for key in [k for k in os.environ if k.startswith("AGILE_")]:
...
def _fake_self_upgrade_argv0(monkeypatch, tmp_path, env_name, path_parts):
    ...
    fake_specify = fake_dir / ("specify.exe" if os.name == "nt" else "agile")
...
def uv_tool_argv0(monkeypatch, tmp_path):
    ...
    if os.name == "nt":
        return _fake_self_upgrade_argv0(
            monkeypatch, tmp_path, "LOCALAPPDATA", ("uv", "tools", "wepld-agile", "bin")
        )
    return _fake_self_upgrade_argv0(
        monkeypatch,
        tmp_path,
        "HOME",
        (".local", "share", "uv", "tools", "wepld-agile", "bin"),
    )
...
def pipx_argv0(monkeypatch, tmp_path):
    ...
    if os.name == "nt":
        return _fake_self_upgrade_argv0(
            monkeypatch, tmp_path, "LOCALAPPDATA", ("pipx", "venvs", "wepld-agile", "bin")
        )
    return _fake_self_upgrade_argv0(
        monkeypatch, tmp_path, "HOME", (".local", "pipx", "venvs", "wepld-agile", "bin")
    )
