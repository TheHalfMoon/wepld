...
- [ ] T020 After any repair or reconnaissance addition changes the head, rerun both deterministic gates and obtain a fresh independently qualified exact-head correctness/engineering review regardless of whether the reviewer is internal or external. If external review is used, perform a new exact-head egress preflight before the trigger. If no independently qualified reviewer is available, record `REVIEW_BLOCKED` and stop.
- [ ] T021 Perform the final live-evidence race: re-read current GitHub PR state, base SHA, head SHA, exact changed-file set, Foundation result, trusted-base `s1-admission-integrity` result, review submissions/threads, and the exact unresolved-material-findings value; record the exact values compared before any acceptance decision.
