 ...
 *      `.pictorial/config.json` declares `projectRoots`)
 *   4. $PICTORIAL_CONTEXT_DIR (absolute or cwd-relative) — power-user
...
import { PICTORIAL_COMMAND, PICTORIAL_PROVIDER_ID } from './lib/provider.mjs';
...
// can offer `npx @wepld/pictorial update`. Everything here is best-effort and
// silent on failure: a network problem, sandbox, or missing cache must never
// block context output or print an error.

const UPDATE_HOST = (process.env.PICTORIAL_UPDATE_HOST || 'https://github.com/TheHalfMoon/wepld').replace(/\/$/, '');
const UPDATE_CACHE_PATH =
  process.env.PICTORIAL_UPDATE_CACHE || path.join(os.homedir(), '.pictorial', 'update-check.json');
...
function resolveEnvContextDir(cwd) {
  const envDir = process.env.PICTORIAL_CONTEXT_DIR;
...
// outside both the project and repo roots (PICTORIAL_CONTEXT_DIR override).
...
// itself. Pictorial `projectRoots` patterns govern every path they match:
...
// deeper folder would silently resolve there. Paths the Pictorial group does
...
function isSelectableCandidate(repoRoot, rel, patternGroups) {
  ...
  const [pictorialPatterns, packagePatterns] = patternGroups;
  if (isExcludedByWorkspacePattern(relSegments, pictorialPatterns)) return false;
  for (const pattern of pictorialPatterns) {
...
// `projectRoots` globs in .pictorial config, then package-manager workspace
// declarations. A path matched by any Pictorial pattern — positive or
// negated — is governed by the Pictorial group alone; package-manager
// patterns only apply to paths the Pictorial group does not match. Within a
// group, negations win over positives.
function readProjectPatternGroups(repoRoot) {
  return [
    readPictorialProjectRoots(repoRoot),
    [
...
function readPictorialProjectRoots(repoRoot) {
  const patterns = [];
  for (const name of ['config.json', 'config.local.json']) {
    const cfg = readJson(path.join(repoRoot, '.pictorial', name));
...
function buildUpdateDirective(localVersion, latestVersion) {
  return (
    `UPDATE_AVAILABLE: A newer Pictorial skill is available ` +
    `(installed v${localVersion}, latest v${latestVersion}). ` +
    `Mention it once, in this form: "A newer Pictorial (v${latestVersion}) is available. ` +
    `Update now? It runs \`npx @wepld/pictorial update\`." ` +
    `Do not run \`npx @wepld/pictorial update\` in this turn, whatever the user answers: it rewrites the skill files ` +
 ...
 * the agent never nags. Opt out entirely with PICTORIAL_NO_UPDATE_CHECK=1.
 */
...
function updateCheckDisabledByConfig(cwd = process.cwd()) {
  ...
  for (const name of ['config.json', 'config.local.json']) {
    try {
      const raw = JSON.parse(fs.readFileSync(path.join(cwd, '.pictorial', name), 'utf-8'));
...
async function computeUpdateDirective(now = Date.now()) {
  try {
    if (process.env.PICTORIAL_NO_UPDATE_CHECK) return null;
  ...
  if (!ctx.hasProduct) {
    ...
    const parts = ctx.hasVisualImplementation
      ? [
          ...
          `offer \`${PICTORIAL_COMMAND} init\` as a follow-up.`,
      ...
      : [
          ...
          `context and offer \`${PICTORIAL_COMMAND} init\` as a suggestion (do not block).`,
...
const HOOK_MANIFESTS_BY_PROVIDER = Object.freeze({
  ...
  github: ['.github/hooks/pictorial.json'],
  grok: ['.grok/hooks/pictorial.json'],
});
...
function valueHasHookMarker(value) {
  if (typeof value === 'string') {
    return value.includes('skills/pictorial/scripts/hook.mjs')
      || value.includes('skills/pictorial/scripts/hook-before-edit.mjs');
  }
...
function hookEnabledAt(root) {
  if (truthyEnv(process.env.PICTORIAL_HOOK_DISABLED)) return false;
  let enabled = true;
  for (const name of ['.pictorial/config.json', '.pictorial/config.local.json']) {
...
function automaticHookMode(ctx) {
  ...
  const manifests = HOOK_MANIFESTS_BY_PROVIDER[PICTORIAL_PROVIDER_ID] || [];
  ...
  for (const root of roots) {
    for (const rel of manifests) {
      ...
      if (raw?.hooks && valueHasHookMarker(raw.hooks)) {
        return STOP_REVIEW_PROVIDERS.has(PICTORIAL_PROVIDER_ID) ? 'stop' : 'per-edit';
      }
    }
  }
  return 'none';
...
function readBuildPathAt(root) {
  ...
  let source = null;
  for (const name of ['config.json', 'config.local.json']) {
    const raw = readJson(path.join(root, '.pictorial', name));
    if (raw?.buildPath === 'comp' || raw?.buildPath === 'code') {
      value = raw.buildPath;
      source = `.pictorial/${name}`;
    }
  }
...
function appendAutonomyCounterDirective(parts) {
  parts.push([
    ...
    "Pictorial's interview and decision steps stay live: probe once with the structured question tool or the decision page.",
...
function appendDetectorFallback(parts, ctx) {
  ...
  const scriptsPath = path.dirname(fileURLToPath(import.meta.url));
  parts.push([
    'MANUAL_DETECTOR_REQUIRED: No automatic Pictorial design hook is active this session.',
...
// Tier 1 staleness: schema drift in Pictorial's own project files, measured
...
function projectRootsDiagnostic(ctx, options) {
  ...
  const patterns = readPictorialProjectRoots(ctx.repoRoot);
...
function buildTargetSelectionDirective(selection) {
  return (
    ...
    'Ask the user which app Pictorial should use, then rerun Pictorial helper commands from that child app cwd using this same scripts directory. ' +
