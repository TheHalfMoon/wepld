...
function writeStaticFixture(files) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'pictorial-static-'));
...
describe('walkDir', () => {
  ...
  test('finds Blade templates during directory scans', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'pictorial-walk-'));
    try {
  ...
  // Issue #303: when pictorial (or any agent tool) is installed into a
  ...
  test('skips hidden dirs (AI-harness installs) during recursion', () => {
    ...
    try {
      ...
      write('src/app.css');
      write('.claude/skills/pictorial/scripts/detector.js');
      write('.cursor/skills/pictorial/example.css');
      write('.pictorial/live/preview.html');
...
describe('CLI', () => {
  ...
  test('local DESIGN.md enables design-system rules by default and --no-design-system disables them', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'pictorial-cli-design-system-'));
    try {
...
`);
  ...
  test('--scope filters CLI output to a design domain', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'pictorial-cli-scope-'));
    try {
      fs.writeFileSync(path.join(dir, 'DESIGN.md'), `---
typography:
  body:
...
`);
  ...
  test('detector designSystem.enabled=false disables CLI design-system rules', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'pictorial-cli-design-disabled-'));
    try {
      fs.mkdirSync(path.join(dir, '.pictorial'), { recursive: true });
      fs.writeFileSync(path.join(dir, '.pictorial', 'config.json'), JSON.stringify({
...
`);
  ...
  test('respects .pictorial config ignoreFiles like the hook', async () => {
    await withStaticFixture({
      '.pictorial/config.json': JSON.stringify({
  ...
  test('respects .pictorial config ignoreRules like the hook', async () => {
    await withStaticFixture({
      '.pictorial/config.json': JSON.stringify({
  ...
  test('respects .pictorial config ignoreValues like the hook', async () => {
    await withStaticFixture({
      '.pictorial/config.json': JSON.stringify({
        detector: {
          ignoreValues: [
