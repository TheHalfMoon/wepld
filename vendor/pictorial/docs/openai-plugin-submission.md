...
Use this packet for the initial public submission of Pictorial. The release build creates the upload at `dist/openai-plugin.zip`.

## Submission type

Choose **Skills only**. Pictorial has no MCP server. The package includes one skill, its referenced scripts and files, a branded icon, and a `PostToolUse` lifecycle hook.

The hook runs locally after supported file edits, checks changed UI files for known anti-patterns, and returns findings to the active coding-agent session. It does not call an Pictorial service. Codex asks each user to review and trust the hook before it can run.

## Listing

- Plugin name: Pictorial
- Publisher: Renaissance Geek Inc
- Short description: Design and refine interfaces
- Long description: Create, critique, and refine frontend interfaces with your coding agent. Pictorial provides 23 focused design commands, live browser iteration for exploring visual directions, and automatic checks that flag common design anti-patterns as you work.
- Category: Creativity
- Website: https://github.com/TheHalfMoon/wepld
- Support: https://github.com/TheHalfMoon/wepld/issues
- Privacy policy: https://github.com/TheHalfMoon/wepld/privacy
- Terms: https://github.com/TheHalfMoon/wepld/blob/main/LICENSE
- Logo: `dist/openai/pictorial/assets/icon.png`
...
3. start pictorial live so I can explore bolder or more delightful variants.
...
- Expected behavior: Do not invoke Pictorial. Explain that the plugin is scoped to frontend interface work and continue with suitable general coding help if available.
...
- Expected behavior: Do not use Pictorial as the asset generator. Route to image generation when available, or explain that the plugin handles interface design rather than standalone raster artwork.
...
Initial submission of Pictorial, a frontend design skill for ChatGPT and Codex. It packages one skill with 23 design workflows, referenced scripts and guidance, a local anti-pattern detector, and an optional `PostToolUse` hook. No MCP server, authentication, demo account, or private network is required. The only network request made by the skill is the documented optional daily version check to `https://github.com/TheHalfMoon/wepld/api/version`.
...
- Upload `dist/openai-plugin.zip` and confirm the scanner recognizes `skills/pictorial/SKILL.md` and `hooks/hooks.json`.
