...
OM-S6-008 depends on all of `contracts/runtime-distributed-safety-addendum.md`: authenticated enrollment, monotonic ownership epochs and enforcing fences, exact harness identity, causal event sequencing/dedupe and replay gaps, the closed ExecutionEnvelope.resource_admission union (a live matching Reserved value when reservation_requirement is REQUIRED; otherwise QualifiedNoReservation for NOT_REQUIRED with the exact qualification reference and nonempty determination evidence), negotiated protocol/schema compatibility and reconciliation before uncertain work resumes. Recovery tests include partitioned old-owner survival, stale reservations and incompatible reconnects; no recovery task may omit these because its local list is shorter.
