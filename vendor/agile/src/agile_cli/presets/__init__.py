"""
Preset Manager for Agile

Handles installation, removal, and management of Agile presets.
...
def _substitute_core_template(
    ...
    # Derive the short name (strip "agile." prefix) used by core command templates.
    short_name = cmd_name
    ...
    #    command name (e.g. agile.selftest.extension → commands/selftest.md).
...
class PresetManifest:
    ...
    def _validate(self):
        ...
        if "agile_version" not in requires:
            raise PresetValidationError("Missing requires.agile_version")
        ...
        # traceback naming no field. An unquoted ``agile_version: 1.0`` is an
...
class PresetRegistry:
    ...
    def __init__(self, packs_dir: Path):
        ...
        Args:
            packs_dir: Path to .agile/presets/ directory
        """
...
class PresetManager:
    ...
    def __init__(self, project_root: Path):
        ...
        self.presets_dir = project_root / ".agile" / "presets"
    ...
    def check_compatibility(
        ...
        """Check if preset is compatible with current agile version.
        ...
        if not version_satisfies(agile_version, required):
            raise PresetCompatibilityError(
                f"Preset requires agile {required}, "
    ...
    def _register_commands(
        ...
        # named extension is installed. Namespaced names (agile.<ns>.<cmd>)
        # are treated exactly like short names (agile.<cmd>) — they are NOT
        # filtered out just because ``.agile/extensions/<ns>/`` is absent.
    ...
    def _merge_extension_registered_commands(
        ...
        if not written:
            return
        registry = ExtensionRegistry(self.project_root / ".agile" / "extensions")
    ...
    def _reconcile_composed_commands(
        ...
        # NOT filtered by the ``agile.<ns>.<cmd>`` shape: a self-contained
            ...
            if not has_composition:
                ...
                if not registered:
                    ...
                    if source.startswith("extension:"):
                        ...
                        ext_dir = (
                            self.project_root / ".agile" / "extensions" / extension_id
                        )
                            ...
                            try:
                                ...
                                if matching_cmds:
                                    written = registrar.register_commands_for_non_skill_agents(
                                        ...
                                        self.project_root,
                                        context_note=f"\n<!-- Extension: {extension_id} -->\n<!-- Config: .agile/extensions/{extension_id}/ -->\n",
    ...
    def _register_command_from_path(
        ...
        if source_id and not source_id.startswith("preset:"):
            try:
                from ..extensions import ExtensionManifest
                for ext_dir in (self.project_root / ".agile" / "extensions").iterdir():
    ...
    def _reconcile_skills(
        ...
        # ``agile.<ns>.<cmd>`` shape here either: a self-contained preset
    ...
    def _skill_names_for_command(cmd_name: str) -> tuple[str, str]:
        ...
        modern_skill_name = f"agile-{raw_short_name.replace('.', '-')}"
        legacy_skill_name = f"agile.{raw_short_name}"
    ...
    def _skill_title_from_command(cmd_name: str) -> str:
        ...
        title_name = cmd_name
        if title_name.startswith("agile."):
            title_name = title_name[len("agile."):]
    ...
    def _resolve_skill_command_refs(
        ...
        """Render ``__AGILE_COMMAND_*__`` tokens in a skill body as invocations.

        Looks up the agent's invoke separator and rewrites each
        ``__AGILE_COMMAND_<NAME>__`` placeholder into the matching
        agent-native invocation -- ``/agile-<cmd>`` or ``$agile-<cmd>`` for
        a ``-`` separator, ``/agile.<cmd>`` for ``.``, or
        ``/skill:agile-<cmd>`` for skill-colon agents (e.g. Kimi) -- the
    ...
    def _register_skills(
        ...
        *,
        ...
        # rule _register_commands() uses. No ``agile.<ns>.<cmd>`` name-shape
        ...
        for cmd_tmpl in command_templates:
            ...
            enhanced_desc = original_desc or SKILL_DESCRIPTIONS.get(
                short_name,
                f"Agile workflow command: {short_name}",
            )
            frontmatter = dict(frontmatter)
    ...
    def _validate_skill_subdir(
        ...
        *parent* skills directory), this validates the skill's own
        subdirectory — e.g. ``.claude/skills/agile-specify`` — so a
        ...
        *,
        ...
        Args:
            restore_from_bundled_core: When True, a missing project-local
                core template (the common case — ``agile init`` never
                populates ``.agile/templates/commands``) falls back to
    ...
    def _unregister_skills_in_dir(
        ...
        *,
        ...
        core_templates_dir = self.project_root / ".agile" / "templates" / "commands"
        registrar = CommandRegistrar()
        ...
        for skill_name in skill_names:
            ...
            # Derive command name from skill name (agile-specify -> specify)
            short_name = skill_name
            if short_name.startswith("agile-"):
                short_name = short_name[len("agile-"):]
            elif short_name.startswith("agile."):
                short_name = short_name[len("agile."):]
    ...
    def _reconcile_constitution(self, *, create_if_missing: bool = False) -> None:
        ...
        memory_constitution = (
            self.project_root / ".agile" / "memory" / "constitution.md"
        )
    ...
    def install_from_archive(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            return self.install_from_directory(pack_dir, agile_version, priority, force=force)
...
class PresetCatalog:
    ...
    def get_active_catalogs(self) -> List[PresetCatalogEntry]:
        ...
        Resolution order:
        1. AGILE_PRESET_CATALOG_URL env var — single catalog replacing all defaults
        2. Project-level .agile/preset-catalogs.yml
        3. User-level ~/.agile/preset-catalogs.yml
        ...
        # 1. AGILE_PRESET_CATALOG_URL env var replaces all defaults
        if env_value := os.environ.get("AGILE_PRESET_CATALOG_URL"):
            ...
            return [PresetCatalogEntry(url=catalog_url, name="custom", priority=1, install_allowed=True, description="Custom catalog via AGILE_PRESET_CATALOG_URL")]

        # 2. Project-level config overrides all defaults
        project_config_path = self.project_root / ".agile" / "preset-catalogs.yml"
        catalogs = self._load_catalog_config(project_config_path)
        if catalogs is not None:
            return catalogs

        # 3. User-level config
        user_config_path = Path.home() / ".agile" / "preset-catalogs.yml"
...
class PresetResolver:
    ...
    Resolution order:
    1. .agile/templates/overrides/          - Project-local overrides
    2. .agile/presets/<preset-id>/          - Installed presets
    3. .agile/extensions/<ext-id>/templates/ - Extension-provided templates
    4. .agile/templates/                    - Core templates (shipped with Agile)
    """
    ...
    def __init__(self, project_root: Path):
        ...
        """
        self.project_root = project_root
        self.templates_dir = project_root / ".agile" / "templates"
        self.presets_dir = project_root / ".agile" / "presets"
    ...
    def _core_stem(template_name: str) -> Optional[str]:
        ...
        or ``None`` otherwise.
        """
        if template_name.startswith("agile."):
            return template_name[len("agile."):]
        return None
    ...
    def resolve(
        ...
        # agile's built-in command/template files and must always be checked
        # so that strategy:wrap presets can locate {CORE_TEMPLATE}.
        from agile_cli import _locate_core_pack, _repo_root  # local import to avoid cycles
        _core_pack = _locate_core_pack()
        if _core_pack is not None:
            # Wheel install path
            if template_type == "template":
    ...
    def resolve_extension_command_via_manifest(self, cmd_name: str) -> Optional[Path]:
        ...
        (e.g. ``agile.selftest.extension`` → ``commands/selftest.md``).
    ...
    def _find_bundled_core(
        ...
        ``.agile/templates/`` doesn't contain the core file.
        """
        try:
            from agile_cli import _locate_core_pack, _repo_root
        except ImportError:
            return None
