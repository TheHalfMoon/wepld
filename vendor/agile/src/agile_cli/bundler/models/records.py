...
Records are stored as JSON at ``.agile/bundle-records.json``. Each record
...
def records_path(project_root: Path) -> Path:
    return Path(project_root) / ".agile" / RECORDS_FILENAME
...
def _check_schema_version(value: Any, *, path: Path, required: bool) -> None:
    ...
    if seen.split(".")[0] != RECORDS_SCHEMA_VERSION.split(".")[0]:
        raise BundlerError(
            ...
            f"Agile understands version {RECORDS_SCHEMA_VERSION}. The file may "
...
def load_records(project_root: Path) -> list[InstalledBundleRecord]:
    ...
    # read through a symlinked or traversal-escaping ``.agile`` that resolves
