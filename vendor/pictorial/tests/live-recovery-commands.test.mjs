...
function withTempProject(fn) {
  const cwd = mkdtempSync(join(tmpdir(), 'pictorial-live-recovery-'));
  try { return fn(cwd); }
...
describe('live recovery CLI commands', () => {
  it('prints active durable session status without a running helper server', () => withTempProject((cwd) => {
    ...
    store.appendEvent({ type: 'generate', id: 'cli-recover-1', action: 'pictorial', count: 3, pageUrl: '/', element: { outerHTML: '<button>Go</button>' } });
  ...
  it('leaves every session file untouched while reporting status', () => withTempProject((cwd) => {
    const store = createLiveSessionStore({ cwd });
    store.appendEvent({ type: 'generate', id: 'cli-readonly-1', action: 'pictorial', count: 2, pageUrl: '/', element: { outerHTML: '<section>Hero</section>' } });
    store.appendEvent({ type: 'variants_ready', id: 'cli-readonly-1', file: 'src/App.jsx', arrivedVariants: 2 });

    const sessionsDir = join(cwd, '.pictorial', 'live', 'sessions');
  ...
  it('resumes the pending event and reports the next safe agent action', () => withTempProject((cwd) => {
    const store = createLiveSessionStore({ cwd });
    store.appendEvent({ type: 'generate', id: 'cli-recover-2', action: 'pictorial', count: 2, pageUrl: '/', element: { outerHTML: '<section>Hero</section>' } });
  ...
  it('reports render truth so published is never read as rendered', () => withTempProject((cwd) => {
    const store = createLiveSessionStore({ cwd });
    store.appendEvent({ type: 'generate', id: 'cli-render-1', action: 'pictorial', count: 3, pageUrl: '/', element: { outerHTML: '<section>Hero</section>' } });
  ...
  it('turns a browser mount failure into an actionable next step', () => withTempProject((cwd) => {
    const store = createLiveSessionStore({ cwd });
    store.appendEvent({ type: 'generate', id: 'cli-render-2', action: 'pictorial', count: 3, pageUrl: '/', element: { outerHTML: '<section>Hero</section>' } });
  ...
  it('marks a session completed through the canonical completion command', () => withTempProject((cwd) => {
    const store = createLiveSessionStore({ cwd });
    store.appendEvent({ type: 'generate', id: 'cli-recover-3', action: 'pictorial', count: 1, pageUrl: '/', element: { outerHTML: '<p>Copy</p>' } });
