...
When Agile needs a template (e.g. `spec-template`), the `PresetResolver` walks a priority stack and returns the first match:

```mermaid
flowchart TD
    A["resolve_template('spec-template')"] --> B{Override exists?}
    B -- Yes --> C[".agile/templates/overrides/spec-template.md"]
    B -- No --> D{Preset provides it?}
    D -- Yes --> E[".agile/presets/‹preset-id›/templates/spec-template.md"]
    D -- No --> F{Extension provides it?}
    F -- Yes --> G[".agile/extensions/‹ext-id›/templates/spec-template.md"]
    F -- No --> H[".agile/templates/spec-template.md"]
...
|----------|--------|------|----------|
| 1 (highest) | Override | `.agile/templates/overrides/` | One-off project-local tweaks |
| 2 | Preset | `.agile/presets/<id>/templates/` | Shareable, stackable customizations |
...
- **Python**: `PresetResolver.resolve_content()` in `src/agile_cli/presets.py` (templates, commands, and scripts)
...
`.agile/memory/constitution.md` once. Existing files are preserved byte-for-byte. On subsequent
...
When a preset is installed with `type: "command"` entries, the `PresetManager` registers them into all detected agent directories using the shared `CommandRegistrar` from `src/agile_cli/agents.py`.
...
flowchart TD
    ...
    E -- No --> G{"~/.agile/preset-catalogs.yml exists?"}
