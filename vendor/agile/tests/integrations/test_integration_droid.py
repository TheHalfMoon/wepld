...
from agile_cli.integrations.droid import DroidIntegration
...
class TestDroidIntegration(SkillsIntegrationTests):
    ...
    def test_is_slash_skills_agent(self):
        ...
        /agile-<name>, so is_slash_skills_agent must report True — otherwise
        hook invocations and the init next-steps panel render the dotted
        /agile.<name> form Droid never registers (mirrors grok/trae/zed/devin)."""
        from agile_cli._invocation_style import is_slash_skills_agent
...
class TestDroidInitFlow:
    ...
    def test_integration_droid_creates_skills(self, tmp_path):
        ...
        assert (target / ".factory" / "skills" / "agile-plan" / "SKILL.md").exists()
        assert (target / ".factory" / "skills" / "agile-specify" / "SKILL.md").exists()
...
class TestDroidBuildExecArgs:
    ...
    def test_default_argv_uses_exec_subcommand(self):
        ...
        must pass it through ``AGILE_INTEGRATION_DROID_EXTRA_ARGS``.
        """
        ...
        assert "--skip-permissions-unsafe" not in args, (
            "Agile must not auto-apply --skip-permissions-unsafe; "
    ...
    def test_model_is_appended(self):
        ...
        args = i.build_exec_args(
            "/agile-specify", model="claude-opus-4-7", output_json=False
        )
        assert args == [
            "droid",
            "exec",
    ...
    def test_extra_args_inserted_after_canonical_flags(self, monkeypatch):
        """Operator-injected extra args land after Agile's canonical
        ``--model`` / ``--output-format`` flags so the canonical flags are
        always present in argv regardless of operator override."""
        from agile_cli.integrations import get_integration

        i = get_integration("droid")
        monkeypatch.setenv("AGILE_INTEGRATION_DROID_EXTRA_ARGS", "--foo bar")
        args = i.build_exec_args(
            "/agile-plan", model="claude-sonnet", output_json=True
        )
    ...
    def test_executable_override(self, monkeypatch):
        """``AGILE_INTEGRATION_DROID_EXECUTABLE`` overrides argv[0]."""
        monkeypatch.setenv(
            "AGILE_INTEGRATION_DROID_EXECUTABLE", "/custom/droid"
        )
        i = get_integration("droid")
        args = i.build_exec_args("/agile-plan", output_json=False)
    ...
    def test_returns_none_when_requires_cli_is_false(self, monkeypatch):
        ...
        assert i.build_exec_args("/agile-plan") is None
...
class TestDroidCommandInvocation:
    """Skills agents use the hyphenated ``/agile-<name>`` slash form."""

    def test_build_command_invocation_uses_hyphenated_skill_name(self):
        i = get_integration("droid")
        assert i.build_command_invocation("agile.plan", "feature-x") == (
            "/agile-plan feature-x"
        )
