...
beforeEach(() => {
  cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'pictorial-surface-brief-'));
});
...
describe('surface briefs', () => {
  it('writes one clone-stable brief and resolves primary and related targets', () => {
    ...
    assert.equal(path.relative(cwd, filePath), path.join('.pictorial', 'surfaces', 'src-pages-index-astro.md'));
