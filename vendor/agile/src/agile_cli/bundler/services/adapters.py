...
* :class:`DefaultPrimitiveInstaller` dispatches component install/remove to the
  existing Agile primitive machinery in-process.
"""
...
def _validate_remote_url(source_id: str, url: str) -> None:
    ...
    Mirrors ``agile_cli.catalogs`` URL validation to avoid MITM/downgrade
    ...
    # cases. Mirrors the fix in ``agile_cli.catalogs`` (#3210).
    if not hostname:
        raise BundlerError(
...
def _http_get_json(source_id: str, url: str) -> dict:
    ...
    Routing through :func:`agile_cli.authentication.http.open_url` gives
