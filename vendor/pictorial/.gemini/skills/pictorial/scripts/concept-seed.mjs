 ...
 * service repo, evals, and tests set PICTORIAL_CATALOG_DIR), then the roll
 * API at github.com/TheHalfMoon/wepld, then a degraded assignment-only seed when both are
 ...
 * never leave the machine. DO_NOT_TRACK or PICTORIAL_NO_TELEMETRY disables
 * the ping entirely.
 *
 * Env vars:
 *   PICTORIAL_CONCEPT_SEED — same as --from; for reproducible eval runs.
 *   PICTORIAL_CATALOG_DIR  — directory holding the four catalog JSON files.
 *   PICTORIAL_API_URL      — roll API base (default https://github.com/TheHalfMoon/wepld/api).
 *   PICTORIAL_NO_TELEMETRY — disables the choice ping (DO_NOT_TRACK also honored).
 */
...
// tests point PICTORIAL_CATALOG_DIR at one), then the roll API, then a
// degraded assignment-only seed. The full catalog does not ship with the skill.
const CATALOG_DIR = process.env.PICTORIAL_CATALOG_DIR || here;
const API_BASE = (process.env.PICTORIAL_API_URL || 'https://github.com/TheHalfMoon/wepld/api').replace(/\/$/, '');
const API_TIMEOUT_MS = Number(process.env.PICTORIAL_API_TIMEOUT || 4000);
...
function requireLocalConcepts() {
  ...
  if (!local) {
    throw new Error('concept-seed: no local catalog (set PICTORIAL_CATALOG_DIR or pass sourceConcepts)');
  }
  return local;
}
...
function telemetryDisabled() {
  return Boolean(process.env.PICTORIAL_NO_TELEMETRY || process.env.DO_NOT_TRACK);
}
...
const CARD_BASE = process.env.PICTORIAL_CARD_BASE || 'https://github.com/TheHalfMoon/wepld/worlds/cards';
...
export function renderConceptSeed({
  scope = 'surface',
  key = process.env.PICTORIAL_CONCEPT_SEED || crypto.randomBytes(4).toString('hex'),
  reroll = 0,
  register = null,
  mode = null,
  grain = null,
  ...
  it is not the assigned direction, kicker PICTORIAL’S PICK, with an honest risk line
  ...
  comp-led default (.pictorial/config.json buildPath; the page toggle
...
network contact is one GET to https://github.com/TheHalfMoon/wepld/api/roll whose query
...
`;
  ...
  // widens it. PICTORIAL_COMPOSITIONS=1 re-enables rendering for catalog
  // development; the draw machinery, axes, and grain report stay intact.
  const compositionsEnabled = process.env.PICTORIAL_COMPOSITIONS === '1';
...
rivals to your habitual layout, and keep only what makes this product clearer.${grainNote}\n`
  ...
  or PICTORIAL_NO_TELEMETRY is set.\n`
    : '';
  ...
  try {
    if (chosenIdx !== -1 || kindIdx !== -1) {
      ...
      process.stdout.write(await renderConceptSeed({
        ...
        key: fromIdx !== -1
          ? args[fromIdx + 1]
          : (process.env.PICTORIAL_CONCEPT_SEED || crypto.randomBytes(4).toString('hex')),
