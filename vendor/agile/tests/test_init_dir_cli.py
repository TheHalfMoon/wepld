...
def _make_project(root, name):
    """Create <root>/<name>/.agile (the minimal Agile project marker)."""
    proj = root / name
    (proj / ".agile").mkdir(parents=True)
    return proj
...
# resolves the project, then reads <project>/.agile/workflows/. An empty
...
def test_override_symlinked_specify_errors_bundle_init_no_fallback(tmp_path, monkeypatch):
    """A symlinked override .agile must not make bundle init fall back to cwd."""
    web = tmp_path / "web"
    web.mkdir()
    real = tmp_path / "real-specify"
    real.mkdir()
    try:
        (web / ".agile").symlink_to(real, target_is_directory=True)
    ...
    assert not (elsewhere / ".agile").exists()


def test_override_without_specify_errors_no_fallback(tmp_path, monkeypatch):
    """A path that exists but lacks .agile/ hard-errors, no fallback."""
    cwd_proj = _make_project(tmp_path, "cwd")
...
def test_override_redirects_workflow_run_file(tmp_path, monkeypatch):
    ...
    assert result.exit_code == 0, result.output
    assert (web / ".agile" / "workflows" / "runs").is_dir()
    assert not (elsewhere / ".agile").exists()  # cwd was not used as the project
...
def test_override_rejects_symlinked_specify(tmp_path, monkeypatch):
    """`workflow run <file>` refuses a symlinked .agile under the override
...
def test_override_rejects_symlinked_specify_json_error_stays_off_stdout(tmp_path, monkeypatch):
    ...
    assert result.exit_code != 0
    assert result.stdout == ""
    assert "Refusing to use symlinked .agile path" in result.stderr
