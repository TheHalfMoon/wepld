...
/**
 * `pictorial skills` subcommand
 *
 * Usage:
 *   pictorial help      Show all available skills and commands
 *   pictorial install   Install compiled skills from the universal bundle
 *   pictorial link      Symlink compiled skills from a local checkout
 *   pictorial update    Update skills to latest version
 */
...
import { getHookConsent, setHookConsent } from '../../lib/pictorial-config.mjs';

const __dirname = dirname(fileURLToPath(import.meta.url));
const API_BASE = 'https://github.com/TheHalfMoon/wepld';
...
const PICTORIAL_HOOK_COMMAND_MARKERS = [
  'skills/pictorial/scripts/hook-probe.mjs',
  'skills/pictorial/scripts/hook.mjs',
  'skills/pictorial/scripts/hook-before-edit.mjs',
  'skills/pictorial/scripts/hook-after-edit.mjs',
  'skills/pictorial/scripts/hook-stop.mjs',
];
const PROVIDER_HOOK_ARTIFACTS = {
  ...
  '.github': [
    { sourceProvider: '.github', rel: 'hooks/pictorial.json', destProvider: '.github' },
  ],
  ...
  '.grok': [
    { sourceProvider: '.grok', rel: 'hooks/pictorial.json', destProvider: '.grok' },
  ],
};
...
class PromptAbortError extends Error {
  constructor() {
    ...
    this.code = 'PICTORIAL_PROMPT_ABORT';
  }
}

function isPromptAbortError(error) {
  return error?.code === 'PICTORIAL_PROMPT_ABORT';
}
...
async function showHelp() {
  ...
  try {
    ...
    console.error('Could not fetch command list from github.com/TheHalfMoon/wepld. Check your network connection.');
    process.exit(1);
  }
...
/**
 * Read the skills version from the pictorial SKILL.md frontmatter.
 */
function getSkillsVersion(root, scope) {
  for (const d of PROVIDER_DIRS) {
    for (const skillsDir of providerSkillsDirCandidates(root, d, scope)) {
      const skillMd = join(skillsDir, 'pictorial', 'SKILL.md');
...
async function downloadAndExtractBundle() {
  const localBundle = process.env.PICTORIAL_BUNDLE_PATH;
  if (localBundle) return copyOrExtractLocalBundle(localBundle);

  const tmpZip = join(tmpdir(), `pictorial-update-${Date.now()}.zip`);
  const tmpDir = join(tmpdir(), `pictorial-update-${Date.now()}`);
...
async function copyOrExtractLocalBundle(sourceValue) {
  ...
  const tmpDir = join(tmpdir(), `pictorial-local-bundle-${process.pid}-${Date.now()}`);
  mkdirSync(tmpDir, { recursive: true });
 ...
 * that aren't part of pictorial). Deduplicates providers that share the same
...
// Check if pictorial skills are already present in any provider folder
function isAlreadyInstalled(root, scope) {
  for (const d of PROVIDER_DIRS) {
    for (const skillsDir of existingSkillsDirs(root, d, scope)) {
      try {
        const entries = readdirSync(skillsDir);
        // Look for 'pictorial' skill (or prefixed variant, or legacy 'teach-pictorial')
        if (entries.some(e =>
          e === 'pictorial' || e.endsWith('-pictorial') ||
          e === 'teach-pictorial' || e.endsWith('-teach-pictorial')
        )) {
          return d;
        }
      } catch {}
 ...
 * prefix (default `i-`), renaming the skill to e.g. `i-pictorial`. The prefix
 * only earned its keep when every command was its own skill; with a single
 * `pictorial` skill it does nothing, so it is no longer offered. Rename any
 * prefixed pictorial skill back to the canonical `pictorial` (the fresh
 * install/update content lands there next) so users aren't left with a stale,
 * orphaned `i-pictorial` alongside the new one. Scoped to the pictorial skill
 * by name -- never touches third-party skills that happen to start with `i-`.
 * Returns the number of skills migrated.
 */
function migrateUnprefixPictorial(root, scope) {
  let migrated = 0;
  for (const d of PROVIDER_DIRS) {
    for (const skillsDir of existingSkillsDirs(root, d, scope)) {
      let entries;
      ...
      for (const name of entries) {
        // A prefixed pictorial skill is `<prefix>pictorial`, not the canonical
        // `pictorial` and not an unrelated legacy skill name.
        if (name === 'pictorial' || name === 'teach-pictorial') continue;
        if (!name.endsWith('-pictorial')) continue;
        if (!isRealSkillDir(skillsDir, name)) continue;

        const dest = join(skillsDir, 'pictorial');
        try {
...
function printInstallIntro() {
  ...
  console.log(`${ui.accent(ui.bold('pictorial'))} ${ui.dim('install')}`);
  console.log('');
}
...
// skills. Claude Code's live at `.claude/agents/pictorial-*.md`; project
// agents take precedence over user agents. GitHub Copilot's live at
// `.github/agents/pictorial-*.agent.md`:
...
// the project one, so both paths overwrite existing pictorial-* copies and a
// project install reports any same-named user-level agents that would shadow
// it. Cursor's live at `.cursor/agents/pictorial-*.md`, user scope
...
function hookScriptRelPathForProvider(provider) {
  ...
  const rel = `${provider}/skills/pictorial/scripts/${script}`;
...
function hookScriptPathForProvider(skillRoot, provider) {
  ...
  // pictorial.json`) is a committed, team-shared file that the Copilot cloud
  ...
  if (provider === '.cursor') {
    return join(skillRoot, provider, 'skills', 'pictorial', 'scripts', 'hook-before-edit.mjs');
  }
  if (provider === '.claude' || provider === '.agents') {
    return join(skillRoot, provider, 'skills', 'pictorial', 'scripts', 'hook.mjs');
  }
  return null;
}
...
function rewriteHookCommandsForSkillRoot(value, provider, { skillRoot, absolute }) {
  ...
  if (typeof value === 'string') {
    if (!valueHasPictorialHookMarker(value)) return value;
  ...
  if (value && typeof value === 'object') {
    ...
    if (provider === '.agents' && typeof value.command === 'string' && valueHasPictorialHookMarker(value.command)) {
...
// Whether a hook manifest file actually wires up the Pictorial hook. We parse
// the JSON and scan only the `hooks` subtree (via valueHasPictorialHookMarker),
...
function fileHasPictorialHookMarker(file) {
  ...
  return valueHasPictorialHookMarker(parsed.hooks);
}
...
// Pictorial marker — not mere file existence — because the target files
...
function hookInstalledForProvider(root, provider) {
  ...
  return artifacts.every(({ destProvider, rel, destRel }) => {
    const writeRel = destRel || rel;
    if (fileHasPictorialHookMarker(join(root, destProvider, writeRel))) return true;
    if (writeRel !== rel && fileHasPictorialHookMarker(join(root, destProvider, rel))) return true;
    return false;
  });
}

function valueHasPictorialHookMarker(value) {
  if (typeof value === 'string') {
    const normalized = value.replace(/\\/g, '/');
    return PICTORIAL_HOOK_COMMAND_MARKERS.some(marker => normalized.includes(marker));
  }
  if (Array.isArray(value)) return value.some(valueHasPictorialHookMarker);
  if (value && typeof value === 'object') {
    return Object.values(value).some(valueHasPictorialHookMarker);
  }
  return false;
}

function stripPictorialHookEntry(entry) {
  ...
  if (valueHasPictorialHookMarker(entry.command) || valueHasPictorialHookMarker(entry.args)
    || valueHasPictorialHookMarker(entry.bash) || valueHasPictorialHookMarker(entry.powershell)) {
    return null;
  }
  if (!Array.isArray(entry.hooks)) return entry;

  const strippedHooks = entry.hooks
    .map(stripPictorialHookEntry)
    .filter(Boolean);

  if (strippedHooks.length === 0 && entry.hooks.some(valueHasPictorialHookMarker)) {
    return null;
  }
...
function stripPictorialHookEntries(entries) {
...
function prunePictorialHookFromManifest(manifestPath) {
  if (!fileHasPictorialHookMarker(manifestPath)) return false;
  let parsed;
  try {
  ...
  for (const [event, entries] of Object.entries(existingHooks)) {
    const kept = stripPictorialHookEntries(entries);
...
function mergeHookManifests(existing, fresh) {
  ...
  for (const event of hookEvents) {
    const preserved = stripPictorialHookEntries(existingHooks[event]);
...
function copyProviderHooks(bundleDir, root, providers, { force = false, skillRoot = root } = {}) {
  ...
  for (const provider of targets) {
    for (const { src, dest, sharedDest } of hookArtifactsForProvider(bundleDir, root, provider)) {
      ...
      // twice per edit.
      if (sharedDest && fileHasPictorialHookMarker(sharedDest)) {
        prunePictorialHookFromManifest(dest);
        continue;
      }
...
const HOOK_EXPLAINER = [
  '',
  'Pictorial can install a design hook for this project. In Claude/Codex it',
  ...
  '/pictorial hooks on|off.',
  '',
].join('\n');

// Decide whether to install the design hook. Prompts once (default yes) the
// first time, records the answer in .pictorial/config.local.json, and never
...
function resolveLinkSource(sourceValue, root) {
  const sourcePath = sourceValue || '.pictorial';
...
async function link(flags) {
  ...
  if (!yes) {
    ...
    const ans = await ask(`Link pictorial skills into ${targets.length} folder(s)? (Y/n) `);
    if (ans === 'n' || ans === 'no') {
  ...
  console.log(`Linked pictorial into: ${targets.join(', ')} (${parts.join(', ')}).`);
...
async function install(flags) {
  ...
  if (existing && !force && missingSelectedTargets.length < targets.length) {
    console.log(`Pictorial skills are already installed (found in ${existing}/).`);
    ...
    try {
      ...
      if (!updateCheckSkipped && copyTargets.length > 0 && !isUpToDate(installRoot, copyTargets, bundleDir, scope)) {
        migrateUnprefixPictorial(installRoot, scope);
      ...
      if (!updateCheckSkipped && missingSelectedTargets.length > 0) {
        ...
        console.log(`Installed pictorial into: ${missingSelectedTargets.join(', ')} (${scope === 'user' ? 'global' : 'project'})`);
  ...
  // canonical `pictorial` dir instead of orphaning the prefixed one.
  migrateUnprefixPictorial(installRoot, scope);
  ...
  console.log(`Installed pictorial into: ${targets.join(', ')} (${scope === 'user' ? 'global' : 'project'})`);
  reportProviderAgents(agentResults);
  if (hookTargets.length > 0) console.log(`Installed hooks into: ${hookTargets.join(', ')}`);

  console.log('\nDone! Now type /pictorial init in your AI coding agent\'s chat (not in this terminal) to set up design context.\n');
}
...
// actually holds the PICTORIAL skill (canonical, prefixed, or legacy teach-).
// `update` uses this so it never mistakes a repo that vendors OTHER first-party
// skills under .claude/skills for an pictorial install and drops a copy in
// (issue #399, part 2).
function findPictorialProviders(root, scope) {
  const found = [];
  for (const d of PROVIDER_DIRS) {
    for (const skillsDir of existingSkillsDirs(root, d, scope)) {
      let entries;
...
function resolveUpdateTarget({ projectRoot, home, explicitScope }) {
  ...
  const projectProviders = homeRooted ? [] : findPictorialProviders(projectRoot, 'project');
  const userProviders = findPictorialProviders(home, 'user');
...
function findLinkedProviders(root, providers, scope) {
  return providers.filter(provider => {
    for (const skillsDir of providerSkillsDirCandidates(root, provider, scope)) {
      const skillDir = join(skillsDir, 'pictorial');
      try {
...
async function update(flags = []) {
  ...
  // Download the latest skills directly from github.com/TheHalfMoon/wepld.
  ...
  if (!target) {
    if (explicitScope) {
      ...
      console.log(`No pictorial skill folders found at the ${where}.`);
    } else {
  ...
  if (target.ambiguous) {
    console.log('Pictorial is installed both here and at the user level:');
  ...
  try {
    ...
    // canonical `pictorial` dir rather than orphaning the prefixed copy.
    const migrated = migrateUnprefixPictorial(root, scope);
    if (migrated > 0) console.log('Migrated a prefixed install back to /pictorial (the i- prefix is no longer used).');
...
export async function run(args) {
  ...
  if (!sub || sub === 'help' || sub === '--help' || sub === '-h') {
    ...
    console.error(`Run 'pictorial --help' for available commands.`);
    process.exit(1);
  }
}
