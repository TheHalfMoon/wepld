...
def bash_cmd(repo: Path, script: str, *args: str) -> list[str]:
    return ["bash", str(repo / ".agile" / "scripts" / "bash" / f"{script}.sh"), *args]
...
def py_cmd(repo: Path, script: str, *args: str) -> list[str]:
    ...
    return [
        sys.executable,
        str(repo / ".agile" / "scripts" / "python" / f"{py_name}.py"),
        *args,
    ]
...
def ps_cmd(repo: Path, script: str, *args: str) -> list[str]:
    ...
    return [
        ...
        "-NoProfile",
        "-File",
        str(repo / ".agile" / "scripts" / "powershell" / f"{script}.ps1"),
        *args,
    ]
...
def install_composition_stack(
    ...
    for preset_id, priority, strategy, content in layers:
        preset_dir = repo / ".agile" / "presets" / preset_id
...
def normalize_script_names(text: str, repo: Path, script: str) -> str:
    ...
    py_name = script.replace("-", "_")
    bash_script = str(repo / ".agile" / "scripts" / "bash" / f"{script}.sh")
    py_script = str(repo / ".agile" / "scripts" / "python" / f"{py_name}.py")
