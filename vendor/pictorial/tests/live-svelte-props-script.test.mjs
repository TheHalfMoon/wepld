...
describe('the real scaffolder writes variants that parse', () => {
  ...
  beforeEach(() => {
    scratch = fs.mkdtempSync(path.join(os.tmpdir(), 'pictorial-svelte-scaffold-'));
    ...
    // Symlinking the whole directory pointed `node_modules/.pictorial-live` at
  ...
  for (const [label, originalLines] of Object.entries(CASES)) {
    it(label, () => {
      ...
      const dir = path.join(scratch, 'node_modules', '.pictorial-live', sessionId);
