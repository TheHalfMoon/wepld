...
/**
 * Pictorial Live Variant Mode - Browser Script
 *
 * Injected into the user's page via <script src="http://localhost:PORT/live.js">.
 * The server prepends window.__PICTORIAL_TOKEN__ and window.__PICTORIAL_PORT__
 * before this code.
 *
...
(function () {
  ...
  if (window.__PICTORIAL_LIVE_INIT__) return;
  window.__PICTORIAL_LIVE_INIT__ = true;

  const TOKEN = window.__PICTORIAL_TOKEN__;
  const PORT = window.__PICTORIAL_PORT__;
  const APP_ROOT = window.__PICTORIAL_APP_ROOT__ || null;
  if (!TOKEN || !PORT) {
    window.__PICTORIAL_LIVE_INIT__ = false; // reset so the real load can init
    return;
  }
  ...
  const PREFIX = 'pictorial-live';
  const PICTORIAL_COMMAND = (window.__PICTORIAL_COMMAND_PREFIX__ || '/') + 'pictorial';
  const PICK_CURSOR_STYLE_ID = PREFIX + '-pick-cursor-style';
  const MANUAL_APPLY_STATE_TTL_MS = 15 * 60 * 1000;
  const sessionState = window.__PICTORIAL_LIVE_SESSION__?.createLiveBrowserSessionState({
    prefix: PREFIX,
    storage: localStorage,
    idFactory: () => crypto.randomUUID().replace(/-/g, '').slice(0, 8),
  });
  if (!sessionState) {
    console.error('[pictorial] live-browser-session.js was not loaded. Live mode cannot start safely.');
    window.__PICTORIAL_LIVE_INIT__ = false;
    return;
  }
  ...
  // window.__PICTORIAL_VOCAB__ when it serves /live.js (same injection path as
  ...
  const VOCAB = Array.isArray(window.__PICTORIAL_VOCAB__) ? window.__PICTORIAL_VOCAB__ : [];
  const ICONS = {};
  ...
  // script, so it cannot import that module; the private pictorial-site repo
  ...
  const LIVE_CHROME_MOUNT_CONTRACT = Array.isArray(window.__PICTORIAL_LIVE_MOUNT_CONTRACT__)
    ? window.__PICTORIAL_LIVE_MOUNT_CONTRACT__
    : ['root', 'transport', 'state', 'actions'];
  const LIVE_UI_SURFACES = Array.isArray(window.__PICTORIAL_LIVE_UI_SURFACES__)
    ? window.__PICTORIAL_LIVE_UI_SURFACES__
    : [];
  ...
  const SCROLL_ANCHOR_LOCK_ID = 'pictorial-scroll-anchor-lock';
  const VARIANT_STATE_STYLE_ID = 'pictorial-variant-state';
  const DISCARD_STATE_STYLE_ID = 'pictorial-discard-state';
  ...
  const domHelpers = window.__PICTORIAL_LIVE_DOM__?.createLiveBrowserDomHelpers({
    prefix: PREFIX,
    skipTags: SKIP_TAGS,
    document,
  });
  if (!domHelpers) {
    console.error('[pictorial] live-browser-dom.js was not loaded. Live mode cannot start safely.');
    window.__PICTORIAL_LIVE_INIT__ = false;
    return;
  }
  const {
  ...
  window.__PICTORIAL_LIVE_CHROME_CORE__ = {
    version: 1,
    adapter: window.__PICTORIAL_LIVE_ADAPTER__ || 'dom',
  ...
  function showHighlight(el) {
    ...
    if (el.hasAttribute?.('data-pictorial-insert-placeholder')) return;
  ...
  function onAnnotDown(e) {
    ...
    const resizeEdge = e.target.closest?.('[data-pictorial-placeholder-resize]')?.dataset.pictorialPlaceholderResize;
  ...
  function stripManualEditRuntimeState(root) {
    ...
    const nodes = [root, ...root.querySelectorAll('[data-pictorial-editable], [data-pictorial-original-text], [data-pictorial-text-wrap]')];
    for (const node of nodes) {
      const runtimeEditable = node.hasAttribute('data-pictorial-editable')
        || node.hasAttribute('data-pictorial-original-text');
      node.removeAttribute('data-pictorial-editable');
      node.removeAttribute('data-pictorial-original-text');
      node.removeAttribute('data-pictorial-text-wrap');
      if (runtimeEditable) {
  ...
  function selectionTagLabel(el) {
    if (!el) return '';
    if (el.hasAttribute?.('data-pictorial-insert-placeholder')) return 'slot';
  ...
  function applyPlaceholderSizingStyles(placeholder, sizing) {
    placeholder.dataset.pictorialPlaceholderWidth = sizing.kind;
  ...
  function materializePlaceholderWidth(placeholder) {
    ...
    const kind = placeholder.dataset.pictorialPlaceholderWidth;
    ...
    placeholder.dataset.pictorialPlaceholderWidth = 'explicit';
  }
  ...
  function setLiveState(next) {
    state = next;
    window.__PICTORIAL_LIVE_STATE__ = next;
    syncPageInteractionCursor();
  ...
  function resolveBarAnchor() {
    ...
    if (currentSessionId && (state === 'GENERATING' || state === 'CYCLING')) {
      const wrapper = document.querySelector('[data-pictorial-variants="' + currentSessionId + '"]');
      if (wrapper) {
        const variantCount = wrapper.querySelectorAll('[data-pictorial-variant]:not([data-pictorial-variant="original"])').length;
  ...
  function isInsertGeneratingSession() {
    ...
    const wrapper = document.querySelector('[data-pictorial-variants="' + currentSessionId + '"]');
    return !!wrapper && wrapper.dataset.pictorialMode === 'insert';
  }
  ...
  function buildPlaceholderResizeHandles() {
    ...
    for (const spec of specs) {
      ...
      handle.dataset.pictorialPlaceholderResize = spec.edge;
  ...
  function createInsertPlaceholder(anchor, position, layoutAxis) {
    ...
    placeholder.setAttribute('data-pictorial-insert-placeholder', 'true');
  ...
  function ensureConfigureInputStyle() {
    ...
    s.textContent =
      '@keyframes pictorial-configure-voice-pulse { 0%, 100% { opacity: 0.55; } 50% { opacity: 1; } }' +
      ...
      '#' + PREFIX + '-configure-voice[data-listening="true"] svg, #' + PREFIX + '-insert-voice[data-listening="true"] svg { animation: pictorial-configure-voice-pulse 1.1s ease-in-out infinite; }' +
    ...
    if (hasParams || paramsPending) {
      ...
      if (paramsPending) {
        const spinner = el('span', {
          ...
          animation: 'pictorial-spin 0.6s linear infinite',
  ...
  function recoverEmptyCycling(reason) {
    ...
    try {
      console.warn('[pictorial] Refusing to render empty variant cycling state:', reason);
  ...
  // variant wrapper:
  //
  //   <div data-pictorial-variant="1"
  //        data-pictorial-params='[{"id":"density","kind":"steps",...}]'>
  //
  ...
  function getVisibleVariantEl() {
    ...
    return wrapper.querySelector('[data-pictorial-variant="' + visibleVariant + '"]');
  }

  function parseVariantParams(variantEl) {
    // Svelte component variants can't carry a `data-pictorial-params` attribute:
    ...
    if (!variantEl) return [];
    const raw = variantEl.getAttribute('data-pictorial-params');
    if (!raw) return [];
    try {
      const parsed = JSON.parse(raw);
      return Array.isArray(parsed) ? parsed : [];
    } catch (err) {
      console.warn('[pictorial] Invalid data-pictorial-params JSON:', err.message);
      return [];
    }
  }
  ...
  function applyParamValue(variantEl, param, value) {
    ...
    // [data-pictorial-component-mount] with no [data-pictorial-variant="N"]
  ...
  function wrapMixedContentTextNodes(rootEl) {
    ...
    if (hasText && hasElement) {
      for (const node of children) {
        if (node.nodeType === 3 && /\S/.test(node.nodeValue || '')) {
          ...
          wrap.dataset.pictorialTextWrap = 'true';
    ...
    for (const child of Array.from(rootEl.children)) {
      if (!child.dataset || !child.dataset.pictorialTextWrap) {
  ...
  function unwrapMixedContentTextNodes(rootEl) {
    if (!rootEl || rootEl.nodeType !== 1) return;
    const wraps = rootEl.querySelectorAll('[data-pictorial-text-wrap="true"]');
  ...
  function enableInlineEdit(targetEl) {
    ...
    for (const row of rows) {
      ...
      row.el.dataset.pictorialEditable = 'true';
      row.el.dataset.pictorialOriginalText = row.text;
  ...
  function disableInlineEdit(opts = {}) {
    for (const row of inlineEditRows) {
      ...
      delete row.el.dataset.pictorialEditable;
      delete row.el.dataset.pictorialOriginalText;
  ...
  function restoreInlineEditDrafts() {
    for (const row of inlineEditRows) {
      if (inlineEditDrafts.has(row.el)) {
        row.el.textContent = row.el.dataset.pictorialOriginalText;
      }
    }
  }
  ...
  function documentRefClassSuffix(el) {
    ...
    for (const cls of el.classList) {
      if (!cls || cls.indexOf('pictorial-') === 0) continue;
  ...
  function copyEditLeafContext(el, originalText, newText) {
    ...
    return {
      ...
      id: el.id || null,
      classes: el.classList ? [...el.classList].filter((cls) => cls.indexOf('pictorial-') !== 0) : [],
      originalText,
      newText,
      textContent: (el.textContent || '').slice(0, 500),
  ...
  function nearbyEditableTextsForManualEdit(rows, activeEl, originalText, newText) {
    ...
    for (const row of rows || []) {
      ...
      out.push({
        ...
        classes: row.el?.classList ? [...row.el.classList].filter((cls) => cls.indexOf('pictorial-') !== 0) : [],
        text,
      });
  ...
  async function applyEditing() {
    ...
    try {
      ...
      console.error('[pictorial] manual edit stash failed:', err);
  ...
  function ensureSpinKeyframes() {
    ...
    style.textContent = '@keyframes pictorial-spin { to { transform: rotate(360deg); } }';
    uiAppendStyle(style);
  }
  ...
  async function fetchPendingCount() {
    try {
      ...
      console.warn('[pictorial] failed to fetch pending count:', err);
    }
  }
  ...
  async function onPendingPillClick() {
    ...
    try {
      ...
      if (result.failed && result.failed.length > 0) {
        console.warn('[pictorial] some copy edits failed:', result.failed);
        ...
        if (n > 0) {
          ...
          console.warn('[pictorial] apply returned no verified edits:', result);
          showToast('No edits applied - see console', 4000);
        }
      }
    } catch (err) {
      console.error('[pictorial] commit failed:', err);
  ...
  async function onPendingTrashClick() {
    ...
    try {
      ...
      console.error('[pictorial] discard failed:', err);
  ...
  async function onPendingKeepFixingClick() {
    ...
    try {
      ...
      console.error('[pictorial] repair retry failed:', err);
  ...
  async function onPendingRollbackClick() {
    ...
    try {
      ...
      console.error('[pictorial] manual Apply rollback failed:', err);
  ...
  function restoreDiscardedManualEdits(entries) {
    ...
    if (failures > 0) {
      console.warn('[pictorial] skipped unsafe copy edit DOM restore for', failures, 'edit(s). Refresh to reset the page DOM.');
    }
    return failures;
  }
  ...
  function mixedTextWrapRestoreHint(el) {
    if (!el || !el.dataset || el.dataset.pictorialTextWrap !== 'true' || !el.parentElement) return null;
  ...
  function directMixedTextRestoreNodes(parent) {
    return Array.from(parent?.childNodes || []).filter((node) => {
      ...
      return node.nodeType === 1
        && node.dataset
        && node.dataset.pictorialTextWrap === 'true'
  ...
  function syncEditBadgeHitProxies() {
    ...
    for (const target of targets) {
      ...
      if (!proxy) {
        ...
        proxy.type = 'button';
        proxy.tabIndex = -1;
        proxy.dataset.pictorialEditBadgeProxy = 'true';
  ...
  function initEditBadge() {
    ...
    if (!uiGetById(PREFIX + '-edit-badge-focus-style')) {
      ...
      s.textContent =
        ...
        '[data-pictorial-editable="true"] { outline: none !important; box-shadow: none !important; }' +
        '[data-pictorial-editable="true"]:focus { outline: none !important; box-shadow: none !important; }' +
        '[data-pictorial-editable="true"]:focus-visible { outline: none !important; box-shadow: none !important; }';
      uiAppendStyle(s);
    }
  }
  ...
  function mountedParameterCount() {
    ...
    const wrapper = document.querySelector('[data-pictorial-variants="' + currentSessionId + '"]');
    if (!wrapper) return 0;
    return [...wrapper.querySelectorAll('[data-pictorial-variant]:not([data-pictorial-variant="original"])')]
  ...
  async function showVariantInDOM(sessionId, num) {
    ...
    const wrapper = document.querySelector('[data-pictorial-variants="' + sessionId + '"]');
  ...
  function parseOriginalMarkupElement(originalMarkup) {
    ...
    const doc = parser.parseFromString('<div id="pictorial-anchor">' + originalMarkup + '</div>', 'text/html');
    return doc.getElementById('pictorial-anchor')?.firstElementChild || null;
  }
  ...
  function isUsableInjectionAnchor(el) {
    return !!el
      ...
      && !own(el)
      && !el.closest?.('[data-pictorial-variants]');
  }
  ...
  function waitForVariantAnchorAndRetry({ filePath, sessionId, srcWrapper, checkpointReason }) {
    ...
    const origContent = srcWrapper?.querySelector('[data-pictorial-variant="original"] > :first-child');
    ...
    pendingVariantAnchorRetryObserver = new MutationObserver(() => {
      ...
      const wrapperLanded = !!document.querySelector('[data-pictorial-variants="' + sessionId + '"]');
      if (!wrapperLanded) {
  ...
  async function probePreviewTree(manifest) {
    ...
    try {
      ...
      return { ok: false, tried: err.pictorialTriedUrls || candidates };
    }
  }

  function loadSvelteRuntime(runtimeModule, manifest) {
    const modulePath = runtimeModule || '/src/lib/pictorial/__runtime.js';
  ...
  // componentDir (keyed by variant number), because a `data-pictorial-params`
      ...
      if (entry.kind === 'collection' && entry.item && entry.item.rootTag) {
        ...
        values[entry.prop] = matches.map((itemEl, index) => {
          ...
          if (entry.item.keyField && item[entry.item.keyField] == null) {
            item[entry.item.keyField] = 'pictorial-live-' + index;
          }
          return item;
        });
  ...
  function cloneWithoutElements(rootEl, excludedEls) {
    ...
    const MARK = 'data-pictorial-hydration-excluded';
  ...
  async function mountSvelteComponentVariant(variantNum) {
    ...
    try {
      ...
      console.error('[pictorial] Failed to mount component variant ' + variantNum + ' for ' + sessionId + ':', err);
  ...
  async function injectSvelteComponentsFromManifest(manifestPath, sessionId) {
    ...
    try {
      ...
      const existingWrapper = document.querySelector('[data-pictorial-variants="' + sessionId + '"]');
      ...
      if (!liveEl?.parentElement) {
        console.warn('[pictorial] Could not find original element in live DOM.');
        arrivedVariants = availableVariants;
        expectedVariants = Number(manifest.count) || expectedVariants || arrivedVariants;
      ...
      wrapper.dataset.pictorialVariants = sessionId;
      wrapper.dataset.pictorialVariantCount = String(manifest.count || expectedVariants || 1);
      wrapper.dataset.pictorialPreview = componentPreviewMode;
      wrapper.style.display = 'contents';

      const mountTarget = document.createElement('div');
      mountTarget.dataset.pictorialComponentMount = sessionId;
      ...
      saveSession();
      if (parameterGenerationState === 'loading') completeParameterPublication();
      console.log('[pictorial] Mounted ' + arrivedVariants + ' ' + manifest.framework + ' component variants.');
    } catch (err) {
      console.error('[pictorial] Failed to mount component-preview variants:', err);
  ...
  function renderMountErrorCard() {
    ...
    retry.dataset.pictorialMountRetry = 'true';
  ...
  function abortSvelteComponentInjection(sessionId, details) {
    try {
      if (svelteComponentSession?.sessionId === sessionId) {
        ...
        const orphan = document.querySelector('[data-pictorial-variants="' + sessionId + '"]');
        if (orphan) orphan.remove();
      }
    } catch (err) {
      console.warn('[pictorial] Svelte component abort cleanup failed:', err);
    }
    hideShaderOverlay();
  ...
  function resetSvelteComponentSession(sessionId, message) {
    try {
      ...
      console.warn('[pictorial] Svelte component reset cleanup failed:', err);
    }
    hideShaderOverlay();
    if (variantObserver) { variantObserver.disconnect(); variantObserver = null; }
    if (pendingSvelteComponentRetryObserver) { pendingSvelteComponentRetryObserver.disconnect(); pendingSvelteComponentRetryObserver = null; }
  ...
  function discardOrphanedSession(reason) {
    ...
    if (!sessionId) return;
    console.warn('[pictorial] Discarding orphaned session ' + sessionId + ': ' + reason);
  ...
  function injectVariantsFromSource(filePath, sessionId, opts = {}) {
    ...
    fetch(url)
      ...
      .then(html => {
        ...
        const startMark = '<!-- pictorial-variants-start ' + sessionId + ' -->';
        const endMark = '<!-- pictorial-variants-end ' + sessionId + ' -->';
        ...
        srcWrapper = doc.querySelector('[data-pictorial-variants="' + sessionId + '"]');
        if (!srcWrapper) {
          console.warn('[pictorial] Variant wrapper not found in source file.');
        ...
        const variants = wrapper.querySelectorAll('[data-pictorial-variant]:not([data-pictorial-variant="original"])');
        arrivedVariants = variants.length;
        expectedVariants = parseInt(wrapper.dataset.pictorialVariantCount || arrivedVariants);
        if (arrivedVariants <= 0) {
          if (state === 'GENERATING') {
            ...
            if (!opts.generationCompleted) {
              console.log('[pictorial] Source has scaffold but no variants yet; still generating.');
              return;
            }
            ...
            if (attempt < COMPLETED_SOURCE_FALLBACK_RETRIES) {
              console.log('[pictorial] Generation is done but source shows no variants yet; retrying read ('
        ...
        saveSession();
        if (parameterGenerationState === 'loading') completeParameterPublication();
        console.log('[pictorial] Injected ' + arrivedVariants + ' variants from source file.');
      })
      .catch(err => {
        console.error('[pictorial] Failed to fetch source:', err);
  ...
  function updateSelectedElement() {
    ...
    if (svelteComponentSession?.sessionId === currentSessionId) {
      const anchor = resolveSvelteComponentAnchor();
      if (anchor && !anchor.__pictorialFrozenAnchor) selectedElement = anchor;
      return;
    }
    const wrapper = document.querySelector('[data-pictorial-variants="' + currentSessionId + '"]');
    if (!wrapper) return;
  ...
  function readVisibleVariantFromDOM(sessionId) {
    ...
    for (const variant of variants) {
      ...
      const idx = parseInt(variant.dataset.pictorialVariant || '0', 10);
      if (idx > 0) return idx;
    }
    return 0;
  }
  ...
  function pickVariantContent(wrapper, index) {
    if (!wrapper) return null;
    const variantDiv = wrapper.querySelector('[data-pictorial-variant="' + index + '"]');
  ...
  // single variant (`… > [data-pictorial-variant="N"]`); without it, targets
  // every variant via the bare `[data-pictorial-variant]` attribute.
  function variantStateSelector(sessionId, num) {
    const wrapper = '[data-pictorial-variants="' + sessionId + '"]';
    const variant = num == null
      ? '[data-pictorial-variant]'
      : '[data-pictorial-variant="' + num + '"]';
  ...
  function updateVariantStateStylesheet(sessionId, num) {
    ...
    const hideOthers = variantStateSelector(sessionId)
      + ':not([data-pictorial-variant="' + num + '"]) { ' + VARIANT_HIDE_DECL + ' }';
  ...
  function showOriginalDuringDiscard(sessionId) {
    ...
    const wrapper = '[data-pictorial-variants="' + sessionId + '"]';
    styleEl.textContent = wrapper + ' > [data-pictorial-variant]:not([data-pictorial-variant="original"]) { display:none !important; }\n'
      + wrapper + ' > [data-pictorial-variant="original"] { display:block !important; }';
  }
  ...
  function startScrollLock(sessionId, initialTargetY, initialAnchorTop) {
    ...
    scrollLockObserver = new MutationObserver((mutations) => {
      for (const m of mutations) {
        if (m.target?.closest?.('[data-pictorial-variants="' + sessionId + '"]')) {
          schedule('mutation-in-wrapper');
          return;
        }
        for (const n of m.addedNodes) {
          if (n.nodeType === 1 && (n.matches?.('[data-pictorial-variants="' + sessionId + '"]') || n.querySelector?.('[data-pictorial-variants="' + sessionId + '"]'))) {
  ...
  function startVariantObserver(sessionId) {
    ...
    const obs = new MutationObserver((mutations) => {
      ...
      // Only react to mutations that add nodes with data-pictorial-variant,
      // or mutations inside the variant wrapper. Ignore our own bar/UI changes.
      let dominated = false;
      for (const m of mutations) {
        if (m.target.closest?.('[data-pictorial-variants]')) { dominated = true; break; }
        for (const n of m.addedNodes) {
          if (n.nodeType !== 1) continue;
          // Direct hit: the added node itself is the wrapper or a variant.
          if (n.dataset?.pictorialVariants || n.dataset?.pictorialVariant) {
          ...
          if (n.querySelector?.('[data-pictorial-variants],[data-pictorial-variant]')) {
            dominated = true; break;
          }
        }
        if (dominated) break;
      ...
      if (selectedElement && !document.body.contains(selectedElement)) {
        const isInsert = wrapper.dataset.pictorialMode === 'insert';
        if (isInsert) {
      ...
      const expected = parseInt(wrapper.dataset.pictorialVariantCount || '0');
      ...
      if (arrivedVariants > 0) {
        ...
        recoveryWaitingForAnchor = false;
        hideShaderOverlay();
        if (wrapper.dataset.pictorialMode === 'insert') finalizeInsertSession();
        updateSelectedElement();
        showOrUpdateCyclingBar();
        disableInlineEdit();
  ...
  function startScrollTracking() {
    function tick() {
      if (state === 'CONFIGURING' || state === 'GENERATING' || state === 'CYCLING') {
        ...
        if (hiTarget && !hiTarget.hasAttribute?.('data-pictorial-insert-placeholder')) {
  ...
  function connectSSE() {
    ...
    evtSource.onmessage = (e) => {
      ...
      switch (msg.type) {
        case 'connected':
          ...
          if (!hasProjectContext) showToast(`No PRODUCT.md found. Variants will be brand-agnostic. Run ${PICTORIAL_COMMAND} init to generate one.`, 7000);
          console.log('[pictorial] Live mode connected.');
        ...
        case 'error':
          ...
          if (awaitingAcceptResult?.id && msg.id === awaitingAcceptResult.id) {
            awaitingAcceptResult = null;
            console.error('[pictorial] Accept failed after teardown:', msg.message);
          ...
          if (maybeCompleteSteer(msg)) break;
          console.error('[pictorial] Error:', msg.message);
    ...
    evtSource.onerror = () => {
      ...
      if (sseRetries <= SSE_MAX_RETRIES) {
        console.log('[pictorial] SSE connection lost. Retry ' + sseRetries + '/' + SSE_MAX_RETRIES + '...');
        return; // EventSource auto-reconnects
      }
      // Server is gone. Clean up gracefully.
      console.log('[pictorial] Live server unreachable. Cleaning up UI.');
      evtSource.close();
      evtSource = null;
      handleServerLost();
    };
  ...
  function sendEvent(msg, opts) {
    ...
    function handleFailure(err) {
      if (opts && opts.throwOnError) {
        console.error('[pictorial] Failed to send event:', err);
        throw err;
      }
      console.debug('[pictorial] Dropped optional live event:', err);
      return null;
    }
    // Token in the query string as well as the body: the URL token is what
    // authorizes the CORS preflight when the page runs on a non-loopback
  ...
  function abandonForeignSession(sessionId) {
    ...
    console.warn('[pictorial] The live server has no record of session ' + sessionId + '; clearing stale local state.');
    markSessionHandled();
    cleanup({ instantChrome: true });
  ...
  function handleKeyDown(e) {
    ...
    if (e.target.isContentEditable && isInlineEditActive(e.target)) {
      ...
      e.preventDefault();
      e.stopPropagation();
      const original = e.target.dataset.pictorialOriginalText;
  ...
  function shouldUseAncestorCropShaderProxy(el) {
    ...
    const adapter = String(window.__PICTORIAL_LIVE_ADAPTER__ || '').toLowerCase();
    ...
    const wrapper = el?.closest?.('[data-pictorial-variants]');
    return isFrameworkComponentPreviewMode(wrapper?.dataset?.pictorialPreview);
  }
    ...
    try {
      ...
      if (shouldUseAncestorCropShaderProxy(el)) {
        try {
          ...
          console.warn('[pictorial] Svelte ancestor crop capture failed, falling back to element capture:', err);
        }
      }
  ...
  async function captureAndEmit(el, basePayload, snapshot, rect) {
    ...
    try {
      ...
      console.warn('[pictorial] capture failed, proceeding without screenshot:', err);
    }
    ...
    if (blob && hasAnnotations) {
      try {
        ...
        if (uploadRes.ok) {
          ...
          console.warn('[pictorial] annotation upload failed:', uploadRes.status);
        }
      } catch (err) {
        console.warn('[pictorial] annotation upload failed:', err);
      }
    }
...
void main() {
  ...
  async function showShaderOverlay(el, blob, rect, paper) {
    ...
    try {
      ...
      console.warn('[pictorial] shader setup failed:', err);
      canvas.remove();
      return;
    }
    ...
    try {
      ...
      console.warn('[pictorial] shader bitmap decode failed:', err);
  ...
  async function handleAccept() {
    ...
    const acceptWrapper = document.querySelector('[data-pictorial-variants="' + currentSessionId + '"]');
    ...
    const acceptedIsSvelteComponent = svelteComponentSession?.sessionId === acceptedSessionId
      || isFrameworkComponentPreviewMode(acceptWrapper?.dataset?.pictorialPreview);
    ...
    sendEvent(acceptPayload, { throwOnError: true })
      .then(() => {
        ...
        document.documentElement.dataset.pictorialAcceptToPickingMs = String(Date.now() - acceptPayload.clientSentAt);
  ...
  function snapshotAcceptedVariantDom(sessionId, variantId) {
    const wrapper = document.querySelector('[data-pictorial-variants="' + sessionId + '"]');
    const accepted = wrapper?.querySelector?.('[data-pictorial-variant="' + variantId + '"]');
  ...
  function acceptedDomAlreadyClean(pending) {
    ...
    return matches.length > 0
      && matches.every((el) => !el.closest('[data-pictorial-variants],[data-pictorial-variant],[data-pictorial-carbonize]'));
  }
  ...
  function findAcceptedRuntimeWrappers(sessionId) {
    if (!sessionId) return [];
    return [...new Set([
      ...document.querySelectorAll('[data-pictorial-variants="' + sessionId + '"]'),
      ...document.querySelectorAll('[data-pictorial-carbonize="' + sessionId + '"]'),
    ])];
  }
  ...
  function reloadAfterMissingAcceptedDom(pending) {
    if (acceptedDomAlreadyClean(pending)) return;
    if (pending?.id && document.querySelector('[data-pictorial-variants="' + pending.id + '"]')) return;
    location.reload();
  }
  ...
  function commitAcceptedVariantToDom(sessionId, variantId) {
    ...
    const style = wrapper.querySelector('style[data-pictorial-css]');
    if (style && !document.querySelector('style[data-pictorial-accepted-css="' + sessionId + '"]')) {
      const promotedStyle = style.cloneNode(true);
      promotedStyle.setAttribute('data-pictorial-accepted-css', sessionId);
  ...
  function restoreFromActiveSessions(activeSessions, reason) {
    const wrapper = document.querySelector('[data-pictorial-variants]');
    if (wrapper && !isFrameworkComponentPreviewMode(wrapper.dataset.pictorialPreview)) return false;
  ...
  function recoverMissedGenerationCompletion(activeSessions) {
    ...
    if (!file) return;
    console.log('[pictorial] Reconnected after generation completed; recovering variants from source.');
    setTimeout(() => {
  ...
  function loadSession() {
    ...
    if (saved?.appRoot && APP_ROOT && saved.appRoot !== APP_ROOT) {
      console.warn('[pictorial] Ignoring saved live session from another project (' + saved.appRoot + ').');
  ...
  function cleanup(options) {
    ...
    if (svelteComponentSession?.sessionId === cleanupSessionId) {
      ...
      const wrapper = document.querySelector('[data-pictorial-variants="' + cleanupSessionId + '"]');
      if (wrapper) {
      ...
      setTimeout(function() {
        ...
        const lateWrapper = document.querySelector('[data-pictorial-variants="' + cleanupSessionId + '"]');
        if (!lateWrapper) return;
        const orig = lateWrapper.querySelector('[data-pictorial-variant="original"]');
        if (orig) {
  ...
  // If a [data-pictorial-variants] wrapper exists in the DOM, the agent wrote
  ...
  function resumeSession() {
    ...
    const sessionId = wrapper.dataset.pictorialVariants;
    ...
    // wrapper holds a single mount target, not [data-pictorial-variant] nodes,
    ...
    if (isFrameworkComponentPreviewMode(wrapper.dataset.pictorialPreview)
    ...
    if (isFrameworkComponentPreviewMode(wrapper.dataset.pictorialPreview)) {
      ...
      currentSessionId = sessionId;
      expectedVariants = Number(wrapper.dataset.pictorialVariantCount)
    ...
    currentSessionId = sessionId;
    expectedVariants = parseInt(wrapper.dataset.pictorialVariantCount || '0');
    const variants = wrapper.querySelectorAll('[data-pictorial-variant]:not([data-pictorial-variant="original"])');
    arrivedVariants = variants.length;
      ...
      if (shaderTarget) {
        (async () => {
          try {
            ...
            console.warn('[pictorial] shader resume failed:', err);
          }
        })();
      }
    }
  ...
  const PICK_PREFS_KEY = 'pictorial-live-pick';
  const INTERACTION_PREFS_KEY = 'pictorial-live-interaction';
  ...
  function detectPageTheme() {
    try {
      // Dev override: set localStorage 'pictorial-dev-theme' to 'light' or
      // 'dark' to preview the opposite palette without actually changing the
      // page bg. Used for screenshots and theme QA.
      const override = localStorage.getItem('pictorial-dev-theme');
  ...
  function attachSteerFocusGuard() {
    if (window.__PICTORIAL_STEER_FOCUS_GUARD__) return;
    window.__PICTORIAL_STEER_FOCUS_GUARD__ = true;
  ...
  function steerFocusDebugEnabled() {
    try { return localStorage.getItem('pictorial-steer-debug') === '1'; } catch { return false; }
  }

  function steerFocusLog(reason, extra) {
    if (!steerFocusDebugEnabled()) return;
    console.log('[pictorial.steer]', reason, {
      state,
      pickActive,
  ...
  function attachSteerFocusDebug() {
    if (!steerFocusDebugEnabled()) return;
    if (window.__PICTORIAL_STEER_FOCUS_DEBUG__) return;
    window.__PICTORIAL_STEER_FOCUS_DEBUG__ = true;
  ...
  function buildSteerProcessingDots() {
    ...
    for (let i = 0; i < 3; i++) {
      wrap.appendChild(el('span', {
        ...
        animation: 'pictorial-steer-dot 1.05s ease-in-out ' + (i * 0.14) + 's infinite',
      }));
    }
    return wrap;
  }
  ...
  function startVoice(ctx) {
    ...
    rec.onerror = (event) => {
      ...
      console.warn('[pictorial.voice] recognition error:', code);
    ...
    try {
      ...
      console.warn('[pictorial.voice] start failed:', err);
      stopVoice({
        suppressSubmit: true,
  ...
  function initPageChat(parent, P) {
    ...
    if (!uiGetById(PREFIX + '-page-chat-style')) {
      ...
      s.textContent =
        '@keyframes pictorial-steer-dot { 0%, 70%, 100% { opacity: 0.28; transform: scale(0.82); } 35% { opacity: 1; transform: scale(1); } }' +
        '@keyframes pictorial-steer-processing { 0%, 100% { border-color: oklch(70% 0.12 188 / 0.28); box-shadow: 0 0 0 0 oklch(70% 0.12 188 / 0); } 50% { border-color: oklch(82% 0.07 188 / 0.55); box-shadow: 0 0 14px oklch(70% 0.12 188 / 0.18); } }' +
        '@keyframes pictorial-voice-pulse { 0%, 100% { opacity: 0.55; } 50% { opacity: 1; } }' +
        '#' + PREFIX + '-page-chat[data-processing="true"] { animation: pictorial-steer-processing 1.6s ease-in-out infinite; }' +
        ...
        '#' + PREFIX + '-page-chat-voice[data-listening="true"] svg { animation: pictorial-voice-pulse 1.1s ease-in-out infinite; }' +
  ...
  // Pictorial mark - same paths as site/components/Header.astro + favicon.svg.
  ...
  function syncAgentPollingUi(connected) {
    ...
    globalBarBrandEl.setAttribute('aria-label', agentStatusMessage
      ? 'Pictorial live mode - ' + (agentHasWorkInFlight() ? 'agent is working' : 'agent not polling')
      : 'Pictorial live mode');
  ...
  function initGlobalBar() {
    ...
    if (!uiGetById(PREFIX + '-bar-focus-style')) {
      ...
      s.textContent =
        ...
        '}' +
        '@keyframes pictorial-agent-dot { 0%, 100% { opacity: 0.45; transform: scale(0.9); } 50% { opacity: 1; transform: scale(1); } }' +
        '#' + PREFIX + '-global-bar-brand[data-agent-connected="false"] [data-agent-dot] { animation: pictorial-agent-dot 1.4s ease-in-out infinite; }' +
    ...
    // Brand mark - kinpaku Pictorial icon (site header / favicon paths).
    const brand = el('span', {
      display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
    ...
    brand.setAttribute('aria-label', 'Pictorial live mode - agent not polling');
  ...
  function updateGlobalBarState() {
    ...
    document.querySelectorAll('.pictorial-overlay').forEach(o => {
      o.style.pointerEvents = (pickActive || insertActive) ? 'none' : '';
    });
    syncPageInteractionCursor();
  }

  let detectReady = false; // true once detect script posts 'pictorial-ready'
  ...
  function toggleDetect() {
    ...
    if (detectActive) {
      ...
      window.postMessage({ source: 'pictorial-command', action: 'remove' }, '*');
  ...
  function loadDetectScript() {
    ...
    s.dataset.pictorialExtension = 'true';
  ...
  function onDetectMessage(e) {
    ...
    if (e.data.source === 'pictorial-ready') {
      detectReady = true;
    ...
    if (e.data.source === 'pictorial-results') {
  ...
  function teardown() {
    ...
    removeVariantStateStylesheet();
    window.__PICTORIAL_LIVE_INIT__ = false;
    console.log('[pictorial] Live mode exited.');
  }

  //
  // Design System Panel - visualizes the project's .pictorial/design.json sidecar
  //

  const DESIGN_PREFS_KEY = 'pictorial-live-design-panel';
  ...
  let designState = {
    ...
    sidecar: null,          // .pictorial/design.json v2 payload (extensions + components + narrative)
    hasMd: false,
    hasSidecar: false,
  ...
  // Neutral panel palette - deliberately NOT Pictorial-branded. The panel is
  // a viewer of the project's design system, not an Pictorial surface.
  const DP = {
  ...
  function renderDesignBody() {
    ...
    if (designState.present === false) {
      ...
      empty.innerHTML = `<strong>No DESIGN.md yet</strong>Create one by running <code>${PICTORIAL_COMMAND} document</code> in your terminal, then re-open this panel.`;
  ...
  function renderStaleHint() {
    ...
    box.innerHTML = `
      ...
      <span class="stale-text"><strong>DESIGN.md is newer than .pictorial/design.json.</strong> Run <code>${PICTORIAL_COMMAND} document</code> to refresh the sidecar.</span>
    `;
    return box;
  }
  ...
  function renderParsedMdCta() {
    ...
    box.innerHTML = `<strong>Basic view</strong>This panel reads the tokens in your <code>DESIGN.md</code> frontmatter. Running <code>${PICTORIAL_COMMAND} document</code> also generates a <code>.pictorial/design.json</code> sidecar with your project's actual component snippets (button, input, nav) and tonal ramps, rendered live below the tokens.`;
    return box;
  }
  ...
  function designEmptyMessage() {
    if (designState.hasMd && !designState.hasSidecar) {
      return 'DESIGN.md found, no structured tokens to display. Run ' + PICTORIAL_COMMAND + ' document to generate the .pictorial/design.json sidecar.';
    }
  ...
  function cssSafe(v) {
    ...
    // .pictorial/design.json values rendered into inline style strings.
  ...
  function init() {
    ...
    if (!resumeSession()) {
      console.log('[pictorial] Live variant mode ready. Hover over elements to pick one.');
      ...
      const scout = new MutationObserver(() => {
        ...
        scout.disconnect();
        if (resumeSession()) {
          console.log('[pictorial] Resumed deferred session ' + currentSessionId + ' (post-hydration).');
        }
      });
      scout.observe(document.body, { childList: true, subtree: true });
    } else {
      console.log('[pictorial] Resumed active variant session ' + currentSessionId + ' (' + arrivedVariants + '/' + expectedVariants + ' variants).');
    }
