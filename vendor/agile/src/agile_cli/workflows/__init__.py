...
# The step types Agile ships, snapshotted before any community step can be
...
# "is this bundled with Agile?" in a long-lived process: a step loaded for one
...
def load_custom_steps(project_root: Path) -> list[str]:
    ...
    Scans ``.agile/workflows/steps/`` for installed step packages.
    ...
    steps_dir = Path(project_root) / ".agile" / "workflows" / "steps"

    # Defense-in-depth: refuse to execute step code from a symlinked
    # parent directory under .agile/workflows/steps, which could redirect
    ...
    for _part in (".agile", "workflows", "steps"):
    ...
    for step_dir in steps_dir.iterdir():
        ...
        try:
            ...
            # (e.g. "test-custom" → "_agile_custom_step_test_custom_<hash>").
            ...
            module_name = f"_agile_custom_step_{safe_key}_{key_hash}"
