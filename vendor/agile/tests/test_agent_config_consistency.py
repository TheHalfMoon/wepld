...
class TestAgentConfigConsistency:
    ...
    def test_skills_agents_have_hyphen_invoke_separator_in_agent_configs(self):
        ...
        ``__AGILE_COMMAND_*__`` tokens with the correct hyphen separator.
        """
        cfg = CommandRegistrar.AGENT_CONFIGS
        skills_agents = [
    ...
    def test_skills_agent_command_token_resolves_with_hyphen(self, tmp_path):
        """__AGILE_COMMAND_*__ tokens in extension commands resolve to /agile-<cmd>
        ...
        commands = [
            {"name": "agile.git.feature", "file": "commands/agile.git.feature.md"}
        ]
        ...
        skill_file = (
            tmp_path / ".claude" / "skills" / "agile-git-feature" / "SKILL.md"
        )
        ...
        assert not re.search(r"(?<![a-zA-Z0-9_])/agile\.[a-z]", content), (
            "Found dot-notation command ref (/agile.<cmd>) in generated Claude skill. "
