...
Every consequential event kind MUST define a stable logical transition identity in its trusted versioned payload contract, including ownership, execution, result and authority-adjacent events, not only triggers. `idempotency_or_dedupe_key` may carry that identity; if absent, the consumer must derive the exact declared identity from validated payload fields. A per-delivery `runtime_event_id`, newly generated key, timestamp or arrival order cannot substitute for logical identity. If the kind has no qualified stable identity, or its required fields are absent, ambiguous or mismatched, reject/quarantine before any consequential state transition or effect.

Work controls use the existing Mission-scoped request_id; triggers use the existing logical occurrence key. Ownership kinds bind the exact controlled resource and expected/successor ownership transition identity; execution/result kinds bind the exact Attempt, execution identity and declared transition/result revision; authority-adjacent kinds bind the immutable decision/grant/qualification/approval identity and declared transition. The owning kind must define that precise tuple and conflict rule before activation; these examples do not introduce another event envelope or authorize a default tuple.

The runtime atomically associates each accepted logical identity with its payload identity and state transition in existing persistence. Same identity/same payload is an idempotent acknowledgement; same identity/different payload is a conflict. Duplicate deliveries, reconnects and producer restarts retain the same logical identity for the same transition; a new producer incarnation cannot mint another occurrence or repeat an effect. Retention spans admitted replay/recovery windows; an expired or unprovable dedupe boundary quarantines ambiguous replay instead of assuming it is new. Authentication, current access and ownership/Nawat checks still apply independently.

Transport dedupe keys are scoped by producer identity/runtime incarnation and the owning tenant/work namespace; this transport namespace does not replace the durable event-kind logical identity. Reusing a key with a different payload is a conflict. Sequence gaps produce explicit incomplete state; they are not inferred away by a newer timestamp. Retention/compaction must retain the causal and dedupe boundary needed for admitted replay windows. Client cursor replay is an access-checked projection, not permission to re-emit effects.
