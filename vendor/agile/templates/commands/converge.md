...
`__AGILE_COMMAND_IMPLEMENT__` can complete it. This command MUST run only after
`__AGILE_COMMAND_IMPLEMENT__` has run on the current `tasks.md`, and after `__AGILE_COMMAND_TASKS__` has produced a complete `tasks.md`.
...
- modify, create, or delete any application code — completing the appended tasks is the
  job of `__AGILE_COMMAND_IMPLEMENT__`.
...
- On `tasks_appended`: state how many tasks were appended under which phase, and recommend
  running `__AGILE_COMMAND_IMPLEMENT__` to complete them; note that a follow-up converge
...
After producing the result, check if `.agile/extensions.yml` exists in the project root.
