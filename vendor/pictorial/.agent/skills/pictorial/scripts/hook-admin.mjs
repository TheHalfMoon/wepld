...
/**
 * The Pictorial hooks command manages the design hook runtime
 * via the `hook` key and shared detector ignores via the `detector` key in
 * .pictorial/config.json / .pictorial/config.local.json.
 *
 * Usage:
...
import { PICTORIAL_COMMAND } from './lib/provider.mjs';
...
const HOOK_MANIFEST_TARGETS = [
  {
    provider: '.claude',
    skillRel: '.claude/skills/pictorial',
    destRel: '.claude/settings.local.json',
    sharedDestRel: '.claude/settings.json',
    manifest: () => ({
      description: 'Pictorial design detector: immediate-tier checks after Edit/Write/MultiEdit on UI files, full-rule deep pass on Stop.',
      hooks: {
        PostToolUse: [
          {
            ...
            hooks: [
              {
                type: 'command',
                command: 'node "${CLAUDE_PROJECT_DIR}/.claude/skills/pictorial/scripts/hook.mjs"',
        ...
        Stop: [stopManifestEntry('node "${CLAUDE_PROJECT_DIR}/.claude/skills/pictorial/scripts/hook.mjs"')],
      },
    }),
  },
  {
    provider: '.agents',
    skillRel: '.agents/skills/pictorial',
    ...
    manifest: () => ({
      hooks: {
        PostToolUse: [
          {
            ...
            hooks: [
              {
                type: 'command',
                command: 'node ".agents/skills/pictorial/scripts/hook.mjs"',
                timeout: TIMEOUT_SECONDS,
                statusMessage: STATUS_MESSAGE,
              },
            ],
        ...
        Stop: [stopManifestEntry('node ".agents/skills/pictorial/scripts/hook.mjs"')],
      },
    }),
  },
  {
    provider: '.cursor',
    skillRel: '.cursor/skills/pictorial',
  ...
  },
  {
    ...
    provider: '.github',
    skillRel: '.github/skills/pictorial',
    destRel: '.github/hooks/pictorial.json',
    manifest: () => ({
      version: 1,
      hooks: {
        postToolUse: [
          {
            ...
            bash: 'node "$(git rev-parse --show-toplevel)/.github/skills/pictorial/scripts/hook.mjs"',
...
function writeHookConfig(cwd, hookConfig, opts = {}) {
  ...
  // (consent, quiet, auditLog) survive an Pictorial hooks edit.
...
function statusReport(cwd) {
  ...
  const envKill = process.env.PICTORIAL_HOOK_DISABLED;
  const envState = envKill ? `PICTORIAL_HOOK_DISABLED=${envKill}` : 'unset';
  const cfgPath = path.relative(cwd, getConfigPath(cwd)) || '.pictorial/config.json';
  const localPath = path.relative(cwd, getLocalConfigPath(cwd)) || '.pictorial/config.local.json';
  const cachePath = path.relative(cwd, getCachePath(cwd)) || '.pictorial/hook.cache.json';
  ...
  // `rule=value [files]` shape `pictorial ignores list` already prints.
...
function fileHasPictorialHookMarker(filePath) {
...
function valueHasPictorialHookMarker(value) {
  if (typeof value === 'string') {
    return PICTORIAL_HOOK_COMMAND_MARKERS.some((marker) => value.includes(marker));
  }
  if (Array.isArray(value)) return value.some(valueHasPictorialHookMarker);
  if (value && typeof value === 'object') return Object.values(value).some(valueHasPictorialHookMarker);
  return false;
}
...
function addIgnoreRule(cwd, args) {
  ...
  if (!rule) throw new Error(`Pass a rule id, e.g. ${PICTORIAL_COMMAND} hooks ignore-rule side-tab`);
  if (rule === 'overused-font' && !parsed.allValues) {
    throw new Error(`overused-font is value-specific by default. Use ${PICTORIAL_COMMAND} hooks ignore-value overused-font <font> for a confirmed font, or ${PICTORIAL_COMMAND} hooks ignore-rule overused-font --all-values only when the user asked to ignore overused fonts generally.`);
  }
...
function addIgnoreFile(cwd, args) {
  ...
  if (!glob) throw new Error(`Pass a glob, e.g. ${PICTORIAL_COMMAND} hooks ignore-file "src/legacy/**"`);
...
function parseIgnoreValueArgs(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === '--shared') {
      ...
      // reported success. Matches `pictorial ignores add-value`.
...
function addIgnoreValue(cwd, args) {
  ...
  if (!parsed.rule || !parsed.value) {
    throw new Error(`Pass a rule id and value, e.g. ${PICTORIAL_COMMAND} hooks ignore-value overused-font Inter`);
  }
  ...
  // `files` scope, so require one — matching `pictorial ignores add-value`.
  if (parsed.value === '*' && parsed.files.length === 0) {
    ...
    const projectWide = parsed.rule === 'overused-font'
      ? `${PICTORIAL_COMMAND} hooks ignore-rule ${parsed.rule} --all-values`
      : `${PICTORIAL_COMMAND} hooks ignore-rule ${parsed.rule}`;
    throw new Error(`Wildcard value ignores must be scoped with --file <glob>, e.g. ${PICTORIAL_COMMAND} hooks ignore-value design-system-font-size "*" --file "src/widget.js". To suppress the rule project-wide use ${projectWide}.`);
  }
