...
/**
 * Build-pipeline emitters for the Pictorial design hook.
 *
 ...
 *      - Grok Build:  `.grok/hooks/pictorial.json`
 *
 ...
 * root rather than assuming a `.claude/skills/pictorial/` layout, so it stays
 * correct wherever Claude Code unpacks the plugin.
 */

export const PICTORIAL_HOOK_COMMAND_MARKER = 'skills/pictorial/scripts/hook.mjs';
...
const CLAUDE_PROJECT_HOOK = '${CLAUDE_PROJECT_DIR}/.claude/skills/pictorial/scripts/hook.mjs';
...
const NODE_NOTICE_TEXT = `The pictorial design hook is not running: no Node ${NODE_MAJOR_FLOOR} or newer on PATH. `
  + 'Install one, or remove the pictorial hook from your harness settings.';
// Claude Code and Codex both read `systemMessage`, so one payload serves both.
// The marker under ~/.pictorial holds it to one notice per machine (not per
...
const SYSTEM_MESSAGE_NOTICE = 'D="$HOME/.pictorial"; [ -f "$D/node-unsupported" ] || '
  + '{ mkdir -p "$D" 2>/dev/null && : > "$D/node-unsupported" 2>/dev/null && '
  + `printf '%s' '{"systemMessage":"${NODE_NOTICE_TEXT}"}'; }`;
const CLAUDE_PLUGIN_HOOK = '${CLAUDE_PLUGIN_ROOT}/skills/pictorial/scripts/hook.mjs';
const CODEX_PLUGIN_HOOK = '${PLUGIN_ROOT}/skills/pictorial/scripts/hook.mjs';
...
const codexProjectHook = (skillDir) => `${skillDir}/skills/pictorial/scripts/hook.mjs`;
const CURSOR_BEFORE_EDIT_SCRIPT = '.cursor/skills/pictorial/scripts/hook-before-edit.mjs';
const GITHUB_PROJECT_HOOK = '$(git rev-parse --show-toplevel)/.github/skills/pictorial/scripts/hook.mjs';
...
const GROK_PROJECT_HOOK = '.grok/skills/pictorial/scripts/hook.mjs';
