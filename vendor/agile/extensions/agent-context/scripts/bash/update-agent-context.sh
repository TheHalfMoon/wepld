...
#
# Refresh the managed Agile section in the coding agent's context file(s)
# (e.g. CLAUDE.md, .github/copilot-instructions.md, AGENTS.md).
#
# Reads `context_files` or `context_file`, plus `context_markers.{start,end}`, from the
# agent-context extension config:
#   .agile/extensions/agent-context/agent-context-config.yml
#
# Usage: update-agent-context.sh [plan_path]
#
# When `plan_path` is omitted, the script derives it from `.agile/feature.json`
# (written by /agile-specify). Falls back to the most recently modified
# `specs/**/plan.md` only when feature.json is absent or its plan does not exist yet.

set -euo pipefail

PROJECT_ROOT="$(pwd)"
EXT_CONFIG="$PROJECT_ROOT/.agile/extensions/agent-context/agent-context-config.yml"
DEFAULT_START="<!-- AGILE START -->"
DEFAULT_END="<!-- AGILE END -->"
...
_python=""
_python_candidates=()
[[ -n "${AGILE_PYTHON:-}" ]] && _python_candidates+=("$AGILE_PYTHON")
...
if not context_files:
    ...
    # agent-context-defaults.json file. This is independent of the Agile CLI
    # by design; nothing here imports agile_cli.
    project_root = sys.argv[3] if len(sys.argv) > 3 else "."
    integration_key = ""
    try:
        with open(
            f"{project_root}/.agile/init-options.json", "r", encoding="utf-8"
        ) as fh:
            opts = json.load(fh)
    ...
    if integration_key:
        defaults_path = (
            f"{project_root}/.agile/extensions/agent-context/"
...
PLAN_PATH="${1:-}"
if [[ -z "$PLAN_PATH" ]]; then
  # Prefer .agile/feature.json (written by /agile-specify) over mtime heuristic.
  _feature_json="$PROJECT_ROOT/.agile/feature.json"
...
# created via AGILE_FEATURE_DIRECTORY, e.g. specs/<scope>/<feature>/plan.md,
