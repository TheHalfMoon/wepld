...
from agile_cli.bundler.models.records import load_records, records_path
from agile_cli.bundler.services.installer import install_bundle, remove_bundle
...
def test_remove_record_save_failure_reports_partial_state(tmp_path: Path):
    ...
    with pytest.MonkeyPatch.context() as mp:
        mp.setattr(
            "agile_cli.bundler.lib.yamlio.json.dump",
            fail_dump,
        )
        with pytest.raises(BundlerError) as exc_info:
            remove_bundle(tmp_path, "demo-bundle", installer)
...
def test_remove_record_save_failure_without_remove_attempt_is_not_partial(
    ...
    with pytest.MonkeyPatch.context() as mp:
        mp.setattr(
            "agile_cli.bundler.services.installer.save_records",
            fail_save,
        )
        with pytest.raises(BundlerError) as exc_info:
            remove_bundle(tmp_path, "demo-bundle", installer)
...
def test_install_result_changed_reports_uninstalled():
    ...
    from agile_cli.bundler.services.installer import InstallResult
