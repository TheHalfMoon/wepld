...
def _setup_project(tmp_path: Path, *, git: bool = True) -> Path:
    """Create a project directory with core scripts and .agile."""
    ...
    # .agile structure
    (tmp_path / ".agile" / "templates").mkdir(parents=True)

    # Extension scripts (as if installed)
    ext_bash = tmp_path / ".agile" / "extensions" / "git" / "scripts" / "bash"
    ...
    ext_ps = tmp_path / ".agile" / "extensions" / "git" / "scripts" / "powershell"
    ext_ps.mkdir(parents=True)
    for f in EXT_PS.iterdir():
        shutil.copy(f, ext_ps / f.name)

    # Copy extension.yml
    shutil.copy(EXT_DIR / "extension.yml", tmp_path / ".agile" / "extensions" / "git" / "extension.yml")
...
def _write_config(project: Path, content: str) -> Path:
    ...
    config_path = project / ".agile" / "extensions" / "git" / "git-config.yml"
...
def _run_bash(script_name: str, cwd: Path, *args: str, env_extra: dict | None = None) -> subprocess.CompletedProcess:
    ...
    script = cwd / ".agile" / "extensions" / "git" / "scripts" / "bash" / script_name
...
def _run_pwsh(script_name: str, cwd: Path, *args: str, env_extra: dict | None = None) -> subprocess.CompletedProcess:
    ...
    script = cwd / ".agile" / "extensions" / "git" / "scripts" / "powershell" / script_name
    env = {**os.environ, **_GIT_ENV, **(env_extra or {})}
    return subprocess.run(
...
class TestGitExtensionManifest:
    ...
    def test_manifest_commands(self):
        ...
        assert "agile.git.feature" in names
        assert "agile.git.validate" in names
        assert "agile.git.remote" in names
        assert "agile.git.initialize" in names
        assert "agile.git.commit" in names
    ...
    def test_manifest_hooks(self):
        ...
        assert m.hooks["before_constitution"]["command"] == "agile.git.initialize"
        assert m.hooks["before_specify"]["command"] == "agile.git.feature"
...
class TestGitExtensionInstall:
    ...
    def test_install_copies_scripts(self, tmp_path: Path):
        ...
        ext_installed = tmp_path / ".agile" / "extensions" / "git"
...
class TestAutoCommitBash:
    ...
    def test_no_config_file_skips(self, tmp_path: Path):
        ...
        config = project / ".agile" / "extensions" / "git" / "git-config.yml"
...
class TestAutoCommitBashCommitStyle:
    ...
    def test_fixed_is_default_when_commit_style_absent(self, tmp_path: Path):
        ...
        _write_config(project, (
            ...
            "  after_specify:\n"
            "    enabled: true\n"
            '    message: "[Agile] Add specification"\n'
        ))
        (project / "new-file.txt").write_text("content")
        result = _run_bash("auto-commit.sh", project, "after_specify")
        assert result.returncode == 0
        ...
        assert "[Agile] Add specification" in log.stdout
    ...
    def test_conventional_uses_generated_message(self, tmp_path: Path):
        ...
        assert "[Agile] Add specification" not in log.stdout
...
class TestGitCommonBash:
    def test_has_git_true(self, tmp_path: Path):
        ...
        project = _setup_project(tmp_path, git=True)
        script = project / ".agile" / "extensions" / "git" / "scripts" / "bash" / "git-common.sh"
        result = subprocess.run(
...
class TestGitCommonPowerShell:
    def test_test_feature_branch_accepts_single_prefix(self, tmp_path: Path):
        project = _setup_project(tmp_path)
        script = project / ".agile" / "extensions" / "git" / "scripts" / "powershell" / "git-common.ps1"
        result = subprocess.run(
            [
                "pwsh",
                "-NoProfile",
