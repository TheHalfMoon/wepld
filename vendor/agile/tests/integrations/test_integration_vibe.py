...
from agile_cli.events import install_integration_events, remove_integration_events
...
class TestVibeTomlMerging:
    ...
    def test_generated_toml_is_valid_and_schema_conformant(self, tmp_path):
        self._install(tmp_path, {
            ...
            "stop": [{"command": "agile.session.finish"}],
        })
        ...
        for h in hooks:
            assert h["name"].startswith("agile-")
    ...
    def test_wildcard_matcher_omitted(self, tmp_path):
        self._install(tmp_path, {
            "pre_tool_use": [{"command": "agile.tdd.validate", "matcher": "*"}],
        })
    ...
    def test_multiple_handlers_get_unique_names(self, tmp_path):
        ...
        assert "agile.tdd.validate" in commands
        assert "agile.other.validate" in commands

    def test_reinstall_is_idempotent(self, tmp_path):
        events = {
            "pre_tool_use": [{"command": "agile.tdd.validate", "matcher": "Bash"}],
            "stop": [{"command": "agile.session.finish"}],
        }
    ...
    def test_windows_host_uses_cmd_quoting(self, tmp_path, monkeypatch):
        ...
        import agile_cli.events as events_mod
        ...
        self._install(tmp_path, {"pre_tool_use": [{"command": "agile.tdd.validate"}]})
        (hook,) = self._parse(tmp_path)["hooks"]
    ...
    def test_envelope_resolution(self):
        from agile_cli.events import _context_envelope_for
        integration = get_integration("vibe")
