 ...
 *
 * `pin audit` creates a lightweight audit skill that redirects to Pictorial's audit workflow.
...
const PIN_MARKER = '<!-- pictorial-pinned-skill -->';
...
/**
 * Find harness skill directories that have an pictorial skill installed.
 */
function findHarnessDirs(projectRoot) {
  const dirs = [];
  for (const harness of HARNESS_DIRS) {
    const skillsDir = join(projectRoot, harness, 'skills');
    // Only pin in harness dirs that already have pictorial installed
    const pictorialDir = join(skillsDir, 'pictorial');
    if (existsSync(pictorialDir) || existsSync(join(skillsDir, 'i-pictorial'))) {
      dirs.push(skillsDir);
    }
  }
  return dirs;
...
function generatePinnedSkill(command, metadata, commandPrefix, isCodex) {
  const desc = metadata[command]?.description || `Shortcut for ${commandPrefix}pictorial ${command}.`;
...
This is a pinned shortcut for \`${commandPrefix}pictorial ${command}\`.

Invoke ${commandPrefix}pictorial ${command}, passing along any arguments provided here, and follow its instructions.
`;
}
...
function pin(command, projectRoot) {
  ...
  if (harnessDirs.length === 0) {
    console.log('No harness directories with pictorial installed found.');
    return false;
  }
...
function unpin(command, projectRoot) {
  ...
  if (removed > 0) {
    ...
    console.log(`Use Pictorial's '${command}' workflow directly to access it.`);
  } else {
