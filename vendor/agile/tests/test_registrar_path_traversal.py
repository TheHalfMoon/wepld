...
from agile_cli._utils import relative_extension_path_violation
...
class TestCommandFileTraversal:
    ...
    def test_claude_skips_traversal_in_file_field(self, tmp_path, bad_file):
        ...
        registered = registrar.register_commands(
            "claude",
            [{"name": "agile.myext.hello", "file": _resolve_payload(bad_file, outside_file), "aliases": []}],
            "myext",
            ext_dir,
            project,
        )
    ...
    def test_non_string_file_is_skipped(self, tmp_path, bad_value):
        ...
        registered = registrar.register_commands(
            "gemini",
            [{"name": "agile.myext.hello", "file": bad_value, "aliases": []}],
            "myext",
            ext_dir,
            project,
        )
    ...
    def test_dotdot_rejected_even_when_target_is_in_bounds(self, tmp_path):
        ...
        registered = registrar.register_commands(
            "gemini",
            [{"name": "agile.myext.hello", "file": "commands/../cmd.md", "aliases": []}],
            "myext",
            ext_dir,
            project,
        )
...
class TestReadSkipWarning:
    ...
    def test_unreadable_target_warns_and_skips(self, tmp_path):
        ...
        with pytest.warns(UserWarning):
            registered = registrar.register_commands(
                "gemini",
                [{"name": "agile.myext.hello", "file": "cmd.md", "aliases": []}],
                "myext",
                ext_dir,
                project,
            )
    ...
    def test_safe_command_and_alias_still_register(self, tmp_path):
        ...
        registered = registrar.register_commands(
            "claude",
            [_cmd("agile.myext.hello", ["agile.myext.hi"])],
            "myext",
            ext_dir,
            project,
        )

        assert "agile.myext.hello" in registered
        assert "agile.myext.hi" in registered
        assert (
            project
            / ".claude"
            / "skills"
    ...
    def test_copilot_nested_alias_creates_companion_prompt(self, tmp_path):
        ...
        registered = registrar.register_commands(
            "copilot",
            [_cmd("agile.myext.hello", ["group/run"])],
            "myext",
            ext_dir,
            project,
        )

        assert registered == ["agile.myext.hello", "group/run"]
