...
class TestExtensionLayout:
    ...
    def test_command_file_exists(self):
        cmd = EXT_DIR / "commands" / "agile.agent-context.update.md"
        assert cmd.is_file()
    ...
    def test_command_file_documents_context_file_constraints(self):
        text = (
            EXT_DIR / "commands" / "agile.agent-context.update.md"
...
class TestCatalogEntry:
    def test_catalog_lists_agent_context_as_bundled(self):
        ...
        assert entry["author"] == "agile-core"
...
def _run_powershell_agent_context_script_with_env(
    ...
    *,
    ...
    script = EXT_DIR / "scripts" / "powershell" / "update-agent-context.ps1"
    env = _bundled_script_env(project_root, agile_python=agile_python)
    return subprocess.run(
        [
            POWERSHELL,
            "-NoProfile",
...
class TestBundledUpdaterPathValidation:
    def test_bundled_script_env_makes_yaml_importable(self, tmp_path):
        ...
        result = subprocess.run(
            [env["AGILE_PYTHON"], "-c", "import yaml"],
            env=env,
            capture_output=True,
            text=True,
            timeout=30,
    ...
    @requires_bash
    def test_bash_script_falls_back_from_invalid_agile_python(self, tmp_path):
        project = tmp_path / "project"
        project.mkdir()
        _install_agent_context_config(
            project,
        ...
        result = _run_bash_agent_context_script(
            project,
            agile_python="/definitely/missing/python",
        )
    ...
    @pytest.mark.skipif(POWERSHELL is None, reason="PowerShell not available")
    def test_powershell_script_falls_back_from_invalid_agile_python(self, tmp_path):
        project = tmp_path / "project"
        project.mkdir()
        _install_agent_context_config(
            project,
        ...
        result = _run_powershell_agent_context_script_with_env(
            project,
            agile_python=str(project / "missing-python"),
        )
...
class TestCliDoesNotManageExtensionConfig:
    ...
    def test_config_helpers_are_removed(self):
        ...
        for name in (
            ...
            assert not hasattr(agile_cli, name), name

    def test_no_agent_context_config_symbols_in_source(self):
        src = PROJECT_ROOT / "src" / "agile_cli"
        offenders = []
...
class TestExtensionSelfSeed:
    ...
    agent->context-file defaults map (no Agile CLI dependency)."""
...
class TestMdcFrontmatter:
    ...
    def test_bash_script_skips_frontmatter_for_non_mdc(self, tmp_path):
        ...
        assert text.startswith("<!-- AGILE START -->")
...
_LEGACY_CONTEXT = (
    ...
    "Some user notes.\n\n"
    "<!-- AGILE START -->\n"
    "Legacy managed section written by an older Agile version.\n"
    "<!-- AGILE END -->\n\n"
    "More user notes.\n"
)
...
class TestBackwardCompatibility:
    ...
    def test_integration_setup_leaves_legacy_artifacts_untouched(self, tmp_path):
        ...
        cfg_path = (
            project / ".agile" / "extensions" / "agent-context"
            / "agent-context-config.yml"
        )
