 ...
 *     window.__PICTORIAL_LIVE_UI_SURFACES__ in the /live.js prelude.
 *   - skill/scripts/live-browser.js — publishes it on
 *     window.__PICTORIAL_LIVE_CHROME_CORE__ for adapters and E2E probes. That
 ...
 *   - the private pictorial-site repo — site/components/LiveUiGallery.astro
...
export const LIVE_UI_PREFIX = 'pictorial-live';

const id = (suffix) => `${LIVE_UI_PREFIX}-${suffix}`;

/**
 * The mount contract every Live chrome adapter (DOM, Svelte, ...) satisfies.
 * Published alongside the surfaces on __PICTORIAL_LIVE_CHROME_CORE__.
 */
