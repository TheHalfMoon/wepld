...
describe('just-in-time event instructions', () => {
  it('attaches situation-specific _instructions per event type', async () => {
    ...
    const svelteGen = instructionsForEvent({
      type: 'generate', id: 'ev3', count: 3, action: 'pictorial',
      scaffold: { previewMode: 'svelte-component', componentDir: 'node_modules/.pictorial-live/ev3', file: 'node_modules/.pictorial-live/ev3/manifest.json', sourceFile: 'src/routes/+page.svelte' },
    }, { scriptsPath: sp });
    assert.match(svelteGen, /EDIT the existing stubs node_modules\/\.pictorial-live\/ev3\/v1\.svelte/);
