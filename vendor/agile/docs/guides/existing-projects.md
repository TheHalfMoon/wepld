# Adopting Agile in an Existing Project

You do not need to recreate an existing system from specifications before using
Agile. Initialize the repository in place, capture the rules that matter,
...
```bash
agile init --here --force --integration <key>
```
...
`.agile/` project files and the command or skill files required by your
...
> **git** extension. Add it with `agile extension add git` if you want that
> workflow.

## 2. Capture Project Guardrails

Run `/agile.constitution` with principles that are already true for the
repository or that the team has explicitly agreed to adopt:

```text
/agile.constitution Preserve public API compatibility. Follow the existing
...
1. Run `/agile.clarify` to resolve uncertain behavior and compatibility
   requirements.
2. Run `/agile.plan` and verify that the proposed design reuses the existing
   architecture, dependencies, and test conventions.
3. Run `/agile.tasks`, then `/agile.analyze` to check consistency before
   implementation.
4. Run `/agile.implement` and review code and artifact changes together.
5. Run `/agile.converge` to find remaining gaps. If it adds tasks, repeat
