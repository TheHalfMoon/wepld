...
/**
 * Pictorial DevTools Extension - Content Script
 *
 ...
 */
(function () {
  if (window.__PICTORIAL_CS_LOADED__) return;
  window.__PICTORIAL_CS_LOADED__ = true;
  ...
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    if (msg.action === 'scan') {
      ...
      window.postMessage({ source: 'pictorial-command', action: 'toggle-overlays' }, '*');
      sendResponse({ ok: true });
      ...
      window.postMessage({ source: 'pictorial-command', action: 'highlight', selector: msg.selector }, '*');
      sendResponse({ ok: true });
    } else if (msg.action === 'unhighlight') {
      window.postMessage({ source: 'pictorial-command', action: 'unhighlight' }, '*');
      sendResponse({ ok: true });
    }
    return true;
  });
  ...
  window.addEventListener('message', (e) => {
    ...
    if (e.data.source === 'pictorial-overlays-toggled') {
      chrome.runtime.sendMessage({
  ...
  function sendScanCommand() {
    const msg = { source: 'pictorial-command', action: 'scan' };
  ...
  function injectAndScan() {
    ...
    script.dataset.pictorialExtension = 'true';
    pendingScan = true;
