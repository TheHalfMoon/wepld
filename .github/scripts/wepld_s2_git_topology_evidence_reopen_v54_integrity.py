#!/usr/bin/env python3
"""Authorize a bounded, single-use reopen of the frozen S2-S002/S2-S004
`crates/core/tests/project_v1.rs` test evidence gap.

v54 is an append-only policy successor over canonical v53 at main
420ce3f50406ec91c20964e4ad8d0ea861d2ca4b. Founder standing authorization for
...
test path so real adversarial fixtures for S2-S002 (symlink loop / broken
link) and S2-S004 (case-only path identity / never generic-lowercase, threat
model T-003) can be recorded. `crates/core/tests/project_v1.rs` is frozen by
v25 (`v25 freezes the completed canonical v24 Core observation tranche`), so a
fresh single-use reopen is required; it is not a wholly new file.

v54 grants exactly one thing and nothing else:
...
widen any authority: the reopened tests may only exercise the already-
qualified S2 observation surface through its existing public functions. Every
dangerous authority (general shell, arbitrary process, package install,
project-native command execution, Git mutation, `safe.directory` mutation,
remediation execution, network, model/provider execution, S3+) remains
`NONE`/unchanged and is asserted against the inherited v53 chain.

Package-load / resting-view note: v54 follows the v45..v53 discipline. It
owns a fresh `LocalRepositoryView` of the exact checked-out head, imports
frozen v53 under an exact v54->v53 workflow-entrypoint reversal, and inherits
every v53 hook by reference. The one path this successor reopens is subject to
...
predecessor delegation, exactly the way v45/v52/v53 intercept their own
...
P = ".github/scripts/wepld_s2_git_topology_evidence_reopen_v54_integrity.py"
T = ".github/scripts/wepld_s2_git_topology_evidence_reopen_v54_selftest.py"
T_BLOB = "4556d97c9a573b90437bcd37bee7fe473672500f"

V53_P_BLOB = "9e86a4844fd94f5fea1c07a7d527ec5334561451"
V53_T_BLOB = "ae19ffd896ed86a4832c789bb5bc6ac7316eda01"
...
_V53_ENTRYPOINT = b"wepld_s2_git_topology_evidence_reopen_v53_integrity.py"
_WORKFLOW_ENTRYPOINT_COUNTS = {FW: 3, AW: 2}

# Do not inherit a predecessor module's resting/projection view. v54 bases
...
def _v53_workflow_bytes(data: bytes, path: str) -> bytes:
    count = data.count(_V54_ENTRYPOINT)
    if count != _WORKFLOW_ENTRYPOINT_COUNTS[path]:
        base.fail(
            "v54 workflow entrypoint count drifted before v53 projection: "
            f"{path} expected={_WORKFLOW_ENTRYPOINT_COUNTS[path]} actual={count}"
        )
    return data.replace(_V54_ENTRYPOINT, _V53_ENTRYPOINT)


def _import_v53_under_workflow_projection() -> Any:
    """Import frozen v53 while it observes exact v53 workflow bytes.

    v53 (hence v50..v45..v36) reads workflow bytes while its module is
    imported, and the v53->v54 entrypoint migration ships in this same
    candidate, so v53 must not observe its own successor's bytes. Only
    ...
    def _v53_import_read_bytes(local_view: Any, relative: str, limit: int) -> bytes:
        data = original_read_bytes(local_view, relative, limit)
        if relative in (FW, AW):
            data = _v53_workflow_bytes(data, relative)
            if len(data) > limit:
                base.fail(
                    f"v54 v53-import workflow projection exceeds read bound: {relative}"
                )
        return data

    base.LocalRepositoryView.read_bytes = _v53_import_read_bytes
    try:
        return importlib.import_module(
            "wepld_s2_git_topology_evidence_reopen_v53_integrity"
        )
    finally:
        base.LocalRepositoryView.read_bytes = original_read_bytes


q = _import_v53_under_workflow_projection()
...
# --- S2-S002/S2-S004 project_v1.rs test-evidence reopen allowlist ---
...
REOPEN_TASKS = frozenset({"S2-S002", "S2-S004"})

# The exact pre-reopen frontier blob of the one reopened path, at canonical
# main 420ce3f50406ec91c20964e4ad8d0ea861d2ca4b (project_v1.rs as frozen by
# v25's Core-observation-tranche freeze).
...
# consumed.
EXACT_FROZEN_BLOB = "1e6d66e73983f2eb5b989dd202bc4117bcea3617"
...
for _path, _expected in ((q.P, V53_P_BLOB), (q.T, V53_T_BLOB), (T, T_BLOB)):
    _actual = V25.blob(raw_root.read_bytes(_path, base.MAX_POLICY_FILE_BYTES))
    if _actual != _expected:
        base.fail(
            f"frozen v54 package input drifted: {_path}: "
...
def _workflow_replacements(view: Any) -> dict[str, bytes]:
    ...
    for path in (FW, AW):
        ...
        predecessor = _v53_workflow_bytes(data, path)
        actual = V25.sha(predecessor)
        if actual != Q_WF[path]:
            base.fail(
                "v54 workflow does not reverse to exact canonical v53 predecessor: "
...
def req_v53(view: Any) -> None:
    for path, expected in ((q.P, V53_P_BLOB), (q.T, V53_T_BLOB)):
        if path not in V25.ps(view):
            base.fail(f"v54 candidate/base is missing frozen v53 predecessor: {path}")
        actual = V25.blob(view.read_bytes(path, base.MAX_POLICY_FILE_BYTES))
        if actual != expected:
            base.fail(
                f"frozen v53 predecessor drifted: {path}: "
                f"expected={expected} actual={actual}"
            )


def _project_for_v53(view: Any) -> Any:
...
def _v53_views(candidate: Any, policy_base: Any) -> tuple[Any, Any]:
    """Project the candidate to v53's view always; project the policy base
    only when it is a real post-v54 base. A pre-v54 bootstrap base predates
    the v53->v54 workflow migration and carries no v54 policy files, so it
    must reach v53's frozen hooks unprojected."""
    projected_candidate = _project_for_v53(candidate)
    if bootbase(policy_base):
        return projected_candidate, policy_base
    return projected_candidate, _project_for_v53(policy_base)


def _boot_base_for_selftest() -> Any:
    return _project_for_v53(raw_root)


def run_predecessor_selftests() -> None:
    """Run frozen v53's own self-tests once, under a v54->v53 workflow
    reversal.

    v53's corrected hooks are inherited by reference. Only ``read_bytes`` is
    wrapped here for the v54->v53 workflow reversal; the wrap is restored in
    ...
    def _v53_selftest_read_bytes(local_view: Any, relative: str, limit: int) -> bytes:
        if relative in workflow_reversal:
            data = workflow_reversal[relative]
            if len(data) > limit:
                base.fail(
                    f"v54 v53-selftest workflow projection exceeds read bound: {relative}"
                )
            return data
        return original_read_bytes(local_view, relative, limit)

    base.LocalRepositoryView.read_bytes = _v53_selftest_read_bytes
    try:
        _call("v53 self-tests under v54->v53 workflow reversal", q.selftest)
    finally:
        base.LocalRepositoryView.read_bytes = original_read_bytes
...
def _reopen_available(view: Any) -> bool:
    if REOPEN_TEST not in V25.ps(view):
        base.fail(f"v54 reopen base is missing the frozen reopened path: {REOPEN_TEST}")
...
def _require_reopen_base(view: Any) -> None:
    if not _reopen_available(view):
        base.fail(
            "v54 Git-topology evidence reopen already consumed; "
...
def _verify_text_reopen_file(view: Any, path: str) -> None:
    if path not in V25.ps(view):
        base.fail(f"v54 reopen path missing: {path}")
    if V25.mode(view, path) != "100644":
        base.fail(f"v54 reopen path mode invalid: {path}")
    data = view.read_bytes(path, base.MAX_POLICY_FILE_BYTES)
    if not data:
        base.fail(f"v54 reopen path must not be empty: {path}")
    if len(data) > MAX_REOPEN_FILE_BYTES:
        base.fail(f"v54 reopen path exceeds bounded size: {path}")
    try:
        data.decode("utf-8", errors="strict")
    except UnicodeError as exc:
        base.fail(f"v54 reopen path must be UTF-8: {path}: {exc}")
...
def _verify_reopen_candidate(candidate: Any, policy_base: Any) -> None:
    ...
    for path in (CORE_MANIFEST, ROOT_CARGO, ROOT_CARGO_LOCK, GIT_TOPOLOGY_MODULE, PROJECT_MODULE, CORE_EXPORT):
        if candidate.read_bytes(path, base.MAX_POLICY_FILE_BYTES) != policy_base.read_bytes(
            path, base.MAX_POLICY_FILE_BYTES
        ):
            base.fail(f"v54 Git-topology evidence reopen must not change frozen path: {path}")
    for relative in sorted(V25.FROZEN_STATE_PATHS):
        if candidate.read_bytes(relative, V25.MAX_S1_STATE_BYTES) != policy_base.read_bytes(
            ...
            base.fail(f"v54 candidate changed frozen S1 state: {relative}")
...
def delta(candidate: Any, policy_base: Any) -> None:
    ...
    if bootbase(policy_base):
        if paths != BOOT:
            if paths & BOOT:
                base.fail(
                    "v54 bootstrap delta must be exactly two v54 policy files plus two integrity workflows"
                )
            base.fail("v54 bootstrap base authorizes only exact Git-topology evidence reopen policy activation")
        req_v53(candidate)
        req_v53(policy_base)
        return

    if paths & CONTROLLED_FILES:
        base.fail("canonical v54 policy files are frozen after activation")

    reopen_changed = frozenset(paths & REOPEN_FILES)
    if reopen_changed:
        if paths != REOPEN_FILES:
            base.fail(
                "v54 Git-topology evidence reopen must not mix with non-reopen paths"
            )
        _verify_reopen_candidate(candidate, policy_base)
        return

    q.delta(_project_for_v53(candidate), _project_for_v53(policy_base))


def basectrl(candidate: Any, policy_base: Any) -> None:
    if not bootbase(policy_base):
        q.basectrl(*_v53_views(candidate, policy_base))
        return
    for path in sorted(base.BASE_CONTROLLED_PATHS):
        ...
        if path in (FW, AW):
            if V25.sha(candidate_bytes) != WF[path] or V25.sha(base_bytes) != Q_WF[path]:
                base.fail(f"v54 bootstrap workflow drifted: {path}")
...
def ext(candidate: Any, policy_base: Any, safe: Any) -> None:
    ...
    for path in sorted(CONTROLLED_FILES & safe_paths):
        if path not in V25.ps(candidate):
            base.fail(f"v54 controlled file missing: {path}")
        if bootbase(policy_base):
            if path in V25.ps(policy_base):
                base.fail(f"v54 controlled file unexpectedly exists in bootstrap base: {path}")
        elif path not in V25.ps(policy_base) or candidate.read_bytes(
            path, base.MAX_POLICY_FILE_BYTES
        ) != policy_base.read_bytes(path, base.MAX_POLICY_FILE_BYTES):
            base.fail(f"v54 steady-state controlled file drifted: {path}")
    ...
    if rest:
        projected_candidate, projected_base = _v53_views(candidate, policy_base)
...
def files(view: Any) -> None:
    ...
    for path in sorted(CONTROLLED_FILES):
        if path not in V25.ps(view):
            base.fail(f"v54 controlled file missing: {path}")
        if V25.mode(view, path) != "100644":
            base.fail(f"v54 controlled file mode invalid: {path}")
        if view.read_bytes(path, base.MAX_POLICY_FILE_BYTES) != approved[path]:
            base.fail(f"v54 controlled file content drifted: {path}")
    if not _reopen_available(view):
...
def verify_component_base(
    ...
    *,
    ...
    if _PREDECESSOR_COMPONENT_BASE is None:
        base.fail("v54 predecessor component-base hook unavailable")
    path_set = set(paths)
    if path_set & REOPEN_FILES:
        remaining = path_set - REOPEN_FILES
        _call(
            "v54 projected predecessor component-base verifier",
            _PREDECESSOR_COMPONENT_BASE,
            _project_for_v53(view),
            remaining - CONTROLLED_FILES,
            allow_core_main_change=False,
        )
        return
    _call(
        "v54 predecessor component-base verifier",
        _PREDECESSOR_COMPONENT_BASE,
        _project_for_v53(view),
...
def freeze_s1_007_state(candidate: Any, policy_base: Any) -> None:
    if _PREDECESSOR_FREEZE_S1 is None:
        base.fail("v54 predecessor S1 freeze hook unavailable")
    paths = V25.changed(V25.v24.v23, candidate, policy_base)
    if paths == REOPEN_FILES:
        for relative in sorted(V25.FROZEN_STATE_PATHS):
            if candidate.read_bytes(relative, V25.MAX_S1_STATE_BYTES) != policy_base.read_bytes(
    ...
    projected_candidate, projected_base = _v53_views(candidate, policy_base)
    _call(
        "v54 predecessor S1 state freeze",
...
def printer(stage: str, mode_: str) -> None:
    ...
    print("wepld_policy_successor_v54=S2_GIT_TOPOLOGY_EVIDENCE_REOPEN_ONLY")
    print(f"v54_authority={AUTH}")
    print(f"s2_implementation_authority_v54={S2_IMPLEMENTATION_AUTHORITY}")
    print(
        "git_topology_evidence_reopen_authority_v54="
        f"{GIT_TOPOLOGY_EVIDENCE_REOPEN_AUTHORITY}"
    )
    print(f"reopen_available_v54={_reopen_available(raw_root)}")
    print(f"doctor_cli_authority_v54={DOCTOR_CLI_AUTHORITY}")
    print(f"general_shell_authority_v54={GENERAL_SHELL_AUTHORITY}")
    print(f"arbitrary_process_authority_v54={ARBITRARY_PROCESS_AUTHORITY}")
    print(f"package_install_authority_v54={PACKAGE_INSTALL_AUTHORITY}")
    print(f"project_native_command_execution_v54={PROJECT_NATIVE_COMMAND_EXECUTION}")
    print(f"git_mutation_authority_v54={GIT_MUTATION_AUTHORITY}")
    print(f"safe_directory_mutation_authority_v54={SAFE_DIRECTORY_MUTATION_AUTHORITY}")
    print(f"remediation_execution_authority_v54={REMEDIATION_EXECUTION_AUTHORITY}")
    print(f"git_process_admission_v54={GIT_PROCESS_ADMISSION}")
    print(f"git_execution_authority_v54={GIT_EXECUTION_AUTHORITY}")
    print(f"external_process_authority_v54={EXTERNAL_PROCESS_AUTHORITY}")
    print(f"network_authority_v54={NETWORK_AUTHORITY}")
    print(f"model_provider_execution_v54={MODEL_PROVIDER_EXECUTION}")
    print(f"source_admission_v54={SOURCE_ADMISSION}")
    print(f"dependency_admission_v54={DEPENDENCY_ADMISSION}")
    print(f"s3_plus_authority_v54={S3_PLUS_AUTHORITY}")
    print(f"next_authority_gate_v54={NEXT_AUTHORITY_GATE}")
...
def prepare_q() -> None:
    for module in _chain():
        ...
        if current not in (Q_WF, dict(WF)):
            base.fail(f"v54 predecessor workflow identity map drifted: actual={current}")
    for module in _chain():
        module.WF = dict(WF)
...
def overlay() -> None:
    ...
    if any(actual is not expected for actual, expected in pairs):
        base.fail("v54 installed overlay drifted")
    if any(dict(module.WF) != dict(WF) for module in _chain()):
        base.fail("v54 workflow identity projection drifted")
    for name in (
        ...
        if getattr(q, name) != globals()[name]:
            base.fail(f"v54 inherited authority drifted: {name}")
...
def install() -> None:
    ...
    shell, routing, _, desktop, execution = V25.topo()
    pairs = (
        (_attr(routing, "IMPL_REQUIRE_EXACT_DELTA", "v53 routing hook"), q.delta),
        (base.compare_base_controlled, q.basectrl),
        (_attr(desktop, "verify_extension_controlled_paths", "v53 desktop hook"), q.dext),
        (_attr(execution, "verify_extension_controlled_paths", "v53 execution hook"), q.eext),
        (_attr(shell, "validate_allowed_paths", "v53 allowed hook"), q.allowed),
        (_attr(shell, "verify_policy_files", "v53 files hook"), q.files),
        (_attr(shell, "print_success", "v53 printer"), q.printer),
    )
    if any(actual is not expected for actual, expected in pairs):
        base.fail("v54 predecessor hook drifted")
    ...
    _bind(desktop, "EXTENSION_CONTROLLED_PATHS", desktop_extensions, "v54 desktop registration")
    _bind(execution, "EXTENSION_CONTROLLED_PATHS", execution_extensions, "v54 execution registration")
    _bind(routing, "IMPL_REQUIRE_EXACT_DELTA", delta, "v54 routing hook")
    base.compare_base_controlled = basectrl
    _bind(desktop, "verify_extension_controlled_paths", dext, "v54 desktop hook")
    _bind(execution, "verify_extension_controlled_paths", eext, "v54 execution hook")
    _bind(shell, "validate_allowed_paths", allowed, "v54 allowed hook")
    _bind(shell, "verify_policy_files", files, "v54 files hook")
    _bind(shell, "print_success", printer, "v54 printer hook")
    _bind(execution, "_verify_component_base", verify_component_base, "v54 component-base hook")
    _bind(execution, "freeze_s1_007_state", freeze_s1_007_state, "v54 S1 state freeze hook")
    _INST = True
    overlay()


def selftest() -> None:
    from wepld_s2_git_topology_evidence_reopen_v54_selftest import run
