...
``agile bundle install`` into an uninitialized directory must scaffold a Agile project first (FR-012), choosing the integration by precedence (FR-013):
...
from agile_cli.commands.bundle import _resolve_init_integration
...
def test_install_initializes_uninitialized_project(tmp_path: Path):
    ...
    assert (project / ".agile").is_dir()
    marker = project / ".agile" / "integration.json"
