...
from agile_cli.authentication import AUTH_REGISTRY, _register, get_provider
from agile_cli.authentication.azure_devops import AzureDevOpsAuth
from agile_cli.authentication.base import AuthProvider
from agile_cli.authentication.config import (
    AuthConfigEntry,
    find_entries_for_url,
    load_auth_config,
)
from agile_cli.authentication.github import GitHubAuth
...
class TestAzureDevOpsAuth:
    ...
    def test_resolve_token_azure_cli_success(self):
        ...
        with patch("agile_cli.authentication.azure_devops.subprocess.run", return_value=result):
    ...
    def test_resolve_token_azure_cli_resolves_executable(self):
        ...
        with patch(
            "agile_cli.authentication.azure_devops.shutil.which",
            return_value=resolved_path,
        ), patch(
            "agile_cli.authentication.azure_devops.subprocess.run",
            return_value=result,
        ) as run:
    ...
    def test_resolve_token_azure_cli_not_installed_returns_none(self):
        ...
        with patch("agile_cli.authentication.azure_devops.subprocess.run", side_effect=OSError("not found")):
            assert AzureDevOpsAuth().resolve_token(entry) is None
    ...
    def test_resolve_token_azure_cli_undecodable_output_returns_none(self):
        ...
        with patch("agile_cli.authentication.azure_devops.subprocess.run", side_effect=boom):
            assert AzureDevOpsAuth().resolve_token(entry) is None
    ...
    def test_resolve_token_azure_ad_oversized_response_returns_none(
        ...
        from agile_cli._download_security import MAX_JSON_METADATA_BYTES
        ...
        with patch("urllib.request.build_opener", return_value=mock_opener), patch(
            "agile_cli.authentication.azure_devops._json.loads",
...
class TestAuthenticatedHttp:
    def _set_config(self, monkeypatch, entries):
        from agile_cli.authentication import http as _mod
...
class TestRedirectStripping:
    def test_redirect_within_hosts_preserves_auth(self):
        from agile_cli.authentication.http import _StripAuthOnRedirect
        from urllib.request import Request
        import io
...
class TestFetchLatestReleaseTagDelegation:
    ...
    def test_gh_token_forwarded_when_configured(self, monkeypatch):
        from unittest.mock import MagicMock, patch
        from agile_cli._version import _fetch_latest_release_tag
