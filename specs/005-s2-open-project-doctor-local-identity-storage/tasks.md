...
`S2-P014..S2-P021` are the `acceptance.md` §A planning-package acceptance steps for the current repaired planning head. They are a separate track from S2 implementation/acceptance (`acceptance.md` §B..§J) and are **not** a direct prerequisite for `S2-A006` or §J `CLOSED_CANONICAL` — neither references `S2-P*`. The written transitive edge `S2-AUTH-001` ("re-read canonical S2 planning from live `main` after S2-P021") is superseded by canonical practice: six implementation tranches (PR #240, #274, #280, #283, #286, #287) merged under staged policy successors v25..v52 without S2-P021; `S2-AUTH-013/014/015` are `[x]` while `S2-AUTH-001..012` remain `[ ]`; the canonical state marker above is `NEXT_AUTHORITY_GATE = S2-ACCEPTANCE`. These rows are recorded here as an explicit separate track — not skipped, and not marked complete without their own §A evidence — and are surfaced at the `S2-A006` decision for Founder visibility.
...
- [x] **S2-CLI010** Shell-completion surface for new commands only if existing CLI architecture admits it. `SHELL_COMPLETION = PERMITTED_LIMITATION`: no WePLD CLI shell-completion generator is admitted for this binary to extend, and `acceptance.md` §F requires no completion surface. Recorded as a canonical permitted limitation, not a defect.
...
- [~] **S2-S003** Windows junction/reparse/extended-length path tests where runner capability permits. PARTIAL: extended-length verbatim-prefix path normalization (`\\?\` and `\\?\UNC\` forms) is proven by `crates/core/tests/identity_store_v1.rs::windows_root_forms_normalise_without_loss` (injected inputs, executed on Linux/macOS). Real Windows junction/reparse behavior is NOT proven — no native Windows `wepld-core` runtime (`acceptance.md` §H.1). Carried forward as an open, unowned cross-slice obligation to the owning later slice (expected S3); S2 acceptance does not discharge it.
