...
If `$ARGUMENTS` is empty, you must tell the user to provide an extension name, for example: `/agile.selftest.extension linear`.

## Steps

### Step 1: Catalog Discovery Validation

Check if the extension exists in the Agile catalog.
...
```bash
agile extension info "$ARGUMENTS"
```
...
```bash
agile extension add "$ARGUMENTS" --from "<download_url>"
```
...
```bash
cat .agile/extensions/.registry/$ARGUMENTS.json
```
...
test_selftest_discovery.py::test_catalog_search [PASS/FAIL]
  Details: [Provide execution result of agile extension search]

test_selftest_installation.py::test_extension_add [PASS/FAIL]
  Details: [Provide execution result of agile extension add]
