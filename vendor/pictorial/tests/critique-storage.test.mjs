...
describe('slugFromTarget', () => {
  ...
  it('normalizes URL casing and trailing slash', () => {
    assert.equal(
      slugFromTarget('https://Pictorial.Style/docs/audit/', { cwd }),
      'pictorial-style-docs-audit',
    );
  });
...
describe('writeSnapshot + readLatestSnapshot', () => {
  ...
  it('picks the newest snapshot across target slugs', () => {
    ...
    writeFileSync(join(cwd, '.pictorial', 'critique', 'ignore.md'), '# Critique ignores\n');
    writeFileSync(join(cwd, '.pictorial', 'critique', '9999-not-a-snapshot.md'), '# Draft\n');
  ...
  it('caller-supplied meta cannot override computed timestamp or slug', () => {
    // Defends against a corrupt PICTORIAL_CRITIQUE_META blob (parsed from
