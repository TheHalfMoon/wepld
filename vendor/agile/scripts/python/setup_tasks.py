...
def main(argv: list[str] | None = None) -> int:
    ...
    if not paths.impl_plan.is_file():
        ...
        print(
            f"Run {format_agile_command('plan', paths.repo_root)} first to create the implementation plan.",
            file=sys.stderr,
        )
        return 1
    ...
    if not paths.feature_spec.is_file():
        ...
        print(
            f"Run {format_agile_command('agile', paths.repo_root)} first to create the feature structure.",
            file=sys.stderr,
        )
        return 1
    ...
    if tasks_template_content is None:
        ...
        print(
            ...
            ".agile/templates/overrides/tasks-template.md, or run 'agile init' "
            "/ reinstall shared infra to restore the core "
            ".agile/templates/tasks-template.md template.",
            file=sys.stderr,
        )
        return 1
