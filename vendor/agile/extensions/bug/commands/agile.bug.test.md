...
Validate that the fix recorded by `__AGILE_COMMAND_BUG_FIX__` actually resolves the bug described by `__AGILE_COMMAND_BUG_ASSESS__`. The output is a verification report at `.agile/bugs/<slug>/test.md`.
...
2. **Conversation context** — if the current session has just run `__AGILE_COMMAND_BUG_ASSESS__` or `__AGILE_COMMAND_BUG_FIX__`, the slug it reported is the working slug. Reuse it without re-prompting. Confirm it by checking that `.agile/bugs/<slug>/fix.md` exists; if it does not, fall through.
3. **Single candidate on disk** — list `.agile/bugs/*/fix.md`. If exactly one bug has a `fix.md`, use it.
4. **Disambiguate**:
...
- `BUG_DIR/fix.md` MUST exist. If not, stop and instruct the user to run `__AGILE_COMMAND_BUG_FIX__` first.
...
4. **Write the verification report**
   ...
   - "Reopen — symptom still reproduces; rerun `__AGILE_COMMAND_BUG_ASSESS__`."
   ```

5. **Report back** with:
   - The slug and `BUG_DIR/test.md` path.
   - The result (`verified`, `partial`, `failed`).
   - If the result is `failed`, recommend re-running `__AGILE_COMMAND_BUG_ASSESS__` with the new evidence captured in `test.md`.

## Guardrails

- This command MUST NOT modify source code. It only runs checks and writes inside `.agile/bugs/<slug>/`.
