"""`agile init` must render user-supplied values literally, not as Rich markup.
...
* ``agile init "proj [v2]"`` succeeded and created the directory, but the
  Next Steps panel printed ``cd proj`` -- a command that fails when pasted.
* ``agile init "app[/red]x"`` created the directory and then died with
...
from agile_cli.commands.init import _shell_quote_arg
...
def _init(tmp_path: Path, name: str):
    """Run a fully offline, non-interactive `agile init <name>`."""
    previous = os.getcwd()
    os.chdir(tmp_path)
    try:
