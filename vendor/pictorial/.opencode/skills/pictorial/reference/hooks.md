...
Manual `npx pictorial detect` scans use the same project filter config by default: `detector.ignoreRules`, `detector.ignoreFiles`, `detector.ignoreValues`, and `detector.designSystem.enabled`. `hook.enabled` only controls automatic hook execution, not manual CLI scans. Use `npx pictorial detect --no-config ...` for a raw detector run that ignores project config/context. Use `npx pictorial ignores ...` for direct CLI CRUD on the same detector ignores.
...
- If the finding has no value-specific command, such as `side-tab`, scope that one rule to the file: `ignore-value <id> "*" --file <path>`. Run `npx pictorial detect <path>` first to see what actually fires there.
