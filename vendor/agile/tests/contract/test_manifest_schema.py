...
def test_explicit_null_agile_version_is_reported_as_missing():
    data = valid_manifest_dict()
    data["requires"]["agile_version"] = None
    manifest = BundleManifest.from_dict(data)
    assert manifest.requires.agile_version == ""
    assert any("agile_version" in e for e in manifest.structural_errors())
...
@pytest.mark.parametrize("bad", [[], "", 0, False, "agile>=0.1"])
...
def test_absent_provides_and_requires_do_not_raise_mapping_error():
    ...
    # non-mappings. (Structural completeness, e.g. requires.agile_version, is
