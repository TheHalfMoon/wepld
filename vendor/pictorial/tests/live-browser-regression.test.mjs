...
describe('live-browser.js regression guards', () => {
  ...
  it('uses a framework-component-gated painted-ancestor crop proxy for shader capture', () => {
    ...
    assert.match(
      SOURCE,
      /function shouldUseAncestorCropShaderProxy\(el\) \{[\s\S]{0,260}?window\.__PICTORIAL_LIVE_ADAPTER__[\s\S]{0,280}?isFrameworkComponentPreviewMode\(currentPreviewMode\) \|\| svelteComponentSession[\s\S]{0,260}?isFrameworkComponentPreviewMode\(wrapper\?\.dataset\?\.pictorialPreview\);/,
  ...
  it('discards variants without hiding the original or animating stale chrome', () => {
    assert.match(SOURCE, /function showOriginalDuringDiscard\(sessionId\)[\s\S]{0,900}?data-pictorial-variant="original"/);
  ...
  it('stores live state off the document root and preserves the selected anchor top', () => {
    assert.match(SOURCE, /window\.__PICTORIAL_LIVE_STATE__ = next/);
    assert.doesNotMatch(SOURCE, /document\.documentElement\.dataset\.pictorialLiveState/);
  ...
  it('SSE error reply clears the durable session checkpoint like discarded', () => {
    ...
    // pictorial-live* keys in the console. The error path must tear down the
  ...
  it('lets page editables keep Enter and arrow keydown events', () => {
    ...
    assert.match(
      ...
      'pictorial inline edit rows must keep their existing Escape-cancel path',
    );
  });
  ...
  it('pick mode preference persists in localStorage', () => {
    assert.match(
      SOURCE,
      /const INTERACTION_PREFS_KEY = 'pictorial-live-interaction';[\s\S]{0,3000}?function saveInteractionPrefs\(\)/,
  ...
  it('detect mode shows an empty result toast once per requested scan', () => {
    ...
    assert.match(
      SOURCE,
      /window\.postMessage\(\{ source: 'pictorial-command', action: 'remove' \}, '\*'\);[\s\S]{0,80}?activeDetectScanId = null;[\s\S]{0,80}?pendingDetectScanId = null;/,
  ...
  it('insert mode UI and generate payload guards', () => {
    ...
    assert.match(SOURCE, /data-pictorial-insert-placeholder/, 'placeholder element must be marked');
    assert.match(
      SOURCE,
      /showHighlight\(el\)[\s\S]{0,120}?data-pictorial-insert-placeholder/,
    ...
    assert.match(SOURCE, /data-pictorial-placeholder-resize/, 'placeholder edge handles on annotation overlay');
  ...
  it('reports mount failures to the server instead of only logging them', () => {
    assert.match(
      SOURCE,
      /console\.error\('\[pictorial\] Failed to mount component variant[\s\S]{0,200}?reportVariantMountFailed\(sessionId, variantNum, moduleUrl, err\);/,
