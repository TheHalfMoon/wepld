"""Tests for --integration flag on agile init (CLI-level)."""
...
class TestCliDiagnosticFormatting:
    def test_cli_error_detail_flattens_newlines(self):
        ...
        assert agile_cli._cli_error_detail(RuntimeError("line one\nline two")) == "line one line two"

    def test_cli_error_detail_handles_empty_message(self):
        import agile_cli

        assert agile_cli._cli_error_detail(RuntimeError()) == "RuntimeError"

    def test_cli_phase_label_includes_target(self):
        import agile_cli

        assert (
            agile_cli._cli_phase_label("rollback", "integration", "codex")
...
class TestInitIntegrationFlag:
    ...
    def test_integration_copilot_creates_files(self, tmp_path):
        ...
        assert (project / ".agile" / "scripts" / "bash" / "common.sh").exists()
        ...
        ext_cfg_path = project / ".agile" / "extensions" / "agent-context" / "agent-context-config.yml"
        assert not ext_cfg_path.exists(), "init must not create the agent-context extension config"

        assert (project / ".agile" / "integrations" / "copilot.manifest.json").exists()
    ...
    def test_noninteractive_init_defaults_to_copilot(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(agile_cli, "select_with_arrows", fail_select)
        ...
        assert f"defaulting to '{agile_cli.DEFAULT_INIT_INTEGRATION}'" in result.output
        assert (
            project / ".github" / "skills" / "agile-plan" / "SKILL.md"
        ).exists()

        data = json.loads((project / ".agile" / "integration.json").read_text(encoding="utf-8"))
        assert data["integration"] == agile_cli.DEFAULT_INIT_INTEGRATION
    ...
    def test_noninteractive_flag_skips_pickers_when_stdin_is_a_tty(
        ...
        from agile_cli import app
        import agile_cli
        import agile_cli.commands.init as init_mod
    ...
    def test_noninteractive_flag_here_force_completes_without_script_flag(
        ...
        assert result.exit_code == 0, result.output
        assert (project / ".agile" / "init-options.json").exists()
    ...
    def test_noninteractive_init_honors_default_integration_env_var(
        ...
        monkeypatch.setattr(agile_cli, "select_with_arrows", fail_select)
        monkeypatch.setenv(
            agile_cli.DEFAULT_INIT_INTEGRATION_ENV_VAR, "gemini"
        )
    ...
    def test_interactive_init_picker_default_honors_env_var(
        ...
        # AGILE_INTEGRATION_DEFAULT value as its default_key, not the
    ...
    def test_init_with_local_preset_seeds_manifest_constitution(
        ...
        assert result.exit_code == 0, result.output
        assert (
            project / ".agile" / "memory" / "constitution.md"
    ...
    def test_integration_claude_here_preserves_preexisting_commands(self, tmp_path):
        ...
        skill_dir = commands_dir / "agile-specify"
        ...
        assert command_file.exists()
        assert "agile-specify" in command_file.read_text(encoding="utf-8")
    ...
    def test_shared_infra_skips_existing_files_without_force(self, tmp_path):
        ...
        scripts_dir = project / ".agile" / "scripts" / "bash"
        ...
        templates_dir = project / ".agile" / "templates"
    ...
    def test_shared_infra_installs_python_scripts_for_py(self, tmp_path):
        ...
        assert (
            project / ".agile" / "scripts" / "python" / "common.py"
        ).exists()
    ...
    def test_shared_infra_removes_stale_managed_script(self, tmp_path):
        ...
        stale_rel = ".agile/scripts/bash/update-agent-context.sh"
        (scripts_dir / "update-agent-context.sh").write_text("# legacy orphan\n", encoding="utf-8")
        manifest = IntegrationManifest("agile", project, version="test")
        ...
        refreshed = IntegrationManifest.load("agile", project)
        ...
        assert ".agile/scripts/bash/common.sh" in refreshed.files
    ...
    def test_shared_infra_preserves_modified_stale_script(self, tmp_path):
        ...
        manifest = IntegrationManifest("agile", project, version="test")
        manifest.record_existing(".agile/scripts/bash/update-agent-context.sh")
        manifest.save()
    ...
    def test_shared_infra_empty_script_source_keeps_tracked_scripts(self, tmp_path, monkeypatch):
        ...
        from agile_cli import _install_shared_infra, shared_infra
        from agile_cli.integrations.manifest import IntegrationManifest
        ...
        scripts_dir = project / ".agile" / "scripts" / "bash"
        scripts_dir.mkdir(parents=True)
        tracked_rel = ".agile/scripts/bash/common.sh"
    ...
    def test_shared_infra_stale_cleanup_ignores_unsafe_manifest_keys(self, tmp_path):
        ...
        scripts_dir = project / ".agile" / "scripts" / "bash"
        scripts_dir.mkdir(parents=True)
        manifest_dir = project / ".agile" / "integrations"
        manifest_dir.mkdir(parents=True)

        # A file the traversal key would resolve to (outside scripts/bash/).
        victim = project / ".agile" / "scripts" / "keep-me.sh"
        ...
        traversal_key = ".agile/scripts/bash/../keep-me.sh"
        (manifest_dir / "agile.manifest.json").write_text(
            json.dumps({
                "integration": "agile",
                "version": "test",
    ...
    def test_shared_infra_stale_cleanup_skips_escaping_key_without_failing(
        ...
        from agile_cli import _install_shared_infra
        from agile_cli.integrations import manifest as manifest_mod
        from agile_cli.integrations.manifest import IntegrationManifest
    ...
    def test_shared_infra_warns_when_manifest_cannot_be_loaded(self, tmp_path, capsys):
        ...
        project.mkdir()
        integrations_dir = project / ".agile" / "integrations"
        integrations_dir.mkdir(parents=True)
        manifest_path = integrations_dir / "agile.manifest.json"
    ...
    def test_shared_template_refresh_refuses_symlinked_destination(self, tmp_path):
        ...
        from agile_cli import _refresh_shared_templates
    ...
    def test_shared_infra_refuses_symlinked_specify_directory_before_mkdir(self, tmp_path):
        """Shared infra installs must not follow a symlinked .agile directory."""
        from agile_cli import _install_shared_infra
        ...
        outside.mkdir()
        os.symlink(outside, project / ".agile")

        with pytest.raises(ValueError, match="symlinked"):
            _install_shared_infra(project, "sh", force=True)
        # Nothing should have been written under the symlinked .agile target.
    ...
    def test_shared_infra_refuses_symlinked_shared_manifest(self, tmp_path):
        ...
        from agile_cli.shared_infra import install_shared_infra
        ...
        outside.write_text("# outside\n", encoding="utf-8")
        os.symlink(outside, integrations_dir / "agile.manifest.json")
    ...
    def test_shared_template_refresh_preflights_before_writing(self, tmp_path):
        ...
        from agile_cli.shared_infra import refresh_shared_templates
    ...
    def test_shared_infra_install_supports_nested_script_sources(self, tmp_path):
        ...
        nested_dest = project / ".agile" / "scripts" / "bash" / "nested" / "deep.sh"
    ...
    def test_shared_infra_skip_warning_uses_posix_paths(self, tmp_path):
        ...
        project.mkdir()
        nested_dest = project / ".agile" / "scripts" / "bash" / "nested"
        nested_dest.mkdir(parents=True)
        (nested_dest / "deep.sh").write_text("# existing script\n", encoding="utf-8")

        templates_dest = project / ".agile" / "templates"
        ...
        assert ".agile/scripts/bash/nested/deep.sh" in output
        assert ".agile/templates/plan-template.md" in output
    ...
    def test_shared_template_writes_are_not_world_writable(self, tmp_path):
        ...
        written = project / ".agile" / "templates" / "plan-template.md"
    ...
    def test_init_here_force_overwrites_shared_infra(self, tmp_path):
        """E2E: agile init --here --force overwrites shared infra files."""
        from typer.testing import CliRunner
        from agile_cli import app
    ...
    def test_init_here_force_reapplies_installed_presets(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            with (
                patch(
                    "agile_cli.integrations._helpers._register_presets_for_agent",
                    mock_presets,
                ),
                patch(
                    "agile_cli.integrations._helpers._register_extensions_for_agent",
                    mock_extensions,
                ),
            ):
...
class TestForceExistingDirectory:
    ...
    def test_force_merges_into_existing_dir(self, tmp_path):
        """agile init <dir> --force succeeds when the directory already exists."""
        from typer.testing import CliRunner
        from agile_cli import app
        ...
        # Agile files should be installed
        assert (target / ".agile" / "init-options.json").exists()
        assert (target / ".agile" / "templates" / "spec-template.md").exists()

    def test_without_force_errors_on_existing_dir(self, tmp_path):
        """agile init <dir> without --force errors when directory exists."""
        from typer.testing import CliRunner
        from agile_cli import app
...
class TestGitExtensionOptIn:
    """Tests verifying that the git extension is opt-in (not auto-installed) during agile init."""
    ...
    def test_git_extension_not_auto_installed(self, tmp_path):
        ...
        ext_dir = project / ".agile" / "extensions" / "git"
    ...
    def test_git_extension_commands_not_registered_by_default(self, tmp_path):
        ...
        git_skills = [f for f in claude_skills.iterdir() if f.name.startswith("agile-git-")]
        assert len(git_skills) == 0, "git extension commands should not be registered by default"


class TestSharedInfraCommandRefs:
    """Verify _install_shared_infra resolves __AGILE_COMMAND_*__ in shared infra."""
    ...
    def _combined_script_content(project, script_type):
        ...
        return "\n".join(
            (project / ".agile" / "scripts" / script_dir / name).read_text(
                encoding="utf-8"
            )
            for name in names
        )
    ...
    def test_dot_separator_in_page_templates(self, tmp_path):
        """Markdown agents get /agile.<name> in page templates."""
        from agile_cli import _install_shared_infra
        ...
        assert "__AGILE_COMMAND_" not in content, "unresolved placeholder in plan-template.md"
        assert "/agile.plan" in content

        checklist = project / ".agile" / "templates" / "checklist-template.md"
        content = checklist.read_text(encoding="utf-8")
        assert "__AGILE_COMMAND_" not in content
        assert "/agile.checklist" in content

    def test_hyphen_separator_in_page_templates(self, tmp_path):
        """Skills agents get /agile-<name> in page templates."""
        from agile_cli import _install_shared_infra
        ...
        assert "__AGILE_COMMAND_" not in content, "unresolved placeholder in plan-template.md"
        assert "/agile-plan" in content
        assert "/agile.plan" not in content, "dot-notation leaked into skills page template"

        tasks = project / ".agile" / "templates" / "tasks-template.md"
        content = tasks.read_text(encoding="utf-8")
        assert "__AGILE_COMMAND_" not in content
        assert "/agile-tasks" in content

    def test_dollar_prefix_in_page_templates(self, tmp_path):
        """Dollar-style skills agents get $agile-<name> in page templates."""
        from agile_cli import _install_shared_infra
    ...
    def test_dot_separator_in_shared_scripts(self, tmp_path, script_type):
        """Markdown agents get /agile.<name> in shared script hints."""
        from agile_cli import _install_shared_infra
    ...
    def test_hyphen_separator_in_shared_scripts(self, tmp_path, script_type):
        """Skills agents get /agile-<name> in shared script hints."""
        from agile_cli import _install_shared_infra
    ...
    def test_dollar_prefix_in_shared_scripts(self, tmp_path, script_type):
        ...
        if script_type == "py":
            ...
            common = project / ".agile" / "scripts" / "python" / "common.py"
            namespace = runpy.run_path(str(common))
            assert namespace["format_agile_command"]("plan", project) == (
                "$agile-plan"
            )
            return
        ...
        assert "/agile-tasks" not in content
        if script_type == "sh":
            assert r"\$agile-specify" in content
            assert r"\$agile-plan" in content
            assert r"\$agile-tasks" in content
    ...
    def test_full_init_claude_resolves_page_templates(self, tmp_path):
        ...
        plan = project / ".agile" / "templates" / "plan-template.md"
        content = plan.read_text(encoding="utf-8")
        assert "/agile-plan" in content, "Claude (skills) should use /agile-plan"
        assert "__AGILE_COMMAND_" not in content
    ...
    def test_full_init_copilot_resolves_page_templates(self, tmp_path):
        ...
        plan = project / ".agile" / "templates" / "plan-template.md"
        content = plan.read_text(encoding="utf-8")
        assert "/agile-plan" in content, "Copilot skills should use /agile-plan"
        assert "/agile.plan" not in content
        assert "__AGILE_COMMAND_" not in content
    ...
    def test_full_init_copilot_commands_resolves_page_templates(self, tmp_path):
        ...
        plan = project / ".agile" / "templates" / "plan-template.md"
        content = plan.read_text(encoding="utf-8")
        assert "/agile.plan" in content, "Copilot --commands should use /agile.plan"
        assert "/agile-plan" not in content
        assert "__AGILE_COMMAND_" not in content
...
class TestIntegrationCatalogDiscoveryCLI:
    ...
    def _patch_catalog(self, monkeypatch, integrations=None):
        ...
        from agile_cli.integrations.catalog import IntegrationCatalog
    ...
    def test_integration_install_failure_reports_phase_target_and_rollback(
        ...
        from agile_cli.integrations import INTEGRATION_REGISTRY
        from agile_cli.integrations.base import IntegrationBase
    ...
    def test_integration_upgrade_failure_reports_phase_and_target(
        ...
        (project / ".agile" / "integrations").mkdir(parents=True, exist_ok=True)
        (project / ".agile" / "integration.json").write_text(
            json.dumps(
                {
                    "version": 1,
        ...
        (
            project / ".agile" / "integrations" / "upgrade-broken.manifest.json"
        ).write_text(
            json.dumps(
                {
                    "integration": "upgrade-broken",
    ...
    def test_integration_switch_cleanup_warning_reports_phase_and_targets(
        ...
        (project / ".agile" / "integrations" / "copilot.manifest.json").write_text(
            json.dumps(
                {
                    "integration": "copilot",
                    "version": "0.0.0",
    ...
    def test_primary_integration_commands_require_specify_project(self, tmp_path):
        ...
        for command in commands:
            ...
            assert "Not a Agile project" in result.output, failure_context

    def test_integration_commands_require_specify_directory(self, tmp_path):
        project = tmp_path / "bad"
        project.mkdir()
        (project / ".agile").write_text("not a directory")
    ...
    def test_catalog_config_output_uses_posix_paths(self, tmp_path):
        ...
        assert "Config saved to .agile/preset-catalogs.yml" in preset_add.output

        preset_list = self._invoke(["preset", "catalog", "list"], project)
        assert preset_list.exit_code == 0, preset_list.output
        assert "Config: .agile/preset-catalogs.yml" in preset_list.output
    ...
    def test_extension_catalog_add_rejects_non_mapping_config_root(self, tmp_path):
        ...
        assert "Invalid catalog config .agile/extension-catalogs.yml" in output
    ...
    def test_search_rejects_non_utf8_integration_json_before_catalog_lookup(
        ...
        (project / ".agile" / "integration.json").write_bytes(b"\xff\xfe\x00\x00")
    ...
    def test_search_local_config_error_shows_local_config_tip(
        ...
        """`integration search` must point at .agile/integration-catalogs.yml
        for local-config errors (not the generic 'temporarily unavailable')."""
        project = self._make_project(tmp_path)
        monkeypatch.setenv("HOME", str(tmp_path))
        monkeypatch.setenv("USERPROFILE", str(tmp_path))
        monkeypatch.delenv("AGILE_INTEGRATION_CATALOG_URL", raising=False)
        # Corrupt YAML to drive _load_catalog_config -> IntegrationValidationError.
        cfg = project / ".agile" / "integration-catalogs.yml"
        ...
        assert ".agile/integration-catalogs.yml" in normalized_output
        assert "~/.agile/integration-catalogs.yml" in normalized_output
    ...
    def test_search_invalid_env_catalog_url_shows_env_tip(
        ...
        normalized_output = _normalize_cli_output(result.output)
        assert result.exit_code == 1, result.output
        assert "AGILE_INTEGRATION_CATALOG_URL environment variable" in normalized_output
    ...
    def test_search_whitespace_env_catalog_url_uses_generic_catalog_tip(
        ...
        project = self._make_project(tmp_path)
        monkeypatch.setenv("AGILE_INTEGRATION_CATALOG_URL", "   ")

        from agile_cli.integrations.catalog import (
            IntegrationCatalog,
        ...
        assert (
            "AGILE_INTEGRATION_CATALOG_URL environment variable"
    ...
    def test_info_unknown_with_invalid_env_catalog_url_shows_env_tip(
        ...
        normalized_output = _normalize_cli_output(result.output)
        assert result.exit_code == 1, result.output
        assert "AGILE_INTEGRATION_CATALOG_URL" in normalized_output
        assert "unset it to use the configured catalog files" in normalized_output
        assert "Try again when online" not in normalized_output
    ...
    def test_catalog_add_then_remove_roundtrip(self, tmp_path, monkeypatch):
        ...
        cfg_path = project / ".agile" / "integration-catalogs.yml"
    ...
    def test_catalog_list_env_override_supersedes_project_config(
        ...
        normalized_output = _normalize_cli_output(result.output)
        assert result.exit_code == 0, result.output
        assert "AGILE_INTEGRATION_CATALOG_URL is set" in normalized_output
...
def test_refresh_shared_templates_preserves_recovered_user_file(tmp_path):
    ...
    """
    from agile_cli.shared_infra import (
        load_agile_manifest,
        refresh_shared_templates,
    )
    ...
    manifest = load_agile_manifest(project, version="test", console=_NoopConsole())
    rel = ".agile/templates/spec-template.md"
...
class TestExtensionFlag:
    """Tests for the --extension flag on agile init."""
    ...
    def _run_init(self, tmp_path, args, project_name="ext-test"):
        ...
        try:
            os.chdir(project)
            runner = CliRunner()
            # Patch get_agile_version to return a stable (non-dev) version so that
            # the extension compatibility check (SpecifierSet(">=0.2.0")) passes.
            with patch(
                "agile_cli.commands.init.get_agile_version",
                return_value="0.8.2",
            ):
                result = runner.invoke(app, [
                    "init", "--here",
    ...
    def test_multiple_extensions_installed(self, tmp_path):
        ...
        ext_dir_git = project / ".agile" / "extensions" / "git"
        ext_dir_selftest = project / ".agile" / "extensions" / "selftest"
    ...
    def test_local_path_extension_installed(self, tmp_path):
        ...
        from agile_cli import _locate_bundled_extension
    ...
    def test_url_extension_rejects_non_https(self, tmp_path):
        ...
        assert not (project / ".agile" / "extensions" / "ext").exists()
    ...
    def test_url_extension_skipped_without_trust(self, tmp_path):
        ...
        with patch(
            "agile_cli.commands.init._stdin_is_interactive", return_value=False
        ), patch("agile_cli.authentication.http.open_url") as mock_open:
            project, result = self._run_init(
                tmp_path,
        ...
        assert not (project / ".agile" / "extensions" / "git").exists()
    ...
    def test_noninteractive_flag_trust_urls_installs_without_confirm(
        ...
        def _cache_dir_stand_in(project_root):
            d = project_root / ".agile" / "extensions" / ".cache" / "downloads"
        ...
        with patch("typer.confirm", side_effect=fail_confirm), patch(
            ...
            "agile_cli.extensions._commands._validate_safe_cache_dir",
            side_effect=_cache_dir_stand_in,
        ), patch(
            "agile_cli.extensions._commands._safe_open_download_zip",
        ...
        assert result.exit_code == 0, f"init failed:\n{result.output}"
        assert (project / ".agile" / "extensions" / "git").exists()
    ...
    def test_url_extension_interactive_confirm_installs(self, tmp_path):
        ...
        with patch(
            "agile_cli.commands.init._stdin_is_interactive", return_value=True
    ...
    def test_url_extension_installs_zip(self, tmp_path):
        ...
        cache_dir = project / ".agile" / "extensions" / ".cache" / "downloads"
