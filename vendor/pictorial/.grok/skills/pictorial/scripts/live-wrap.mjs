...
export async function wrapCli() {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`Usage: pictorial wrap [options]
...
The agent should insert variant HTML at insertLine.`);
  ...
  const wrapperLines = isJsx ? [
    indent + '<div data-pictorial-variants="' + id + '" data-pictorial-variant-count="' + count + '"' + ' ' + styleContents + '>',
    indent + '  ' + commentSyntax.open + ' pictorial-variants-start ' + id + ' ' + commentSyntax.close,
    indent + '  ' + commentSyntax.open + ' Original ' + commentSyntax.close,
    indent + '  <div data-pictorial-variant="original">',
    reindentOriginal('    '),
    indent + '  </div>',
    indent + '  ' + commentSyntax.open + ' Variants: insert below this line ' + commentSyntax.close,
    indent + '  ' + commentSyntax.open + ' pictorial-variants-end ' + id + ' ' + commentSyntax.close,
    indent + '</div>',
  ] : [
    indent + commentSyntax.open + ' pictorial-variants-start ' + id + ' ' + commentSyntax.close,
    indent + '<div data-pictorial-variants="' + id + '" data-pictorial-variant-count="' + count + '"' + ' ' + styleContents + '>',
...
function buildCssSelectorPrefixExamples(styleMode, count) {
  ...
  return Array.from({ length: count }, (_, i) => `[data-pictorial-variant="${i + 1}"]`);
}
...
function buildCssAuthoring(styleMode, count) {
  ...
  if (styleMode.mode === 'astro-global-prefixed') {
    return {
      ...
      rulePattern: '[data-pictorial-variant="N"] > .variant-class { ... }',
      selectorExamples: variantNumbers.map((n) => `[data-pictorial-variant="${n}"] > .variant-class`),
      requirements: [
        ...
        'Prefix every preview selector with the matching [data-pictorial-variant="N"] selector.',
  ...
  return {
    ...
    styleTag: styleMode.styleTag,
    strategy: 'scope-rule',
    rulePattern: '@scope ([data-pictorial-variant="N"]) { :scope > .variant-class { ... } }',
    selectorExamples: variantNumbers.map((n) => `@scope ([data-pictorial-variant="${n}"]) { :scope > .variant-class { ... } }`),
    requirements: [
      'Use @scope blocks keyed to each [data-pictorial-variant="N"] wrapper.',
    ...
    forbidden: [
      'Do not use global [data-pictorial-variant="N"] selector prefixes for this styleMode.',
 ...
 *
 * Only `node_modules`, `.git`, and `.pictorial` are skipped outright.
...
function findElement(lines, query, tag = null) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (lines[i].includes('data-pictorial-variant')) continue;
