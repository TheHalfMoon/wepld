...
if (-not (Test-Path $paths.IMPL_PLAN -PathType Leaf)) {
    ...
    $planCommand = Format-AgileCommand -CommandName 'plan' -RepoRoot $paths.REPO_ROOT
...
if (-not (Test-Path $paths.FEATURE_SPEC -PathType Leaf)) {
    ...
    $specifyCommand = Format-AgileCommand -CommandName 'agile' -RepoRoot $paths.REPO_ROOT
...
if ($null -eq $tasksTemplateContent) {
    ...
    [Console]::Error.WriteLine("Template 'tasks-template' was not found in any supported location (overrides, presets, extensions, or shared core). Add an override at .agile/templates/overrides/tasks-template.md, or run 'agile init' / reinstall shared infra to restore the core .agile/templates/tasks-template.md template.")
    exit 1
}
