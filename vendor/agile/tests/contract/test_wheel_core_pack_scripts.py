...
``agile init --script <type>`` installs from ``agile_cli/core_pack/scripts/``
...
def test_every_script_variant_is_bundled_into_core_pack():
    ...
    for variant in variants:
        assert force_include.get(f"scripts/{variant}") == (
            f"agile_cli/core_pack/scripts/{variant}"
...
def test_python_script_variant_is_bundled():
    ...
    # invoked python3 .agile/scripts/python/*.py while the wheel bundled
    # only the bash and PowerShell variants.
    assert _force_include()["scripts/python"] == "agile_cli/core_pack/scripts/python"
