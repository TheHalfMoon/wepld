...
def test_plan_expands_all_components():
    plan = resolve_install_plan(
        _manifest(), agile_version="0.11.2", active_integration="copilot"
    )
...
def test_version_gate_refuses_incompatible():
    manifest = _manifest(requires={"agile_version": ">=99.0.0"})
    with pytest.raises(BundlerError, match="requires Agile"):
...
def test_pinned_integration_with_indeterminate_active_fails():
    ...
    with pytest.raises(BundlerError, match="could not be determined"):
        resolve_install_plan(
            manifest, agile_version="0.11.2", active_integration=None
        )
...
def test_pinned_integration_with_blank_active_fails(blank):
    ...
    with pytest.raises(BundlerError, match="could not be determined"):
        resolve_install_plan(
            manifest, agile_version="0.11.2", active_integration=blank
        )
...
def test_padded_active_integration_is_not_a_clash_with_itself():
    ...
    manifest = _manifest(integration={"id": "claude"})
    plan = resolve_install_plan(
        manifest, agile_version="0.11.2", active_integration="  claude  "
    )
...
def test_tool_requirements_become_warnings():
    manifest = _manifest(requires={"agile_version": ">=0.1.0", "tools": ["docker"]})
    plan = resolve_install_plan(
        manifest, agile_version="0.11.2", active_integration="copilot"
    )
