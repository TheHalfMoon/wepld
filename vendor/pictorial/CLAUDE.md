...
There is **one** user-invocable skill, `pictorial`, with **23 commands** underneath it. Users type `/pictorial polish`, `/pictorial audit`, etc. The skill is defined in `skill/`:
...
- `scripts/pin.mjs` — creates/removes lightweight redirect shims so users can have `/audit` as a standalone shortcut that delegates to `/pictorial audit`.
...
2. **Mode is not stored in PRODUCT.md.** It persists only in that surface's brief under `.pictorial/surfaces/`. There is no `## Register` field and no `extractRegister()`; PRODUCT.md's only bare-value field is `## Platform`. A `## Register` section left over from v3 is reported at boot as deprecated (see `lib/staleness.mjs`) and read by nothing.
...
Pictorial writes files into user projects, so a released version has to cope with artifacts an older one wrote. Three kinds of drift travel under "out of date" and they are handled separately:
...
**`doctor` is a utility command, not a design command.** It follows the `hooks` and `pin` pattern (a line in SKILL.src.md plus `reference/doctor.md`), not the Commands-table pattern. It is deliberately **not** in `PICTORIAL_SUB_COMMANDS`, `command-metadata.json`, `SKILL_CATEGORIES`, or `pin.mjs`'s `VALID_COMMANDS`, and it does not count toward the 23. Keep maintenance tooling out of the design menu.
...
- `skill/scripts/concept-seed.mjs` has no local catalog. It resolves data via `PICTORIAL_CATALOG_DIR` (private repo, evals, tests), then the roll API at github.com/TheHalfMoon/wepld, then a degraded promotion-only seed. Tests run against `tests/fixtures/concept-catalog/`.
- The choice-ping telemetry (`--chosen`) honors `DO_NOT_TRACK` and `PICTORIAL_NO_TELEMETRY` and only fires for API-dealt rolls.
- Site copy, changelog, theme, and count validation for site pages happen in pictorial-site; this repo's `validateProse` scans only the READMEs.
- The release script reads the changelog from `../pictorial-site/site/pages/changelog.astro` when releasing from here.
...
Editorial brief is at `docs/STYLE.md`. Read it before editing the READMEs or any user-facing copy. The rules exist because the project has been called out for AI prose before; site copy applies them in pictorial-site.
...
`validateProse` scans `README.md` and `README.npm.md`; site copy is validated in pictorial-site.
...
The build system compiles the pictorial skill from `skill/` to provider-specific formats in `dist/`. The default build is source-first and does not sync tracked root harness folders; the release build performs the tracked distribution sync:
...
- `{{available_commands}}` — auto-populated list of commands (from `PICTORIAL_SUB_COMMANDS` in `scripts/lib/utils.js`)
...
```text
This contribution was prepared by an AI agent that tried to ship unchecked vibes across a human boundary. Pictorial asks for an issue and maintainer approval first.
```
...
| `cli/bin/commands/skills.mjs` | `bun run test:cli-remote-e2e` | hits github.com/TheHalfMoon/wepld |
...
PICTORIAL_E2E_ONLY=vite8-react-modal bun run test:live-e2e # scope to one fixture
PICTORIAL_E2E_DEBUG=1 bun run test:live-e2e                # dump page DOM + dev-server tail on failure
```
...
- **Roots.** `skill/scripts/live/roots.mjs` resolves appRoot/repoRoot/contextRoot once at boot and persists `.pictorial/live/roots.json`; every live CLI calls `enterLiveRoot()` in its main guard and chdirs onto the manifest's appRoot. Never derive a live path from ambient cwd in a new script; go through the manifest.
- **Svelte preview modules must live under `node_modules/.pictorial-live`.** SvelteKit restricts vite `server.fs.allow` to src/lib, src/routes, .svelte-kit, and node_modules; a preview tree under `.pictorial/` 403s. Staleness is handled by per-publish revision dirs (`r<N>/`, bumped by the server on every done-reply), not by file watching.
...
**LLM agent (opt-in)**: set `PICTORIAL_E2E_AGENT=llm` to swap the fake agent for `tests/live-e2e/agents/llm-agent.mjs`. Default provider/model: OpenAI `gpt-5.6-terra` at medium reasoning effort (a frontier tier, matching what drives real live sessions); Anthropic and DeepSeek remain selectable via `PICTORIAL_E2E_LLM_PROVIDER`. Requires the selected provider's key in env (`OPENAI_API_KEY` by default); the test runner skips with a clear message when it's unset. Override the model with `PICTORIAL_E2E_LLM_MODEL` and the effort with `PICTORIAL_E2E_LLM_EFFORT`. Caching is on — live.md is the cacheable prefix, and after the first call subsequent fixtures pay only the cache-read rate. Pass rate on a typical sweep is 18/19; the modal fixture's intrinsic state-loss flake is amplified by LLM latency and may need a re-run. **This path hits the API and costs money** — keep it out of CI unless you really want it there.
...
- After bumping, run `bun run build:release` so the committed `./plugin` subtree (`plugin/.claude-plugin/plugin.json` + `plugin/skills/pictorial/SKILL.md`) is regenerated to the new version. The build validator (`validatePluginVersions` in `scripts/build.js`) fails if `marketplace.json`, the `./plugin` manifest, or the bundled `SKILL.md` frontmatter disagree with `plugin.json` — this guards the marketplace install path against version drift (issue #274).

**Chrome extension**:
- `extension/manifest.json` → `version`
- Bump when: extension code changes (`extension/`)

**Website changelog** (`site/pages/changelog.astro` in the private pictorial-site repo):
...
- User-facing only. Every item must be something an pictorial user would notice or act on (a new command behavior, rule, or fix). Leave out internal build/tooling/refactor details, dependency bumps, and generated-output syncs.
...
All commands live under `/pictorial`. To add a new one:
...
4. Add the command name to `PICTORIAL_SUB_COMMANDS` in `scripts/lib/utils.js`
...
8. Add its relationships to `COMMAND_RELATIONSHIPS` in pictorial-site's `sub-pages-data.js`
9. In the private pictorial-site repo: add the category to `site/scripts/data.js`, the symbol/number to `framework-viz.js`, and optionally an editorial wrapper under `site/content/skills/`
...
- pictorial-site: `site/pages/index.astro` meta descriptions and hero box
...
| pictorial-site `site/public/js/generated/counts.js` | its own build |
...
The eval framework lives in a separate private repo at `~/code/pictorial-evals/`. It measures whether the `/pictorial` skill improves or harms AI-generated frontend design by running the same brief through a model with and without the skill loaded.
...
```bash
cd ~/code/pictorial-evals
bun run serve            # dashboard on http://localhost:8723
```

The eval runners read this repo's skill from `../pictorial/skill/` and staged provider skills from `../pictorial/build/_data/dist/*`. Run `bun run build` in this repo before an eval sweep if you want the Claude/Gemini staged skills to reflect your latest edits.
