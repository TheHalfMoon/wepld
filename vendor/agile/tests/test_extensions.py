...
from agile_cli import extensions as _ext_module
from agile_cli.extensions import (
    CatalogEntry,
    CORE_COMMAND_NAMES,
...
from agile_cli._utils import version_satisfies
...
def _validate_safe_cache_dir_test_stand_in(project_root):
    ...
    download_dir = project_root / ".agile" / "extensions" / ".cache" / "downloads"
...
def valid_manifest_data():
    ...
    return {
        ...
        "requires": {
            "agile_version": ">=0.1.0",
            "commands": ["agile.tasks"],
        },
        "provides": {
            "commands": [
                {
                    "name": "agile.test-ext.hello",
...
def project_dir(temp_dir):
    """Create a mock agile project directory."""
    proj_dir = temp_dir / "project"
    proj_dir.mkdir()

    # Create .agile directory
    specify_dir = proj_dir / ".agile"
    specify_dir.mkdir()
...
class TestExtensionManifest:
    ...
    def test_valid_manifest(self, extension_dir):
        ...
        assert manifest.commands[0]["name"] == "agile.test-ext.hello"
    ...
    def test_load_core_command_names_discovers_from_source_checkout(self, monkeypatch):
        ...
        import agile_cli.extensions as ext
    ...
    def test_load_core_command_names_prefers_wheel_core_pack(self, monkeypatch):
        ...
        tree (#3274)."""
        from agile_cli.extensions import _load_core_command_names
        import agile_cli.extensions as ext
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            5,              # unquoted int
            True,           # YAML `yes`/`true`
            None,           # `agile_version:` written but left empty
            [">=0.1.0"],    # iterable: slips past SpecifierSet() entirely
            {"min": "0.1"},  # iterable: same
        ],
    )
    def test_non_string_agile_version(self, temp_dir, valid_manifest_data, bad):
        """A non-string requires.agile_version must be a ValidationError.
        ...
        valid_manifest_data["requires"]["agile_version"] = bad
        ...
        with pytest.raises(
            ValidationError, match="Invalid requires.agile_version"
        ):
            ExtensionManifest(manifest_path)

    def test_empty_agile_version(self, temp_dir, valid_manifest_data):
        """A blank requires.agile_version must be rejected, not treated as any."""
        import yaml

        valid_manifest_data["requires"]["agile_version"] = "   "
    ...
    def test_command_name_autocorrect_agile_prefix(self, temp_dir, valid_manifest_data):
        """Test that 'agile.command' is auto-corrected to 'agile.{ext_id}.command'."""
        import yaml

        valid_manifest_data["provides"]["commands"][0]["name"] = "agile.hello"
        ...
        assert "agile.hello" in manifest.warnings[0]
        assert "agile.test-ext.hello" in manifest.warnings[0]

    def test_command_name_autocorrect_matching_ext_id_prefix(self, temp_dir, valid_manifest_data):
        """Test that '{ext_id}.command' is auto-corrected to 'agile.{ext_id}.command'."""
        import yaml
        ...
        assert manifest.commands[0]["name"] == "agile.docguard.guard"
        assert len(manifest.warnings) == 1
        assert "docguard.guard" in manifest.warnings[0]
        assert "agile.docguard.guard" in manifest.warnings[0]
    ...
    def test_alias_free_form_accepted(self, temp_dir, valid_manifest_data):
        """Aliases are free-form — a 'agile.command' alias must be accepted unchanged."""
        import yaml

        valid_manifest_data["provides"]["commands"][0]["aliases"] = ["agile.hello"]
        ...
        assert manifest.commands[0]["aliases"] == ["agile.hello"]
    ...
    def test_required_section_not_mapping_rejected(
        ...
        so one bad extension made `agile extension list` exit 1 instead of
        listing the others.
        """
        import yaml
    ...
    def test_extension_metadata_field_not_string_rejected(
        ...
        `agile extension list` exit 1 with a raw traceback, hiding every
    ...
    def test_one_bad_manifest_does_not_hide_healthy_extensions(self, temp_dir):
        ...
        """
        ext_root = temp_dir / ".agile" / "extensions"
...
""",
    ...
    def test_non_dict_hook_entry_raises_validation_error(self, temp_dir, valid_manifest_data):
        ...
        valid_manifest_data["hooks"]["after_tasks"] = "agile.test-ext.hello"
    ...
    def test_hook_single_mapping_still_accepted(self, extension_dir):
        ...
        assert manifest.hooks["after_tasks"]["command"] == "agile.test-ext.hello"
    ...
    def test_hook_list_of_mappings_accepted(self, temp_dir, valid_manifest_data):
        ...
        valid_manifest_data["provides"]["commands"].append({
            "name": "agile.test-ext.bye",
            "file": "commands/bye.md",
            "description": "Second test command",
        })
        valid_manifest_data["hooks"]["after_tasks"] = [
            {"command": "agile.test-ext.hello", "description": "first"},
            {"command": "agile.test-ext.bye", "description": "second"},
        ]
...
class TestExtensionManager:
    ...
    def test_check_compatibility_invalid(self, extension_dir, project_dir):
        ...
        with pytest.raises(CompatibilityError, match="Extension requires agile"):
    ...
    def test_check_compatibility_non_string_specifier(self, project_dir, bad):
        ...
        manager = ExtensionManager(project_dir)
        manifest = SimpleNamespace(requires_agile_version=bad)
    ...
    def test_check_compatibility_allows_prerelease_builds(self, extension_dir, project_dir):
        """Prerelease agile builds should satisfy compatible version ranges."""
        manager = ExtensionManager(project_dir)
        manifest = ExtensionManifest(extension_dir / "extension.yml")
    ...
    def test_install_from_directory(self, extension_dir, project_dir):
        ...
        ext_dir = project_dir / ".agile" / "extensions" / "test-ext"
    ...
    def test_install_restores_execute_bit_on_shipped_scripts(
        ...
        ``.agile/extensions/<id>/scripts/...`` invocation would fail with
        ...
        installed = (
            project_dir / ".agile" / "extensions" / "test-ext" / "scripts" / "gate.sh"
        )
        manager = ExtensionManager(project_dir)
    ...
    def test_command_registrar_default_does_not_recover_active_skills_dir(
        ...
        from agile_cli.agents import CommandRegistrar as AgentCommandRegistrar
    ...
    def test_install_from_install_dir_is_rejected_without_data_loss(
        ...
        manager.install_from_directory(extension_dir, "0.1.0", register_commands=False)
        install_dir = project_dir / ".agile" / "extensions" / "test-ext"
    ...
    def test_install_rejects_extension_id_in_core_namespace(self, temp_dir, project_dir):
        ...
        manifest_data = {
            ...
            "requires": {"agile_version": ">=0.1.0"},
            "provides": {
                "commands": [
                    {
                        "name": "agile.analyze.extra",
    ...
    def test_install_accepts_free_form_alias(self, temp_dir, project_dir):
        """Aliases are free-form — a short 'agile.shortcut' alias must be preserved unchanged."""
        import yaml
        ...
        manifest_data = {
            ...
            "provides": {
                "commands": [
                    {
                        "name": "agile.alias-shortcut.cmd",
                        "file": "commands/cmd.md",
                        "aliases": ["agile.shortcut"],
                    }
                ]
            },
        }
        ...
        assert manifest.commands[0]["aliases"] == ["agile.shortcut"]
        assert manifest.warnings == []
    ...
    def test_install_rejects_namespace_squatting(self, temp_dir, project_dir):
        ...
        manifest_data = {
            ...
            "provides": {
                "commands": [
                    {
                        "name": "agile.other-ext.cmd",
                        "file": "commands/cmd.md",
                        "aliases": ["agile.squat-ext.ok"],
                    }
                ]
            },
        }
    ...
    def test_install_rejects_command_collision_with_installed_extension(self, temp_dir, project_dir):
        ...
        first_manifest = {
            ...
            "provides": {
                "commands": [
                    {
                        "name": "agile.ext-one.sync",
                        "file": "commands/cmd.md",
                        "aliases": ["agile.shared.sync"],
                    }
                ]
            },
        }
        ...
        installed_ext_dir = project_dir / ".agile" / "extensions" / "ext-one"
    ...
    def test_config_backup_on_remove(self, extension_dir, project_dir):
        ...
        backup_dir = project_dir / ".agile" / "extensions" / ".backup" / "test-ext"
...
"""
    ...
    def test_render_frontmatter_keeps_long_description_on_one_line(self):
        ...
        frontmatter = {"name": "agile-implement", "description": long_description}
    ...
    def test_adjust_script_paths_does_not_mutate_input(self):
        ...
        assert adjusted["scripts"]["sh"] == ".agile/scripts/bash/setup-plan.sh {ARGS}"
        assert adjusted["scripts"]["ps"] == ".agile/scripts/powershell/setup-plan.ps1 {ARGS}"

    def test_adjust_script_paths_preserves_extension_local_paths(self):
        """Extension-local script paths should not be rewritten into .agile/.agile."""
        from agile_cli.agents import CommandRegistrar as AgentCommandRegistrar
        registrar = AgentCommandRegistrar()
        original = {
            "scripts": {
                "sh": ".agile/extensions/test-ext/scripts/setup.sh {ARGS}",
                "ps": "scripts/powershell/setup-plan.ps1 {ARGS}",
            }
        }
        ...
        assert adjusted["scripts"]["sh"] == ".agile/extensions/test-ext/scripts/setup.sh {ARGS}"
        assert adjusted["scripts"]["ps"] == ".agile/scripts/powershell/setup-plan.ps1 {ARGS}"
    ...
    def test_adjust_script_paths_rewrites_extension_top_level_scripts(self):
        ...
        assert (
            adjusted["scripts"]["sh"]
            == ".agile/extensions/test-ext/scripts/bash/resolve-skill.sh {ARGS}"
        )
        assert (
            adjusted["scripts"]["ps"]
            == ".agile/scripts/powershell/setup-plan.ps1 -Json"
        )
    ...
    def test_rewrite_project_relative_paths_preserves_extension_local_body_paths(self):
        ...
        body = (
            "Read `.agile/extensions/test-ext/templates/spec.md`\n"
            "Run scripts/bash/setup-plan.sh\n"
        )

        rewritten = AgentCommandRegistrar.rewrite_project_relative_paths(body)

        assert ".agile/extensions/test-ext/templates/spec.md" in rewritten
        assert ".agile/scripts/bash/setup-plan.sh" in rewritten
    ...
    def test_rewrite_project_relative_paths_uses_extension_context_for_scripts(self):
        ...
        assert ".agile/extensions/test-ext/scripts/bash/ensure-skills.sh" in rewritten
        assert ".agile/scripts/bash/setup-plan.sh" in rewritten
        assert ".agile/templates/checklist.md" in rewritten
    ...
    def test_register_commands_for_claude(self, extension_dir, project_dir):
        ...
        assert "agile.test-ext.hello" in registered

        # Check command file was created
        cmd_file = claude_dir / "agile-test-ext-hello" / "SKILL.md"
    ...
    def test_command_with_aliases(self, project_dir, temp_dir):
        ...
        manifest_data = {
            ...
            "provides": {
                "commands": [
                    {
                        "name": "agile.ext-alias.cmd",
                        "file": "commands/cmd.md",
                        "aliases": ["agile.ext-alias.shortcut"],
                    }
                ]
            },
        }
        ...
        assert "agile.ext-alias.cmd" in registered
        assert "agile.ext-alias.shortcut" in registered
        assert (claude_dir / "agile-ext-alias-cmd" / "SKILL.md").exists()
        assert (claude_dir / "agile-ext-alias-shortcut" / "SKILL.md").exists()
    ...
    def test_unregister_commands_for_codex_skills_uses_mapped_names(self, project_dir):
        ...
        (skills_dir / "agile-specify").mkdir(parents=True)
        (skills_dir / "agile-specify" / "SKILL.md").write_text("body")
        (skills_dir / "agile-shortcut").mkdir(parents=True)
        (skills_dir / "agile-shortcut" / "SKILL.md").write_text("body")
        ...
        assert not (skills_dir / "agile-specify" / "SKILL.md").exists()
        assert not (skills_dir / "agile-shortcut" / "SKILL.md").exists()
    ...
    def test_unregister_commands_handles_legacy_dot_notated_files(self, project_dir):
        ...
        # Command name: agile.git.commit
        # Formatted name: agile-git-commit
        cmd_name = "agile.git.commit"
        formatted_name = "agile-git-commit"
    ...
    def test_codex_skill_registration_writes_skill_frontmatter(self, extension_dir, project_dir):
        ...
        skill_file = skills_dir / "agile-test-ext-hello" / "SKILL.md"
        assert skill_file.exists()

        content = skill_file.read_text()
        assert "name: agile-test-ext-hello" in content
        assert "description: Test hello command" in content
...
"""
        ...
        init_options = project_dir / ".agile" / "init-options.json"
        ...
        skill_file = skills_dir / "agile-ext-scripted-plan" / "SKILL.md"
        assert skill_file.exists()
        ...
        assert '.agile/scripts/bash/setup-plan.sh --json "$ARGUMENTS"' in content
    ...
    def test_all_skill_agents_register_commands_with_resolved_placeholders(
        ...
        manifest_data = {
            ...
            "provides": {
                "commands": [
                    {
                        "name": f"agile.ext-{agent_name}.run",
        ...
        skill_dir_name = f"agile-ext-{agent_name}-run"
        ...
        assert '.agile/scripts/bash/setup-plan.sh' in content
    ...
    def test_codex_skill_alias_frontmatter_matches_alias_name(self, project_dir, temp_dir):
        ...
        manifest_data = {
            ...
            "provides": {
                "commands": [
                    {
                        "name": "agile.ext-alias-skill.cmd",
                        "file": "commands/cmd.md",
                        "aliases": ["agile.ext-alias-skill.shortcut"],
                    }
                ]
            },
        }
        ...
        primary = skills_dir / "agile-ext-alias-skill-cmd" / "SKILL.md"
        alias = skills_dir / "agile-ext-alias-skill-shortcut" / "SKILL.md"

        assert primary.exists()
        assert alias.exists()
        assert "name: agile-ext-alias-skill-cmd" in primary.read_text()
        assert "name: agile-ext-alias-skill-shortcut" in alias.read_text()
    ...
    def test_codex_skill_registration_uses_fallback_script_variant_without_init_options(
        ...
        manifest_data = {
            ...
            "provides": {
                "commands": [
                    {
                        "name": "agile.ext-script-fallback.plan",
                        "file": "commands/plan.md",
                    }
                ]
            },
...
"""
        ...
        # Intentionally do NOT create .agile/init-options.json
        skills_dir = project_dir / ".agents" / "skills"
        skills_dir.mkdir(parents=True)
        ...
        skill_file = skills_dir / "agile-ext-script-fallback-plan" / "SKILL.md"
        assert skill_file.exists()

        content = skill_file.read_text()
        assert "{SCRIPT}" not in content
        if platform.system().lower().startswith("win"):
            assert ".agile/scripts/powershell/setup-plan.ps1 -Json" in content
        else:
            assert '.agile/scripts/bash/setup-plan.sh --json "$ARGUMENTS"' in content
    ...
    def test_codex_skill_registration_handles_non_dict_init_options(
        ...
        manifest_data = {
            ...
            "provides": {
                "commands": [
                    {
                        "name": "agile.ext-script-list-init.plan",
                        "file": "commands/plan.md",
                    }
                ]
            },
...
"""
        ...
        content = (skills_dir / "agile-ext-script-list-init-plan" / "SKILL.md").read_text()
        assert '.agile/scripts/bash/setup-plan.sh --json "$ARGUMENTS"' in content
    ...
    def test_codex_skill_registration_fallback_prefers_powershell_on_windows(
        ...
        monkeypatch.setattr(
            "agile_cli.integrations.base.platform.system", lambda: "Windows"
        )
        ...
        manifest_data = {
            ...
            "provides": {
                "commands": [
                    {
                        "name": "agile.ext-script-windows-fallback.plan",
                        "file": "commands/plan.md",
                    }
                ]
            },
...
"""
        ...
        skill_file = skills_dir / "agile-ext-script-windows-fallback-plan" / "SKILL.md"
        assert skill_file.exists()

        content = skill_file.read_text()
        assert ".agile/scripts/powershell/setup-plan.ps1 -Json" in content
        assert ".agile/scripts/bash/setup-plan.sh" not in content
    ...
    def test_codex_skill_registration_rewrites_extension_subdir_paths(
        ...
        content = (skills_dir / "agile-echelon-run" / "SKILL.md").read_text()
        assert ".agile/extensions/echelon/agents/control/commander.md" in content
        assert ".agile/extensions/echelon/knowledge-base/agent-scores.yaml" in content
        assert ".agile/extensions/echelon/templates/kill-report.md" in content
        ...
        assert ".agile/extensions/echelon/specs/" not in content
    ...
    def test_skill_registration_rewrites_extension_subdir_paths_in_aliases(
        ...
        ext_dir = self._make_subdir_extension(
            temp_dir, ext_id="ext-alias-paths", aliases=["agile.ext-alias-paths.go"]
        )
        ...
        alias_content = (
            skills_dir / "agile-ext-alias-paths-go" / "SKILL.md"
        ).read_text()
        assert (
            ".agile/extensions/ext-alias-paths/agents/control/commander.md"
            in alias_content
        )
    ...
    def test_markdown_registration_rewrites_extension_subdir_paths(
        ...
        content = (amp_dir / "agile.ext-md-paths.run.md").read_text()
        assert ".agile/extensions/ext-md-paths/agents/control/commander.md" in content
        assert "Read agents/" not in content
    ...
    def test_rewrite_extension_paths_only_rewrites_existing_subdirs(self, temp_dir):
        ...
        assert ".agile/extensions/ext-existing/agents/one.md" in rewritten
        assert "Also .agile/extensions/ext-existing/agents/three.md" in rewritten
        ...
        assert ".agile/extensions/ext-existing/.hidden/" not in rewritten
    ...
    def test_rewrite_extension_paths_handles_regex_special_replacement_text(
        ...
        assert f".agile/extensions/ext\\1/{weird_subdir}/file.md" in rewritten
    ...
    def test_register_commands_for_copilot(self, extension_dir, project_dir):
        ...
        cmd_file = agents_dir / "agile.test-ext.hello.agent.md"
        assert cmd_file.exists()

        # Verify NO plain .md file was created
        plain_md_file = agents_dir / "agile.test-ext.hello.md"
    ...
    def test_dev_register_commands_symlinks_rendered_copilot_agent(
        ...
        assert registered == ["agile.test-ext.hello"]

        cmd_file = agents_dir / "agile.test-ext.hello.agent.md"
        assert cmd_file.is_symlink()

        target = cmd_file.resolve()
        assert ".agile-dev" in target.parts
    ...
    def test_dev_register_commands_replaces_codex_dev_symlink(
        ...
        assert "name: agile-test-ext-hello" in skill_file.read_text(
            encoding="utf-8"
        )
    ...
    def test_dev_register_commands_falls_back_to_copy_when_symlink_fails(
        ...
        monkeypatch.setattr("agile_cli.agents.os.symlink", raise_symlink_error)
    ...
    def test_dev_register_commands_falls_back_to_copy_when_relpath_fails(
        ...
        monkeypatch.setattr("agile_cli.agents.os.path.relpath", raise_relpath_error)
    ...
    def test_copilot_companion_prompt_created(self, extension_dir, project_dir):
        ...
        prompt_file = project_dir / ".github" / "prompts" / "agile.test-ext.hello.prompt.md"
        assert prompt_file.exists()

        # Verify content has correct agent frontmatter
        content = prompt_file.read_text()
        assert content == "---\nagent: agile.test-ext.hello\n---\n"
    ...
    def test_copilot_aliases_get_companion_prompts(self, project_dir, temp_dir):
        ...
        manifest_data = {
            ...
            "provides": {
                "commands": [
                    {
                        "name": "agile.ext-alias-copilot.cmd",
                        "file": "commands/cmd.md",
                        "aliases": ["agile.ext-alias-copilot.shortcut"],
                    }
                ]
            },
        }
        ...
        assert (prompts_dir / "agile.ext-alias-copilot.cmd.prompt.md").exists()
        assert (prompts_dir / "agile.ext-alias-copilot.shortcut.prompt.md").exists()
    ...
    def test_unregister_skill_removes_parent_directory(self, project_dir, temp_dir):
        ...
        registrar = CommandRegistrar()
        from agile_cli.extensions import ExtensionManifest
        manifest = ExtensionManifest(ext_dir / "extension.yml")
        registrar.register_commands_for_agent("codex", manifest, ext_dir, project_dir)

        skill_subdir = skills_dir / "agile-cleanup-ext-run"
        assert skill_subdir.exists(), "Skill subdirectory should exist after registration"
        assert (skill_subdir / "SKILL.md").exists()

        registrar.unregister_commands({"codex": ["agile.cleanup-ext.run"]}, project_dir)
...
class TestIntegration:
    ...
    def test_full_install_and_remove_workflow(self, extension_dir, project_dir):
        ...
        cmd_file = project_dir / ".claude" / "skills" / "agile-test-ext-hello" / "SKILL.md"
        assert cmd_file.exists()
    ...
    def test_copilot_cleanup_removes_prompt_files(self, extension_dir, project_dir):
        ...
        agent_file = agents_dir / "agile.test-ext.hello.agent.md"
        prompt_file = project_dir / ".github" / "prompts" / "agile.test-ext.hello.prompt.md"
...
class TestExtensionCatalog:
    ...
    def test_catalog_initialization(self, temp_dir):
        ...
        project_dir.mkdir()
        (project_dir / ".agile").mkdir()

        catalog = ExtensionCatalog(project_dir)

        assert catalog.project_root == project_dir
        assert catalog.cache_dir == project_dir / ".agile" / "extensions" / ".cache"
    ...
    def test_search_all_extensions(self, temp_dir):
        ...
        config_path = project_dir / ".agile" / "extension-catalogs.yml"
    ...
    def test_info_renders_non_numeric_downloads(self, downloads):
        ...
        from unittest.mock import MagicMock
        from agile_cli.extensions._commands import _print_extension_info
    ...
    def test_search_survives_non_numeric_downloads(self, temp_dir, downloads):
        """`agile extension search` must not abort when a catalog entry's
    ...
    def test_search_and_info_tolerate_non_list_tags(self, temp_dir):
        ...
        with patch.object(ExtensionCatalog, "_get_merged_extensions", return_value=merged), \
                patch("agile_cli.extensions._commands._require_specify_project",
    ...
    def test_fetch_single_catalog_sends_auth_header(self, temp_dir, monkeypatch):
        ...
        with patch("agile_cli.authentication.http.urllib.request.build_opener", return_value=mock_opener):
    ...
    def test_fetch_single_catalog_rejects_malformed_cached_payload(
        ...
        Without this, a cache poisoned by an older agile version (or a
    ...
    def test_download_extension_sends_auth_header(self, temp_dir, monkeypatch):
        ...
        with patch.object(catalog, "get_extension_info", return_value=ext_info), \
             patch("agile_cli.authentication.http.urllib.request.build_opener", return_value=mock_opener):
    ...
    def test_download_extension_rejects_oversized_body_without_output(
        ...
        from unittest.mock import patch
        from agile_cli._download_security import (
...
class TestCatalogStack:
    ...
    def _make_project(self, temp_dir: Path) -> Path:
        """Create a minimal agile project directory."""
        project_dir = temp_dir / "project"
        project_dir.mkdir()
        (project_dir / ".agile").mkdir()
        return project_dir
    ...
    def _write_catalog_config(self, project_dir: Path, catalogs: list) -> None:
        """Write extension-catalogs.yml to project .agile dir."""
        import yaml as yaml_module
    ...
    def test_env_var_overrides_default_stack(self, temp_dir, monkeypatch):
        """AGILE_CATALOG_URL replaces the entire default stack."""
        project_dir = self._make_project(temp_dir)
        custom_url = "https://example.com/catalog.json"
        monkeypatch.setenv("AGILE_CATALOG_URL", custom_url)
    ...
    def test_env_var_invalid_url_raises(self, temp_dir, monkeypatch):
        """AGILE_CATALOG_URL with http:// (non-localhost) raises ValidationError."""
        project_dir = self._make_project(temp_dir)
        monkeypatch.setenv("AGILE_CATALOG_URL", "http://example.com/catalog.json")
    ...
    def test_load_catalog_config_missing_file(self, temp_dir):
        ...
        result = catalog._load_catalog_config(project_dir / ".agile" / "nonexistent.yml")
...
class TestExtensionIgnore:
    ...
    def test_no_extensionignore(self, temp_dir, valid_manifest_data):
        ...
        proj_dir = temp_dir / "project"
        proj_dir.mkdir()
        (proj_dir / ".agile").mkdir()

        manager = ExtensionManager(proj_dir)
        manager.install_from_directory(ext_dir, "0.1.0", register_commands=False)

        dest = proj_dir / ".agile" / "extensions" / "test-ext"
...
class TestExtensionAddCLI:
    ...
    def test_catalog_add_escapes_config_saved_path_markup(self, tmp_path):
        ...
        display_path = "project[red]/.agile/extension-catalogs.yml"

        runner = CliRunner()
        with patch.object(Path, "cwd", return_value=project_dir), \
             patch("agile_cli.extensions._commands._display_project_path", return_value=display_path):
            result = runner.invoke(
                app,
                [
                    "extension",
    ...
    def test_catalog_add_escapes_config_read_exception_markup(self, tmp_path):
        ...
        with patch.object(Path, "cwd", return_value=project_dir), \
             patch(
                 "agile_cli.extensions._commands.yaml.safe_load",
    ...
    def test_add_dev_links_copilot_agent_when_supported(
        ...
        if can_create_symlink(temp_dir):
            ...
            assert ".agile-dev" in agent_file.resolve().parts
        else:
    ...
    def test_add_makes_shipped_scripts_executable(self, extension_dir, project_dir):
        ...
        non-executable and a documented `.agile/extensions/<id>/scripts/...`
    ...
    def test_add_dev_falls_back_to_copy_when_windows_symlinks_unavailable(
        ...
        monkeypatch.setattr(
            "agile_cli.agents.os.symlink", raise_windows_symlink_error
        )
    ...
    def test_add_by_display_name_uses_resolved_id_for_download(self, tmp_path):
        ...
        project_dir.mkdir()
        (project_dir / ".agile").mkdir()
        (project_dir / ".agile" / "extensions").mkdir(parents=True)
        ...
        with patch("agile_cli.extensions.ExtensionCatalog", return_value=mock_catalog), \
    ...
    def test_command_safe_id_rejects_leading_hyphen(self):
        ...
        from agile_cli.extensions._commands import _command_safe_id
    ...
    def test_info_discovery_only_shows_candidate_archive_url(self, tmp_path):
        ...
        with patch("agile_cli.extensions.ExtensionCatalog", return_value=mock_catalog), \
             patch("agile_cli.extensions.ExtensionManager") as mock_mgr, \
             patch.object(Path, "cwd", return_value=project_dir):
        ...
        assert "agile extension add acme-thing --from <archive-url>" in output
    ...
    def test_add_bundled_extension_not_found_gives_clear_error(self, tmp_path):
        ...
        with patch("agile_cli.extensions.ExtensionCatalog", return_value=mock_catalog), \
             patch("agile_cli._locate_bundled_extension", return_value=None), \
             patch.object(Path, "cwd", return_value=project_dir):
            result = runner.invoke(
                app,
                ["extension", "add", "git"],
        ...
        assert result.exit_code != 0
        assert "bundled with agile" in result.output
    ...
    def test_add_from_url_prompts_before_spinner(self, tmp_path):
        ...
        runner = CliRunner()
        with patch.object(Path, "cwd", return_value=project_dir), \
             patch("agile_cli.console.status", side_effect=record_status), \
    ...
    def test_add_from_invalid_url_exits_before_prompt(self, tmp_path, url):
        ...
        with patch.object(Path, "cwd", return_value=project_dir), \
             ...
             patch("agile_cli.authentication.http.open_url") as open_url:
            result = runner.invoke(
                app,
    ...
    def test_add_post_install_hint_escapes_manifest_id_markup(self, tmp_path):
        ...
        with patch.object(Path, "cwd", return_value=project_dir), \
             ...
             patch("agile_cli.extensions._commands._validate_safe_cache_dir", side_effect=_validate_safe_cache_dir_test_stand_in), \
             patch("agile_cli.authentication.http.open_url", return_value=FakeResponse(_MINIMAL_ZIP_BYTES)), \
             patch("agile_cli.extensions._commands._safe_open_download_zip", side_effect=_open_test_download_zip), \
        ...
        assert result.exit_code == 0, result.output
        assert ".agile/extensions/[red]bad[/red]/" in result.output
    ...
    def test_add_from_url_escapes_download_exception_markup(self, tmp_path):
        ...
        with patch.object(Path, "cwd", return_value=project_dir), \
             ...
             patch("agile_cli.extensions._commands._validate_safe_cache_dir", side_effect=_validate_safe_cache_dir_test_stand_in), \
             patch(
                 "agile_cli.authentication.http.open_url",
    ...
    def test_add_from_url_rejects_oversized_download_before_install(
        ...
        from unittest.mock import patch
        from agile_cli import app
        from agile_cli.extensions import _commands as extension_commands
    ...
    def test_add_from_url_resolves_ghes_release_asset(self, tmp_path):
        ...
        with patch.object(Path, "cwd", return_value=project_dir), \
             patch("typer.confirm", return_value=True), \
             patch("agile_cli.extensions._commands._validate_safe_cache_dir", side_effect=_validate_safe_cache_dir_test_stand_in), \
             patch("agile_cli.authentication.http.github_provider_hosts", return_value=("ghes.example",)), \
             patch("agile_cli.authentication.http.open_url", side_effect=fake_open_url), \
             patch("agile_cli.extensions._commands._safe_open_download_zip", side_effect=_open_test_download_zip), \
    ...
    def test_add_from_url_uses_cache_tempfile_for_untrusted_extension_name(self, tmp_path):
        ...
        project_dir.mkdir()
        (project_dir / ".agile").mkdir()
        downloads_dir = project_dir / ".agile" / "extensions" / ".cache" / "downloads"
        installed = {}
...
class TestDownloadExtensionBundled:
    ...
    def test_download_extension_raises_for_bundled(self, temp_dir):
        ...
        with patch.object(catalog, "get_extension_info", return_value=bundled_ext_info):
            with pytest.raises(ExtensionError, match="bundled with agile"):
...
class TestExtensionUpdateCLI:
    ...
    def _create_catalog_zip(
        ...
        manifest = {
            ...
            "requires": {"agile_version": ">=0.1.0"},
            "provides": {"commands": [{"name": "agile.test-ext.hello", "file": "commands/hello.md"}]},
        }
    ...
    def test_update_preflights_entry_count_before_opening_zip(
        ...
        with patch.object(Path, "cwd", return_value=project_dir), \
             ...
             patch(
                 "agile_cli._download_security.zipfile.ZipFile",
    ...
    def test_update_success_preserves_installed_at(
        ...
        original_config_content = (
            project_dir / ".agile" / "extensions" / "test-ext" / "linear-config.yml"
        ).read_text()
        ...
        def fake_install_from_zip(self_obj, _zip_path, agile_version):
            return self_obj.install_from_directory(v2_dir, agile_version)
    ...
    def test_update_failure_rolls_back_registry_hooks_and_commands(self, tmp_path, monkeypatch):
        ...
        hooks_before = yaml.safe_load((project_dir / ".agile" / "extensions.yml").read_text())

        registered_commands = backup_registry_entry.get("registered_commands", {})
        command_files = []
        from agile_cli.agents import CommandRegistrar as AgentRegistrar
        ...
        hooks_after = yaml.safe_load((project_dir / ".agile" / "extensions.yml").read_text())
    ...
    def test_update_failure_after_skill_registration_restores_old_skills(
        ...
        assert not (skills_dir / "agile-test-ext-fresh").exists()
        for command_name in ("hello", "new", "fresh"):
            qualified_name = f"agile.test-ext.{command_name}"
            assert not (
...
class TestExtensionPriority:
    ...
    def test_corrupted_extension_entry_not_picked_up_as_unregistered(self, project_dir):
        ...
        extensions_dir = project_dir / ".agile" / "extensions"
        ...
        from agile_cli.presets import PresetResolver
...
class TestExtensionPriorityCLI:
    ...
    def test_set_priority_not_installed(self, project_dir):
        ...
        # Ensure .agile exists
        (project_dir / ".agile").mkdir(parents=True, exist_ok=True)
...
class TestHookExecutorRegistration:
    ...
    def test_register_hooks_single_mapping_back_compat(self, project_dir):
        ...
        executor.register_hooks(
            _StubManifest("ext-a", {"after_tasks": {"command": "agile.ext-a.go"}})
        )
        ...
        assert entries[0]["command"] == "agile.ext-a.go"
    ...
    def test_register_hooks_multiple_entries_same_event(self, project_dir):
        ...
        executor.register_hooks(
            _StubManifest(
                ...
                {
                    "after_tasks": [
                        {"command": "agile.ext-a.first", "description": "1st"},
                        {"command": "agile.ext-a.second", "description": "2nd"},
                    ]
                },
            )
        )
    ...
    def test_register_hooks_dedup_on_extension_and_command(self, project_dir):
        ...
        manifest = _StubManifest(
            ...
            {
                "after_tasks": [
                    {"command": "agile.ext-a.first", "description": "v1"},
                    {"command": "agile.ext-a.second", "description": "v1"},
                ]
            },
        )
        ...
        entries = executor.get_project_config()["hooks"]["after_tasks"]
        assert len(entries) == 2
        first = next(e for e in entries if e["command"] == "agile.ext-a.first")
    ...
    def test_register_hooks_shape_change_removes_orphans(self, project_dir):
        ...
        executor.register_hooks(
            _StubManifest(
                ...
                {
                    "after_tasks": [
                        {"command": "agile.ext-a.first"},
                        {"command": "agile.ext-a.second"},
                    ]
                },
            )
        )
        ...
        executor.register_hooks(
            _StubManifest("ext-a", {"after_tasks": {"command": "agile.ext-a.first"}})
        )

        entries = executor.get_project_config()["hooks"]["after_tasks"]
        assert [e["command"] for e in entries] == ["agile.ext-a.first"]
    ...
    def test_register_hooks_skips_entry_without_command(self, project_dir):
        ...
        entries = executor.get_project_config()["hooks"]["after_tasks"]
        assert [e["command"] for e in entries] == ["agile.ext-a.go"]
    ...
    def test_register_hooks_skips_non_dict_entry(self, project_dir):
        ...
        executor.register_hooks(
            _StubManifest(
                "ext-a",
                {"after_tasks": [{"command": "agile.ext-a.go"}, "not-a-mapping"]},
            )
        )
    ...
    def test_register_hooks_purges_dropped_event_orphans(self, project_dir):
        ...
        executor.register_hooks(
            _StubManifest(
                "ext-a",
                {
                    "after_tasks": {"command": "agile.ext-a.tasks"},
                    "after_plan": {"command": "agile.ext-a.plan"},
                    "after_implement": {"command": "agile.ext-a.impl"},
                },
            )
        )
        executor.register_hooks(
            _StubManifest("ext-b", {"after_plan": {"command": "agile.ext-b.plan"}})
        )

        executor.register_hooks(
            _StubManifest("ext-a", {"after_tasks": {"command": "agile.ext-a.tasks"}})
        )

        hooks = executor.get_project_config()["hooks"]
        assert [e["command"] for e in hooks["after_tasks"]] == ["agile.ext-a.tasks"]
        assert [e["command"] for e in hooks["after_plan"]] == ["agile.ext-b.plan"]
    ...
    def test_register_hooks_dropping_all_hooks_purges_orphans(self, project_dir):
        ...
        executor.register_hooks(
            _StubManifest("ext-b", {"after_tasks": {"command": "agile.ext-b.go"}})
        )

        executor.register_hooks(_StubManifest("ext-a", {}))

        hooks = executor.get_project_config()["hooks"]
        assert [e["command"] for e in hooks["after_tasks"]] == ["agile.ext-b.go"]
    ...
    def test_register_hooks_duplicate_command_moves_to_end(self, project_dir):
        ...
        executor.register_hooks(
            _StubManifest(
                ...
                {
                    "after_tasks": [
                        {"command": "agile.ext-a.dup", "description": "first"},
                        {"command": "agile.ext-a.other"},
                        {"command": "agile.ext-a.dup", "description": "last"},
                    ]
                },
            )
        )
    ...
    def test_get_hooks_for_event_sorts_by_priority(self, project_dir):
        ...
        executor.register_hooks(
            _StubManifest(
                ...
                {
                    "after_tasks": [
                        {"command": "agile.ext-a.mid", "priority": 10},
                        {"command": "agile.ext-a.first", "priority": 1},
                        {"command": "agile.ext-a.late", "priority": 20},
                        {"command": "agile.ext-a.mid-tied", "priority": 10},
                    ]
                },
            )
        )
    ...
    def test_get_hooks_for_event_orders_across_extensions(self, project_dir):
        ...
        executor.register_hooks(
            _StubManifest(
                "ext-report",
                {"after_plan": {"command": "agile.ext-report.run", "priority": 20}},
            )
        )
        executor.register_hooks(
            _StubManifest(
                "ext-verify",
                {"after_plan": {"command": "agile.ext-verify.run", "priority": 5}},
            )
        )

        ordered = executor.get_hooks_for_event("after_plan")
        assert [e["command"] for e in ordered] == [
            "agile.ext-verify.run",
            "agile.ext-report.run",
        ]
        ...
        executor.save_project_config({
            ...
            "hooks": {
                "after_tasks": [
                    {
                        "extension": "legacy",
                        "command": "agile.legacy.go",
                        "enabled": True,
                    },
                    {
                        "extension": "newer",
                        "command": "agile.newer.first",
                        "enabled": True,
                        "priority": 1,
                    },
                ]
        ...
        executor.save_project_config({
            ...
            "hooks": {
                "after_tasks": [
                    {
                        "extension": "corrupt",
                        "command": "agile.corrupt.go",
                        "enabled": True,
                        "priority": "not-a-number",
                    },
                    {
                        "extension": "early",
                        "command": "agile.early.go",
                        "enabled": True,
                        "priority": 1,
                    },
                ]
    ...
    def test_unregister_hooks_removes_all_extension_entries(self, project_dir):
        ...
        executor.register_hooks(
            _StubManifest("ext-b", {"after_tasks": {"command": "agile.ext-b.solo"}})
        )
...
class TestHookInvocationRendering:
    ...
    def test_kimi_hooks_render_skill_invocation(self, project_dir):
        """Kimi projects should render /skill:agile-* invocations."""
        init_options = project_dir / ".agile" / "init-options.json"
        init_options.parent.mkdir(parents=True, exist_ok=True)
        ...
        assert "Executing: `/skill:agile-plan`" in message
        assert "EXECUTE_COMMAND: agile.plan" in message
        assert "EXECUTE_COMMAND_INVOCATION: /skill:agile-plan" in message

    def test_codex_hooks_render_dollar_skill_invocation(self, project_dir):
        """Codex projects with skills mode should render $agile-* invocations."""
        init_options = project_dir / ".agile" / "init-options.json"
        init_options.parent.mkdir(parents=True, exist_ok=True)
        ...
        assert execution["command"] == "agile.tasks"
        assert execution["invocation"] == "$agile-tasks"

    def test_zcode_hooks_render_dollar_skill_invocation(self, project_dir):
        """ZCode projects with skills mode should render $agile-* invocations."""
        init_options = project_dir / ".agile" / "init-options.json"
        init_options.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_cline_hooks_render_hyphenated_invocation(self, project_dir):
        """Cline projects should render /agile-* invocations."""
        init_options = project_dir / ".agile" / "init-options.json"
        init_options.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_cline_hooks_render_extension_command(self, project_dir):
        """Cline projects should render /agile-my-ext-cmd for extension hooks."""
        init_options = project_dir / ".agile" / "init-options.json"
        init_options.parent.mkdir(parents=True, exist_ok=True)
        init_options.write_text(json.dumps({"ai": "cline"}))

        hook_executor = HookExecutor(project_dir)
        # Test with a non-agile. command
        execution = hook_executor.execute_hook(
            {
                "extension": "test-ext",
        ...
        assert execution["invocation"] == "/agile-my-extension-do-something"

    def test_forge_hooks_render_hyphenated_invocation(self, project_dir):
        """Forge projects should render /agile-* invocations (like Cline)."""
        init_options = project_dir / ".agile" / "init-options.json"
        init_options.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_forge_hooks_render_extension_command(self, project_dir):
        """Forge projects should render /agile-my-ext-cmd for extension hooks."""
        init_options = project_dir / ".agile" / "init-options.json"
        init_options.parent.mkdir(parents=True, exist_ok=True)
        init_options.write_text(json.dumps({"ai": "forge"}))
    ...
    def test_extension_command_uses_hyphenated_skill_invocation(self, project_dir):
        ...
        assert "Executing: `/skill:agile-test-ext-hello`" in message
        assert "EXECUTE_COMMAND: agile.test-ext.hello" in message
        assert "EXECUTE_COMMAND_INVOCATION: /skill:agile-test-ext-hello" in message
    ...
    def test_hook_executor_caches_init_options_lookup(self, project_dir, monkeypatch):
        ...
        monkeypatch.setattr("agile_cli.load_init_options", fake_load_init_options)

        hook_executor = HookExecutor(project_dir)
        assert hook_executor._render_hook_invocation("agile.plan") == "/skill:agile-plan"
        assert hook_executor._render_hook_invocation("agile.tasks") == "/skill:agile-tasks"
...
class TestExtensionRemoveCLI:
    """CLI tests for `agile extension remove` confirmation prompt wording."""
    ...
    def test_remove_confirmation_singular_command(self, tmp_path, extension_dir):
        ...
        manager.registry.update("test-ext", {"registered_commands": {"claude": ["agile.test-ext.hello"]}})
    ...
    def test_remove_confirmation_plural_commands(self, tmp_path, extension_dir):
        ...
        manager.registry.update("test-ext", {"registered_commands": {"claude": ["agile.test-ext.hello", "agile.test-ext.run"]}})
    ...
    def test_remove_output_escapes_extension_id_markup(self, tmp_path):
        ...
        assert result.exit_code == 0, result.output
        assert ".agile/extensions/.backup/[red]bad[/red]/" in result.output
        assert "agile extension add [red]bad[/red]" in result.output
...
class TestExtensionStateCLI:
    ...
    def test_disable_reenable_hint_escapes_extension_id_markup(self, tmp_path):
        ...
        assert result.exit_code == 0, result.output
        assert "agile extension enable [red]bad[/red]" in result.output
    ...
    def _setup_mock_extension(self, tmp_path, ai_name):
        ...
        manifest_data = {
            ...
            "provides": {
                "commands": [
                    {
                        "name": "agile.mock-ext.hello",
                        "file": "commands/hello.md",
                        "description": "Test hello command",
                        "aliases": ["agile.mock-ext.greet"]
                    }
                ]
            }
        }
        ...
        # Command file with dotted agile references in frontmatter and body
        cmd_content = """---
description: "Test hello command"
agent: agile.tasks
handoffs:
  - agent: agile.iterate.start
    message: "Hand off to start"
---

# Test Hello Command

Please refer to agile.mock-ext.greet for instructions.
$ARGUMENTS
"""
    ...
    def test_cline_extension_hyphenation(self, tmp_path):
        ...
        # Note: We assert that the primary command 'agile-mock-ext-hello' is printed,
        # but we do not assert that the alias 'agile-mock-ext-greet' is printed in the console
        # because manifest.commands only lists primary commands.
        assert "agile-mock-ext-hello" in result.output
        assert "agile.mock-ext.hello" not in result.output

        # Verify on-disk command names are hyphenated
        hello_file = cline_workflows_dir / "agile-mock-ext-hello.md"
        greet_file = cline_workflows_dir / "agile-mock-ext-greet.md"
        ...
        assert hello_fm["agent"] == "agile-tasks"
        assert hello_fm["handoffs"][0]["agent"] == "agile-iterate-start"

        # Verify body references are hyphenated for Cline
        assert "agile-mock-ext-greet" in hello_body
        assert "agile.mock-ext.greet" not in hello_body
    ...
    def test_non_cline_extension_no_hyphenation(self, tmp_path):
        ...
        hello_file = agents_commands_dir / "agile.mock-ext.hello.md"
        greet_file = agents_commands_dir / "agile.mock-ext.greet.md"
...
class TestExtensionForceCLI:
    """CLI tests for `agile extension add --dev --force`."""
...
def test_extension_wrapper_resolves_ghes_asset_when_host_configured(tmp_path, monkeypatch):
    ...
    from agile_cli.authentication import http as _auth_http
    from agile_cli.authentication.config import AuthConfigEntry
    from agile_cli.extensions import ExtensionCatalog
...
class TestConfigManagerNonMappingYaml:
    ...
    def _make(self, tmp_path, body: str):
        ext_dir = tmp_path / ".agile" / "extensions" / "jira"
...
class TestConfigManagerEnvPrefixCollision:
    ...
    def test_scalar_then_nested_yields_nested(self, tmp_path, monkeypatch):
        """AGILE_X_CONNECTION=x then AGILE_X_CONNECTION_URL=y.
        ...
        """
        monkeypatch.setenv("AGILE_TESTEXT_CONNECTION", "x")
        monkeypatch.setenv("AGILE_TESTEXT_CONNECTION_URL", "y")
    ...
    def test_colliding_env_does_not_disable_hook_condition(self, tmp_path, monkeypatch):
        ...
        """
        ext_dir = tmp_path / ".agile" / "extensions" / "testext"
        ext_dir.mkdir(parents=True)
    ...
    def test_malformed_env_names_ignored(self, tmp_path, monkeypatch):
        """A name with no key (AGILE_X_) or empty parts (consecutive
        underscores) must not create an entry under an empty key."""
        monkeypatch.setenv("AGILE_TESTEXT_", "orphan")  # no key at all
        monkeypatch.setenv("AGILE_TESTEXT_A__B", "z")   # empty middle part
        cm = ConfigManager(tmp_path, "testext")
...
class TestConfigManagerCrossExtensionEnvLeak:
    ...
    Before the fix, ``AGILE_GIT_HOOKS_URL`` (intended for a ``git-hooks``
    extension) also surfaced inside the ``git`` extension's config as
    ``{'hooks': {'url': ...}}`` because ``AGILE_GIT_`` is a strict prefix of
    ``AGILE_GIT_HOOKS_``.
    """

    def _install(self, project_root, ext_id):
        extensions_dir = project_root / ".agile" / "extensions"
    ...
    def test_sibling_owns_longer_prefix_env(self, tmp_path, monkeypatch):
        """AGILE_GIT_HOOKS_URL belongs to git-hooks when co-installed with git."""
        self._install(tmp_path, "git")
        self._install(tmp_path, "git-hooks")
        monkeypatch.setenv("AGILE_GIT_URL", "for_git")
        monkeypatch.setenv("AGILE_GIT_HOOKS_URL", "for_git_hooks")
    ...
    def test_no_sibling_installed_keeps_legacy_absorption(self, tmp_path, monkeypatch):
        ...
        preserved: ``AGILE_GIT_HOOKS_URL`` is absorbed as a nested key of
    ...
    def test_boundary_prevents_false_positive(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("AGILE_GIT_HOOKS_URL", "for_git_key_hooks")
    ...
    def test_missing_extensions_dir_does_not_crash(self, tmp_path, monkeypatch):
        """A ConfigManager built against a project without ``.agile/extensions``
        ...
        monkeypatch.setenv("AGILE_TESTEXT_URL", "v")
    ...
    def test_non_utf8_registry_does_not_crash(self, tmp_path, monkeypatch):
        ...
        pre-fix behaviour.
        """
        extensions_dir = tmp_path / ".agile" / "extensions"
...
def test_forge_extension_install_listing_hyphenates_command_names(
    ...
    /agile-<name> command names for a Forge project (Forge registers
    ...
    assert "agile-test-ext-hello" in result.output
    assert "agile.test-ext.hello" not in result.output
...
def test_forge_extension_info_hyphenates_command_names(
    ...
    /agile-<name> command names on a Forge project, matching the names Forge
    ...
    from agile_cli.extensions import _commands
    ...
    assert "agile-test-ext-hello" in output, output
    assert "agile.test-ext.hello" not in output, output
...
class TestExtensionConfigScaffolding:
    ...
    def test_scaffold_config_deploys_template(self, tmp_path):
        """Config template lands where ConfigManager reads it, not in .agile/ root."""
        from agile_cli.extensions import ExtensionManager
        project = tmp_path / "project"
        specify_dir = project / ".agile"
        ...
        # .agile/extensions/<id>/<name>, so that is where scaffolding must
        # put it. Deploying to the .agile/ root left the file somewhere the
    ...
    def test_scaffold_config_malformed_manifest(self, tmp_path):
        ...
        from agile_cli.extensions import ExtensionManager, ExtensionManifest
        import yaml
        project = tmp_path / "project"
        specify_dir = project / ".agile"
        specify_dir.mkdir(parents=True)
    ...
    def test_scaffold_config_missing_manifest_returns_consistent_result(self, tmp_path):
        ...
        from agile_cli.extensions import ExtensionManager
        project = tmp_path / "project"
        (project / ".agile").mkdir(parents=True)
    ...
    def test_scaffold_config_rejects_symlinked_config_root(self, tmp_path):
        """A symlinked .agile must not become the containment root.

        Resolving .agile first and trusting the result lets a symlink point
        ...
        outside.mkdir()
        (project / ".agile").symlink_to(outside, target_is_directory=True)
