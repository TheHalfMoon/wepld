...
from agile_cli.bundler.lib.yamlio import ensure_within, is_safe_relpath
...
def test_load_records_refuses_symlinked_specify_escape(tmp_path: Path):
    ...
    # a symlinked .agile pointing outside project_root is refused.
...
def test_active_integration_refuses_symlinked_specify_escape(tmp_path: Path):
    # Reading the integration marker must not follow a .agile symlink that
    # resolves outside project_root; an escape is treated as "not determinable".
    from agile_cli.bundler.lib.project import active_integration
...
def test_read_catalog_config_refuses_symlinked_specify_escape(tmp_path: Path):
    from agile_cli.bundler.commands_impl import catalog_config as cc
...
def test_load_source_stack_refuses_symlinked_specify_dir(tmp_path: Path):
    from agile_cli.bundler.models.catalog import load_source_stack
...
def test_find_project_root_ignores_symlinked_specify(tmp_path: Path):
    ...
    try:
        ...
        pytest.skip("symlinks not supported on this platform")
    # A symlinked .agile must not be accepted as a project root.
