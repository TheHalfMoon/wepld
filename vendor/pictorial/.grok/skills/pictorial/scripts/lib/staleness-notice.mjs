...
function cachePath() {
  return process.env.PICTORIAL_STALENESS_CACHE
    || path.join(os.homedir(), '.pictorial', 'staleness-check.json');
}
 ...
 * the file accumulate one entry per directory Pictorial has ever booted in
...
/**
 * Opt out with PICTORIAL_NO_STALENESS_CHECK=1 or `"stalenessCheck": false` in
 * .pictorial/config.json. Local config overrides shared, matching how
 * updateCheck resolves.
 */
export function stalenessCheckDisabled(roots = [process.cwd()]) {
  if (process.env.PICTORIAL_NO_STALENESS_CHECK) return true;
  let value;
...
export function buildStalenessDirective(findings) {
  ...
  const lines = [
    ...
    "Pictorial's own project files have drifted from what this version reads. "
