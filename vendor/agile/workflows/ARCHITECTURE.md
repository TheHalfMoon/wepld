...
Steps execute sequentially. Each step receives a `StepContext` containing resolved inputs, accumulated step results, and workflow-level defaults. After execution, the step's output is stored in `context.steps[step_id]` and made available to subsequent steps via expressions like `{{ steps.agile.output.file }}`.
...
The engine ships with 11 built-in step types, each in its own subpackage under `src/agile_cli/workflows/steps/`:

| Type Key | Class | Purpose | Returns `next_steps`? |
|----------|-------|---------|-----------------------|
| `command` | `CommandStep` | Invoke an installed Agile command via integration CLI | No |
...
| `init` | `InitStep` | Bootstrap a project (equivalent to `agile init`) | No |
...
All step types register into `STEP_REGISTRY` via `_register_builtin_steps()` in `src/agile_cli/workflows/__init__.py`. The registry maps `type_key` strings to step instances:
...
Workflow definitions use Jinja2-like `{{ expression }}` syntax for dynamic values. The expression engine in `src/agile_cli/workflows/expressions.py` supports:
...
flowchart TD
    ...
    B --> C{AGILE_WORKFLOW_CATALOG_URL set?}
    C -- Yes --> D["Single custom catalog"]
    C -- No --> E{.agile/workflow-catalogs.yml exists?}
    E -- Yes --> F["Project-level catalog stack"]
    E -- No --> G{"~/.agile/workflow-catalogs.yml exists?"}
...
Catalogs are fetched with a 1-hour cache (per-URL, SHA256-hashed cache files in `.agile/workflows/.cache/`). Each catalog entry has a `priority` (for merge ordering) and `install_allowed` flag.

When `specify workflow add <id>` installs from catalog, it downloads the workflow YAML from the catalog entry's `url` field into `.agile/workflows/<id>/workflow.yml`.

## State and Configuration Locations

| Component | Location | Format | Purpose |
|-----------|----------|--------|---------|
| Workflow definitions | `.agile/workflows/{id}/workflow.yml` | YAML | Installed workflow definitions |
| Workflow registry | `.agile/workflows/workflow-registry.json` | JSON | Installed workflows metadata |
| Run state | `.agile/workflows/runs/{run_id}/state.json` | JSON | Persisted execution state |
| Run inputs | `.agile/workflows/runs/{run_id}/inputs.json` | JSON | Resolved input values |
| Run log | `.agile/workflows/runs/{run_id}/log.jsonl` | JSONL | Append-only event log |
| Catalog cache | `.agile/workflows/.cache/*.json` | JSON | Cached catalog entries (1hr TTL) |
| Project catalogs | `.agile/workflow-catalogs.yml` | YAML | Project-level catalog sources |
| User catalogs | `~/.agile/workflow-catalogs.yml` | YAML | User-level catalog sources |
...
│       ├── init/            # Bootstrap a project (agile init)
