#!/usr/bin/env python3
"""Authorize a bounded, single-use reopen of the frozen S2-S005 Git-topology
test evidence gap.

v53 is an append-only policy successor over canonical v52 at main
206fe43c713913a60dea788595894ff1c33ab021. Founder standing authorization for
...
test path so the S2-S005 real ownership-based `safe.directory` refusal oracle
(and, in the same file, the acceptance.md section C "bare repository is
explicit" fixture) can be recorded. Both are named as explicit open
qualification obligations in `crates/core/tests/git_topology_v1.rs`'s own
header. v45's Git-topology product-tranche freeze re-applies to
`crates/core/tests/git_topology_v1.rs` after every canonical landing (it is
frozen forever once canonical; v52's own single-use reopen was permanently
consumed by PR #287), so a fresh single-use reopen is required.

v53 grants exactly one thing and nothing else:
...
`NONE`/unchanged and is asserted against the inherited v52 chain.

Package-load / resting-view note: v53 follows the v45..v52 discipline. It
owns a fresh `LocalRepositoryView` of the exact checked-out head, imports
frozen v52 under an exact v53->v52 workflow-entrypoint reversal, and
inherits every v52 hook by reference. The one path this successor reopens
is not subject to any frozen-content check anywhere in the inherited
v33..v52 cascade other than v45's own product-tranche freeze inside its
...
P = ".github/scripts/wepld_s2_git_topology_evidence_reopen_v53_integrity.py"
T = ".github/scripts/wepld_s2_git_topology_evidence_reopen_v53_selftest.py"
T_BLOB = "ae19ffd896ed86a4832c789bb5bc6ac7316eda01"

V52_P_BLOB = "33cf300d4f2075f60df495bac4d1e3fcd13509c1"
V52_T_BLOB = "621fc312e41720b93ba99a675f8a218e2356b7bd"
...
# Do not inherit a predecessor module's resting/projection view. v53 bases
...
def _v52_workflow_bytes(data: bytes, path: str) -> bytes:
    count = data.count(_V53_ENTRYPOINT)
    if count != _WORKFLOW_ENTRYPOINT_COUNTS[path]:
        base.fail(
            "v53 workflow entrypoint count drifted before v52 projection: "
            f"{path} expected={_WORKFLOW_ENTRYPOINT_COUNTS[path]} actual={count}"
        )
    return data.replace(_V53_ENTRYPOINT, _V52_ENTRYPOINT)


def _import_v52_under_workflow_projection() -> Any:
    """Import frozen v52 while it observes exact v52 workflow bytes.

    v52 (hence v50..v45..v36) reads workflow bytes while its module is
    imported, and the v52->v53 entrypoint migration ships in this same
    candidate, so v52 must not observe its own successor's bytes. Only
    ...
    def _v52_import_read_bytes(local_view: Any, relative: str, limit: int) -> bytes:
        data = original_read_bytes(local_view, relative, limit)
        if relative in (FW, AW):
            data = _v52_workflow_bytes(data, relative)
            if len(data) > limit:
                base.fail(
                    f"v53 v52-import workflow projection exceeds read bound: {relative}"
                )
        return data

    base.LocalRepositoryView.read_bytes = _v52_import_read_bytes
    try:
        return importlib.import_module(
            "wepld_s2_git_topology_evidence_reopen_v52_integrity"
        )
    finally:
        base.LocalRepositoryView.read_bytes = original_read_bytes


q = _import_v52_under_workflow_projection()
...
# --- S2-S005 Git-topology test-evidence reopen allowlist ---
...
REOPEN_TASKS = frozenset({"S2-S005"})

# The exact pre-reopen frontier blob of the one reopened path, at canonical
# main 206fe43c713913a60dea788595894ff1c33ab021 (the content re-frozen after
# v52's PR #287 reopen landed, unchanged by the specs-only PR #289/#290).
...
# consumed.
EXACT_FROZEN_BLOB = "940806b059935067797e4673e6cb7c54af3e513f"
...
for _path, _expected in ((q.P, V52_P_BLOB), (q.T, V52_T_BLOB), (T, T_BLOB)):
    _actual = V25.blob(raw_root.read_bytes(_path, base.MAX_POLICY_FILE_BYTES))
    if _actual != _expected:
        base.fail(
            f"frozen v53 package input drifted: {_path}: "
...
def _workflow_replacements(view: Any) -> dict[str, bytes]:
    ...
    for path in (FW, AW):
        ...
        predecessor = _v52_workflow_bytes(data, path)
        actual = V25.sha(predecessor)
        if actual != Q_WF[path]:
            base.fail(
                "v53 workflow does not reverse to exact canonical v52 predecessor: "
...
def req_v52(view: Any) -> None:
    for path, expected in ((q.P, V52_P_BLOB), (q.T, V52_T_BLOB)):
        if path not in V25.ps(view):
            base.fail(f"v53 candidate/base is missing frozen v52 predecessor: {path}")
        actual = V25.blob(view.read_bytes(path, base.MAX_POLICY_FILE_BYTES))
        if actual != expected:
            base.fail(
                f"frozen v52 predecessor drifted: {path}: "
                f"expected={expected} actual={actual}"
            )


def _project_for_v52(view: Any) -> Any:
...
def _v52_views(candidate: Any, policy_base: Any) -> tuple[Any, Any]:
    """Project the candidate to v52's view always; project the policy base
    only when it is a real post-v53 base. A pre-v53 bootstrap base predates
    the v52->v53 workflow migration and carries no v53 policy files, so it
    must reach v52's frozen hooks unprojected."""
    projected_candidate = _project_for_v52(candidate)
    if bootbase(policy_base):
        return projected_candidate, policy_base
    return projected_candidate, _project_for_v52(policy_base)


def _boot_base_for_selftest() -> Any:
    return _project_for_v52(raw_root)


def run_predecessor_selftests() -> None:
    """Run frozen v52's own self-tests once, under a v53->v52 workflow
    reversal.

    v52's corrected hooks are inherited by reference. Only ``read_bytes`` is
    wrapped here for the v53->v52 workflow reversal; the wrap is restored in
    ...
    def _v52_selftest_read_bytes(local_view: Any, relative: str, limit: int) -> bytes:
        if relative in workflow_reversal:
            data = workflow_reversal[relative]
            if len(data) > limit:
                base.fail(
                    f"v53 v52-selftest workflow projection exceeds read bound: {relative}"
                )
            return data
        return original_read_bytes(local_view, relative, limit)

    base.LocalRepositoryView.read_bytes = _v52_selftest_read_bytes
    try:
        _call("v52 self-tests under v53->v52 workflow reversal", q.selftest)
    finally:
        base.LocalRepositoryView.read_bytes = original_read_bytes
...
def _reopen_available(view: Any) -> bool:
    if REOPEN_TEST not in V25.ps(view):
        base.fail(f"v53 reopen base is missing the frozen reopened path: {REOPEN_TEST}")
...
def _require_reopen_base(view: Any) -> None:
    if not _reopen_available(view):
        base.fail(
            "v53 Git-topology evidence reopen already consumed; "
...
def _verify_text_reopen_file(view: Any, path: str) -> None:
    if path not in V25.ps(view):
        base.fail(f"v53 reopen path missing: {path}")
    if V25.mode(view, path) != "100644":
        base.fail(f"v53 reopen path mode invalid: {path}")
    data = view.read_bytes(path, base.MAX_POLICY_FILE_BYTES)
    if not data:
        base.fail(f"v53 reopen path must not be empty: {path}")
    if len(data) > MAX_REOPEN_FILE_BYTES:
        base.fail(f"v53 reopen path exceeds bounded size: {path}")
    try:
        data.decode("utf-8", errors="strict")
    except UnicodeError as exc:
        base.fail(f"v53 reopen path must be UTF-8: {path}: {exc}")
...
def _verify_reopen_candidate(candidate: Any, policy_base: Any) -> None:
    ...
    for path in (CORE_MANIFEST, ROOT_CARGO, ROOT_CARGO_LOCK, GIT_TOPOLOGY_MODULE, CORE_EXPORT):
        if candidate.read_bytes(path, base.MAX_POLICY_FILE_BYTES) != policy_base.read_bytes(
            ...
            base.fail(f"v53 Git-topology evidence reopen must not change frozen path: {path}")
    for relative in sorted(V25.FROZEN_STATE_PATHS):
        if candidate.read_bytes(relative, V25.MAX_S1_STATE_BYTES) != policy_base.read_bytes(
            ...
            base.fail(f"v53 candidate changed frozen S1 state: {relative}")
...
def delta(candidate: Any, policy_base: Any) -> None:
    ...
    if bootbase(policy_base):
        if paths != BOOT:
            if paths & BOOT:
                base.fail(
                    "v53 bootstrap delta must be exactly two v53 policy files plus two integrity workflows"
                )
            base.fail("v53 bootstrap base authorizes only exact Git-topology evidence reopen policy activation")
        req_v52(candidate)
        req_v52(policy_base)
        return

    if paths & CONTROLLED_FILES:
        base.fail("canonical v53 policy files are frozen after activation")

    reopen_changed = frozenset(paths & REOPEN_FILES)
    if reopen_changed:
        if paths != REOPEN_FILES:
            base.fail(
                "v53 Git-topology evidence reopen must not mix with non-reopen paths"
            )
        _verify_reopen_candidate(candidate, policy_base)
        return

    q.delta(_project_for_v52(candidate), _project_for_v52(policy_base))


def basectrl(candidate: Any, policy_base: Any) -> None:
    if not bootbase(policy_base):
        q.basectrl(*_v52_views(candidate, policy_base))
        return
    for path in sorted(base.BASE_CONTROLLED_PATHS):
        ...
        if path in (FW, AW):
            if V25.sha(candidate_bytes) != WF[path] or V25.sha(base_bytes) != Q_WF[path]:
                base.fail(f"v53 bootstrap workflow drifted: {path}")
...
def ext(candidate: Any, policy_base: Any, safe: Any) -> None:
    ...
    for path in sorted(CONTROLLED_FILES & safe_paths):
        if path not in V25.ps(candidate):
            base.fail(f"v53 controlled file missing: {path}")
        if bootbase(policy_base):
            if path in V25.ps(policy_base):
                base.fail(f"v53 controlled file unexpectedly exists in bootstrap base: {path}")
        elif path not in V25.ps(policy_base) or candidate.read_bytes(
            path, base.MAX_POLICY_FILE_BYTES
        ) != policy_base.read_bytes(path, base.MAX_POLICY_FILE_BYTES):
            base.fail(f"v53 steady-state controlled file drifted: {path}")
    ...
    if rest:
        projected_candidate, projected_base = _v52_views(candidate, policy_base)
...
def files(view: Any) -> None:
    ...
    for path in sorted(CONTROLLED_FILES):
        if path not in V25.ps(view):
            base.fail(f"v53 controlled file missing: {path}")
        if V25.mode(view, path) != "100644":
            base.fail(f"v53 controlled file mode invalid: {path}")
        if view.read_bytes(path, base.MAX_POLICY_FILE_BYTES) != approved[path]:
            base.fail(f"v53 controlled file content drifted: {path}")
    if not _reopen_available(view):
...
def verify_component_base(
    ...
    *,
    ...
    if _PREDECESSOR_COMPONENT_BASE is None:
        base.fail("v53 predecessor component-base hook unavailable")
    path_set = set(paths)
    if path_set & REOPEN_FILES:
        remaining = path_set - REOPEN_FILES
        _call(
            "v53 projected predecessor component-base verifier",
            _PREDECESSOR_COMPONENT_BASE,
            _project_for_v52(view),
            remaining - CONTROLLED_FILES,
            allow_core_main_change=False,
        )
        return
    _call(
        "v53 predecessor component-base verifier",
        _PREDECESSOR_COMPONENT_BASE,
        _project_for_v52(view),
...
def freeze_s1_007_state(candidate: Any, policy_base: Any) -> None:
    if _PREDECESSOR_FREEZE_S1 is None:
        base.fail("v53 predecessor S1 freeze hook unavailable")
    paths = V25.changed(V25.v24.v23, candidate, policy_base)
    if paths == REOPEN_FILES:
        for relative in sorted(V25.FROZEN_STATE_PATHS):
            if candidate.read_bytes(relative, V25.MAX_S1_STATE_BYTES) != policy_base.read_bytes(
    ...
    projected_candidate, projected_base = _v52_views(candidate, policy_base)
    _call(
        "v53 predecessor S1 state freeze",
...
def printer(stage: str, mode_: str) -> None:
    ...
    print("wepld_policy_successor_v53=S2_GIT_TOPOLOGY_EVIDENCE_REOPEN_ONLY")
    print(f"v53_authority={AUTH}")
    print(f"s2_implementation_authority_v53={S2_IMPLEMENTATION_AUTHORITY}")
    print(
        "git_topology_evidence_reopen_authority_v53="
        f"{GIT_TOPOLOGY_EVIDENCE_REOPEN_AUTHORITY}"
    )
    print(f"reopen_available_v53={_reopen_available(raw_root)}")
    print(f"doctor_cli_authority_v53={DOCTOR_CLI_AUTHORITY}")
    print(f"general_shell_authority_v53={GENERAL_SHELL_AUTHORITY}")
    print(f"arbitrary_process_authority_v53={ARBITRARY_PROCESS_AUTHORITY}")
    print(f"package_install_authority_v53={PACKAGE_INSTALL_AUTHORITY}")
    print(f"project_native_command_execution_v53={PROJECT_NATIVE_COMMAND_EXECUTION}")
    print(f"git_mutation_authority_v53={GIT_MUTATION_AUTHORITY}")
    print(f"safe_directory_mutation_authority_v53={SAFE_DIRECTORY_MUTATION_AUTHORITY}")
    print(f"remediation_execution_authority_v53={REMEDIATION_EXECUTION_AUTHORITY}")
    print(f"git_process_admission_v53={GIT_PROCESS_ADMISSION}")
    print(f"git_execution_authority_v53={GIT_EXECUTION_AUTHORITY}")
    print(f"external_process_authority_v53={EXTERNAL_PROCESS_AUTHORITY}")
    print(f"network_authority_v53={NETWORK_AUTHORITY}")
    print(f"model_provider_execution_v53={MODEL_PROVIDER_EXECUTION}")
    print(f"source_admission_v53={SOURCE_ADMISSION}")
    print(f"dependency_admission_v53={DEPENDENCY_ADMISSION}")
    print(f"s3_plus_authority_v53={S3_PLUS_AUTHORITY}")
    print(f"next_authority_gate_v53={NEXT_AUTHORITY_GATE}")
...
def prepare_q() -> None:
    for module in _chain():
        ...
        if current not in (Q_WF, dict(WF)):
            base.fail(f"v53 predecessor workflow identity map drifted: actual={current}")
    for module in _chain():
        module.WF = dict(WF)
...
def overlay() -> None:
    ...
    if any(actual is not expected for actual, expected in pairs):
        base.fail("v53 installed overlay drifted")
    if any(dict(module.WF) != dict(WF) for module in _chain()):
        base.fail("v53 workflow identity projection drifted")
    for name in (
        ...
        if getattr(q, name) != globals()[name]:
            base.fail(f"v53 inherited authority drifted: {name}")
...
def install() -> None:
    ...
    shell, routing, _, desktop, execution = V25.topo()
    pairs = (
        (_attr(routing, "IMPL_REQUIRE_EXACT_DELTA", "v52 routing hook"), q.delta),
        (base.compare_base_controlled, q.basectrl),
        (_attr(desktop, "verify_extension_controlled_paths", "v52 desktop hook"), q.dext),
        (_attr(execution, "verify_extension_controlled_paths", "v52 execution hook"), q.eext),
        (_attr(shell, "validate_allowed_paths", "v52 allowed hook"), q.allowed),
        (_attr(shell, "verify_policy_files", "v52 files hook"), q.files),
        (_attr(shell, "print_success", "v52 printer"), q.printer),
    )
    if any(actual is not expected for actual, expected in pairs):
        base.fail("v53 predecessor hook drifted")
    ...
    _bind(desktop, "EXTENSION_CONTROLLED_PATHS", desktop_extensions, "v53 desktop registration")
    _bind(execution, "EXTENSION_CONTROLLED_PATHS", execution_extensions, "v53 execution registration")
    _bind(routing, "IMPL_REQUIRE_EXACT_DELTA", delta, "v53 routing hook")
    base.compare_base_controlled = basectrl
    _bind(desktop, "verify_extension_controlled_paths", dext, "v53 desktop hook")
    _bind(execution, "verify_extension_controlled_paths", eext, "v53 execution hook")
    _bind(shell, "validate_allowed_paths", allowed, "v53 allowed hook")
    _bind(shell, "verify_policy_files", files, "v53 files hook")
    _bind(shell, "print_success", printer, "v53 printer hook")
    _bind(execution, "_verify_component_base", verify_component_base, "v53 component-base hook")
    _bind(execution, "freeze_s1_007_state", freeze_s1_007_state, "v53 S1 state freeze hook")
    _INST = True
    overlay()


def selftest() -> None:
    from wepld_s2_git_topology_evidence_reopen_v53_selftest import run
