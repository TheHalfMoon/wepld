...
class TestCodexInitFlow:
    ...
    def test_integration_codex_creates_skills(self, tmp_path):
        ...
        assert (target / ".agents" / "skills" / "agile-plan" / "SKILL.md").exists()
    ...
    def test_plan_skill_has_no_context_placeholder(self, tmp_path):
        ...
        plan_skill = target / ".agents" / "skills" / "agile-plan" / "SKILL.md"
...
class TestCodexHookCommandNote:
    ...
    ``agile.git.commit`` but Codex skills are named with hyphens
    (``agile-git-commit``). Without this note, Codex emits
    ``/agile.git.commit``, which does not resolve.
    """
    ...
    def test_hook_note_injected_in_skills_with_hooks(self, tmp_path):
        ...
        specify_skill = tmp_path / ".agents/skills/agile-specify/SKILL.md"
