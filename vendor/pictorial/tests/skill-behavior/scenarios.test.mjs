...
const CRAFT_PROMPT = '/pictorial craft a landing page for the project in this workspace';
const SHAPE_PROMPT = '/pictorial shape a landing page for the project in this workspace';
const NATURAL_BUILD_PROMPT = 'Build a landing page for the project in this workspace.';
const TEACH_PROMPT = '/pictorial teach';
const PRIMER_PROMPT =
  'Take a quick look at the project. What context should guide later design work? Run the pictorial context loader once if you need to.';

const VERBOSE = process.env.PICTORIAL_SKILL_BEHAVIOR_VERBOSE === '1';
...
function executedUpdateCommands(trace) {
  ...
  return executableSegments.filter((segment) =>
    /^(?:(?:npx|bunx|pnpx)\s+)?(?:pictorial|skills)\s+update\b/.test(segment),
  );
}
...
for (const modelId of resolveModelList()) {
  ...
  describe(`skill behavior :: ${modelId}`, () => {
    ...
    it('scenario 4: context already loaded in prior turn', async () => {
      ...
      try {
        ...
        const turn2 = await runTurn({
          workspace,
          model,
          userPrompt: 'Now, /pictorial craft a landing page based on what you saw.',
    ...
    it('scenario 6: sub-command routing (`/pictorial polish` loads polish.md)', async () => {
      const workspace = prepareWorkspace({
        files: {
      ...
      try {
        const { trace, text } = await runTurn({
          workspace,
          model,
          userPrompt: '/pictorial polish index.html',
          maxSteps: setupMaxSteps,
        });
        logTrace('S6', 'polish-routing', modelId, trace, { textSample: text.slice(0, 300) });
        assert.ok(
          fileLoaded(trace, 'polish.md'),
          `agent should load polish.md when /pictorial polish is invoked.\n` +
            `Trace: ${JSON.stringify(summarizeTrace(trace), null, 2)}`,
        );
      } finally {
        cleanupWorkspace(workspace);
    ...
    it('scenario 7: sub-command routing (`/pictorial audit` loads audit.md)', async () => {
      const workspace = prepareWorkspace({
        files: {
          'PRODUCT.md': PRODUCT_MD_SAMPLE,
          'DESIGN.md': DESIGN_MD_SAMPLE,
      ...
      try {
        const { trace, text } = await runTurn({
          workspace,
          model,
          userPrompt: '/pictorial audit index.html',
          maxSteps: setupMaxSteps,
        });
        logTrace('S7', 'audit-routing', modelId, trace, { textSample: text.slice(0, 300) });
        assert.ok(
          fileLoaded(trace, 'audit.md'),
          `agent should load audit.md when /pictorial audit is invoked.\n` +
            `Trace: ${JSON.stringify(summarizeTrace(trace), null, 2)}`,
        );
      } finally {
        cleanupWorkspace(workspace);
    ...
    it('scenario 8: existing SvelteKit project (agent explores design system)', async () => {
      ...
      try {
        const { trace, text } = await runTurn({
          workspace,
          model,
          userPrompt: '/pictorial polish src/routes/+page.svelte',
          maxSteps: 8,
        });
    ...
    it('scenario 9: update-available directive is surfaced, never auto-run', async () => {
      ...
      const workspace = prepareWorkspace({
        files: {
          'PRODUCT.md': PRODUCT_MD_SAMPLE,
          'index.html': MINIMAL_LANDING_HTML,
          '.pictorial-update.json': JSON.stringify({ lastCheck: Date.now(), latestVersion: '99.0.0' }),
        },
        skillVersion: '3.5.0',
      });
      try {
        const { trace, text } = await runTurn({
          ...
          userPrompt: '/pictorial polish index.html',
          maxSteps: setupMaxSteps,
          env: { PICTORIAL_UPDATE_CACHE: path.join(workspace, '.pictorial-update.json') },
        });
    ...
    it('scenario 10: scoped command with no PRODUCT.md proceeds without forcing init', async () => {
      ...
      try {
        ...
        assert.equal(
          initLoaded,
          false,
          `scoped /pictorial polish on existing code should not divert into init.md when PRODUCT.md is missing.\n` +
            `Trace: ${JSON.stringify(summarizeTrace(trace), null, 2)}`,
        );
      } finally {
        cleanupWorkspace(workspace);
    ...
    it('scenario 13: teach alias with no PRODUCT.md diverts into init', async () => {
      ...
      try {
        ...
        assert.ok(
          initLoaded,
          `/pictorial teach should behave like init and load init.md when PRODUCT.md is missing.\n` +
            `Trace: ${JSON.stringify(summarizeTrace(trace), null, 2)}`,
        );
      } finally {
        cleanupWorkspace(workspace);
    ...
    it('scenario 14: native iOS project (context loads ios.md)', async () => {
      ...
      try {
        const { trace, text } = await runTurn({
          workspace,
          model,
          userPrompt: '/pictorial craft a tide detail screen for the project in this workspace',
    ...
    it('scenario 15: native audit routes to the native command variant', async () => {
      ...
      try {
        const { trace, text } = await runTurn({
          workspace,
          model,
          userPrompt: '/pictorial audit the app in this workspace',
          maxSteps: 6,
        });
