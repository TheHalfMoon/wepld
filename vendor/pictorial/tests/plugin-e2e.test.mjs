...
const MARKETPLACE_NAME = 'pictorial-e2e';
const PLUGIN_REF = `pictorial@${MARKETPLACE_NAME}`;
...
describe('committed plugin subtree loads in a real Claude Code', { skip }, () => {
  ...
  before(() => {
    workDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pictorial-plugin-e2e-'));
    ...
    fs.cpSync(PLUGIN_DIR, path.join(marketplaceDir, 'pictorial'), { recursive: true });
    fs.writeFileSync(
      path.join(marketplaceDir, '.claude-plugin', 'marketplace.json'),
      JSON.stringify({
        name: MARKETPLACE_NAME,
        owner: { name: 'pictorial plugin E2E' },
        plugins: [
          { name: 'pictorial', source: './pictorial', description: 'committed ./plugin subtree' },
        ],
      }, null, 2),
    );
  ...
  it('parses and loads the pictorial skill', () => {
    assert.equal(componentCount('Skills'), 1);
    assert.match(detailsOutput, /Skills\s+\(1\)\s+pictorial/);
  });
