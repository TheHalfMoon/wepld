...
class WorkflowRegistry:
    ...
    def __init__(self, project_root: Path) -> None:
        ...
        self.workflows_dir = project_root / ".agile" / "workflows"
        self.registry_path = self.workflows_dir / self.REGISTRY_FILE
        self.data = self._load()

    def _has_symlinked_parent(self) -> bool:
        """Return True if any directory under .agile/workflows is a symlink."""
        current = self.project_root
        for part in (".agile", "workflows"):
...
class WorkflowCatalog:
    ...
    1. ``AGILE_WORKFLOW_CATALOG_URL`` env var (overrides all)
    2. Project-level ``.agile/workflow-catalogs.yml``
    3. User-level ``~/.agile/workflow-catalogs.yml``
    ...
    def get_active_catalogs(self) -> list[WorkflowCatalogEntry]:
        ...
        env_url = os.environ.get("AGILE_WORKFLOW_CATALOG_URL", "").strip()
        if env_url:
            ...
            return [
                WorkflowCatalogEntry(
                    ...
                    description="From AGILE_WORKFLOW_CATALOG_URL",
                )
            ]

        # 2. Project-level config
        project_config = self.project_root / ".agile" / "workflow-catalogs.yml"
        ...
        # 3. User-level config
        home = Path.home()
        user_config = home / ".agile" / "workflow-catalogs.yml"
    ...
    def add_catalog(self, url: str, name: str | None = None) -> None:
        ...
        self._validate_catalog_url(url)
        config_path = self.project_root / ".agile" / "workflow-catalogs.yml"
...
class StepRegistry:
    ...
    ``.agile/workflows/steps/step-registry.json``.
    """
    ...
    def __init__(self, project_root: Path) -> None:
        self.project_root = project_root
        self.steps_dir = project_root / ".agile" / "workflows" / "steps"
        self.registry_path = self.steps_dir / self.REGISTRY_FILE
        self.data = self._load()

    def _has_symlinked_parent(self) -> bool:
        """Return True if any directory under .agile/workflows/steps is a symlink."""
        current = self.project_root
    ...
    def _load(self) -> dict[str, Any]:
        ...
        # under .agile/workflows/steps is a symlink, which could redirect the
...
class StepCatalog:
    ...
    Resolution order for catalog sources:
    1. ``AGILE_STEP_CATALOG_URL`` env var (overrides all)
    2. Project-level ``.agile/step-catalogs.yml``
    3. User-level ``~/.agile/step-catalogs.yml``
    4. Built-in defaults (official + community)
    """
    ...
    def _is_cache_path_safe(self) -> bool:
        ...
        current = self.project_root
        for part in (".agile", "workflows", "steps", ".cache"):
            current = current / part
            if current.is_symlink():
                return False
        return True
    ...
    def get_active_catalogs(self) -> list[StepCatalogEntry]:
        ...
        # 1. Environment variable override
        env_url = os.environ.get("AGILE_STEP_CATALOG_URL", "").strip()
        if env_url:
            self._validate_catalog_url(env_url)
            return [
                StepCatalogEntry(
                    ...
                    priority=1,
                    install_allowed=True,
                    description="From AGILE_STEP_CATALOG_URL",
                )
            ]

        # 2. Project-level config
        project_config = self.project_root / ".agile" / "step-catalogs.yml"
        project_entries = self._load_catalog_config(project_config)
        if project_entries is not None:
            return project_entries
        ...
        # 3. User-level config
        home = Path.home()
        user_config = home / ".agile" / "step-catalogs.yml"
        user_entries = self._load_catalog_config(user_config)
        if user_entries is not None:
            return user_entries
    ...
    def add_catalog(self, url: str, name: str | None = None) -> None:
        """Add a catalog source to the project-level config."""
        self._validate_catalog_url(url)
        config_path = self.project_root / ".agile" / "step-catalogs.yml"
