...
class TestGooseCommandPlaceholderResolution:
    ...
    def test_register_commands_resolves_placeholders_in_recipe(self, tmp_path):
        ...
        commands = [{"name": "agile.example", "file": "commands/example.md"}]
        registrar.register_commands("goose", commands, "test-ext", ext_dir, tmp_path)

        recipe = tmp_path / ".goose" / "recipes" / "agile.example.yaml"
        ...
        prompt = data["prompt"]
        assert ".agile/scripts/" in prompt  # {SCRIPT} -> resolved script path
...
class TestGooseCliDispatch:
    ...
    def test_dotted_extension_command_maps_to_recipe(self):
        integration = get_integration("goose")
        args = integration.build_exec_args("/agile.git.commit msg", output_json=False)
        assert args[args.index("--recipe") + 1] == (
            ".goose/recipes/agile.git.commit.yaml"
        )
    ...
    def test_non_agile_slash_prompt_is_not_treated_as_a_recipe(self):
        """`/help` is a goose session command, not a Agile recipe.
        ...
        the `agile.` namespace must not become
        `--recipe .goose/recipes/agile.help.yaml` — `setup()` only ever
        writes `command_filename(stem)` = `agile.<name>.yaml`.
        """
        integration = get_integration("goose")
    ...
    def test_non_agile_slash_prompt_is_not_promoted_to_a_recipe(self):
        """`/plan` is goose's own command and must not run agile.plan.

        `command_filename()` re-adds the `agile.` prefix, so the old
        unconditional call silently promoted the free-form goose command
        `/plan` into a real Agile recipe run. Dispatch always spells
        commands `/agile.plan` (`IntegrationBase.build_command_invocation`),
    ...
    def test_bare_agile_prefix_falls_through_to_text(self):
        """`/agile.` alone has no stem and must not yield `agile..yaml`."""
        integration = get_integration("goose")
        args = integration.build_exec_args("/agile.", output_json=False)
        assert "--recipe" not in args
        assert args[-2:] == ["-t", "/agile."]
