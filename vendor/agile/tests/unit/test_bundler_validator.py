...
from agile_cli.bundler.services import validator as validator_mod
from agile_cli.bundler.services.validator import validate_manifest
...
def test_invalid_agile_constraint_reported_as_error():
    manifest = _manifest(requires={"agile_version": ">>bad"})
    report = validate_manifest(manifest)
    assert not report.ok
    assert any("agile_version" in e for e in report.errors)
...
def test_non_bundler_error_not_swallowed(monkeypatch):
    ...
    manifest = _manifest(requires={"agile_version": ">=1.0.0"})
