...
from agile_cli.integrations.kimi import (
...
class TestKimiLegacyMigration:
    ...
    def test_migrate_dotted_to_hyphenated(self, tmp_path):
        ...
        assert (skills_dir / "agile-plan" / "SKILL.md").exists()
    ...
    def test_setup_migrate_legacy_moves_old_skills_dir(self, tmp_path):
        ...
        legacy = old_skills_dir / "agile-oldcmd"
        legacy.mkdir(parents=True)
        ...
        assert (new_skills_dir / "agile-oldcmd" / "SKILL.md").exists()
        # New skills from templates should also exist
        assert (new_skills_dir / "agile-specify" / "SKILL.md").exists()
...
class TestKimiTeardownLegacyCleanup:
    ...
    def test_teardown_removes_legacy_agile_skills(self, tmp_path):
        i = get_integration("kimi")

        legacy_skill = tmp_path / ".kimi" / "skills" / "agile-plan" / "SKILL.md"
...
class TestKimiCommandInvocation:
    ...
    def test_build_command_invocation_uses_skill_prefix(self):
        i = get_integration("kimi")
        assert i.build_command_invocation("agile") == "/skill:agile-specify"
        assert i.build_command_invocation("agile.plan") == "/skill:agile-plan"

    def test_build_command_invocation_dotted_extension(self):
        i = get_integration("kimi")
        assert (
            i.build_command_invocation("agile.git.commit")
            == "/skill:agile-git-commit"
        )

    def test_build_command_invocation_appends_args(self):
        i = get_integration("kimi")
        assert (
            i.build_command_invocation("agile", "my feature")
            == "/skill:agile-specify my feature"
        )
...
class TestKimiLegacySymlinkSafety:
    ...
    def test_migrate_skips_symlinked_legacy_skills_dir(self, tmp_path):
        ...
        # (the bundled templates never create "agile-evillegacy").
        outside = tmp_path / "outside"
        (outside / "agile-evillegacy").mkdir(parents=True)
        (outside / "agile-evillegacy" / "SKILL.md").write_text("# evil\n")
        ...
        assert (outside / "agile-evillegacy" / "SKILL.md").exists()
        assert not (
            project / ".kimi-code" / "skills" / "agile-evillegacy"
        ).exists()
    ...
    def test_migrate_skips_symlinked_legacy_parent_dir(self, tmp_path):
        ...
        project = tmp_path / "project"
        unrelated = project / "skills" / "agile-evillegacy"
    ...
    def test_teardown_skips_symlinked_legacy_parent_dir(self, tmp_path):
        project = tmp_path / "project"
        project.mkdir()
        # Looks Agile-generated, so only the symlink check protects it.
        unrelated = project / "skills" / "agile-evillegacy"
        unrelated.mkdir(parents=True)
    ...
    def test_migrate_skips_symlinked_target_dir(self, tmp_path):
        # The destination `.kimi-code/skills/agile-foo` already exists but is
        ...
        project = tmp_path / "project"
        legacy = project / ".kimi" / "skills" / "agile-foo"
        legacy.mkdir(parents=True)
        ...
        target = project / ".kimi-code" / "skills" / "agile-foo"
...
class TestKimiNextSteps:
    ...
    def test_next_steps_show_skill_invocation(self, tmp_path):
        """Kimi next-steps guidance should display /skill:agile-* usage."""
        import os
        ...
        assert "/skill:agile-constitution" in result.output
