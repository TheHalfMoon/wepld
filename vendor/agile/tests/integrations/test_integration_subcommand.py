...
def _init_project(tmp_path, integration="copilot", integration_options=None):
    """Helper: init a agile project with the given integration."""
...
def _write_invalid_manifest(project, key):
    manifest = project / ".agile" / "integrations" / f"{key}.manifest.json"
...
def _move_kilocode_install_to_legacy_layout(project):
    ...
    manifest_path = project / ".agile" / "integrations" / "kilocode.manifest.json"
...
class TestIntegrationList:
    def test_list_requires_agile_project(self, tmp_path):
        old_cwd = os.getcwd()
        try:
            os.chdir(tmp_path)
            ...
            os.chdir(old_cwd)
        assert result.exit_code != 0
        assert "Not a Agile project" in result.output
    ...
    def test_list_rejects_newer_integration_state_schema(self, tmp_path):
        project = _init_project(tmp_path, "claude")
        int_json = project / ".agile" / "integration.json"
...
class TestIntegrationStatus:
    def test_status_requires_agile_project(self, tmp_path, monkeypatch):
    ...
    def test_status_json_reports_healthy_project(self, copilot_project):
        ...
        assert payload["manifest_checked_integrations"] == ["copilot", "agile"]
    ...
    def test_status_reports_invalid_integration_json(self, copilot_project):
        (copilot_project / ".agile" / "integration.json").write_text("{", encoding="utf-8")
    ...
    def test_status_reports_supported_schema_for_newer_integration_state(self, copilot_project):
        state_path = copilot_project / ".agile" / "integration.json"
    ...
    def test_status_reports_missing_integration_json(self, copilot_project):
        (copilot_project / ".agile" / "integration.json").unlink()

        result = _run_in_project(copilot_project, ["integration", "status"])

        assert result.exit_code != 0
        assert "integration-state-missing" in result.output
        assert ".agile/integration.json is missing" in result.output
        assert "Multi-install safe: unknown" in result.output
    ...
    def test_status_json_reports_no_installed_integrations_as_warning(self, copilot_project):
        ...
        assert payload["multi_install_safe"] is None
        assert payload["manifest_checked_integrations"] == ["agile"]
        assert payload["findings"][0]["code"] == "no-installed-integrations"
        assert "agile" in payload["manifests"]
        assert payload["manifests"]["agile"]["readable"] is True
    ...
    def test_status_checks_shared_manifest_when_no_integrations_installed(self, copilot_project):
        ...
        (copilot_project / ".agile" / "integrations" / "agile.manifest.json").unlink()
    ...
    def test_status_reports_default_integration_not_installed(self, claude_project):
        ...
        assert payload["manifest_checked_integrations"] == ["claude", "agile"]
        assert any(
    ...
    def test_status_reports_missing_manifest(self, copilot_project):
        (copilot_project / ".agile" / "integrations" / "copilot.manifest.json").unlink()
    ...
    def test_status_reports_modified_managed_files_without_failing(self, copilot_project):
        manifest_path = copilot_project / ".agile" / "integrations" / "copilot.manifest.json"
    ...
    def test_status_reports_missing_shared_managed_files(self, copilot_project):
        shared_file = copilot_project / ".agile" / "scripts" / "bash" / "common.sh"
        ...
        assert result.exit_code != 0
        assert "managed-files-missing" in result.output
        assert "shared Agile infrastructure" in result.output
        assert "Missing managed files: 1" in result.output

    def test_status_does_not_use_exists_precheck_for_managed_files(self, tmp_path, monkeypatch):
        from agile_cli.integration_status import _manifest_file_status
    ...
    def test_status_does_not_mask_runtime_errors_from_manifest_load(self, copilot_project, monkeypatch):
        from agile_cli import integration_status as status_module
    ...
    def test_strip_extended_length_prefix_normalizes_windows_paths(self):
        from agile_cli.integration_status import _strip_extended_length_prefix
    ...
    def test_is_within_project_tolerates_extended_length_prefix(self):
        from agile_cli.integration_status import _is_within_project
    ...
    def test_status_reports_managed_file_collisions(self, claude_project):
        ...
        claude_manifest = claude_project / ".agile" / "integrations" / "claude.manifest.json"
    ...
    def test_status_json_is_not_rich_rendered(self, tmp_path, monkeypatch):
        ...
        project.mkdir()
        (project / ".agile").mkdir()
        (project / ".agile" / "integration.json").write_text(
            json.dumps({
...
class TestIntegrationInstall:
    def test_install_requires_agile_project(self, tmp_path):
        old_cwd = os.getcwd()
        try:
            os.chdir(tmp_path)
    ...
    def test_install_multi_safe_integration(self, tmp_path):
        ...
        assert (project / ".claude" / "skills" / "agile-plan" / "SKILL.md").exists()
        assert (project / ".agents" / "skills" / "agile-plan" / "SKILL.md").exists()
    ...
    def test_install_non_default_refreshes_init_options_version_only(self, tmp_path, monkeypatch):
        ...
        opts["agile_version"] = "0.6.1"
        init_options.write_text(json.dumps(opts), encoding="utf-8")

        import agile_cli.integrations._commands as _int_cmds

        monkeypatch.setattr(_int_cmds, "get_agile_version", lambda: "0.8.11")
        ...
        assert updated["agile_version"] == "0.8.11"
    ...
    def test_install_additional_preserves_shared_manifest(self, tmp_path):
        project = _init_project(tmp_path, "claude")
        shared_manifest = project / ".agile" / "integrations" / "agile.manifest.json"
    ...
    def test_install_into_bare_project(self, tmp_path):
        """Install into a project with .agile/ but no integration."""
        ...
        # Manifest created
        assert (project / ".agile" / "integrations" / "claude.manifest.json").exists()
    ...
    def test_install_bare_project_gets_shared_infra(self, tmp_path):
        ...
        assert (project / ".agile" / "scripts").is_dir()
        assert (project / ".agile" / "templates").is_dir()
    ...
    def test_install_dollar_skill_into_bare_project_gets_native_shared_refs(
        ...
        assert result.exit_code == 0, result.output
        plan = project / ".agile" / "templates" / "plan-template.md"
        plan_content = plan.read_text(encoding="utf-8")
        assert "$agile-plan" in plan_content
        assert "/agile-plan" not in plan_content
    ...
    def test_install_defers_extension_commands_until_use(self, tmp_path):
        ...
        registry_path = project / ".agile" / "extensions" / ".registry"
        ...
        assert not (
            project / ".agents" / "skills" / "agile-git-feature" / "SKILL.md"
        ).exists()
    ...
    def test_install_skills_mode_secondary_agent_defers_extension_artifacts(self, tmp_path):
        ...
        assert (
            project / ".github" / "skills" / "agile-specify" / "SKILL.md"
        ).exists(), "precondition: copilot installed in skills mode"

        # The git extension is not registered for the non-active copilot agent
        # during install.
        git_meta = json.loads(
            (project / ".agile" / "extensions" / ".registry").read_text(encoding="utf-8")
        )["extensions"]["git"]
        assert "copilot" not in git_meta["registered_commands"]
        assert not (
            project / ".github" / "agents" / "agile.git.feature.agent.md"
        ).exists()
        assert not (
            project / ".github" / "skills" / "agile-git-feature" / "SKILL.md"
        ).exists()
        ...
        # skills-mode layout.
        assert "copilot" not in git_meta["registered_commands"]
        assert "agile-git-feature" in git_meta["registered_skills"]
        assert not (
            project / ".github" / "agents" / "agile.git.feature.agent.md"
        ).exists()
        assert (
    ...
    def test_extension_add_registers_active_integration_only(self, tmp_path):
        ...
        assert (
            project / ".claude" / "skills" / "agile-git-feature" / "SKILL.md"
        ).exists()
        assert not (
            project / ".agents" / "skills" / "agile-git-feature" / "SKILL.md"
        ).exists()
    ...
    def test_extension_add_malformed_ai_value_fails_closed(self, tmp_path):
        ...
        init_options_path = project / ".agile" / "init-options.json"
    ...
    def test_extension_add_dangling_init_options_symlink_fails_closed(self, tmp_path):
        ...
        init_options_path.symlink_to(project / ".agile" / "does-not-exist.json")
...
class TestIntegrationUninstall:
    def test_uninstall_requires_agile_project(self, tmp_path):
        old_cwd = os.getcwd()
        try:
            os.chdir(tmp_path)
    ...
    def test_uninstall_removes_files(self, tmp_path):
        ...
        assert not (project / ".claude" / "skills" / "agile-plan" / "SKILL.md").exists()

        # Manifest removed
        assert not (project / ".agile" / "integrations" / "claude.manifest.json").exists()

        # integration.json removed
        assert not (project / ".agile" / "integration.json").exists()

    def test_uninstall_preserves_modified_files(self, tmp_path):
        """Full lifecycle: install → modify → uninstall → modified file kept."""
        project = _init_project(tmp_path, "claude")
        plan_file = project / ".claude" / "skills" / "agile-plan" / "SKILL.md"
        ...
        assert ".claude/skills/agile-plan/SKILL.md" in result.output
    ...
    def test_uninstall_non_default_preserves_default(self, tmp_path):
        ...
        try:
            ...
            os.chdir(old_cwd)
        assert result.exit_code == 0, result.output
        assert not (project / ".agents" / "skills" / "agile-plan" / "SKILL.md").exists()
        assert (project / ".claude" / "skills" / "agile-plan" / "SKILL.md").exists()
    ...
    def test_uninstall_default_refreshes_templates_for_fallback(self, tmp_path):
        ...
        template = project / ".agile" / "templates" / "plan-template.md"
        script = project / ".agile" / "scripts" / "bash" / "check-prerequisites.sh"
        assert "/agile.plan" in template.read_text(encoding="utf-8")
        assert "/agile.plan" in script.read_text(encoding="utf-8")
    ...
    def test_uninstall_preserves_shared_infra(self, tmp_path):
        ...
        project = _init_project(tmp_path, "claude")
        shared_script = project / ".agile" / "scripts" / "bash" / "common.sh"
...
class TestIntegrationUse:
    def test_use_installed_integration_sets_default(self, tmp_path):
        ...
        opts = json.loads((project / ".agile" / "init-options.json").read_text(encoding="utf-8"))
    ...
    def test_use_preserves_copilot_skills_mode(self, tmp_path):
        ...
        # skills-mode agent.
        assert not (project / ".github" / "agents" / "agile.git.feature.agent.md").exists()
        assert not (project / ".github" / "prompts" / "agile.git.feature.prompt.md").exists()
        assert (
            project / ".github" / "skills" / "agile-git-feature" / "SKILL.md"
        ).exists()
    ...
    def test_use_reregisters_presets_highest_precedence_last(self, tmp_path):
        ...
        claude_skill = project / ".claude" / "skills" / "agile-specify" / "SKILL.md"
        ...
        codex_skill = project / ".agents" / "skills" / "agile-specify" / "SKILL.md"
    ...
    def test_use_refreshes_shared_templates_between_command_styles(self, tmp_path):
        ...
        try:
            ...
            assert "/agile.plan" in template.read_text(encoding="utf-8")
            assert "/agile.plan" in script.read_text(encoding="utf-8")
            assert "/agile-plan" not in script.read_text(encoding="utf-8")
    ...
    def test_use_preserves_modified_templates_unless_forced(self, tmp_path):
        project = _init_project(tmp_path, "claude")
        template = project / ".agile" / "templates" / "plan-template.md"
        template.write_text("custom template with /agile-plan\n", encoding="utf-8")
    ...
    def test_use_does_not_persist_default_when_shared_infra_refresh_fails(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setattr(agile_cli, "_install_shared_infra", fail_refresh)
...
class TestIntegrationSwitch:
    def test_switch_requires_agile_project(self, tmp_path):
        old_cwd = os.getcwd()
        try:
            os.chdir(tmp_path)
    ...
    def test_switch_between_integrations(self, tmp_path):
        ...
        assert (project / ".claude" / "skills" / "agile-plan" / "SKILL.md").exists()
        shared_script = project / ".agile" / "scripts" / "bash" / "check-prerequisites.sh"
    ...
    def test_switch_rejects_conflicting_copilot_modes_before_uninstall(
        ...
        project = _init_project(tmp_path, "claude")
        claude_skill = (
            project / ".claude" / "skills" / "agile-plan" / "SKILL.md"
        )
        before_state = json.loads(
            (project / ".agile" / "integration.json").read_text(
                encoding="utf-8"
            )
        )
    ...
    def test_switch_preserves_target_options_with_fallback_integration(
        ...
        assert result.exit_code == 0, result.output
        assert (
            project / ".github" / "agents" / "agile.plan.agent.md"
        ).exists()
        assert not (project / ".github" / "skills").exists()
        state = json.loads(
            (project / ".agile" / "integration.json").read_text(
    ...
    def test_switch_migrates_extension_commands(self, tmp_path):
        ...
        kimi_git_feature = project / ".kimi-code" / "skills" / "agile-git-feature" / "SKILL.md"
        ...
        opencode_git_feature = project / ".opencode" / "commands" / "agile.git.feature.md"
        ...
        claude_git_feature = project / ".claude" / "skills" / "agile-git-feature" / "SKILL.md"
    ...
    def test_switch_migrates_copilot_skills_extension_commands(self, tmp_path):
        ...
        copilot_git_feature = project / ".github" / "skills" / "agile-git-feature" / "SKILL.md"
        copilot_agent_file = project / ".github" / "agents" / "agile.git.feature.agent.md"
    ...
    def test_switch_refreshes_managed_shared_script_refs(self, tmp_path):
        ...
        project = _init_project(tmp_path, "claude")
        shared_script = project / ".agile" / "scripts" / "bash" / "setup-tasks.sh"
        assert shared_script.exists()
        shared_content = shared_script.read_text(encoding="utf-8")
        assert "/agile-plan" in shared_content
        ...
        assert "/agile.plan" in updated
        assert "/agile-plan" not in updated
    ...
    def test_switch_refreshes_stale_managed_shared_infra(self, tmp_path):
        ...
        project = _init_project(tmp_path, "claude")
        shared_script = project / ".agile" / "scripts" / "bash" / "setup-tasks.sh"
        assert "/agile-plan" in shared_script.read_text(encoding="utf-8")

        # Simulate a stale vendored script: write truncated content as bytes
        # (write_text would translate \n→\r\n on Windows and break the hash)
        # and update the agile manifest hash so the stale copy is treated
        # as "managed" (installed by agile, not a user customization).
        stale_bytes = b"#!/usr/bin/env bash\n# stale vendored copy\n"
        shared_script.write_bytes(stale_bytes)

        manifest_path = project / ".agile" / "integrations" / "agile.manifest.json"
        manifest_data = json.loads(manifest_path.read_text(encoding="utf-8"))
        manifest_data["files"][".agile/scripts/bash/setup-tasks.sh"] = (
    ...
    def test_switch_preserves_recovered_files(self, tmp_path):
        ...
        manifest_path = project / ".agile" / "integrations" / "agile.manifest.json"
        manifest_data = json.loads(manifest_path.read_text(encoding="utf-8"))
        rel = ".agile/scripts/bash/setup-tasks.sh"
    ...
    def test_switch_skips_symlinked_parent_directory(self, tmp_path):
        """Regression: if .agile/scripts/bash is a symlink, switch must not write through it.
        ...
        project = _init_project(tmp_path, "claude")
        bash_dir = project / ".agile" / "scripts" / "bash"
        outside = tmp_path / "outside"
        outside.mkdir()
    ...
    def test_failed_switch_rescaffolds_fallback_extensions(self, tmp_path):
        ...
        data = json.loads(
            (project / ".agile" / "integration.json").read_text(encoding="utf-8")
        )
...
class TestIntegrationUpgrade:
    ...
    def test_upgrade_refreshes_init_options_agile_version(self, tmp_path, monkeypatch):
        project = _init_project(tmp_path, "claude")
        init_options = project / ".agile" / "init-options.json"
        opts = json.loads(init_options.read_text(encoding="utf-8"))
        opts["agile_version"] = "0.6.1"
    ...
    def test_upgrade_does_not_persist_state_when_shared_infra_refresh_fails(self, tmp_path, monkeypatch):
        ...
        int_json = project / ".agile" / "integration.json"
        init_options = project / ".agile" / "init-options.json"
        manifest_path = project / ".agile" / "integrations" / "claude.manifest.json"
        ...
        real_install_shared_infra = agile_cli._install_shared_infra
        calls = {"count": 0}
    ...
    def test_upgrade_default_refreshes_shared_script_refs_for_option_separator_change(self, tmp_path):
        ...
        template = project / ".agile" / "templates" / "plan-template.md"
        managed_script = project / ".agile" / "scripts" / "bash" / "check-prerequisites.sh"
    ...
    def test_upgrade_preserves_historical_copilot_commands_without_options(
        ...
        state_path = project / ".agile" / "integration.json"
        state = json.loads(state_path.read_text(encoding="utf-8"))
        ...
        init_options = json.loads(
            (project / ".agile" / "init-options.json").read_text(
                encoding="utf-8"
            )
        )
    ...
    def test_upgrade_migrates_opencode_legacy_dir(self, tmp_path):
        ...
        manifest_path = project / ".agile" / "integrations" / "opencode.manifest.json"
        manifest_data = json.loads(manifest_path.read_text(encoding="utf-8"))
        patched_files = {}
        for path, info in manifest_data.get("files", {}).items():
        ...
        old_commands = sorted(legacy.glob("agile.*.md"))
        assert len(old_commands) > 0, "Legacy dir should have agile command files"
        ...
        new_commands = sorted(canonical.glob("agile.*.md"))
        ...
        core_remaining = [
            f for f in legacy.glob("agile.*.md")
            if "agent-context" not in f.name
        ]
        assert len(core_remaining) == 0, (
            f"Legacy .opencode/command/ should have no core agile files after upgrade, "
    ...
    def test_upgrade_migrates_kilocode_legacy_dir(self, tmp_path):
        ...
        old_commands = sorted(legacy.glob("agile.*.md"))
        assert old_commands, "Legacy dir should have agile command files"
        ...
        assert core_remaining == [], (
            "Legacy .kilocode/workflows/ should have no core agile files "
    ...
    def test_upgrade_migrates_qodercli_extension_commands_to_skills(self, tmp_path):
        ...
        manifest_path = (
            project / ".agile" / "integrations" / "qodercli.manifest.json"
        )
        manifest_data = json.loads(manifest_path.read_text(encoding="utf-8"))
        legacy_manifest_files = {}
        for path, info in manifest_data["files"].items():
            skill_path = project / path
            command_name = skill_path.parent.name.replace("agile-", "agile.", 1)
        ...
        for command_name in registered_commands:
            skill_name = command_name.replace("agile.", "agile-", 1).replace(
                ".", "-"
            )
        ...
        missing_replacement = commands / "agile.git.missing.md"
        missing_replacement.write_text("# preserve until replaced\n", encoding="utf-8")
        registered_commands.append("agile.git.missing")
    ...
    def test_upgrade_kilocode_legacy_dir_rejects_installed_preset_overrides(
        ...
        preset_file = legacy / "agile.plan.md"
        preset_file.write_text("# preset plan override\n", encoding="utf-8")

        presets_dir = project / ".agile" / "presets"
            ...
            json.dumps({
                "presets": {
                    "my-preset": {
                        "version": "1.0.0",
                        "enabled": True,
                        "registered_commands": {"kilocode": ["agile.plan"]},
                        "registered_skills": [],
                    }
                }
            }),
    ...
    def test_upgrade_reconciles_kilocode_legacy_extension_artifacts(self, tmp_path):
        ...
        result = _run_in_project(project, ["extension", "add", "git"])
        assert result.exit_code == 0, f"extension add failed: {result.output}"
        assert sorted(legacy.glob("agile.git.*.md")), (
        ...
        assert sorted(canonical.glob("agile.git.*.md")), (
            "enabled git extension commands should be recreated in .kilo/commands"
        )
        assert not sorted(legacy.glob("agile.git.*.md")), (
    ...
    def test_upgrade_preserves_disabled_kilocode_legacy_extension_and_user_file(
        ...
        disabled_extension_files = sorted(legacy.glob("agile.git.*.md"))
        assert disabled_extension_files, "disabled extension artifact should remain pre-upgrade"

        user_file = legacy / "agile.user-owned.md"
        ...
        assert not sorted(canonical.glob("agile.git.*.md")), (
    ...
    def test_upgrade_secondary_kilocode_legacy_dir_cleans_commands_without_backfill(
        ...
        skill = project / ".github" / "skills" / "agile-git-feature" / "SKILL.md"
        ...
        canonical, legacy = _move_kilocode_install_to_legacy_layout(project)
        legacy_git_command = legacy / "agile.git.feature.md"
    ...
    def test_upgrade_bob_skills_migration_preserves_manifest(self, tmp_path):
        ...
        (``.bob/skills/agile-*/SKILL.md``) and stale-removes the old command
        ...
        manifest_path = (
            project / ".agile" / "integrations" / "bob.manifest.json"
        )
        assert commands.is_dir() and sorted(commands.glob("agile.*.md"))
        ...
        assert sorted(skills.glob("agile-*")), "expected migrated skill dirs"
        core_commands = [
            f for f in commands.glob("agile.*.md")
            if "agent-context" not in f.name
    ...
    def test_upgrade_bob_layout_change_reconciles_extension_artifacts(self, tmp_path):
        ...
        assert sorted(commands.glob("agile.git.*.md")), (
            "legacy Bob should render the git extension as command files"
        )
        assert not list(skills.glob("agile-git-*")) if skills.exists() else True
        ...
        assert not sorted(commands.glob("agile.git.*.md")), (
            "git extension command files must be removed after --skills migration"
        )
        assert sorted(skills.glob("agile-git-*")), (
        ...
        assert not sorted(skills.glob("agile-git-*")), (
    ...
    def test_upgrade_layout_change_preserves_extension_artifacts_when_reregistration_fails(
        ...
        (
            project / ".agile" / "extensions" / "git" / "extension.yml"
    ...
    def test_upgrade_active_layout_change_rejected_before_missing_preset_source_can_lose_override(
        ...
        preset_src = tmp_path / "cmd-preset"
        (preset_src / "commands").mkdir(parents=True)
        (preset_src / "commands" / "agile.plan.md").write_text(
        ...
        cmd_file = commands / "agile.plan.md"
    ...
    def test_upgrade_secondary_layout_change_rejected_with_presets_installed(
        ...
        commands = project / ".bob" / "commands"
        skills = project / ".bob" / "skills"
        assert sorted(commands.glob("agile.*.md"))
            ...
            json.dumps({
                "presets": {
                    "my-preset": {
                        "version": "1.0.0",
                        "enabled": True,
                        "registered_commands": {"bob": ["agile.plan"]},
                        "registered_skills": {},
                    }
                }
            }),
    ...
    def test_upgrade_active_integration_reregisters_extensions(self, tmp_path):
        ...
        cmd_file = project / ".claude" / "skills" / "agile-git-feature" / "SKILL.md"
    ...
    def test_upgrade_copilot_skills_restores_extension_skill_over_regenerated_dir(
        ...
        skill_dir = project / ".github" / "skills" / "agile-git-feature"
    ...
    def test_upgrade_active_integration_reregisters_presets(self, tmp_path):
        ...
        (preset_src / "commands").mkdir(parents=True)
        (preset_src / "commands" / "agile.upgrade-check.md").write_text(
        ...
        manifest = {
            ...
            "provides": {
                "templates": [
                    {
                        "type": "command",
                        "name": "agile.upgrade-check",
                        "file": "commands/agile.upgrade-check.md",
                    }
                ]
            },
        }
        ...
        skill_dir = (
            project / ".claude" / "skills" / "agile-upgrade-check"
        )
        skill_file = skill_dir / "SKILL.md"
    ...
    def test_installed_presets_affecting_agent_absent_vs_unreadable(self, tmp_path):
        ...
        """
        from agile_cli.integrations._migrate_commands import (
        ...
        registry.write_text(
            json.dumps({
                "presets": {
                    "p1": {"registered_commands": {"bob": ["agile.plan"]}},
                    "p2": {"registered_commands": {"codex": ["agile.plan"]}},
                    "p3": {"registered_skills": ["agile-x"]},
                    "p4": {"registered_skills": {"bob": ["agile-y"]}},
                    "p5": {"registered_skills": {"codex": ["agile-z"]}},
                    "p6": {"registered_skills": {"bob": []}},
                    "p7": {
                        "enabled": False,
                        "registered_commands": {"bob": ["agile.tasks"]},
                    },
                }
            }),
            encoding="utf-8",
...
class TestParseIntegrationOptionsEqualsForm:
    def test_equals_form_parsed(self):
        ...
        from agile_cli.integrations._commands import _parse_integration_options
        from agile_cli.integrations import get_integration
...
class TestUninstallNoManifestClearsInitOptions:
    def test_init_options_cleared_on_no_manifest_uninstall(self, tmp_path):
        ...
        opts_json = project / ".agile" / "init-options.json"
