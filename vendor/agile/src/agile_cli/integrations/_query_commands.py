...
def integration_search(
    ...
    try:
        ...
        console.print(
            ...
            "(for example, .agile/integration-catalogs.yml or ~/.agile/integration-catalogs.yml)."
        )
        raise typer.Exit(1)
    except IntegrationCatalogError as exc:
        console.print(f"[red]Error:[/red] {exc}")
        if os.environ.get("AGILE_INTEGRATION_CATALOG_URL", "").strip():
            console.print(
                "\nTip: Check the AGILE_INTEGRATION_CATALOG_URL environment variable for an invalid "
                "catalog URL, or unset it to use the configured catalog files "
                "(.agile/integration-catalogs.yml or ~/.agile/integration-catalogs.yml)."
            )
        else:
...
def integration_info(
    ...
    if catalog_error:
        ...
        elif os.environ.get("AGILE_INTEGRATION_CATALOG_URL", "").strip():
            console.print(
                "\nCheck whether AGILE_INTEGRATION_CATALOG_URL is set correctly and reachable, "
...
def integration_catalog_list():
    ...
    project_root = _require_specify_project()
    catalog = IntegrationCatalog(project_root)
    env_override = os.environ.get("AGILE_INTEGRATION_CATALOG_URL", "").strip()
    ...
    if env_override:
        console.print(
            "  AGILE_INTEGRATION_CATALOG_URL is set; it supersedes configured catalog files."
        )
        console.print(
