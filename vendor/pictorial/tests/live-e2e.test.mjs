...
// to iterate on. Set PICTORIAL_E2E_ONLY=<name>[,<name>...] to scope the run.
const onlyNames = parseFixtureFilter(process.env.PICTORIAL_E2E_ONLY);
...
if (missingOnlyNames.length > 0) {
  throw new Error(`Unknown PICTORIAL_E2E_ONLY fixture(s): ${missingOnlyNames.join(', ')}`);
}

const manualOnly = process.env.PICTORIAL_E2E_MANUAL_ONLY === '1'
  || process.env.PICTORIAL_E2E_MANUAL_ONLY === 'true';
const reloadVariants = process.env.PICTORIAL_E2E_RELOAD_VARIANTS === '1'
  || process.env.PICTORIAL_E2E_RELOAD_VARIANTS === 'true';
const scenarioNames = parseFixtureFilter(process.env.PICTORIAL_E2E_SCENARIOS);
const liveE2eTestTimeoutMs = readPositiveIntEnv('PICTORIAL_E2E_TEST_TIMEOUT_MS');
...
const atomicDelayMs = readPositiveIntEnv('PICTORIAL_E2E_ATOMIC_DELAY_MS') || 0;
...
const LIVE_TREE_URL_RE = /\.pictorial-live|pictorial\/live\/preview/i;
...
for (const { name, fixture } of fixtures) {
  describe(`live-e2e · ${name} (${fixture.runtime.styling || 'unknown-styling'})`, () => {
    it('drives the full click → Go → cycle → accept cycle', liveE2eTestOptions, async (t) => {
      ...
      if (manualOnly || process.env.PICTORIAL_E2E_MANUAL_SCENARIO) {
      ...
      // Pick the agent. `PICTORIAL_E2E_AGENT=llm` opts into Claude first,
      ...
      const agentMode = process.env.PICTORIAL_E2E_AGENT || 'fake';
      let agent;
      if (agentMode === 'llm') {
        const llmConfig = resolveLlmAgentConfig({
          model: process.env.PICTORIAL_E2E_LLM_MODEL,
        });
        ...
        if (!agent) {
          t.skip(`PICTORIAL_E2E_AGENT=llm with provider=${llmConfig.provider} requires ${llmConfig.requiredEnv}`);
          return;
        }
      ...
      const variantContentSelectorFor = (variantNum) => (isInsert
        ? (usesSvelteComponentPreview ? '.inserted-copy' : `[data-pictorial-variant="${variantNum}"] .inserted-copy`)
        : usesSvelteComponentPreview
        ? pickSelector
        : `[data-pictorial-variant="${variantNum}"] > :first-child`);
      ...
      try {
        ...
        if (isInsert) {
          ...
          if (process.env.PICTORIAL_E2E_DEBUG) {
            const barText = await page.evaluate(() => {
              const bar = document.querySelector('#pictorial-live-bar');
        ...
        if (svelteComponentSession) {
          ...
          assert.doesNotMatch(routeBody, /data-pictorial-variants="/, 'route source is not edited during component preview');
        } else {
          assert.match(after, /data-pictorial-variants="/, 'wrapper inserted');
        }
        if (isInsert) {
          if (svelteComponentSession) {
            assert.equal(svelteComponentSession.manifest.mode, 'insert', 'Svelte insert uses component preview mode');
          } else {
            assert.match(after, /data-pictorial-mode="insert"/, 'insert mode wrapper');
            assert.doesNotMatch(after, /data-pictorial-variant="original"/, 'insert has no original variant');
          }
        ...
        if (svelteComponentSession) {
          ...
          assert.match(after, /<style is:inline data-pictorial-css="/, 'Astro live CSS uses an inline compiler-bypassing style block');
          assert.match(
            after,
            /\[data-pictorial-variant="1"\]\s*>\s*(?:h1|\.[\w-]+)/,
          ...
          assert.doesNotMatch(after, /@scope \(\[data-pictorial-variant="1"\]\)/, 'Astro live CSS does not use raw @scope');
        } else {
          assert.match(after, /<style data-pictorial-css="/, 'colocated <style> block present');
          assert.match(after, /@scope \(\[data-pictorial-variant="1"\]\)/, 'scoped CSS for variant 1');
          assert.match(after, /@scope \(\[data-pictorial-variant="2"\]\)/, 'scoped CSS for variant 2');
          assert.match(after, /@scope \(\[data-pictorial-variant="3"\]\)/, 'scoped CSS for variant 3');
        }
        ...
        if (agentMode === 'fake') {
          const paramsSource = svelteComponentSession
            ...
            : after;
          assert.match(paramsSource, svelteComponentSession ? /"1"\s*:/ : /data-pictorial-params=/, 'params manifest emitted');
          for (const kind of ['range', 'steps', 'toggle']) {
            assert.match(paramsSource, new RegExp(`"kind"\\s*:\\s*"${kind}"`), `param kind ${kind} present`);
          }
          await page.waitForFunction(() => {
            const root = window.__PICTORIAL_LIVE_CHROME_CORE__?.root?.()
              || window.__PICTORIAL_LIVE_UI_ROOT__
              || document;
        ...
        const assertVisibleVariantStyle = async (variantNum) => {
          ...
          if (variantWeight !== expected) {
            const styleSnapshot = await evaluatePageWithTimeout(
              page,
              (sel) => {
                const query = window.__pictorialLiveQuery || ((s) => document.querySelector(s));
                const el = query(sel) || document.querySelector(sel);
                const styleEl = document.querySelector('style[data-pictorial-css]');
                const rules = [];
        ...
        if (svelteComponentTarget) {
          ...
          const snapshotPath = join(appRoot, '.pictorial/live/sessions', `${svelteComponentTarget.manifest.id}.snapshot.json`);
          const snapshot = JSON.parse(readFileSync(snapshotPath, 'utf-8'));
          assert.equal(snapshot.phase, 'completed');
          assert.equal(snapshot.sourceFile, svelteComponentTarget.manifest.sourceFile);
          assert.doesNotMatch(snapshot.sourceFile, /node_modules\/\.pictorial-live|\.pictorial\/live\/previews/);
        }
        assert.doesNotMatch(final, /data-pictorial-variants="/,    'variants wrapper removed');
        assert.doesNotMatch(final, /pictorial-variants-start/,      'variants-start marker removed');
        assert.doesNotMatch(final, /pictorial-carbonize-start/,     'carbonize-start marker removed');
        assert.doesNotMatch(final, /pictorial-carbonize-end/,       'carbonize-end marker removed');
        assert.doesNotMatch(final, /data-pictorial-carbonize=/,     'carbonize wrapper removed');
        assert.doesNotMatch(final, /data-pictorial-variant="/,      'no leftover variant scaffolding');
        if (isInsert) {
          if (agentMode === 'fake') {
        ...
        //     may 404 or fail. A missing `.pictorial-live` module means the
      ...
      if (shouldRunScenario('mount-failure')) {
        it('surfaces a broken variant without losing the session, and recovers on Retry', liveE2eTestOptions, async (t) => {
          ...
          try {
            ...
            writeFileSync(variantPath, '<div class="pictorial-broken-variant">{ not valid svelte\n', 'utf-8');
            ...
            const barVisible = await page.evaluate(() => {
              const bar = window.__pictorialLiveQuery('#pictorial-live-bar');
    ...
    if (shouldRunScenario('missed-done') && fixture.runtime.missedDoneReloadScenario) {
      it('recovers when the preflight reload makes the browser miss the done broadcast', liveE2eTestOptions, async (t) => {
        ...
        try {
          ...
          for (;;) {
            if (/data-pictorial-variant="3"/.test(readFileSync(scaffoldSourceFile, 'utf-8'))) break;
    ...
    if (shouldRunScenario('orphan') && fixture.runtime.orphanedWrapperScenario) {
      it('self-discards an orphaned session when its wrapper is edited out of source', liveE2eTestOptions, async (t) => {
        ...
        try {
          ...
          const snapshotPath = join(appRoot, '.pictorial/live/sessions', `${saved.id}.snapshot.json`);
          const snapshot = JSON.parse(readFileSync(snapshotPath, 'utf-8'));
          assert.equal(
            snapshot.phase,
            'discarded',
        ...
        try {
          ...
          for (const foreign of cases) {
            ...
            await page.evaluate((saved) => {
              localStorage.setItem('pictorial-live-session', JSON.stringify(saved));
              localStorage.removeItem('pictorial-live-session-handled');
            }, {
              id: foreign.id,
              state: 'CYCLING',
              expected: 3,
            ...
            const sessionsDir = join(appRoot, '.pictorial/live/sessions');
            assert.equal(
    ...
    if (shouldRunScenario('manual') && Array.isArray(fixture.runtime.manualEditScenarios) && fixture.runtime.manualEditScenarios.length > 0) {
      const manualScenarioFilter = process.env.PICTORIAL_E2E_MANUAL_SCENARIO || '';
    ...
    if (shouldRunScenario('annotations') && fixture.runtime.liveChrome?.annotations) {
      it('uploads annotations with generate and still accepts the variant', liveE2eTestOptions, async (t) => {
        ...
        try {
          ...
          assert.match(generateEvent.screenshotPath, /\.pictorial\/live\/annotations\//, 'annotation screenshot is stored under live annotations');
...
function canRunFakeAgentScenario(t) {
  ...
  if ((process.env.PICTORIAL_E2E_AGENT || 'fake') !== 'fake') {
...
async function waitForJournalEvent(appRoot, sessionId, type, timeoutMs = 15_000) {
  const journalPath = join(appRoot, '.pictorial', 'live', 'sessions', `${sessionId}.jsonl`);
...
function assertNestedAppRoots(session) {
  ...
  assert.ok(
    existsSync(join(appRoot, '.pictorial/live/roots.json')),
    'the resolved manifest is persisted under the app',
  );
  assert.ok(
    existsSync(join(appRoot, '.pictorial/live/server.json')),
    'the live server registers itself under the app',
  );
  assert.ok(
    existsSync(join(tmp, '.pictorial/live/app-root.json')),
    'the repo root gets a pointer to the app',
  );
  assert.equal(
    existsSync(join(tmp, '.pictorial/live/server.json')),
    false,
...
function countSourceVariants(source) {
  return (String(source).match(/<div\s+data-pictorial-variant="(?!original")/g) || []).length;
}
...
async function waitForGenerationTimings(tmp, id, { timeoutMs = 5_000, requireAllVariants = true } = {}) {
  const snapshotPath = join(tmp, '.pictorial', 'live', 'sessions', `${id}.snapshot.json`);
  const journalPath = join(tmp, '.pictorial', 'live', 'sessions', `${id}.jsonl`);
  const deadline = Date.now() + timeoutMs;
...
async function captureLiveE2eFailure({ name, fixture, session, sourceFile, error, log = () => {} }) {
  const root = process.env.PICTORIAL_E2E_ARTIFACT_DIR;
  ...
  try {
    ...
    copyDirIfExists(join(tmp, '.pictorial', 'live'), join(dir, 'pictorial-live'));
    copyDirIfExists(join(tmp, 'node_modules', '.pictorial-live'), join(dir, 'pictorial-live-preview'));
    copyDirIfExists(join(tmp, '.pictorial', 'live', 'previews'), join(dir, 'pictorial-live-previews'));
...
async function createManualScenarioAgent(t, scenario = {}) {
  const requested = (process.env.PICTORIAL_E2E_MANUAL_AGENT || process.env.PICTORIAL_E2E_AGENT || 'auto')
    .trim()
    .toLowerCase();
...
function maybeWrapMalformedAckProbe(agent, scenario, probeState, t) {
  ...
  return {
    ...
    async applyManualEdits(event, context = {}) {
      ...
      const buffer = JSON.parse(readFileSync(join(context.tmp, '.pictorial/live/pending-manual-edits.json'), 'utf-8'));
...
async function waitForAcceptedSelectionReady(page, selector, { timeout }) {
  await page.waitForFunction(
    (sel) => {
      ...
      for (const el of all) {
        if (el.closest('[data-pictorial-variants],[data-pictorial-variant]')) return false;
      }
      return true;
    },
    selector,
...
async function readLiveSessionStorage(page) {
  return page.evaluate(() => {
    const raw = localStorage.getItem('pictorial-live-session');
...
async function waitForVariantCounter(page, variant, count, { timeout = 15_000 } = {}) {
  try {
    await page.waitForFunction(
      ({ variant, count }) => {
        const query = window.__pictorialLiveQuery || ((sel) => document.querySelector(sel));
        const bar = query('#pictorial-live-bar');
    ...
    const snapshot = await page.evaluate(() => {
      ...
      return {
        ...
        debugState: window.__PICTORIAL_LIVE_CHROME_CORE__?.debugState?.() || null,
        storage: localStorage.getItem('pictorial-live-session'),
        wrapper: wrapper ? { preview: wrapper.dataset.pictorialPreview, count: wrapper.dataset.pictorialVariantCount, html: wrapper.outerHTML.slice(0, 500) } : null,
      };
...
async function waitForAcceptedDom(page, selector, { allowVariantRoot = false, timeout = 20_000 } = {}) {
  await page.waitForFunction(
    ({ sel, allowVariantRoot }) => {
      ...
      if (all.length < 1) return false;
      for (const el of all) {
        if (el.closest('[data-pictorial-variants]')) return false;
        if (el.closest('[data-pictorial-carbonize]')) return false;
        if (!allowVariantRoot && el.closest('[data-pictorial-variant]')) return false;
      }
      return true;
    },
...
async function waitForSourceClean(filePath, timeoutMs, { svelteComponentTarget: knownSvelteTarget = null } = {}) {
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    const dirty =
      ...
      last.includes('data-pictorial-variants=') ||
      last.includes('pictorial-variants-start') ||
      last.includes('pictorial-carbonize-start') ||
      last.includes('data-pictorial-carbonize=') ||
      last.includes('data-pictorial-variant=');
...
function sourceShadowTargetFor(filePath) {
  ...
  if (!body.includes('data-pictorial-preview="source-shadow"')) return null;
  const match = body.match(/\bdata-pictorial-source-file=(["'])(.*?)\1/);
  if (!match) return null;
  const root = filePath.includes('/.pictorial/')
    ? filePath.slice(0, filePath.indexOf('/.pictorial/'))
    : dirname(filePath);
...
function svelteComponentTargetFor(filePath) {
  ...
  const markers = [
    `${sep}.pictorial${sep}live${sep}previews${sep}`,
    `${sep}node_modules${sep}.pictorial-live${sep}`,
    `${sep}src${sep}lib${sep}pictorial${sep}`,
    `${sep}app${sep}.pictorial-live${sep}`,
  ];
...
async function locateSessionFile(tmp) {
  ...
  for (const f of candidates) {
    ...
    if (
      body.includes('data-pictorial-variants=') ||
      body.includes('pictorial-carbonize-start') ||
      body.includes('pictorial-variants-start')
    ) {
      return f;
    }
  }
...
function walkComponentManifests(root) {
  const results = [];
  const stack = [
    join(root, '.pictorial/live/previews'),
    join(root, 'node_modules/.pictorial-live'),
    join(root, 'src/lib/pictorial'),
    join(root, 'app/.pictorial-live'),
  ];
  while (stack.length) {
    const dir = stack.pop();
    let entries;
