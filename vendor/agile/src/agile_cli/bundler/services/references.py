...
Used by ``agile bundle validate`` (FR-005 / SC-007) to confirm that every
declared component points at something installable. Resolution is offline-first:
a reference resolves when the component is bundled with Agile or already
...
def _resolved_locally(root: Path, component: ComponentRef) -> bool:
    ...
    try:
        ...
        if kind == "steps":
            ...
            # Step types ship with Agile as built-ins (shell, gate, if, ...)
            # rather than as an on-disk asset directory, so there is no
            # ``_locate_bundled_step`` to mirror the three lookups above.
            # ``BUILTIN_STEP_TYPES`` is the bundled-with-Agile check for this
