---
description: "Auto-commit changes after a Agile command completes"
---

# Auto-Commit Changes

Automatically stage and commit all changes after a Agile command completes.
...
2. Checks `.agile/extensions/git/git-config.yml` for the `auto_commit` section
...
Controlled by the `commit_style` key in `.agile/extensions/git/git-config.yml`:

- **`fixed`** (default): use the per-command `message` if configured, otherwise a generic `[Agile] Auto-commit <phase> <command>` message.
...
- **Bash**: `.agile/extensions/git/scripts/bash/auto-commit.sh <event_name> [--message-file <path>]`
- **PowerShell**: `.agile/extensions/git/scripts/powershell/auto-commit.ps1 <event_name> [-MessageFile <path>]`

Replace `<event_name>` with the actual hook event (e.g., `after_specify`, `before_plan`, `after_implement`). Only pass a generated message when `commit_style: conventional` is configured — first check `.agile/extensions/git/git-config.yml` for the value of `commit_style`:
...
In `.agile/extensions/git/git-config.yml`:
...
auto_commit:
  ...
  after_plan:
    enabled: false
    message: "[Agile] Add implementation plan"
```
