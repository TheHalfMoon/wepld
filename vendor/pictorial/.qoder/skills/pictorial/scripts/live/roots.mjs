 ...
 *   sessionRoot <appRoot>/.pictorial/live — durable live state.
 *
 ...
 *
 * The resolved manifest is persisted at <appRoot>/.pictorial/live/roots.json
 * plus a pointer at <repoRoot>/.pictorial/live/app-root.json when the two
...
function isAppRoot(dir) {
  ...
  return hasDevConfig(dir) || exists(path.join(dir, '.pictorial', 'live', 'config.json'));
}
...
export function resolveRoots({ cwd = process.cwd(), targetPath = null } = {}) {
  ...
  return {
    manifest: {
      ...
      contextRoot,
      sessionRoot: path.join(appRoot, '.pictorial', 'live'),
      productPath,
      designPath,
      resolvedFrom,
    },
...
function rootsFilePath(appRoot) {
  return path.join(appRoot, '.pictorial', 'live', ROOTS_FILE);
}

function pointerFilePath(repoRoot) {
  return path.join(repoRoot, '.pictorial', 'live', POINTER_FILE);
}
...
function hasLiveServer(appRoot) {
  ...
  let token;
  try {
    const info = JSON.parse(fs.readFileSync(path.join(appRoot, '.pictorial', 'live', 'server.json'), 'utf-8'));
...
function hasActiveDurableSession(appRoot) {
  const dir = path.join(appRoot, '.pictorial', 'live', 'sessions');
  let entries;
  try {
 ...
 *
 *   1. Walk up from cwd looking for .pictorial/live/roots.json.
 *   2. At the git root, follow .pictorial/live/app-root.json to the app.
      ...
      if (candidates.length > 0) {
        ...
        if (tier.length > 1) {
          ...
          process.stderr.write(
            `[pictorial live] Multiple apps in this repo have live state; using ${chosen}. `
...
export function enterLiveRoot(cwd = process.cwd()) {
  ...
  try {
    ...
    console.error(`[pictorial live] ${err.message}`);
    process.exit(1);
  }
  ...
  if (path.resolve(cwd) !== path.resolve(appRoot)) {
    ...
    // to guess.
    if (!isDir(appRoot)) {
      console.error(`[pictorial live] resolved app root does not exist: ${appRoot} (stale roots manifest? re-run the live boot, or pass --target <path>)`);
      process.exit(1);
    }
    try {
      process.chdir(appRoot);
    } catch (err) {
      console.error(`[pictorial live] could not enter app root ${appRoot}: ${err.message}`);
      process.exit(1);
    }
  }
