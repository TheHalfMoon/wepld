...
You are updating the project constitution at `.agile/memory/constitution.md`. The active
constitution scaffold is resolved at command time from `constitution-template` through the Agile
...
1. Run `{SCRIPT}` from the repository root and parse `TEMPLATE_CONTENT` as the active template.
   ...
   - If it fails, stop and report the resolution error; do not continue with only one contributing
     template layer.
   - If `.agile/memory/constitution.md` exists, load it as the source of current project-specific
...
6. Write the completed constitution back to `.agile/memory/constitution.md` (overwrite).
...
Write only `.agile/memory/constitution.md`; do not create or modify template source files.
