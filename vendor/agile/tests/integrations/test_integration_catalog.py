...
class TestCatalogFetch:
    ...
    def _patch_urlopen(self, monkeypatch, catalog_data):
        ...
        import agile_cli.authentication.http as _auth_http
    ...
    def test_poisoned_cache_shape_is_dropped_and_refetched(self, tmp_path, monkeypatch):
        ...
        cache_dir = tmp_path / ".agile" / "integrations" / ".cache"
        data_files = [
    ...
    def test_fetch_rejects_oversized_catalog_response(
        ...
        import agile_cli.integrations.catalog as catalog_module
        ...
        from agile_cli.integrations.catalog import IntegrationCatalogEntry
    ...
    def test_fetch_wraps_non_utf8_catalog_response(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("USERPROFILE", str(tmp_path))
        monkeypatch.delenv("AGILE_INTEGRATION_CATALOG_URL", raising=False)
        (tmp_path / ".agile").mkdir(exist_ok=True)
        cat = IntegrationCatalog(tmp_path)
...
class TestIntegrationDescriptor:
    ...
    def test_valid_descriptor(self, tmp_path):
        ...
        assert desc.requires_agile_version == ">=0.6.0"
    ...
    def test_missing_agile_version(self, tmp_path):
        data = {**VALID_DESCRIPTOR, "requires": {}}
        p = self._write(tmp_path, data)
        with pytest.raises(IntegrationDescriptorError, match="requires.agile_version"):
            IntegrationDescriptor(p)
...
class TestIntegrationUpgrade:
    ...
    def test_upgrade_requires_agile_project(self, tmp_path):
        from typer.testing import CliRunner
        from agile_cli import app
        runner = CliRunner()
        old = os.getcwd()
    ...
    def test_upgrade_blocks_on_modified_files(self, tmp_path):
        ...
        manifest_path = project / ".agile" / "integrations" / "copilot.manifest.json"
...
class TestCatalogSourceManagement:
    ...
    def test_add_catalog_auto_derives_name_and_priority(self, tmp_path, monkeypatch):
        ...
        data = yaml.safe_load(
            (tmp_path / ".agile" / "integration-catalogs.yml").read_text(encoding="utf-8")
        )
    ...
    def test_add_catalog_rejects_invalid_url(self, tmp_path, monkeypatch):
        ...
        assert not (tmp_path / ".agile" / "integration-catalogs.yml").exists()
