...
describe('live reference authoring contract', () => {
  it('keeps setup guidance focused on routing live to its reference', () => {
    ...
    assert.doesNotMatch(skillSrc, /Use this same scripts directory for all Pictorial helper commands/);
  ...
  it('keeps the live prompt focused on the foreground poll loop', () => {
    const liveMd = readFileSync(join(ROOT, 'skill/reference/live.md'), 'utf-8');
    const manualAgentMd = readFileSync(join(ROOT, 'skill/agents/pictorial-manual-edit-applier.md'), 'utf-8');
    ...
    assert.doesNotMatch(liveMd, /PICTORIAL_LIVE_COPY_AGENT|mock/);
    ...
    assert.match(liveMd, /delegate source edits to `pictorial_manual_edit_applier`/);
    ...
    assert.match(manualAgentMd, /codex-name: pictorial_manual_edit_applier/);
  ...
  it('passes cssAuthoring into the LLM E2E agent instead of hard-coding scoped CSS', () => {
    ...
    assert.doesNotMatch(
      llmAgent,
      /with @scope \(\[data-pictorial-variant=/,
