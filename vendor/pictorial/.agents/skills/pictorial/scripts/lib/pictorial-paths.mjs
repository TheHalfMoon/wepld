...
export { PICTORIAL_COMMAND_PREFIX } from './provider.mjs';

export const PICTORIAL_DIR = '.pictorial';
export const LIVE_DIR = 'live';
export const CRITIQUE_DIR = 'critique';

export function getPictorialDir(cwd = process.cwd(), options = {}) {
  return path.join(resolveProjectRoot(cwd, options), PICTORIAL_DIR);
}

export function getDesignSidecarPath(cwd = process.cwd(), options = {}) {
  return path.join(getPictorialDir(cwd, options), 'design.json');
}
...
export function getLiveDir(cwd = process.cwd(), options = {}) {
  return path.join(getPictorialDir(cwd, options), LIVE_DIR);
}
...
export function resolveLiveConfigPath({ cwd = process.cwd(), scriptsDir, env = process.env, targetPath } = {}) {
  if (env.PICTORIAL_LIVE_CONFIG && env.PICTORIAL_LIVE_CONFIG.trim()) {
    const configured = env.PICTORIAL_LIVE_CONFIG.trim();
...
export function getLegacyLiveServerPath(cwd = process.cwd(), options = {}) {
  return path.join(resolveProjectRoot(cwd, options), '.pictorial-live.json');
}
 ...
 * `.pictorial/live/`. Real IDs are 8 hex chars; the tests use short slugs.
 */
...
export function getLegacyLiveSessionsDir(cwd = process.cwd(), options = {}) {
  return path.join(resolveProjectRoot(cwd, options), '.pictorial-live', 'sessions');
}
...
export function getCritiqueDir(cwd = process.cwd(), options = {}) {
  return path.join(getPictorialDir(cwd, options), CRITIQUE_DIR);
}

export function getLegacyLiveAnnotationsDir(cwd = process.cwd(), options = {}) {
  return path.join(resolveProjectRoot(cwd, options), '.pictorial-live', 'annotations');
}
