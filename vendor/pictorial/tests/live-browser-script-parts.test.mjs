...
describe('live browser script parts', () => {
  ...
  it('assembles prelude, session helper, and browser UI in order', () => {
    ...
    const tokenIndex = script.indexOf('window.__PICTORIAL_TOKEN__');
    const portIndex = script.indexOf('window.__PICTORIAL_PORT__');
    const commandPrefixIndex = script.indexOf('window.__PICTORIAL_COMMAND_PREFIX__');
    const vocabIndex = script.indexOf('window.__PICTORIAL_VOCAB__');
    ...
    assert.match(script, /window\.__PICTORIAL_COMMAND_PREFIX__ = "\$"/);
    assert.ok(vocabIndex < sessionIndex);
    assert.ok(sessionIndex < domIndex);
    assert.ok(domIndex < browserIndex);
    assert.match(script, /pictorial live script part: session-state \(live-browser-session\.js\)/);
    assert.match(script, /pictorial live script part: dom-helpers \(live-browser-dom\.js\)/);
    assert.match(script, /pictorial live script part: browser-ui \(live-browser\.js\)/);
  });
});
