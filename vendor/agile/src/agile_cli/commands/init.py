...
def _install_extension_during_init(project_path: Path, ext_spec: str, agile_version: str) -> str:
    """Install a single extension during ``agile init``.
    ...
    if parsed.scheme in ("http", "https"):
        try:
            manifest = install_extension_from_url(
                manager, project_path, ext_spec, agile_version
            )
    ...
    if ext_spec.startswith(("./", "../", "/", "~/", ".\\", "..\\")) or Path(ext_spec).is_absolute():
        ...
        manifest = manager.install_from_directory(source_path, agile_version)
        return f"{manifest.name} v{manifest.version} installed"
    ...
    if bundled_path is not None:
        ...
        manifest = manager.install_from_directory(bundled_path, agile_version)
        return f"{manifest.name} v{manifest.version} installed"
    ...
    if ext_info.get("bundled") and not ext_info.get("download_url"):
        ...
        raise ValueError(
            f"Extension '{resolved_id}' is bundled with agile but not found in the installed package. "
            f"Try reinstalling agile: {REINSTALL_COMMAND}"
        )
    ...
    try:
        manifest = manager.install_from_zip(zip_path, agile_version)
    finally:
...
def _shell_quote_arg(value: str) -> str:
    ...
    ``agile init``, so it is quoted for the host the same way
...
def ensure_constitution_from_template(
    ...
    memory_constitution = project_path / ".agile" / "memory" / "constitution.md"
...
def register(app: typer.Typer) -> None:
    ...
    def init(
        ...
        integration: str = typer.Option(
            None,
            "--integration",
            help="AI coding agent integration to use (e.g. --integration copilot). See 'agile check' for available integrations.",
        ),
        ...
        Project files are scaffolded from assets bundled inside the wepld-agile
        ...
        3. Install bundled Agile templates, scripts, workflow, and shared
           project infrastructure
        4. Set up coding agent integration commands and optional presets

        Examples:
            agile init my-project
            agile init my-project --integration claude
            agile init --ignore-agent-tools my-project
            agile init . --integration claude         # Initialize in current directory
            agile init .                     # Initialize in current directory (interactive integration selection)
            agile init --here --integration claude    # Alternative syntax for current directory
            agile init --here --integration codex --integration-options="--skills"
            agile init --here --integration codebuddy
            agile init --here --integration vibe      # Initialize with Mistral Vibe support
            agile init --here
            agile init --here --force  # Skip confirmation when current directory not empty
            agile init my-project --non-interactive  # CI/agent: defaults, no prompts
            agile init --here --force --non-interactive --integration claude  # Scripted init, no hang
            agile init my-project --integration claude   # Claude installs skills by default
            agile init --here --integration gemini
            agile init my-project --integration generic --integration-options="--commands-dir .myagent/commands/"  # Bring your own agent; requires --commands-dir
            agile init my-project --integration claude --preset healthcare-compliance  # With preset
            agile init my-project --integration copilot --extension git  # With bundled extension
            agile init my-project --extension git --extension selftest  # Multiple extensions
            agile init my-project --extension ./my-extensions/custom-ext  # Local path extension
            agile init my-project --extension https://example.com/extensions/my-ext.zip --trust-extension-urls  # URL extension (non-interactive)
        """
        ...
        if here:
            ...
            if existing_items:
                ...
                if force:
                    ...
                    # `echo y | agile init --here` keeps reaching the
        ...
        if selected_ai == "generic" and not integration_options:
            ...
            console.print(
                '[dim]Example: agile init my-project --integration generic --integration-options="--commands-dir .myagent/commands/"[/dim]'
            )
            raise typer.Exit(1)
        ...
        with Live(
            ...
            try:
                ...
                try:
                    bundled_wf = _locate_bundled_workflow("agile")
                    if bundled_wf:
                        from ..workflows.catalog import WorkflowRegistry
                        from ..workflows.engine import WorkflowDefinition

                        wf_registry = WorkflowRegistry(project_path)
                        if wf_registry.is_installed("agile"):
                            tracker.complete("workflow", "already installed")
                        else:
                            import shutil as _shutil

                            dest_wf = (
                                project_path / ".agile" / "workflows" / "agile"
                            )
                            ...
                            tracker.complete("workflow", "agile installed")
                    else:
                ...
                init_opts = {
                    ...
                    "agile_version": get_agile_version(),
                }
                        ...
                        if local_path.is_dir() and (local_path / "preset.yml").exists():
                            ...
                            if bundled_path:
                                ...
                                if not pack_info:
                                    ...
                                    console.print(
                                        f"[yellow]Warning:[/yellow] Preset '{preset}' is bundled with agile "
                                        f"but could not be found in the installed package."
                                    )
                                    console.print(
                                        "This usually means the agile installation is incomplete or corrupted."
                                    )
                                    console.print(
                    ...
                    for i, ext_spec in enumerate(extensions):
                        ...
                        try:
                            status_msg = _install_extension_during_init(
                                project_path, ext_spec, agile_ver
                            )
        ...
        if codex_skill_mode:
            steps_lines.append(
                f"{step_num}. Start Codex in this project directory; agile skills were installed to [cyan].agents/skills[/cyan]"
            )
            step_num += 1
        if zcode_skill_mode:
            steps_lines.append(
                f"{step_num}. Start ZCode in this project directory; agile skills were installed to [cyan].zcode/skills[/cyan]"
            )
            step_num += 1
        if claude_skill_mode:
            steps_lines.append(
                f"{step_num}. Start Claude in this project directory; agile skills were installed to [cyan].claude/skills[/cyan]"
            )
            step_num += 1
        if cursor_agent_skill_mode:
            steps_lines.append(
                f"{step_num}. Start Cursor Agent in this project directory; agile skills were installed to [cyan].cursor/skills[/cyan]"
            )
            step_num += 1
        if devin_skill_mode:
            steps_lines.append(
                f"{step_num}. Start Devin in this project directory; agile skills were installed to [cyan].devin/skills[/cyan]"
            )
            step_num += 1
        if zed_skill_mode:
            steps_lines.append(
                f"{step_num}. Start Zed in this project directory; agile skills were installed to [cyan].agents/skills[/cyan]"
            )
            step_num += 1
        if grok_skill_mode:
            steps_lines.append(
                f"{step_num}. Start Grok Build in this project directory; agile skills were installed to [cyan].grok/skills[/cyan]"
            )
            step_num += 1
        if bob_skill_mode:
            steps_lines.append(
                f"{step_num}. Start Bob in this project directory; agile skills were installed to [cyan].bob/skills[/cyan]"
            )
            step_num += 1
        ...
        steps_lines.append(
            f"   {step_num}.2 [cyan]{_display_cmd('agile')}[/] - Create baseline specification"
        )
        steps_lines.append(
