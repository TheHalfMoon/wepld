...
class TestGrokInitFlow:
    ...
    def test_integration_grok_creates_skills(self, tmp_path):
        ...
        assert (target / ".grok" / "skills" / "agile-plan" / "SKILL.md").exists()
        assert (target / ".grok" / "skills" / "agile-specify" / "SKILL.md").exists()
    ...
    def test_plan_skill_has_no_context_placeholder(self, tmp_path):
        ...
        plan_skill = target / ".grok" / "skills" / "agile-plan" / "SKILL.md"
...
class TestGrokNextSteps:
    ...
    def test_init_next_steps_show_grok_skill_guidance(self, tmp_path):
        """init --integration grok should guide users to .grok/skills and /agile-*."""
        from typer.testing import CliRunner
        from agile_cli import app
        ...
        assert "/agile.plan" not in result.output, (
            f"Should not show /agile.plan for Grok skills mode:\n{result.output}"
        )
...
class TestGrokInitOptions:
    ...
    def test_init_persists_ai_skills_for_grok(self, tmp_path, monkeypatch):
        """agile init --integration grok must persist ai_skills: true,
        ...
        assert "Executing: `/agile-plan`" in message, (
            "Hook rendering must produce /agile-plan for Grok without hint injection"
        )
