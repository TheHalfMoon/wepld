...
def _create_init_options(
    ...
    """Write a .agile/init-options.json file."""
    opts_dir = project_root / ".agile"
...
def _create_skills_dir(project_root: Path, ai: str = "claude") -> Path:
    ...
    # Match the logic in _get_skills_dir() from agile_cli
    from agile_cli import AGENT_CONFIG
...
def _create_extension_dir_with_aliases(
    ...
    the primary command name is ``agile.<ext_id>.<short_name>`` and each
    ...
    for short_name, aliases in command_specs:
        commands.append({
            "name": f"agile.{ext_id}.{short_name}",
...
class TestExtensionManagerGetSkillsDir:
    ...
    def test_returns_none_when_no_ai_skills(self, no_skills_project):
        ...
        from agile_cli import _get_skills_dir as resolve_skills_dir
...
class TestExtensionSkillRegistration:
    ...
    def test_skills_created_when_ai_skills_active(self, skills_project, extension_dir):
        ...
        assert "agile-test-ext-hello" in skill_dirs
        assert "agile-test-ext-world" in skill_dirs
    ...
    def test_skill_md_content_correct(self, skills_project, extension_dir):
        ...
        assert "author: github-agile" in content
    ...
    def test_skill_md_has_parseable_yaml(self, skills_project, extension_dir):
        ...
        assert parsed["name"] == "agile-test-ext-hello"
    ...
    def test_argument_hint_preserved_for_extension_command(
        ...
        manifest_data = {
            ...
            "provides": {
                "commands": [
                    {
                        "name": "agile.hint-ext.build-context",
        ...
        skill_file = skills_dir / "agile-hint-ext-build-context" / "SKILL.md"
        assert skill_file.exists()
    ...
    def test_argument_hint_not_added_for_non_claude_agent(self, project_dir, temp_dir):
        ...
        manifest_data = {
            ...
            "provides": {
                "commands": [
                    {
                        "name": "agile.hint-ext-kimi.build-context",
                        "file": "commands/build-context.md",
        ...
        skill_file = skills_dir / "agile-hint-ext-kimi-build-context" / "SKILL.md"
        assert skill_file.exists()
    ...
    def test_skill_md_unicode(self, skills_project, temp_dir):
        ...
        skill_file = skills_dir / "agile-uni-ext-hello" / "SKILL.md"
        content = skill_file.read_text(encoding="utf-8")
    ...
    def test_existing_skill_not_overwritten(self, skills_project, extension_dir):
        ...
        custom_dir = skills_dir / "agile-test-ext-hello"
        ...
        metadata = manager.registry.get(manifest.id)
        assert "agile-test-ext-world" in metadata["registered_skills"]
        # The pre-existing one should NOT be in registered_skills (it was skipped)
        assert "agile-test-ext-hello" not in metadata["registered_skills"]
    ...
    def test_dev_skill_symlink_refreshes_existing_cache(
        ...
        assert "agile-test-ext-hello" in written
    ...
    def test_codex_dev_skill_registration_preserves_unrelated_symlink(
        ...
        assert "agile-test-ext-hello" not in written
        assert skill_file.is_symlink()
    ...
    def test_dev_skill_registration_falls_back_to_copy_when_symlink_fails(
        ...
        monkeypatch.setattr(
            "agile_cli.extensions.os.symlink", raise_windows_symlink_error
        )
    ...
    def test_dev_skill_registration_falls_back_to_copy_when_relpath_fails(
        ...
        monkeypatch.setattr(
            "agile_cli.extensions.os.path.relpath", raise_relpath_error
        )
    ...
    def test_registered_skills_in_registry(self, skills_project, extension_dir):
        ...
        assert "agile-test-ext-hello" in metadata["registered_skills"]
        assert "agile-test-ext-world" in metadata["registered_skills"]
    ...
    def test_kimi_creates_skills_when_ai_skills_disabled(self, project_dir, temp_dir):
        ...
        assert "agile-test-ext-hello" in metadata["registered_skills"]
        assert "agile-test-ext-world" in metadata["registered_skills"]
        assert (skills_dir / "agile-test-ext-hello" / "SKILL.md").exists()
    ...
    def test_skill_registration_resolves_script_placeholders(self, project_dir, temp_dir):
        ...
        content = (skills_dir / "agile-scripted-ext-plan" / "SKILL.md").read_text()
    ...
    def test_skill_registration_uses_extension_local_script_paths(self, project_dir, temp_dir):
        ...
        content = (skills_dir / "agile-scripted-ext-check" / "SKILL.md").read_text()
        assert "{SCRIPT}" not in content
        assert "{ARGS}" not in content
        assert (
            '.agile/extensions/scripted-ext/scripts/bash/resolve-skill.sh "$ARGUMENTS"'
            in content
        )
        assert ".agile/extensions/scripted-ext/scripts/bash/ensure-skills.sh" in content
        assert ".agile/scripts/bash/resolve-skill.sh" not in content
        assert ".agile/scripts/bash/ensure-skills.sh" not in content
    ...
    def test_skill_registration_rewrites_extension_subdir_paths(self, project_dir, temp_dir):
        ...
        content = (skills_dir / "agile-path-ext-run" / "SKILL.md").read_text()
        assert ".agile/extensions/path-ext/agents/control/commander.md" in content
        assert ".agile/extensions/path-ext/knowledge-base/agent-scores.yaml" in content
        # extension's own templates/ dir must resolve under the extension,
        # not the project-level .agile/templates/
        assert ".agile/extensions/path-ext/templates/kill-report.md" in content
    ...
    def test_skill_registration_resolves_command_ref_tokens(
        ...
        manifest_data = {
            ...
            "provides": {
                "commands": [
                    {
                        "name": "agile.command-ref-ext.run",
                        "file": "commands/run.md",
                        "description": "Run command",
                    }
                ]
        ...
        (ext_dir / "commands" / "run.md").write_text(
            ...
            "description: Run command\n"
            "---\n\n"
            "Use __AGILE_COMMAND_PLAN__ before proceeding.\n"
        )

        manager = ExtensionManager(project_dir)
        manager.install_from_directory(ext_dir, "0.1.0", register_commands=False)

        content = (skills_dir / "agile-command-ref-ext-run" / "SKILL.md").read_text()
        assert "__AGILE_COMMAND_PLAN__" not in content
        assert expected_invocation in content

    def test_skill_registration_does_not_rewrite_literal_agile_text(
        self, project_dir, temp_dir
    ):
        """Auto-registered skills should leave literal agile text untouched."""
        _create_init_options(project_dir, ai="codex", ai_skills=True)
        skills_dir = _create_skills_dir(project_dir, ai="codex")
        ...
        manifest_data = {
            ...
            "provides": {
                "commands": [
                    {
                        "name": "agile.literal-ref-ext.run",
                        "file": "commands/run.md",
                        "description": "Run command",
                    }
                ]
        ...
        (ext_dir / "commands" / "run.md").write_text(
            ...
            "description: Run command\n"
            "---\n\n"
            "Literal slash form: /agile.foo.bar\n"
            "Literal skill form: /agile-plan\n"
            "Literal bare form: agile.foo.bar\n"
        )

        manager = ExtensionManager(project_dir)
        manager.install_from_directory(ext_dir, "0.1.0", register_commands=False)

        content = (skills_dir / "agile-literal-ref-ext-run" / "SKILL.md").read_text()
        assert "/agile.foo.bar" in content
        assert "/agile-plan" in content
        assert "agile.foo.bar" in content
        assert "/agile-foo-bar" not in content
    ...
    def test_missing_command_file_skipped(self, skills_project, temp_dir):
        ...
        manifest_data = {
            ...
            "provides": {
                "commands": [
                    {
                        "name": "agile.missing-cmd-ext.exists",
                        "file": "commands/exists.md",
                        "description": "Exists",
                    },
                    {
                        "name": "agile.missing-cmd-ext.ghost",
        ...
        metadata = manager.registry.get(manifest.id)
        assert "agile-missing-cmd-ext-exists" in metadata["registered_skills"]
        assert "agile-missing-cmd-ext-ghost" not in metadata["registered_skills"]
    ...
    def test_skills_registered_when_dir_missing(self, project_dir, temp_dir, ai):
        ...
        Regression test for https://github.com/TheHalfMoon/wepld/issues/2682:
        ...
        assert (skills_dir / "agile-early-ext-hello" / "SKILL.md").exists()
        assert (skills_dir / "agile-early-ext-world" / "SKILL.md").exists()

        # Registry should record them
        metadata = manager.registry.get(manifest.id)
        assert len(metadata["registered_skills"]) == 2
        assert "agile-early-ext-hello" in metadata["registered_skills"]
        assert "agile-early-ext-world" in metadata["registered_skills"]
    ...
    def test_commands_registered_when_claude_skills_dir_missing(self, project_dir, temp_dir):
        ...
        assert metadata["registered_commands"] == {
            "claude": [
                "agile.early-ext.hello",
                "agile.early-ext.world",
            ]
        }
        assert metadata["registered_skills"] == []

        skill_file = skills_dir / "agile-early-ext-hello" / "SKILL.md"
        assert skill_file.exists()
        content = skill_file.read_text(encoding="utf-8")
    ...
    def test_hermes_global_skills_dir_used_when_marker_is_recovered(
        ...
        assert (
            global_skills_dir / "agile-early-ext-hello" / "SKILL.md"
        ).exists()
        assert (
            global_skills_dir / "agile-early-ext-world" / "SKILL.md"
        ).exists()

        marker = project_dir / ".hermes" / "skills"
        assert marker.is_dir()
        assert list(marker.glob("agile-*/SKILL.md")) == []
    ...
    def test_one_failing_extension_does_not_abort_the_rest(
        ...
        """
        from agile_cli.extensions import CommandRegistrar
    ...
    def test_skill_registration_failure_preserves_registered_commands(
        ...
        assert metadata["registered_commands"] == {
            "claude": [
                "agile.skill-fail.hello",
                "agile.skill-fail.world",
            ]
        }
        assert metadata["registered_skills"] == []
    ...
    def test_rescaffold_toggle_command_to_skills_removes_stale_extension_command_file(
        ...
        cmd_file = agents_dir / "agile.toggle-ext.hello.agent.md"
        ...
        skill_file = skills_dir / "agile-toggle-ext-hello" / "SKILL.md"
    ...
    def test_toggle_command_to_skills_preserves_old_extension_command_on_skills_failure(
        ...
        agents_dir = project_dir / ".github" / "agents"
        cmd_file = agents_dir / "agile.toggle-fail-ext.hello.agent.md"
        assert cmd_file.exists(), "sanity: command mode should write .agent.md"
    ...
    def test_toggle_command_to_skills_empty_result_preserves_old_extension_command(
        ...
        agents_dir = project_dir / ".github" / "agents"
        hello_cmd_file = agents_dir / "agile.toggle-empty-ext.hello.agent.md"
        world_cmd_file = agents_dir / "agile.toggle-empty-ext.world.agent.md"
        ...
        assert set(registered_commands) == {
            "agile.toggle-empty-ext.hello", "agile.toggle-empty-ext.world",
        }, (
            "registered_commands must keep tracking copilot's still-live "
            "command files when nothing was actually replaced (#2948)"
        )
        skills_dir = project_dir / ".github" / "skills"
        assert not (skills_dir / "agile-toggle-empty-ext-hello").exists(), (
            "sanity: no skill should have been written when both sources "
            "were missing"
        )
        assert not (skills_dir / "agile-toggle-empty-ext-world").exists()
    ...
    def test_toggle_command_to_skills_partial_result_only_removes_replaced_extension_command(
        ...
        agents_dir = project_dir / ".github" / "agents"
        hello_cmd_file = agents_dir / "agile.toggle-partial-ext.hello.agent.md"
        world_cmd_file = agents_dir / "agile.toggle-partial-ext.world.agent.md"
        assert hello_cmd_file.exists() and world_cmd_file.exists(), (
            "sanity: command mode should have written both command files"
        )
        ...
        registered_commands = metadata.get("registered_commands", {}).get("copilot", [])
        assert registered_commands == ["agile.toggle-partial-ext.world"], (
            "registered_commands must stop tracking hello (retired) but "
            "keep tracking world (still live) (#2948)"
        )
        skills_dir = project_dir / ".github" / "skills"
        assert (skills_dir / "agile-toggle-partial-ext-hello" / "SKILL.md").exists()
        assert not (skills_dir / "agile-toggle-partial-ext-world").exists()
    ...
    def test_toggle_skills_to_command_empty_result_preserves_old_extension_skill(
        ...
        skills_dir = project_dir / ".github" / "skills"
        hello_skill_file = skills_dir / "agile-toggle-skill-empty-ext-hello" / "SKILL.md"
        world_skill_file = skills_dir / "agile-toggle-skill-empty-ext-world" / "SKILL.md"
        ...
        assert {
            "agile-toggle-skill-empty-ext-hello",
            "agile-toggle-skill-empty-ext-world",
    ...
    def test_toggle_skills_to_command_partial_result_only_removes_replaced_extension_skill(
        ...
        skills_dir = project_dir / ".github" / "skills"
        hello_skill_file = skills_dir / "agile-toggle-skill-partial-ext-hello" / "SKILL.md"
        world_skill_file = skills_dir / "agile-toggle-skill-partial-ext-world" / "SKILL.md"
        assert hello_skill_file.exists() and world_skill_file.exists(), (
            "sanity: skills mode should have written both SKILL.md mirrors"
        )
        ...
        registered_skills = metadata.get("registered_skills", [])
        assert "agile-toggle-skill-partial-ext-hello" not in registered_skills, (
            "hello must stop being tracked as a skill once its artifact "
            "has been unregistered (#2948)"
        )
        assert "agile-toggle-skill-partial-ext-world" in registered_skills, (
    ...
    def test_toggle_command_to_skills_retires_alias_group_when_primary_skill_lands(
        ...
        *primary* command name's skill. Without grouping by primary, the
        ...
        agents_dir = project_dir / ".github" / "agents"
        primary_cmd_file = agents_dir / "agile.alias-group-success-ext.hello.agent.md"
        alias_cmd_file = agents_dir / "agile.hello-alias.agent.md"
        ...
        assert set(metadata["registered_commands"].get("copilot", [])) == {
            "agile.alias-group-success-ext.hello", "agile.hello-alias",
        ...
        skill_file = (
            ...
            / "agile-alias-group-success-ext-hello" / "SKILL.md"
        )
    ...
    def test_toggle_command_to_skills_keeps_alias_group_when_primary_skill_missing(
        ...
        agents_dir = project_dir / ".github" / "agents"
        primary_cmd_file = agents_dir / "agile.alias-group-failure-ext.hello.agent.md"
        alias_cmd_file = agents_dir / "agile.hello-alias-2.agent.md"
        assert primary_cmd_file.exists() and alias_cmd_file.exists(), (
            "sanity: command mode should have written both the primary "
            "and alias command files"
        )
        ...
        assert set(metadata["registered_commands"].get("copilot", [])) == {
            "agile.alias-group-failure-ext.hello", "agile.hello-alias-2",
        }, (
        ...
        skill_file = (
            project_dir / ".github" / "skills"
            / "agile-alias-group-failure-ext-hello" / "SKILL.md"
        )
    ...
    def test_rescaffold_toggle_skills_to_command_removes_stale_extension_skill_file(
        ...
        skills_dir = project_dir / ".github" / "skills"
        skill_file = skills_dir / "agile-toggle-ext2-hello" / "SKILL.md"
        assert skill_file.exists(), "sanity: skills mode should write SKILL.md"
        ...
        agents_dir = project_dir / ".github" / "agents"
        cmd_file = agents_dir / "agile.toggle-ext2.hello.agent.md"
        assert cmd_file.exists(), "sanity: command mode should write .agent.md"
        ...
        registered_skills = metadata.get("registered_skills", [])
        assert "agile-toggle-ext2-hello" not in registered_skills, (
    ...
    def test_toggle_to_command_preserves_tracking_for_mirror_in_other_agent_dir(
        ...
        auggie_hello = auggie_skills_dir / "agile-multi-agent-ext-hello" / "SKILL.md"
        auggie_world = auggie_skills_dir / "agile-multi-agent-ext-world" / "SKILL.md"
        ...
        copilot_hello = copilot_skills_dir / "agile-multi-agent-ext-hello" / "SKILL.md"
        copilot_world = copilot_skills_dir / "agile-multi-agent-ext-world" / "SKILL.md"
        ...
        assert set(registered_skills) == {
            "agile-multi-agent-ext-hello",
            "agile-multi-agent-ext-world",
        }, (
    ...
    def test_remove_while_second_agent_still_in_skills_mode_cleans_up_first_agent_mirror(
        ...
        auggie_skills_dir = project_dir / ".augment" / "skills"
        auggie_hello = auggie_skills_dir / "agile-remove-multi-agent-ext-hello" / "SKILL.md"
        auggie_world = auggie_skills_dir / "agile-remove-multi-agent-ext-world" / "SKILL.md"
        assert auggie_hello.exists() and auggie_world.exists(), (
            "sanity: auggie's skills-mode activation should mirror both "
            "extension commands as SKILL.md files"
        )
        ...
        copilot_skills_dir = project_dir / ".github" / "skills"
        copilot_hello = copilot_skills_dir / "agile-remove-multi-agent-ext-hello" / "SKILL.md"
        copilot_world = copilot_skills_dir / "agile-remove-multi-agent-ext-world" / "SKILL.md"
        assert copilot_hello.exists() and copilot_world.exists(), (
            "sanity: copilot's skills-mode activation should also mirror "
            "both extension commands"
        )
    ...
    def test_unscoped_cleanup_preserves_unqualified_hermes_home_skill(
        ...
        monkeypatch.setattr(Path, "home", lambda: home)
        _create_init_options(project_dir, ai="hermes", ai_skills=True)
        skill_name = "agile-hermes-cleanup-hello"
        skill_dir = home / ".hermes" / "skills" / skill_name
        skill_dir.mkdir(parents=True)
        (skill_dir / "SKILL.md").write_text(
            "---\n"
            "name: agile-hermes-cleanup-hello\n"
            "metadata:\n"
    ...
    def test_unregister_agent_artifacts_stays_scoped_when_agent_dir_absent(
        ...
        auggie_skills_dir = project_dir / ".augment" / "skills"
        auggie_hello = auggie_skills_dir / "agile-scoped-unregister-ext-hello" / "SKILL.md"
        auggie_world = auggie_skills_dir / "agile-scoped-unregister-ext-world" / "SKILL.md"
        assert auggie_hello.exists() and auggie_world.exists(), (
            "sanity: auggie's skills-mode activation should mirror both "
            "extension commands as SKILL.md files"
        )
        ...
        copilot_skills_dir = project_dir / ".github" / "skills"
        copilot_hello = copilot_skills_dir / "agile-scoped-unregister-ext-hello" / "SKILL.md"
        copilot_world = copilot_skills_dir / "agile-scoped-unregister-ext-world" / "SKILL.md"
        assert copilot_hello.exists() and copilot_world.exists(), (
            "sanity: copilot's skills-mode activation should also mirror "
            "both extension commands"
        )
    ...
    def test_unregister_agent_artifacts_preserves_tracking_for_other_agent_mirror(
        ...
        auggie_skills_dir = project_dir / ".augment" / "skills"
        auggie_hello = auggie_skills_dir / "agile-dual-agent-unregister-ext-hello" / "SKILL.md"
        auggie_world = auggie_skills_dir / "agile-dual-agent-unregister-ext-world" / "SKILL.md"
        assert auggie_hello.exists() and auggie_world.exists(), (
            "sanity: auggie's skills-mode activation should mirror both "
            "extension commands as SKILL.md files"
        )
        ...
        copilot_skills_dir = project_dir / ".github" / "skills"
        copilot_hello = copilot_skills_dir / "agile-dual-agent-unregister-ext-hello" / "SKILL.md"
        copilot_world = copilot_skills_dir / "agile-dual-agent-unregister-ext-world" / "SKILL.md"
        assert copilot_hello.exists() and copilot_world.exists(), (
            "sanity: copilot's skills-mode activation should also mirror "
            "both extension commands"
        )
        ...
        assert "agile-dual-agent-unregister-ext-hello" in tracked, (
        ...
        assert "agile-dual-agent-unregister-ext-world" in tracked, (
            "registered_skills tracking must be preserved for names still "
            "owned by copilot's live mirror, even though they were removed "
            "from auggie's own (now nonexistent) directory — reconciling "
            "against only the just-cleaned agent's directory incorrectly "
    ...
    def test_unregister_agent_artifacts_preserves_dashed_description_mirror_tracking(
        ...
        skill_name = f"agile-{extension_id}-hello"
    ...
    def test_extension_owned_skill_names_rejects_symlinked_candidate_directory(
        ...
        external_skill_subdir = external_dir / "agile-sym-escape-ext-hello"
        external_skill_subdir.mkdir()
        (external_skill_subdir / "SKILL.md").write_text(
            "---\n"
            "name: agile-sym-escape-ext-hello\n"
        ...
        owned = manager._extension_owned_skill_names(
            ["agile-sym-escape-ext-hello"], "sym-escape-ext"
        )
    ...
    def test_unregister_extension_skills_fallback_does_not_follow_symlinked_dir(
        ...
        external_skill_subdir = external_dir / "agile-sym-escape-ext2-hello"
        external_skill_subdir.mkdir()
        external_skill_md = external_skill_subdir / "SKILL.md"
        external_skill_md.write_text(
            "---\n"
            "name: agile-sym-escape-ext2-hello\n"
            "description: external marker-matching skill\n"
            "metadata:\n"
        ...
        manager._unregister_extension_skills(
            ["agile-sym-escape-ext2-hello"], "sym-escape-ext2"
        )
    ...
    def test_unregister_extension_skills_fast_path_rejects_symlinked_explicit_dir(
        ...
        precious_file = external_dir / "precious_file.txt"
        precious_file.write_text("do not touch", encoding="utf-8")
        external_skill_subdir = external_dir / "agile-sym-escape-ext3-hello"
        external_skill_subdir.mkdir()
        (external_skill_subdir / "SKILL.md").write_text(
            "---\n"
            "name: agile-sym-escape-ext3-hello\n"
            "description: external marker-matching skill\n"
            "metadata:\n"
    ...
    def test_extension_owned_skill_names_rejects_symlinked_child_skill_dir(
        ...
        *parent* ``skills_dir`` for symlink escape, then resolved
        ...
        real_skill_dir = skills_dir / "agile-child-sym-real"
        real_skill_dir.mkdir()
        (real_skill_dir / "SKILL.md").write_text(
            "---\n"
            "name: agile-child-sym-real\n"
        ...
        alias_name = "agile-child-sym-alias"
    ...
    def test_unregister_extension_skills_explicit_dir_rejects_symlinked_child(
        ...
        alias_name = "agile-child-sym-alias2"
    ...
    def test_unregister_extension_skills_fallback_rejects_symlinked_child(
        ...
        precious_skill_dir = skills_dir / "agile-child-sym-precious3"
        precious_skill_dir.mkdir()
        precious_skill_md = precious_skill_dir / "SKILL.md"
        precious_skill_md.write_text(
            "---\n"
            "name: agile-child-sym-precious3\n"
            "description: precious skill\n"
            "metadata:\n"
        ...
        alias_name = "agile-child-sym-alias3"
        os.symlink(str(precious_skill_dir), str(skills_dir / alias_name))
...
class TestRegisterExtensionSkillsForceFlag:
    ...
    def test_force_false_skips_existing_skill(self, project_dir, temp_dir):
        ...
        skill_subdir = skills_dir / "agile-test-ext-hello"
...
class TestExtensionSkillUnregistration:
    ...
    def test_fallback_scan_rejects_symlinked_root_outside_project(
        ...
        from agile_cli import AGENT_CONFIG, DEFAULT_SKILLS_DIR

        skill_name = "agile-test-ext-hello"
        frontmatter = yaml.safe_dump(
            {
                "name": skill_name,
    ...
    def test_skills_removed_on_extension_remove(self, skills_project, extension_dir):
        ...
        # Verify skills exist
        assert (skills_dir / "agile-test-ext-hello" / "SKILL.md").exists()
        assert (skills_dir / "agile-test-ext-world" / "SKILL.md").exists()
        ...
        assert not (skills_dir / "agile-test-ext-hello").exists()
        assert not (skills_dir / "agile-test-ext-world").exists()
    ...
    def test_skills_removed_when_description_contains_dashes(
        ...
        skill_dir = skills_dir / "agile-dash-ext-hello"
    ...
    def test_skills_removed_with_dashes_via_fallback_scan(
        ...
        (project_dir / ".agile" / "init-options.json").unlink()
    ...
    def test_remove_handles_already_deleted_skills(self, skills_project, extension_dir):
        ...
        shutil.rmtree(skills_dir / "agile-test-ext-hello")
        shutil.rmtree(skills_dir / "agile-test-ext-world")
...
class TestExtensionSkillEdgeCases:
    ...
    def test_command_without_frontmatter(self, skills_project, temp_dir):
        ...
        skill_file = skills_dir / "agile-nofm-ext-plain" / "SKILL.md"
        assert skill_file.exists()
        content = skill_file.read_text()
        assert "name: agile-nofm-ext-plain" in content
        # Fallback description when no frontmatter description
        assert "Extension command: agile.nofm-ext.plain" in content
    ...
    def test_multiple_extensions_independent_skills(self, skills_project, temp_dir):
        ...
        assert (skills_dir / "agile-ext-a-hello" / "SKILL.md").exists()
        assert (skills_dir / "agile-ext-b-hello" / "SKILL.md").exists()

        # Remove ext-a
        manager.remove("ext-a", keep_config=False)

        # ext-a skills gone, ext-b skills preserved
        assert not (skills_dir / "agile-ext-a-hello").exists()
        assert (skills_dir / "agile-ext-b-hello" / "SKILL.md").exists()
    ...
    def test_malformed_frontmatter_handled(self, skills_project, temp_dir):
        ...
        skill_file = skills_dir / "agile-badfm-ext-broken" / "SKILL.md"
        assert skill_file.exists()
        content = skill_file.read_text()
        # Fallback description since frontmatter was invalid
        assert "Extension command: agile.badfm-ext.broken" in content
    ...
    def test_remove_cleans_up_when_init_options_deleted(self, skills_project, extension_dir):
        ...
        init_opts = project_dir / ".agile" / "init-options.json"
        init_opts.unlink()
