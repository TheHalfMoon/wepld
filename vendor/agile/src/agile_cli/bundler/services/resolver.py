...
(SC-002 transparency). Resolution also enforces the Agile version gate
...
def resolve_install_plan(
    ...
    *integration_explicit* signals that ``active_integration`` came from an
    ...
    # FR-016: Agile version gate — refuse incompatible installs.
    if enforce_version and manifest.requires.agile_version:
        if not satisfies(agile_version, manifest.requires.agile_version):
            raise BundlerError(
                f"Bundle '{manifest.bundle.id}' requires Agile "
                f"{manifest.requires.agile_version}, but this project uses "
                f"{agile_version}. Update Agile or choose a compatible bundle."
            )
    ...
    if manifest.integration is not None:
        ...
        if active_integration is None and not integration_explicit:
            raise BundlerError(
                ...
                "(missing or unreadable .agile/integration.json). Re-run with "
