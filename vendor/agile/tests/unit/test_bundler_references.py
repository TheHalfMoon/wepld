...
from agile_cli.bundler.services.references import make_reference_checker
...
def test_builtin_step_type_resolves(tmp_path: Path):
    ...
    Agile ships 11 step types as built-ins registered in ``STEP_REGISTRY``
    ...
    types installed under ``.agile/workflows/steps/`` — so every built-in step
    type was reported as an unresolved reference.
    """
    from agile_cli.workflows import BUILTIN_STEP_TYPES
...
def test_community_step_is_not_treated_as_bundled(tmp_path: Path):
    ...
    """
    from agile_cli.workflows import (
        BUILTIN_STEP_TYPES,
        STEP_REGISTRY,
        _register_step,
    )
