"""Contract test for the `agile bundle` CLI surface (Typer integration).
...
from agile_cli.bundler.services.packager import build_bundle
...
def _configure_markup_catalog(project: Path, **overrides: object) -> dict:
    entry = catalog_entry_dict(
        ...
        requires={"agile_version": "[italic]>=0.1.0[/italic]"},
        **overrides,
    )
    ...
    (project / ".agile" / "bundle-catalogs.yml").write_text(
...
def test_commands_outside_project_fail_with_guidance(tmp_path: Path, monkeypatch):
    monkeypatch.chdir(tmp_path)  # no .agile/
    result = runner.invoke(app, ["bundle", "list"])
    assert result.exit_code == 1
    assert "Agile project" in result.output
...
def test_remove_reports_clean_error_when_primitive_raises_raw_exception(
    ...
    `agile bundle remove` -- the command only catches BundlerError, so
    ...
    from agile_cli.bundler.models.manifest import BundleManifest
    from agile_cli.bundler.models.records import load_records
    from agile_cli.bundler.services.adapters import DefaultPrimitiveInstaller
    from agile_cli.bundler.services.installer import install_bundle
    from agile_cli.bundler.services.resolver import resolve_install_plan
    from tests.bundler_helpers import FakeInstaller

    manifest = BundleManifest.from_dict(valid_manifest_dict())
    plan = resolve_install_plan(
        manifest, agile_version="0.11.2", active_integration="copilot"
    )
...
def test_fail_writes_error_to_stderr_not_stdout(capsys):
    ...
    from agile_cli.commands.bundle import _fail
...
def test_search_works_without_a_project(tmp_path: Path, monkeypatch):
    ...
    # not require a Agile project (matches README/quickstart examples).
    monkeypatch.chdir(tmp_path)  # no .agile/
...
def test_info_unknown_bundle_without_project_reports_not_found(tmp_path: Path, monkeypatch):
    ...
    assert result.exit_code == 1
    assert "Agile project" not in result.output
...
def test_validate_escapes_manifest_markup_in_errors(project: Path):
    ...
    data["requires"] = {"agile_version": ">=1.0[/bold]"}
    (project / "bundle.yml").write_text(yaml.safe_dump(data), encoding="utf-8")
...
def test_list_escapes_markup_in_records(project: Path):
    ...
    """
    (project / ".agile" / "bundle-records.json").write_text(
        json.dumps(
            {
                "schema_version": "1.0",
                "bundles": [
...
def _mock_manifest_download(monkeypatch, source_path: Path) -> None:
    ...
    """
    from agile_cli.commands.bundle import _local_manifest_source

    monkeypatch.setattr(
        "agile_cli.commands.bundle._download_manifest",
...
def test_info_escapes_catalog_markup(project: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agile_cli.commands.bundle._manifest_component_view",
        lambda manifest: [
            {
                "kind": "extensions",
    ...
    monkeypatch.setattr(
        "agile_cli.commands.bundle._bundle_overlaps",
...
def test_update_refuses_discovery_only_source(project: Path):
    ...
    from agile_cli.bundler.models.manifest import ComponentRef
    from agile_cli.bundler.models.records import (
...
def test_bundle_info_resolves_github_browser_release_url(project: Path):
    ...
    with patch("agile_cli.authentication.http.open_url", side_effect=fake_open_url):
...
def test_bundle_info_resolves_ghes_browser_release_url(project: Path):
    ...
    with patch("agile_cli.authentication.http.open_url", side_effect=fake_open_url), \
         patch("agile_cli.authentication.http.github_provider_hosts", return_value=(ghes_host,)):
        result = runner.invoke(app, ["bundle", "info", "demo-bundle", "--json"])
...
def test_bundle_download_rejects_oversized_response(project: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agile_cli.commands.bundle.MAX_DOWNLOAD_BYTES", 100
    )
