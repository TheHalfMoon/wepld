"""Persistence for the project-scoped catalog config (``.agile/bundle-catalogs.yml``).
...
def _config_path(project_root: Path) -> Path:
    return Path(project_root) / ".agile" / CONFIG_FILENAME


def _read(project_root: Path) -> list[dict]:
    # Confine the read (parity with the write path's within= guard): refuse to
    # follow a symlinked or traversal-escaping .agile that resolves outside
    # project_root.
    ...
    if schema_version is not None and (
        ...
        raise BundlerError(
            ...
            f"'{str(schema_version).strip()}' at {path}; this Agile "
...
def add_source(
    ...
    *,
    ...
    if parsed.scheme.lower() in {"http", "https"}:
        # Mirror agile_cli.catalogs._validate_catalog_url (#3209/#3210):
