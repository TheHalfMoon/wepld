...
# Agile fails fast instead of being parsed under the wrong assumptions.
...
class CatalogEntry:
    ...
    def from_dict(cls, data: Any) -> "CatalogEntry":
        ...
        return cls(
            ...
            requires_agile_version=str(requires.get("agile_version", "")).strip(),
            sha256=(
                None
    ...
    def with_provenance(self, source: CatalogSource) -> "CatalogEntry":
        return CatalogEntry(
            ...
            requires_agile_version=self.requires_agile_version,
            sha256=self.sha256,
...
def load_source_stack(project_root: Path, user_config_dir: Path | None = None) -> list[CatalogSource]:
    ...
    # Confine the project-scoped read: refuse a symlinked .agile/ that
    # resolves outside the project root (consistent with other guarded reads).
    project_config = Path(project_root) / ".agile" / CONFIG_FILENAME
...
def _merge_config(by_id: dict[str, CatalogSource], config_path: Path, scope: Scope) -> None:
    ...
    # newer/incompatible Agile was silently parsed under v1 assumptions on
    ...
    if schema_version is not None and (
        ...
        raise BundlerError(
            ...
            f"'{str(schema_version).strip()}' at {config_path}; this Agile "
