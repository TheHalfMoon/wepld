...
from agile_cli.integrations.{package_name} import {class_name}
from agile_cli.integrations.base import {template.base_class}
...
def _is_agile_repo_root(project_root: Path) -> bool:
    """Return True when `project_root` looks like the Agile repository root."""
    return all(
        (
            (project_root / "pyproject.toml").is_file(),
            (project_root / "src" / "agile_cli" / "__init__.py").is_file(),
            (project_root / "src" / "agile_cli" / "integrations").is_dir(),
            (
                project_root / "src" / "agile_cli" / "integrations" / "__init__.py"
            ).is_file(),
...
def scaffold_integration(
    ...
    integrations_root = project_root / "src" / "agile_cli" / "integrations"
    tests_root = project_root / "tests" / "integrations"
    if not _is_agile_repo_root(project_root):
        raise ValueError("Run this command from the Agile repository root.")
    ...
    next_steps = (
        f"Register {class_name} in src/agile_cli/integrations/__init__.py.",
