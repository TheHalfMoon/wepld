...
describe('live source-search — search roots', () => {
  ...
  it('never treats pictorial state as project source', () => {
    assert.ok(NEVER_SOURCE_DIRS.includes('.pictorial'));
...
describe('live source-search — findSourceFile', () => {
  let tmp;
  beforeEach(() => { tmp = mkdtempSync(join(tmpdir(), 'pictorial-src-search-')); });
  ...
  it('skips .pictorial artifacts that carry the same marker', () => {
    // The bug this guards: staged revision artifacts hold the marker too, and
    // dot-directories sort before letters in the `.` walk.
    write('.pictorial/live/artifacts/abc-r1.html', '<!-- pictorial-variants-start abc -->\n');
    const real = write('views/home.html', '<!-- pictorial-variants-start abc -->\n');
    assert.equal(findSourceFile({ query: 'pictorial-variants-start abc', cwd: tmp, extensions: EXTS }), real);
  });
