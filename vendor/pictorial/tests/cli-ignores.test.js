...
describe('pictorial ignores CLI', () => {
  let root;
  ...
  function readConfig(name = 'config.json') {
    return JSON.parse(readFileSync(join(root, '.pictorial', name), 'utf-8'));
  }
  ...
  test('rejects broad wildcard value ignores', () => {
    ...
    expect(existsSync(join(root, '.pictorial', 'config.json'))).toBe(false);
  });

  test('removes an existing broad wildcard value ignore', () => {
    mkdirSync(join(root, '.pictorial'), { recursive: true });
    writeFileSync(join(root, '.pictorial', 'config.json'), JSON.stringify({
      detector: {
