...
Presets are stackable, priority-ordered collections of template and command overrides for Agile. They let you customize both the artifacts produced by the Spec-Driven Development workflow (specs, plans, tasks, checklists, constitutions) and the commands that guide the LLM in creating them — without forking or modifying core files.

## How It Works

When Agile needs a template (e.g. `spec-template`), it walks a resolution stack:

1. `.agile/templates/overrides/` — project-local one-off overrides
2. `.agile/presets/<preset-id>/templates/` — installed presets (sorted by priority)
...
Template resolution happens **at runtime** — although preset files are copied into `.agile/presets/<id>/` during installation, Agile walks the resolution stack on every template lookup rather than merging templates into a single location.

`constitution-template` follows the same runtime model. Project initialization seeds
`.agile/memory/constitution.md` once so downstream commands always have a constitution to read.
...
Presets are discovered through catalogs. By default, Agile uses the official and community catalogs:
...
|----------|-------------|---------|
| `AGILE_PRESET_CATALOG_URL` | Override the full catalog stack with a single URL (replaces all defaults) | Built-in default stack |
...
|------|-------|-------------|
| `.agile/preset-catalogs.yml` | Project | Custom catalog stack for this project |
| `~/.agile/preset-catalogs.yml` | User | Custom catalog stack for all projects |
