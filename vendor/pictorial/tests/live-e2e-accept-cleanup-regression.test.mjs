 ...
 * exposes the accepted element under stale data-pictorial runtime wrappers.
 *
 * Run with:
...
describe('live-e2e accept cleanup regression', () => {
  it('cleans accepted LLM DOM wrappers after carbonize cleanup', async (t) => {
    ...
    const llmConfig = resolveLlmAgentConfig({
      provider: process.env.PICTORIAL_E2E_LLM_PROVIDER || 'deepseek',
    });
    ...
    try {
      ...
      assert.doesNotMatch(finalSource, /data-pictorial-variants="/, 'source variants wrapper removed');
      assert.doesNotMatch(finalSource, /pictorial-variants-start/, 'source variants markers removed');
      assert.doesNotMatch(finalSource, /pictorial-carbonize-start/, 'source carbonize markers removed');
      assert.doesNotMatch(finalSource, /data-pictorial-variant="/, 'source variant wrapper removed');
...
async function collectAcceptedDomDiagnostic(page, selector) {
  return page.evaluate((sel) => {
    const matches = [...document.querySelectorAll(sel)];
    const clean = matches.filter((el) => !el.closest('[data-pictorial-variants],[data-pictorial-variant],[data-pictorial-carbonize]'));
    const stale = matches.filter((el) => el.closest('[data-pictorial-variants],[data-pictorial-variant],[data-pictorial-carbonize]'));
    const local = {};
    for (const key of [
      'pictorial-live-session',
      'pictorial-live-session-handled',
      'pictorial-live-session-scroll',
    ]) {
    ...
    return {
      ...
      remainingVariantsWrapperCount: document.querySelectorAll('[data-pictorial-variants]').length,
      remainingVariantWrapperCount: document.querySelectorAll('[data-pictorial-variant]').length,
      liveBarText: document.querySelector('#pictorial-live-bar')?.textContent || '',
      localStorage: local,
      heroHtml: matches.map((el) => el.outerHTML.slice(0, 500)),
      staleAncestorHtml: stale.map((el) => {
        const ancestor = el.closest('[data-pictorial-variants],[data-pictorial-variant]');
...
function isSourceClean(source) {
  return !source.includes('data-pictorial-variants=')
    && !source.includes('pictorial-variants-start')
    && !source.includes('pictorial-carbonize-start')
    && !source.includes('data-pictorial-variant=');
}
