...
export const SUITES = {
  core: {
    ...
    triggers: [
      ...
      /^skill\/(SKILL\.src\.md|agents\/|reference\/|scripts\/(cleanup-deprecated|concept-seed|context|context-signals|critique-storage|design-parser|doctor|hook|pictorial-paths|is-generated|lib\/(artifact-schema|composition-catalog|concept-catalog|provider|staleness|staleness-deep|staleness-notice|surface-briefs|target-slug|template-extensions)|pin|surface-brief))/,
      /^README(\.npm)?\.md$/,
      /^cli\/bin\//,
    ],
    commands: [
      {
        ...
        files: [
          ...
          'tests/lib/pictorial-config.test.js',
      ...
      },
      {
        ...
        files: [
          ...
          'tests/hook.test.mjs',
          'tests/pictorial-paths.test.mjs',
  ...
  'cli-remote-e2e': {
    description: 'Remote CLI install/update smoke tests against github.com/TheHalfMoon/wepld.',
    optIn: true,
    triggers: [
      ...COMMON_INFRA_PATTERNS,
    ...
    commands: [
      {
        runner: 'bun',
        env: { PICTORIAL_CLI_REMOTE_E2E: '1' },
        files: ['tests/skills-cli.test.js'],
      },
    ],
  },
