 ...
 * the sibling pictorial-evals harness; this suite measures skill protocol.
 *
 ...
 *
 * .env is loaded from the repo root (copied from pictorial-evals). Tests
 ...
 * Override with PICTORIAL_SKILL_BEHAVIOR_EFFORT=xhigh.
 */
export function getProviderOptions(modelId) {
  ...
  if (provider === 'openai') {
    const effort = process.env.PICTORIAL_SKILL_BEHAVIOR_EFFORT || 'high';
...
export function resolveModelList() {
  const override = process.env.PICTORIAL_SKILL_BEHAVIOR_MODELS;
