...
function stageFixture(name) {
  ...
  const tmp = mkdtempSync(join(tmpdir(), 'pictorial-fixture-'));
  cpSync(join(fixtureRoot, 'files'), tmp, { recursive: true });
  writeFileSync(join(tmp, '.gitignore'), gitignore);
  mkdirSync(join(tmp, '.pictorial', 'live'), { recursive: true });
  writeFileSync(join(tmp, '.pictorial', 'live', 'config.json'), JSON.stringify(fixture.config));
    ...
    it('live-inject --port adds the script tag to every config file', () => {
      ...
      try {
        ...
        const ignored = execFileSync('git', [
          ...
          '.pictorial/live/sessions/example.jsonl',
          '.pictorial/live/previews/example/v1.html',
          '.pictorial/live/artifacts/example-r1.jsx',
          '.pictorial/live/accept-receipts/example.json',
          '.pictorial/live/locks/example.lock',
          '.pictorial/live/deferred-svelte-component-accepts.json',
          'src/lib/pictorial/PictorialLiveRoot.svelte',
          'src/lib/pictorial/__runtime.js',
          'src/lib/pictorial/a4ac4e74/v3.svelte',
        ], { cwd: tmp, encoding: 'utf-8' });
        assert.match(ignored, /\.pictorial\/live\/server\.json/);
        assert.match(ignored, /\.pictorial\/live\/sessions\/example\.jsonl/);
        assert.match(ignored, /\.pictorial\/live\/previews\/example\/v1\.html/);
        assert.match(ignored, /\.pictorial\/live\/artifacts\/example-r1\.jsx/);
        assert.match(ignored, /\.pictorial\/live\/accept-receipts\/example\.json/);
        assert.match(ignored, /\.pictorial\/live\/locks\/example\.lock/);
        assert.match(ignored, /\.pictorial\/live\/deferred-svelte-component-accepts\.json/);
        assert.match(ignored, /src\/lib\/pictorial\/PictorialLiveRoot\.svelte/);
        assert.match(ignored, /src\/lib\/pictorial\/__runtime\.js/);
        assert.match(ignored, /src\/lib\/pictorial\/a4ac4e74\/v3\.svelte/);
        if (result.adapter === 'sveltekit') {
          ...
          const root = readFileSync(join(tmp, 'src/lib/pictorial/PictorialLiveRoot.svelte'), 'utf-8');
          assert.match(layout, /pictorial-live-svelte-start/, 'SvelteKit layout got the adapter marker');
          assert.match(layout, /PictorialLiveRoot/, 'SvelteKit layout renders the adapter host');
          assert.doesNotMatch(appHtml, /pictorial-live-start/, 'SvelteKit app.html must remain untouched');
        ...
        if (result.adapter === 'nuxt') {
          ...
          assert.match(body, /pictorial-live-nuxt-plugin/);
        ...
        if (result.adapter === 'tanstack-start') {
          ...
          assert.match(rootDoc, /pictorial-live-tanstack-start/, 'root document got the adapter marker');
          assert.match(rootDoc, /<PictorialLiveRoot \/>/, 'root document renders the mount component');
          assert.doesNotMatch(rootDoc, /pictorial-live-start/, 'root document must not get the raw script block');
        ...
        for (const r of result.results) {
          ...
          assert.match(body, /pictorial-live-start/);
          assert.match(body, /localhost:9999\/live\.js/);
        }
      } finally {
        rmSync(tmp, { recursive: true, force: true });
    ...
    it('live-inject --remove strips the script tag cleanly', () => {
      ...
      try {
        ...
        if (result.adapter === 'sveltekit') {
          const layout = readFileSync(join(tmp, 'src/routes/+layout.svelte'), 'utf-8');
          const appHtml = readFileSync(join(tmp, 'src/app.html'), 'utf-8');
          assert.doesNotMatch(layout, /PictorialLiveRoot/);
          assert.doesNotMatch(layout, /pictorial-live-svelte-start/);
          assert.doesNotMatch(appHtml, /pictorial-live-start/);
          assert.equal(existsSync(join(tmp, 'src/lib/pictorial/PictorialLiveRoot.svelte')), false);
          return;
        }
        if (result.adapter === 'nuxt') {
        ...
        if (result.adapter === 'tanstack-start') {
          const adapterResult = result.results[0];
          const rootDoc = readFileSync(join(tmp, adapterResult.file), 'utf-8');
          assert.doesNotMatch(rootDoc, /PictorialLiveRoot/);
          assert.doesNotMatch(rootDoc, /pictorial-live-tanstack-start/);
          assert.equal(existsSync(join(tmp, adapterResult.componentFile)), false, 'TanStack mount component was removed');
          return;
        }
        for (const r of result.results) {
          const body = readFileSync(join(tmp, r.file), 'utf-8');
          assert.doesNotMatch(body, /pictorial-live-start/);
