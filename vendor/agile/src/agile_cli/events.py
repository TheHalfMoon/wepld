...
# An older installed agile_cli.events (uvx-init plus a stale global
# install) would otherwise run unconfined script tokens.
EVENT_SCRIPT_PATH_CONFINEMENT = True

EVENTS_DISPATCHER_DIR = Path(".agile")
...
# .as_posix()). On Windows, str(Path(".agile")/"events.py") yields
# ".agile\\events.py", which never matched a manifest key, so the shared-
...
YAML_OVERRIDE_FILENAME = Path(".agile") / "integration-events.yml"

_AGILE_MARKER = "__agile_event__"
...
"""Agile CLI Event Dispatcher — dispatches agent runtime events.

Generated by: agile integration install/upgrade
Do not edit manually.

Self-contained: it prefers `agile_cli` when the package is importable
(durable pip/pipx/uv-tool install) and falls back to an inline stdlib-only
resolver when Agile is not installed at runtime — e.g. a one-time `uvx`
init whose environment is discarded after `agile init` finishes (R1). In
both cases it resolves the event's command template and runs its script
directly, without requiring a persistent `agile` executable on PATH.
"""
import json
import os
import re
...
def _find_command_template(command_name, project_root):
    ...
    exts_dir = project_root / ".agile" / "extensions"
    disabled_ids = set()
    ...
    stem = command_name.replace("agile.", "").replace("spec.", "")
    ...
    core = project_root / ".agile" / "templates" / "commands"
    if core.is_dir():
...
def _script_variant(project_root):
    ...
    init_opts = project_root / ".agile" / "init-options.json"
    try:
...
def _resolve_argv(template_path, project_root, ext_id):
    ...
    base = (project_root / ".agile" / "extensions" / ext_id) if ext_id else (project_root / ".agile")
    try:
...
def _run_inline(command_name, payload, project_root, timeout, envelope="plain", native_event=""):
    """Resolve and run the event command with stdlib only (no agile_cli)."""
...
def main():
    ...
    # Preferred path: agile_cli is importable (durable install) — delegate to
    ...
    try:
        ...
        if _confine_ok is not True:
            raise ImportError("agile_cli.events lacks script path confinement")
        sys.exit(
            resolve_and_run_event_command(
...
function canImportSpecifyCli(py: string): boolean {{
  // R2: a project-local venv commonly lacks Agile (installed globally or
  // via uv tool). Probe the interpreter can import agile_cli before
  // selecting it, so an unrelated venv doesn't shadow the PATH fallback.
  try {{
    execFileSync(py, ['-c', 'import agile_cli'], {{
...
function resolveDispatcher(directory: string): void {{
  DISPATCHER = path.join(directory, '.agile', 'events.py');
  // Prefer a project-local venv interpreter that can import agile_cli (R2),
...
def _find_command_template(command_name: str, project_root: Path) -> tuple[Path | None, str | None]:
    ...
    #    differs from the command name (e.g. ``agile.selftest.extension`` →
...
def _resolve_event_command_argv(
    ...
    # .agile/ (core) or .agile/extensions/<id>/ (extension). All variants
    ...
    if ext_id:
        base = project_root / ".agile" / "extensions" / ext_id
    else:
        base = project_root / ".agile"
...
def resolve_events(
    ...
    4. User YAML override (``.agile/integration-events.yml``) — replaces the
...
def collect_extension_events(project_root: Path) -> ResolvedEvents:
    ...
    ``agile.my-ext.boot``) is reflected — otherwise refresh would emit the
...
def _shell_quote(value: str, target_os: str) -> str:
    ...
    tokens — ``python3``, ``agile.ext.cmd`` — pass through bare, so a
...
def install_integration_events(
    ...
    # event integration doesn't orphan .agile/events.py permanently (the
    ...
    if fmt == "ts-plugin":
        ...
        plugin_rel = ".opencode/plugin/agile-events.ts"
        ...
        # Copilot dedicated .github/hooks/agile.json. Each handler becomes
        ...
        for ev, handlers in filtered.items():
            ...
            for cfg in handlers:
                ...
                lines.append('agile_marker = true')
                lines.append('')
        ...
        # segment (e.g. agile.a.validate vs agile.b.validate).
        lines: list[str] = []
        ...
        for ev, handlers in filtered.items():
            ...
            for cfg in handlers:
                ...
                base_name = f"agile-{native}-{command_stem}"
                hook_name = base_name
                suffix = 2
...
def _other_event_integrations_reference_dispatcher(
    ...
    references the shared ``.agile/events.py`` dispatcher (#10).
...
def _cleanup_shared_dispatcher(
    ...
    doesn't orphan ``.agile/events.py`` permanently (S3).
    """
...
def remove_integration_events(
    ...
    The shared ``.agile/events.py`` dispatcher is deleted only when no other
...
def events_stale_exclusions(integration_key: str) -> set[str]:
    ...
    if integration_key == "opencode":
        exclusions.add(".opencode/plugin/agile-events.ts")
...
def refresh_integration_events(project_root: Path) -> None:
    ...
    ``agile init`` flow is inert and disabled/removed extension events stay
    ...
    for ev, handlers in filtered_events.items():
        ...
        if native.startswith("tool.execute."):
            ...
            plugin_returns.append(
                ...
                f"      output.parts.push({{ id: base + \".agile\" + Math.random().toString(36).slice(2, 8), sessionID: input.sessionID, messageID: output.message.id, type: \"text\", text: ctx, synthetic: true }});\n"
                f"    }},"
            )
        else:
            event_entries.append(
...
def _merge_opencode_plugin_ref(config_path: Path, ref: str) -> bool:
    """Merge the agile-events plugin ref into opencode.json.
...
def _remove_opencode_entries(config_path: Path) -> bool:
    """Remove the agile-events plugin ref from opencode.json (#23).
    ...
    if isinstance(plugins, list):
        ref = "./.opencode/plugin/agile-events.ts"
...
def _merge_vibe_toml_fragment(dst: Path, fragment: str) -> bool:
    ...
    # Match [[hooks]] ... agile_marker = true (with any content in between)
    existing = re.sub(
        r'\[\[hooks\]\]\n(?:(?!\[\[hooks\]\]).)*?agile_marker = true\n*',
        "",
        existing,
        flags=re.DOTALL,
    )
...
def _merge_copilot_json(dst: Path, new_hooks: dict[str, list]) -> bool:
    ...
    A pre-existing user-authored ``.github/hooks/agile.json`` is merged
...
def _remove_copilot_entries(dst: Path) -> bool:
    ...
    if cleaned:
        ...
        existing.pop("hooks", None)
    # Dedicated Agile file: delete when only the (Agile-invented)
...
def _ensure_safe_destination(dst: Path) -> None:
    ...
    project — e.g. a symlinked ``.claude`` or ``.agile`` directory pointing
...
def _remove_json_entries(dst: Path) -> bool:
    ...
    Returns True if the file was deleted (Agile created it and no user
    ...
    # Agile-created Cursor file retains {"version": 1} after all owned
...
def _has_marker(entry: Any) -> bool:
    ...
    if not isinstance(entry, dict):
        return False
    if entry.get(_AGILE_MARKER, False) is True:
        return True
    inner = entry.get("hooks")
    if isinstance(inner, list):
