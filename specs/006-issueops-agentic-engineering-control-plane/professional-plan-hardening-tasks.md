...
Every credential/recovery hardening task consumes the complete mandatory CredentialCapability and ExecutionEnvelope contracts in `contracts/runtime-execution-fabric.md` and the full `contracts/runtime-distributed-safety-addendum.md`. Credential tests include exact delivery-mode/envelope binding and direct-exposure exception qualification/authority/expiry evidence (missing, forged, stale or broker-required cases refuse exposure), plus account/resource/method/Attempt/operation/expiry/use/revocation constraints and HTTPS-only credential-bearing transport on every hop; credentials are withheld on HTTP/downgrade or out-of-scope redirects, and every allowed redirect is requalified before forwarding. Recovery tests include authenticated host binding, enforcing epoch fences, causal dedupe/replay, capacity admission and version compatibility. These requirements are not postponed by a summary task's shorter wording.
