...
from agile_cli.extensions import ExtensionRegistry
...
class TestPresetManifest:
    ...
    def test_valid_manifest(self, pack_dir):
        ...
        assert manifest.requires_agile_version == ">=0.1.0"
    ...
    def test_one_bad_manifest_does_not_hide_healthy_presets(self, temp_dir):
        ...
        """
        preset_root = temp_dir / ".agile" / "presets"
...
""",
    ...
    def test_missing_agile_version(self, temp_dir, valid_pack_data):
        """Test missing requires.agile_version."""
        del valid_pack_data["requires"]["agile_version"]
        manifest_path = temp_dir / "preset.yml"
        with open(manifest_path, 'w') as f:
            yaml.dump(valid_pack_data, f)
        with pytest.raises(PresetValidationError, match="Missing requires.agile_version"):
            PresetManifest(manifest_path)
    ...
    def test_non_string_agile_version(self, temp_dir, valid_pack_data, bad):
        """A non-string requires.agile_version must be a PresetValidationError.
        ...
        """
        valid_pack_data["requires"]["agile_version"] = bad
        manifest_path = temp_dir / "preset.yml"
        with open(manifest_path, 'w') as f:
            yaml.dump(valid_pack_data, f)
        with pytest.raises(
            PresetValidationError, match="Invalid requires.agile_version"
        ):
            PresetManifest(manifest_path)
    ...
    def test_multiple_templates(self, temp_dir, valid_pack_data):
        ...
        valid_pack_data["provides"]["templates"] = [
            ...
            {"type": "command", "name": "agile", "file": "commands/specify.md"},
    ...
    def test_duplicate_template_name_and_type_raises_validation_error(
        ...
        """
        valid_pack_data["provides"]["templates"] = [
            {"type": "command", "name": "agile", "file": "commands/specify-v1.md"},
            {"type": "command", "name": "agile", "file": "commands/specify-v2.md"},
        ]
        manifest_path = temp_dir / "preset.yml"
        with open(manifest_path, 'w') as f:
            yaml.dump(valid_pack_data, f)
    ...
    def test_same_name_different_type_templates_allowed(
        ...
        valid_pack_data["provides"]["templates"] = [
            {"type": "template", "name": "agile", "file": "templates/specify.md"},
            {"type": "command", "name": "agile", "file": "commands/specify.md"},
        ]
        manifest_path = temp_dir / "preset.yml"
        with open(manifest_path, 'w') as f:
...
class TestPresetManager:
    ...
    def test_install_from_directory(self, project_dir, pack_dir):
        ...
        installed_dir = project_dir / ".agile" / "presets" / "test-pack"
    ...
    def test_install_incompatible(self, project_dir, temp_dir, valid_pack_data):
        ...
        valid_pack_data["requires"]["agile_version"] = ">=99.0.0"
    ...
    def test_check_compatibility_prerelease(self, pack_dir, temp_dir):
        ...
        with pytest.raises(PresetCompatibilityError, match="Preset requires agile"):
    ...
    def test_check_compatibility_invalid(self, pack_dir, temp_dir):
        ...
        manager = PresetManager(temp_dir)
        manifest = PresetManifest(pack_dir / "preset.yml")
        manifest.data["requires"]["agile_version"] = "not-a-specifier"
    ...
    def test_check_compatibility_non_string_specifier(self, pack_dir, temp_dir, bad):
        ...
        manager = PresetManager(temp_dir)
        manifest = PresetManifest(pack_dir / "preset.yml")
        manifest.data["requires"]["agile_version"] = bad
        with pytest.raises(PresetCompatibilityError, match="Invalid version specifier"):
            manager.check_compatibility(manifest, "0.1.5")
...
class TestPresetResolver:
    ...
    def test_resolver_ignores_traversing_registry_ids(self, project_dir):
        ...
        for registry_dir, registry_key, outside_name in (
            ...
            installed = project_dir / ".agile" / registry_dir
    ...
    def _install_pack_with_manifest_file(self, project_dir, *, extra_file=False):
        ...
        """
        presets_dir = project_dir / ".agile" / "presets"
    ...
    def test_resolve_extension_provided_templates(self, project_dir):
        ...
        ext_dir = project_dir / ".agile" / "extensions" / "my-ext"
    ...
    def test_resolve_disabled_extension_templates_skipped(self, project_dir):
        ...
        # Create extension with templates
        ext_dir = project_dir / ".agile" / "extensions" / "disabled-ext"
        ext_templates_dir = ext_dir / "templates"
        ext_templates_dir.mkdir(parents=True)
    ...
    def test_resolve_disabled_extension_not_picked_up_as_unregistered(self, project_dir):
        ...
        ext_dir = project_dir / ".agile" / "extensions" / "test-disabled-ext"
        ext_templates_dir = ext_dir / "templates"
        ext_templates_dir.mkdir(parents=True)
    ...
    def test_resolve_skips_hidden_extension_dirs(self, project_dir):
        ...
        ext_dir = project_dir / ".agile" / "extensions" / ".backup"
        ext_templates_dir = ext_dir / "templates"
        ext_templates_dir.mkdir(parents=True)
    ...
    def test_collect_all_layers_finds_bundled_core_without_specify_commands(
        ...
        .agile/templates/commands/ has no matching file.
        ...
        resolver = PresetResolver(project_dir)
        layers = resolver.collect_all_layers("agile.implement", "command")
    ...
    def test_resolve_command_falls_back_to_bundled_core(self, project_dir):
        ...
        .agile/templates/commands/ lacks it (regression for #3086)."""
        resolver = PresetResolver(project_dir)
        result = resolver.resolve("agile.implement", "command")
        assert result is not None
...
class TestResolveCore:
    ...
    def test_resolve_core_does_not_return_preset_files(self, project_dir):
        """resolve_core must not return files from .agile/presets/."""
        preset_cmd_dir = project_dir / ".agile" / "presets" / "my-preset" / "commands"
        preset_cmd_dir.mkdir(parents=True)
        (preset_cmd_dir / "specify.md").write_text("---\ndescription: preset wrap\n---\n\nwrap body\n")

        resolver = PresetResolver(project_dir)
        result = resolver.resolve_core("agile", "command")
    ...
    def test_resolve_core_returns_core_template(self, project_dir):
        ...
        core_cmd_dir = project_dir / ".agile" / "templates" / "commands"
    ...
    def test_resolve_core_returns_override(self, project_dir):
        ...
        override_dir = project_dir / ".agile" / "templates" / "overrides"
    ...
    def test_resolve_core_returns_extension_template(self, project_dir):
        ...
        ext_cmd_dir = project_dir / ".agile" / "extensions" / "myext" / "commands"
    ...
    def test_resolve_extension_command_via_manifest_skips_oserror_manifests(self, project_dir):
        ...
        ext_dir = project_dir / ".agile" / "extensions" / "bad-ext"
        ...
        with mock.patch("builtins.open", side_effect=PermissionError("denied")):
            result = resolver.resolve_extension_command_via_manifest("agile.bad-ext.mycmd")
...
class TestPresetCatalog:
    ...
    def test_search_with_cached_data(self, project_dir, monkeypatch):
        ...
        monkeypatch.delenv("AGILE_PRESET_CATALOG_URL", raising=False)
        catalog = PresetCatalog(project_dir)
        catalog.cache_dir.mkdir(parents=True, exist_ok=True)
    ...
    def test_env_var_catalog_url(self, project_dir, monkeypatch):
        ...
        monkeypatch.setenv("AGILE_PRESET_CATALOG_URL", "https://custom.example.com/catalog.json")
        catalog = PresetCatalog(project_dir)
    ...
    def test_fetch_single_catalog_rejects_oversized_body_without_cache(
        ...
        import agile_cli.presets as preset_module
        from unittest.mock import patch
...
class TestPresetCatalogMultiCatalog:
    ...
    def test_catalog_add_escapes_rich_markup(self, project_dir):
        ...
        config = yaml.safe_load(
            (project_dir / ".agile" / "preset-catalogs.yml").read_text(
                encoding="utf-8"
            )
        )
    ...
    def test_catalog_remove_escapes_rich_markup(self, project_dir):
        ...
        name = "[/red]my-catalog"
        (project_dir / ".agile" / "preset-catalogs.yml").write_text(
            yaml.dump({
                "catalogs": [
                    {
                        "name": name,
    ...
    def test_catalog_mutation_rejects_non_mapping_config_root(
        ...
        config_path = project_dir / ".agile" / "preset-catalogs.yml"
        original = "[]\n"
    ...
    def test_env_var_overrides_catalogs(self, project_dir, monkeypatch):
        """Test that AGILE_PRESET_CATALOG_URL env var overrides defaults."""
        monkeypatch.setenv(
            "AGILE_PRESET_CATALOG_URL",
    ...
    def test_load_catalog_config_nonexistent(self, project_dir):
        ...
        result = catalog._load_catalog_config(
            project_dir / ".agile" / "nonexistent.yml"
        )
        assert result is None
...
SELF_TEST_WRAP_WARNING = (
    r"Cannot compose command 'agile\.wrap-test': no base layer\. "
...
def install_self_test_preset(manager: PresetManager, agile_version: str = "0.1.5") -> PresetManifest:
    ...
    with warnings.catch_warnings():
        warnings.filterwarnings(
            ...
            module=r"agile_cli\.presets",
        )
        return manager.install_from_directory(SELF_TEST_PRESET_DIR, agile_version)
...
class TestSelfTestPreset:
    ...
    The self-test preset ships a wrap-strategy command (``agile.wrap-test``)
    ...
    def test_self_test_overrides_all_core_templates(self, project_dir):
        ...
        templates_dir = project_dir / ".agile" / "templates"
        for name in CORE_TEMPLATE_NAMES:
    ...
    def test_self_test_removal_restores_core(self, project_dir):
        ...
        memory = project_dir / ".agile" / "memory" / "constitution.md"
    ...
    def test_self_test_registers_commands_for_claude(self, project_dir):
        ...
        cmd_file = claude_dir / "agile-specify" / "SKILL.md"
    ...
    def test_self_test_preserves_mutable_project_core_copy(self, project_dir):
        ...
        project_template = (
            project_dir / ".agile" / "templates" / "constitution-template.md"
        )
    ...
    def test_convention_constitution_removal_preserves_edited_content(
        ...
        from agile_cli.commands.init import ensure_constitution_from_template
    ...
    def test_constitution_seed_rejects_symlinked_memory_directory(
        ...
        outside.mkdir()
        try:
            (project_dir / ".agile" / "memory").symlink_to(
    ...
    def test_constitution_seed_rejects_dangling_destination_symlink(
        ...
        memory = project_dir / ".agile" / "memory"
    ...
    def test_selfcontained_namespaced_command_scaffolds_without_extension(self, project_dir, temp_dir):
        """A preset shipping a self-contained ``agile.<ns>.<cmd>`` command
        ...
        must render just like a short ``agile.<cmd>`` command. It is not
        dropped merely because ``.agile/extensions/fakeext/`` is absent.
        """
        claude_dir = project_dir / ".claude" / "skills"
        claude_dir.mkdir(parents=True)
        ...
        (preset_dir / "commands" / "agile.fakeext.cmd.md").write_text(
        ...
        manifest_data = {
            ...
            "provides": {
                "templates": [
                    {
                        "type": "command",
                        "name": "agile.fakeext.cmd",
                        "file": "commands/agile.fakeext.cmd.md",
                        "description": "Override fakeext cmd",
                    }
                ]
            },
        ...
        skill_file = claude_dir / "agile-fakeext-cmd" / "SKILL.md"
    ...
    def test_extension_command_registered_when_extension_present(self, project_dir, temp_dir):
        ...
        claude_dir = project_dir / ".claude" / "skills"
        claude_dir.mkdir(parents=True)
        (project_dir / ".agile" / "extensions" / "fakeext").mkdir(parents=True)
        ...
        cmd_file = claude_dir / "agile-fakeext-cmd" / "SKILL.md"
...
class TestInitOptions:
    ...
    def test_save_and_load_round_trip(self, project_dir):
        from agile_cli import save_init_options, load_init_options
    ...
    def test_save_and_load_available_from_init_options_module(self, project_dir):
        from agile_cli._init_options import load_init_options, save_init_options
    ...
    def test_save_uses_utf8_encoding(self, project_dir, monkeypatch):
        from agile_cli import save_init_options

        original_write_text = Path.write_text
        seen: dict[str, str | None] = {}

        def spy_write_text(path, data, *args, **kwargs):
            if path == project_dir / ".agile" / "init-options.json":
    ...
    def test_load_uses_utf8_encoding(self, project_dir, monkeypatch):
        from agile_cli import load_init_options

        opts_file = project_dir / ".agile" / "init-options.json"
    ...
    def test_is_ai_skills_enabled_requires_boolean_true(self, value, expected):
        from agile_cli._init_options import is_ai_skills_enabled
...
class TestResolveActiveAgentForRegistration:
    ...
    def test_missing_file_returns_sentinel(self, project_dir):
        from agile_cli._init_options import (
    ...
    def test_valid_active_agent_returns_string(self, project_dir):
        from agile_cli import save_init_options
        from agile_cli._init_options import resolve_active_agent_for_registration
    ...
    def test_dangling_symlink_fails_closed(self, project_dir):
        ...
        opts_file = project_dir / ".agile" / "init-options.json"
        opts_file.parent.mkdir(parents=True, exist_ok=True)
        opts_file.symlink_to(project_dir / ".agile" / "does-not-exist.json")
...
class TestPresetSkills:
    ...
    def test_skill_overridden_on_preset_install(self, project_dir, temp_dir):
        ...
        metadata = manager.registry.get("self-test")
        assert "agile-specify" in metadata.get("registered_skills", {}).get("claude", [])
    ...
    def _install_arg_hint_preset(self, project_dir, temp_dir, ai, skills_dir, description, arg_hint):
        ...
        self._create_skill(skills_dir, "agile-hinttest-cmd")
        (project_dir / ".agile" / "extensions" / "hinttest").mkdir(parents=True, exist_ok=True)

        preset_dir = temp_dir / f"hint-preset-{ai}"
        preset_dir.mkdir()
        (preset_dir / "commands").mkdir()
        (preset_dir / "commands" / "agile.hinttest.cmd.md").write_text(
            "---\n"
        ...
        manifest_data = {
            ...
            "provides": {
                "templates": [
                    {
                        "type": "command",
                        "name": "agile.hinttest.cmd",
                        "file": "commands/agile.hinttest.cmd.md",
                    }
                ]
            },
        }
        ...
        manager = PresetManager(project_dir)
        manager.install_from_directory(preset_dir, "0.1.5")
        return skills_dir / "agile-hinttest-cmd" / "SKILL.md"
    ...
    def test_wrap_preset_inherits_argument_hint_from_core(self, project_dir, temp_dir):
        ...
        core_cmds = project_dir / ".agile" / "templates" / "commands"
    ...
    def test_wrap_preset_inherits_argument_hint_for_unmapped_command(self, project_dir, temp_dir):
        ...
        preset_description = "Wrapped agile.myfeature — extra project context added"
        self._write_init_options(project_dir, ai="claude")
        skills_dir = project_dir / ".claude" / "skills"
        self._create_skill(skills_dir, "agile-myfeature")
        ...
        preset_dir.mkdir()
        (preset_dir / "commands").mkdir()
        (preset_dir / "commands" / "agile.myfeature.md").write_text(
            "---\n"
            f'description: "{preset_description}"\n'
            "strategy: wrap\n"
            "---\n\n"
        ...
        manifest_data = {
            ...
            "provides": {
                "templates": [
                    {
                        "type": "command",
                        "name": "agile.myfeature",
                        "file": "commands/agile.myfeature.md",
                        "strategy": "wrap",
                    }
                ]
            },
        ...
        skill_file = skills_dir / "agile-myfeature" / "SKILL.md"
        assert skill_file.exists()
        parsed = yaml.safe_load(skill_file.read_text(encoding="utf-8").split("---", 2)[1])
        # argument-hint must be inherited from core, not dropped
        assert parsed.get("argument-hint") == core_arg_hint, (
    ...
    def test_register_skills_resolves_command_refs(self, project_dir, temp_dir):
        """Preset skill overrides must resolve __AGILE_COMMAND_*__ tokens (issue #2717).
        ...
        instead of rendering as ``/agile-<cmd>`` like the command layer.
        """
        self._write_init_options(project_dir, ai="claude")
        skills_dir = project_dir / ".claude" / "skills"
        self._create_skill(skills_dir, "agile-specify")
        ...
        assert "__AGILE_COMMAND_" not in content, "raw command token leaked into SKILL.md"
        # Claude's invoke_separator is "-", so tokens render as /agile-<cmd>.
    ...
    def test_restore_skill_resolves_command_refs(self, project_dir, temp_dir):
        ...
        content = (skills_dir / "agile-specify" / "SKILL.md").read_text()
        assert "__AGILE_COMMAND_" not in content, "raw command token leaked on restore"
        assert "/agile-plan" in content
    ...
    def test_reconcile_override_skill_resolves_command_refs(self, project_dir, temp_dir):
        ...
        the override body — which must also render ``__AGILE_COMMAND_*__`` tokens.
        """
        self._write_init_options(project_dir, ai="claude")
        skills_dir = project_dir / ".claude" / "skills"
        self._create_skill(skills_dir, "agile-specify")
        ...
        # command cross-reference token. No core template exists for "agile",
    ...
    def test_extension_restore_resolves_command_refs(self, project_dir, temp_dir):
        ...
        extension command body, ``__AGILE_COMMAND_*__`` tokens in that body
        must render as slash-command invocations like the core-template path.
        """
        self._write_init_options(project_dir, ai="claude")
        skills_dir = project_dir / ".claude" / "skills"
        self._create_skill(skills_dir, "agile-fakeext-cmd", body="original extension skill")

        extension_dir = project_dir / ".agile" / "extensions" / "fakeext"
        ...
        content = (skills_dir / "agile-fakeext-cmd" / "SKILL.md").read_text()
        assert "source: extension:fakeext" in content, "skill should be restored from the extension"
        assert "__AGILE_COMMAND_" not in content, "raw command token leaked on extension restore"
        assert "/agile-plan" in content
    ...
    def test_core_command_override_skill_uses_preset_command_description(self, project_dir, temp_dir):
        ...
        self._write_init_options(project_dir, ai="claude")
        skills_dir = project_dir / ".claude" / "skills"
        self._create_skill(skills_dir, "agile-taskstoissues")

        preset_dir = temp_dir / "taskstoissues-description"
        preset_dir.mkdir()
        (preset_dir / "commands").mkdir()
        (preset_dir / "commands" / "agile.repro.taskstoissues.md").write_text(
            "---\n"
        ...
        manifest_data = {
            ...
            "provides": {
                "templates": [
                    {
                        "type": "command",
                        "name": "agile.taskstoissues",
                        "file": "commands/agile.repro.taskstoissues.md",
                        "description": "MANIFEST-DESCRIPTION",
                        "replaces": "agile.taskstoissues",
                        "strategy": "replace",
                    }
                ]
            },
        ...
        skill_file = skills_dir / "agile-taskstoissues" / "SKILL.md"
        content = skill_file.read_text()
    ...
    def test_override_skill_reconcile_uses_override_command_description(self, project_dir, temp_dir):
        ...
        overrides_dir = project_dir / ".agile" / "templates" / "overrides"
        overrides_dir.mkdir(parents=True)
        (overrides_dir / "agile.taskstoissues.md").write_text(
            "---\n"
        ...
        assert "Convert tasks from tasks.md into GitHub issues." not in content
        assert "source: override:agile.taskstoissues" in content
    ...
    def test_skill_restored_on_preset_remove_without_project_core_templates(self, project_dir):
        ...
        has no ``.agile/templates/commands`` directory of its own — which
        is the normal case, since ``agile init`` never populates it. The
        ...
        # project — agile init never populates project-local overrides
    ...
    def test_extension_wins_over_bundled_core_on_preset_remove(
        ...
        The extension-command namespace rules (``agile.<ext>.<command>``)
        ...
        fake_restore_index = {
            "agile-specify": {
                "command_name": "agile.fakeext.agile",
    ...
    def test_skill_restored_on_remove_resolves_script_placeholders(self, project_dir):
        ...
        skills_dir = project_dir / ".claude" / "skills"
        self._create_skill(skills_dir, "agile-specify", body="old")
        (project_dir / ".claude" / "skills").mkdir(parents=True, exist_ok=True)
        ...
        (core_cmds / "specify.md").write_text(
            ...
            "description: Core specify command\n"
            "scripts:\n"
            "  sh: .agile/scripts/bash/create-new-feature.sh --json \"{ARGS}\"\n"
            "---\n\n"
            "Run:\n"
            "{SCRIPT}\n"
        )
        ...
        assert ".agile/scripts/bash/create-new-feature.sh --json \"$ARGUMENTS\"" in content
    ...
    def test_skill_not_overridden_when_skill_path_is_file(self, project_dir):
        ...
        (skills_dir / "agile-specify").write_text("not-a-directory")

        manager = PresetManager(project_dir)
        install_self_test_preset(manager)

        assert (skills_dir / "agile-specify").is_file()
        metadata = manager.registry.get("self-test")
        assert "agile-specify" not in metadata.get("registered_skills", {}).get("qwen", [])
    ...
    def test_extension_skill_override_matches_hyphenated_multisegment_name(self, project_dir, temp_dir):
        """Preset overrides for agile.<ext>.<cmd> should target agile-<ext>-<cmd> skills."""
        self._write_init_options(project_dir, ai="codex")
        skills_dir = project_dir / ".agents" / "skills"
        self._create_skill(skills_dir, "agile-fakeext-cmd", body="untouched")
        (project_dir / ".agile" / "extensions" / "fakeext").mkdir(parents=True, exist_ok=True)
        ...
        skill_file = skills_dir / "agile-fakeext-cmd" / "SKILL.md"
        assert skill_file.exists()
        content = skill_file.read_text()
        assert "preset:ext-skill-override" in content
        assert "name: agile-fakeext-cmd" in content
        assert "# Agile Fakeext Cmd Skill" in content

        metadata = manager.registry.get("ext-skill-override")
        assert "agile-fakeext-cmd" in metadata.get("registered_skills", {}).get("codex", [])
    ...
    def test_extension_skill_restored_on_preset_remove(self, project_dir, temp_dir):
        ...
        # registration.
        assert ".agile/extensions/fakeext/agents/control/commander.md" in content
    ...
    def test_preset_remove_skips_skill_dir_without_skill_file(self, project_dir, temp_dir):
        ...
        self._write_init_options(project_dir, ai="codex")
        skills_dir = project_dir / ".agents" / "skills"
        stray_skill_dir = skills_dir / "agile-fakeext-cmd"
        ...
        manager.registry.add(
            "ext-skill-missing-file",
            {
                ...
                "provides_templates": ["agile.fakeext.cmd"],
                "registered_skills": ["agile-fakeext-cmd"],
                "priority": 10,
            },
        )
    ...
    def test_kimi_new_skill_created_even_when_ai_skills_disabled(self, project_dir, temp_dir):
        ...
        preset_dir.mkdir()
        (preset_dir / "commands").mkdir()
        (preset_dir / "commands" / "agile.research.md").write_text(
            "---\n"
                ...
                "templates": [
                    {
                        "type": "command",
                        "name": "agile.research",
                        "file": "commands/agile.research.md",
                    }
                ]
            },
        }
        ...
        skill_file = skills_dir / "agile-research" / "SKILL.md"
        assert skill_file.exists()
        content = skill_file.read_text()
        assert "preset:kimi-new-skill" in content
        assert "name: agile-research" in content

        metadata = manager.registry.get("kimi-new-skill")
        assert "agile-research" in metadata.get("registered_skills", {}).get("kimi", [])
    ...
    def test_kimi_preset_skill_override_resolves_script_placeholders(self, project_dir, temp_dir):
        ...
        assert ".agile/scripts/bash/create-new-feature.sh --json \"$ARGUMENTS\"" in content
        assert ".agile/templates/checklist.md" in content
        assert ".agile/memory/constitution.md" in content
    ...
    def test_agy_skill_restored_on_preset_remove(self, project_dir, temp_dir):
        ...
        skills_dir = project_dir / ".agents" / "skills"
        self._create_skill(skills_dir, "agile-specify", body="before override")

        core_command = project_dir / ".agile" / "templates" / "commands" / "specify.md"
        ...
        assert "name: agile-specify" in restored
    ...
    def test_preset_skill_registration_handles_non_dict_init_options(self, project_dir, temp_dir):
        ...
        skill_content = (skills_dir / "agile-specify" / "SKILL.md").read_text()
    ...
    def test_hermes_rescaffold_reconciles_global_skill_output(
        ...
        skill_file = (
            home / ".hermes" / "skills" / "agile-specify" / "SKILL.md"
        )
        assert skill_file.exists()
        content = skill_file.read_text(encoding="utf-8")
        assert "Override body" in content
    ...
    def test_rescaffold_toggle_command_to_skills_removes_stale_command_file(
        ...
        skill_file = project_dir / ".github" / "skills" / "agile-specify" / "SKILL.md"
    ...
    def test_rescaffold_scaffolds_selfcontained_namespaced_commands(
        ...
        """A self-contained ``agile.<ns>.<cmd>`` preset command scaffolds and
        ...
        like a short ``agile.<cmd>`` command — both at install and through a
        later reconciliation/rescaffold pass. It is not dropped by the
        ``agile.<ns>.<cmd>`` name shape (#4076).
        """
        self._write_init_options(project_dir, ai="copilot", ai_skills=False)
        ...
        preset_dir = self._create_command_preset(
            temp_dir, "ext-scoped-preset", "agile.git.feature",
            "Ext override", "ext body",
        )
        manager = PresetManager(project_dir)
        manager.install_from_directory(preset_dir, "0.1.5")

        ext_cmd = commands_dir / "agile.git.feature.agent.md"
    ...
    def test_rescaffold_scaffolds_selfcontained_namespaced_skills(
        ...
        """A self-contained ``agile.<ns>.<cmd>`` preset command renders its
    ...
    def test_uncomposable_wrap_command_skips_skill_in_skills_mode(
        ...
        preset_dir.mkdir()
        (preset_dir / "commands").mkdir()
        # agile.git.feature has no core command template and no installed
        # extension, so there is no base layer to wrap.
        (preset_dir / "commands" / "agile.git.feature.md").write_text(
            "---\ndescription: Wrap\nstrategy: wrap\n---\n\n"
            "wrap start\n{CORE_TEMPLATE}\nwrap end\n"
        )
        manifest_data = {
            ...
            "provides": {
                "templates": [
                    {
                        "type": "command",
                        "name": "agile.git.feature",
                        "file": "commands/agile.git.feature.md",
                        "strategy": "wrap",
                    }
                ]
            },
        ...
        skill_file = skills_dir / "agile-git-feature" / "SKILL.md"
    ...
    def test_same_mode_partial_command_rescaffold_keeps_skipped_tracking(
        ...
        (installed_dir / "commands" / "agile.plan.md").unlink()
        manager.register_enabled_presets_for_agent("copilot")
    ...
    def test_same_mode_partial_skill_rescaffold_keeps_skipped_tracking(
        ...
        skills_dir = project_dir / ".github" / "skills"
        self._create_skill(skills_dir, "agile-specify")
        self._create_skill(skills_dir, "agile-plan")
        preset_dir = self._create_multi_command_preset(
            temp_dir,
    ...
    def test_toggle_command_to_skills_partial_result_only_removes_replaced_command(
        ...
        # exception.
        (manager.presets_dir / "toggle-partial-result-preset" / "commands" / "agile.plan.md").unlink()
        ...
        assert "agile.plan" in tracked_commands, (
            "plan must keep being tracked as a command since its old "
            "artifact is still on disk (#2948)"
        )
        specify_skill_file = (
            project_dir / ".github" / "skills" / "agile-specify" / "SKILL.md"
        )
        ...
        plan_skill_file = project_dir / ".github" / "skills" / "agile-plan"
    ...
    def test_remove_after_partial_command_to_skills_toggle_keeps_skills_mode_agent_command_free(
        ...
        lower_dir = self._create_command_preset(
            temp_dir, "stale-toggle-lower-preset", "agile.plan",
            "Lower preset", "Lower body",
        )
        higher_dir = self._create_command_preset(
            temp_dir, "stale-toggle-higher-preset", "agile.plan",
        ...
        command_file = copilot_commands_dir / "agile.plan.agent.md"
        ...
        (manager.presets_dir / "stale-toggle-higher-preset" / "commands" / "agile.plan.md").unlink()
        self._write_init_options(project_dir, ai="copilot", ai_skills=True)
        manager.register_enabled_presets_for_agent("copilot")

        metadata = manager.registry.get("stale-toggle-higher-preset")
        assert "agile.plan" in metadata["registered_commands"].get("copilot", []), (
    ...
    def test_remove_after_partial_skills_to_command_toggle_deletes_stale_skill(
        ...
        lower_dir = self._create_command_preset(
            temp_dir, "inverse-toggle-lower-preset", "agile.plan",
            "Lower preset", "Lower body",
        )
        higher_dir = self._create_command_preset(
            temp_dir, "inverse-toggle-higher-preset", "agile.plan",
            "Higher preset", "Higher body",
        )
        manager = PresetManager(project_dir)
        manager.install_from_directory(lower_dir, "0.1.5", priority=20)
        ...
        skill_dir = project_dir / ".github" / "skills" / "agile-plan"
        ...
        (manager.presets_dir / "inverse-toggle-higher-preset" / "commands" / "agile.plan.md").unlink()
        self._write_init_options(project_dir, ai="copilot", ai_skills=False)
        manager.register_enabled_presets_for_agent("copilot")
    ...
    def test_project_override_skill_retires_stale_preset_command(
        ...
        manager.registry.update(
            "override-skill-toggle-preset",
            {"registered_skills": {"copilot": ["agile-specify"]}},
        )
    ...
    def test_partial_skill_registration_is_persisted_before_later_failure(
        ...
        assert "agile-specify" in metadata["registered_skills"].get(
            "copilot", []
        ), (
    ...
    def test_remove_cleans_native_skill_missing_from_partial_skill_map(
        ...
        metadata = manager.registry.get("partial-agent-skill-map-preset")
        assert "agile.partial-native" in metadata[
            "registered_commands"
        ].get("codex", [])
    ...
    def test_toggle_skills_to_command_empty_result_preserves_old_skill(
        ...
        assert "agile-specify" in metadata["registered_skills"].get("copilot", []), (
    ...
    def test_toggle_skills_to_command_partial_result_only_removes_replaced_skill(
        ...
        specify_skill_file = skills_dir / "agile-specify" / "SKILL.md"
        plan_skill_file = skills_dir / "agile-plan" / "SKILL.md"
        ...
        # replacement is silently skipped (missing source), while
        # specify's succeeds.
        (manager.presets_dir / "toggle-skill-partial-result-preset" / "commands" / "agile.plan.md").unlink()
    ...
    def test_toggle_command_to_skills_partial_multi_group_only_retires_successful_group(
        ...
        (manager.presets_dir / "alias-group-partial-preset" / "commands" / "agile.plan.md").unlink()
        ...
        assert tracked_commands == {"agile.plan", "agile.plan-alt"}, (
    ...
    def test_skill_switch_then_remove_restores_every_skill_agent_dir(
        ...
        claude_skill = claude_skills_dir / "agile-specify" / "SKILL.md"
        ...
        codex_skill = codex_skills_dir / "agile-specify" / "SKILL.md"
    ...
    def test_rescaffold_migrates_legacy_flat_list_registered_skills(
        ...
        assert registered_skills.get("claude") == ["agile-specify"]
    ...
    def test_rescaffold_legacy_flat_list_direct_switch_preserves_original_agent(
        ...
        manager.registry.update(
            ...
            {"registered_skills": ["agile-specify"]},
        )
    ...
    def test_rescaffold_legacy_flat_list_infers_command_backed_skills_owner(
        ...
        copilot_skill = copilot_skills_dir / "agile-specify" / "SKILL.md"
    ...
    def test_infer_legacy_skill_provenance_skips_invalid_utf8(
        ...
        self._write_init_options(project_dir, ai="claude", ai_skills=True)
        skill_dir = (
            project_dir / ".claude" / "skills" / "agile-specify"
        )
        ...
        assert manager._infer_legacy_skill_provenance(
            ["agile-specify"], "some-pack", "claude"
        ) == {"claude": ["agile-specify"]}
    ...
    def test_infer_legacy_skill_provenance_excludes_home_outputs(
        ...
        home = temp_dir / "home"
        monkeypatch.setattr(Path, "home", lambda: home)
        skill_dir = home / ".hermes" / "skills" / "agile-specify"
        skill_dir.mkdir(parents=True)
        (skill_dir / "SKILL.md").write_text(
            "---\n"
            "metadata:\n"
        ...
        assert inferred == {"claude": ["agile-specify"]}
    ...
    def test_symlinked_skills_dir_rejected_on_removal(self, project_dir, temp_dir):
        ...
        # "agile-specify" directory that must not be touched.
        outside_target = temp_dir / "outside-claude-skills"
        outside_skill_dir = outside_target / "agile-specify"
    ...
    def test_preset_removal_does_not_touch_other_presets_skill_dir(
        ...
        *different* agent directories. Before the provenance fix, removing
        ...
        claude_skill_file = claude_skills_dir / "agile-specify" / "SKILL.md"
    ...
    def test_remove_does_not_recreate_empty_skill_dir(
        ...
        self._write_init_options(project_dir, ai="claude", ai_skills=True)
        skills_dir = project_dir / ".claude" / "skills"
        self._create_skill(skills_dir, "agile-orphan")
        preset_dir = self._create_command_preset(
            temp_dir,
            "orphan-skill-preset",
            "agile.orphan",
        ...
        skill_dir = skills_dir / "agile-orphan"
        assert skill_dir.exists()
    ...
    def test_shared_skills_dir_restored_once_using_active_agent(
        ...
        self._create_skill(shared_skills_dir, "agile-specify")
        ...
        skill_file = shared_skills_dir / "agile-specify" / "SKILL.md"
        content = skill_file.read_text()
    ...
    def test_composed_none_unregister_respects_active_agent(
        ...
        # onto.
        cmd_name = "agile.fake-compose-test"
    ...
    def test_extension_reconciliation_tracks_new_historical_agent(
        ...
        ext_dir = project_dir / ".agile" / "extensions" / "tracked-ext"
        ...
        ExtensionRegistry(ext_dir.parent).add(
            "tracked-ext",
            {
                ...
                "enabled": True,
                "registered_commands": {
                    "opencode": ["agile.tracked-ext.tracked"]
                },
            },
        )
        ...
        manager._reconcile_composed_commands(
            ["agile.tracked-ext.tracked"], extra_agents={"gemini"}
        )
    ...
    def test_remove_reconciles_skill_for_every_historical_agent(
        ...
        codex_skill_file = codex_skills_dir / "agile-specify" / "SKILL.md"
    ...
    def test_skill_reconciliation_preserves_per_directory_names(
        ...
        self._write_init_options(project_dir, ai="claude", ai_skills=True)
        claude_dir = project_dir / ".claude" / "skills"
        self._create_skill(claude_dir, "agile-alpha")
        alpha_dir = self._create_command_preset(
            temp_dir, "partial-alpha", "agile.alpha",
            "Alpha", "alpha body",
        )
        ...
        self._create_skill(codex_dir, "agile-beta")
        beta_dir = self._create_command_preset(
            temp_dir, "partial-beta", "agile.beta",
            "Beta", "beta body",
        )
        ...
        manager._reconcile_skills(
            ["agile.alpha", "agile.beta"],
            extra_skills_dirs=affected,
        )

        assert (claude_dir / "agile-alpha" / "SKILL.md").exists()
        assert (codex_dir / "agile-beta" / "SKILL.md").exists()
        assert not (claude_dir / "agile-beta").exists()
        assert not (codex_dir / "agile-alpha").exists()
    ...
    def test_symlinked_skill_subdir_rejected_on_restore(self, project_dir, temp_dir):
        ...
        ``.claude/skills/agile-specify``) has a perfectly safe parent and
        ...
        sentinel.write_text("do-not-touch")
        (claude_skills_dir / "agile-specify").symlink_to(
            outside_target, target_is_directory=True
        )

        manager = PresetManager(project_dir)
        manager._unregister_skills_in_dir(
            ["agile-specify"], claude_skills_dir, "claude"
        )
        ...
        assert (claude_skills_dir / "agile-specify").is_symlink(), (
            "the symlink itself should be left alone, not rmtree'd through"
        )
    ...
    def test_symlinked_skill_subdir_rejected_on_write(self, project_dir, temp_dir):
        ...
        outside_target.mkdir()
        (skills_dir / "agile-specify").symlink_to(
            outside_target, target_is_directory=True
        )
        ...
        assert (skills_dir / "agile-specify").is_symlink(), (
    ...
    def test_symlinked_skill_file_rejected_on_write(self, project_dir, temp_dir):
        ...
        (project_dir / ".github" / "agents").mkdir(parents=True)
        skill_dir = (
            project_dir / ".github" / "skills" / "agile-specify"
        )
        skill_dir.mkdir(parents=True)
    ...
    def test_symlinked_skill_file_rejected_on_restore(
        ...
        self._write_init_options(project_dir, ai="claude", ai_skills=True)
        core_commands = project_dir / ".agile" / "templates" / "commands"
        ...
        with pytest.raises(ValueError):
            manager._unregister_skills_in_dir(
                ["agile-specify"], skill_dir.parent, "claude"
            )
    ...
    def test_is_safe_registry_skill_name_rejects_unsafe_values(self, project_dir):
        ...
        assert is_safe("agile-specify") is True
        assert is_safe("") is False
        assert is_safe(None) is False
        assert is_safe(123) is False
        assert is_safe(["agile-specify"]) is False
        assert is_safe(".") is False
        assert is_safe("..") is False
    ...
    def test_unregister_skills_rejects_unknown_agent_provenance(
        ...
        skill_dir = self._create_skill(
            shared_skills_dir, "agile-specify", "user-owned content"
        )
        core_commands = project_dir / ".agile" / "templates" / "commands"
        (core_commands / "specify.md").write_text(
            "---\ndescription: Core specify\n---\n\nCore body\n",
        ...
        manager._unregister_skills(
            {"unknown": ["agile-specify"]}, project_dir
        )
    ...
    def test_unregister_legacy_fallback_skips_non_owned_skill(
        ...
        skills_dir = project_dir / ".claude" / "skills"
        skill_dir = self._create_skill(
            skills_dir, "agile-specify", "user-owned content"
        )
        core_commands = project_dir / ".agile" / "templates" / "commands"
        (core_commands / "specify.md").write_text(
            "---\ndescription: Core specify\n---\n\nCore body\n",
    ...
    def test_unregister_skills_in_dir_unreadable_core_template_skips(
        ...
        ``.agile/templates/commands/`` raised a raw ``UnicodeDecodeError``
        ...
        skills_dir = project_dir / ".claude" / "skills"
        skill_dir = self._create_skill(
            skills_dir, "agile-specify", "installed content"
        )
        core_commands = project_dir / ".agile" / "templates" / "commands"
        ...
        manager = PresetManager(project_dir)
        with pytest.warns(UserWarning, match="agile-specify"):
            mutated = manager._unregister_skills_in_dir(
                ["agile-specify"], skills_dir, "claude"
            )
    ...
    def test_unregister_skills_in_dir_unreadable_extension_source_skips(
        ...
        skills_dir = project_dir / ".claude" / "skills"
        skill_dir = self._create_skill(
            skills_dir, "agile-fakeext-cmd", "installed content"
        )
        ...
        manager = PresetManager(project_dir)
        with pytest.warns(UserWarning, match="agile-fakeext-cmd"):
            mutated = manager._unregister_skills_in_dir(
                ["agile-fakeext-cmd"], skills_dir, "claude"
            )
    ...
    def test_copilot_skills_registration_restored_after_process_restart(
        ...
        skill_file = copilot_skills_dir / "agile-specify" / "SKILL.md"
    ...
    def test_unregister_native_agent_persists_skills_metadata_pop(
        ...
        manager.registry.update(
            "native-metadata-cleanup-preset",
            {"registered_skills": {"agy": ["agile-shared-cleanup"]}},
        )
    ...
    def test_unregister_agent_artifacts_migrates_legacy_skill_list_scoped(
        ...
        assert "claude" in registered_skills and "agile-specify" in registered_skills["claude"], (
    ...
    def test_short_and_namespaced_commands_scaffold_consistently(
        ...
        """A preset's ``agile.<cmd>`` and ``agile.<ns>.<cmd>`` commands must
        scaffold identically in command mode, with no installed extension.

        Regression: the 3-part (``agile.<ns>.<cmd>``) form was silently
        dropped by a name-shape guard whenever ``.agile/extensions/<ns>/``
        ...
        short_preset = self._create_command_preset(
            temp_dir, "short-cmd", "agile.newcmd", "Short", "short body",
        )
        ns_preset = self._create_command_preset(
            temp_dir, "ns-cmd", "agile.fakeext.newcmd", "Namespaced", "ns body",
        )
        ...
        short_file = gemini_commands_dir / "agile.newcmd.toml"
        ns_file = gemini_commands_dir / "agile.fakeext.newcmd.toml"
...
class TestPresetPriorityBackwardsCompatibility:
    ...
    def test_legacy_preset_without_priority_field(self, temp_dir):
        ...
        presets_dir = temp_dir / ".agile" / "presets"
...
class TestPresetEnableDisable:
    ...
    def test_disabled_preset_excluded_from_resolution(self, project_dir, pack_dir):
        ...
        preset_template = project_dir / ".agile" / "presets" / "test-pack" / "templates" / "test-template.md"
...
@pytest.mark.parametrize(
    ...
    [
        CORE_CONSTITUTION_COMMAND,
        LEAN_PRESET_DIR / "commands" / "agile.constitution.md",
    ],
    ids=["core", "lean"],
)
...
class TestLeanPreset:
    ...
    def test_lean_commands_have_no_scripts(self):
        ...
        for name in LEAN_COMMAND_NAMES:
            cmd_path = LEAN_PRESET_DIR / "commands" / f"agile.{name.split('.')[-1]}.md"
            content = cmd_path.read_text()
...
class TestBundledPresetLocator:
    ...
    def test_locate_bundled_lean_preset(self):
        ...
        from agile_cli import _locate_bundled_preset
    ...
    def test_bundled_preset_add_via_cli(self, project_dir):
        ...
        runner = CliRunner()
        with patch.object(Path, "cwd", return_value=project_dir), \
             patch("agile_cli.get_agile_version", return_value="0.6.0"):
    ...
    def test_preset_add_from_url_rejects_insecure_redirect(self, project_dir, monkeypatch):
        ...
        import typer
        from agile_cli.presets._commands import preset_add
        ...
        monkeypatch.setattr("agile_cli._require_specify_project", lambda: project_dir)
        monkeypatch.setattr("agile_cli.get_agile_version", lambda: "0.6.0")
        ...
        monkeypatch.setattr("agile_cli.authentication.http.open_url", fake_open_url)

        installed = False

        def fake_install_from_zip(self, zip_path, agile_version, priority=10):
            nonlocal installed
            installed = True
    ...
    def test_preset_add_from_url_reads_in_bounded_chunks(self, project_dir, monkeypatch):
        ...
        def fake_install_from_zip(self, zip_path, agile_version, priority=10):
            ...
            installed["agile_version"] = agile_version
    ...
    def test_preset_add_from_url_rejects_oversized_download(
        ...
        from agile_cli.presets import _commands as preset_commands
        ...
        monkeypatch.setattr(
            "agile_cli._require_specify_project",
            lambda: project_dir,
        )
        monkeypatch.setattr("agile_cli.get_agile_version", lambda: "0.6.0")
        monkeypatch.setattr(
    ...
    def test_bundled_preset_download_raises_error(self, project_dir):
        ...
        with patch.object(catalog, "_get_merged_packs", return_value=catalog_data):
            with pytest.raises(PresetError, match="bundled with agile"):
    ...
    def test_bundled_preset_missing_locally_cli_error(self, project_dir):
        ...
        with patch.object(Path, "cwd", return_value=project_dir), \
             patch("agile_cli._locate_bundled_preset", return_value=None), \
             patch("agile_cli.presets.PresetCatalog") as MockCatalog:
...
class TestPresetAddFromUrlResolution:
    ...
    def test_preset_add_from_github_release_url_resolves_and_downloads(self, project_dir):
        ...
        runner = CliRunner()
        with patch.object(Path, "cwd", return_value=project_dir), \
             patch("agile_cli.get_agile_version", return_value="1.0.0"), \
...
class TestWrapStrategy:
    ...
    def test_substitute_core_template_replaces_placeholder(self, project_dir):
        ...
        from agile_cli.presets import _substitute_core_template
        from agile_cli.agents import CommandRegistrar

        # Set up a core command template
        core_dir = project_dir / ".agile" / "templates" / "commands"
        ...
        body = "## Pre-Logic\n\nBefore stuff.\n\n{CORE_TEMPLATE}\n\n## Post-Logic\n\nAfter stuff.\n"
        result, core_fm = _substitute_core_template(body, "agile", project_dir, registrar)
    ...
    def test_substitute_core_template_unreadable_core_treated_as_missing(
        ...
        project-owned override in ``.agile/templates/commands/`` crashed
        ...
        with pytest.warns(UserWarning, match="Ignoring core template"):
            result, core_fm = _substitute_core_template(
                body, "agile", project_dir, registrar
            )
        assert result == body
        assert core_fm == {}
    ...
    def test_end_to_end_wrap_via_self_test_preset(self, project_dir):
        ...
        from agile_cli.presets import PresetManager
        ...
        skills_dir = project_dir / ".claude" / "skills"
        skills_dir.mkdir(parents=True, exist_ok=True)
        skill_subdir = skills_dir / "agile-wrap-test"
        skill_subdir.mkdir()
        (skill_subdir / "SKILL.md").write_text("---\nname: agile-wrap-test\n---\n\nold content\n")

        # Write init-options so _register_skills finds the claude skills dir
        import json
        (project_dir / ".agile" / "init-options.json").write_text(
    ...
    def test_register_skills_preset_scripts_take_precedence_over_core(self, project_dir):
        ...
        registrar = CommandRegistrar()
        body = "{CORE_TEMPLATE}"
        _, core_fm = _substitute_core_template(body, "agile", project_dir, registrar)
    ...
    def test_extension_command_resolves_via_extension_directory(self, project_dir):
        """Extension commands (e.g. agile.git.feature) resolve from the extension directory.
        ...
        and finds agile.git.feature.md in the extension commands directory.
        """
        from agile_cli.presets import _substitute_core_template
        from agile_cli.agents import CommandRegistrar

        # Place the template where a real extension would install it
        ext_cmd_dir = project_dir / ".agile" / "extensions" / "git" / "commands"
        ext_cmd_dir.mkdir(parents=True, exist_ok=True)
        (ext_cmd_dir / "agile.git.feature.md").write_text(
            "---\ndescription: git feature core\n---\n\n# Git Feature Core\n"
        )
        # Ensure a hyphenated or dot-separated fallback does NOT exist
        assert not (project_dir / ".agile" / "templates" / "commands" / "git.feature.md").exists()
        assert not (project_dir / ".agile" / "templates" / "commands" / "git-feature.md").exists()
        ...
        result, _ = _substitute_core_template(body, "agile.git.feature", project_dir, registrar)
    ...
    def test_extension_command_resolves_via_manifest_when_filename_differs(self, project_dir):
        ...
        The selftest extension maps agile.selftest.extension → commands/selftest.md.
        Name-based lookup would look for commands/agile.selftest.extension.md and fail;
        manifest-based lookup must find the actual file declared in the manifest.
        """
        from agile_cli.presets import _substitute_core_template
        from agile_cli.agents import CommandRegistrar

        ext_dir = project_dir / ".agile" / "extensions" / "selftest"
        cmd_dir = ext_dir / "commands"
        cmd_dir.mkdir(parents=True, exist_ok=True)

        # File is named selftest.md, NOT agile.selftest.extension.md
        ...
        registrar = CommandRegistrar()
        body = "## Wrapper\n\n{CORE_TEMPLATE}\n"
        result, _ = _substitute_core_template(body, "agile.selftest.extension", project_dir, registrar)
    ...
    def test_extension_template_resolves_via_manifest_when_filename_differs(self, project_dir):
        ...
        manifest validation.
        """
        ext_dir = project_dir / ".agile" / "extensions" / "reportext"
    ...
    def test_extension_script_resolves_via_manifest_when_filename_differs(self, project_dir):
        ...
        doesn't sit at the conventional path."""
        ext_dir = project_dir / ".agile" / "extensions" / "collectext"
    ...
    def test_extension_template_convention_lookup_unaffected_when_undeclared(self, project_dir):
        ...
        ext_dir = project_dir / ".agile" / "extensions" / "conventionext"
    ...
    def test_extension_manifest_wins_over_stale_conventional_file(self, project_dir):
        ...
        ext_dir = project_dir / ".agile" / "extensions" / "bothpathsext"
    ...
    def test_extension_manifest_declared_but_missing_file_does_not_fall_back(self, project_dir):
        ...
        ext_dir = project_dir / ".agile" / "extensions" / "missingfileext"
    ...
    def test_extension_script_resolve_and_resolve_with_source_parity(self, project_dir):
        ...
        ext_dir = project_dir / ".agile" / "extensions" / "collectext2"
        script_dir = ext_dir / "scripts" / "bash"
        script_dir.mkdir(parents=True, exist_ok=True)
...
class TestResolveContent:
    ...
    def test_resolve_content_unreadable_winning_layer_returns_none(self, project_dir):
        ...
        presets_dir = project_dir / ".agile" / "presets"
        command_path = (
            presets_dir / "legacy-pack" / "commands" / "agile.legacy.md"
        )
        ...
        resolver = PresetResolver(project_dir)
        content = resolver.resolve_content("agile.legacy", "command")
        assert content is None
    ...
    def test_resolve_content_unreadable_base_under_composing_layer(
        ...
        core_spec = project_dir / ".agile" / "templates" / "spec-template.md"
    ...
    def test_resolve_content_unreadable_composing_layer(
        ...
        layer_path = (
            project_dir / ".agile" / "presets" / "append-pack"
    ...
    def test_resolve_content_wrap_strategy_script(self, project_dir, temp_dir, valid_pack_data):
        ...
        # Create core script
        scripts_dir = project_dir / ".agile" / "templates" / "scripts"
    ...
    def test_resolve_content_command_type(self, project_dir, temp_dir, valid_pack_data):
        ...
        # Create core command using stem naming (matches real layout: plan.md, not agile.plan.md)
        commands_dir = project_dir / ".agile" / "templates" / "commands"
        ...
        (pack_dir / "commands" / "agile.plan.md").write_text("## Additional Instructions\n")

        manager = PresetManager(project_dir)
        manager.install_from_directory(pack_dir, "0.1.5")

        resolver = PresetResolver(project_dir)
        content = resolver.resolve_content("agile.plan", "command")
        assert content is not None
    ...
    def test_resolve_content_command_frontmatter_stripping(self, project_dir, temp_dir, valid_pack_data):
        ...
        with open(pack_dir / "preset.yml", 'w') as f:
            yaml.dump(pack_data, f)
        (pack_dir / "commands").mkdir()
        (pack_dir / "commands" / "agile.check.md").write_text(
        ...
        resolver = PresetResolver(project_dir)
        content = resolver.resolve_content("agile.check", "command")
        assert content is not None
    ...
    def test_resolve_content_rewrites_extension_base_subdir_paths(
        ...
        resolver = PresetResolver(project_dir)
        content = resolver.resolve_content("agile.fakeext.cmd", "command")
        assert content is not None
        assert ".agile/extensions/fakeext/agents/control/commander.md" in content
        assert "Read agents/control" not in content
        assert "## Extra" in content
...
class TestRemoveReconciliation:
    ...
    def test_install_composes_extension_command_and_rewrites_subdir_paths_for_non_skill_agent(
        ...
        agile.fakeext.cmd, generated Gemini content retains the bare path."""
        gemini_dir = project_dir / ".gemini" / "commands"
        gemini_dir.mkdir(parents=True)
...
def test_preset_wrapper_resolves_ghes_asset_when_host_configured(tmp_path, monkeypatch):
    ...
    from agile_cli.authentication import http as _auth_http
    from agile_cli.authentication.config import AuthConfigEntry
    from agile_cli.presets import PresetCatalog
...
class TestInstalledPresetRichMarkup:
    ...
    def test_resolve_accepts_dotted_command_name(self, project_dir):
        ...
        result = self._invoke(
            project_dir,
            ["preset", "resolve", "agile.constitution"],
        )
...
class TestConstitutionSyncPreset:
    ...
    def test_wrap_composes_over_core_constitution(self, project_dir):
        ...
        resolver = PresetResolver(project_dir)
        layers = resolver.collect_all_layers("agile.constitution", "command")
    ...
    def test_resolved_content_embeds_core_and_sync_pass(self, project_dir):
        ...
        resolver = PresetResolver(project_dir)
        content = resolver.resolve_content("agile.constitution", "command")
        assert content is not None
