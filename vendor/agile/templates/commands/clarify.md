...
Note: This clarification workflow is expected to run (and be completed) BEFORE invoking `__AGILE_COMMAND_PLAN__`. If the user explicitly states they are skipping clarification (e.g., exploratory spike), you may proceed, but must warn that downstream rework risk increases.
...
1. Run `{SCRIPT}` from repo root **once** (combined `--json --paths-only` mode / `-Json -PathsOnly`). Parse minimal JSON payload fields:
   ...
   - If JSON parsing fails, abort and instruct user to re-run `__AGILE_COMMAND_SPECIFY__` or verify feature branch environment.
...
5. Sequential questioning loop (interactive):
    ...
    - **Question writing quality (applies to every question, MC or short-answer):**
       ...
       - Use everyday wording; introduce jargon only if defined in the same sentence. Self-check: a reader who does not know Agile must be able to answer from the Question line alone. Terse is fine; cryptic labels are not.
...
- If spec file missing, instruct user to run `__AGILE_COMMAND_SPECIFY__` first (do not create a new spec here).
...
- If any Outstanding or Deferred remain, recommend whether to proceed to `__AGILE_COMMAND_PLAN__` or run `__AGILE_COMMAND_CLARIFY__` again later post-plan.
