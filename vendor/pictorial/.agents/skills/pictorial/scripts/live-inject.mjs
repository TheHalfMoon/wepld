 ...
 *
 * On first live run, the agent generates `.pictorial/live/config.json`
...
import { resolveLiveConfigPath } from './lib/pictorial-paths.mjs';
import {
...
const IGNORE_MARKER_OPEN = '# pictorial-live-ignore-start';
const IGNORE_MARKER_CLOSE = '# pictorial-live-ignore-end';
...
export const LIVE_IGNORE_PATTERNS = Object.freeze([
  ...
  '.pictorial/live/roots.json',
  '.pictorial/live/app-root.json',
  '.pictorial/live/inject-journal.json',
  '.pictorial/live/sessions/',
  '.pictorial/live/previews/',
  '.pictorial/live/annotations/',
  '.pictorial/live/artifacts/',
  '.pictorial/live/accept-receipts/',
  '.pictorial/live/locks/',
  ...
  'app/.pictorial-live/',
  'src/.pictorial-live/',
  'node_modules/.pictorial-live/',
  'src/lib/pictorial/PictorialLiveRoot.svelte',
  'src/lib/pictorial/__runtime.js',
  'src/lib/pictorial/[0-9a-f]*/',
  'plugins/pictorial-live.client.ts',
  'app/plugins/pictorial-live.client.ts',
  'src/plugins/pictorial-live.client.ts',
]);
...
Reads configuration from .pictorial/live/config.json.

Modes:
  --port PORT   Insert script tag pointing at http://localhost:PORT/live.js
  --remove      Remove the script tag (if present)
  --check       Print whether .pictorial/live/config.json exists and its content
...
Output (JSON):
  ...
  if (!token) {
    try {
      const info = JSON.parse(fs.readFileSync(path.join(cwd, '.pictorial', 'live', 'server.json'), 'utf-8'));
  ...
  try {
    const rootsManifest = JSON.parse(fs.readFileSync(path.join(cwd, '.pictorial', 'live', 'roots.json'), 'utf-8'));
