...
async function measureContentHiddenAfterReveal(page) {
  ...
  return page.evaluate(() => {
    if (typeof window.pictorialMeasureHiddenText !== 'function') return null;
    return window.pictorialMeasureHiddenText();
  });
}
...
async function runVisualContrastFallback(page, serializedGroups, options, profile, target) {
  ...
  if (options?.visualContrastBrowser !== false) {
    const browserFindings = await profileFindingsAsync(profile, {
      ...
      browserAnalyses = await page.evaluate(async ({ maxCandidates, scrollOffscreen }) => {
        if (typeof window.pictorialAnalyzeVisualContrast !== 'function') return [];
        return window.pictorialAnalyzeVisualContrast({ maxCandidates, scrollOffscreen });
  ...
  if (candidates.length === 0) {
    candidates = await profileStepAsync(profile, {
      ...
      if (typeof window.pictorialCollectVisualContrastCandidates !== 'function') return [];
      return window.pictorialCollectVisualContrastCandidates({ maxCandidates });
    }, { maxCandidates }));
  }
  ...
  try {
    ...
    results = await profileFindingsAsync(profile, {
      ...
      serializedGroups = await page.evaluate(() => {
        if (!window.pictorialDetect) return [];
        return window.pictorialDetect({ decorate: false, serialize: true });
      });
