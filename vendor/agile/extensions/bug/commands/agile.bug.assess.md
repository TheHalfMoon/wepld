...
Triage a bug report against the current codebase: understand the symptom, locate the suspected root cause, judge severity, and propose a remediation. The output is a single assessment file at `.agile/bugs/<slug>/assessment.md` that downstream commands (`__AGILE_COMMAND_BUG_FIX__`, `__AGILE_COMMAND_BUG_TEST__`) consume.
...
Each bug gets its own directory under `.agile/bugs/<slug>/`. Resolve the slug in this order:
...
3. **Automated / non-interactive mode** (no human to ask): Generate a concise slug yourself from the bug summary (2–4 kebab-case words, e.g. `login-timeout-500`). The generated slug **MUST** produce a unique directory — if `.agile/bugs/<slug>/` already exists, append the shortest disambiguating suffix needed (`-2`, `-3`, …) or a short ISO-style date (`-20260605`) to make it unique. Never overwrite an existing bug directory.

After resolution, set `BUG_SLUG` and `BUG_DIR = .agile/bugs/<BUG_SLUG>`.

## Prerequisites

- Ensure the directory `.agile/bugs/<BUG_SLUG>/` (i.e., `BUG_DIR`) exists, creating it (including any missing parents) if necessary. Use whatever mechanism is appropriate for the current environment.
...
5. **Propose a remediation**
   ...
   - Identify files to change and the shape of the change (without writing the patch yet — that is `__AGILE_COMMAND_BUG_FIX__`'s job).
...
7. **Report back** with:
   ...
   - The path `.agile/bugs/<BUG_SLUG>/assessment.md`.
   - The verdict and severity.
   - The next suggested step: `__AGILE_COMMAND_BUG_FIX__ slug=<BUG_SLUG>`.

## Guardrails

- Never modify source files during assessment — this command only reads and writes inside `.agile/bugs/<slug>/`.
