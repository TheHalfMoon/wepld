...
1. Read `.agile/templates/plan-template.md` and ensure any "Constitution Check" or rules align
   ...
   constitution.
2. Read `.agile/templates/spec-template.md` for scope/requirements alignment — update if the
   constitution adds/removes mandatory sections or constraints.
3. Read `.agile/templates/tasks-template.md` and ensure task categorization reflects new or
   removed principle-driven task types (e.g., observability, versioning, testing discipline).
4. Read each installed Agile command file for your agent (including this one) — named
   `agile.*` or `agile-*` (dot or hyphen depending on the agent), or laid out as
   `agile-<name>/SKILL.md` for skills-based integrations, e.g. in `.github/agents/`,
...
Then extend the Sync Impact Report at the top of `.agile/memory/constitution.md` with:
...
`.agile/templates/` scaffolds and to command files that are not managed by a preset or extension.
