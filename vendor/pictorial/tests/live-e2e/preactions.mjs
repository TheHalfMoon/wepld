...
export async function runPreActions(page, actions) {
  ...
  const wasActive = await page.evaluate((sel) =>
    window.__pictorialLiveQuery?.(sel)?.dataset.active === 'true',
...
async function liveUiSnapshot(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      hasShadowRoot: Boolean(document.getElementById('pictorial-live-root')?.shadowRoot),
      ...
      wrapper: wrapper ? { preview: wrapper.dataset.pictorialPreview, count: wrapper.dataset.pictorialVariantCount, html: wrapper.outerHTML.slice(0, 800) } : null,
