 ...
 * `.pictorial/live/config.json`. This module owns that block: building it,
...
export const MARKER_OPEN_TEXT = 'pictorial-live-start';
export const MARKER_CLOSE_TEXT = 'pictorial-live-end';

/** Markers that identify a file as still carrying our tag-strategy patch. */
export const TAG_PATCH_MARKERS = Object.freeze([MARKER_OPEN_TEXT, 'data-pictorial-csp-original']);
...
export function removeTag(content, _syntax) {
  const patterns = [
    /([ \t]*)<!--\s*pictorial-live-start\s*-->[\s\S]*?<!--\s*pictorial-live-end\s*-->([ \t]*(?:\r\n|\n|\r|$)?)/,
    /([ \t]*)\{\/\*\s*pictorial-live-start\s*\*\/\}[\s\S]*?\{\/\*\s*pictorial-live-end\s*\*\/\}([ \t]*(?:\r\n|\n|\r|$)?)/,
  ];
...
// and stash the original `content` value in a `data-pictorial-csp-original`
...
const CSP_MARKER_ATTR = 'data-pictorial-csp-original';
