"""
Agent Command Registrar for Agile
...
def _build_agent_configs() -> dict[str, Any]:
    ...
    for key, integration in INTEGRATION_REGISTRY.items():
        ...
        if integration.registrar_config:
            ...
            # when register_commands() resolves __AGILE_COMMAND_*__ tokens.
...
class CommandRegistrar:
    ...
    def _hyphenate_frontmatter_refs(val: Any) -> Any:
        """Recursively find any dotted references starting with agile. and hyphenate them."""
        if isinstance(val, dict):
            ...
            return re.sub(
                r"\bagile\.[A-Za-z0-9-_]+(?:\.[A-Za-z0-9-_]+)*\b",
    ...
    def _hyphenate_body_refs(body: str) -> str:
        """Hyphenate dotted agile references in command body text."""
        return re.sub(
            r"\bagile\.[A-Za-z0-9-_]+(?:\.[A-Za-z0-9-_]+)*\b",
            lambda m: m.group(0).replace(".", "-"),
            body,
    ...
    def _adjust_script_paths(
        ...
        ``memory/``) to the ``.agile/...`` paths used in generated projects.
    ...
    def rewrite_extension_paths(
        ...
        those files live under ``.agile/extensions/<id>/``, so bare
        ...
        for subdir in subdirs:
            ...
            replacement = f".agile/extensions/{extension_id}/{subdir}/"
            text = re.sub(
    ...
    # gemini/tabnine renderer in ``agile_cli.integrations.base`` via
    ...
    def render_yaml_command(
        ...
        """
        from agile_cli.integrations.base import YamlIntegration
    ...
    def render_skill_command(
        ...
        Technical debt note:
        Agile currently has multiple SKILL.md generators (template packaging,
        ...
        description = frontmatter.get(
            "description", f"Agile workflow command: {skill_name}"
        )
    ...
    def register_commands(
        ...
        # ``/agile-*`` whenever that agent is active in skills mode — even
        # though a command-layout file must use ``/agile.*``.  Deriving it
        ...
        try:
            from agile_cli.integrations import get_integration  # noqa: PLC0415
        ...
        for cmd_info in commands:
            ...
            # Resolve __AGILE_COMMAND_*__ tokens using the agent's invoke separator.
            ...
            from agile_cli.integrations.base import IntegrationBase  # noqa: PLC0415
            ...
            _integration = None
            if agent_config["extension"] != "/SKILL.md":
                from agile_cli.integrations import (  # noqa: PLC0415
                    get_integration,
                )
    ...
    def _write_registered_output(
        ...
        cache_root = source_dir / ".agile-dev" / "agent-commands" / agent_name
    ...
    def write_copilot_prompt(project_root: Path, cmd_name: str) -> None:
        ...
        Args:
            project_root: Path to project root
            cmd_name: Command name (e.g. 'agile.my-ext.example')
        """
        name_reason = relative_extension_path_violation(cmd_name)
        if name_reason:
            raise ValueError(
                ...
                for target_dir in dirs_to_clean:
                    for name in names_to_clean:
                        ...
                        if cmd_file.exists() or cmd_file.is_symlink():
                            ...
                            # subdirectory (e.g. .agents/skills/agile-ext-cmd/
