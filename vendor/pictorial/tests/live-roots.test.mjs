...
describe('live roots resolution', () => {
  ...
  beforeEach(() => {
    tmp = realpathSync(mkdtempSync(join(tmpdir(), 'pictorial-roots-')));
  });
  ...
  it('roots a targeted file at the nested app, context at the git root', () => {
    ...
    assert.equal(manifest.sessionRoot, join(tmp, 'website', '.pictorial', 'live'));
  });
  ...
  it('treats a live-configured directory as an app root without a dev config', () => {
    mkdirSync(join(tmp, '.git'), { recursive: true });
    write(tmp, 'site/.pictorial/live/config.json', '{"files":["index.html"]}');
  ...
  it('ignores a stale manifest that claims a different appRoot', () => {
    setupPlainNestedApp();
    write(tmp, 'website/.pictorial/live/roots.json', JSON.stringify({
      version: 1,
...
describe('review regressions: walk bounds', () => {
  it('does not climb past a target outside the cwd git repo (m5)', () => {
    const outer = realpathSync(mkdtempSync(join(tmpdir(), 'pictorial-roots-outer-')));
    try {
...
describe('review regressions: multi-app pointer', () => {
  it('prefers the app whose live server is running over the last boot', async () => {
    const repo = realpathSync(mkdtempSync(join(tmpdir(), 'pictorial-roots-multi-')));
    try {
      ...
      try {
        write(repo, 'siteA/.pictorial/live/server.json', JSON.stringify({ pid: process.pid, port: livePort, token: 't' }));
        write(repo, 'siteB/.pictorial/live/server.json', JSON.stringify({ pid: 999999999, port: 2, token: 't' }));
  ...
  it('reads a legacy single-value pointer', () => {
    const repo = realpathSync(mkdtempSync(join(tmpdir(), 'pictorial-roots-legacy-')));
    try {
      mkdirSync(join(repo, '.git'), { recursive: true });
      ...
      write(repo, '.pictorial/live/app-root.json', JSON.stringify({ appRoot: join(repo, 'app') }));
      const resolved = resolveLiveRoots(repo);
...
describe('review regressions: stopped-session recovery', () => {
  it('prefers the app with an active durable session when no server is alive', () => {
    const repo = realpathSync(mkdtempSync(join(tmpdir(), 'pictorial-roots-stopped-')));
    try {
      mkdirSync(join(repo, '.git'), { recursive: true });
      for (const name of ['siteA', 'siteB']) {
        write(repo, `${name}/vite.config.js`, 'export default {};');
      ...
      write(repo, 'siteA/.pictorial/live/sessions/ab12cd34.snapshot.json',
        JSON.stringify({ id: 'ab12cd34', phase: 'variants_ready' }));
      write(repo, 'siteB/.pictorial/live/sessions/ff00ff00.snapshot.json',
...
describe('review regressions: helper --target', () => {
  it('enterLiveRoot honors --target and strips it from argv', () => {
    const repo = realpathSync(mkdtempSync(join(tmpdir(), 'pictorial-roots-target-')));
    try {
      mkdirSync(join(repo, '.git'), { recursive: true });
      ...
      write(repo, 'appA/.pictorial/live/server.json', JSON.stringify({ pid: process.pid, port: 1, token: 't' }));
      write(repo, 'appB/.pictorial/live/server.json', JSON.stringify({ pid: process.pid, port: 2, token: 't' }));
  ...
  it('enterLiveRoot exits with an error on a valueless --target rather than picking an app', () => {
    const repo = realpathSync(mkdtempSync(join(tmpdir(), 'pictorial-roots-target-bad-')));
    try {
      mkdirSync(join(repo, '.git'), { recursive: true });
...
describe('review regressions: pid reuse', () => {
  it('does not classify a non-node process reusing the recorded pid as a live server', () => {
    const repo = realpathSync(mkdtempSync(join(tmpdir(), 'pictorial-roots-pidreuse-')));
    try {
      mkdirSync(join(repo, '.git'), { recursive: true });
      for (const name of ['appA', 'appB']) {
        write(repo, `${name}/vite.config.js`, 'export default {};');
      ...
      write(repo, 'appB/.pictorial/live/server.json', JSON.stringify({ pid: 1, port: 2, token: 't' }));
      // A holds the interrupted session the user is recovering.
      write(repo, 'appA/.pictorial/live/sessions/aa11bb22.snapshot.json',
...
describe('review regressions: discovery parity', () => {
  it('discovers a live-configured static site with no bundler markers', () => {
    const repo = realpathSync(mkdtempSync(join(tmpdir(), 'pictorial-roots-static-')));
    try {
      mkdirSync(join(repo, '.git'), { recursive: true });
      write(repo, 'package.json', '{"name":"cli"}');
      write(repo, 'docs-site/.pictorial/live/config.json', '{"files":["index.html"]}');
