...
describe('buildInsertWrapperLines', () => {
  it('produces an insert scaffold without an original variant', () => {
    ...
    assert.match(joined, /pictorial-variants-start abc12345/);
    assert.match(joined, /data-pictorial-variants="abc12345"/);
    assert.match(joined, /data-pictorial-mode="insert"/);
    assert.match(joined, /Variants: insert below this line/);
    assert.doesNotMatch(joined, /data-pictorial-variant="original"/);
  });
  ...
  it('keeps marker comments inside the wrapper for JSX', () => {
    ...
    const joined = lines.join('\n');
    assert.match(joined, /^\s*<div data-pictorial-variants="jsx12345"/m);
    assert.match(joined, /{\/\* pictorial-variants-start jsx12345 \*\/}/);
    assert.doesNotMatch(joined, /data-pictorial-variant="original"/);
  });
});
...
describe('live-insert CLI integration', () => {
  ...
  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), 'pictorial-insert-test-'));
  });
...
<body>
  ...
  <section class="features">
    ...
    assert.match(after, /data-pictorial-mode="insert"/);
    assert.match(after, /pictorial-variants-start ins12345/);
    assert.ok(after.indexOf('class="hero"') < after.indexOf('pictorial-variants-start ins12345'));
    assert.ok(after.indexOf('pictorial-variants-start ins12345') < after.indexOf('class="features"'));
    assert.doesNotMatch(after, /data-pictorial-variant="original"/);
  });
  ...
  <section class="cta">CTA</section>
    ...
    assert.ok(after.indexOf('pictorial-variants-start ins99999') < after.indexOf('class="cta"'));
    assert.ok(after.indexOf('class="hero"') < after.indexOf('pictorial-variants-start ins99999'));
  });
  ...
  return (
    ...
    assert.match(after, /data-pictorial-mode="insert"/);
    assert.match(after, /{\/\* pictorial-variants-start jsxins01 \*\/}/);
  });
