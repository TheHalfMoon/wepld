 ...
 *
 * Selector strategy: live-browser.js uses deterministic ids (`pictorial-live-*`)
...
const BAR_ID = '#pictorial-live-bar';
const GLOBAL_BAR_ID = '#pictorial-live-global-bar';
const PICKER_ID = '#pictorial-live-picker';
const EDIT_BADGE_ID = '#pictorial-live-edit-badge';
const PENDING_DOCK_ID = '#pictorial-live-pending-dock';
...
const STEER_CHAT_ID = '#pictorial-live-page-chat';
const STEER_INPUT_ID = '#pictorial-live-page-chat-input';
const PICK_TOGGLE = '#pictorial-live-pick-toggle';
...
const INSERT_TOGGLE = '#pictorial-live-insert-toggle';
const DETECT_TOGGLE = '#pictorial-live-detect-toggle';
const DETECT_BADGE = '#pictorial-live-detect-badge';
const DESIGN_TOGGLE = '#pictorial-live-design-toggle';
const DESIGN_HOST = '#pictorial-live-design-host';
const EXIT_BUTTON = '#pictorial-live-exit';
const INSERT_INPUT_ID = '#pictorial-live-insert-input';
const INSERT_CREATE_ID = '#pictorial-live-insert-create';
const ANNOTATION_ID = '#pictorial-live-annot';
const ANNOTATION_PINS_ID = '#pictorial-live-annot-pins';
const ANNOTATION_CLEAR_ID = '#pictorial-live-annot-clear';

/**
 * Wait for the live handshake to complete:
 *   - window.__PICTORIAL_LIVE_INIT__ set
 *   - global bar mounted
...
export async function waitForHandshake(page, { timeout = 20_000 } = {}) {
  ...
  await page.waitForFunction(
    (sel) => Boolean(window.__pictorialLiveQuery?.(sel)),
    GLOBAL_BAR_ID,
    { timeout },
  );
...
export async function assertBottomBarIdle(page, { timeout = 5_000 } = {}) {
  await installLiveQueryHelpers(page);
  await page.waitForFunction(
    ({ ids }) => ids.every((sel) => Boolean(window.__pictorialLiveQuery(sel))),
    {
      ids: [
        GLOBAL_BAR_ID,
        PICK_TOGGLE,
        ...
        STEER_CHAT_ID,
        STEER_INPUT_ID,
        '#pictorial-live-page-chat-voice',
        EXIT_BUTTON,
      ],
    },
    { timeout },
  ...
  const snapshot = await page.evaluate(({ pickSel, insertSel, detectSel, designSel }) => {
    const q = window.__pictorialLiveQuery;
    return {
...
async function clickLiveControl(page, selector) {
  ...
  const clicked = await page.evaluate((sel) => {
    const el = window.__pictorialLiveQuery(sel);
...
async function readControlActive(page, selector) {
  await installLiveQueryHelpers(page);
  return page.evaluate((sel) => window.__pictorialLiveQuery(sel)?.dataset.active === 'true', selector);
}
...
async function ensureLiveControlActive(page, selector, active) {
  ...
  await page.waitForFunction(
    ({ sel, expected }) => window.__pictorialLiveQuery(sel)?.dataset.active === (expected ? 'true' : 'false'),
...
export async function runPickInsertToggleSmoke(page) {
  ...
  await page.waitForFunction(
    ({ pickSel, insertSel }) =>
      window.__pictorialLiveQuery(pickSel)?.dataset.active === 'true'
      && window.__pictorialLiveQuery(insertSel)?.dataset.active === 'false',
  ...
  await page.waitForFunction(
    ({ pickSel, insertSel }) =>
      window.__pictorialLiveQuery(pickSel)?.dataset.active === 'false'
      && window.__pictorialLiveQuery(insertSel)?.dataset.active === 'true',
    { pickSel: PICK_TOGGLE, insertSel: INSERT_TOGGLE },
    { timeout: 5_000 },
  );
...
export async function runDetectSmoke(page, { expectMinCount = 1 } = {}) {
  ...
  await page.waitForFunction(
    ({ badgeSel, expectMin }) => {
      const badge = window.__pictorialLiveQuery(badgeSel);
      const count = parseInt(badge?.textContent || '0', 10);
      const overlays = document.querySelectorAll('.pictorial-overlay').length;
  ...
  await ensureLiveControlActive(page, PICK_TOGGLE, true);
  await page.waitForFunction(
    () => [...document.querySelectorAll('.pictorial-overlay')]
  ...
  await page.waitForFunction(
    ({ badgeSel }) => {
      const badge = window.__pictorialLiveQuery(badgeSel);
      return document.querySelectorAll('.pictorial-overlay').length === 0
...
export async function runDesignPanelSmoke(page, { title = '', rawText = '' } = {}) {
  ...
  await page.waitForFunction(
    ({ hostSel, titleText }) => {
      const host = window.__pictorialLiveQuery(hostSel);
  ...
  await page.evaluate((hostSel) => {
    const root = window.__pictorialLiveQuery(hostSel)?.shadowRoot;
    const raw = [...(root?.querySelectorAll('.tab') || [])].find((btn) => /Raw/i.test(btn.textContent || ''));
    raw?.click();
  }, DESIGN_HOST);
  await page.waitForFunction(
    ({ hostSel, expected }) => {
      const text = window.__pictorialLiveQuery(hostSel)?.shadowRoot?.textContent || '';
  ...
  await page.waitForFunction(
    ({ hostSel, toggleSel }) => {
      const panel = window.__pictorialLiveQuery(hostSel)?.shadowRoot?.querySelector('.panel');
      const toggle = window.__pictorialLiveQuery(toggleSel);
...
export async function clickExitLiveMode(page) {
  ...
  await page.waitForFunction(
    ({ barSel }) => {
      const bar = window.__pictorialLiveQuery?.(barSel);
      return window.__PICTORIAL_LIVE_INIT__ === false && (!bar || !bar.isConnected);
    },
...
export async function drawAnnotationPinAndStroke(page, {
  ...
  await page.waitForFunction(
    (pinsSel) => Boolean(window.__pictorialLiveQuery(pinsSel)?.querySelector('input')),
    ANNOTATION_PINS_ID,
    { timeout: 5_000 },
  );
  await page.evaluate(({ pinsSel, value }) => {
    const input = window.__pictorialLiveQuery(pinsSel)?.querySelector('input');
  ...
  await page.waitForFunction(
    ({ pinsSel, expected }) => {
      const pins = window.__pictorialLiveQuery(pinsSel);
  ...
  await page.waitForFunction(
    ({ annotSel, clearSel }) => {
      const annot = window.__pictorialLiveQuery(annotSel);
      const clear = window.__pictorialLiveQuery(clearSel);
...
async function waitForAnnotationRect(page) {
  ...
  return page.evaluate((sel) => {
    const rect = window.__pictorialLiveQuery(sel).getBoundingClientRect();
    return {
      left: rect.left,
      top: rect.top,
      width: rect.width,
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    if (attempt === 2) {
      const snapshot = await page.evaluate(({ selector, barSel, pickSel }) => {
        ...
        return {
          liveState: window.__PICTORIAL_LIVE_STATE__ || null,
...
async function hideAnnotationOverlay(page) {
  await page.evaluate(() => {
    const annot = window.__pictorialLiveQuery('#pictorial-live-annot');
...
async function ensurePickerActive(page) {
  ...
  const clicked = await page.evaluate((sel) => {
    const btn = window.__pictorialLiveQuery(sel);
    if (!btn) return false;
    btn.click();
    return true;
  }, PICK_TOGGLE_ID);
  ...
  await page.waitForFunction(
    (sel) => window.__pictorialLiveQuery(sel)?.dataset.active === 'true',
    PICK_TOGGLE_ID,
    { timeout: 5_000 },
  );
}
...
/** Select a named Pictorial sub-command from the configure-row picker. */
export async function selectAction(page, action) {
  const pickerSelector = '#pictorial-live-picker';
  const opened = await page.evaluate(({ barSel, pickerSel }) => {
    const query = window.__pictorialLiveQuery || ((selector) => document.querySelector(selector));
    const bar = query(barSel);
...
export async function waitForCycling(page, expectedCount, { timeout = 30_000 } = {}) {
  ...
  try {
    ...
    ({ barSel, expected }) => {
      ...
      if (!m) return false;
      const wrapper = window.__pictorialLiveQuery('[data-pictorial-variants]');
      const debugState = window.__PICTORIAL_LIVE_CHROME_CORE__?.debugState?.();
      const arrived = /^(?:svelte|vue)-component$/.test(wrapper?.dataset.pictorialPreview || '')
        ? Number(debugState?.arrivedVariants || 0)
        : wrapper
          ? wrapper.querySelectorAll('[data-pictorial-variant]:not([data-pictorial-variant="original"])').length
          : 0;
    ...
    if (process.env.PICTORIAL_E2E_DEBUG) {
      const snapshot = await page.evaluate((barSel) => {
        const query = window.__pictorialLiveQuery || ((sel) => document.querySelector(sel));
        const root = window.__PICTORIAL_LIVE_CHROME_CORE__?.root?.() || window.__PICTORIAL_LIVE_UI_ROOT__ || null;
        const bar = query(barSel);
        const toast = query('#pictorial-live-toast');
        const wrapper = query('[data-pictorial-variants]');
        return {
          liveInit: window.__PICTORIAL_LIVE_INIT__,
          adapter: window.__PICTORIAL_LIVE_ADAPTER__,
          rootText: root?.textContent?.replace(/\s+/g, ' ').trim().slice(0, 600) || null,
          bar: bar ? { display: bar.style.display, text: bar.textContent } : null,
          toast: toast?.textContent || null,
          wrapper: wrapper ? { preview: wrapper.dataset.pictorialPreview, count: wrapper.dataset.pictorialVariantCount, html: wrapper.outerHTML.slice(0, 600) } : null,
          debugState: window.__PICTORIAL_LIVE_CHROME_CORE__?.debugState?.() || null,
          storage: localStorage.getItem('pictorial-live-session'),
          scripts: document.querySelectorAll('script[data-pictorial-live-script]').length,
...
export async function waitForVariantSettled(page, expected, count, { timeout = 15_000 } = {}) {
  ...
  try {
    ...
    const debugState = await page
      .evaluate(() => window.__PICTORIAL_LIVE_CHROME_CORE__?.debugState?.() || null)
      .catch(() => null);
    throw new Error(
    ...
    return await withTimeout(
      page.evaluate((barSel) => {
        ...
        if (wrapper) {
          ...
          const idx = visible ? parseInt(visible.dataset.pictorialVariant || '0', 10) : 0;
          if (idx > 0) return idx;
        }
        const bar = window.__pictorialLiveQuery(barSel);
        if (!bar) return null;
...
const PARAMS_PANEL_ID = '#pictorial-live-params-panel';
...
export async function openTunePanel(page, { timeout = 5_000 } = {}) {
  ...
  await page.waitForFunction(
    (sel) => {
      const tune = window.__pictorialLiveQuery(sel);
  ...
  await page.waitForFunction(
    (sel) => (window.__pictorialLiveQuery(sel)?.querySelectorAll(':scope > div > div').length || 0) > 0,
    PARAMS_PANEL_ID,
    { timeout },
  );
}
...
export async function setTuneRange(page, label, value) {
  ...
  const applied = await page.evaluate(({ panelSel, label, value }) => {
    const panel = window.__pictorialLiveQuery(panelSel);
...
async function describeTunePanel(page) {
  const snapshot = await page.evaluate((panelSel) => {
    ...
    const debugState = window.__PICTORIAL_LIVE_CHROME_CORE__?.debugState?.() || null;
    return {
      rows,
...
const MOUNT_ERROR_ID = '#pictorial-live-mount-error';
const MOUNT_RETRY = '[data-pictorial-mount-retry="true"]';
...
export async function waitForMountErrorCard(page, { variant, timeout = 20_000 } = {}) {
  ...
  await page.waitForFunction(
    ({ sel, variant }) => {
      const card = window.__pictorialLiveQuery(sel);
  ...
  return page.evaluate((sel) => window.__pictorialLiveQuery(sel)?.textContent || '', MOUNT_ERROR_ID);
}

export async function isMountErrorCardVisible(page) {
  await installLiveQueryHelpers(page);
  return page.evaluate((sel) => Boolean(window.__pictorialLiveQuery(sel)), MOUNT_ERROR_ID);
}
...
export async function clickMountRetry(page) {
  ...
  const clicked = await page.evaluate(({ cardSel, retrySel }) => {
    const button = window.__pictorialLiveQuery(cardSel)?.querySelector(retrySel);
...
export async function waitForMountErrorCardGone(page, { timeout = 20_000 } = {}) {
  await installLiveQueryHelpers(page);
  await page.waitForFunction(
    (sel) => !window.__pictorialLiveQuery(sel),
    MOUNT_ERROR_ID,
    { timeout },
  );
}
...
export async function waitForComputedFontWeight(page, selector, expected, { timeout = 10_000 } = {}) {
  try {
    ...
    const snapshot = await page.evaluate((sel) => {
      ...
      const debugState = window.__PICTORIAL_LIVE_CHROME_CORE__?.debugState?.() || null;
      const mount = document.querySelector('[data-pictorial-component-mount]');
      return {
        found: Boolean(el),
...
export async function clickEditCopy(page) {
  ...
  await page.waitForFunction(
    () => window.__pictorialLiveQuery('[data-pictorial-editable="true"]')?.isContentEditable === true,
    { timeout: 5_000 },
  );
}
...
export async function clickSaveEdit(page) {
  ...
  await page.waitForFunction(
    () => !window.__pictorialLiveQuery('[data-pictorial-editable="true"]'),
    { timeout: 5_000 },
  );
}
...
async function clickEditBadgeButton(page, label) {
  const proxyRect = await page.evaluate((text) => {
    const proxies = [...document.querySelectorAll('[data-pictorial-edit-badge-proxy="true"]')];
...
export async function assertApplyDockVisible(page, expectedCount, { timeout = 5_000 } = {}) {
  await page.waitForFunction(
    ({ dockSel, expected }) => {
      const dock = window.__pictorialLiveQuery(dockSel);
...
export async function preparePageForBarInteraction(page) {
  await page.evaluate(() => {
    for (const el of window.__pictorialLiveQueryAll('astro-dev-toolbar')) {
...
export async function waitForSteerInputFocused(page, { timeout = 5_000 } = {}) {
  await page.waitForFunction(
    (inputSel) => {
      const input = window.__pictorialLiveQuery(inputSel);
      const active = window.__PICTORIAL_LIVE_CHROME_CORE__?.activeElementDeep?.()
...
export async function waitForSteerInputValue(page, value, { timeout = 5_000 } = {}) {
  await page.waitForFunction(
    ({ inputSel, value: expected }) => window.__pictorialLiveQuery(inputSel)?.value === expected,
...
export async function waitForSteerLocked(page, { timeout = 5_000 } = {}) {
  await page.waitForFunction(
    (sel) => window.__pictorialLiveQuery(sel)?.dataset.processing === 'true',
    STEER_CHAT_ID,
    { timeout },
  );
}
...
export async function waitForSteerUnlocked(page, { timeout = 15_000 } = {}) {
  await page.waitForFunction(
    (sel) => {
      const chat = window.__pictorialLiveQuery(sel);
      const input = window.__pictorialLiveQuery('#pictorial-live-page-chat-input');
...
async function ensureToggleActive(page, selector, shouldBeActive) {
  ...
  await page.locator(selector).click({ timeout: 5_000 });
  await page.waitForFunction(
    ({ sel, active }) => window.__pictorialLiveQuery(sel)?.dataset.active === (active ? 'true' : 'false'),
...
export async function runInsertFlow(page, {
  ...
  await page.waitForFunction(() => {
    const line = window.__pictorialLiveQuery('#pictorial-live-insert-line');
  ...
  await page.waitForFunction(
    ({ sel, value }) => window.__pictorialLiveQuery(sel)?.value === value,
