"""``agile bundle`` command group — discover, install, author Agile bundles.
...
bundle_app = typer.Typer(
    name="bundle",
    help="Discover, install, and author Agile bundles",
...
def _user_config_dir() -> Path:
    # User-scope Agile config lives under ~/.agile (same convention as
...
def _agile_version() -> str:
    from ..._assets import get_agile_version

    return get_agile_version()
...
def _run_init(integration: str, *, script_type: str, offline: bool = False) -> None:
    """Idempotently scaffold a Agile project here via the existing ``init`` machinery.

    Reuses the real ``agile init`` command callback in-process (Principle I)
    ...
    try:
        ...
        if exc.exit_code:
            raise BundlerError(
                f"Failed to initialize a Agile project (integration '{integration}')."
            ) from exc
...
def bundle_info(
    ...
    if as_json:
        payload = {
            ...
            "requires": {"agile_version": entry.requires_agile_version},
    ...
    if entry.requires_agile_version:
        console.print(
            f"  Requires Agile: "
            f"{_escape_markup(str(entry.requires_agile_version))}"
        )
...
def bundle_list(
    ...
    if not records:
        ...
        console.print("\nInstall one with: [cyan]agile bundle install <id>[/cyan]")
        return
...
def bundle_install(
    ...
    try:
        ...
        if project_root is None:
            ...
            # Resolve all hard compatibility gates before ``agile init``.
            ...
            resolve_install_plan(
                manifest,
                agile_version=_agile_version(),
                active_integration=init_integration,
                integration_explicit=True,
            )
            console.print(
                f"[cyan]No Agile project here; initializing with integration "
...
def bundle_init(
    ...
    try:
        ...
        if project_root is None:
            ...
            console.print(
                f"[cyan]Initializing a Agile project with integration "
                f"'{_escape_markup(str(init_integration))}'…[/cyan]"
            )
            _run_init(init_integration, script_type=_default_script_type(), offline=offline)
            project_root = require_project_root()
    ...
    console.print(
        f"[green]✓[/green] Agile project ready at "
...
def _download_manifest(resolved, *, offline: bool):
    ...
    (``agile bundle install ./path/to/bundle.yml`` — a bundle directory or a
    ...
    if scheme in ("", "file") or re.match(r"^[A-Za-z]:[\\/]", url):
        raise BundlerError(
            ...
            "'agile bundle install <path-to-bundle.yml | bundle-dir | .zip>'."
        )
