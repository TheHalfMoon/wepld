...
/**
 * Anti-Pattern Browser Detector for Pictorial
...
// ─── Scoped ignores: data-pictorial-ignore ─────────────────────────────────
//
// An element-scoped waiver that travels with the markup: any element carrying
// `data-pictorial-ignore="rule-a rule-b"` (or `*`, or an empty value, for
...
// `pictorial-disable` comment directives, which the browser cannot apply (a
// live DOM has no line numbers), and the generalization of the one-off
// `data-pictorial-allow-kickers` opt-out.
//
...
function scopedIgnoreActive(el, ruleId) {
  ...
  while (cur && cur.nodeType === 1) {
    const attr = typeof cur.getAttribute === 'function' ? cur.getAttribute('data-pictorial-ignore') : null;
    if (attr != null) {
...
function checkElementQualityDOM(el) {
  ...
  const rect = el.getBoundingClientRect();
  const lineMax = (typeof window !== 'undefined' && window.__PICTORIAL_CONFIG__?.lineLengthMax) || 80;
...
function checkTypography() {
  ...
  for (const el of document.querySelectorAll('p, h1, h2, h3, h4, h5, h6, li, td, th, dd, blockquote, figcaption, a, button, label, span')) {
    // Skip pictorial's own elements
    if (el.closest && el.closest('.pictorial-overlay, .pictorial-label, .pictorial-banner, .pictorial-tooltip')) continue;
...
if (IS_BROWSER) {
  ...
  const EXTENSION_MODE = (_myScript && _myScript.dataset.pictorialExtension === 'true')
    || document.documentElement.dataset.pictorialExtension === 'true';
  ...
  styleEl.textContent = `
    @keyframes pictorial-reveal {
      from { opacity: 0; }
      to { opacity: 1; }
    }
    .pictorial-overlay:not(.pictorial-banner) {
      pointer-events: none;
      outline: 2px solid ${OUTLINE_COLOR};
      border-radius: 4px;
      transition: outline-color 0.15s ease;
      animation: pictorial-reveal 0.4s cubic-bezier(0.16, 1, 0.3, 1) both;
      animation-play-state: paused;
      border-top-left-radius: 0;
    }
    .pictorial-overlay.pictorial-visible {
      animation-play-state: running;
    }
    .pictorial-overlay.pictorial-hover {
      outline-color: ${BRAND_COLOR_HOVER};
      z-index: 100001 !important;
    }
    .pictorial-overlay.pictorial-hover .pictorial-label {
      background: ${BRAND_COLOR_HOVER};
    }
    .pictorial-overlay.pictorial-spotlight {
      z-index: 100002 !important;
    }
    .pictorial-overlay.pictorial-spotlight-dimmed {
      opacity: 0.15 !important;
      animation: none !important;
      filter: blur(3px);
    }
    .pictorial-spotlight-backdrop {
      position: fixed;
    ...
    .pictorial-spotlight-backdrop.pictorial-visible {
      opacity: 1;
    }
    .pictorial-hidden .pictorial-overlay${EXTENSION_MODE ? '' : ':not(.pictorial-banner)'} {
  ...
  function getSpotlightBackdrop() {
    if (!spotlightBackdrop) {
      ...
      spotlightBackdrop.className = 'pictorial-spotlight-backdrop';
  ...
  function showSpotlight(target) {
    ...
    if (window.__PICTORIAL_CONFIG__?.spotlightBlur === false) {
    ...
    bd.classList.add('pictorial-visible');
  }

  function hideSpotlight() {
    spotlightTarget = null;
    if (spotlightBackdrop) spotlightBackdrop.classList.remove('pictorial-visible');
  }
  ...
  function repositionOverlays() {
    for (const o of overlays) {
      if (!o._targetEl || o.classList.contains('pictorial-banner')) continue;
  ...
  const visibilityObserver = new IntersectionObserver((entries) => {
    for (const entry of entries) {
      const overlay = entry.target._pictorialOverlay;
      if (!overlay) continue;
      if (entry.isIntersecting) {
        ...
        if (!overlay._revealed) {
          ...
          requestAnimationFrame(() => {
            overlay.classList.add('pictorial-visible');
  ...
  function detachOverlay(overlay) {
    ...
    if (overlay._targetEl && overlay._targetEl._pictorialOverlay === overlay) {
      visibilityObserver.unobserve(overlay._targetEl);
      delete overlay._targetEl._pictorialOverlay;
    }
  ...
  document.addEventListener('transitionend', (e) => {
    ...
    for (const o of overlays) {
      if (!o._targetEl || o.classList.contains('pictorial-banner') || o.style.display === 'none') continue;
  ...
  const highlight = function(el, findings) {
    ...
    label.className = 'pictorial-label';
    ...
    const onMouseEnter = () => {
      isHovered = true;
      outline.classList.add('pictorial-hover');
    ...
    const onMouseLeave = () => {
      isHovered = false;
      outline.classList.remove('pictorial-hover');
  ...
  const showPageBanner = function(findings) {
    ...
    banner.className = 'pictorial-overlay pictorial-banner';
    ...
    if (!EXTENSION_MODE) {
      ...
      toggle.addEventListener('click', () => {
        ...
        document.body.classList.toggle('pictorial-hidden', !overlaysVisible);
  ...
  function buildSelectorSegment(el) {
    ...
    if (el.classList && el.classList.length > 0) {
      const classes = [...el.classList]
        .filter(c => !c.startsWith('pictorial-') && !isLikelyHashedClass(c))
        .slice(0, 2);
      if (classes.length > 0) {
  ...
  function collectVisualContrastCandidates(options = {}) {
    ...
    for (const el of document.querySelectorAll('*')) {
      ...
      if (el.closest('.pictorial-overlay, .pictorial-label, .pictorial-banner, .pictorial-tooltip')) continue;
      if (el.closest('[id^="pictorial-live-"]')) continue;
  ...
  async function sampleVisualBackgroundAtPoint(el, point, textColor, depth = 0) {
    ...
    for (const node of nodes) {
      ...
      if (node.closest?.('.pictorial-overlay, .pictorial-label, .pictorial-banner, .pictorial-tooltip')) continue;
  ...
  const printSummary = function(allFindings) {
    if (allFindings.length === 0) {
      console.log('%c[pictorial] No anti-patterns found.', 'color: #22c55e; font-weight: bold');
      return;
    }
    console.group(
      `%c[pictorial] ${allFindings.length} anti-pattern${allFindings.length === 1 ? '' : 's'} found`,
  ...
  function addBrowserFindings(groupMap, el, findings) {
    ...
    // Element-scoped waivers: a data-pictorial-ignore ancestor suppresses
  ...
  function browserDesignSystemConfig() {
    const raw = window.__PICTORIAL_CONFIG__?.designSystem;
  ...
  function collectBrowserFindings() {
    ...
    const _disabled = EXTENSION_MODE ? (window.__PICTORIAL_CONFIG__?.disabledRules || []) : [];
    ...
    for (const el of document.querySelectorAll('*')) {
      // Skip pictorial's own elements and any descendants (overlays, labels, banner, nav buttons)
      if (el.closest('.pictorial-overlay, .pictorial-label, .pictorial-banner, .pictorial-tooltip')) continue;
      ...
      // Skip the pictorial live-mode overlay (highlight, tooltip, bar, picker, toast).
    ...
    // Clone the document and strip pictorial-live overlay nodes before the
    ...
    for (const node of docClone.querySelectorAll('[id^="pictorial-live-"]')) {
      node.remove();
    }
    ...
    // ground truth in the browser), and a match under a data-pictorial-ignore
  ...
  function visualContrastMode(options = {}) {
    const explicit = typeof options.visualContrast === 'boolean'
      ...
      : typeof window.__PICTORIAL_CONFIG__?.visualContrast === 'boolean'
        ? window.__PICTORIAL_CONFIG__.visualContrast
        : null;
  ...
  function visualContrastOptions(options = {}) {
    const config = window.__PICTORIAL_CONFIG__ || {};
  ...
  function postExtensionError(err) {
    if (!EXTENSION_MODE) return;
    window.postMessage({
      source: 'pictorial-error',
      message: err?.message || String(err),
    }, '*');
  }

  function reportVisualContrastError(err, detail = {}) {
    window.dispatchEvent(new CustomEvent('pictorial-visual-contrast-error', {
      detail: {
        ...detail,
        message: err?.message || String(err),
      },
    ...
    if (EXTENSION_MODE) {
      ...
      console.warn('[pictorial] visual contrast scan failed', err);
    }
  }
        ...
        waitForVisualPaint()
          ...
          .then(result => {
            ...
            if (added) {
              ...
              window.dispatchEvent(new CustomEvent('pictorial-visual-contrast-resolved', {
                detail: {
  ...
  if (EXTENSION_MODE) {
    ...
    window.addEventListener('message', (e) => {
      if (e.source !== window || !e.data || e.data.source !== 'pictorial-command') return;
      if (e.data.action === 'scan') {
        if (e.data.config) window.__PICTORIAL_CONFIG__ = e.data.config;
        try {
          scan(e.data.config || {});
        } catch (err) {
          postExtensionError(err);
      ...
      if (e.data.action === 'toggle-overlays') {
        const visible = !document.body.classList.contains('pictorial-hidden');
        document.body.classList.toggle('pictorial-hidden', visible);
        window.postMessage({ source: 'pictorial-overlays-toggled', visible: !visible }, '*');
      }
      if (e.data.action === 'remove') {
        clearOverlays();
        styleEl.remove();
        if (spotlightBackdrop) { spotlightBackdrop.remove(); spotlightBackdrop = null; }
        document.body.classList.remove('pictorial-hidden');
      }
      if (e.data.action === 'highlight') {
        try {
          ...
          if (target) {
            ...
            for (const o of overlays) {
              if (o.classList.contains('pictorial-banner')) continue;
              const isMatch = o._targetEl === target;
              o.classList.toggle('pictorial-spotlight', isMatch);
              o.classList.toggle('pictorial-spotlight-dimmed', !isMatch);
              if (isMatch) {
                // Force the matching overlay visible immediately, don't wait for IntersectionObserver
                o.style.display = '';
                o.style.animation = 'none';
                o.classList.add('pictorial-visible');
                o._revealed = true;
                positionOverlay(o);
              }
            }
      ...
      if (e.data.action === 'unhighlight') {
        hideSpotlight();
        for (const o of overlays) {
          o.classList.remove('pictorial-spotlight');
          o.classList.remove('pictorial-spotlight-dimmed');
        }
      }
    });
    window.postMessage({ source: 'pictorial-ready' }, '*');
  } else {
    if (window.__PICTORIAL_CONFIG__?.autoScan !== false) {
      const runAutoScan = () => {
        try {
          scan();
        } catch (err) {
          console.warn('[pictorial] scan failed', err);
        }
      };
  ...
  window.pictorialDetect = detect;
  window.pictorialDetectAsync = detectAsync;
  window.pictorialScan = scan;
  window.pictorialScanAsync = scanAsync;
  ...
  window.pictorialMeasureHiddenText = measureHiddenTextDOM;
  window.pictorialCollectVisualContrastCandidates = collectVisualContrastCandidates;
  window.pictorialAnalyzeVisualContrast = analyzeVisualContrast;
  window.pictorialGetLastVisualContrastAnalyses = () => lastVisualContrastAnalyses.slice();
}
