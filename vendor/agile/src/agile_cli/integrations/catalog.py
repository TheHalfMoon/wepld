...
class IntegrationCatalog(CatalogStackBase):
    ...
    def __init__(self, project_root: Path) -> None:
        ...
        self.cache_dir = project_root / ".agile" / "integrations" / ".cache"

    def get_active_catalogs(self) -> List[IntegrationCatalogEntry]:
        """Return the ordered list of active integration catalogs.

        Resolution:
        1. ``AGILE_INTEGRATION_CATALOG_URL`` env var
        2. Project ``.agile/integration-catalogs.yml``
        3. User ``~/.agile/integration-catalogs.yml``
        4. Built-in defaults (built-in + community)
        """
        import sys

        env_value = os.environ.get("AGILE_INTEGRATION_CATALOG_URL", "").strip()
        if env_value:
            ...
            return [
                IntegrationCatalogEntry(
                    ...
                    description="Custom catalog via AGILE_INTEGRATION_CATALOG_URL",
                )
            ]

        project_cfg = self.project_root / ".agile" / self.CONFIG_FILENAME
        catalogs = self._load_catalog_config(project_cfg)
        if catalogs is not None:
            return catalogs

        user_cfg = Path.home() / ".agile" / self.CONFIG_FILENAME
    ...
    def get_project_catalog_configs(self) -> Optional[List[Dict[str, Any]]]:
        ...
        config_path = self.project_root / ".agile" / self.CONFIG_FILENAME
    ...
    def remove_catalog(self, index: int) -> str:
        ...
        if any(_is_removable_catalog_entry(item) for item in catalogs):
            ...
            # manually deletes `.agile/integration-catalogs.yml`.
...
class IntegrationDescriptor:
    ...
    def _validate(self) -> None:
        ...
        if "agile_version" not in requires:
            raise IntegrationDescriptorError(
                "Missing requires.agile_version"
            )
        if not isinstance(requires["agile_version"], str) or not requires["agile_version"].strip():
            raise IntegrationDescriptorError(
                "requires.agile_version must be a non-empty string"
            )
