...
from agile_cli.integrations.manifest import IntegrationManifest, _sha256
...
class TestManifestPersistence:
    ...
    def test_manifest_path(self, tmp_path):
        ...
        assert m.manifest_path == tmp_path / ".agile" / "integrations" / "copilot.manifest.json"
...
class TestManifestLoadValidation:
    def test_load_non_dict_raises(self, tmp_path):
        path = tmp_path / ".agile" / "integrations" / "bad.manifest.json"
    ...
    def test_load_filters_recovered_files_not_in_files(self, tmp_path):
        ...
        path = tmp_path / ".agile" / "integrations" / "test.manifest.json"
        path.parent.mkdir(parents=True)
...
class TestManifestUnreadableFile:
    ...
    def test_check_modified_treats_unreadable_as_modified(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "agile_cli.integrations.manifest._sha256", raise_perm
        )
