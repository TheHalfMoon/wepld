...
generated `.agile/memory/constitution.md` when constitution-providing presets are installed,
...
**entire** Agile ecosystem — not just the SDD commands, but every command, template, script and
...
It also ships a single `wrap`-strategy override of `agile.constitution`. It composes on top of the
...
- Aligns `plan/spec/tasks-template.md` in `.agile/templates/` with the updated principles.
- Updates **project-local** command files and guidance docs to correct stale references.
- Extends the Sync Impact Report in `.agile/memory/constitution.md` with the files it touched.
...
- It does **not** edit versioned, package-owned files — templates or command files provided or
  ...
  only ever writes into your project's own `.agile/templates/` scaffolds and command files that
...
The preset resolution stack is how Agile composes templates and commands going forward: they are
...
`plan`/`agile`/`tasks`/`analyze` commands or templates come from other presets or extensions,
