...
Kimi uses the ``.kimi-code/skills/agile-<name>/SKILL.md`` layout with
``/skill:agile-<name>`` invocation syntax.
...
class KimiIntegration(SkillsIntegration):
    ...
    def build_command_invocation(self, command_name: str, args: str = "") -> str:
        """Build Kimi's native skill invocation: ``/skill:agile-<stem>``.

        Kimi Code CLI invokes installed skills with a ``/skill:<name>``
        slash command (e.g. ``/skill:agile-plan``), not the bare
        ``/agile-<name>`` form produced by the generic skills base
        ...
        invocation = "/skill:agile-" + stem.replace(".", "-")
        if args:
        ...
        return [
            ...
            IntegrationOption(
                ...
                help=(
                    ...
                    ".kimi/skills/ → .kimi-code/skills/ and agile.xxx → agile-xxx"
                ),
            ),
        ]
    ...
    def setup(
        ...
        **opts: Any,
        ...
        # Run base setup first so new-path targets (agile-*) exist,
    ...
    def teardown(
        ...
        *,
        ...
        if _is_safe_legacy_dir(old_skills_dir, project_root):
            legacy_dirs = sorted(
                [*old_skills_dir.glob("agile-*"), *old_skills_dir.glob("agile.*")]
            )
            for legacy_dir in legacy_dirs:
                if legacy_dir.is_symlink() or not legacy_dir.is_dir():
                    continue
                if _is_agile_generated_skill(legacy_dir):
                    try:
...
def _migrate_legacy_kimi_skills_dir(
    ...
    Handles both hyphenated (``agile-xxx``) and dotted (``agile.xxx``)
    ...
    legacy_dirs = sorted(old_skills_dir.glob("agile-*")) + sorted(
        old_skills_dir.glob("agile.*")
    )
...
def _legacy_to_target_name(legacy_name: str) -> str:
    ...
    if legacy_name.startswith("agile-"):
        return legacy_name
    if legacy_name.startswith("agile."):
        suffix = legacy_name[len("agile.") :]
        if suffix:
            return f"agile-{suffix.replace('.', '-')}"
    return ""


def _is_agile_generated_skill(skill_dir: Path) -> bool:
    """Return True when *skill_dir* contains a Agile-generated SKILL.md.
    ...
    # A symlinked SKILL.md is never treated as Agile-generated, so teardown
    ...
    # Agile-generated skill would not be recognized on teardown).
...
def _migrate_legacy_kimi_dotted_skills(skills_dir: Path) -> tuple[int, int]:
    ...
    source and destination, so it processes every ``agile-*`` and
    ``agile.*`` entry under *skills_dir*. Because the two paths are
    ...
    ``agile.xxx`` dirs to hyphenated ``agile-xxx`` in place and never
