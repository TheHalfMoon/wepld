...
function printUsage() {
  console.log(`Usage: pictorial ignores <action> [options]

Manage detector ignores in .pictorial config.
...
Scope:
  --shared                              Write .pictorial/config.json (default)
  --local                               Write .pictorial/config.local.json
...
Examples:
  pictorial ignores add-file "src/legacy/**"
  pictorial ignores add-value overused-font Inter --reason "Brand font"
  pictorial ignores add-value design-system-color "*" --file "src/demo.css"
  pictorial ignores remove-value overused-font Inter`);
}
...
function parseValueArgs(args, { allowUnscopedWildcard = false } = {}) {
  ...
  if (!rule || !value) throw new Error('Pass a rule id and value, e.g. pictorial ignores add-value overused-font Inter');
...
function addRule(cwd, args) {
  ...
  if (!rule) throw new Error('Pass a rule id, e.g. pictorial ignores add-rule side-tab');
...
function addFile(cwd, args) {
  ...
  if (!glob) throw new Error('Pass a glob, e.g. pictorial ignores add-file "src/legacy/**"');
  const config = readScopeConfig(cwd, local);
...
function removeRule(cwd, args) {
  return removeFromScopes(cwd, args, (config, rest) => {
    ...
    if (!rule) throw new Error('Pass a rule id, e.g. pictorial ignores remove-rule side-tab');
...
function removeFile(cwd, args) {
  return removeFromScopes(cwd, args, (config, rest) => {
    const glob = String(rest[0] || '').trim();
    if (!glob) throw new Error('Pass a glob, e.g. pictorial ignores remove-file "src/legacy/**"');
...
export async function run(args = [], opts = {}) {
  ...
  if (!action) {
    throw new Error(`Unknown ignores action: ${actionArg}. Run "pictorial ignores --help".`);
  }
