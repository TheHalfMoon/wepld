...
export function stageFixture(name, fixture, { fixtureRoot = join(FIXTURES_DIR, name) } = {}) {
  ...
  const tmp = mkdtempSync(join(tmpdir(), 'pictorial-e2e-'));
  ...
  mkdirSync(join(appRoot, '.pictorial', 'live'), { recursive: true });
  writeFileSync(join(appRoot, '.pictorial', 'live', 'config.json'), JSON.stringify(fixture.config));
...
export function runInstall(tmp, command, { timeoutMs = readTimeoutEnv('PICTORIAL_E2E_INSTALL_TIMEOUT_MS', 180_000) } = {}) {
...
export function runLiveBoot(cwd, appRoot) {
  ...
  let pid = null;
  try {
    pid = JSON.parse(readFileSync(join(appRoot, '.pictorial', 'live', 'server.json'), 'utf-8')).pid;
...
export function startDevServer(tmp, runtime) {
  ...
  const ready = new Promise((resolve, reject) => {
    const readyTimeoutMs = readTimeoutEnv(
      'PICTORIAL_E2E_DEV_READY_TIMEOUT_MS',
...
export async function bootFixtureSession({
  ...
  try {
    ...
    page.on('console', (msg) => {
      ...
      } else if (process.env.PICTORIAL_E2E_CONSOLE && /\[pictorial\]|\[vite\]/.test(msg.text())) {
    ...
    if (process.env.PICTORIAL_E2E_CONSOLE) {
