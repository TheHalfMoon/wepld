 ...
 *   1. Check .pictorial/live/config.json (returns config_missing if first-ever run)
 ...
 *   - Open the project's live dev/preview URL in the browser (optional, if browser automation exists)—not `serverPort`; that port is the Pictorial helper for /live.js and /poll
...
import { readLiveServerInfo } from './lib/pictorial-paths.mjs';
...
Prepare everything for live variant mode in a single command:
  - Checks .pictorial/live/config.json (required, created once per project)
...
The agent should then:
  ...
  if (targetSelection) {
    console.log(JSON.stringify({
      ...
      ...targetSelection,
      hint: 'Ask the user which app Pictorial should use, then rerun live from that child app cwd. Use --target <path> only as a fallback or explicit path diagnostic.',
    }, null, 2));
    process.exit(0);
  }
  ...
  try {
    // Briefs live under .pictorial/surfaces, which in a nested-app repo sits
