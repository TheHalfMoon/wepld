...
function loadFactory(doc = createDocument(), extras = {}) {
  ...
  return { context, createHelpers: context.__PICTORIAL_LIVE_DOM__.createLiveBrowserDomHelpers };
}
...
describe('live-browser-dom helpers', () => {
  it('detects owned chrome and pickable page elements', () => {
    ...
    const helpers = createHelpers({ prefix: 'pictorial-live', skipTags: new Set(['script']) });

    assert.equal(helpers.own(createElement({ id: 'pictorial-live-bar' })), true);
    assert.ok(helpers.own(createElement({ closestResult: createElement({ id: 'pictorial-live-root' }) })));
  ...
  it('mounts chrome in the configured live UI root and styles in head by default', () => {
    ...
    const helpers = createHelpers({ prefix: 'pictorial-live', document: doc });
    const uiRoot = createAppendTarget();
    context.__PICTORIAL_LIVE_UI_ROOT__ = uiRoot;
    ...
    context.__PICTORIAL_LIVE_UI_ROOT__ = null;
    const styleInHead = {};
  ...
  it('escapes ids while looking inside the live UI root before document fallback', () => {
    ...
    context.__PICTORIAL_LIVE_UI_ROOT__ = {
      appendChild() {},
    ...
    const helpers = createHelpers({ prefix: 'pictorial-live', document: doc, css: context.CSS });
  ...
  it('freezes usable anchors and follows nested shadow active elements', () => {
    ...
    assert.equal(frozen.__pictorialFrozenAnchor, true);
  ...
  it('defangs modal outside handlers on live chrome roots', () => {
    ...
    const helpers = createHelpers({ prefix: 'pictorial-live' });
