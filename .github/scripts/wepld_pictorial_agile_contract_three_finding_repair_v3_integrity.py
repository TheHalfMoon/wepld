#!/usr/bin/env python3
"""Bounded follow-up repair for one post-merge Pictorial/Agile policy type-drift finding."""
...
POLICY_SCRIPT = ".github/scripts/wepld_pictorial_agile_contract_three_finding_repair_v3_integrity.py"
PRIOR_POLICY_PATH = ".github/scripts/wepld_pictorial_agile_contract_three_finding_repair_v2_integrity.py"
EXPECTED_PRIOR_POLICY_GIT_BLOB_SHA1 = "9695d610d618ddf9e1b31fe8ddd7ec1dc74e5678"
...
EXPECTED_TARGET_GIT_BLOB_SHA1 = "f3aedc464392d666ec55ae01d717207307c4c582"
...
REPAIR_AUTHORIZATION = "EXACT_ONE_POLICY_FAIL_CLOSED_TYPE_DRIFT_REPAIR"
REPAIR_FINDINGS = ("FAIL_CLOSED_MALFORMED_TOPOLOGY_TYPE_DRIFT",)
...
def _require_path_set(value: Any, label: str) -> frozenset[str]:
    if not isinstance(value, (set, frozenset)):
        base.fail(f"Pictorial/Agile type-drift repair {label} topology is malformed: expected path set")
    if any(not isinstance(path, str) for path in value):
        base.fail(f"Pictorial/Agile type-drift repair {label} topology is malformed: non-string path")
...
def _bind_prior_policy_before_import() -> None:
    ...
    if actual != EXPECTED_PRIOR_POLICY_GIT_BLOB_SHA1:
        base.fail(
            "frozen Pictorial/Agile repair-v2 policy drifted before import: "
            f"expected={EXPECTED_PRIOR_POLICY_GIT_BLOB_SHA1} actual={actual}"
        )


_bind_prior_policy_before_import()
import wepld_pictorial_agile_contract_three_finding_repair_v2_integrity as prior  # noqa: E402
...
def _topology() -> tuple[Any, Any, Any, Any, Any]:
    try:
        ...
        base.fail(f"Pictorial/Agile type-drift repair inherited topology is missing or malformed: {exc}")
    if not isinstance(topology, tuple) or len(topology) != 5:
        base.fail("Pictorial/Agile type-drift repair inherited topology is malformed: expected five components")
    return topology
...
def _activate_contract() -> None:
    try:
        ...
        base.fail(f"Pictorial/Agile type-drift repair activation topology is missing or malformed: {exc}")
    if current_target != EXPECTED_TARGET_GIT_BLOB_SHA1:
        base.fail(
            "Pictorial/Agile type-drift repair predecessor target drifted: "
            f"expected={EXPECTED_TARGET_GIT_BLOB_SHA1} actual={current_target}"
        )
    if current_workflows not in (
        dict(PRIOR_EXPECTED_WORKFLOW_SHA256),
        dict(EXPECTED_WORKFLOW_SHA256),
    ):
        base.fail("Pictorial/Agile type-drift repair predecessor workflow hashes drifted")
    prior.EXPECTED_WORKFLOW_SHA256 = dict(EXPECTED_WORKFLOW_SHA256)
    try:
        prior._activate_contract()
    except (AttributeError, TypeError) as exc:
        base.fail(f"Pictorial/Agile type-drift repair activation delegate is missing or malformed: {exc}")
...
def _require_prior_policy_base(view: base.RepositoryView) -> None:
    if PRIOR_POLICY_PATH not in _paths(view):
        base.fail("Pictorial/Agile type-drift repair requires the canonical repair-v2 predecessor policy")
    actual = _git_blob_sha1(view.read_bytes(PRIOR_POLICY_PATH, base.MAX_POLICY_FILE_BYTES))
    if actual != EXPECTED_PRIOR_POLICY_GIT_BLOB_SHA1:
        base.fail(
            "Pictorial/Agile type-drift repair base policy drifted: "
            f"expected={EXPECTED_PRIOR_POLICY_GIT_BLOB_SHA1} actual={actual}"
        )
...
def _delegate_exact_delta(candidate: base.RepositoryView, policy_base: base.RepositoryView) -> None:
    try:
        ...
        base.fail(f"Pictorial/Agile type-drift repair inherited exact-delta topology is missing: {exc}")
    if delegated is not PRIOR_REQUIRE_EXACT_DELTA:
        base.fail("Pictorial/Agile type-drift repair inherited exact-delta delegate drifted")
    if not callable(delegated):
        base.fail("Pictorial/Agile type-drift repair inherited exact-delta topology is malformed: delegate not callable")
    try:
        delegated(candidate, policy_base)
    except TypeError as exc:
        base.fail(f"Pictorial/Agile type-drift repair inherited exact-delta topology is malformed: {exc}")
...
def _require_exact_delta_repair(candidate: base.RepositoryView, policy_base: base.RepositoryView) -> None:
    ...
    try:
        ...
        base.fail(f"Pictorial/Agile type-drift repair changed-path topology is missing: {exc}")
    if not callable(changed_paths):
        base.fail("Pictorial/Agile type-drift repair changed-path topology is malformed: delegate not callable")
    try:
        changed = _require_path_set(changed_paths(candidate, policy_base), "changed-path")
    except TypeError as exc:
        base.fail(f"Pictorial/Agile type-drift repair changed-path topology is malformed: {exc}")
    ...
    if _is_bootstrap_base(policy_base):
        ...
        if changed & BOOTSTRAP_DELTA_PATHS:
            base.fail(
                "Pictorial/Agile type-drift repair bootstrap delta must be exactly the repair policy plus two workflows"
            )
        if TARGET_PATH in changed:
            base.fail("Pictorial/Agile contract cannot transition during type-drift repair bootstrap")
...
def _compare_base_controlled_repair(candidate: base.RepositoryView, policy_base: base.RepositoryView) -> None:
    ...
    try:
        ...
        base.fail(f"Pictorial/Agile type-drift repair base-control topology is missing: {exc}")
    for relative in sorted(controlled_paths):
        ...
        if relative in BOOTSTRAP_WORKFLOWS:
            ...
            if _sha256(cb) != expected_candidate:
                base.fail(f"Pictorial/Agile type-drift repair workflow candidate drifted: {relative}")
            expected_base = PRIOR_EXPECTED_WORKFLOW_SHA256[relative] if bootstrap else expected_candidate
            if _sha256(bb) != expected_base:
                base.fail(f"Pictorial/Agile type-drift repair trusted-base workflow drifted: {relative}")
            if not bootstrap and cb != bb:
                base.fail(f"Pictorial/Agile type-drift repair steady-state workflow changed: {relative}")
            continue
        if cb != bb:
...
def _verify_extension_paths(
    ...
    if POLICY_SCRIPT in safe_controlled:
        if POLICY_SCRIPT not in candidate_paths:
            base.fail("Pictorial/Agile type-drift repair policy wrapper is missing")
        if bootstrap:
            if POLICY_SCRIPT in base_paths:
                base.fail("Pictorial/Agile type-drift repair wrapper unexpectedly exists in bootstrap base")
        else:
            if POLICY_SCRIPT not in base_paths:
                base.fail("Pictorial/Agile type-drift repair steady-state base lacks wrapper")
            if (
                ...
                base.fail("Pictorial/Agile type-drift repair steady-state wrapper changed")
    ...
    for relative in sorted(BOOTSTRAP_WORKFLOWS & safe_controlled):
        ...
        expected_candidate = EXPECTED_WORKFLOW_SHA256[relative]
        if _sha256(cb) != expected_candidate:
            base.fail(f"Pictorial/Agile type-drift repair controlled workflow candidate drifted: {relative}")
        expected_base = PRIOR_EXPECTED_WORKFLOW_SHA256[relative] if bootstrap else expected_candidate
        if _sha256(bb) != expected_base:
            base.fail(f"Pictorial/Agile type-drift repair controlled workflow base drifted: {relative}")
        if not bootstrap and cb != bb:
            base.fail(f"Pictorial/Agile type-drift repair controlled workflow changed: {relative}")
    ...
    if delegated:
        try:
            ...
            base.fail(f"Pictorial/Agile type-drift repair inherited extension topology is missing: {exc}")
        if verifier is not PRIOR_VERIFY_EXTENSION_PATHS:
            base.fail("Pictorial/Agile type-drift repair inherited extension verifier drifted")
        if not callable(verifier):
            base.fail("Pictorial/Agile type-drift repair inherited extension topology is malformed: verifier not callable")
        try:
            verifier(candidate, policy_base, delegated)
        except TypeError as exc:
            base.fail(f"Pictorial/Agile type-drift repair inherited extension topology is malformed: {exc}")
...
def _verify_execution_extension_paths(candidate: base.RepositoryView, policy_base: base.RepositoryView) -> None:
    ...
    try:
        ...
        base.fail(f"Pictorial/Agile type-drift repair execution extension topology is missing: {exc}")
...
def _verify_desktop_extension_paths(candidate: base.RepositoryView, policy_base: base.RepositoryView) -> None:
    ...
    try:
        ...
        base.fail(f"Pictorial/Agile type-drift repair desktop extension topology is missing: {exc}")
...
def _validate_allowed_paths_repair(paths: set[str], stage: str) -> None:
    try:
        ...
        base.fail(f"Pictorial/Agile type-drift repair inherited allowlist topology is missing: {exc}")
    if delegated is not PRIOR_VALIDATE_ALLOWED_PATHS:
        base.fail("Pictorial/Agile type-drift repair inherited allowlist delegate drifted")
    if not callable(delegated):
        base.fail("Pictorial/Agile type-drift repair inherited allowlist topology is malformed: delegate not callable")
    try:
        delegated(paths, stage)
    except TypeError as exc:
        base.fail(f"Pictorial/Agile type-drift repair inherited allowlist topology is malformed: {exc}")


def _verify_policy_files(view: base.RepositoryView) -> None:
    actual = _git_blob_sha1(view.read_bytes(PRIOR_POLICY_PATH, base.MAX_POLICY_FILE_BYTES))
    if actual != EXPECTED_PRIOR_POLICY_GIT_BLOB_SHA1:
        base.fail(f"frozen Pictorial/Agile repair-v2 predecessor policy drifted: {actual}")
    try:
        verifier = prior._verify_policy_files
    except AttributeError as exc:
        base.fail(f"Pictorial/Agile type-drift repair inherited policy-file topology is missing: {exc}")
    if not callable(verifier):
        base.fail("Pictorial/Agile type-drift repair inherited policy-file topology is malformed: verifier not callable")
    try:
        verifier(view)
    except TypeError as exc:
        base.fail(f"Pictorial/Agile type-drift repair inherited policy-file topology is malformed: {exc}")
...
def _require_prebind_identity(shell: Any, retention: Any, desktop: Any, execution: Any) -> None:
    try:
        ...
        base.fail(f"Pictorial/Agile type-drift repair pre-bind topology is missing: {exc}")
    for actual, wanted, label in expected:
        if actual is not wanted:
            base.fail(f"Pictorial/Agile type-drift repair pre-bind {label} hook drifted")
...
def _require_overlay_identity() -> None:
    ...
    for actual, wanted, label in checks:
        if actual is not wanted:
            base.fail(f"Pictorial/Agile type-drift repair {label} hook drifted")
    ...
    try:
        hook_identity(retention_exact_delta, "pictorial-agile-contract-three-finding-repair-v3-overlay")
    except TypeError as exc:
        base.fail(f"Pictorial/Agile type-drift repair overlay topology is malformed: {exc}")

    if POLICY_SCRIPT not in safe_desktop_paths:
        base.fail("Pictorial/Agile type-drift repair desktop path registration drifted")
...
def _print_success(stage: str, mode: str) -> None:
    if _PRIOR_PRINT_SUCCESS is None or not callable(_PRIOR_PRINT_SUCCESS):
        base.fail("prior Pictorial/Agile success printer is missing or malformed")
    try:
        _PRIOR_PRINT_SUCCESS(stage, mode)
    except TypeError as exc:
        base.fail(f"Pictorial/Agile type-drift repair success-printer topology is malformed: {exc}")
    print(f"pictorial_agile_post_merge_policy_repair_authorization={REPAIR_AUTHORIZATION}")
    print("pictorial_agile_post_merge_policy_repair_findings=" + ",".join(REPAIR_FINDINGS))
...
def _install_policy() -> None:
    ...
    try:
        ...
        base.fail(f"Pictorial/Agile type-drift repair predecessor installation topology is missing or malformed: {exc}")
    ...
    try:
        ...
        base.fail(f"Pictorial/Agile type-drift repair installation topology is missing: {exc}")
    if not callable(prior_print_success):
        base.fail("Pictorial/Agile type-drift repair installation topology is malformed: success printer not callable")
...
def _selftest_workflows() -> None:
    ...
    for path in BOOTSTRAP_WORKFLOWS:
        ...
        if actual != EXPECTED_WORKFLOW_SHA256[path]:
            base.fail(f"Pictorial/Agile type-drift repair workflow drifted: {path}")
...
def _selftest_deltas() -> None:
    ...
    base.expect_failure_matching(
        "type-drift repair mixed bootstrap rejection",
...
def _selftest_type_drift() -> None:
    _, retention, impl, desktop, execution = _topology()

    original_hook_identity = retention._require_exact_delta_hook_identity
    retention._require_exact_delta_hook_identity = None
    try:
        base.expect_failure_matching(
            "type-drift repair noncallable hook identity",
            "overlay topology is malformed",
            _require_overlay_identity,
        )
    finally:
        retention._require_exact_delta_hook_identity = original_hook_identity

    original_retention_exact_delta = retention._require_exact_delta_retention
    retention._require_exact_delta_retention = None
    try:
        base.expect_failure_matching(
            "type-drift repair noncallable retention exact-delta",
            "overlay topology is malformed",
            _require_overlay_identity,
        )
    finally:
        retention._require_exact_delta_retention = original_retention_exact_delta

    original_desktop_paths = desktop.EXTENSION_CONTROLLED_PATHS
    desktop.EXTENSION_CONTROLLED_PATHS = None
    try:
        base.expect_failure_matching(
            "type-drift repair malformed desktop paths",
            "desktop-extension topology is malformed",
            _require_overlay_identity,
        )
    finally:
    ...
    execution.EXTENSION_CONTROLLED_PATHS = None
    try:
        base.expect_failure_matching(
            "type-drift repair malformed execution paths",
            "execution-extension topology is malformed",
            _require_overlay_identity,
        )
    finally:
        execution.EXTENSION_CONTROLLED_PATHS = original_execution_paths

    original_changed_paths = impl._changed_paths
    impl._changed_paths = None
    try:
        candidate, policy_base = _bootstrap_views()
        base.expect_failure_matching(
            "type-drift repair noncallable changed-path delegate",
            "changed-path topology is malformed",
            _require_exact_delta_repair,
            candidate,
            policy_base,
        )
    finally:
        impl._changed_paths = original_changed_paths

    original_extension_verifier = prior._verify_extension_paths
    prior._verify_extension_paths = None
    try:
        base.expect_failure_matching(
            "type-drift repair noncallable inherited extension verifier",
            "inherited extension verifier drifted",
            _verify_extension_paths,
            _memory_view({"README.md": b"x"}),
            _memory_view({"README.md": b"x"}),
            frozenset({"README.md"}),
        )
    finally:
...
def selftest() -> None:
    ...
    try:
        ...
        base.fail(f"Pictorial/Agile type-drift repair predecessor selftest topology is malformed: {exc}")
    _install_policy()
    _selftest_workflows()
    _selftest_deltas()
    _selftest_type_drift()
    _, _, impl, _, _ = _topology()
    if base.REPOSITORY != impl.CANONICAL_REPOSITORY:
        base.fail("canonical repository identity drifted")
    print("wepld Pictorial/Agile post-merge type-drift repair v3 self-tests: PASS")
...
def main(argv: list[str]) -> int:
    try:
        ...
        try:
            ...
            base.fail(f"Pictorial/Agile type-drift repair runtime topology is missing: {exc}")
        if not callable(runner):
            base.fail("Pictorial/Agile type-drift repair runtime topology is malformed: main not callable")
        try:
            return runner(argv)
        except TypeError as exc:
            base.fail(f"Pictorial/Agile type-drift repair runtime topology is malformed: {exc}")
