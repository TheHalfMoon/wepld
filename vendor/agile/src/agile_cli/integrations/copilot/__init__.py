...
- Skills are the default and use ``agile-<name>/SKILL.md`` directories under
  ``.github/skills/``
...
def _allow_all() -> bool:
    ...
    Checks ``AGILE_COPILOT_ALLOW_ALL_TOOLS`` first (new canonical name).
    Falls back to the deprecated ``AGILE_ALLOW_ALL_TOOLS`` if set,
    emitting a deprecation warning.  Default when neither is set: enabled.
    """
    new_var = os.environ.get("AGILE_COPILOT_ALLOW_ALL_TOOLS")
    if new_var is not None:
        return new_var != "0"

    old_var = os.environ.get("AGILE_ALLOW_ALL_TOOLS")
    if old_var is not None:
        warnings.warn(
            "AGILE_ALLOW_ALL_TOOLS is deprecated; "
            "use AGILE_COPILOT_ALLOW_ALL_TOOLS instead.",
            UserWarning,
            stacklevel=2,
        )
        return old_var != "0"
...
class CopilotIntegration(IntegrationBase):
    ...
    events_config_file = ".github/hooks/agile.json"
    ...
    def is_skills_mode(
        ...
        projects retain their managed Agile layout while fresh projects use
        ...
        if project_root is not None:
            ...
            if manifest_path.is_file():
                ...
                if manifest_files is not None and any(
                    path.startswith(".github/skills/agile-")
                    and path.endswith("/SKILL.md")
                    for path in manifest_files
                ):
                    return True
                if manifest_files is not None and any(
                    path.startswith(".github/agents/agile.")
            ...
            has_managed_commands = any(
                (
                    github_dir
                    / "agents"
                    / f"agile.{command}.agent.md"
                ).is_file()
                or (
                    github_dir
                    / "prompts"
                    / f"agile.{command}.prompt.md"
                ).is_file()
                for command in _COPILOT_CORE_COMMANDS
            )
    ...
    def invoke_separator_for_mode(self, skills_enabled: bool) -> str:
        """Skills projects render ``/agile-<cmd>``; commands use ``.``.
    ...
    def _resolve_executable(self) -> str:
        ...
        Checks ``AGILE_INTEGRATION_COPILOT_EXECUTABLE`` first.  Falls
        ...
        """
        env_name = "AGILE_INTEGRATION_COPILOT_EXECUTABLE"
    ...
    def build_exec_args(
        ...
        *,
        ...
        # Controlled by AGILE_COPILOT_ALLOW_ALL_TOOLS env var
        # (default: enabled).  The deprecated AGILE_ALLOW_ALL_TOOLS
    ...
    def build_command_invocation(self, command_name: str, args: str = "") -> str:
        ...
        Skills mode (default): ``/agile-<stem>`` slash-command dispatch.
        """
        if self._skills_mode:
            stem = command_name
            if stem.startswith("agile."):
                stem = stem[len("agile."):]
            invocation = "/agile-" + stem.replace(".", "-")
            if args:
    ...
    def dispatch_command(
        ...
        *,
        ...
        """Dispatch via ``--agent agile.<stem>`` instead of slash-commands.
        ...
        if skills_mode:
            prompt = "/agile-" + stem.replace(".", "-")
            if args:
                prompt = f"{prompt} {args}"
        else:
            agent_name = f"agile.{stem}"
            prompt = args or ""

        cli_args = [self._resolve_executable(), "-p", prompt]
        # Honour AGILE_INTEGRATION_COPILOT_EXTRA_ARGS for real workflow
    ...
    def command_filename(self, template_name: str) -> str:
        ...
        return f"agile.{template_name}.agent.md"
        ...
        **opts: Any,
        ...
        for src_file in templates:
            resolved_template = preset_resolver.resolve(
                f"agile.{src_file.stem}", template_type="command"
            )
        ...
        for src_file in templates:
            cmd_name = f"agile.{src_file.stem}"
            prompt_content = f"---\nagent: {cmd_name}\n---\n"
