...
Bundles compose existing Agile components — extensions, presets, workflows, and steps — into a single role or team stack. They are useful when a user should be able to install a tested set of components together instead of following several separate install commands.

Accepted community bundle entries are published in [`bundles/catalog.community.json`](https://github.com/TheHalfMoon/wepld/blob/main/bundles/catalog.community.json) and listed below. The built-in community source is discovery-only: `agile bundle search` and `agile bundle info` can inspect entries, but installing by ID requires explicitly adding an install-allowed catalog. Explicit catalogs use a higher default precedence than the built-in community source. To submit a bundle for review, file a [Bundle Submission](https://github.com/TheHalfMoon/wepld/issues/new?template=bundle_submission.yml) issue.
...
- A versioned GitHub release with a bundle artifact created by `agile bundle build`.
...
- Test evidence from a clean Agile project.
...
If your bundle depends on components that are not available from the default Agile catalogs, include the required catalog URLs in the submission and in your README. Test the full install path from a clean project with those catalogs added before submitting.
...
agile bundle catalog add https://example.com/bundles.json --id example-bundle-catalog --policy install-allowed
agile bundle install example-bundle
```
...
To update a submitted bundle, file another [Bundle Submission](https://github.com/TheHalfMoon/wepld/issues/new?template=bundle_submission.yml) issue with the new version, download URL, changed component list, and updated test evidence. Mention that the issue updates an existing bundle entry.
