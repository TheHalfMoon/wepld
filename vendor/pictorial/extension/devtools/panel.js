...
/**
 * Pictorial DevTools Extension - Panel
 *
...
function getPort() {
  if (port) return port;
  port = chrome.runtime.connect({ name: `pictorial-panel-${tabId}` });
...
// Maps each anti-pattern to the most relevant Pictorial skill(s) for fixing it.
...
async function formatFindingsForCopy(findings) {
  if (!findings.length) return 'Pictorial found no anti-patterns on this page.';
  const url = await getInspectedUrl();
  const lines = ['# Pictorial findings'];
  ...
  if (skills.length) {
    lines.push(`Suggested Pictorial skills to fix: ${skills.join(', ')}`);
    lines.push('');
  }

  lines.push('---');
  lines.push('Detected by [Pictorial](https://github.com/TheHalfMoon/wepld). Skills are suggestions, not required.');
  return lines.join('\n');
}
...
async function formatSingleFindingForCopy(item, finding) {
  ...
  const lines = [`# Pictorial: ${finding.name}`];
  if (url) lines.push(`URL: ${url}`);
  ...
  lines.push('');
  lines.push(`Suggested Pictorial skill(s) to fix: ${fixSkillFor(finding.type)}`);
  return lines.join('\n');
}
