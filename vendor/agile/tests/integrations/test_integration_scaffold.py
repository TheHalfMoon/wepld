...
from agile_cli.integration_scaffold import scaffold_integration
...
def _repo_root(tmp_path: Path) -> Path:
    root = tmp_path / "agile"
    (root / "src" / "agile_cli" / "integrations").mkdir(parents=True)
    (root / "tests" / "integrations").mkdir(parents=True)
    (root / "pyproject.toml").write_text("[project]\nname = \"wepld-agile\"\n", encoding="utf-8")
    (root / "src" / "agile_cli" / "__init__.py").write_text("", encoding="utf-8")
    (root / "src" / "agile_cli" / "integrations" / "__init__.py").write_text(
        "",
        encoding="utf-8",
    )
    return root
...
def test_integration_scaffold_creates_markdown_files(tmp_path, monkeypatch):
    ...
    integration_file = root / "src" / "agile_cli" / "integrations" / "my_agent" / "__init__.py"
    ...
    assert "from agile_cli.integrations.my_agent import MyAgentIntegration" in test_content
...
def test_integration_scaffold_rejects_unknown_type_before_scaffolding(tmp_path, monkeypatch):
    ...
    assert not (root / "src" / "agile_cli" / "integrations" / "my_agent").exists()
...
def test_integration_scaffold_reports_filesystem_errors_cleanly(tmp_path, monkeypatch):
    ...
    import agile_cli.integration_scaffold as scaffold_module
...
def test_scaffold_rolls_back_partial_files_on_write_failure(tmp_path, monkeypatch):
    root = _repo_root(tmp_path)
    integration_dir = root / "src" / "agile_cli" / "integrations" / "my_agent"
...
def test_scaffold_creates_only_leaf_integration_directory(tmp_path, monkeypatch):
    ...
    assert any(
        path == root / "src" / "agile_cli" / "integrations" / "my_agent"
...
def test_scaffold_refuses_symlinked_target_directory(tmp_path):
    ...
    integrations = root / "src" / "agile_cli" / "integrations"
...
def test_integration_scaffold_accepts_uppercase_type(tmp_path, monkeypatch):
    ...
    content = (
        root / "src" / "agile_cli" / "integrations" / "my_agent" / "__init__.py"
