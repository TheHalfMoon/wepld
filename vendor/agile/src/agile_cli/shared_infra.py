"""Shared Agile infrastructure installation helpers."""
...
# Managed ``.agile/.gitignore``. Keeps machine-local Agile state out of
...
def load_agile_manifest(
    ...
    *,
    ...
    manifest_path = project_path / ".agile" / "integrations" / "agile.manifest.json"
    if manifest_path.exists():
        try:
            manifest = IntegrationManifest.load("agile", project_path)
    ...
    return IntegrationManifest("agile", project_path, version=version)
...
_BASH_FORMAT_COMMAND_RE = re.compile(
    r"\$\(\s*format_agile_command\s+(['\"]?)([A-Za-z0-9_.-]+)\1(?:\s+[^)]*)?\)"
)
_POWERSHELL_FORMAT_COMMAND_RE = re.compile(
    r"Format-AgileCommand\s+-CommandName\s+(['\"])([A-Za-z0-9_.-]+)\1(?:\s+-RepoRoot\s+[^\r\n]+)?"
)
_PYTHON_FORMAT_COMMAND_RETURN_RE = re.compile(
    r'return f"/agile\{separator\}\{name\}"'
)
_BASH_FORMATTER_RETURN_RE = re.compile(
    r'''printf '/agile%s%s\\n' "\$separator" "\$command_name"'''
)
_POWERSHELL_FORMATTER_RETURN_RE = re.compile(
    r'return "/agile\$separator\$name"'
)


def _format_agile_command(
    command_name: str, separator: str, prefix: str = "/"
) -> str:
    name = command_name.strip().lstrip("/")
    if name.startswith("agile."):
        name = name[len("agile.") :]
    elif name.startswith("agile-"):
        name = name[len("agile-") :]
    name = name.replace(".", separator)
    return f"{prefix}agile{separator}{name}"
...
def refresh_shared_templates(
    ...
    *,
    ...
    manifest = load_agile_manifest(project_path, version=version, console=console)
    tracked_files = manifest.files
    modified = set(manifest.check_modified())
    skipped_files: list[str] = []
    planned_updates: list[tuple[Path, str, str]] = []

    dest_templates = project_path / ".agile" / "templates"
...
def install_shared_infra(
    ...
    *,
    ...
    if scripts_src.is_dir():
        dest_scripts = project_path / ".agile" / "scripts"
                ...
                for src_path in variant_src.rglob("*"):
                    ...
                    if not write:
                        if bucket == "preserved":
                            ...
                            # ``.agile/`` tree does not silently drop it (#2107).
    ...
    # Managed ``.agile/.gitignore`` — keeps machine-local state (the
    ...
    # tracked in ``agile.manifest.json`` (not the per-integration manifest) and
    # is therefore intentionally left in place by ``integration uninstall``.
    specify_dir = project_path / ".agile"
    ...
    if scanned_variant_dirs:
        ...
        script_prefixes = tuple(
            f".agile/scripts/{variant_dir}/" for variant_dir in scanned_variant_dirs
        )
