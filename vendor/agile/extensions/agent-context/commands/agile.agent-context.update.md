---
description: "Refresh the managed Agile section in coding agent context file(s)"
---

# Update Coding Agent Context

Refresh the managed Agile section inside the active coding agent's context/instruction file (e.g. `CLAUDE.md`, `.github/copilot-instructions.md`, `AGENTS.md`).

## Behavior

The script reads the agent-context extension config at
`.agile/extensions/agent-context/agent-context-config.yml` to discover:
...
- `context_markers.start` / `.end` — the delimiters surrounding the managed section. Defaults to `<!-- AGILE START -->` and `<!-- AGILE END -->` when the field is missing.
...
- **Bash**: `.agile/extensions/agent-context/scripts/bash/update-agent-context.sh [plan_path]`
- **PowerShell**: `.agile/extensions/agent-context/scripts/powershell/update-agent-context.ps1 [plan_path]`
