...
Extension system for [Agile](https://github.com/TheHalfMoon/wepld) - add new functionality without bloating the core framework.

## Extension Catalogs

Agile provides two catalog files with different purposes:

### Your Catalog (`catalog.json`)

- **Purpose**: Default upstream catalog of extensions used by the Agile CLI
- **Default State**: Empty by design in the upstream project - you or your organization populate a fork/copy with extensions you trust
- **Location (upstream)**: `extensions/catalog.json` in the GitHub-hosted agile repo
- **CLI Default**: The `agile extension` commands use the upstream catalog URL by default, unless overridden
- **Org Catalog**: Point `AGILE_CATALOG_URL` at your organization's fork or hosted catalog JSON to use it instead of the upstream default
...
agile extension search  # Now uses your organization's catalog instead of the upstream default
```
...
- **Submission**: Open to community contributions via [issue template](https://github.com/TheHalfMoon/wepld/issues/new?template=extension_submission.yml)
...
4. **Team members** can now discover and install them:
   - `agile extension search` shows your curated catalog
   - `agile extension add <name>` installs from your catalog
...
**Tradeoff**: Extensions installed this way won't appear in `agile extension search` for other team members unless you also add them to your `catalog.json`.
...
3. **File an issue** using the [Extension Submission](https://github.com/TheHalfMoon/wepld/issues/new?template=extension_submission.yml) template with all required metadata
...
agile extension search                  # See what's in your catalog
agile extension add <extension-name>    # Install by name

# Direct from URL (bypasses catalog)
agile extension add <extension-name> --from https://github.com/<org>/<repo>/archive/refs/tags/<version>.zip
