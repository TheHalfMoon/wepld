...
> Pictorial's *emission targets*, not the support landscape (see its note).
...
- Codex CLI uses a separate `agents/openai.yaml` sidecar for skill metadata (icons, branding, MCP tools, invocation control). Codex also auto-discovers subagents bundled inside an installed skill's `agents/` folder (TOML), which is how Pictorial ships its asset-producer. Standalone custom agents can still live under `.codex/agents/` or `~/.codex/agents/`, but Pictorial no longer installs anything there.
...
## Hook surface used by Pictorial
...
## Native Subagent Directory Structure (Pictorial emission targets)

> **Scope:** this table is **where Pictorial emits native subagent files**, not a
...
> GitHub Copilot, and Google Antigravity ship them too. Pictorial only writes
