...
Historical source observation: Omnigent uses a parent-side credential proxy where the real credential remains outside the sandbox. The upstream HTTP(S) proxy can inject a credential for a bound host; any cleartext credential-bearing behavior is rejected for WePLD. The WePLD candidate permits credential injection only into authenticated HTTPS on every hop. Clients that require a local credential can receive a non-secret placeholder which is swapped only for its bound destination; misuse against another host is refused.
