...
- `user-bot.mjs` is a module plus CLI. Given a workspace dir it resolves the
  running daemon from `.pictorial/questions/<key>.state.json`, opens the page,
...
- `PICTORIAL_IMAGE_GEN_FAKE=1` switches `skill/scripts/generate-image.mjs` to
...
- `PICTORIAL_IMAGE_GEN_FAKE` still stands in for image spend, so a full
