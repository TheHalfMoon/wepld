...
| Observation refreshed 2026-09-20 | Value |
|---|---|
| Repository | TheHalfMoon/wepld; ID 1334408699; public; default main |
| Main / planning base | 765f9d4ae0588ca06b0f65cd76de16eaa8a5c246 |
| Tree | 20c0a95fac2b27c8324ae2923d22870cb63d72a4 |
| Planning branch | codex/astro-master-plan-20260920 |
| Latest merged policy | PR #339 / v71; S3-C001..C013 only |
| Frontier | S1 implemented; S2 CLOSED_CANONICAL with eight carried limitations; S3 contract implementation absent |
| Inventory | 342 branches; 324 PRs: 181 merged, 135 closed unmerged, 8 open drafts; 15 ordinary issues, 11 open |
| Enforcement | No rulesets returned; main protection endpoint returns Branch not protected |
| Source baseline | 402 named registry entries and 43 artifact records; frozen archive unchanged |
| License metadata | null; component-specific rights still apply |
...
WePLD should increase accepted engineering outcomes achieved by the user's available models and workers through relevant context, clear intent, bounded execution, independent assurance and recoverability. “Best” is an empirical objective, not a release claim. Compare the same worker, model, task and budget with and without WePLD.

The first complete loop is one authorized person, one local project, one maintenance objective, one qualified worker route, one ChangeUnit and one human acceptance decision. Open project → Doctor → specify → inspect plan/context → authorize bounded work → Review/Test/Security → separately authorized repair → accept/reject → inspect recovery. The current repository does not yet implement this loop.

Enterprise Teams is a first-class target. Organization, principal and scope seams must enter before worker execution; cross-host collaboration, identity federation and administration ship only after isolation and recovery qualification. This prevents a single-user prototype from becoming an insecure team product while avoiding an enterprise service dependency for local work.

## One roadmap

Every entry gate includes fresh trusted bootstrap, Spec Kit, Ponytail FULL, acquisition, exact path authority and applicable data-handling decisions. Every exit includes deterministic qualification, independent review, finding reconciliation, acceptance evidence and Build Learning. Failure, partial coverage and unsupported platforms remain visible outcomes.

| Slice | Entry and bounded result | Exit evidence / recovery |
|---|---|---|
| P0 | Existing governance; reconcile new decisions without rewriting history | Exact proposal accepted independently; rights/status ledger preserved |
| S1 | Existing Desktop–Core handshake | Retain protocol identity, replay protection and owned child evidence |
| S2 | Existing identity/Doctor/store | Carry S2-S001/S003/S005/S007/Q001/Q004/Q008/Q009 until specific proof or accepted carry-forward |
| S3 | Existing C001..C013 authority only; host/observe/spawn need separate grants | Pure contracts first, then Windows qualification, owned process observation/cancellation; PTY not containment |
| S3-D | Separately authorized deterministic assurance seed | Exact target, producer, coverage and failure envelopes before workers |
| S4 / S4-G | S2 evidence and qualified parser sources | File/blob facts, lexical and syntax context, semantic adapter seam, provenance, bounded ContextPackage; rebuildable indexes |
| S5 | Usable context and AGILLE contracts | Intent/spec/plan qualification, task DAG, debate/requirements helpers; plans do not execute |
| S6 / S6-AH / S6-N | Qualified host, context and plan | One durable Mission Runtime, UWC, Mirefa, minimum Edara, Nawat effect-time checks, scope and budget; restart reconciliation |
| S7 / S7-S | Qualified producers and immutable targets | Visible Review/Test/Security, independent findings, coverage, AMAN overlays, no automatic fixes |
| S8 | S7 plus valid effect grants | Bounded repair, explicit reassignment, nonconvergence and Trusted Completion; no silent fallback |
| S9 / S9-P | Complete effect/outcome evidence | ChangeUnit, QualityPassport, delivery and separate source/store/external recovery drill |
...
| Staffing, delegation, adaptive teams | Edara | S6; measured topology optimization later |
| Scope, grants, identity, revocation | Nawat | S6-N; enterprise administration later |
| Code facts, architecture maps, legacy analysis | Fehrest.Maemar | S4-G; runtime/binary analysis later |
| Knowledge, research, memory, context, OKF | Fehrest | S4; research/meeting expansion |
| Design, canvas, documents and office exports | Work + Presentation | Follow-on S5/S9 profile |
| Browser, WebMCP, computer, live voice/video | UWC + Runtime | Qualified S6–S9 profiles |
| Review, tests, findings, coverage, formal checks | Assurance | S3-D/S7 |
| Security posture, supply chain and risk graph | AMAN | Admission/S7-S |
| Repair, acceptance, passport, recovery | Runtime + Trusted Completion + Work | S8/S9 |
| Skills, MCP, tools, plugins, Community Hub | Mirefa + UWC + Nawat | S6 packages; Hub distribution later |
| Observability and experiment analytics | Fehrest evidence + Byan | Evidence early; analytics S10 |
| Team governance, private registry, SSO, residency | Nawat + Work | Seams before S6, deployment after qualification |
