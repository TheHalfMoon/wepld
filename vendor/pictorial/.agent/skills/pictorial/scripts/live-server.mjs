...
import {
  ...
  PICTORIAL_COMMAND_PREFIX,
  readLiveServerInfo,
  removeLiveServerInfo,
  resolveDesignSidecarPath,
  writeLiveServerInfo,
} from './lib/pictorial-paths.mjs';
...
// that outlives an `pictorial document` run (or a context file created after
...
const DEBUG_MANUAL_EDIT_EVENTS = /^(1|true|yes)$/i.test(process.env.PICTORIAL_LIVE_DEBUG_EVENTS || '');
...
function loadBrowserScripts() {
  ...
  const detectPaths = [
    ...
    path.join(__dirname, '..', '..', '..', '..', 'cli', 'engine', 'detect-antipatterns-browser.js'),
    path.join(process.cwd(), 'node_modules', 'pictorial', 'cli', 'engine', 'detect-antipatterns-browser.js'),
  ];
  let detectScript = '';
  ...
  return (req, res) => {
    ...
    if (p === '/live.js') {
      ...
      const body = assembleLiveBrowserScript({
        ...
        commandPrefix: PICTORIAL_COMMAND_PREFIX,
        appRoot: process.cwd(),
        parts,
      });
      res.writeHead(200, {
    ...
    //   /design-system.json    returns both parsed DESIGN.md and .pictorial/design.json
    ...
    //                          - sidecar: .pictorial/design.json contents when present.
    ...
    if (p === '/design-system.json' || p === '/design-system/raw') {
      ...
      if (jsonStat) {
        try {
          ...
          response.sidecarError = 'Failed to parse .pictorial/design.json: ' + err.message;
        }
      }
...
function sessionFileMetadataFromPollReply(file) {
  ...
  if (!metadataFile.includes('.pictorial/live/previews/')
      && !metadataFile.includes('node_modules/.pictorial-live/')
      && !metadataFile.includes('src/lib/pictorial/')
      && !metadataFile.includes('/.pictorial-live/')) return base;
...
function cleanupSvelteComponentSessionsBeforeExit() {
  try {
    ...
    console.warn('[pictorial] Svelte component session cleanup failed:', err.message);
  }
}
...
function sweepOrphanSvelteComponentSessionsOnStartup() {
  try {
    ...
    if (result.removed.length > 0 || result.removedRoot) {
      console.log('[pictorial] swept orphaned Svelte component sessions:', JSON.stringify(result));
    }
  } catch (err) {
    console.warn('[pictorial] Svelte component session sweep failed:', err.message);
  }
}
...
function sweepStaleAcceptReceiptsOnStartup() {
  try {
    ...
    if (removed > 0) console.log(`[pictorial] removed ${removed} accept receipt(s) older than 14 days`);
  } catch (err) {
    console.warn('[pictorial] accept receipt retention sweep failed:', err.message);
  }
}
...
function applyLegacyDeferredAcceptsOnStartup() {
  try {
    ...
    if (result.applied > 0 || result.failed > 0) {
      console.log('[pictorial] applied legacy deferred Svelte component accepts:', JSON.stringify(result));
    }
  } catch (err) {
    console.warn('[pictorial] legacy deferred Svelte component accept apply failed:', err.message);
  }
}
...
httpServer.listen(state.port, '127.0.0.1', () => {
  ...
  console.log(`\nPictorial live server running on ${url}`);
