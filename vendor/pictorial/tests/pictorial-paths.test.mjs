...
/**
 * Tests for project-local Pictorial path resolution.
 * Run with: node --test tests/pictorial-paths.test.mjs
 */
...
describe('pictorial project paths', () => {
  let tmp;

  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), 'pictorial-paths-'));
  });
  ...
  it('resolves the generated design sidecar under .pictorial', () => {
    assert.equal(getDesignSidecarPath(tmp), join(tmp, '.pictorial', 'design.json'));
  ...
  it('uses .pictorial/live/config.json as the default live config path', () => {
  ...
  it('falls back to legacy scripts/config.json when no new live config exists', () => {
    const scriptsDir = join(tmp, 'skills', 'pictorial', 'scripts');
  ...
  it('lets PICTORIAL_LIVE_CONFIG override both new and legacy config locations', () => {
    ...
    assert.equal(
      resolveLiveConfigPath({ cwd: tmp, scriptsDir: join(tmp, 'scripts'), env: { PICTORIAL_LIVE_CONFIG: 'custom-live-config.json' } }),
      override,
    );
  });

  it('places live server, session, and annotation state under .pictorial/live', () => {
    assert.equal(getLiveServerPath(tmp), join(tmp, '.pictorial', 'live', 'server.json'));
    assert.equal(getLiveSessionsDir(tmp), join(tmp, '.pictorial', 'live', 'sessions'));
    assert.equal(getLiveAnnotationsDir(tmp), join(tmp, '.pictorial', 'live', 'annotations'));
  });

  it('places .pictorial state under the active monorepo child project', () => {
    ...
    assert.equal(getDesignSidecarPath(tmp, options), join(projectRoot, '.pictorial', 'design.json'));
    assert.equal(getLiveConfigPath(tmp, options), join(projectRoot, '.pictorial', 'live', 'config.json'));
    assert.equal(getLiveServerPath(tmp, options), join(projectRoot, '.pictorial', 'live', 'server.json'));
    assert.equal(getLiveSessionsDir(tmp, options), join(projectRoot, '.pictorial', 'live', 'sessions'));
    assert.equal(getLiveAnnotationsDir(tmp, options), join(projectRoot, '.pictorial', 'live', 'annotations'));
    assert.equal(getCritiqueDir(tmp, options), join(projectRoot, '.pictorial', 'critique'));
    assert.equal(getLegacyLiveServerPath(tmp, options), join(projectRoot, '.pictorial-live.json'));
  ...
  it('does not let a root live config shadow a child project live config path', () => {
    ...
    mkdirSync(join(tmp, '.pictorial', 'live'), { recursive: true });
    writeFileSync(join(tmp, '.pictorial', 'live', 'config.json'), '{"source":"root"}');

    assert.equal(
      resolveLiveConfigPath({ cwd: tmp, scriptsDir: join(tmp, 'scripts'), env: {}, targetPath: 'apps/admin/src/App.jsx' }),
      join(tmp, 'apps', 'admin', '.pictorial', 'live', 'config.json'),
    );
  });
