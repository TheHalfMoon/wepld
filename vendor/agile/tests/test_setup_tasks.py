...
def _install_ps_scripts(repo: Path) -> None:
    d = repo / ".agile" / "scripts" / "powershell"
    d.mkdir(parents=True, exist_ok=True)
...
def _write_feature_json(
    ...
    (repo / ".agile" / "feature.json").write_text(
...
def _run_bash_format_command(repo: Path, command_name: str) -> subprocess.CompletedProcess:
    script = repo / ".agile" / "scripts" / "bash" / "common.sh"
    return subprocess.run(
        ["bash", "-c", 'source "$1"; format_agile_command "$2" "$PWD"', "bash", str(script), command_name],
        cwd=repo,
        capture_output=True,
        text=True,
        check=False,
...
def _run_powershell_format_command(repo: Path, command_name: str) -> subprocess.CompletedProcess:
    script = repo / ".agile" / "scripts" / "powershell" / "common.ps1"
    ...
    return subprocess.run(
        [
            ...
            "-NoProfile",
            "-Command",
            '& { param($common, $commandName) . $common; Format-AgileCommand -CommandName $commandName -RepoRoot (Get-Location).Path }',
            str(script),
            command_name,
        ],
        cwd=repo,
...
def test_setup_tasks_bash_core_template_resolved(tasks_repo: Path) -> None:
    ...
    script = tasks_repo / ".agile" / "scripts" / "bash" / "setup-tasks.sh"
...
def test_setup_tasks_bash_override_wins(tasks_repo: Path) -> None:
    """
    When an override exists at .agile/templates/overrides/tasks-template.md,
    setup-tasks.sh --json must return the override path, not the core path.
    """
    _minimal_feature(tasks_repo)

    # Create the override
    overrides_dir = tasks_repo / ".agile" / "templates" / "overrides"
...
def test_setup_tasks_bash_extension_wins_over_core(tasks_repo: Path) -> None:
    ...
    # FIX: real extension layout is .agile/extensions/<id>/templates/<name>.md
    extension_dir = (
        tasks_repo / ".agile" / "extensions" / "test-extension" / "templates"
    )
...
def test_setup_tasks_bash_preset_wins_over_extension(tasks_repo: Path) -> None:
    ...
    # FIX: real preset layout is .agile/presets/<id>/templates/<name>.md
    preset_dir = tasks_repo / ".agile" / "presets" / "test-preset" / "templates"
...
def test_setup_tasks_bash_preset_priority_order(tasks_repo: Path) -> None:
    ...
    .agile/presets/.registry wins.
    """
    _minimal_feature(tasks_repo)

    # resolve_template reads .agile/presets/.registry as a JSON object with a
    ...
    high_priority_dir = (
        tasks_repo / ".agile" / "presets" / "priority-1-preset" / "templates"
    )
    ...
    low_priority_dir = (
        tasks_repo / ".agile" / "presets" / "priority-2-preset" / "templates"
    )
    ...
    registry_json = tasks_repo / ".agile" / "presets" / ".registry"
...
def test_setup_tasks_bash_missing_template_errors(tasks_repo: Path) -> None:
    ...
    core = tasks_repo / ".agile" / "templates" / "tasks-template.md"
    core.unlink()
...
def test_bash_command_hint_defaults_to_dot_without_integration_json(tasks_repo: Path) -> None:
    integration_json = tasks_repo / ".agile" / "integration.json"
    ...
    assert result.stdout.strip() == "/agile.plan"
...
def test_bash_command_hint_normalizes_mixed_separators(tasks_repo: Path) -> None:
    ...
    result = _run_bash_format_command(tasks_repo, "/agile-git.commit")

    assert result.returncode == 0, result.stderr
    assert result.stdout.strip() == "/agile.git.commit"
...
def test_bash_command_hint_preserves_hyphens_inside_segments(tasks_repo: Path) -> None:
    ...
    result = _run_bash_format_command(tasks_repo, "agile.jira.sync-status")

    assert result.returncode == 0, result.stderr
    assert result.stdout.strip() == "/agile.jira.sync-status"
...
def test_installed_bash_formatter_uses_skill_colon_prefix(tmp_path: Path) -> None:
    ...
    assert result.returncode == 0, result.stderr
    assert result.stdout.strip() == "/skill:agile-plan"
...
def test_bash_command_hint_falls_back_to_awk_when_jq_and_python3_broken(
    ...
    wrong hint like `/agile.plan`. The awk fallback must recover `-`.
    """
    _write_integration_state(tasks_repo, "forge", "-")
    stub_dir = _install_broken_json_tool_stubs(tasks_repo)

    script = tasks_repo / ".agile" / "scripts" / "bash" / "common.sh"
    env = _clean_env()
    ...
    result = subprocess.run(
        [
            "bash",
            "-c",
            'source "$1"; format_agile_command "$2" "$PWD"',
            "bash",
            str(script),
            "plan",
        ],
...
def test_bash_command_hint_caches_invoke_separator_per_process(tasks_repo: Path) -> None:
    ...
    result = subprocess.run(
        [
            "bash",
            "-c",
            'source "$1"; format_agile_command plan "$PWD"; printf "%s" "$2" > .agile/integration.json; format_agile_command tasks "$PWD"',
            "bash",
            str(script),
    ...
    assert result.returncode == 0, result.stderr
    assert result.stdout.splitlines() == ["/agile-plan", "/agile-tasks"]
...
def test_setup_tasks_bash_uses_invoke_separator_in_plan_hint(tasks_repo: Path) -> None:
    ...
    assert result.returncode != 0
    assert "Run /agile-plan first" in result.stderr
    assert "/agile.plan" not in result.stderr
...
def test_check_prerequisites_bash_uses_invoke_separator_in_tasks_hint(
    ...
    script = tasks_repo / ".agile" / "scripts" / "bash" / "check-prerequisites.sh"
    ...
    assert result.returncode != 0
    assert "Run /agile-tasks first" in result.stderr
    assert "/agile.tasks" not in result.stderr
...
def test_setup_tasks_ps_core_template_resolved(tasks_repo: Path) -> None:
    ...
    """
    _minimal_feature(tasks_repo)
    script = tasks_repo / ".agile" / "scripts" / "powershell" / "setup-tasks.ps1"
    exe = "pwsh" if HAS_PWSH else _WINDOWS_POWERSHELL
...
def test_powershell_command_hint_normalizes_mixed_separators(
    ...
    result = _run_powershell_format_command(tasks_repo, "/agile-git.commit")
...
def test_powershell_command_hint_preserves_hyphens_inside_segments(
    ...
    result = _run_powershell_format_command(tasks_repo, "agile.jira.sync-status")
...
def test_setup_tasks_ps_uses_invoke_separator_in_plan_hint(tasks_repo: Path) -> None:
    ...
    assert result.returncode != 0
    assert "Run /agile-plan first" in output
    assert "/agile.plan" not in output
...
def test_check_prerequisites_ps_uses_invoke_separator_in_tasks_hint(
    ...
    script = tasks_repo / ".agile" / "scripts" / "powershell" / "check-prerequisites.ps1"
    exe = "pwsh" if HAS_PWSH else _WINDOWS_POWERSHELL
    ...
    output = result.stderr + result.stdout
    assert result.returncode != 0
    assert "Run /agile-tasks first" in output
    assert "/agile.tasks" not in output
