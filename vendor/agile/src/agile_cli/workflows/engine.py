...
class WorkflowDefinition:
    ...
    def __init__(self, data: dict[str, Any], source_path: Path | None = None) -> None:
        ...
        # Advisory pre-conditions (agile version / integrations a workflow
...
# pre-conditions documented for workflows (``agile_version`` and
...
_RECOGNIZED_REQUIRES_KEYS = frozenset({"agile_version", "integrations"})
...
def validate_workflow(definition: WorkflowDefinition) -> list[str]:
    ...
    # -- Requires ---------------------------------------------------------
    # ``requires`` declares advisory pre-conditions (the agile version and
...
class RunState:
    ...
    # ``.agile/workflows/runs/`` directory or be mis-interpreted by the
    ...
    def _validate_run_id(cls, run_id: str) -> None:
        ...
        outside ``.agile/workflows/runs/<run_id>/``.
        """
    ...
    def runs_dir(self) -> Path:
        return self.project_root / ".agile" / "workflows" / "runs" / self.run_id
    ...
    def load(cls, run_id: str, project_root: Path) -> RunState:
        ...
        *after* the file lookup — too late to prevent the disclosure.
        ...
        runs_dir = project_root / ".agile" / "workflows" / "runs" / run_id
...
class WorkflowEngine:
    ...
    def execute(
        ...
        run_id:
            Optional run ID (uses AGILE_WORKFLOW_RUN_ID when set, otherwise auto-generated).
        installed_workflow_id, installed_registry_root:
        ...
        if effective_run_id is None:
            env_run_id = os.environ.get("AGILE_WORKFLOW_RUN_ID", "").strip()
            if env_run_id:
        ...
        run_dir = self.project_root / ".agile" / "workflows" / "runs" / state.run_id
    ...
    def resume(
        ...
        run_dir = self.project_root / ".agile" / "workflows" / "runs" / run_id
    ...
    def _resolve_default(self, name: str, default: Any) -> Any:
        ...
        recorded in ``.agile/integration.json`` so workflows dispatch to the
    ...
    def _load_project_integration(self) -> str | None:
        """Read the default integration key from ``.agile/integration.json``.
    ...
    def list_runs(self) -> list[dict[str, Any]]:
        ...
        runs_dir = self.project_root / ".agile" / "workflows" / "runs"
