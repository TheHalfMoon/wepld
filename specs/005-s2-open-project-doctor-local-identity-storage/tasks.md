...
- [~] **S2-S006** Malicious `.git`/gitfile/topology parsing tests. PARTIAL: the hook-execution angle of a hostile `.git` directory is proven — see `topology_observation_never_triggers_a_repository_hook` cited under S2-S007 below. Malformed/adversarial `worktree list --porcelain -z` machine output is proven by `crates/core/tests/git_topology_v1.rs::malformed_worktree_machine_output_fails_closed` (PR #240-era, pre-existing). NOT YET COVERED: a synthetically malicious `.git` *gitfile* redirect (the one-line `gitdir: <path>` pointer used by worktrees/submodules) that names a path outside the expected repository bounds, and a generally malformed/adversarial `.git` directory structure beyond the machine-output and hook-execution angles already proven.
- [~] **S2-S007** External Git output/environment/parser/timeout/no-hook/no-network tests if Git adapter admitted. PARTIAL, landed via the v60 single-use reopen (policy PR #309, product PR #310): no-hook is proven by `crates/core/tests/git_topology_v1.rs::topology_observation_never_triggers_a_repository_hook` — a hostile repository plants an executable marker script under every hook name Git recognizes that could plausibly relate to a checkout/commit/push/rewrite/reference update (`pre-commit`, `post-commit`, `post-checkout`, `post-merge`, `pre-push`, `pre-rebase`, `post-rewrite`, `reference-transaction`, `pre-auto-gc`, `post-index-change`, `post-applypatch`, `fsmonitor-watchman`), with `core.hooksPath` pinned repo-locally so an inherited override cannot make the result vacuous (a real CodeRabbit finding, fixed before merge); none of the twelve ever fire during `observe_git_topology`'s real `rev-parse`/`worktree list --porcelain -z` calls. `#[cfg(unix)]`, executed natively on ubuntu-latest + macos-latest. Parser robustness against adversarial machine output is proven by the pre-existing `malformed_worktree_machine_output_fails_closed`. Environment scrubbing (`sanitized_git_environment_from`) and the config-value oversized-input parser are proven by their own pre-existing unit tests inside `crates/core/src/git_topology.rs`'s `#[cfg(test)]` module, not by a dedicated integration fixture. NOT YET COVERED: no test forces `GIT_STDOUT_MAX_BYTES`/`GIT_STDERR_MAX_BYTES` overflow through a real oversized Git process output and asserts the forced-termination path; the hard-timeout fixture (a real slow/hanging Git process hitting the hard timeout) remains an explicit, already-recorded obligation in `git_topology_v1.rs`'s own module doc, not claimed here; no dedicated fixture proves the adapter makes no network syscall (distinct from the CLI-level `bin_source_starts_no_project_task_and_opens_no_socket`, which covers the whole `wepld` binary's source, not the Git adapter's argv/environment specifically).
