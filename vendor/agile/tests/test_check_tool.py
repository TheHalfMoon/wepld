...
Covers issue https://github.com/TheHalfMoon/wepld/issues/550:
  `agile check` reports "Claude Code CLI (not found)" even when claude is
...
from agile_cli import app, check_tool
...
class TestCheckToolClaude:
    ...
    def test_detected_via_migrate_installer_path(self, tmp_path):
        ...
        with patch("agile_cli.CLAUDE_LOCAL_PATH", fake_claude), \
             patch("agile_cli._utils.CLAUDE_LOCAL_PATH", fake_claude), \
             patch("agile_cli.CLAUDE_NPM_LOCAL_PATH", fake_missing), \
             patch("agile_cli._utils.CLAUDE_NPM_LOCAL_PATH", fake_missing), \
    ...
    def test_detected_via_npm_local_path(self, tmp_path):
        ...
        with patch("agile_cli.CLAUDE_LOCAL_PATH", fake_migrate), \
             patch("agile_cli._utils.CLAUDE_LOCAL_PATH", fake_migrate), \
             patch("agile_cli.CLAUDE_NPM_LOCAL_PATH", fake_npm_claude), \
             patch("agile_cli._utils.CLAUDE_NPM_LOCAL_PATH", fake_npm_claude), \
             patch("shutil.which", return_value=None):
            assert check_tool("claude") is True
    ...
    def test_detected_via_path(self, tmp_path):
        ...
        with patch("agile_cli.CLAUDE_LOCAL_PATH", fake_missing), \
             patch("agile_cli._utils.CLAUDE_LOCAL_PATH", fake_missing), \
             patch("agile_cli.CLAUDE_NPM_LOCAL_PATH", fake_missing), \
             patch("agile_cli._utils.CLAUDE_NPM_LOCAL_PATH", fake_missing), \
...
class TestCheckTip:
    """`agile check` should point users to the existing version check."""

    def test_check_shows_self_check_tip(self):
        with patch("agile_cli.check_tool", return_value=True):
            result = runner.invoke(app, ["check"])

        output = strip_ansi(result.output)
        assert result.exit_code == 0
        assert (
            "Tip: Run 'agile self check' to verify you have the latest CLI version"
            in output
        )

    def test_check_tip_does_not_fetch_latest_release(self):
        with (
            patch("agile_cli.check_tool", return_value=True),
            patch(
                "agile_cli._version._fetch_latest_release_tag",
