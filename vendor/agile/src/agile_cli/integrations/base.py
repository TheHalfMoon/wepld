...
- ``SkillsIntegration`` — concrete base for integrations that install
  commands as agent skills (``agile-<name>/SKILL.md`` layout).
"""
...
class IntegrationBase(ABC):
    ...
    * ``multi_install_safe`` — declare the integration safe to install
    ...
    """Separator used in slash-command invocations (``"."`` → ``/agile.plan``)."""
    ...
    def _resolve_executable(self) -> str:
        ...
        Checks ``AGILE_INTEGRATION_<KEY>_EXECUTABLE`` first, allowing
        ...
        ``AGILE_INTEGRATION_KIRO_CLI_EXECUTABLE``.
        ...
        """
        env_name = (
            f"AGILE_INTEGRATION_{self.key.upper().replace('-', '_')}_EXECUTABLE"
        )
        override = os.environ.get(env_name, "").strip()
        return override if override else self.key

    def _apply_extra_args_env_var(self, args: list[str]) -> None:
        """Append `AGILE_INTEGRATION_<KEY>_EXTRA_ARGS` env-var value to *args*.
        ...
        e.g. `AGILE_INTEGRATION_CLAUDE_EXTRA_ARGS="--dangerously-skip-permissions"`.
        The `INTEGRATION` segment scopes the variable to this subsystem
        so it does not collide with other Agile env-var namespaces.
        Hyphens in the integration key are replaced with underscores
        and the key is uppercased
        (e.g. `kiro-cli` → `AGILE_INTEGRATION_KIRO_CLI_EXTRA_ARGS`).
        ...
        """
        env_name = (
            f"AGILE_INTEGRATION_{self.key.upper().replace('-', '_')}_EXTRA_ARGS"
        )
    ...
    def build_command_invocation(self, command_name: str, args: str = "") -> str:
        """Build the native slash-command invocation for a Agile command.
    ...
    def dispatch_command(
        ...
        *,
        ...
        """Dispatch a Agile command through this integration's CLI.
    ...
    def command_filename(self, template_name: str) -> str:
        ...
        *template_name* is the stem of the source file (e.g. ``"plan"``).
        Default: ``agile.{template_name}.md``.  Subclasses override
        to change the extension or naming convention.
        """
        return f"agile.{template_name}.md"
    ...
    def install_scripts(
        ...
        ``.agile/integrations/<key>/scripts/`` in the project.  Shell
        ...
        created: list[Path] = []
        scripts_dest = project_root / ".agile" / "integrations" / self.key / "scripts"
    ...
    def resolve_command_refs(
        ...
        """Replace ``__AGILE_COMMAND_<NAME>__`` placeholders with invocations.

        Each placeholder encodes a command name in upper-case with
        underscores (e.g. ``__AGILE_COMMAND_PLAN__``,
        ``__AGILE_COMMAND_GIT_COMMIT__``).  The replacement uses
        *separator* to join the segments:

        * ``separator="."`` → ``/agile.plan``, ``/agile.git.commit``
        * ``separator="-"`` → ``/agile-plan``, ``/agile-git-commit``
        ...
        *prefix* defaults to ``"/"`` but may be ``"$"`` for agents whose
        ...
        """
        return re.sub(
            r"__AGILE_COMMAND_([A-Z][A-Z0-9_]*)__",
            lambda m: prefix
            + "agile"
            + separator
    ...
    def process_template(
        ...
        6. Rewrite paths: ``scripts/`` → ``.agile/scripts/`` etc.
        7. Replace ``__AGILE_COMMAND_<NAME>__`` with invocation strings
        """
        ...
        # 8. Replace __AGILE_COMMAND_<NAME>__ with invocation strings
...
class TomlIntegration(IntegrationBase):
    ...
    def command_filename(self, template_name: str) -> str:
        ...
        return f"agile.{template_name}.toml"
    ...
    # extension/preset renderer in ``agile_cli.agents`` via
    # ``agile_cli._toml_string`` so the two never drift apart.
...
class YamlIntegration(IntegrationBase):
    ...
    def command_filename(self, template_name: str) -> str:
        ...
        return f"agile.{template_name}.yaml"
    ...
    def _human_title(identifier: str) -> str:
        ...
        Strips a leading ``agile.`` prefix and replaces ``.``, ``-``,
        and ``_`` with spaces before title-casing.
        """
        text = identifier
        if text.startswith("agile."):
            text = text[len("agile.") :]
...
class SkillsIntegration(IntegrationBase):
    ...
    Skills use the ``agile-<name>/SKILL.md`` directory layout following
    ...
    ``agile-<name>/SKILL.md`` file with skills-oriented frontmatter.
    """
    ...
    def build_command_invocation(self, command_name: str, args: str = "") -> str:
        ...
        invocation = prefix + "agile-" + stem.replace(".", "-")
        if args:
            invocation = f"{invocation} {args}"
        return invocation
    ...
    def _inject_hook_command_note(
        ...
        if invocation_prefix != "/":
            note = note.replace(
                "`/agile-git-commit`",
                f"`{invocation_prefix}agile-git-commit`",
            )
    ...
    def post_process_skill_content(self, content: str) -> str:
        ...
        hyphenated command invocation (e.g. ``/agile-git-commit`` or
        ``$agile-git-commit``).  Subclasses may override -- see
        ``ClaudeIntegration``.
        """
    ...
    def setup(
        ...
        **opts: Any,
        ...
        Creates ``agile-<name>/SKILL.md`` for each shared command
        ...
        for src_file in templates:
            ...
            skill_name = f"agile-{command_name.replace('.', '-')}"
            ...
            description = frontmatter.get("description", "")
            if not description:
                description = f"Agile: {command_name} workflow"
            ...
            skill_content = (
                ...
                f"compatibility: {yaml_quote('Requires agile project structure with .agile/ directory')}\n"
                f"metadata:\n"
                f"  author: {yaml_quote('github-agile')}\n"
