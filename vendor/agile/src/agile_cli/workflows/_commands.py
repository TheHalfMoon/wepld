...
`agile_cli._require_specify_project` keeps working.
"""
...
def _error_console(json_output: bool):
    ...
    stderr-only error routing already used by ``agile bundle``.
    """
...
def _reject_unsafe_dir(path: Path, label: str) -> None:
    ...
    A symlinked ``.agile`` (or ``.agile/workflows``) could redirect
...
def _reject_unsafe_workflow_storage(project_root: Path) -> None:
    ...
    _reject_unsafe_dir(project_root / ".agile", ".agile")
    _reject_unsafe_dir(project_root / ".agile" / "workflows", ".agile/workflows")
    _reject_unsafe_dir(
        project_root / ".agile" / "workflows" / "runs",
        ".agile/workflows/runs",
    )
    _reject_unsafe_dir(
        project_root / ".agile" / "workflows" / "overlays",
        ".agile/workflows/overlays",
    )


def _scan_for_workflow_owner(parts: tuple[str, ...]) -> int | None:
    """Find the *nearest* (innermost) ``.agile/workflows/<id>`` owner in
    *parts*, scanning from the end of the path.
    ...
    happens to reuse the same ``.agile``/``workflows`` segment names: the
    ...
    Returns the index of the owning ``.agile`` segment, or ``None`` if no
    owner segment is present.
    """
    for i in range(len(parts) - 3, -1, -1):
        if (
            parts[i].casefold() == ".agile"
...
def _resolve_installed_workflow_ownership(
    ...
    1. Lexically: the path's own (symlink-preserving) segments identify
       ``.agile/workflows/<id>`` -- collapsing ``..``/``.`` but
    ...
    2. Via an intermediate alias target whose lexical path identifies
       ``.agile/workflows/<id>`` before a symlinked storage ancestor is
       resolved away.
    ...
    def ownership_for(candidate: Path) -> tuple[Path, str] | None:
        ...
        canonical_specify = registry_root / ".agile"
        ...
        # project's own .agile is itself a symlink to an
        ...
        _reject_unsafe_dir(canonical_specify, ".agile")
        _reject_unsafe_dir(canonical_workflows, ".agile/workflows")
        _reject_unsafe_dir(candidate_specify, ".agile")
        _reject_unsafe_dir(candidate_workflows, ".agile/workflows")
        try:
        ...
        for k in range(i + 2, len(parts) + 1):
            if Path(*parts[:k]).is_symlink():
                err.print(
                    ...
                    f".agile/workflows/{_escape_markup(registered_id)} "
    ...
    # Full resolution can erase .agile/workflows ownership segments when
...
def _safe_workflow_id_dir(workflows_dir: Path, workflow_id: str) -> Path:
    ...
    _reject_unsafe_dir(dest_dir, f".agile/workflows/{safe_id}")
    try:
    ...
    if workflow_yml.is_symlink():
        console.print(
            ...
            f".agile/workflows/{safe_id}/workflow.yml"
        )
        raise typer.Exit(1)
    if workflow_yml.exists() and not workflow_yml.is_file():
        console.print(
            "[red]Error:[/red] "
            f".agile/workflows/{safe_id}/workflow.yml exists but is not a file"
        )
        raise typer.Exit(1)
    return dest_dir
...
# `agile_cli._require_specify_project` keeps working after the move.
...
def workflow_add(
    ...
    workflows_dir = project_root / ".agile" / "workflows"
    ...
    if download_url is not None:
        from urllib.parse import urlparse
        from agile_cli.authentication.http import open_url as _open_url
        ...
        from agile_cli._github_http import resolve_github_release_asset_api_url as _resolve_gh_asset
        from agile_cli.authentication.http import github_provider_hosts as _github_provider_hosts
...
def _remove_workflow_locked(
    ...
    if workflow_dir.exists() and not workflow_dir.is_dir():
        console.print(
            f"[red]Error:[/red] .agile/workflows/{safe_id} exists "
...
# IDs that map to internal names used under .agile/workflows/steps/ and must
...
def _resolve_steps_base_dir_or_exit(project_root: Path) -> Path:
    """Resolve .agile/workflows/steps while refusing symlinked parent directories."""
    project_root_resolved = project_root.resolve()
    steps_base_dir_unresolved = project_root / ".agile" / "workflows" / "steps"

    current = project_root
    for part in (".agile", "workflows", "steps"):
...
def workflow_step_add(
    ...
    try:
        ...
        tmp_path = Path(tempfile.mkdtemp(prefix="agile_step_tmp_", dir=steps_base_dir))
    except OSError as exc:
