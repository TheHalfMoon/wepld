...
def _run_init(project, *flags: str) -> Result:
    """Run ``agile init --here`` in *project* with the given extra flags.
...
def rovodev_init_project(tmp_path):
    """Run ``agile init --integration rovodev`` once and return the project root.
...
class TestRovodevIntegration:
    ...
    def test_build_exec_args_without_json(self):
        impl = get_integration(self.KEY)
        args = impl.build_exec_args("/agile.plan add OAuth", output_json=False)
        assert args == ["acli", "rovodev", "run", "/agile.plan add OAuth"]

    def test_build_exec_args_executable_env_override(self, monkeypatch):
        """AGILE_INTEGRATION_ROVODEV_EXECUTABLE overrides the binary path.
        ...
        """
        monkeypatch.setenv("AGILE_INTEGRATION_ROVODEV_EXECUTABLE", "/opt/atl/bin/acli")
        impl = get_integration(self.KEY)
    ...
    def test_build_exec_args_executable_env_blank_falls_back(self, monkeypatch):
        ...
        monkeypatch.setenv("AGILE_INTEGRATION_ROVODEV_EXECUTABLE", "   ")
        impl = get_integration(self.KEY)
        args = impl.build_exec_args("hello", output_json=False)
        assert args[0] == "acli"

    def test_build_exec_args_extra_args_env_injection(self, monkeypatch):
        """AGILE_INTEGRATION_ROVODEV_EXTRA_ARGS injects extra CLI flags.
        ...
        """
        monkeypatch.setenv("AGILE_INTEGRATION_ROVODEV_EXTRA_ARGS", "--quiet --no-color")
        impl = get_integration(self.KEY)
        args = impl.build_exec_args("hello", output_json=False)
        assert args == [
    ...
    def test_setup_creates_prompts_and_manifest(self, tmp_path):
        ...
        prompt_files = sorted(prompts_dir.glob("agile-*.prompt.md"))
        skill_dirs = sorted(d for d in skills_dir.iterdir() if d.is_dir() and d.name.startswith("agile-"))
    ...
    def test_prompts_manifest_entries_well_formed(self, tmp_path):
        ...
        for entry in entries:
            assert entry["name"].startswith("agile-")
    ...
    def test_init_skill_files_well_formed(self, rovodev_init_project):
        """Every agile-* SKILL.md from full init has valid frontmatter +
        ...
        for skill_dir in skill_dirs:
            ...
            for placeholder in ("{SCRIPT}", "__AGENT__", "__CONTEXT_FILE__", "__AGILE_COMMAND_"):
                assert placeholder not in body, (
                    f"{skill_file} body contains unprocessed placeholder {placeholder!r}"
                )
            # Skills agents must use hyphen-style refs in body.
            assert "/agile." not in body, (
                f"{skill_file} body contains dot-notation /agile. reference"
            )
    ...
    def test_init_writes_integration_manifest_and_options(self, rovodev_init_project):
        ...
        manifest_path = project / ".agile" / "integrations" / "rovodev.manifest.json"
        agile_manifest = project / ".agile" / "integrations" / "agile.manifest.json"
        assert manifest_path.exists(), "rovodev integration manifest missing"
        assert agile_manifest.exists(), "agile shared manifest missing"
    ...
    def test_integration_flag_creates_expected_files(self, tmp_path):
        ...
        result = _run_init(project, "--integration", "rovodev")
        assert result.exit_code == 0, result.output
        assert (project / ".rovodev" / "skills" / "agile-plan" / "SKILL.md").exists()
        assert (project / ".rovodev" / "prompts.yml").exists()
        assert (project / ".agile" / "integrations" / "rovodev.manifest.json").exists()
