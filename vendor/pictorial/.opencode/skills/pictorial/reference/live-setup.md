...
Create the file at the `path` the boot reported (default `.pictorial/live/config.json`):
...
**Framework adapters (auto-detected at inject time).** Every inject records what it wrote in `.pictorial/live/inject-journal.json`; the next inject or remove heals artifacts a crash or wrong-directory stop left behind. SvelteKit, Nuxt, and TanStack Start server-render their document shell, so a raw `<script>` in the entry template will not execute reliably; `live-inject.mjs` detects them and routes to a dedicated adapter (SvelteKit: dev-only root component from `+layout.svelte`; Nuxt: dev-only `.client.ts` plugin; TanStack Start: a generated dev-only `PictorialLiveRoot` component in `__root`). The `files` value stays a valid detection/CSP hint but is not the literal insertion site. A plain TanStack Router SPA takes the baseline Vite path.
...
```bash
node .opencode/skills/pictorial/scripts/detect-csp.mjs
```
...
Declare near the top of the file that holds the CSP arrays, then append `...__pictorialLiveDev` to the script-src and connect-src arrays:
...
```ts
// Dev-only allowance so pictorial live mode can load.
const __pictorialLiveDev =
  process.env.NODE_ENV === "development" ? " http://localhost:8400" : "";
```

- `script-src 'self' 'unsafe-inline'` becomes `` `script-src 'self' 'unsafe-inline'${__pictorialLiveDev}` ``
- `connect-src 'self'` becomes `` `connect-src 'self'${__pictorialLiveDev}` ``
...
If the user said "no" to the CSP patch and later reports live not working: their dev CSP blocks `http://localhost:8400`. Delete `cspChecked` from `.pictorial/live/config.json` and re-run `live.mjs`; setup asks again.
