...
import { filterDetectionFindings } from '../cli/lib/pictorial-config.mjs';
...
describe('detectUrl — browser-only fixtures', () => {
  ...
  it('image-backed text: the overlay default pass pixel-samples the image itself', async () => {
    // Drives the OVERLAY entry (pictorialDetectAsync with default options),
    ...
    try {
      ...
      const groups = await page.evaluate(() => window.pictorialDetectAsync());
  ...
  it('scoped-ignore: data-pictorial-ignore waives its subtree in the browser walk', async () => {
  ...
  it('low-contrast: a gradient body ground with oklch stops is measured, never assumed white', async () => {
    // The github.com/TheHalfMoon/wepld FP class: `background: linear-gradient(oklch(7%…),
  ...
  it('typography side-by-side: element-level flag cases get regular overlays', async () => {
    ...
    try {
      ...
      await page.evaluate(() => { window.__PICTORIAL_CONFIG__ = { autoScan: false }; });
      await page.evaluate(browserScript);
      const result = await page.evaluate(() => {
        const groups = window.pictorialScan();
        ...
        return {
          ...
          hasBanner: Boolean(document.querySelector('.pictorial-banner')),
          overlays: document.querySelectorAll('.pictorial-overlay:not(.pictorial-banner)').length,
        };
      });
  ...
  it('browser API: visual contrast fallback resolves readable image backgrounds without overlays', async () => {
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        const before = document.querySelectorAll('.pictorial-overlay, .pictorial-label, .pictorial-banner').length;
        const analyses = await window.pictorialAnalyzeVisualContrast({ maxCandidates: 20, scrollOffscreen: true });
        const after = document.querySelectorAll('.pictorial-overlay, .pictorial-label, .pictorial-banner').length;
        return {
          before,
          after,
  ...
  it('browser API: visual contrast scan decorates visible findings without scrolling by default', async () => {
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        ...
        const syncScanResult = window.pictorialScan({
          visualContrast: true,
          visualContrastMaxCandidates: 20,
        });
        const syncDetectResult = window.pictorialDetect({
          visualContrast: true,
          serialize: true,
        });
        const groups = await window.pictorialScanAsync({
          visualContrast: true,
          visualContrastMaxCandidates: 20,
        });
        ...
        return {
          ...
          overlays: document.querySelectorAll('.pictorial-overlay:not(.pictorial-banner)').length,
          labels: document.querySelectorAll('.pictorial-label').length,
          analyses: window.pictorialGetLastVisualContrastAnalyses().filter(item => item.status === 'fail').length,
          scrollEvents,
          maxScrollY,
          finalScrollY: window.scrollY,
          syncScanIsArray: Array.isArray(syncScanResult),
          syncDetectIsArray: Array.isArray(syncDetectResult),
          hasAsyncApi: typeof window.pictorialScanAsync === 'function' && typeof window.pictorialDetectAsync === 'function',
        };
      });
      ...
      assert.equal(result.syncScanIsArray, true, `pictorialScan should keep a synchronous Array return: ${JSON.stringify(result)}`);
      assert.equal(result.syncDetectIsArray, true, `pictorialDetect should keep a synchronous Array return: ${JSON.stringify(result)}`);
      ...
      const refreshedOverlayResult = await page.evaluate(async () => {
        ...
        const initialGroups = window.pictorialScan({
          visualContrast: true,
          visualContrastMaxCandidates: 20,
        });
        ...
        while (
          Date.now() < deadline &&
          !/low contrast/i.test(target?._pictorialOverlay?.textContent || '')
        ) {
          const nextButton = target?._pictorialOverlay?.querySelector('button:last-of-type');
          if (nextButton) nextButton.click();
          await new Promise(resolve => setTimeout(resolve, 25));
        }
        const labelVariants = [];
        const overlay = target?._pictorialOverlay;
        for (let i = 0; i < 3; i++) {
        ...
        return {
          ...
          labelText: target?._pictorialOverlay?.textContent || '',
          labelVariants,
          overlayConnected: Boolean(target?._pictorialOverlay?.isConnected),
        };
      });
      ...
      const lazyResult = await page.evaluate(async () => {
        ...
        return {
          ...
          labels: document.querySelectorAll('.pictorial-label').length,
          analyses: window.pictorialGetLastVisualContrastAnalyses().filter(item => item.status === 'fail').length,
          targetHasOverlay: Boolean(target?._pictorialOverlay),
      ...
      const staleOverlayResult = await page.evaluate(async () => {
        ...
        window.scrollTo(0, 0);
        await new Promise(resolve => setTimeout(resolve, 50));
        await window.pictorialScanAsync({
          visualContrast: true,
          visualContrastMaxCandidates: 20,
        });
        const staleCleared = !target?._pictorialOverlay;
        target?.scrollIntoView({ block: 'center', inline: 'nearest', behavior: 'instant' });
        await new Promise(resolve => setTimeout(resolve, 250));
        return {
          staleCleared,
          targetHasOverlay: Boolean(target?._pictorialOverlay),
          targetOverlayConnected: Boolean(target?._pictorialOverlay?.isConnected),
          overlays: document.querySelectorAll('.pictorial-overlay:not(.pictorial-banner)').length,
          analyses: window.pictorialGetLastVisualContrastAnalyses().filter(item => item.status === 'fail').length,
        };
      });
  ...
  it('extension mode remove cancels pending lazy visual contrast work', async () => {
    ...
    try {
      ...
      await page.evaluate(() => {
        ...
        window.__pictorialMessages = [];
        window.addEventListener('message', event => {
          if (event.source !== window || !event.data?.source?.startsWith('pictorial-')) return;
          window.__pictorialMessages.push(event.data);
        });
      });
      await page.evaluate(browserScript);
      const result = await page.evaluate(async () => {
        ...
        while (
          ...
          !window.pictorialGetLastVisualContrastAnalyses()
            .some(item => item.status === 'unresolved' && item.reason === 'text outside viewport')
        ) {
          await new Promise(resolve => setTimeout(resolve, 25));
        }
        const unresolvedBeforeRemove = window.pictorialGetLastVisualContrastAnalyses()
        ...
        const resultsAfterRemove = window.__pictorialMessages
          .filter(message => message.source === 'pictorial-results').length;
        return {
          unresolvedBeforeRemove,
          overlayCount: document.querySelectorAll('.pictorial-overlay').length,
          targetHasOverlay: Boolean(target?._pictorialOverlay),
          resultsAfterRemove,
        };
      });
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        ...
        try {
          ...
          while (
            Date.now() < deadline &&
            !window.__pictorialMessages.some(message => message.source === 'pictorial-error')
          ) {
            await new Promise(resolve => setTimeout(resolve, 25));
          }
          return {
            ready: window.__pictorialMessages.some(message => message.source === 'pictorial-ready'),
            results: window.__pictorialMessages.some(message => message.source === 'pictorial-results'),
            errors: window.__pictorialMessages
              .filter(message => message.source === 'pictorial-error')
  ...
  it('extension mode echoes scan ids on result messages', async () => {
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        ...
        while (
          Date.now() < deadline &&
          !window.__pictorialMessages.some(message =>
            message.source === 'pictorial-results' &&
            message.scanId === 'scan-2'
          )
        ) {
          await new Promise(resolve => setTimeout(resolve, 25));
        }
        const resultMessage = window.__pictorialMessages.find(message =>
          message.source === 'pictorial-results' &&
          message.scanId === 'scan-2'
        );
        return {
  ...
  it('browser API: pictorialDetect is pure, pictorialScan decorates', async () => {
    const puppeteer = await import('puppeteer');
    const browser = await puppeteer.default.launch({
      headless: true,
      args: process.env.CI ? ['--no-sandbox', '--disable-setuid-sandbox'] : [],
    ...
    try {
      ...
      const pure = await page.evaluate(() => {
        const before = document.querySelectorAll('.pictorial-overlay, .pictorial-label, .pictorial-banner').length;
        const findings = window.pictorialDetect({ decorate: false, serialize: true });
        const after = document.querySelectorAll('.pictorial-overlay, .pictorial-label, .pictorial-banner').length;
      ...
      const decorated = await page.evaluate(() => {
        const groups = window.pictorialScan();
        const overlays = document.querySelectorAll('.pictorial-overlay, .pictorial-label, .pictorial-banner').length;
  ...
  it('browser API: async scan and detect reject instead of throwing synchronously', async () => {
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        ...
        try {
          const scan = await window.pictorialScanAsync().then(
            () => ({ state: 'resolved' }),
            error => ({ state: 'rejected', message: error?.message || String(error) }),
          );
          const detect = await window.pictorialDetectAsync().then(
            () => ({ state: 'resolved' }),
            error => ({ state: 'rejected', message: error?.message || String(error) }),
          );
          return { scan, detect };
  ...
  // into a false "on #ffffff" finding (two live scans of github.com/TheHalfMoon/wepld
