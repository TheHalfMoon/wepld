"""agile extension * and catalog * command handlers — app objects and register().
...
`agile_cli.<helper>` keeps working.
"""
...
from .._assets import get_agile_version
...
catalog_app = typer.Typer(
    ...
    help=(
        ...
        "something you found there, either use 'agile extension add <name> --from "
...
# `agile_cli.<name>` keeps working after the move.
...
def _resolve_installed_extension(
    ...
    if len(name_matches) == 1:
        ...
        console.print(f"  [bold]agile extension {command_name} <extension-id>[/bold]")
        raise typer.Exit(1)
    else:
...
def extension_list(
    ...
    if not installed and not (available or all_extensions):
        ...
        console.print("  agile extension add <extension-name>")
        return
    ...
    if available or all_extensions:
        ...
        console.print("  [cyan]agile extension add <name>[/cyan]")
...
def catalog_list():
    ...
    if any(not entry.install_allowed for entry in active_catalogs):
        console.print(
            ...
            "'agile extension add <name> --from <url>', or add it to a catalog you "
            "control. Don't flip a discovery-only catalog to install_allowed.[/dim]\n"
        )

    config_path = project_root / ".agile" / "extension-catalogs.yml"
    user_config_path = Path.home() / ".agile" / "extension-catalogs.yml"
    if os.environ.get("AGILE_CATALOG_URL"):
        console.print("[dim]Catalog configured via AGILE_CATALOG_URL environment variable.[/dim]")
    else:
        try:
        ...
        if proj_loaded:
            ...
            try:
                ...
                user_loaded = False
            if user_loaded:
                console.print("[dim]Config: ~/.agile/extension-catalogs.yml[/dim]")
            else:
                console.print("[dim]Using built-in default catalog stack.[/dim]")
                console.print(
                    "[dim]Add .agile/extension-catalogs.yml to customize.[/dim]"
                )
...
def catalog_add(
    ...
    """Add a catalog to .agile/extension-catalogs.yml."""
    from . import ExtensionCatalog, ValidationError

    project_root = _require_specify_project()
    specify_dir = project_root / ".agile"
    ...
    for existing in catalogs:
        if isinstance(existing, dict) and existing.get("name") == name:
            ...
            console.print("Use 'agile extension catalog remove' first, or choose a different name.")
            raise typer.Exit(1)
...
def catalog_remove(
    ...
    """Remove a catalog from .agile/extension-catalogs.yml."""
    project_root = _require_specify_project()
    specify_dir = project_root / ".agile"
...
_CACHE_REL_PARTS = (".agile", "extensions", ".cache", "downloads")
    ...
    try:
        with console.status(f"[cyan]Installing extension: {safe_extension}[/cyan]"):
            if dev:
                ...
                # archive). Same path used by ``agile init --extension <url>``.
                ...
                # Try bundled extensions first (shipped with agile)
                bundled_path = _locate_bundled_extension(extension)
                if bundled_path is not None:
                    manifest = manager.install_from_directory(
                        bundled_path, agile_version, priority=priority, force=force
                    )
                else:
                    ...
                    if not ext_info:
                        ...
                        console.print("  agile extension search")
                        raise typer.Exit(1)
                    ...
                    if bundled_path is None:
                        ...
                        if ext_info.get("bundled") and not ext_info.get("download_url"):
                            console.print(
                                f"[red]Error:[/red] Extension '{_escape_markup(ext_info['id'])}' is bundled with agile "
                                f"but could not be found in the installed package."
                            )
                            console.print(
                                "\nThis usually means the agile installation is incomplete or corrupted."
                            )
                            console.print("Try reinstalling agile:")
                        ...
                        if not ext_info.get("_install_allowed", True):
                            ...
                            console.print(
                                ...
                                f"      agile extension add {resolved_id} --from <archive-url>"
                            )
                            console.print(
        ...
        config_home = f".agile/extensions/{_escape_markup(str(manifest.id))}"
        if deployed:
...
def extension_remove(
    ...
    if not force:
        ...
        console.print(f"   • Extension directory: .agile/extensions/{safe_extension_id}/")
        if not keep_config:
    ...
    if success:
        ...
        if keep_config:
            console.print(f"\nConfig files preserved in .agile/extensions/{safe_extension_id}/")
        else:
            console.print(f"\nConfig files backed up to .agile/extensions/.backup/{safe_extension_id}/")

        # #1: regenerate native event config so the removed extension's events
        # are stripped from installed integrations.
        _refresh_events_and_warn(project_root)
        console.print(f"\nTo reinstall: agile extension add {safe_extension_id}")
    else:
    ...
    try:
        ...
        if not results:
            ...
            if query or tag or author or verified:
                ...
                console.print("  • agile extension search (show all)")
            raise typer.Exit(0)
        ...
        for ext in results:
            ...
            if install_allowed:
                console.print(f"\n  [cyan]Install:[/cyan] agile extension add {cmd_id}")
            else:
                console.print(f"\n  [yellow]⚠[/yellow]  Not directly installable from '{catalog_name}' (discovery-only).")
                console.print(
                    f"  Once vetted, install it directly: agile extension add {cmd_id} --from <archive-url>"
                )
                console.print(
    ...
    if resolved_installed_id:
        ...
        if ext_manifest:
            ...
            if ext_manifest.commands:
                ...
                # `/agile-jira-sync`, not the manifest's dotted
                # `agile.jira.sync`), so mirror the same formatting used by
                ...
                if selected_ai == "cline":
                    from agile_cli.integrations.cline import (
                        format_cline_command_name as _format_command_name,
                    )
                elif selected_ai == "forge":
                    from agile_cli.integrations.forge import (
        ...
        console.print(f"\nTo remove: agile extension remove {_escape_markup(str(resolved_installed_id))}")
        return
...
def _print_extension_info(ext_info: dict, manager):
    ...
    if ext_info.get('requires'):
        ...
        if reqs.get('agile_version'):
            console.print(f"  • Agile: {_escape_markup(str(reqs['agile_version']))}")
        if reqs.get('tools'):
    ...
    if is_installed:
        ...
        console.print(f"[dim]Priority:[/dim] {priority}")
        console.print(f"\nTo remove: agile extension remove {cmd_id}")
        ...
        if download_url:
            ...
            console.print(
                f"Once vetted, install directly: agile extension add {cmd_id} --from <archive-url>"
            )
        else:
            console.print(
                f"Once you've vetted its release archive, install directly: "
                f"agile extension add {cmd_id} --from <archive-url>"
            )
        console.print(
                ...
                try:
                    ...
                    with tempfile.TemporaryDirectory(
                        ...
                        manager.check_compatibility(
                            preflight_manifest, agile_version
                        )
                    ...
                    _ = manager.install_from_zip(archive_path, agile_version)
...
def extension_enable(
    ...
    config_home = f".agile/extensions/{_escape_markup(str(extension_id))}"
    if deployed:
        console.print("\n[bold cyan]Config scaffolded:[/bold cyan]")
        for cfg in deployed:
            console.print(f"  • {config_home}/{_escape_markup(str(cfg))}")
...
def extension_disable(
    ...
    console.print(f"To re-enable: agile extension enable {_escape_markup(str(extension_id))}")
