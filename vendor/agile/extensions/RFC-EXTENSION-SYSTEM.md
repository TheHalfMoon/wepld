...
Introduce an extension system to Agile that allows modular integration with external tools (Jira, Linear, Azure DevOps, etc.) without bloating the core framework. Extensions are self-contained packages installed into `.agile/extensions/` with declarative manifests, versioned independently, and discoverable through a central catalog.
...
1. **Monolithic Growth**: Adding Jira integration to core agile creates:
...
1. **Modularity**: Core agile remains lean, extensions are opt-in
...
- Standard directory structure (`.agile/extensions/{name}/`)
- Declarative manifest (`extension.yml`)
- Predictable command naming (`agile.{extension}.{command}`)
...
- Simple installation: `agile extension add jira`
...
│  ┌──────────────────────────────────────────────────┐   │
│  │  Jira Extension                                  │   │
│  │  - /agile.jira.specstoissues                   │   │
│  │  - /agile.jira.discover-fields                 │   │
│  └──────────────────────────────────────────────────┘   │
│  ┌──────────────────────────────────────────────────┐   │
│  │  Linear Extension                                │   │
│  │  - /agile.linear.sync                          │   │
...
extension:
  ...
  description: "Create Jira Epics, Stories, and Issues from agile artifacts"
  author: "Stats Perform"       # Author/organization
  repository: "https://github.com/statsperform/agile-jira"
  license: "MIT"                # SPDX license identifier
  homepage: "https://github.com/statsperform/agile-jira/blob/main/README.md"

# Compatibility requirements (REQUIRED)
requires:
  # Agile version (semantic version range)
  agile_version: ">=0.1.0,<2.0.0"
  ...
  # Core agile commands this extension depends on
  commands:
    - "agile.tasks"             # Extension needs tasks command
...
hooks:
  # Hook fired after /agile.tasks completes
  after_tasks:
    command: "agile.jira.specstoissues"
    optional: true
    prompt: "Create Jira issues from tasks?"
    description: "Automatically create Jira hierarchy after task generation"

  # Hook fired after /agile.implement completes
  after_implement:
    command: "agile.jira.sync-status"
    optional: true
...
changelog: "https://github.com/statsperform/agile-jira/blob/main/CHANGELOG.md"

# Support information (OPTIONAL)
support:
  documentation: "https://github.com/statsperform/agile-jira/blob/main/docs/"
  issues: "https://github.com/statsperform/agile-jira/issues"
  discussions: "https://github.com/statsperform/agile-jira/discussions"
...
4. **Extract**: Unpack to `.agile/extensions/jira/`
5. **Configure**: Copy config templates
6. **Register**: Add commands to AI agent config
7. **Record**: Update `.agile/extensions/.registry`

**Registry Format** (`.agile/extensions/.registry`):
...
vim .agile/extensions/jira/jira-config.yml
```
...
4. Environment variables (`AGILE_JIRA_*`)
...
1. AI agent finds command in `.claude/commands/agile.jira.specstoissues.md`
...
3. Remove from `.agile/extensions/jira/`
4. Update registry
...
**Output**: `.claude/commands/agile.jira.specstoissues.md`
...
tools:
  ...
  - 'jira-mcp-server/story_create'
scripts:
  sh: .agile/scripts/bash/check-prerequisites.sh --json
  ps: .agile/scripts/powershell/check-prerequisites.ps1 -Json
---
...
## Steps
1. Load jira-config.yml from .agile/extensions/jira/
2. Parse spec.md and tasks.md
3. Create Jira items
```
...
**Output**: `.gemini/commands/agile.jira.specstoissues.toml`
...
**Location**: `src/agile_cli/extensions.py`
...
def register_extension_commands(
    ...
    ext_dir = project_path / ".agile" / "extensions" / ext_id
...
```yaml
# .agile/extensions/jira/jira-config.yml (Project config)
project:
  key: "MSATS"
...
```yaml
# .agile/extensions/jira/jira-config.local.yml (Local overrides - gitignored)
project:
...
export AGILE_JIRA_PROJECT_KEY="DEVTEST"
```
...
```bash
config_json=$(bash .agile/extensions/jira/scripts/parse-jira-config.sh)
echo "$config_json"
```
...
**Script**: `.agile/extensions/jira/scripts/parse-jira-config.sh`

```bash
#!/usr/bin/env bash
set -euo pipefail

EXT_DIR=".agile/extensions/jira"
...
if [ -n "${AGILE_JIRA_PROJECT_KEY:-}" ]; then
  defaults=$(echo "$defaults" | jq ".project.key = \"$AGILE_JIRA_PROJECT_KEY\"")
fi
...
schema = load_yaml(".agile/extensions/jira/extension.yml")['config_schema']
...
**File**: `.agile/extensions.yml` (project-level extension config)
...
if [ -f ".agile/extensions.yml" ]; then
  # Parse hooks for after_tasks
  hooks=$(yq eval '.hooks.after_tasks[] | select(.enabled == true)' .agile/extensions.yml -o=json)
...
The AI agent sees `EXECUTE_COMMAND: agile.jira.specstoissues` in output and automatically invokes that command.
...
Agile uses two catalog files with different purposes:
...
1. **Discover**: `agile extension search` searches both catalogs — community extensions appear automatically
2. **Review**: Evaluate community extensions for security, quality, and organizational fit
3. **Curate**: Copy approved entries from community catalog to your `catalog.json`, or add to `.agile/extension-catalogs.yml` with `install_allowed: true`
4. **Install**: Use `agile extension add <name>` — only allowed from `install_allowed: true` catalogs
...
```json
{
  ...
  "extensions": {
    "jira": {
      ...
      "version": "1.0.0",
      "download_url": "https://github.com/statsperform/agile-jira/releases/download/v1.0.0/agile-jira-1.0.0.zip",
      "repository": "https://github.com/statsperform/agile-jira",
      "homepage": "https://github.com/statsperform/agile-jira/blob/main/README.md",
      "documentation": "https://github.com/statsperform/agile-jira/blob/main/docs/",
      "changelog": "https://github.com/statsperform/agile-jira/blob/main/CHANGELOG.md",
    ...
    "linear": {
      ...
      "id": "linear",
      "description": "Sync agile tasks with Linear issues",
      "author": "Community",
      "version": "0.9.0",
      "download_url": "https://github.com/example/agile-linear/releases/download/v0.9.0/agile-linear-0.9.0.zip",
      "repository": "https://github.com/example/agile-linear",
      "requires": {
        "agile_version": ">=0.1.0"
      },
...
# Search by tag
agile extension search --tag issue-tracking

# Show extension details
agile extension info jira
```

### Custom Catalogs

Agile supports a **catalog stack** — an ordered list of catalogs that the CLI merges and searches across. This allows organizations to maintain their own org-approved extensions alongside an internal catalog and community discovery, all at once.

#### Catalog Stack Resolution

The active catalog stack is resolved in this order (first match wins):

1. **`AGILE_CATALOG_URL` environment variable** — single catalog replacing all defaults (backward compat)
2. **Project-level `.agile/extension-catalogs.yml`** — full control for the project
3. **User-level `~/.agile/extension-catalogs.yml`** — personal defaults
...
This means `agile extension search` surfaces community extensions out of the box, while `agile extension add` is still restricted to entries from catalogs with `install_allowed: true`.

#### `.agile/extension-catalogs.yml` Config File
...
catalogs:
  ...
  - name: "internal"
    url: "https://internal.company.com/agile/catalog.json"
    priority: 2
    install_allowed: true
...
A user-level equivalent lives at `~/.agile/extension-catalogs.yml`. When a project-level config is present with one or more catalog entries, it takes full control and the built-in defaults are not applied. An empty `catalogs: []` list is treated the same as no config file, falling back to defaults.
...
agile extension catalog add --name "internal" --install-allowed \
  https://internal.company.com/agile/catalog.json
...
Extensions from discovery-only catalogs are shown in `agile extension search` results but cannot be installed directly:

```
⚠  'linear' is available in the 'community' catalog but installation is not allowed from that catalog.

To enable installation, add 'linear' to an approved catalog (install_allowed: true) in .agile/extension-catalogs.yml.
```

#### `AGILE_CATALOG_URL` (Backward Compatibility)

The `AGILE_CATALOG_URL` environment variable still works — it is treated as a single `install_allowed: true` catalog, **replacing both defaults** for full backward compatibility:

```bash
# Point to your organization's catalog
export AGILE_CATALOG_URL="https://internal.company.com/agile/catalog.json"

# All extension commands now use your custom catalog
agile extension search       # Uses custom catalog
agile extension add jira     # Installs from custom catalog
```
...
export AGILE_CATALOG_URL="http://localhost:8000/catalog.json"
agile extension search
```
...
Installed Extensions:
  ✓ Jira Integration (v1.0.0)
     jira
     Create Jira issues from agile artifacts
     Commands: 3 | Hooks: 2 | Priority: 10 | Status: Enabled

  ✓ Linear Integration (v0.9.0)
     linear
     Create Linear issues from agile artifacts
...
│ Jira Integration                                        │
│                                                         │
│ Create Jira Epics, Stories, and Issues from agile   │
│ artifacts                                               │
│                                                         │
│ Author: Stats Perform                                   │
...
│ Downloads: 1,250                                        │
│                                                         │
│ Repository: github.com/statsperform/agile-jira       │
│ Documentation: github.com/.../docs                      │
└─────────────────────────────────────────────────────────┘

Install: agile extension add jira
```
...
Description:
  Create Jira Epics, Stories, and Issues from agile artifacts
...
Provides:
  Commands:
    • agile.jira.specstoissues - Create Jira hierarchy from spec and tasks
    • agile.jira.discover-fields - Discover Jira custom fields
    • agile.jira.sync-status - Sync task completion status
...
✓ Downloaded agile-jira-1.0.0.zip (245 KB)
✓ Validated manifest
✓ Checked compatibility (agile 0.1.0 ≥ 0.1.0)
✓ Extracted to .agile/extensions/jira/
✓ Registered 3 commands with claude
✓ Installed config template (jira-config.yml)

⚠  Configuration required:
   Edit .agile/extensions/jira/jira-config.yml to set your Jira project key

Extension installed successfully!

Next steps:
  1. Configure: vim .agile/extensions/jira/jira-config.yml
  2. Discover fields: /agile.jira.discover-fields
  3. Use commands: /agile.jira.specstoissues
```
...
⚠  This will remove:
   ...
   • Extension directory: .agile/extensions/jira/
   • Config file: jira-config.yml (will be backed up)

Continue? (yes/no): yes

✓ Unregistered commands
✓ Backed up config to .agile/extensions/.backup/jira-config.yml
✓ Removed extension directory
✓ Updated registry

Extension removed successfully.

To reinstall: agile extension add jira
```
...
✓ Downloaded agile-jira-1.1.0.zip
✓ Validated manifest
...
Changelog: https://github.com/statsperform/agile-jira/blob/main/CHANGELOG.md#v110
```
...
```bash
$ agile extension set-priority jira 5
...
def check_compatibility(extension_manifest: dict) -> bool:
    ...
    # 1. Check agile version
    current_agile = get_agile_version()  # e.g., "0.1.5"
    required_agile = requires['agile_version']  # e.g., ">=0.1.0,<2.0.0"

    if not version_satisfies(current_agile, required_agile):
        raise IncompatibleVersionError(
            f"Extension requires agile {required_agile}, "
            f"but {current_agile} is installed. "
            f"Upgrade agile with: uv tool install wepld-agile --force"
        )
    ...
    for cmd in requires.get('commands', []):
        if not command_exists(cmd):
            raise MissingCommandError(
                ...
                f"Update agile to latest version"
            )
...
provides:
  commands:
    - name: "agile.jira.old-command"
      file: "commands/old-command.md"
      deprecated: true
      deprecated_message: "Use agile.jira.new-command instead"
...
```text
⚠️  Warning: /agile.jira.old-command is deprecated
   Use /agile.jira.new-command instead
...
- Code reviewed by agile maintainers
...
permissions:
  - "filesystem:read:.agile/extensions/jira/"  # Can only read own config
  - "filesystem:write:.agile/memory/"          # Can write to memory
  - "network:external:*.atlassian.net"           # Can call Jira API
  - "env:read:AGILE_JIRA_*"                    # Can read own env vars
```
...
"jira": {
  ...
  "signature": "https://github.com/.../agile-jira-1.0.0.sig",
...
**Goal**: Existing agile projects work without changes.

**Strategy**:

1. **Core commands unchanged**: `/agile.tasks`, `/agile.implement`, etc. remain in core
...
agile extension add github-projects
/agile.github.taskstoissues
```
...
provides:
  commands:
    - name: "agile.github.taskstoissues"
      file: "commands/taskstoissues.md"
      aliases: ["agile.github.sync-taskstoissues"]  # Alternate namespaced entry point
```

AI agents register both names, so callers can migrate to the alternate alias without relying on deprecated global shortcuts like `/agile.taskstoissues`.
...
- [x] CLI commands:
  - [x] `agile extension list`
  - [x] `agile extension add` (from URL and local `--dev`)
  - [x] `agile extension remove`
- [x] Extension registry (`.agile/extensions/.registry`)
...
- [x] Create `agile-jira` repository
...
- [x] Central catalog (`extensions/catalog.json` in agile repo)
- [x] Community catalog (`extensions/catalog.community.json`)
- [x] Catalog fetch and parsing with multi-catalog support
- [x] CLI commands:
  - [x] `agile extension search`
  - [x] `agile extension info`
  - [x] `agile extension catalog list`
  - [x] `agile extension catalog add`
  - [x] `agile extension catalog remove`
...
- [x] Project extensions config (`.agile/extensions.yml`)
- [x] CLI commands:
  - [x] `agile extension update` (with atomic backup/restore)
  - [x] `agile extension enable/disable`
...
- [x] **Cache invalidation**: Cache invalidated when `AGILE_CATALOG_URL` changes
...
**Decision**: **Option C** - Both prefixed and aliases are supported. Commands use `agile.{extension}.{command}` as canonical name, with optional aliases defined in manifest.
...
**Decision**: **Option A** - Extension directory (`.agile/extensions/{ext-id}/{ext-id}-config.yml`). This keeps extensions self-contained and easier to manage.
...
**Decision**: **Option A** - Hooks are registered in `.agile/extensions.yml` and executed by the AI agent when it sees the hook trigger. Hook state (enabled/disabled) is managed per-extension.
...
**Complete structure of `agile-jira` extension:**
...
| Extension Feature | Agile Version | AI Agent Support |
|-------------------|------------------|------------------|
...
This RFC proposes a comprehensive extension system for Agile that:
