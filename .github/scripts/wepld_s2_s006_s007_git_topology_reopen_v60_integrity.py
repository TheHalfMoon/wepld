#!/usr/bin/env python3
"""Authorize a bounded, single-use reopen of the frozen S2-S006/S2-S007
`crates/core/tests/git_topology_v1.rs` test evidence gap.

v60 is an append-only policy successor over canonical v59 at main
961e15671a7511f204f1ebf08f5ddda8ab519a00. Founder standing authorization for
...
test path so a real adversarial fixture for S2-S006 (malicious `.git`/gitfile/
topology parsing) and S2-S007 (no-hook proof for read-only topology
observation; the hard-timeout half of S2-S007 remains a separately recorded,
explicit obligation and is not claimed here) can be recorded.
`crates/core/tests/git_topology_v1.rs` was reopened before by v52 (for
S2-I006/S2-I007, PR #287) and by v53 (for S2-S005, PR #292); both grants are
long consumed, so the path is frozen again by the same v25 Core-observation-
tranche freeze mechanism those successors themselves reopened against. A
fresh single-use reopen is required. It is not a wholly new file.

v60 grants exactly one thing and nothing else:
...
it only writes hook scripts to a fixture repository's `.git/hooks/` directory
and asserts, via marker files, that the adapter's read-only `rev-parse`/
`worktree list --porcelain -z` calls never execute any of them -- Git's own
design never invokes a hook for either command, and this fixture proves that
rather than assuming it. No `unsafe` code (the file keeps
...
against the inherited v59 chain.

Package-load / resting-view note: v60 follows the v45..v59 discipline. It
owns a fresh `LocalRepositoryView` of the exact checked-out head, imports
frozen v59 under an exact v60->v59 workflow-entrypoint reversal, and inherits
every v59 hook by reference. The one path this successor reopens is subject
...
predecessor delegation, exactly the way v45/v52/v53/v54 intercept their own
reopened paths - so no further predecessor-facing projection is required
...
scanned exactly as written, exactly like v52/v53's own git_topology_v1.rs
grants.
"""
...
P = ".github/scripts/wepld_s2_s006_s007_git_topology_reopen_v60_integrity.py"
T = ".github/scripts/wepld_s2_s006_s007_git_topology_reopen_v60_selftest.py"
T_BLOB = "a13d7925ba93ca7c2e4e0bcc5ac9634fe4d6533e"

V59_P_BLOB = "5184b8ca796e51cace5bd26353633cfe8ce7998f"
V59_T_BLOB = "381667a670a134f05f6b20ac425a7e6dfeafd231"
...
_V59_ENTRYPOINT = b"wepld_s2_s001_project_v1_reopen_v59_integrity.py"
_WORKFLOW_ENTRYPOINT_COUNTS = {FW: 3, AW: 2}

# Do not inherit a predecessor module's resting/projection view. v60 bases
...
def _v59_workflow_bytes(data: bytes, path: str) -> bytes:
    count = data.count(_V60_ENTRYPOINT)
    if count != _WORKFLOW_ENTRYPOINT_COUNTS[path]:
        base.fail(
            "v60 workflow entrypoint count drifted before v59 projection: "
            f"{path} expected={_WORKFLOW_ENTRYPOINT_COUNTS[path]} actual={count}"
        )
    return data.replace(_V60_ENTRYPOINT, _V59_ENTRYPOINT)


def _import_v59_under_workflow_projection() -> Any:
    """Import frozen v59 while it observes exact v59 workflow bytes.

    v59 (hence v57..v45..v36) reads workflow bytes while its module is
    imported, and the v59->v60 entrypoint migration ships in this same
    candidate, so v59 must not observe its own successor's bytes. Only
    ...
    def _v59_import_read_bytes(local_view: Any, relative: str, limit: int) -> bytes:
        data = original_read_bytes(local_view, relative, limit)
        if relative in (FW, AW):
            data = _v59_workflow_bytes(data, relative)
            if len(data) > limit:
                base.fail(
                    f"v60 v59-import workflow projection exceeds read bound: {relative}"
                )
        return data

    base.LocalRepositoryView.read_bytes = _v59_import_read_bytes
    try:
        return importlib.import_module(
            "wepld_s2_s001_project_v1_reopen_v59_integrity"
        )
    finally:
        base.LocalRepositoryView.read_bytes = original_read_bytes


q = _import_v59_under_workflow_projection()
...
REOPEN_TASKS = frozenset({"S2-S006", "S2-S007"})
...
# consumed.
EXACT_FROZEN_BLOB = "c365612eda55c1f224249f5b3fec869b18c6eac1"

AUTH = "S2_S006_S007_GIT_TOPOLOGY_REOPEN_ONLY"
...
# `observe_git_topology`) through a real Git fixture repository and its own
# `.git/hooks/` directory.
S2_S006_S007_GIT_TOPOLOGY_REOPEN_AUTHORITY = (
    "SINGLE_USE_TEST_ONLY_REOPEN_OF_GIT_TOPOLOGY_V1_RS_FOR_S2_S006_S007"
)

S2_S006_S007_GIT_TOPOLOGY_REOPEN_CONTRACT = (
    "TEST_FILE_ONLY",
    "SINGLE_PATH_SCOPE",
    "SINGLE_USE_REOPEN",
    "NO_SOURCE_CHANGE",
...
for _path, _expected in ((q.P, V59_P_BLOB), (q.T, V59_T_BLOB), (T, T_BLOB)):
    _actual = V25.blob(raw_root.read_bytes(_path, base.MAX_POLICY_FILE_BYTES))
    if _actual != _expected:
        base.fail(
            f"frozen v60 package input drifted: {_path}: "
...
def _workflow_replacements(view: Any) -> dict[str, bytes]:
    ...
    for path in (FW, AW):
        ...
        predecessor = _v59_workflow_bytes(data, path)
        actual = V25.sha(predecessor)
        if actual != Q_WF[path]:
            base.fail(
                "v60 workflow does not reverse to exact canonical v59 predecessor: "
...
def req_v59(view: Any) -> None:
    for path, expected in ((q.P, V59_P_BLOB), (q.T, V59_T_BLOB)):
        if path not in V25.ps(view):
            base.fail(f"v60 candidate/base is missing frozen v59 predecessor: {path}")
        actual = V25.blob(view.read_bytes(path, base.MAX_POLICY_FILE_BYTES))
        if actual != expected:
            base.fail(
                f"frozen v59 predecessor drifted: {path}: "
                f"expected={expected} actual={actual}"
            )


def _project_for_v59(view: Any) -> Any:
...
def _v59_views(candidate: Any, policy_base: Any) -> tuple[Any, Any]:
    """Project the candidate to v59's view always; project the policy base
    only when it is a real post-v60 base. A pre-v60 bootstrap base predates
    the v59->v60 workflow migration and carries no v60 policy files, so it
    must reach v59's frozen hooks unprojected."""
    projected_candidate = _project_for_v59(candidate)
    if bootbase(policy_base):
        return projected_candidate, policy_base
    return projected_candidate, _project_for_v59(policy_base)


def run_predecessor_selftests() -> None:
    """Run frozen v59's own self-tests once, under a v60->v59 workflow
    reversal.

    v59's corrected hooks are inherited by reference. Only ``read_bytes`` is
    wrapped here for the v60->v59 workflow reversal; the wrap is restored in
    ...
    def _v59_selftest_read_bytes(local_view: Any, relative: str, limit: int) -> bytes:
        if relative in workflow_reversal:
            data = workflow_reversal[relative]
            if len(data) > limit:
                base.fail(
                    f"v60 v59-selftest workflow projection exceeds read bound: {relative}"
                )
            return data
        return original_read_bytes(local_view, relative, limit)

    base.LocalRepositoryView.read_bytes = _v59_selftest_read_bytes
    try:
        _call("v59 self-tests under v60->v59 workflow reversal", q.selftest)
    finally:
        base.LocalRepositoryView.read_bytes = original_read_bytes
...
def _reopen_available(view: Any) -> bool:
    if REOPEN_TEST not in V25.ps(view):
        base.fail(f"v60 reopen base is missing the frozen reopened path: {REOPEN_TEST}")
...
def _require_reopen_base(view: Any) -> None:
    if not _reopen_available(view):
        base.fail(
            "v60 S2-S006/S2-S007 git_topology_v1.rs evidence reopen already consumed; "
...
def _verify_text_reopen_file(view: Any, path: str) -> None:
    if path not in V25.ps(view):
        base.fail(f"v60 reopen path missing: {path}")
    if V25.mode(view, path) != "100644":
        base.fail(f"v60 reopen path mode invalid: {path}")
    data = view.read_bytes(path, base.MAX_POLICY_FILE_BYTES)
    if not data:
        base.fail(f"v60 reopen path must not be empty: {path}")
    if len(data) > MAX_REOPEN_FILE_BYTES:
        base.fail(f"v60 reopen path exceeds bounded size: {path}")
    try:
        data.decode("utf-8", errors="strict")
    except UnicodeError as exc:
        base.fail(f"v60 reopen path must be UTF-8: {path}: {exc}")
...
def _verify_reopen_candidate(candidate: Any, policy_base: Any) -> None:
    ...
    for path in (
        ...
        if candidate.read_bytes(path, base.MAX_POLICY_FILE_BYTES) != policy_base.read_bytes(
            ...
            base.fail(f"v60 S2-S006/S2-S007 git_topology_v1.rs evidence reopen must not change frozen path: {path}")
    for relative in sorted(V25.FROZEN_STATE_PATHS):
        if candidate.read_bytes(relative, V25.MAX_S1_STATE_BYTES) != policy_base.read_bytes(
            ...
            base.fail(f"v60 candidate changed frozen S1 state: {relative}")
...
def delta(candidate: Any, policy_base: Any) -> None:
    ...
    if bootbase(policy_base):
        if paths != BOOT:
            if paths & BOOT:
                base.fail(
                    "v60 bootstrap delta must be exactly two v60 policy files plus two integrity workflows"
                )
            base.fail("v60 bootstrap base authorizes only exact S2-S006/S2-S007 git_topology_v1.rs reopen policy activation")
        req_v59(candidate)
        req_v59(policy_base)
        return

    if paths & CONTROLLED_FILES:
        base.fail("canonical v60 policy files are frozen after activation")

    reopen_changed = frozenset(paths & REOPEN_FILES)
    if reopen_changed:
        if paths != REOPEN_FILES:
            base.fail("v60 S2-S006/S2-S007 git_topology_v1.rs evidence reopen must not mix with non-reopen paths")
        _verify_reopen_candidate(candidate, policy_base)
        return

    q.delta(_project_for_v59(candidate), _project_for_v59(policy_base))


def basectrl(candidate: Any, policy_base: Any) -> None:
    if not bootbase(policy_base):
        q.basectrl(*_v59_views(candidate, policy_base))
        return
    for path in sorted(base.BASE_CONTROLLED_PATHS):
        ...
        if path in (FW, AW):
            if V25.sha(candidate_bytes) != WF[path] or V25.sha(base_bytes) != Q_WF[path]:
                base.fail(f"v60 bootstrap workflow drifted: {path}")
...
def ext(candidate: Any, policy_base: Any, safe: Any) -> None:
    ...
    for path in sorted(CONTROLLED_FILES & safe_paths):
        if path not in V25.ps(candidate):
            base.fail(f"v60 controlled file missing: {path}")
        if bootbase(policy_base):
            if path in V25.ps(policy_base):
                base.fail(f"v60 controlled file unexpectedly exists in bootstrap base: {path}")
        elif path not in V25.ps(policy_base) or candidate.read_bytes(
            path, base.MAX_POLICY_FILE_BYTES
        ) != policy_base.read_bytes(path, base.MAX_POLICY_FILE_BYTES):
            base.fail(f"v60 steady-state controlled file drifted: {path}")
    ...
    if rest:
        projected_candidate, projected_base = _v59_views(candidate, policy_base)
...
def files(view: Any) -> None:
    ...
    for path in sorted(CONTROLLED_FILES):
        if path not in V25.ps(view):
            base.fail(f"v60 controlled file missing: {path}")
        if V25.mode(view, path) != "100644":
            base.fail(f"v60 controlled file mode invalid: {path}")
        if view.read_bytes(path, base.MAX_POLICY_FILE_BYTES) != approved[path]:
            base.fail(f"v60 controlled file content drifted: {path}")
    if not _reopen_available(view):
...
def verify_component_base(
    ...
    *,
    ...
    if _PREDECESSOR_COMPONENT_BASE is None:
        base.fail("v60 predecessor component-base hook unavailable")
    path_set = set(paths)
    if path_set & REOPEN_FILES:
        remaining = path_set - REOPEN_FILES
        _call(
            "v60 projected predecessor component-base verifier",
            _PREDECESSOR_COMPONENT_BASE,
            _project_for_v59(view),
            remaining - CONTROLLED_FILES,
            allow_core_main_change=False,
        )
        return
    _call(
        "v60 predecessor component-base verifier",
        _PREDECESSOR_COMPONENT_BASE,
        _project_for_v59(view),
...
def freeze_s1_007_state(candidate: Any, policy_base: Any) -> None:
    if _PREDECESSOR_FREEZE_S1 is None:
        base.fail("v60 predecessor S1 freeze hook unavailable")
    paths = V25.changed(V25.v24.v23, candidate, policy_base)
    if paths == REOPEN_FILES:
        for relative in sorted(V25.FROZEN_STATE_PATHS):
            if candidate.read_bytes(relative, V25.MAX_S1_STATE_BYTES) != policy_base.read_bytes(
    ...
    projected_candidate, projected_base = _v59_views(candidate, policy_base)
    _call(
        "v60 predecessor S1 state freeze",
...
def printer(stage: str, mode_: str) -> None:
    ...
    print("wepld_policy_successor_v60=S2_S006_S007_GIT_TOPOLOGY_REOPEN_ONLY")
    print(f"v60_authority={AUTH}")
    print(f"s2_implementation_authority_v60={S2_IMPLEMENTATION_AUTHORITY}")
    print(
        "s2_s006_s007_git_topology_reopen_authority_v60="
        f"{S2_S006_S007_GIT_TOPOLOGY_REOPEN_AUTHORITY}"
    )
    print(f"reopen_available_v60={_reopen_available(raw_root)}")
    print(f"doctor_cli_authority_v60={DOCTOR_CLI_AUTHORITY}")
    print(f"general_shell_authority_v60={GENERAL_SHELL_AUTHORITY}")
    print(f"arbitrary_process_authority_v60={ARBITRARY_PROCESS_AUTHORITY}")
    print(f"package_install_authority_v60={PACKAGE_INSTALL_AUTHORITY}")
    print(f"project_native_command_execution_v60={PROJECT_NATIVE_COMMAND_EXECUTION}")
    print(f"git_mutation_authority_v60={GIT_MUTATION_AUTHORITY}")
    print(f"safe_directory_mutation_authority_v60={SAFE_DIRECTORY_MUTATION_AUTHORITY}")
    print(f"remediation_execution_authority_v60={REMEDIATION_EXECUTION_AUTHORITY}")
    print(f"git_process_admission_v60={GIT_PROCESS_ADMISSION}")
    print(f"git_execution_authority_v60={GIT_EXECUTION_AUTHORITY}")
    print(f"external_process_authority_v60={EXTERNAL_PROCESS_AUTHORITY}")
    print(f"network_authority_v60={NETWORK_AUTHORITY}")
    print(f"model_provider_execution_v60={MODEL_PROVIDER_EXECUTION}")
    print(f"source_admission_v60={SOURCE_ADMISSION}")
    print(f"dependency_admission_v60={DEPENDENCY_ADMISSION}")
    print(f"s3_plus_authority_v60={S3_PLUS_AUTHORITY}")
    print(f"test_child_process_authority_v60={TEST_CHILD_PROCESS_AUTHORITY}")
    print(f"next_authority_gate_v60={NEXT_AUTHORITY_GATE}")
...
def prepare_q() -> None:
    for module in _chain():
        ...
        if current not in (Q_WF, dict(WF)):
            base.fail(f"v60 predecessor workflow identity map drifted: actual={current}")
    for module in _chain():
        module.WF = dict(WF)
...
def overlay() -> None:
    ...
    if any(actual is not expected for actual, expected in pairs):
        base.fail("v60 installed overlay drifted")
    if any(dict(module.WF) != dict(WF) for module in _chain()):
        base.fail("v60 workflow identity projection drifted")
    for name in _INHERITED_AUTHORITY_NAMES:
        if getattr(q, name) != globals()[name]:
            base.fail(f"v60 inherited authority drifted: {name}")
...
def install() -> None:
    ...
    shell, routing, _, desktop, execution = V25.topo()
    pairs = (
        (_attr(routing, "IMPL_REQUIRE_EXACT_DELTA", "v59 routing hook"), q.delta),
        (base.compare_base_controlled, q.basectrl),
        (_attr(desktop, "verify_extension_controlled_paths", "v59 desktop hook"), q.dext),
        (_attr(execution, "verify_extension_controlled_paths", "v59 execution hook"), q.eext),
        (_attr(shell, "validate_allowed_paths", "v59 allowed hook"), q.allowed),
        (_attr(shell, "verify_policy_files", "v59 files hook"), q.files),
        (_attr(shell, "print_success", "v59 printer"), q.printer),
    )
    if any(actual is not expected for actual, expected in pairs):
        base.fail("v60 predecessor hook drifted")
    ...
    _bind(desktop, "EXTENSION_CONTROLLED_PATHS", desktop_extensions, "v60 desktop registration")
    _bind(execution, "EXTENSION_CONTROLLED_PATHS", execution_extensions, "v60 execution registration")
    _bind(routing, "IMPL_REQUIRE_EXACT_DELTA", delta, "v60 routing hook")
    base.compare_base_controlled = basectrl
    _bind(desktop, "verify_extension_controlled_paths", dext, "v60 desktop hook")
    _bind(execution, "verify_extension_controlled_paths", eext, "v60 execution hook")
    _bind(shell, "validate_allowed_paths", allowed, "v60 allowed hook")
    _bind(shell, "verify_policy_files", files, "v60 files hook")
    _bind(shell, "print_success", printer, "v60 printer hook")
    _bind(execution, "_verify_component_base", verify_component_base, "v60 component-base hook")
    _bind(execution, "freeze_s1_007_state", freeze_s1_007_state, "v60 S1 state freeze hook")
    _INST = True
    overlay()


def selftest() -> None:
    from wepld_s2_s006_s007_git_topology_reopen_v60_selftest import run
