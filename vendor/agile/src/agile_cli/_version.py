"""Version checking and self-update commands for agile_cli.
...
def _get_installed_version() -> str:
    """Return the installed wepld-agile distribution version or 'unknown'.
    ...
    try:
        return importlib.metadata.version("wepld-agile")
...
_INSTALLER_PATH_PREFIXES: dict[str, list[str]] = {
    "uv-tool": [
        "~/.local/share/uv/tools/wepld-agile/",
        "%LOCALAPPDATA%\\uv\\tools\\wepld-agile\\",
    ],
    "pipx": [
        "~/.local/pipx/venvs/wepld-agile/",
        "%LOCALAPPDATA%\\pipx\\venvs\\wepld-agile\\",
    ],
    "uvx-ephemeral": [
        "~/.cache/uv/archive-v0/",
...
def _resolved_argv0_path(argv0: str | None = None) -> Path:
    ...
    if "agile" not in lookup_names:
        lookup_names.append("agile")
...
def _looks_like_specify_entrypoint(path: Path) -> bool:
    """Return whether a path looks like the `agile` CLI entrypoint."""
    return path.name.lower() in {"agile", "specify.exe", "wepld-agile", "wepld-agile.exe"}
...
def _uv_tool_list_contains_agile_cli(stdout: str) -> bool:
    """Return whether `uv tool list` output includes an exact `wepld-agile` entry."""
    for raw_line in stdout.splitlines():
        line = raw_line.strip()
        if not line:
            continue
        first_token = line.split(None, 1)[0]
        if first_token == "wepld-agile":
            return True
    return False
...
def _editable_direct_url_path() -> Path | None:
    ...
    try:
        dist = _md.distribution("wepld-agile")
    except tuple(metadata_errors):
        return None
    ...
    if _tier3_registry_lookup_allowed(argv0_path):
        ...
        if uv_bin is not None:
            ...
            try:
                ...
                if result.returncode == 0 and _uv_tool_list_contains_agile_cli(
                    result.stdout or ""
                ):
                    uv_tool_match = True
        ...
        if pipx_bin is not None:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    if isinstance(venvs, dict) and "wepld-agile" in venvs:
                        pipx_match = True
            except (subprocess.TimeoutExpired, OSError, ValueError):
                pass
...
_GITHUB_SOURCE_URL = "git+https://github.com/TheHalfMoon/wepld.git"
...
_VERIFY_VERSION_LINE_RE = re.compile(
    r"^\s*(?:specify|wepld-agile)\b(?P<rest>.*)$",
    flags=re.IGNORECASE,
)
...
def _emit_guidance(method: _InstallMethod, target_tag: str | None) -> None:
    ...
    if method == _InstallMethod.UNSUPPORTED:
        ...
        console.print(
            f"  uv tool install wepld-agile --force --from "
...
def _rollback_hint(plan: _UpgradePlan) -> str:
    ...
    if plan.pre_upgrade_snapshot == "unknown":
        return (
            ...
            "reinstall manually from: https://github.com/TheHalfMoon/wepld/releases"
        )
    ...
    if plan.method == _InstallMethod.PIPX:
        return (
            ...
            f"git+https://github.com/TheHalfMoon/wepld.git@{rollback_tag}"
        )
    return (
        f"To pin back to the previous version: uv tool install wepld-agile --force "
        f"--from git+https://github.com/TheHalfMoon/wepld.git@{rollback_tag}"
    )
...
self_app = typer.Typer(
    name="self",
    help=(
        "Manage the agile CLI itself: check for newer releases, "
...
def self_upgrade(
    ...
    """Upgrade wepld-agile to the latest release (or a pinned --tag).
    ...
    argv_str = _render_argv(plan.installer_argv) if plan.installer_argv else ""
    console.print(
        f"{verb} wepld-agile {plan.current_version} → {plan.target_tag} "
    ...
    console.print(
        f"Upgraded wepld-agile: {pre_upgrade_display} → {verified_display}",
        soft_wrap=True,
    )
