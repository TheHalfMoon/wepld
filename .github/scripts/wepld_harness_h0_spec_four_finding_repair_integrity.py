#!/usr/bin/env python3
"""Authorize one exact four-finding repair of the Harness H0 Spec Kit package.

This wrapper is intentionally layered over the canonical H0 Spec Kit recovery policy.
...
1. one bounded four-finding-repair policy/workflow bootstrap over the exact canonical
   recovery policy; and
2. after canonical activation of this wrapper, one exact ten-file
   `specs/002-harness-h0-screening/` package that closes the four material planning
   findings proven across PRs #44 and #51:
   - explicit implementation/runner prerequisite closure before H0-SCREEN;
   - explicit binding to `docs/canonical/EXTERNAL_REVIEW_EGRESS_POLICY.md`;
   - all-started-trial manual-recovery denominator consistency; and
   - one canonical H0 Spec Kit state token.
...
POLICY_SCRIPT = ".github/scripts/wepld_harness_h0_spec_four_finding_repair_integrity.py"
PRIOR_POLICY_PATH = ".github/scripts/wepld_harness_h0_spec_recovery_integrity.py"
EXPECTED_PRIOR_POLICY_GIT_BLOB_SHA1 = "390a1fbd809eab2dc82f9f0add7453d1ab9e0a00"
...
FOUR_FINDING_SPEC_KIT_PATHS = frozenset(FOUR_FINDING_SPEC_KIT_BLOBS)
...
HARNESS_H0_SPEC_FOUR_FINDING_REPAIR_AUTHORIZED = "EXACT_FOUR_REVIEW_FINDING_REPAIR_PACKAGE"
...
def _bind_prior_policy_before_import() -> None:
    ...
    if actual != EXPECTED_PRIOR_POLICY_GIT_BLOB_SHA1:
        base.fail(
            "frozen H0 Spec Kit recovery policy runner drifted: "
            f"expected={EXPECTED_PRIOR_POLICY_GIT_BLOB_SHA1} actual={actual}"
        )


_bind_prior_policy_before_import()
import wepld_harness_h0_spec_recovery_integrity as prior  # noqa: E402

shell = prior.shell
PRIOR_REQUIRE_EXACT_DELTA = prior._require_exact_delta_recovery


def _activate_four_finding_contract() -> None:
    prior.RECOVERED_SPEC_KIT_BLOBS = dict(FOUR_FINDING_SPEC_KIT_BLOBS)
    prior.RECOVERED_SPEC_KIT_PATHS = frozenset(FOUR_FINDING_SPEC_KIT_PATHS)
    prior.EXPECTED_WORKFLOW_SHA256 = dict(EXPECTED_WORKFLOW_SHA256)
    prior._activate_recovered_contract()
...
def _verify_policy_files(view: base.RepositoryView) -> None:
    ...
    if actual != EXPECTED_PRIOR_POLICY_GIT_BLOB_SHA1:
        base.fail(
            "frozen H0 Spec Kit recovery policy runner drifted in repository view: "
            f"expected={EXPECTED_PRIOR_POLICY_GIT_BLOB_SHA1} actual={actual}"
        )
    prior._verify_policy_files(view)


def _is_four_finding_bootstrap_base(view: base.RepositoryView) -> bool:
    paths = _paths(view)
...
def _require_prior_recovery_base(view: base.RepositoryView) -> None:
    paths = _paths(view)
    if PRIOR_POLICY_PATH not in paths:
        base.fail("H0 Spec Kit four-finding repair requires the canonical recovery policy")
    data = view.read_bytes(PRIOR_POLICY_PATH, base.MAX_POLICY_FILE_BYTES)
    actual = _git_blob_sha1(data)
    if actual != EXPECTED_PRIOR_POLICY_GIT_BLOB_SHA1:
        base.fail(
            "H0 Spec Kit four-finding repair base recovery policy drifted: "
            f"expected={EXPECTED_PRIOR_POLICY_GIT_BLOB_SHA1} actual={actual}"
        )
    prior._require_prior_repair_base(view)
    if paths & set(FOUR_FINDING_SPEC_KIT_PATHS):
        base.fail(
            "H0 Spec Kit four-finding repair bootstrap requires package paths to remain absent"
        )


def _require_exact_delta_four_finding_repair(
    candidate: base.RepositoryView,
    policy_base: base.RepositoryView,
) -> None:
    changed = _changed_paths(candidate, policy_base)
    bootstrap = _is_four_finding_bootstrap_base(policy_base)

    if bootstrap:
        if changed == set(BOOTSTRAP_DELTA_PATHS):
            _require_prior_recovery_base(policy_base)
            return
        if changed & set(FOUR_FINDING_SPEC_KIT_PATHS):
            base.fail(
                "four-finding H0 Spec Kit package cannot transition before "
                "four-finding repair policy activation"
            )
...
def _compare_base_controlled_four_finding_repair(
    candidate: base.RepositoryView,
    policy_base: base.RepositoryView,
) -> None:
    bootstrap = _is_four_finding_bootstrap_base(policy_base)
    for relative in sorted(base.BASE_CONTROLLED_PATHS):
        ...
        if relative in BOOTSTRAP_WORKFLOWS:
            ...
            if candidate_hash != expected_candidate:
                base.fail(
                    "H0 Spec Kit four-finding repair workflow candidate drifted: "
            ...
            if base_hash != expected_base:
                ...
                base.fail(
                    f"H0 Spec Kit four-finding repair {phase} trusted-base workflow drifted: "
                    f"{relative}: expected={expected_base} actual={base_hash}"
                )
            if not bootstrap and candidate_bytes != base_bytes:
                base.fail(
                    f"H0 Spec Kit four-finding repair steady-state workflow changed: {relative}"
                )
            continue
...
def _verify_extension_paths_four_finding_repair(
    candidate: base.RepositoryView,
    policy_base: base.RepositoryView,
    ...
    if POLICY_SCRIPT in controlled_paths:
        if POLICY_SCRIPT not in candidate_paths:
            base.fail("H0 Spec Kit four-finding repair policy wrapper is missing from candidate")
        if bootstrap:
            if POLICY_SCRIPT in base_paths:
                base.fail(
                    "H0 Spec Kit four-finding repair wrapper unexpectedly exists in bootstrap base"
                )
        else:
            if POLICY_SCRIPT not in base_paths:
                base.fail(
                    "H0 Spec Kit four-finding repair steady-state base is missing wrapper"
                )
            if candidate.read_bytes(
                ...
                base.fail("H0 Spec Kit four-finding repair steady-state policy wrapper changed")
    ...
    for relative in sorted(BOOTSTRAP_WORKFLOWS & controlled_paths):
        ...
        if candidate_hash != expected_candidate:
            base.fail(
                "H0 Spec Kit four-finding repair controlled workflow candidate drifted: "
                f"{relative}: expected={expected_candidate} actual={candidate_hash}"
            )
        expected_base = (
            PRIOR_EXPECTED_WORKFLOW_SHA256[relative]
        ...
        if base_hash != expected_base:
            phase = "bootstrap" if bootstrap else "steady-state"
            base.fail(
                "H0 Spec Kit four-finding repair controlled workflow "
    ...
    if delegated:
        prior._verify_extension_paths_recovery(candidate, policy_base, delegated)
...
def _verify_execution_extension_paths(
    ...
    _verify_extension_paths_four_finding_repair(
        candidate,
        policy_base,
...
def _print_success(stage: str, mode: str) -> None:
    if _PRIOR_PRINT_SUCCESS is None:
        base.fail("prior H0 Spec Kit recovery success printer is not installed")
    _PRIOR_PRINT_SUCCESS(stage, mode)
    print(
        "harness_h0_spec_four_finding_repair_authorized="
        f"{HARNESS_H0_SPEC_FOUR_FINDING_REPAIR_AUTHORIZED}"
    )
    print(
...
def _install_four_finding_repair_policy() -> None:
    global _INSTALLED, _PRIOR_PRINT_SUCCESS
    if _INSTALLED:
        return

    _activate_four_finding_contract()
    prior._install_recovery_policy()
    _PRIOR_PRINT_SUCCESS = shell.print_success

    base.compare_base_controlled = _compare_base_controlled_four_finding_repair
    prior.prior.prior.prior.v24.v19._require_exact_delta = (
        _require_exact_delta_four_finding_repair
    )
...
def _selftest_workflow_binding() -> None:
    ...
    for relative in BOOTSTRAP_WORKFLOWS:
        ...
        if actual != expected:
            base.fail(
                "H0 Spec Kit four-finding repair workflow drifted: "
...
def _selftest_four_finding_bootstrap_delta() -> None:
    base_files, _ = _fixture_canonical_base()
    base_files[FOUNDATION_WORKFLOW] = b"recovery-foundation"
    base_files[ADMISSION_WORKFLOW] = b"recovery-admission"

    candidate_files = dict(base_files)
    candidate_files[POLICY_SCRIPT] = b"four-finding-repair-policy"
    candidate_files[FOUNDATION_WORKFLOW] = b"four-finding-foundation"
    candidate_files[ADMISSION_WORKFLOW] = b"four-finding-admission"
    trees = {
        POLICY_SCRIPT: "a" * 40,
    ...
    first_path = sorted(FOUR_FINDING_SPEC_KIT_PATHS)[0]
    premature[first_path] = b"premature-package"
    base.expect_failure_matching(
        "H0 Spec Kit four-finding premature package transition",
        "cannot transition before four-finding repair policy activation",
        _require_exact_delta_four_finding_repair,
...
def _selftest_steady_state_wrapper() -> None:
    ...
    if _is_four_finding_bootstrap_base(base_view):
        base.fail("H0 Spec Kit four-finding steady-state self-test misclassified base")
    mutated = base.MemoryView({POLICY_SCRIPT: policy_bytes + b"\n# drift\n"})
    base.expect_failure_matching(
        "H0 Spec Kit four-finding wrapper refreeze",
...
def selftest() -> None:
    _activate_four_finding_contract()
    prior.selftest()
    _install_four_finding_repair_policy()
    _selftest_workflow_binding()
    _selftest_four_finding_bootstrap_delta()
    ...
    print("wepld Harness H0 Spec Kit four-finding repair policy self-tests: PASS")
