...
const selectedProviders = (legacySmokeArg(args.providers) || defaultProviders.join(','))
  ...
  .filter(Boolean);
const smokeDir = join(targetRepo, '.pictorial', 'provider-smoke');
const summaryPath = join(smokeDir, 'summary.json');
const directSmokeFile = 'src/__pictorial_provider_smoke_direct.html';
const providerSmoke = {
  claude: {
    fixture: 'src/__pictorial_provider_smoke_claude.html',
    confirmedFixture: 'src/__pictorial_provider_smoke_confirmed_claude.html',
    agentChoiceFixture: 'src/__pictorial_provider_smoke_font_choice_claude.html',
    admin: '.claude/skills/pictorial/scripts/hook-admin.mjs',
    hook: '.claude/skills/pictorial/scripts/hook.mjs',
    event: (file) => postToolUseEvent('confirmed-claude', file, 'Edit'),
  },
  codex: {
    fixture: 'src/__pictorial_provider_smoke_codex.html',
    confirmedFixture: 'src/__pictorial_provider_smoke_confirmed_codex.html',
    agentChoiceFixture: 'src/__pictorial_provider_smoke_font_choice_codex.html',
    admin: '.agents/skills/pictorial/scripts/hook-admin.mjs',
    hook: '.agents/skills/pictorial/scripts/hook.mjs',
    event: (file) => postToolUseEvent('confirmed-codex', file, 'apply_patch'),
  },
  cursor: {
    fixture: 'src/__pictorial_provider_smoke_cursor.html',
    confirmedFixture: 'src/__pictorial_provider_smoke_confirmed_cursor.html',
    agentChoiceFixture: 'src/__pictorial_provider_smoke_font_choice_cursor.html',
    admin: '.cursor/skills/pictorial/scripts/hook-admin.mjs',
    hook: '.cursor/skills/pictorial/scripts/hook-before-edit.mjs',
    event: (file) => ({
...
const results = [];
const hookConfigFiles = ['.pictorial/config.json', '.pictorial/config.local.json'];
...
async function reinstallFresh() {
  cleanInstalledPictorial();
  const packDir = makeTempDir('pictorial-provider-smoke-pack-');
  ...
  const env = { PICTORIAL_BUNDLE_PATH: bundlePath };
  run('npx', ['--yes', '--package', tarball, 'pictorial', 'skills', 'install', '-y', '--force', '--providers=claude,cursor,codex'], {
    cwd: targetRepo,
    env,
    logName: 'skills-install.log',
    timeoutMs: 5 * 60 * 1000,
  });
  run('npx', ['--yes', '--package', tarball, 'pictorial', 'skills', 'update', '-y'], {
    cwd: targetRepo,
    env,
    logName: 'skills-update.log',
    timeoutMs: 5 * 60 * 1000,
  });
  record('fresh install/update', true, 'installed through local npx + PICTORIAL_BUNDLE_PATH');
}
...
function cleanInstalledPictorial() {
  clearRuntimeState();
  cleanSmokeFiles();

  for (const rel of [
    '.claude/skills/pictorial',
    '.cursor/skills/pictorial',
    '.agents/skills/pictorial',
    '.claude/hooks/hooks.json',
    '.agents/hooks',
    '.agents/plugins/marketplace.json',
    '.cursor/pre-log.mjs',
    '.cursor/rules/pictorial-design-hook.mdc',
    'plugin-codex',
  ]) {
  ...
  run('claude', ['plugin', 'uninstall', 'pictorial@pictorial', '--scope', 'user'], {
    cwd: targetRepo,
    logName: 'claude-plugin-uninstall.log',
    allowFailure: true,
    timeoutMs: 60 * 1000,
  });
  run('claude', ['plugin', 'marketplace', 'remove', 'pictorial', '--scope', 'user'], {
    cwd: targetRepo,
    logName: 'claude-marketplace-remove.log',
    allowFailure: true,
    timeoutMs: 60 * 1000,
  });
  run('codex', ['plugin', 'remove', 'pictorial@pictorial'], {
    cwd: targetRepo,
    logName: 'codex-plugin-remove.log',
    allowFailure: true,
    timeoutMs: 60 * 1000,
  });
  run('codex', ['plugin', 'marketplace', 'remove', 'pictorial'], {
    cwd: targetRepo,
  ...
  for (const abs of [
    join(homedir(), '.claude/plugins/cache/pictorial'),
    join(homedir(), '.claude/plugins/data/pictorial-pictorial'),
    join(homedir(), '.codex/plugins/cache/pictorial'),
    join(homedir(), '.codex/plugins/data/pictorial-pictorial'),
  ]) {
...
function ensureTargetGitExclude() {
  ...
  const block = [
    '# pictorial-provider-smoke-start',
    '.pictorial/provider-smoke/',
    'src/__pictorial_provider_smoke_*.html',
    '# pictorial-provider-smoke-end',
  ].join('\n');
  const current = readMaybe(excludePath);
  const next = current.includes('# pictorial-provider-smoke-start')
    ? current.replace(/# pictorial-provider-smoke-start[\s\S]*?# pictorial-provider-smoke-end/g, block)
    : `${current.replace(/\s*$/, '')}\n${block}\n`;
...
function stripManifest(rel) {
  ...
  for (const [event, entries] of Object.entries(hooks)) {
    const preserved = Array.isArray(entries)
      ? entries.map(stripPictorialHookEntry).filter(Boolean)
      : entries;
...
function stripPictorialHookEntry(entry) {
  ...
  if (containsPictorialHook(entry)) return null;
  if (Array.isArray(entry.hooks)) {
    const hooks = entry.hooks.map(stripPictorialHookEntry).filter(Boolean);
    if (hooks.length === 0 && entry.hooks.some(containsPictorialHook)) return null;
    return { ...entry, hooks };
  }
  return entry;
}

function containsPictorialHook(value) {
  if (typeof value === 'string') return value.includes('skills/pictorial/scripts/hook') || value.includes('.cursor/pre-log.mjs');
  if (Array.isArray(value)) return value.some(containsPictorialHook);
  if (value && typeof value === 'object') return Object.values(value).some(containsPictorialHook);
  return false;
}
...
function verifyInstallShape() {
  ...
  assertCount(claude, '.claude/skills/pictorial/scripts/hook.mjs', 1, 'Claude hook.mjs');
  assertCount(codex, '.agents/skills/pictorial/scripts/hook.mjs', 1, 'Codex hook.mjs');
  assertCount(cursor, '.cursor/skills/pictorial/scripts/hook-before-edit.mjs', 1, 'Cursor preToolUse');
  assertCount(cursor, '.cursor/skills/pictorial/scripts/hook-after-edit.mjs', 0, 'Cursor afterFileEdit');
  assertCount(cursor, '.cursor/skills/pictorial/scripts/hook-stop.mjs', 0, 'Cursor stop');
  ...
  for (const rel of [
    '.claude/skills/pictorial/scripts/hook.mjs',
    '.claude/skills/pictorial/scripts/hook-lib.mjs',
    '.claude/skills/pictorial/scripts/detector/cli/main.mjs',
    '.agents/skills/pictorial/scripts/hook.mjs',
    '.agents/skills/pictorial/scripts/hook-lib.mjs',
    '.agents/skills/pictorial/scripts/detector/cli/main.mjs',
    '.cursor/skills/pictorial/scripts/hook-before-edit.mjs',
    '.cursor/skills/pictorial/scripts/hook-lib.mjs',
    '.cursor/skills/pictorial/scripts/detector/cli/main.mjs',
  ]) {
    assertPath(join(targetRepo, rel), rel);
  }
  for (const rel of [
    '.cursor/skills/pictorial/scripts/hook-after-edit.mjs',
    '.cursor/skills/pictorial/scripts/hook-stop.mjs',
  ]) {
...
function assertNoPluginInstall() {
  ...
  for (const [name, text] of [
    ...
    if (/pictorial@pictorial|Marketplace `pictorial`|pictorial-design-hook-impl/.test(text)) {
      throw new Error(`${name} still contains Pictorial plugin install`);
    }
  }
}
...
function runDirectContractChecks() {
  ...
  const env = { PICTORIAL_HOOK_LOG: join(smokeDir, 'direct.ndjson') };
  const claude = run('node', ['.claude/skills/pictorial/scripts/hook.mjs'], {
    cwd: targetRepo,
    env,
  ...
  clearRuntimeState();
  const codex = run('node', ['.agents/skills/pictorial/scripts/hook.mjs'], {
    cwd: targetRepo,
    env,
  ...
  clearRuntimeState();
  const pre = run('node', ['.cursor/skills/pictorial/scripts/hook-before-edit.mjs'], {
    cwd: targetRepo,
    env,
...
function assertNoSpecificFontIgnoreConfig(provider) {
  const file = join(targetRepo, '.pictorial', 'config.json');
  if (!existsSync(file)) return;
...
function readSharedHookConfig() {
  const raw = readJson(join(targetRepo, '.pictorial', 'config.json'));
  if (!raw || typeof raw !== 'object' || Array.isArray(raw) || !raw.hook || typeof raw.hook !== 'object') {
    throw new Error('Missing .pictorial/config.json hook config');
  }
  return raw.hook;
}
...
function runInstalledProviderHook(provider, file, logName) {
  ...
  const env = { PICTORIAL_HOOK_LOG: join(smokeDir, logName) };
...
function runClaudeProviderSmoke() {
  clearRuntimeState();
  const env = { PICTORIAL_HOOK_LOG: join(smokeDir, 'claude.ndjson') };
...
function runCodexProviderSmoke() {
  clearRuntimeState();
  const env = { PICTORIAL_HOOK_LOG: join(smokeDir, 'codex.ndjson') };
  ...
  const cacheEvidence = `${readMaybe(join(targetRepo, '.pictorial', 'hook.cache.json'))}\n${readMaybe(join(targetRepo, '.pictorial', 'hook.pending.json'))}`;
...
function runCursorProviderSmoke() {
  ensureCursorAgent();
  clearRuntimeState();
  const env = { PICTORIAL_HOOK_LOG: join(smokeDir, 'cursor.ndjson') };
  ...
  const evidence = `${res.stdout}\n${res.stderr}\n${readMaybe(join(smokeDir, 'cursor.ndjson'))}\n${readMaybe(join(targetRepo, '.pictorial', 'hook.pending.json'))}\n${readMaybe(join(targetRepo, '.pictorial', 'hook.cache.json'))}`;
  ...
  if (existsSync(fixturePath)) {
    ...
    if (/border-left\s*:\s*[2-9]\d*px/i.test(fixtureContent)) {
      if (!intentionalIgnore || !/ignoreFiles|ignore-file/i.test(evidence)) {
        throw new Error('Cursor provider left the blocked side-tab fixture on disk without an explicit Pictorial ignore-file escape hatch');
      }
    }
  }
...
function badFixtureContent() {
  return [
    ...
    '      }',
    '    </style>',
    '    <div class="card">Temporary Pictorial provider smoke fixture</div>',
    '  </body>',
    '</html>',
    '',
  ].join('\n');
...
function confirmedFixtureContent() {
  return [
    ...
    '  <body>',
    '    <style>',
    '      /* pictorial: ignore overused-font */',
    '      body { font-family: "Roboto", ui-sans-serif, system-ui, sans-serif; }',
    '      .card { border-radius: 16px; padding: 16px; }',
    '    </style>',
    '    <div class="card">Temporary Pictorial confirmed exception fixture using Roboto.</div>',
    '  </body>',
    '</html>',
    '',
  ].join('\n');
...
function providerPrompt(rel) {
  return [
    `Create ${rel} as an intentionally bad Pictorial hook test fixture.`,
...
function fontExceptionPrompt(provider, rel) {
  return [
    `Read the installed Pictorial hooks reference for ${provider}, then persist a confirmed hook exception for Roboto specifically in ${rel}.`,
    'The user confirms Roboto is intentional for this fixture, but did not ask to ignore overused fonts generally.',
    'Use the /pictorial hooks / hook-admin flow; do not edit .pictorial/config.json by hand and do not edit the source fixture.',
