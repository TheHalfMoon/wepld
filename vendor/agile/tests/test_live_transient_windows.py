...
def _invoke_select_with_arrows(platform: str) -> bool:
    ...
    with (
        ...
        patch("agile_cli._console.Live", side_effect=fake_live),
        patch("agile_cli._console.readchar.readkey", return_value=readchar.key.ENTER),
        patch("sys.stdin.isatty", return_value=True),
    ):
        from agile_cli._console import select_with_arrows
...
class TestSourceContainsPlatformGuard:
    ...
    def test_init_has_win32_guard(self):
        ...
        init_src = Path(__file__).resolve().parent.parent / "src" / "agile_cli" / "commands" / "init.py"
    ...
    def test_console_has_win32_guard(self):
        ...
        console_src = Path(__file__).resolve().parent.parent / "src" / "agile_cli" / "_console.py"
