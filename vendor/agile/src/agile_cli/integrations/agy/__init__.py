...
Antigravity uses ``.agents/skills/agile-<name>/SKILL.md`` layout (enforced since v1.20.5).
"""
...
# Without this, agy emits ``/agile.git.commit`` (which does not
# resolve) instead of ``/agile-git-commit``.
_HOOK_COMMAND_NOTE = (
...
class AgyIntegration(SkillsIntegration):
    ...
    def build_exec_args(
        ...
        *,
        ...
        # Honor AGILE_INTEGRATION_AGY_EXTRA_ARGS (operator-supplied flags),
