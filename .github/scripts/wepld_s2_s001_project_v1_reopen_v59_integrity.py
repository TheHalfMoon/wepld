...
`crates/core/tests/project_v1.rs` test evidence gap.

v59 is an append-only policy successor over canonical v58 at main
1279073a1dc5a3f4fc33d42001897b91950dca72. Founder standing authorization for
...
test path so a real adversarial fixture for S2-S001 (path
traversal/canonicalization/TOCTOU test suite; specifically threat model T-002,
TOCTOU replacement, for the identity reservation/recovery flow) can be
recorded. `crates/core/tests/project_v1.rs` was reopened once before by v54
for S2-S002/S2-S004 and that grant is long consumed, so it is now frozen again
by the same v25 Core-observation-tranche freeze mechanism v54 itself reopened
against; a fresh single-use reopen is required. It is not a wholly new file.

v59 grants exactly one thing and nothing else:
...
qualified S2 observation/identity surface through its existing public
functions (`observe_project_locator`, `ProjectMatchFacts`, `build_reservation`,
`recover_reservation`), using only `std::fs`/`std::os::unix::fs::symlink` --
no process, no network, no unsafe code (the file keeps
...
against the inherited v58 chain.

Package-load / resting-view note: v59 follows the v45..v58 discipline. It owns
a fresh `LocalRepositoryView` of the exact checked-out head, imports frozen
v58 under an exact v59->v58 workflow-entrypoint reversal, and inherits every
v58 hook by reference. The one path this successor reopens is subject to
v25's Core-observation-tranche freeze inside the inherited cascade's
...
scanned exactly as written, exactly like v54's own project_v1.rs grant.
"""
...
P = ".github/scripts/wepld_s2_s001_project_v1_reopen_v59_integrity.py"
T = ".github/scripts/wepld_s2_s001_project_v1_reopen_v59_selftest.py"
T_BLOB = "381667a670a134f05f6b20ac425a7e6dfeafd231"

V58_P_BLOB = "d171e6b2a89649c90435dcabbe1c288ade15a792"
V58_T_BLOB = "8f5f2cc6a4ddac75c1f67c68685e300ec5dcd7b2"
...
# Do not inherit a predecessor module's resting/projection view. v59 bases
...
def _v58_workflow_bytes(data: bytes, path: str) -> bytes:
    count = data.count(_V59_ENTRYPOINT)
    if count != _WORKFLOW_ENTRYPOINT_COUNTS[path]:
        base.fail(
            "v59 workflow entrypoint count drifted before v58 projection: "
            f"{path} expected={_WORKFLOW_ENTRYPOINT_COUNTS[path]} actual={count}"
        )
    return data.replace(_V59_ENTRYPOINT, _V58_ENTRYPOINT)


def _import_v58_under_workflow_projection() -> Any:
    """Import frozen v58 while it observes exact v58 workflow bytes.

    v58 (hence v57..v45..v36) reads workflow bytes while its module is
    imported, and the v58->v59 entrypoint migration ships in this same
    candidate, so v58 must not observe its own successor's bytes. Only
    ...
    def _v58_import_read_bytes(local_view: Any, relative: str, limit: int) -> bytes:
        data = original_read_bytes(local_view, relative, limit)
        if relative in (FW, AW):
            data = _v58_workflow_bytes(data, relative)
            if len(data) > limit:
                base.fail(
                    f"v59 v58-import workflow projection exceeds read bound: {relative}"
                )
        return data

    base.LocalRepositoryView.read_bytes = _v58_import_read_bytes
    try:
        return importlib.import_module(
            "wepld_s2_e015_v57_selftest_cascade_projection_repair_v58_integrity"
        )
    finally:
        base.LocalRepositoryView.read_bytes = original_read_bytes


q = _import_v58_under_workflow_projection()
...
# main 1279073a1dc5a3f4fc33d42001897b91950dca72 (project_v1.rs as re-frozen
# once v54's own S2-S002/S2-S004 grant was consumed).
...
# consumed.
EXACT_FROZEN_BLOB = "ff030016ff399ee9193ad44aaee518270d69338e"

AUTH = "S2_S001_PROJECT_V1_REOPEN_ONLY"
...
# existing `project.rs`/`identity.rs` functions through real filesystem
# fixtures.
S2_S001_PROJECT_V1_RS_REOPEN_AUTHORITY = (
    "SINGLE_USE_TEST_ONLY_REOPEN_OF_PROJECT_V1_RS_FOR_S2_S001"
)

S2_S001_PROJECT_V1_REOPEN_CONTRACT = (
    "TEST_FILE_ONLY",
    "SINGLE_PATH_SCOPE",
    "SINGLE_USE_REOPEN",
    "NO_SOURCE_CHANGE",
    ...
    "REUSES_QUALIFIED_OBSERVATION_AND_IDENTITY_SURFACE_ONLY",
)

for _path, _expected in ((q.P, V58_P_BLOB), (q.T, V58_T_BLOB), (T, T_BLOB)):
    _actual = V25.blob(raw_root.read_bytes(_path, base.MAX_POLICY_FILE_BYTES))
    if _actual != _expected:
        base.fail(
            f"frozen v59 package input drifted: {_path}: "
...
def _workflow_replacements(view: Any) -> dict[str, bytes]:
    ...
    for path in (FW, AW):
        ...
        predecessor = _v58_workflow_bytes(data, path)
        actual = V25.sha(predecessor)
        if actual != Q_WF[path]:
            base.fail(
                "v59 workflow does not reverse to exact canonical v58 predecessor: "
...
def req_v58(view: Any) -> None:
    for path, expected in ((q.P, V58_P_BLOB), (q.T, V58_T_BLOB)):
        if path not in V25.ps(view):
            base.fail(f"v59 candidate/base is missing frozen v58 predecessor: {path}")
        actual = V25.blob(view.read_bytes(path, base.MAX_POLICY_FILE_BYTES))
        if actual != expected:
            base.fail(
                f"frozen v58 predecessor drifted: {path}: "
                f"expected={expected} actual={actual}"
            )


def _project_for_v58(view: Any) -> Any:
...
def _v58_views(candidate: Any, policy_base: Any) -> tuple[Any, Any]:
    """Project the candidate to v58's view always; project the policy base
    only when it is a real post-v59 base. A pre-v59 bootstrap base predates
    the v58->v59 workflow migration and carries no v59 policy files, so it
    must reach v58's frozen hooks unprojected."""
    projected_candidate = _project_for_v58(candidate)
    if bootbase(policy_base):
        return projected_candidate, policy_base
    return projected_candidate, _project_for_v58(policy_base)


def run_predecessor_selftests() -> None:
    """Run frozen v58's own self-tests once, under a v59->v58 workflow
    reversal.

    v58's corrected hooks are inherited by reference. Only ``read_bytes`` is
    wrapped here for the v59->v58 workflow reversal; the wrap is restored in
    ...
    def _v58_selftest_read_bytes(local_view: Any, relative: str, limit: int) -> bytes:
        if relative in workflow_reversal:
            data = workflow_reversal[relative]
            if len(data) > limit:
                base.fail(
                    f"v59 v58-selftest workflow projection exceeds read bound: {relative}"
                )
            return data
        return original_read_bytes(local_view, relative, limit)

    base.LocalRepositoryView.read_bytes = _v58_selftest_read_bytes
    try:
        _call("v58 self-tests under v59->v58 workflow reversal", q.selftest)
    finally:
        base.LocalRepositoryView.read_bytes = original_read_bytes
...
def _reopen_available(view: Any) -> bool:
    if REOPEN_TEST not in V25.ps(view):
        base.fail(f"v59 reopen base is missing the frozen reopened path: {REOPEN_TEST}")
...
def _require_reopen_base(view: Any) -> None:
    if not _reopen_available(view):
        base.fail(
            "v59 S2-S001 project_v1.rs evidence reopen already consumed; "
...
def _verify_text_reopen_file(view: Any, path: str) -> None:
    if path not in V25.ps(view):
        base.fail(f"v59 reopen path missing: {path}")
    if V25.mode(view, path) != "100644":
        base.fail(f"v59 reopen path mode invalid: {path}")
    data = view.read_bytes(path, base.MAX_POLICY_FILE_BYTES)
    if not data:
        base.fail(f"v59 reopen path must not be empty: {path}")
    if len(data) > MAX_REOPEN_FILE_BYTES:
        base.fail(f"v59 reopen path exceeds bounded size: {path}")
    try:
        data.decode("utf-8", errors="strict")
    except UnicodeError as exc:
        base.fail(f"v59 reopen path must be UTF-8: {path}: {exc}")
...
def _verify_reopen_candidate(candidate: Any, policy_base: Any) -> None:
    ...
    for path in (
        ...
        if candidate.read_bytes(path, base.MAX_POLICY_FILE_BYTES) != policy_base.read_bytes(
            ...
            base.fail(f"v59 S2-S001 project_v1.rs evidence reopen must not change frozen path: {path}")
    for relative in sorted(V25.FROZEN_STATE_PATHS):
        if candidate.read_bytes(relative, V25.MAX_S1_STATE_BYTES) != policy_base.read_bytes(
            ...
            base.fail(f"v59 candidate changed frozen S1 state: {relative}")
...
def delta(candidate: Any, policy_base: Any) -> None:
    ...
    if bootbase(policy_base):
        if paths != BOOT:
            if paths & BOOT:
                base.fail(
                    "v59 bootstrap delta must be exactly two v59 policy files plus two integrity workflows"
                )
            base.fail("v59 bootstrap base authorizes only exact S2-S001 project_v1.rs reopen policy activation")
        req_v58(candidate)
        req_v58(policy_base)
        return

    if paths & CONTROLLED_FILES:
        base.fail("canonical v59 policy files are frozen after activation")

    reopen_changed = frozenset(paths & REOPEN_FILES)
    if reopen_changed:
        if paths != REOPEN_FILES:
            base.fail("v59 S2-S001 project_v1.rs evidence reopen must not mix with non-reopen paths")
        _verify_reopen_candidate(candidate, policy_base)
        return

    q.delta(_project_for_v58(candidate), _project_for_v58(policy_base))


def basectrl(candidate: Any, policy_base: Any) -> None:
    if not bootbase(policy_base):
        q.basectrl(*_v58_views(candidate, policy_base))
        return
    for path in sorted(base.BASE_CONTROLLED_PATHS):
        ...
        if path in (FW, AW):
            if V25.sha(candidate_bytes) != WF[path] or V25.sha(base_bytes) != Q_WF[path]:
                base.fail(f"v59 bootstrap workflow drifted: {path}")
...
def ext(candidate: Any, policy_base: Any, safe: Any) -> None:
    ...
    for path in sorted(CONTROLLED_FILES & safe_paths):
        if path not in V25.ps(candidate):
            base.fail(f"v59 controlled file missing: {path}")
        if bootbase(policy_base):
            if path in V25.ps(policy_base):
                base.fail(f"v59 controlled file unexpectedly exists in bootstrap base: {path}")
        elif path not in V25.ps(policy_base) or candidate.read_bytes(
            path, base.MAX_POLICY_FILE_BYTES
        ) != policy_base.read_bytes(path, base.MAX_POLICY_FILE_BYTES):
            base.fail(f"v59 steady-state controlled file drifted: {path}")
    ...
    if rest:
        projected_candidate, projected_base = _v58_views(candidate, policy_base)
...
def files(view: Any) -> None:
    ...
    for path in sorted(CONTROLLED_FILES):
        if path not in V25.ps(view):
            base.fail(f"v59 controlled file missing: {path}")
        if V25.mode(view, path) != "100644":
            base.fail(f"v59 controlled file mode invalid: {path}")
        if view.read_bytes(path, base.MAX_POLICY_FILE_BYTES) != approved[path]:
            base.fail(f"v59 controlled file content drifted: {path}")
    if not _reopen_available(view):
...
def verify_component_base(
    ...
    *,
    ...
    if _PREDECESSOR_COMPONENT_BASE is None:
        base.fail("v59 predecessor component-base hook unavailable")
    path_set = set(paths)
    if path_set & REOPEN_FILES:
        remaining = path_set - REOPEN_FILES
        _call(
            "v59 projected predecessor component-base verifier",
            _PREDECESSOR_COMPONENT_BASE,
            _project_for_v58(view),
            remaining - CONTROLLED_FILES,
            allow_core_main_change=False,
        )
        return
    _call(
        "v59 predecessor component-base verifier",
        _PREDECESSOR_COMPONENT_BASE,
        _project_for_v58(view),
...
def freeze_s1_007_state(candidate: Any, policy_base: Any) -> None:
    if _PREDECESSOR_FREEZE_S1 is None:
        base.fail("v59 predecessor S1 freeze hook unavailable")
    paths = V25.changed(V25.v24.v23, candidate, policy_base)
    if paths == REOPEN_FILES:
        for relative in sorted(V25.FROZEN_STATE_PATHS):
            if candidate.read_bytes(relative, V25.MAX_S1_STATE_BYTES) != policy_base.read_bytes(
    ...
    projected_candidate, projected_base = _v58_views(candidate, policy_base)
    _call(
        "v59 predecessor S1 state freeze",
...
def printer(stage: str, mode_: str) -> None:
    ...
    print("wepld_policy_successor_v59=S2_S001_PROJECT_V1_REOPEN_ONLY")
    print(f"v59_authority={AUTH}")
    print(f"s2_implementation_authority_v59={S2_IMPLEMENTATION_AUTHORITY}")
    print(
        "s2_s001_project_v1_rs_reopen_authority_v59="
        f"{S2_S001_PROJECT_V1_RS_REOPEN_AUTHORITY}"
    )
    print(f"reopen_available_v59={_reopen_available(raw_root)}")
    print(f"doctor_cli_authority_v59={DOCTOR_CLI_AUTHORITY}")
    print(f"general_shell_authority_v59={GENERAL_SHELL_AUTHORITY}")
    print(f"arbitrary_process_authority_v59={ARBITRARY_PROCESS_AUTHORITY}")
    print(f"package_install_authority_v59={PACKAGE_INSTALL_AUTHORITY}")
    print(f"project_native_command_execution_v59={PROJECT_NATIVE_COMMAND_EXECUTION}")
    print(f"git_mutation_authority_v59={GIT_MUTATION_AUTHORITY}")
    print(f"safe_directory_mutation_authority_v59={SAFE_DIRECTORY_MUTATION_AUTHORITY}")
    print(f"remediation_execution_authority_v59={REMEDIATION_EXECUTION_AUTHORITY}")
    print(f"git_process_admission_v59={GIT_PROCESS_ADMISSION}")
    print(f"git_execution_authority_v59={GIT_EXECUTION_AUTHORITY}")
    print(f"external_process_authority_v59={EXTERNAL_PROCESS_AUTHORITY}")
    print(f"network_authority_v59={NETWORK_AUTHORITY}")
    print(f"model_provider_execution_v59={MODEL_PROVIDER_EXECUTION}")
    print(f"source_admission_v59={SOURCE_ADMISSION}")
    print(f"dependency_admission_v59={DEPENDENCY_ADMISSION}")
    print(f"s3_plus_authority_v59={S3_PLUS_AUTHORITY}")
    print(f"test_child_process_authority_v59={TEST_CHILD_PROCESS_AUTHORITY}")
    print(f"next_authority_gate_v59={NEXT_AUTHORITY_GATE}")
...
def prepare_q() -> None:
    for module in _chain():
        ...
        if current not in (Q_WF, dict(WF)):
            base.fail(f"v59 predecessor workflow identity map drifted: actual={current}")
    for module in _chain():
        module.WF = dict(WF)
...
def overlay() -> None:
    ...
    if any(actual is not expected for actual, expected in pairs):
        base.fail("v59 installed overlay drifted")
    if any(dict(module.WF) != dict(WF) for module in _chain()):
        base.fail("v59 workflow identity projection drifted")
    for name in _INHERITED_AUTHORITY_NAMES:
        if getattr(q, name) != globals()[name]:
            base.fail(f"v59 inherited authority drifted: {name}")
...
def install() -> None:
    ...
    shell, routing, _, desktop, execution = V25.topo()
    pairs = (
        (_attr(routing, "IMPL_REQUIRE_EXACT_DELTA", "v58 routing hook"), q.delta),
        (base.compare_base_controlled, q.basectrl),
        (_attr(desktop, "verify_extension_controlled_paths", "v58 desktop hook"), q.dext),
        (_attr(execution, "verify_extension_controlled_paths", "v58 execution hook"), q.eext),
        (_attr(shell, "validate_allowed_paths", "v58 allowed hook"), q.allowed),
        (_attr(shell, "verify_policy_files", "v58 files hook"), q.files),
        (_attr(shell, "print_success", "v58 printer"), q.printer),
    )
    if any(actual is not expected for actual, expected in pairs):
        base.fail("v59 predecessor hook drifted")
    ...
    _bind(desktop, "EXTENSION_CONTROLLED_PATHS", desktop_extensions, "v59 desktop registration")
    _bind(execution, "EXTENSION_CONTROLLED_PATHS", execution_extensions, "v59 execution registration")
    _bind(routing, "IMPL_REQUIRE_EXACT_DELTA", delta, "v59 routing hook")
    base.compare_base_controlled = basectrl
    _bind(desktop, "verify_extension_controlled_paths", dext, "v59 desktop hook")
    _bind(execution, "verify_extension_controlled_paths", eext, "v59 execution hook")
    _bind(shell, "validate_allowed_paths", allowed, "v59 allowed hook")
    _bind(shell, "verify_policy_files", files, "v59 files hook")
    _bind(shell, "print_success", printer, "v59 printer hook")
    _bind(execution, "_verify_component_base", verify_component_base, "v59 component-base hook")
    _bind(execution, "freeze_s1_007_state", freeze_s1_007_state, "v59 S1 state freeze hook")
    _INST = True
    overlay()


def selftest() -> None:
    from wepld_s2_s001_project_v1_reopen_v59_selftest import run
