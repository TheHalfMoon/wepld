...
describe('live target-aware monorepo context', () => {
  ...
  beforeEach(() => {
    tmp = realpathSync(mkdtempSync(join(tmpdir(), 'pictorial-live-target-')));
    setupMonorepo(tmp);
  });
  ...
  it('does not let root live config shadow the child project config path', () => {
    ...
    assert.equal(payload.path, join(tmp, 'apps', 'dashboard', '.pictorial', 'live', 'config.json'));
  ...
  it('boots live from the child project and inherits root context when child files are missing', async () => {
    ...
    try {
      ...
      assert.equal(payload.liveConfigPath, join(tmp, 'apps', 'dashboard', '.pictorial', 'live', 'config.json'));

      assert.equal(existsSync(join(tmp, 'apps', 'dashboard', '.pictorial', 'live', 'server.json')), true);
      assert.equal(existsSync(join(tmp, '.pictorial', 'live', 'server.json')), false);
  ...
  it('boots live from the child cwd without --target after the app is selected', async () => {
    ...
    try {
      ...
      assert.match(payload.product, /ROOT PRODUCT LIVE INHERIT/);
      assert.match(payload.design, /ROOT DESIGN LIVE INHERIT/);
      assert.equal(payload.liveConfigPath, join(childRoot, '.pictorial', 'live', 'config.json'));
      assert.equal(existsSync(join(childRoot, '.pictorial', 'live', 'server.json')), true);
      assert.equal(existsSync(join(tmp, '.pictorial', 'live', 'server.json')), false);
    } finally {
      stopLive(tmp);
    }
  ...
  it('asks for an app before starting live from a monorepo root', () => {
    ...
    assert.equal(existsSync(join(tmp, '.pictorial', 'live', 'server.json')), false);
    assert.equal(existsSync(join(tmp, 'apps', 'dashboard', '.pictorial', 'live', 'server.json')), false);
...
describe('live single-repo context setup guard', () => {
  ...
  beforeEach(() => {
    tmp = realpathSync(mkdtempSync(join(tmpdir(), 'pictorial-live-single-')));
  });
  ...
  it('starts live without app selection when PRODUCT.md and DESIGN.md exist', () => {
    ...
    try {
      ...
      assert.equal(existsSync(join(tmp, '.pictorial', 'live', 'server.json')), true);
    } finally {
      runNode(LIVE_SERVER_SCRIPT, ['stop', '--keep-inject'], tmp);
    }
  });
...
function setupSingleRepo(root, { product = true, design = true } = {}) {
  ...
  write(root, '.pictorial/live/config.json', JSON.stringify({
...
function writeChildLiveConfig(root) {
  write(root, 'apps/dashboard/.pictorial/live/config.json', JSON.stringify({
    files: ['public/index.html'],
    insertBefore: '</body>',
    commentSyntax: 'html',
  }, null, 2));
...
function assertNoLiveBootSideEffects(root) {
  assert.equal(existsSync(join(root, 'apps', 'dashboard', '.pictorial', 'live', 'server.json')), false);
  assert.equal(existsSync(join(root, '.pictorial', 'live', 'server.json')), false);
