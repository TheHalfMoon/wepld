...
class TestWorkflowRunWithoutProject:
    """Tests that specify workflow run works with YAML files without .agile/ dir."""

    def test_workflow_run_yaml_without_project(self, tmp_path):
        """Running a .yml file should work without a .agile/ directory."""
        ...
        assert (tmp_path / ".agile" / "workflows" / "runs").is_dir()

    def test_workflow_run_yaml_with_tilde_and_uppercase_suffix(self, tmp_path, monkeypatch):
        """Running ~/file.YML should work without a .agile/ directory."""
        from typer.testing import CliRunner
        from agile_cli import app
    ...
    def test_workflow_run_id_still_requires_project(self, tmp_path):
        """Running a workflow by ID should still require a .agile/ directory."""
        from typer.testing import CliRunner
        from agile_cli import app
    ...
    def test_workflow_run_yaml_rejects_symlinked_specify_dir(self, tmp_path):
        """Running local YAML should fail when .agile is a symlink."""
        from typer.testing import CliRunner
        from agile_cli import app
        ...
        target_dir.mkdir()
        try:
            (tmp_path / ".agile").symlink_to(target_dir, target_is_directory=True)
        ...
        assert result.exit_code != 0
        assert "Refusing to use symlinked .agile path" in result.output

    def test_workflow_run_yaml_rejects_symlinked_workflows_dir(self, tmp_path):
        """Running local YAML should fail when .agile/workflows is a symlink."""
        from typer.testing import CliRunner
        from agile_cli import app
        ...
        target_dir.mkdir()
        try:
            (tmp_path / ".agile" / "workflows").symlink_to(
        ...
        assert result.exit_code != 0
        assert "Refusing to use symlinked .agile/workflows path" in result.output

    def test_workflow_run_yaml_rejects_symlinked_runs_dir(self, tmp_path):
        """Running local YAML should fail when .agile/workflows/runs is a symlink."""
        from typer.testing import CliRunner
        from agile_cli import app
        ...
        (tmp_path / ".agile" / "workflows").mkdir(parents=True)
        target_dir = tmp_path / "real-runs-dir"
        target_dir.mkdir()
        try:
            (tmp_path / ".agile" / "workflows" / "runs").symlink_to(
                target_dir, target_is_directory=True
            )
        except (OSError, NotImplementedError):
            pytest.skip("Symlinks are not available in this environment")
        ...
        assert result.exit_code != 0
        assert "Refusing to use symlinked .agile/workflows/runs path" in result.output

    def test_workflow_run_yaml_rejects_non_directory_specify_path(self, tmp_path):
        """Running local YAML should fail when .agile is not a directory."""
        from typer.testing import CliRunner
        from agile_cli import app
        ...
        workflow_file.write_text(yaml.dump(workflow_content), encoding="utf-8")
        (tmp_path / ".agile").write_text("not a directory", encoding="utf-8")
        ...
        assert result.exit_code != 0
        assert ".agile path exists but is not a directory" in result.output
