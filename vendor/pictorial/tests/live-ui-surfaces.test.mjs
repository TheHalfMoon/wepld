 ...
 * tooling, and the private pictorial-site repo, whose Live UI lab fails its
...
describe('live UI surfaces module', () => {
  ...
  it('is not redefined inside live-browser.js', () => {
    ...
    assert.match(LIVE_BROWSER_SOURCE, /window\.__PICTORIAL_LIVE_UI_SURFACES__/);
  });
});
...
describe('live UI surfaces reach the browser bundle', () => {
  ...
  it('serializes the canonical inventory into the prelude by default', () => {
    ...
    const surfaces = JSON.parse(
      script.match(/window\.__PICTORIAL_LIVE_UI_SURFACES__ = (\[.*?\]);\n/s)[1],
    );
    assert.deepEqual(surfaces, JSON.parse(JSON.stringify(LIVE_UI_SURFACES)));

    const contract = JSON.parse(
      script.match(/window\.__PICTORIAL_LIVE_MOUNT_CONTRACT__ = (\[.*?\]);\n/s)[1],
    );
  ...
  it('injects the globals before the script part that reads them', () => {
    const script = assemble();
    assert.ok(
      script.indexOf('window.__PICTORIAL_LIVE_UI_SURFACES__ =')
        < script.indexOf('pictorial live script part: browser-ui'),
    );
  });
});
