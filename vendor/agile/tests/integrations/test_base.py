...
class TestBasePrimitives:
    ...
    def test_command_filename_default(self):
        i = StubIntegration()
        assert i.command_filename("plan") == "agile.plan.md"
    ...
    def test_copy_command_to_directory(self, tmp_path):
        ...
        result = IntegrationBase.copy_command_to_directory(src, dest_dir, "agile.plan.md")
        assert result == dest_dir / "agile.plan.md"
...
class TestBuildCommandInvocation:
    ...
    def test_base_core_command_dotted(self):
        i = StubIntegration()
        assert i.build_command_invocation("agile.plan") == "/agile.plan"

    def test_base_core_command_bare(self):
        i = StubIntegration()
        assert i.build_command_invocation("plan") == "/agile.plan"

    def test_base_core_command_with_args(self):
        i = StubIntegration()
        assert i.build_command_invocation("plan", "my feature") == "/agile.plan my feature"

    def test_base_extension_command(self):
        i = StubIntegration()
        assert i.build_command_invocation("agile.git.commit") == "/agile.git.commit"

    def test_base_extension_command_bare(self):
        i = StubIntegration()
        assert i.build_command_invocation("git.commit") == "/agile.git.commit"
    ...
    def test_skills_extension_command_with_args(self):
        from agile_cli.integrations import get_integration
        i = get_integration("codex")
        assert i.build_command_invocation("agile.git.commit", "fix typo") == "$agile-git-commit fix typo"
    ...
    def test_dollar_skill_post_processing_is_idempotent(self, integration_key):
        ...
        content = (
            "---\nname: test\n---\n\n"
            "Literal slash invocation: /agile-plan\n"
        ...
        assert "$agile-git-commit" in once
        assert "/agile-plan" in once
    ...
    def test_kimi_skill_post_processing_is_idempotent(self):
        ...
        re-runs without requiring the broad /agile- -> /skill:agile- body
    ...
    def test_forge_core_command_hyphenated(self):
        """Forge installs hyphenated slash-commands (/agile-<name>), so the
    ...
    def test_forge_extension_command_hyphenated(self):
        ...
        assert i.build_command_invocation("agile.git.commit") == "/agile-git-commit"
        assert (
            i.build_command_invocation("agile.git.commit", "fix typo")
            == "/agile-git-commit fix typo"
        )

    def test_cline_core_command_hyphenated(self):
        """Cline installs hyphenated slash-commands (/agile-<name>), so the
        dispatch invocation must be hyphenated too — not the dotted default it
        would inherit from MarkdownIntegration."""
        from agile_cli.integrations import get_integration
...
class TestResolveCommandRefs:
    """Tests for __AGILE_COMMAND_<NAME>__ placeholder resolution."""

    def test_dot_separator_core_command(self):
        text = "Run `__AGILE_COMMAND_PLAN__` to plan."
        result = IntegrationBase.resolve_command_refs(text, ".")
        assert result == "Run `/agile.plan` to plan."
    ...
    def test_skill_colon_prefix_core_command(self):
        ...
        assert result == "Run `/skill:agile-plan` to plan."
    ...
    def test_extension_command_dot(self):
        ...
        result = IntegrationBase.resolve_command_refs(text, ".")
        assert result == "Run /agile.git.commit to commit."
    ...
    def test_default_separator_is_dot(self):
        text = "__AGILE_COMMAND_PLAN__"
        assert IntegrationBase.resolve_command_refs(text) == "/agile.plan"
    ...
    def test_process_template_resolves_placeholders(self):
        content = "---\ndescription: test\n---\nRun __AGILE_COMMAND_PLAN__ now."
        result = IntegrationBase.process_template(
            content, "test-agent", "sh", invoke_separator="."
        )
        assert "/agile.plan" in result
        assert "__AGILE_COMMAND_" not in result
    ...
    def test_unclosed_placeholder_unchanged(self):
        text = "Run __AGILE_COMMAND_PLAN to plan."
        assert IntegrationBase.resolve_command_refs(text, ".") == text

    def test_empty_name_not_matched(self):
        text = "Run __AGILE_COMMAND___ to plan."
        assert IntegrationBase.resolve_command_refs(text, ".") == text
    ...
    def test_placeholder_adjacent_to_text(self):
        text = "foo__AGILE_COMMAND_PLAN__bar"
        result = IntegrationBase.resolve_command_refs(text, ".")
        assert result == "foo/agile.planbar"

    def test_placeholder_with_digits(self):
        text = "__AGILE_COMMAND_V2_PLAN__"
        result = IntegrationBase.resolve_command_refs(text, ".")
        assert result == "/agile.v2.plan"
...
class TestResolvePythonInterpreter:
    def test_returns_python_on_path(self, monkeypatch):
        ...
        monkeypatch.setattr("agile_cli.integrations.base.sys.platform", "linux")
        monkeypatch.setattr(
            "agile_cli.integrations.base.shutil.which", fake_which
        )
    ...
    def test_falls_back_to_sys_executable_when_nothing_found(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "agile_cli.integrations.base.shutil.which", lambda name: None
        )
        monkeypatch.setattr(
            "agile_cli.integrations.base.sys.executable", "/opt/py/bin/python"
        )
    ...
    def test_falls_back_to_python3_when_no_interpreter_at_all(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "agile_cli.integrations.base.sys.executable", ""
        )
        assert IntegrationBase.resolve_python_interpreter() == "python3"
    ...
    def test_prefers_project_venv_posix(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "agile_cli.integrations.base.shutil.which",
    ...
    def test_windows_skips_store_alias_stub(self, monkeypatch):
        ...
        monkeypatch.setattr("agile_cli.integrations.base.sys.platform", "win32")
        monkeypatch.setattr(
            "agile_cli.integrations.base.shutil.which",
        ...
        monkeypatch.setattr(
            "agile_cli.integrations.base.sys.executable", "C:\\Python\\python.exe"
        )
...
class TestProcessTemplatePyScriptType:
    ...
    def test_py_prefixes_interpreter(self, monkeypatch):
        ...
        # script path is rewritten to the .agile location.
        monkeypatch.setattr("agile_cli.integrations.base.sys.platform", "linux")
        monkeypatch.setattr(
            "agile_cli.integrations.base.shutil.which",
            lambda name: "/usr/bin/python3" if name == "python3" else None,
        ...
        assert "python3 .agile/scripts/python/check-prerequisites.py --json" in result
    ...
    def test_sh_does_not_prefix_interpreter(self):
        ...
        assert ".agile/scripts/bash/check-prerequisites.sh --json" in result
    ...
    def test_body_scripts_example_does_not_override_frontmatter(self):
        ...
        assert ".agile/scripts/bash/real.sh --json" in result
    ...
    def test_py_quotes_interpreter_with_spaces(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "agile_cli.integrations.base.os", SimpleNamespace(name="posix")
        )
        result = IntegrationBase.process_template(self.CONTENT, "agent", "py")
        assert (
            f"{shlex.quote(interpreter)} "
            ".agile/scripts/python/check-prerequisites.py --json"
        ) in result
    ...
    def test_py_uses_project_venv(self, monkeypatch, tmp_path):
        ...
        assert ".venv/bin/python .agile/scripts/python/check-prerequisites.py" in result
    ...
    def test_setup_py_falls_back_to_platform_shell(
        ...
        expected = (
            ...
            else ".agile/scripts/bash/check-prerequisites.sh"
        )
...
class TestInstallScriptsPython:
    ...
    def test_marks_py_and_sh_executable(self, monkeypatch, tmp_path):
        ...
        dest = project_root / ".agile" / "integrations" / "stub" / "scripts"
