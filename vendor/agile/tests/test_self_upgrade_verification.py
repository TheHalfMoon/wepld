...
class TestVerificationMismatch:
    ...
    def test_verify_accepts_pep440_equivalent_rc_version(
        ...
        with patch("agile_cli.authentication.http.urllib.request.urlopen") as mock_urlopen, patch(
            ...
            "agile_cli._version._get_installed_version", return_value="0.9.0"
        ):
        ...
        assert "Upgraded wepld-agile: 0.9.0 → 1.0.0rc1" in strip_ansi(result.output)

    def test_verify_accepts_agile_cli_binary_name_in_version_output(
        self,
        uv_tool_argv0,
        clean_environ,
    ):
        with patch("agile_cli.authentication.http.urllib.request.urlopen") as mock_urlopen, patch(
            ...
            mock_run.side_effect = [
                _completed_process(0),
                _completed_process(0, stdout="wepld-agile version 0.7.6\n"),
            ]
            result = runner.invoke(app, ["self", "upgrade"])

        assert result.exit_code == 0
        assert "Upgraded wepld-agile: 0.7.5 → 0.7.6" in strip_ansi(result.output)
    ...
    def test_verify_rejects_output_without_parseable_version(
        ...
        with patch("agile_cli.authentication.http.urllib.request.urlopen") as mock_urlopen, patch(
            ...
            mock_run.side_effect = [
                _completed_process(0),
                _completed_process(0, stdout="agile version unknown\n"),
            ]
            result = runner.invoke(app, ["self", "upgrade"])
    ...
    def test_verify_uses_current_entrypoint_when_not_on_path(
        ...
        plan = _UpgradePlan(
            ...
            target_tag="v0.7.6",
            installer_argv=["/usr/bin/uv", "tool", "install", "wepld-agile"],
            preview_summary="",
            pre_upgrade_snapshot="0.7.5",
        )

        with patch(
            "agile_cli._version.shutil.which", side_effect=lambda name: None
        ), patch(
            "agile_cli._version.subprocess.run"
        ) as mock_run, patch(
            "agile_cli._version.os.access", return_value=True
        ):
        ...
        assert mock_run.call_args.kwargs["timeout"] == agile_cli._version._VERIFY_TIMEOUT_SECS
    ...
    def test_verify_falls_back_to_path_when_current_entrypoint_is_not_executable(
        ...
        with patch(
            ...
            side_effect=lambda name: "/usr/local/bin/specify" if name == "agile" else None,
        ), patch("agile_cli._version.subprocess.run") as mock_run, patch(
            "agile_cli._version.os.access", return_value=False
        ):
            mock_run.return_value = _completed_process(0, stdout="specify 0.7.6\n")
            verified = _verify_upgrade(plan)
    ...
    def test_verify_ignores_python_entrypoint_and_falls_back_to_specify(
        ...
        with patch(
            "agile_cli._version.shutil.which", side_effect=lambda name: "/usr/local/bin/specify" if name == "agile" else None
        ), patch("agile_cli._version.subprocess.run") as mock_run, patch(
            "agile_cli._version.sys.argv", [str(fake_python)]
        ), patch(
            "agile_cli._version.os.access", return_value=True
        ):
            mock_run.return_value = _completed_process(0, stdout="specify 0.7.6\n")
    ...
    def test_verify_accepts_agile_cli_named_current_entrypoint(
        self,
        clean_environ,
        tmp_path,
    ):
        fake_agile_cli = tmp_path / "wepld-agile"
        fake_agile_cli.write_text("#!/bin/sh\n")
        fake_agile_cli.chmod(0o755)
        ...
        with patch("agile_cli._version.shutil.which", return_value=None), patch(
            "agile_cli._version.subprocess.run"
        ) as mock_run, patch("agile_cli._version.sys.argv", [str(fake_agile_cli)]), patch(
            "agile_cli._version.os.access", return_value=True
        ):
            mock_run.return_value = _completed_process(0, stdout="specify 0.7.6\n")
            verified = _verify_upgrade(plan)
        ...
        assert verified == "0.7.6"
        assert mock_run.call_args.args[0][0] == str(fake_agile_cli)
...
class TestResolutionFailures:
    ...
    def test_offline_exits_1_with_phase1_string(self, uv_tool_argv0, clean_environ):
        with patch(
            "agile_cli.authentication.http.urllib.request.urlopen",
    ...
    def test_rate_limited_exits_1(self, uv_tool_argv0, clean_environ):
        ...
        with patch("agile_cli.authentication.http.urllib.request.urlopen", side_effect=err):
            result = runner.invoke(app, ["self", "upgrade"])
        assert result.exit_code == 1
        assert (
            "Upgrade aborted: rate limited (configure ~/.agile/auth.json with a GitHub token)"
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                429,
                "Upgrade aborted: rate limited (configure ~/.agile/auth.json "
                "with a GitHub token)",
            ),
...
class TestTagValidation:
    ...
    def test_valid_beta_dot_tag_uses_pep440_equivalent_for_noop(
        ...
        with patch("agile_cli._version.shutil.which", return_value="uv"), patch(
            "agile_cli._version._get_installed_version", return_value="1.0.0b1"
        ):
            result = runner.invoke(
                app,
...
class TestUnknownCurrent:
    ...
    def test_unknown_current_renders_literal_in_notice(
        ...
        with patch("agile_cli.authentication.http.urllib.request.urlopen") as mock_urlopen, patch(
            ...
            "agile_cli._version._get_installed_version", return_value="unknown"
        ):
            mock_urlopen.return_value = mock_urlopen_response({"tag_name": "v0.7.6"})
            mock_run.side_effect = [
                _completed_process(0),
        ...
        assert result.exit_code == 0
        out = strip_ansi(result.output)
        assert "Upgrading wepld-agile unknown → v0.7.6 via uv tool:" in out
        assert "Upgraded wepld-agile: unknown → 0.7.6" in out
    ...
    def test_unknown_current_rollback_hint_degrades(
        ...
        assert "https://github.com/TheHalfMoon/wepld/releases" in out
...
class TestTokenScrubbing:
    ...
    def test_env_passed_to_subprocess_has_no_github_tokens(
        ...
        with patch("agile_cli.authentication.http.urllib.request.urlopen") as mock_urlopen, patch(
            "agile_cli.authentication.http.urllib.request.build_opener"
    ...
    def test_env_scrubbing_removes_github_token_variants(self, monkeypatch):
        ...
        env = agile_cli._version._scrubbed_env()
