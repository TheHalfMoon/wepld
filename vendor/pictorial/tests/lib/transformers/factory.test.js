...
describe('createTransformer factory', () => {
  ...
  test('should render the explicit script provider marker without rewriting executable code', () => {
    ...
    const skills = [{
      ...
      scripts: [{
        ...
        content: [
          ...
          'const command = `${PICTORIAL_COMMAND_PREFIX}pictorial polish`;',
          'const hint = "Run /pictorial audit";',
          'const hook = ".github/hooks/pictorial.json";',
    ...
    const script = fs.readFileSync(
      path.join(TEST_DIR, 'codex/.test/skills/pictorial/scripts/example.mjs'),
      'utf-8',
    );
    expect(script).toContain('PICTORIAL_COMMAND_PREFIX = "$"');
    expect(script).toContain('PICTORIAL_PROVIDER_ID = "codex"');
    expect(script).toContain('`${PICTORIAL_COMMAND_PREFIX}pictorial polish`');
    expect(script).toContain('"Run /pictorial audit"');
    expect(script).toContain('".github/hooks/pictorial.json"');
    expect(script).toContain('"/src/lib/pictorial/__runtime.js"');
    expect(script).toContain('/pictorial\\b/gi');
  });
