...
def project_dir(tmp_path):
    """Create a mock agile project with ``.agile/workflows/`` directory."""
    workflows_dir = tmp_path / ".agile" / "workflows"
    workflows_dir.mkdir(parents=True, exist_ok=True)
    return tmp_path


def _write_workflow(project_root: Path, workflow_id: str, data: dict) -> Path:
    wf_dir = project_root / ".agile" / "workflows" / workflow_id
...
def _write_overlay(project_root: Path, workflow_id: str, overlay_id: str, data: dict) -> Path:
    ov_dir = project_root / ".agile" / "workflows" / "overlays" / workflow_id
...
class TestOverlayCli:
    ...
    def test_overlay_add(self, project_dir, monkeypatch):
        ...
        installed = project_dir / ".agile" / "workflows" / "overlays" / "wf" / "ov1.yml"
    ...
    def test_overlay_add_reuses_yaml_extension(self, project_dir, monkeypatch):
        ...
        existing_yaml = project_dir / ".agile" / "workflows" / "overlays" / "wf" / "ov1.yaml"
    ...
    def test_overlay_set_priority(self, project_dir, monkeypatch):
        ...
        data = yaml.safe_load(
            (
                project_dir / ".agile" / "workflows" / "overlays" / "wf" / "ov1.yml"
            ).read_text(encoding="utf-8")
        )
        assert data["priority"] == 20
        assert list(
            (project_dir / ".agile" / "workflows" / "overlays" / "wf").glob(
                ".ov1.yml.*.bak"
            )
        ) == []
    ...
    def test_workflow_resolve(self, project_dir, monkeypatch):
        ...
        from agile_cli.workflows.overlays._commands import workflow_resolve
    ...
    def test_workflow_add_does_not_copy_overlays(self, project_dir, monkeypatch, tmp_path):
        ...
        installed_overlay = (
            project_dir / ".agile" / "workflows" / "wf" / "overlays" / "ov1.yml"
        )
...
class TestOverlayFilenameVsManifestId:
    ...
    def test_find_overlay_by_manifest_id_not_filename(self, project_dir, monkeypatch):
        ...
        from agile_cli.workflows.overlays._commands import _find_overlay_file
    ...
    def test_enable_disable_with_mismatched_filename(self, project_dir, monkeypatch):
        ...
        assert result.exit_code == 0, result.output
        data = yaml.safe_load(
            (project_dir / ".agile" / "workflows" / "overlays" / "wf" / "custom.yml").read_text(
                encoding="utf-8"
            )
        )
        assert data["enabled"] is False
    ...
    def test_remove_with_mismatched_filename(self, project_dir, monkeypatch):
        ...
        assert result.exit_code == 0, result.output
        assert not (
            project_dir / ".agile" / "workflows" / "overlays" / "wf" / "custom.yml"
        ).exists()
