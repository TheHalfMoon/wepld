...
function mkTmp() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'pictorial-design-system-'));
  tempDirs.push(dir);
  return dir;
}
...
describe('loadDesignSystemForCwd()', () => {
  it('loads DESIGN.md plus .pictorial/design.json and marks stale sidecars', () => {
    const cwd = mkTmp();
    fs.mkdirSync(path.join(cwd, '.pictorial'), { recursive: true });
    const designMd = path.join(cwd, 'DESIGN.md');
    const sidecarJson = path.join(cwd, '.pictorial', 'design.json');
