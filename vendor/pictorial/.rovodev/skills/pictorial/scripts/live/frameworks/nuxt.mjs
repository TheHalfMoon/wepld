...
export const NUXT_PLUGIN_MARKER = 'pictorial-live-nuxt-plugin';
export const NUXT_PLUGIN_NAME = 'pictorial-live.client.ts';
...
const liveSelector = 'script[data-pictorial-live-nuxt]';
...
export default defineNuxtPlugin(() => {
  ...
  script.src = liveSrc;
  script.async = true;
  script.dataset.pictorialLiveNuxt = '';
...
export function applyNuxtLiveAdapter({ cwd = process.cwd(), port, token, project = detectNuxtProject(cwd) }) {
  ...
  if (existing !== null && !existing.includes(NUXT_PLUGIN_MARKER)) {
    return {
      ...
      hint: `${project.pluginFile} already exists and is not managed by Pictorial Live`,
    };
  }
...
export function removeNuxtLiveAdapter({ cwd = process.cwd(), project = detectNuxtProject(cwd) }) {
  ...
  if (!content.includes(NUXT_PLUGIN_MARKER)) {
    return {
      ...
      removed: false,
      error: 'nuxt_plugin_conflict',
      hint: `${project.pluginFile} is not managed by Pictorial Live`,
    };
  }
