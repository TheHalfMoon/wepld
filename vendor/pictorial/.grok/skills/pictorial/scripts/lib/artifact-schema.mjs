...
/**
 * Schema versions for the artifacts Pictorial writes, plus the readers and
...
/** `.pictorial/design.json`, as documented in reference/document.md Step 4b. */
...
/**
 * Headings Pictorial used to read and no longer does, with the reason. The
...
const PRODUCT_STAMP_RE = /^[ \t]*<!--[ \t]*pictorial:product-schema[ \t]+(\d+)[ \t]*-->[ \t]*$/im;
...
export function productStampLine(version = PRODUCT_SCHEMA_VERSION) {
  return `<!-- pictorial:product-schema ${version} -->`;
}
