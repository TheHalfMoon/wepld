...
Built-in integrations are maintained by the Agile core team and ship with the CLI.

### Checklist

1. **Create the integration subpackage** under `src/agile_cli/integrations/<package_dir>/`
...
3. **Register the integration** in `src/agile_cli/integrations/__init__.py`
...
1. **Working integration** — tested with `agile integration install`
...
integration:
  ...
  description: "Integration for My Agent"
  author: "your-name"
  repository: "https://github.com/your-name/agile-my-agent"
  license: "MIT"
requires:
  agile_version: ">=0.6.0"
  tools:
...
| `requires.agile_version` | Required field; specify a version constraint such as `>=0.6.0` (current validation checks presence only) |
...
1. **Fork** the [agile repository](https://github.com/TheHalfMoon/wepld)
...
The `agile integration upgrade` command supports diff-aware upgrades:
