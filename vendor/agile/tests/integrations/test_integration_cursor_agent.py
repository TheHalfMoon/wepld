...
class TestCursorAgentInitFlow:
    ...
    def test_integration_cursor_agent_creates_skills(self, tmp_path):
        ...
        assert (target / ".cursor" / "skills" / "agile-plan" / "SKILL.md").exists()
...
class TestCursorAgentCliDispatch:
    ...
    def test_requires_cli_is_false_for_ide_first_flow(self):
        ...
        ``agile init --integration cursor-agent`` (without ``--ignore-agent-tools``)
    ...
    def test_build_exec_args_default_includes_headless_flags_and_json(self):
        ...
        """
        i = get_integration("cursor-agent")
        args = i.build_exec_args("/agile-specify some-feature")
        assert args == [
            "cursor-agent", "-p", "--trust", "--approve-mcps", "--force",
            "/agile-specify some-feature",
            "--output-format", "json",
        ]

    def test_build_exec_args_text_output_omits_format(self):
        i = get_integration("cursor-agent")
        args = i.build_exec_args("/agile-plan", output_json=False)
        assert args == [
            "cursor-agent", "-p", "--trust", "--approve-mcps", "--force",
            "/agile-plan",
        ]
    ...
    def test_build_exec_args_with_model(self):
        ...
        args = i.build_exec_args(
            "/agile-specify", model="sonnet-4-thinking", output_json=False
        )
        assert args == [
            "cursor-agent", "-p", "--trust", "--approve-mcps", "--force",
            "/agile-specify",
    ...
    def test_build_exec_args_contains_mandatory_headless_flags(self):
        ...
        agile workflow.  Together with ``-p`` and ``--trust`` they
        bring cursor-agent's headless behaviour in line with
        ``claude -p`` / ``codex --exec`` from agile's perspective.
        """
        i = get_integration("cursor-agent")
        args = i.build_exec_args("/agile-implement", output_json=False)
    ...
    def test_build_exec_args_supports_dispatch_without_requires_cli(self):
        ...
        precheck (so ``agile init`` doesn't fail when the CLI isn't on
        ...
        i = get_integration("cursor-agent")
        assert i.config.get("requires_cli") is False
        argv = i.build_exec_args("/agile-plan", output_json=False)
        assert argv is not None
        assert argv[0] == "cursor-agent"

    def test_build_exec_args_honors_executable_override(self, monkeypatch):
        """``AGILE_INTEGRATION_CURSOR_AGENT_EXECUTABLE`` overrides argv[0].
        ...
        """
        monkeypatch.setenv(
            "AGILE_INTEGRATION_CURSOR_AGENT_EXECUTABLE", "/custom/cursor"
        )
        i = get_integration("cursor-agent")
        args = i.build_exec_args("/agile-plan", output_json=False)
    ...
    def test_build_exec_args_honors_extra_args_override(self, monkeypatch):
        """``AGILE_INTEGRATION_CURSOR_AGENT_EXTRA_ARGS`` flags are injected
        *before* Agile's canonical ``--model`` / ``--output-format`` flags.
        ...
        so they cannot clobber, displace, or reorder Agile's canonical
        trailing flags. Exercise with both a model and JSON output so both
        canonical flags are present to pin against.
        """
        monkeypatch.setenv(
            "AGILE_INTEGRATION_CURSOR_AGENT_EXTRA_ARGS", "--foo bar"
        )
        i = get_integration("cursor-agent")
        args = i.build_exec_args(
            "/agile-plan", model="sonnet-4-thinking", output_json=True
        )
        assert "--foo" in args
        assert "bar" in args
    ...
    def test_build_command_invocation_uses_hyphenated_skill_name(self):
        """SkillsIntegration: /agile-plan (not /agile.plan)."""
        i = get_integration("cursor-agent")
        assert i.build_command_invocation("agile.plan", "feature-x") == "/agile-plan feature-x"
    ...
    def test_dispatch_command_resolves_cmd_shim_for_subprocess(self):
        ...
        with patch(
            "agile_cli.integrations.base.shutil.which", return_value=fake_path
        ), patch("subprocess.run", return_value=mock_result) as mock_run:
            result = i.dispatch_command(
                "agile.plan", args="feature-x", stream=False, timeout=5
            )

        assert result["exit_code"] == 0
        argv = mock_run.call_args[0][0]
        assert argv[0] == fake_path, f"expected resolved .CMD path, got: {argv[0]!r}"
        assert argv[1:6] == ["-p", "--trust", "--approve-mcps", "--force", "/agile-plan feature-x"]
    ...
    def test_dispatch_command_passthrough_when_shutil_which_finds_nothing(self):
        ...
        with patch(
            "agile_cli.integrations.base.shutil.which", return_value=None
        ), patch("subprocess.run", return_value=mock_result) as mock_run:
            i.dispatch_command("agile.plan", stream=False, timeout=5)
