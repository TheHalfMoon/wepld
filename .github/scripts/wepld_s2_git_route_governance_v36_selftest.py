#!/usr/bin/env python3
"""Self-tests for the v36 S2 Git-route decision successor."""
    ...
    def read_bytes(self, path: str, max_bytes: int) -> bytes:
        if path in self._replacements:
            ...
            if len(data) > max_bytes:
                base.fail(f"v36 self-test overlay exceeds read bound: {path}")
            return data
...
def _check_authority_markers() -> None:
    ...
    for value, label in (
        ...
        if value != "NONE":
            base.fail(f"v36 must not grant {label}")
    if p.S2_IMPLEMENTATION_AUTHORITY != p.p.S2_IMPLEMENTATION_AUTHORITY:
        base.fail("v36 route decision must not widen S2 implementation authority")
    if p.DEPENDENCY_ADMISSION != p.p.DEPENDENCY_ADMISSION:
        base.fail("v36 route decision must not change dependency admission")
    if p.SOURCE_ADMISSION != p.p.SOURCE_ADMISSION:
        base.fail("v36 route decision must not change source admission")
    if p.NEXT_AUTHORITY_GATE != "S2-AUTH-014":
        base.fail("v36 next authority gate drift")
...
def _check_qualification_contract_is_closed() -> None:
    ...
    if observed != required:
        base.fail(
            "v36 Git-route qualification contract drifted: "
            f"expected={sorted(required)} actual={sorted(observed)}"
        )
    if len(p.GIT_ROUTE_QUALIFICATION_CONTRACT) != len(observed):
        base.fail("v36 Git-route qualification contract contains duplicates")
...
def _check_command_family_is_specification_only() -> None:
    if p.GIT_TOPOLOGY_COMMAND_FAMILY != (
        ...
        base.fail("v36 candidate Git command family drift")
    if p.GIT_PROCESS_ADMISSION != "NONE" or p.GIT_EXECUTION_AUTHORITY != "NONE":
        base.fail("v36 command-family specification must not execute itself")


def _check_predecessor_and_frontier_are_exact() -> None:
    p.req_v35(p.root)
    p.req_canonical_frontier(p.root)
    if p.V35_P_BLOB != "e5377d3d546106adb250e9915af667f4bb080eb6":
        base.fail("v36 frozen v35 integrity identity drift")
    if p.V35_T_BLOB != "8284914739d1842f5100480fed969ec04d8667c7":
        base.fail("v36 frozen v35 self-test identity drift")
    if p.REQUIRED_CANONICAL_FRONTIER_BLOBS[p.p.CHECKPOINT] != p.p.FINAL_CHECKPOINT_BLOB:
        base.fail("v36 must bind the corrected canonical checkpoint FINAL")
    if p.REQUIRED_CANONICAL_FRONTIER_BLOBS[p.p.LEDGER] != p.p.FINAL_LEDGER_BLOB:
        base.fail("v36 must bind the canonical Build Learning FINAL")
...
def _check_workflow_projection_reverses() -> None:
    ...
    for path in (p.FW, p.AW):
        ...
        if p.V25.sha(data) != p.P_WF[path]:
            base.fail(f"v36 workflow projection does not reverse to v35: {path}")
        if p._V36_ENTRYPOINT in data:
            base.fail(f"v36 workflow projection left a v36 entrypoint: {path}")
...
def _check_bootstrap_scope_is_closed() -> None:
    ...
    if p.BOOT != expected:
        base.fail(f"v36 bootstrap path set drifted: {sorted(p.BOOT)}")
    if p.CONTROLLED_FILES != frozenset({p.P, p.T}):
        base.fail("v36 controlled file set must be exactly the two policy files")
    if p.BOOT & frozenset(p.REQUIRED_CANONICAL_FRONTIER_BLOBS):
        base.fail("v36 bootstrap must not mutate the bound canonical frontier")
