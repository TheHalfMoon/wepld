...
A row may use more than one of these exact base dispositions when distinct reusable aspects exist, joined with `+`. Ad-hoc variants such as `_CANDIDATE`, `_BY_DEFAULT`, `_WHOLESALE`, `TEST_QUARRY`, or `REJECT_AS_RUNTIME` are not valid disposition values; qualification/reason belongs in the explanation column.

Nothing below is imported automatically. This ledger identifies what deserves exact follow-up before fresh foundation acceptance or later slices.

## Canonical-main candidates

| Old path/family | Disposition | What is worth preserving | Fresh destination / treatment |
|---|---|---|---|
| `LICENSE` | KEEP_AS_IS | Proprietary/closed-source outbound posture is a candidate for preservation, subject to current repository/governance confirmation. | Root legal files only after exact current review. |
| `COPYRIGHT` | KEEP_AS_IS | Ownership notice is a candidate for preservation. | Root legal files only after exact current review. |
...
| `docs/governance/INTELLECTUAL_PROPERTY.md` | COPY_AND_REWRITE | IP/provenance discipline. | Fresh governance policy aligned to the selected repository posture. |
...
| PR #11 as a whole | REJECT | Reject wholesale adoption; replacement-first, then close as superseded under RAT-03. |
...
| `crates/runtime/` | HISTORICAL_REFERENCE + TEST_ORACLE_ONLY | Old runtime ownership/vocabulary conflicts with ratified Mission Runtime/Nawat/Edara boundaries; retain only history and test/failure cases. |
...
| `crates/specification/` | CONCEPT_ONLY + TEST_ORACLE_ONLY | Spec parsing/validation tests may inform AGILLE, but GitHub Spec Kit is the mandatory build method and new typed AGILLE begins later. |
...
| `crates/hermes/` | REJECT + TEST_ORACLE_ONLY | Reject Hermes as the new runtime owner; retain adversarial/golden/governance/lifecycle tests as quarry. |
...
| PR #1 dependency set | REJECT | Fresh tree starts with zero implementation dependencies; re-admit only per exact capability gate. |
