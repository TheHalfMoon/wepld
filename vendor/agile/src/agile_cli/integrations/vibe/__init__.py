...
Vibe uses ``.vibe/skills/agile-<name>/SKILL.md`` layout (enforced since v2.0.0).
"""
...
# but in practice ``/agile-analyze`` returns a 300-500 line report that is
...
class VibeIntegration(SkillsIntegration):
    ...
    def _render_skill(self, template_name: str, frontmatter: dict[str, Any], body: str) -> str:
        ...
        skill_name = f"agile-{template_name.replace('.', '-')}"
        description = frontmatter.get(
            "description",
            f"Agile workflow command: {template_name}",
        )
    ...
    def _skill_stem_from_content(content: str) -> str | None:
        ...
        the ``agile-`` prefix. Returns ``None`` when no name is present.
        """
        dash_count = 0
