...
- [🔧 Agile CLI 参考](#-wepld-agile-参考)
- [🧩 打造你自己的 Agile：扩展与预设](#-打造你自己的-agile扩展与预设)
...
需要 **[uv](https://docs.astral.sh/uv/)**（[安装 uv](./docs/install/uv.md)）。将 `vX.Y.Z` 替换为 [Releases](https://github.com/TheHalfMoon/wepld/releases) 中最新的发布标签 —— 记得保留开头的 `v`（例如 `v0.12.11`，而不是 `0.12.11`）：

```bash
uv tool install wepld-agile --from git+https://github.com/TheHalfMoon/wepld.git@vX.Y.Z
```

更倾向从 PyPI 安装？`wepld-agile` 包同样发布在那里：
...
在项目目录下启动你的编码助手。大多数助手将 agile 暴露为 `/agile.*` 斜杠命令；处于技能（skills）模式的 Codex CLI 则使用 `$agile-*`；GitHub Copilot CLI 使用 `/agents` 来选择助手，或直接在提示词中指定它。

使用 **`/agile.constitution`** 命令来创建项目的治理准则和开发指南，它们将指导后续所有开发工作。
...
使用 **`/agile.tasks`** 从实现方案生成一份可执行的任务清单。
...
使用 **`/agile.implement`** 执行所有任务，按方案构建你的功能。
...
运行 `agile init` 后，你的 AI 编码助手就能使用这些斜杠命令来进行结构化开发。对于支持技能模式的集成，传入 `--integration <agent> --integration-options="--skills"` 会安装助手技能，而不是斜杠命令的提示词文件。
...
| `/agile.implement`     | `agile-implement`    | 执行所有任务，按方案构建功能                               |
| `/agile.converge`      | `agile-converge`     | 对照规范/方案/任务评估代码库，并将剩余工作追加为新任务     |
...
| 命令                 | 助手技能               | 说明                                                                                              |
| -------------------- | ---------------------- | ------------------------------------------------------------------------------------------------- |
| `/agile.clarify`   | `agile-clarify`      | 澄清描述不充分的部分（建议在 `/agile.plan` 之前使用；旧称 `/quizme`）                            |
| `/agile.analyze`   | `agile-analyze`      | 跨制品的一致性与覆盖度分析（在 `/agile.tasks` 之后、`/agile.implement` 之前运行）              |
| `/agile.checklist` | `agile-checklist`    | 生成自定义质量清单，校验需求的完整性、清晰度与一致性（好比"为自然语言写单元测试"）                 |
...
Agile 可通过两套互补的机制进行深度定制 —— **扩展（extensions）** 和 **预设（presets）** —— 以及面向单个项目的本地覆盖，用于临时性调整：

| 优先级 | 组件类型                           | 位置                             |
| -----: | ---------------------------------- | -------------------------------- |
|   ⬆ 1 | 项目本地覆盖                       | `.agile/templates/overrides/`  |
|      2 | 预设 —— 定制核心与扩展             | `.agile/presets/templates/`    |
|      3 | 扩展 —— 新增能力                   | `.agile/extensions/templates/` |
|   ⬇ 4 | Agile 核心 —— 内置 SDD 命令与模板 | `.agile/templates/`            |
...
当你需要 Agile 核心之外的功能时，使用**扩展**。扩展可引入新命令和模板 —— 例如添加核心 SDD 命令未覆盖的领域特定工作流、集成外部工具，或新增全新的开发阶段。它们扩展了 *Agile 能做什么*。
...
当你想改变 Agile 的*工作方式*而不是新增能力时，使用**预设**。预设会覆盖核心及已安装扩展中附带的模板和命令 —— 例如强制使用面向合规的规范格式、采用领域特定术语，或对方案和任务应用组织规范。预设定制的是 Agile 及其扩展生成的制品与指令。
...
agile bundle list
agile bundle update <bundle-id>     # 或 --all
agile bundle remove <bundle-id>     # 仅移除此捆绑包的组件
```

捆绑包从一个**按优先级排序的目录栈**（项目 > 用户 > 内置）中解析。每个来源都带有安装策略：`install-allowed` 来源可用于安装，而 `discovery-only` 来源在 `search`/`info` 中可见但拒绝安装。可通过 `agile bundle catalog list|add|remove` 管理目录栈。

作者在本地校验并打包捆绑包。分发方式是托管构建产物并添加一个目录来源；社区捆绑包投稿请使用 [Bundle Submission](https://github.com/TheHalfMoon/wepld/issues/new?template=bundle_submission.yml) issue 模板，以便对所需的组件目录和安装证据进行审阅：

```bash
agile bundle validate --path ./my-bundle      # 结构与引用检查
agile bundle build --path ./my-bundle         # 生成带版本的 .zip 产物
```
...
对于已有项目，请将 Agile 工具本身的更新与功能制品的演进分开处理：升级时刷新受管理的项目文件，而在预期行为发生变化时更新 `specs/` 制品。[规范演进指南](./docs/guides/evolving-specs.md)介绍了推荐的棕地迭代循环。
...
如需帮助，请提交 [GitHub issue](https://github.com/TheHalfMoon/wepld/issues/new)。我们欢迎缺陷报告、功能建议，以及关于使用规范驱动开发的各类问题。
