...
def test_commands_package_importable():
    mod = importlib.import_module("agile_cli.commands")
    assert mod is not None


def test_commands_init_importable():
    mod = importlib.import_module("agile_cli.commands.init")
...
def test_script_type_choices_includes_python():
    from agile_cli._agent_config import SCRIPT_TYPE_CHOICES
...
def test_workflow_init_valid_script_types_includes_python():
    from agile_cli.workflows.steps.init import VALID_SCRIPT_TYPES
...
def test_agent_config_re_exported_from_init():
    from agile_cli import AGENT_CONFIG, SCRIPT_TYPE_CHOICES
    assert isinstance(AGENT_CONFIG, dict)
    assert "sh" in SCRIPT_TYPE_CHOICES
...
def test_resolve_default_init_integration_re_exported_from_init():
    from agile_cli import resolve_default_init_integration
