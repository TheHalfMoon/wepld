...
def _py_cmd(repo: Path, *args: str) -> list[str]:
    script = repo / ".agile" / "scripts" / "python" / "check_prerequisites.py"
...
def _bash_cmd(repo: Path, *args: str) -> list[str]:
    script = repo / ".agile" / "scripts" / "bash" / "check-prerequisites.sh"
    return ["bash", str(script), *args]


def _ps_cmd(repo: Path, *args: str) -> list[str]:
    script = repo / ".agile" / "scripts" / "powershell" / "check-prerequisites.ps1"
...
def test_python_normal_mode_persists_feature_json(prereq_repo: Path) -> None:
    ...
    assert py.returncode == 0, py.stderr
    data = json.loads(
        (prereq_repo / ".agile" / "feature.json").read_text(encoding="utf-8")
    )
...
class TestGetInvokeSeparatorTolerance:
    ...
    def _load_common():
        ...
        spec = importlib.util.spec_from_file_location("_agile_common_py", COMMON_PY)
    ...
    def _repo(self, tmp_path: Path, body: str | bytes) -> Path:
        (tmp_path / ".agile").mkdir(parents=True, exist_ok=True)
        target = tmp_path / ".agile" / "integration.json"
