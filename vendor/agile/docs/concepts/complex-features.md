...
The `/agile.implement` command accepts free-form user input that the agent
...
Instead of letting `/agile.implement` run through every task at once, tell it
to stop early:

```text
/agile.implement only execute tasks T001-T010, then stop and report progress
```

or scope by phase:

```text
/agile.implement only execute the Setup phase, then stop
```

Because completed tasks are marked `[X]` in `tasks.md`, the next
`/agile.implement` invocation picks up where you left off. This keeps each run
...
GitHub Copilot extension for VS Code), you can instruct `/agile.implement` to
delegate individual tasks:

```text
/agile.implement delegate each parallel [P] task to a sub-agent
```
...
```text
/agile.implement execute only the Core phase, delegate [P] tasks to sub-agents
```
