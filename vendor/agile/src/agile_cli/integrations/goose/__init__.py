...
class GooseIntegration(YamlIntegration):
    ...
    def build_exec_args(
        ...
        *,
        ...
        Agile installs its commands as Goose *recipes* under
        ``.goose/recipes/``, each declaring an optional ``args`` string
        parameter, so a ``/agile.<name> <rest>`` invocation maps onto
        ``--recipe <path> --params args=<rest>``. Only that namespace is
        mapped: ``--recipe`` is a *path* Agile synthesizes, unlike
        ...
        # AGILE_INTEGRATION_GOOSE_EXTRA_ARGS makes goose exit with
