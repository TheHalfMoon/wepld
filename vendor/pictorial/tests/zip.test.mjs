...
function makeUniversalTree(distDir) {
  const skillDir = join(distDir, 'universal', 'skills', 'pictorial');
...
function makeLargeUniversalTree(distDir, { providers = ['.claude', '.cursor', '.agents', '.gemini', '.github', '.kiro'], scriptCount = 8, extraSkills = ['audit', 'polish'] } = {}) {
  ...
  for (const provider of providers) {
    // The pictorial skill ships a scripts/ dir with many files, like the real
    // bundle. This is where most entries live.
    const scriptsDir = join(distDir, 'universal', provider, 'skills', 'pictorial', 'scripts');
    mkdirSync(scriptsDir, { recursive: true });
    writeFileSync(join(distDir, 'universal', provider, 'skills', 'pictorial', 'SKILL.md'), `---\nname: pictorial\n---\nprovider ${provider}\n`);
    files += 1;
...
describe('release bundle zip writer', () => {
  it('createAllZips produces a non-empty universal.zip that unpacks to the skill tree', async () => {
    ...
    const skillMd = join(out, 'skills', 'pictorial', 'SKILL.md');
    assert.ok(existsSync(skillMd), 'unpacked bundle is missing skills/pictorial/SKILL.md');
    assert.match(readFileSync(skillMd, 'utf8'), /name: pictorial/);
