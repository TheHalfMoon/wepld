...
This is an example command that demonstrates how to create commands for Agile extensions.
...
``bash
config_file=".agile/extensions/my-extension/my-extension-config.yml"

if [ ! -f "$config_file" ]; then
  echo "❌ Error: Configuration not found at $config_file"
  echo "Run 'agile extension add my-extension' to install and configure"
  exit 1
fi
...
setting_value="${AGILE_MY_EXTENSION_KEY:-$setting_value}"
...
``bash
output_file=".agile/my-extension-output.json"
...
- `AGILE_MY_EXTENSION_KEY` - Overrides `settings.key`
- `AGILE_MY_EXTENSION_ANOTHER_KEY` - Overrides `settings.another_key`

Example:
``bash
export AGILE_MY_EXTENSION_KEY="override-value"
``
...
export AGILE_MY_EXTENSION_KEY="custom-value"
> /agile.my-extension.example
``
...
*For more information, see the extension README or run `agile extension info my-extension`*
