...
function runInject(cwd, configPath, args) {
  try {
    const out = execFileSync('node', [INJECT, ...args], {
      cwd,
      encoding: 'utf-8',
      env: { ...process.env, PICTORIAL_LIVE_CONFIG: configPath },
...
function runInjectDefault(cwd, args) {
  try {
    const out = execFileSync('node', [INJECT, ...args], {
      cwd,
      encoding: 'utf-8',
      env: { ...process.env, PICTORIAL_LIVE_CONFIG: '' },
      stdio: ['ignore', 'pipe', 'pipe'],
    });
    return JSON.parse(out.trim());
  } catch (err) {
...
describe('live-inject — insert/remove round-trip preserves file bytes', () => {
  let tmp;
  beforeEach(() => { tmp = mkdtempSync(join(tmpdir(), 'pictorial-inject-test-')); });
  afterEach(() => { rmSync(tmp, { recursive: true, force: true }); });

  it('reports .pictorial/live/config.json as the default missing config path', () => {
    const result = runInjectDefault(tmp, ['--check']);

    assert.equal(result.ok, false);
    assert.equal(result.error, 'config_missing');
    assert.equal(result.path, join(realpathSync(tmp), '.pictorial', 'live', 'config.json'));
  });

  it('uses .pictorial/live/config.json without an environment override', () => {
    const original = `<html>
  <body>
    <p>Content</p>
  </body>
...
`;
    ...
    const configDir = join(tmp, '.pictorial', 'live');
...
`;
  ...
  it('round-trips through CSP-meta patch and revert (insert mutates the meta tag, remove restores it)', () => {
    ...
    // and stashes the original directives in `data-pictorial-csp-original`.
...
<html>
  <body>
    <h1>{title}</h1>
    <!-- pictorial-live-start -->
    <script src="http://localhost:8400/live.js"></script>
    <!-- pictorial-live-end --></body>
</html>
`;
    const file = join(tmp, 'Layout.astro');
    writeFileSync(file, original);
    ...
    assert.equal((afterInject.match(/pictorial-live-start/g) || []).length, 1, 'reinjection should leave one live block');
...
`;
  ...
  it('preserves the character after an insertAfter anchor with no trailing newline (#227)', () => {
    ...
    assert.ok(
      afterInject.includes('<!-- pictorial-live-end -->\nX</head>'),
  ...
  it('uses an idempotent dev-only client plugin for a Nuxt 4 app directory', () => {
    ...
    const pluginPath = join(tmp, 'app', 'plugins', 'pictorial-live.client.ts');
    const firstPlugin = readFileSync(pluginPath, 'utf-8');
    assert.equal(first.ok, true);
    assert.equal(first.adapter, 'nuxt');
    assert.equal(first.results[0].file, 'app/plugins/pictorial-live.client.ts');
    assert.equal(first.results[0].changed, true);
    assert.match(firstPlugin, /if \(!import\.meta\.dev/);
    assert.match(firstPlugin, /data-pictorial-live-nuxt/);
  ...
  it('respects a literal Nuxt srcDir and never overwrites a user plugin', () => {
    ...
    const pluginPath = join(tmp, 'client', 'plugins', 'pictorial-live.client.ts');
...
const ORPHAN_PLUGIN = '/* pictorial-live-nuxt-plugin */\nexport default defineNuxtPlugin(() => {});\n';

function stageOrphanedSession(root) {
  mkdirSync(join(root, 'app', 'plugins'), { recursive: true });
  writeFileSync(join(root, 'app', 'plugins', 'pictorial-live.client.ts'), ORPHAN_PLUGIN);
  mkdirSync(join(root, '.pictorial', 'live'), { recursive: true });
  writeFileSync(join(root, '.pictorial', 'live', 'inject-journal.json'), JSON.stringify({
    version: 1,
    appRoot: root,
    framework: 'nuxt',
    port: 8400,
    ...
    artifacts: [{
      kind: 'created',
      path: 'app/plugins/pictorial-live.client.ts',
      marker: 'pictorial-live-nuxt-plugin',
      pruneTo: 'app',
    }],
  }));
}
...
describe('live-inject — crash-safe injection journal', () => {
  it('refuses to heal artifacts outside the project tree (review M1)', async () => {
    ...
    const outside = join(tmp, '..', `pictorial-victim-${Date.now()}`);
    writeFileSync(outside, 'precious');
    try {
      mkdirSync(join(tmp, '.pictorial', 'live'), { recursive: true });
      writeFileSync(join(tmp, '.pictorial', 'live', 'inject-journal.json'), JSON.stringify({
        version: 1,
        artifacts: [
  ...
  let tmp;
  beforeEach(() => { tmp = realpathSync(mkdtempSync(join(tmpdir(), 'pictorial-journal-cli-'))); });
  afterEach(() => { rmSync(tmp, { recursive: true, force: true }); });

  function writeLiveConfig(root) {
    mkdirSync(join(root, '.pictorial', 'live'), { recursive: true });
    writeFileSync(join(root, '.pictorial', 'live', 'config.json'), JSON.stringify({
      files: ['index.html'],
      insertBefore: '</body>',
      commentSyntax: 'html',
    }));
  ...
  it('records what an inject wrote and clears it again on remove', () => {
    ...
    const journal = JSON.parse(readFileSync(join(tmp, '.pictorial', 'live', 'inject-journal.json'), 'utf-8'));
    assert.equal(journal.port, 8400);
    assert.deepEqual(journal.artifacts.map((a) => a.path), ['index.html']);

    runInjectDefault(tmp, ['--remove']);
    assert.equal(existsSync(join(tmp, '.pictorial', 'live', 'inject-journal.json')), false);
  });
  ...
  it('heals artifacts a SIGKILLed session left behind, on the next inject', () => {
    ...
    assert.deepEqual(result.healed, [{ path: 'app/plugins/pictorial-live.client.ts', action: 'removed' }]);
    assert.equal(existsSync(join(tmp, 'app', 'plugins', 'pictorial-live.client.ts')), false);
  ...
  it('heals the appRoot journal when stop runs from the wrong directory', () => {
    ...
    writeFileSync(join(tmp, '.pictorial', 'live', 'roots.json'), JSON.stringify({
      version: 1,
      appRoot: tmp,
      repoRoot: tmp,
      contextRoot: null,
      sessionRoot: join(tmp, '.pictorial', 'live'),
      resolvedFrom: 'cwd',
    }));
    ...
    assert.deepEqual(result.healed, [{ path: 'app/plugins/pictorial-live.client.ts', action: 'removed' }]);
    assert.equal(
      existsSync(join(tmp, 'app', 'plugins', 'pictorial-live.client.ts')),
      false,
