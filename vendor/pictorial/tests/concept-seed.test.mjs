...
// The live catalog is service-side (pictorial-site); the public repo tests
...
function run(scope, extraArgs = [], env = {}) {
  return spawnSync(process.execPath, [SCRIPT, '--scope', scope, '--from', 'stable-test', ...extraArgs], {
    cwd: ROOT,
    encoding: 'utf-8',
    env: { ...process.env, PICTORIAL_CATALOG_DIR: FIXTURE_DIR, ...env },
  });
}
...
describe('concept seed scopes', () => {
    ...
    for (const count of [5, 6, 7]) {
      for (let index = 0; index < 30; index += 1) {
        const result = spawnSync(process.execPath, [SCRIPT, '--scope', 'direction', '--from', `count-${count}-${index}`, '--candidate-count', String(count)], {
          cwd: ROOT,
          encoding: 'utf-8',
          env: { ...process.env, PICTORIAL_CATALOG_DIR: FIXTURE_DIR },
        });
  ...
  it('degrades to a promotion-only seed when catalog and API are both unreachable', () => {
    const degraded = run('direction', ['--mode', 'persuade'], {
      PICTORIAL_CATALOG_DIR: '/nonexistent-catalog-dir',
      PICTORIAL_API_URL: 'http://127.0.0.1:9/api',
      PICTORIAL_API_TIMEOUT: '400',
    });
  ...
  it('keeps composition challengers inside the requested surface mode', () => {
    ...
    const gated = run('direction', ['--mode', 'experience'], { PICTORIAL_COMPOSITIONS: '1' });
  ...
  it('registers steer the round presentation without changing the deal', () => {
    ...
    const degradedSafer = run('direction', ['--reroll', '1', '--register', 'safer'], {
      PICTORIAL_CATALOG_DIR: '/nonexistent-catalog-dir',
      PICTORIAL_API_URL: 'http://127.0.0.1:1',
    });
...
describe('init gate', () => {
  const gateRun = (cwd) => spawnSync(process.execPath, [SCRIPT, '--scope', 'direction', '--from', 'gate-test'], {
    cwd,
    encoding: 'utf-8',
    env: { ...process.env, PICTORIAL_CATALOG_DIR: FIXTURE_DIR, PICTORIAL_CONTEXT_DIR: '' },
  });
  ...
  it('never gates the choice ping', () => {
    ...
    const result = spawnSync(process.execPath, [SCRIPT, '--chosen', 'assigned', '--from', 'gate-test'], {
      cwd: dir,
      encoding: 'utf-8',
      env: { ...process.env, PICTORIAL_CATALOG_DIR: FIXTURE_DIR, PICTORIAL_NO_TELEMETRY: '1' },
    });
    assert.equal(result.status, 0);
    assert.doesNotMatch(result.stdout, /NO_PRODUCT_MD/);
  ...
  it('pingChosen validates kinds, requires ids only for challenger wins, and honors opt-out', async () => {
    ...
    const savedNoTelemetry = process.env.PICTORIAL_NO_TELEMETRY;
    try {
      delete process.env.DO_NOT_TRACK;
      process.env.PICTORIAL_NO_TELEMETRY = '1';
      assert.equal(await pingChosen({ kind: 'assigned', key: 'k' }), false, 'opt-out wins over everything');
      delete process.env.PICTORIAL_NO_TELEMETRY;
      ...
      if (savedNoTelemetry === undefined) delete process.env.PICTORIAL_NO_TELEMETRY;
      else process.env.PICTORIAL_NO_TELEMETRY = savedNoTelemetry;
    }
  });
...
describe('API roll path', () => {
  ...
  const PRELOAD = [
    ...
    'await fetch(`${process.env.PICTORIAL_API_URL}/warmup`).then(r => r.arrayBuffer()).catch(() => {});',
      ...
      const result = await new Promise((resolveRun, rejectRun) => {
        const child = spawn(NODE, [
          ...
          env: {
            ...process.env,
            PICTORIAL_CATALOG_DIR: '/nonexistent-catalog-dir',
            PICTORIAL_API_URL: `http://127.0.0.1:${server.address().port}/api`,
          },
        });
        let stdout = '';
        let stderr = '';
