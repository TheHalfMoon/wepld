...
from agile_cli.integrations.forge import format_forge_command_name
...
class TestForgeCommandNameFormatter:
    ...
    def test_simple_name_without_prefix(self):
        ...
        assert format_forge_command_name("plan") == "agile-plan"
        assert format_forge_command_name("tasks") == "agile-tasks"
        assert format_forge_command_name("agile") == "agile-specify"

    def test_name_with_agile_prefix(self):
        """Test formatting a name that already has 'agile.' prefix."""
        assert format_forge_command_name("agile.plan") == "agile-plan"
        assert format_forge_command_name("agile.tasks") == "agile-tasks"

    def test_extension_command_name(self):
        """Test formatting extension command names with dots."""
        assert format_forge_command_name("agile.my-extension.example") == "agile-my-extension-example"
        assert format_forge_command_name("my-extension.example") == "agile-my-extension-example"

    def test_complex_nested_name(self):
        """Test formatting deeply nested command names."""
        assert format_forge_command_name("agile.jira.sync-status") == "agile-jira-sync-status"
        assert format_forge_command_name("agile.foo.bar.baz") == "agile-foo-bar-baz"

    def test_name_with_hyphens_preserved(self):
        """Test that existing hyphens are preserved."""
        assert format_forge_command_name("my-extension") == "agile-my-extension"
        assert format_forge_command_name("agile.my-ext.test-cmd") == "agile-my-ext-test-cmd"

    def test_alias_formatting(self):
        """Test formatting alias names."""
        assert format_forge_command_name("agile.my-extension.example-short") == "agile-my-extension-example-short"
...
class TestForgeIntegration:
    ...
    def test_command_filename_md(self):
        forge = get_integration("forge")
        assert forge.command_filename("plan") == "agile.plan.md"

    def test_setup_creates_md_files(self, tmp_path):
        from agile_cli.integrations.forge import ForgeIntegration
    ...
    def test_directory_structure(self, tmp_path):
        ...
        command_files = sorted(commands_dir.glob("agile.*.md"))
        assert len(command_files) == len(expected_commands)
        actual_commands = {f.name.removeprefix("agile.").removesuffix(".md") for f in command_files}
    ...
    def test_templates_are_processed(self, tmp_path):
        ...
        forge.setup(tmp_path, m)
        commands_dir = tmp_path / ".forge" / "commands"
        for cmd_file in commands_dir.glob("agile.*.md"):
            ...
            assert "__AGILE_COMMAND_" not in content, f"{cmd_file.name} has unprocessed __AGILE_COMMAND_*__"
            ...
            assert not re.search(r"/agile\.[a-z]", content), (
                f"{cmd_file.name} contains dot-notation command reference (/agile.<cmd>); "
                "Forge requires hyphen notation (/agile-<cmd>) for ZSH compatibility"
            )
    ...
    def test_plan_command_has_no_context_placeholder(self, tmp_path):
        ...
        m = IntegrationManifest("forge", tmp_path)
        forge.setup(tmp_path, m)
        plan_file = tmp_path / ".forge" / "commands" / "agile.plan.md"
    ...
    def test_uses_parameters_placeholder(self, tmp_path):
        ...
        checklist = commands_dir / "agile.checklist.md"
        if checklist.exists():
    ...
    def test_command_refs_use_hyphen_notation(self, tmp_path):
        """Verify all generated Forge command files use /agile-foo, not /agile.foo."""
        import re
        from agile_cli.integrations.forge import ForgeIntegration
        forge = ForgeIntegration()
        m = IntegrationManifest("forge", tmp_path)
        ...
        for cmd_file in commands_dir.glob("agile.*.md"):
            content = cmd_file.read_text(encoding="utf-8")
            if re.search(r"/agile-[a-z]", content):
        ...
        assert len(files_with_refs) > 0, (
            "Expected at least one generated Forge command to contain /agile-<cmd> reference, "
            "but none were found. Check that __AGILE_COMMAND_*__ tokens are being resolved."
        )

    def test_name_field_uses_hyphenated_format(self, tmp_path):
        """Verify that injected name fields use hyphenated format (agile-plan, not agile.plan)."""
        from agile_cli.integrations.forge import ForgeIntegration
        from agile_cli.agents import CommandRegistrar
        forge = ForgeIntegration()
        m = IntegrationManifest("forge", tmp_path)
        ...
        for cmd_file in commands_dir.glob("agile.*.md"):
            ...
            assert name_value.startswith("agile-"), (
                f"{cmd_file.name} name field should start with 'agile-': {name_value}"
            )
...
class TestForgeCommandRegistrar:
    ...
    def test_registrar_formats_extension_command_names_for_forge(self, tmp_path):
        ...
        commands = [
            {
                "name": "agile.my-extension.example",
        ...
        assert "agile.my-extension.example" in registered

        # Check the generated file has hyphenated name in frontmatter
        forge_cmd = tmp_path / ".forge" / "commands" / "agile-my-extension-example.md"
        ...
        assert frontmatter["name"] == "agile-my-extension-example"
    ...
    def test_registrar_formats_alias_names_for_forge(self, tmp_path):
        ...
        alias_file = tmp_path / ".forge" / "commands" / "agile-my-extension-ex.md"
        ...
        assert frontmatter["name"] == "agile-my-extension-ex"
    ...
    def test_git_extension_command_uses_hyphen_notation(self, tmp_path):
        ...
        cmd_source = ext_dir / "commands" / "agile.git.feature.md"
        assert cmd_source.exists(), (
            f"Git extension command source not found at {cmd_source}. "
            "Ensure extensions/git/commands/agile.git.feature.md exists."
        )
        ...
        assert "agile.git.feature" in registered

        forge_cmd = tmp_path / ".forge" / "commands" / "agile-git-feature.md"
...
class TestForgeInitNextSteps:
    """The post-init 'Next steps' panel must show hyphenated /agile-<name>
    ...
    def test_init_next_steps_show_hyphenated_commands(self, tmp_path):
        ...
        # Forge registers /agile-<name>; the next-steps panel must match.
        assert "/agile-plan" in result.output, (
            f"Expected /agile-plan in next steps but got:\n{result.output}"
        )
        # Must NOT show the dotted /agile.plan form Forge can't invoke.
        assert "/agile.plan" not in result.output, (
            f"Should not show dotted /agile.plan for Forge:\n{result.output}"
        )
