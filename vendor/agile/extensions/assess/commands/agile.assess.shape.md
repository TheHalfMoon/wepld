...
- `ASSESS_DIR/problem.md` **MUST** exist. If it does not, stop and instruct the user to run `__AGILE_COMMAND_ASSESS_DEFINE__` first — shaping without a defined problem invites solutionizing in a vacuum.
...
1. **Generate 2–3 distinct options**, spanning the trade-off space. Always include a lightweight "smallest thing that could work" option and, where relevant, a "do nothing / buy instead of build" option. Each option:
   ...
   - **Rabbit holes**: the parts most likely to blow up scope, so `__AGILE_COMMAND_ASSESS_DECIDE__` sees them.
...
**Report back** with the slug (own line), the path to `concept.md`, the recommended option (or "none"), and the next step: `__AGILE_COMMAND_ASSESS_DECIDE__ slug=<ASSESS_SLUG>`.
