...
`source_generation` resolves immutably to the existing KnowledgeSource ingest-generation record, including exact source content, parser artifact/version and material parser configuration identity. Direct byte reads record an explicit qualified no-transform parser identity; omission is never treated as that identity. A DerivedProjection additionally resolves its `projection_generation` to the exact source-generation reference, index/projection implementation artifact/version, material build configuration (including tokenizer/chunker/embedding model where used), index content identity and build provenance. Every material signal contributing to the result retains the corresponding exact parser/index generation references; mixed-generation current views remain forbidden. DirectSource explicitly has no index and cannot claim index-derived signals. This defines provenance through existing generation records rather than adding another provenance store.

Replay/audit must recover those immutable parser and index identities, not the current parser/index associated with a mutable source ID. Changed parser, index implementation/configuration or embedding model creates a successor generation even when input bytes match. Missing or inaccessible required provenance yields an explicit incomplete/unverifiable result and cannot be presented as complete replay/current evidence; retained identifiers do not bypass present access/redaction rules. Existing S4 retrieval and HARD-S4-003/004 tasks include replacement-with-identical-source and missing-provenance negative fixtures.
