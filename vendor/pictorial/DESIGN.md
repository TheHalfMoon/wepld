...
# Design System: Pictorial

## 1. Overview: Neo Kinpaku

**Creative North Star: "Neo Kinpaku"**

Pictorial is now a dark lacquer interface marked by Japanese gold leaf and precise technical geometry. The brand should feel like a crafted object: black urushi, irregular kinpaku seams, quiet measurement marks, circuit traces, and controlled verdigris oxidation. It is refined, technical, and physical.
...
- `.ks-wordmark` — the PICTORIAL wordmark text, solid Alumni Sans (`--ks-font-wordmark`), uppercase, weight 400, letter-spacing 0.15em.
...
**The Gold Carries Brand Rule.** Kinpaku gold is the primary brand signal. If a single accent must represent Pictorial, use gold, not magenta or cyan.
...
- **Brand mark:** Pictorial carved-tile icon (same SVG paths as `site/components/Header.astro` / `favicon.svg`), kinpaku fill on transparent ground. Not a "/" slash or rounded-square placeholder.
...
**The Picker Is Brand Rule.** Live mode UI is Pictorial product chrome, not host-page chrome. It always ships the lacquer-deep fill, the carved-tile mark, and gold on the mark + active control. Brand reads through the mark and the gold accent rather than a gold border ringing the bar; the container itself stays a quiet neutral-edged tool.
