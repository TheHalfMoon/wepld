...
Audit the code.`;
  ...
  test('should read skill with reference files', () => {
    const skillContent = `---
name: pictorial
description: Pictorial design skill
---

Pictorial design instructions.`;
  ...
  test('should fall back to "pictorial" when frontmatter has no name', () => {
    const skillDir = path.join(testRootDir, 'skill');
    ensureDir(skillDir);
    ...
    expect(skills).toHaveLength(1);
    expect(skills[0].name).toBe('pictorial');
  });
...
describe('replacePlaceholders', () => {
  ...
  test('should exclude pictorial from {{available_commands}}', () => {
    const result = replacePlaceholders('Commands: {{available_commands}}', 'claude-code', ['audit', 'pictorial', 'polish']);
    expect(result).toBe('Commands: /audit, /polish');
  });

  test('should exclude legacy teach-pictorial from {{available_commands}}', () => {
    const result = replacePlaceholders('Commands: {{available_commands}}', 'claude-code', ['audit', 'teach-pictorial', 'polish']);
    expect(result).toBe('Commands: /audit, /polish');
  });

  test('lists /pictorial sub-commands for {{available_commands}} when no command names are passed', () => {
    // v3.0 single-skill architecture: with no command names, the list falls back
    // to the PICTORIAL_SUB_COMMANDS sub-commands rendered as `/pictorial <sub>`.
    const result = replacePlaceholders('Commands: {{available_commands}}', 'claude-code', []);
    expect(result.startsWith('Commands: /pictorial ')).toBe(true);
    expect(result).toContain('/pictorial audit');
    expect(result).toContain('/pictorial polish');
  });
  ...
  test('should replace Codex command invocations without rewriting paths', () => {
    const source = [
      'Run /pictorial audit.',
      'Use `/pictorial polish` next.',
      '.github/hooks/pictorial.json',
      '.codex/skills/pictorial/scripts/context.mjs',
      'https://example.com/pictorial',
    ].join('\n');

    const result = replacePlaceholders(source, 'codex', [], ['pictorial']);

    expect(result).toContain('Run $pictorial audit.');
    expect(result).toContain('Use `$pictorial polish` next.');
    expect(result).toContain('.github/hooks/pictorial.json');
    expect(result).toContain('.codex/skills/pictorial/scripts/context.mjs');
    expect(result).toContain('https://example.com/pictorial');
  });
});
...
describe('replaceScriptProviderMarker', () => {
  test('renders only the explicit provider declarations', () => {
    const source = [
      ...
      'const regex = /pictorial\\b/gi;',
      "const runtime = '/src/lib/pictorial/__runtime.js';",
      "const text = 'Run /pictorial audit';",
    ].join('\n');

    const result = replaceScriptProviderMarker(source, 'codex', 'agents');

    expect(result).toContain('export const PICTORIAL_COMMAND_PREFIX = "$";');
    expect(result).toContain('export const PICTORIAL_PROVIDER_ID = "agents";');
    expect(result).toContain('const regex = /pictorial\\b/gi;');
    expect(result).toContain("const runtime = '/src/lib/pictorial/__runtime.js';");
    expect(result).toContain("const text = 'Run /pictorial audit';");
  });
});
