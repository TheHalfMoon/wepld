...
from ._helpers import (
    ...
    _refresh_init_options_agile_version,
...
def _manifest_tracks_skill_layout(manifest) -> bool:
    ...
    A skill scaffold is written as ``.../agile-<name>/SKILL.md``, so a
...
def _installed_presets_affecting_agent(
    ...
    *,
    ...
    registry_path = (
        Path(project_root) / ".agile" / "presets" / PresetRegistry.REGISTRY_FILE
    )
...
def integration_switch(
    ...
    if installed_key:
        ...
        manifest_path = project_root / ".agile" / "integrations" / f"{installed_key}.manifest.json"
    ...
    manifest = IntegrationManifest(
        target_integration.key, project_root, version=_get_agile_version()
    )
...
def integration_upgrade(
    ...
    if key == "kilocode" and legacy_command_root_upgrade_pending:
        ...
        try:
            ...
            console.print(
                ...
                "restore [cyan].agile/presets/.registry[/cyan] and retry."
            )
            raise typer.Exit(1)
        if affected_presets:
    ...
    if _manifest_tracks_skill_layout(old_manifest) != integration.is_skills_mode(
        ...
        try:
            ...
            console.print(
                ...
                "[cyan].agile/presets/.registry[/cyan] and retry."
            )
            raise typer.Exit(1)
        if affected_presets:
            preset_list = ", ".join(sorted(affected_presets))
    ...
    new_manifest = IntegrationManifest(key, project_root, version=_get_agile_version())
    ...
    try:
        ...
        if installed_key == key:
            ...
            _refresh_init_options_agile_version(project_root)
    except Exception as exc:
