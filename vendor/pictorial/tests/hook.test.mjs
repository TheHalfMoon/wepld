...
/**
 * Unit tests for the Pictorial design hook.
...
import { normalizeIgnoreValueEntries as normalizeIgnoreValueEntriesCli } from '../cli/lib/pictorial-config.mjs';
import { detectHtml, detectText } from '../cli/engine/detect-antipatterns.mjs';

function mkTmp() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'pictorial-hook-'));
}
...
describe('readConfig()', () => {
  ...
  it('parses hook runtime and legacy hook detector filters', () => {
    fs.mkdirSync(path.join(cwd, '.pictorial'), { recursive: true });
...
describe('ensureHookGitExcludes()', () => {
  ...
  it('adds hook runtime files to local git info exclude, not tracked .gitignore', () => {
    ...
    assert.match(exclude, /\.pictorial\/hook\.cache\.json/);
    assert.match(exclude, /\.pictorial\/hook\.pending\.json/);
    assert.match(exclude, /\.pictorial\/config\.local\.json/);

    const second = ensureHookGitExcludes(cwd);
    assert.equal(second.changed, false);
    const rewritten = fs.readFileSync(path.join(cwd, '.git', 'info', 'exclude'), 'utf-8');
    assert.equal((rewritten.match(/pictorial-hook-ignore-start/g) || []).length, 1);
  });
});
...
describe('hook-admin.mjs', () => {
  ...
  it('refuses an empty --file glob instead of silently writing a project-wide ignore', () => {
    ...
    assert.equal(fs.existsSync(path.join(cwd, '.pictorial', 'config.json')), false, 'nothing may be written');
  });
  ...
  it('matches an on-disk scope whose glob order differs from the sorted argv form', () => {
    ...
    fs.mkdirSync(path.join(cwd, '.pictorial'), { recursive: true });
    fs.writeFileSync(path.join(cwd, '.pictorial', 'config.json'), JSON.stringify({
    ...
    const cfg = JSON.parse(fs.readFileSync(path.join(cwd, '.pictorial', 'config.json'), 'utf-8'));
  ...
  it('ignore-value --local writes private config and status reports local ignores', () => {
    ...
    assert.match(status, /local file:\s+\.pictorial\/config\.local\.json/);
  ...
  // hook-lib.mjs (skill, ships into harness dirs) and cli/lib/pictorial-config.mjs
  ...
  it('both config normalizers emit identical entries', () => {
    ...
    assert.equal(
      ...
      'skill/scripts/hook-lib.mjs and cli/lib/pictorial-config.mjs must agree, key order included',
    );
  ...
  it('a /pictorial hooks edit preserves sibling hook fields (consent, quiet)', () => {
    fs.mkdirSync(path.join(cwd, '.pictorial'), { recursive: true });
  ...
  it('hooks on accepts declined consent and installs missing provider manifests', () => {
    ...
    for (const provider of ['.claude', '.agents', '.cursor', '.github']) {
      fs.mkdirSync(path.join(cwd, provider, 'skills', 'pictorial', 'scripts'), { recursive: true });
    }
    ...
    fs.writeFileSync(path.join(cwd, '.claude', 'settings.local.json'), JSON.stringify({
      hooks: {
        PostToolUse: [
          ...
          { matcher: 'Edit', hooks: [{ type: 'command', command: 'node ".claude/skills/pictorial/scripts/hook.mjs"' }] },
        ],
      },
    }));
    ...
    // pictorial entry must have been stripped, not accumulated.
    assert.equal(claude.split('skills/pictorial/scripts/hook.mjs').length - 1, 2);
    assert.match(claude, /"Stop"/);
    const claudeManifest = JSON.parse(claude);
    const pictorialGroup = claudeManifest.hooks.PostToolUse.find((group) =>
      group.hooks?.some((hook) => hook.command?.includes('skills/pictorial/scripts/hook.mjs')));
    assert.ok(pictorialGroup, 'repaired Claude settings should contain the Pictorial PostToolUse group');
    assert.equal(pictorialGroup.matcher, 'Edit|Write');

    const codex = fs.readFileSync(path.join(cwd, '.codex', 'hooks.json'), 'utf-8');
    assert.match(codex, /\.agents\/skills\/pictorial\/scripts\/hook\.mjs/);
    const cursor = fs.readFileSync(path.join(cwd, '.cursor', 'hooks.json'), 'utf-8');
    assert.match(cursor, /\.cursor\/skills\/pictorial\/scripts\/hook-before-edit\.mjs/);
    const github = JSON.parse(fs.readFileSync(path.join(cwd, '.github', 'hooks', 'pictorial.json'), 'utf-8'));
    assert.equal(github.hooks.postToolUse[0].matcher, 'edit|create|apply_patch');
    assert.match(github.hooks.postToolUse[0].bash, /\.github\/skills\/pictorial\/scripts\/hook\.mjs/);
  });
...
describe('renderTemplate()', () => {
  it('starts with the versioned envelope and caps to maxFindings', () => {
    ...
    assert.match(text, /\.\.\. and 7 more \(see \/pictorial audit\)\./);
  ...
  it('emits a directive footer (triage branches + executable self-serve ignore + honest provenance)', () => {
    ...
    assert.match(text, /Full suppression ladder: \/pictorial hooks/);
  });
  ...
  it('does not suggest ignore-value for rules that cannot be value-filtered', () => {
    ...
    assert.doesNotMatch(text, /\/pictorial hooks ignore-value side-tab Inter/);
  });
...
describe('writeAuditLog()', () => {
  ...
  it('appends NDJSON when PICTORIAL_HOOK_LOG is set', () => {
    const log = path.join(cwd, 'audit.ndjson');
    writeAuditLog({ PICTORIAL_HOOK_LOG: log }, { event: 'PostToolUse', emitted: true });
    writeAuditLog({ PICTORIAL_HOOK_LOG: log }, { event: 'PostToolUse', emitted: false });
  ...
  it('is a no-op when PICTORIAL_HOOK_LOG is unset', () => {
  ...
  it('prefers the env var over config hook.auditLog', () => {
    ...
    writeAuditLog({ PICTORIAL_HOOK_LOG: envLog }, { event: 'PostToolUse' }, cwd);
  ...
  it('resolves config auditLog from entry.cwd (the event project root), not the fallback cwd', () => {
    ...
    fs.mkdirSync(path.join(projectDir, '.pictorial'), { recursive: true });
    fs.writeFileSync(path.join(projectDir, '.pictorial', 'config.json'),
...
`);
  ...
  it('emits a clean ack when the file has zero findings', async () => {
    ...
    assert.match(r.stdout, /keep following the project design system and the pictorial skill guidance/);
    assert.equal(r.audit.emitted, true);
  ...
  it('PICTORIAL_HOOK_QUIET=1 suppresses clean and pending acks, keeps findings emission', async () => {
    ...
    const rClean = await runHook({
      ...
      env: { PICTORIAL_HOOK_QUIET: '1' }, cwd, detector: detClean,
    });
    ...
    const rFindings = await runHook({
      ...
      env: { PICTORIAL_HOOK_QUIET: '1' }, cwd, detector: detFindings,
    });
  ...
  it('re-entrancy guard short-circuits when PICTORIAL_HOOK_DEPTH is set', async () => {
    const file = writeFixture('src/Card.tsx', 'noop');
    const det = fakeDetector([finding('side-tab', 1)]);
    const r = await runHook({
      stdinJson: JSON.stringify(eventFor(file)),
      env: { PICTORIAL_HOOK_DEPTH: '1' },
      cwd,
      detector: det,
    });
    assert.equal(r.stdout, '');
  ...
  it('PICTORIAL_HOOK_DISABLED kill switch', async () => {
    const file = writeFixture('src/Card.tsx', 'noop');
    const det = fakeDetector([finding('side-tab', 1)]);
    for (const v of ['1', 'true', 'yes', 'on', 'TRUE']) {
      const r = await runHook({
        stdinJson: JSON.stringify(eventFor(file)),
        env: { PICTORIAL_HOOK_DISABLED: v },
        cwd,
        detector: det,
      });
  ...
  it('adds a non-blocking note when DESIGN.md is newer than the sidecar', async () => {
    ...
    assert.match(r.stdout, /No deterministic design-quality issues found/);
    assert.match(r.stdout, /DESIGN\.md is newer than \.pictorial\/design\.json/);
    assert.match(r.stdout, /\/pictorial document/);
  });
  ...
  it('rejects files outside the project, like harness scratchpads', async () => {
    ...
    try {
      ...
      assert.ok(!fs.existsSync(path.join(cwd, '.pictorial')), 'out-of-project edit must not dirty the cache');
    } finally {
      fs.rmSync(scratch, { recursive: true, force: true });
    }
  });
  ...
  it('emits suppressionNotice text on the threshold-crossing edit', async () => {
    ...
    assert.match(r.stdout, /Run \/pictorial audit to revisit/);
  });
  ...
  it('honors an inline pictorial-disable comment so the hook scans the file clean', async () => {
    ...
    const waived = writeFixture('src/Waived.tsx',
      'const css = "font-family: Inter"; // pictorial-disable-line overused-font');
...
describe('runHook() — cache write gating (issues #344, #305)', () => {
  ...
  // `.pictorial/` footprint: skipped files never dirty the cache, and a
  // dirty cache is only persisted when there are fresh findings or the
  // project already opted in (an `.pictorial/` dir exists).
  let cwd;
  beforeEach(() => { cwd = mkTmp(); });
  afterEach(() => fs.rmSync(cwd, { recursive: true, force: true }));
  ...
  it('non-UI edit (.md) does not create .pictorial/', async () => {
    ...
    assert.equal(r.audit.skipped, 'extension');
    assert.ok(!fs.existsSync(path.join(cwd, '.pictorial')), '.pictorial should not exist');
  });

  it('clean UI edit in a project with no footprint does not create .pictorial/, still acks', async () => {
  ...
  it('detector-missing path does not create .pictorial/', async () => {
    const file = write('src/Card.tsx', 'noop');
    const r = await runHook({
      stdinJson: JSON.stringify(eventFor(file)),
      env: {}, cwd, detector: {},
  ...
  it('fresh findings create the cache, and dedup works on the next run', async () => {
    ...
    const first = await runHook({ stdinJson: JSON.stringify(eventFor(file)), env: {}, cwd, detector: det });
    assert.match(first.stdout, /Design hook findings requiring review/);
    assert.ok(fs.existsSync(path.join(cwd, '.pictorial', 'hook.cache.json')), 'cache should exist');
  ...
  it('clean UI edit in an opted-in project (existing .pictorial/) still persists editCount', async () => {
    fs.mkdirSync(path.join(cwd, '.pictorial'), { recursive: true });
    const file = write('src/Card.tsx', 'noop');
  ...
  it('umbrella launch keys the cache to the edited file\'s project root', async () => {
    // cwd is the umbrella: no .git / package.json / .pictorial of its own.
    ...
    assert.ok(fs.existsSync(path.join(child, '.pictorial', 'hook.cache.json')), 'cache should land in the child project');
    assert.ok(!fs.existsSync(path.join(cwd, '.pictorial')), 'umbrella root should stay clean');
  });
});
...
describe('runHook() — session-scoped notices', () => {
  ...
  it('mentions the DESIGN.md staleness note once per session', async () => {
    ...
    const r1 = await runHook({ stdinJson: event(a), env: {}, cwd, detector: det });
    assert.match(r1.stdout, /DESIGN\.md is newer than \.pictorial\/design\.json/);
...
describe('runHook() — clean-ack noise', () => {
  ...
  it('does not spend the clean ack while quiet mode is suppressing output', async () => {
    ...
    const quiet = await runHook({ stdinJson: event(file), env: { PICTORIAL_HOOK_QUIET: '1' }, cwd, detector: det });
...
describe('resolveCacheCwd()', () => {
  ...
  it('keeps the session cwd when it already looks like a project root', () => {
    for (const marker of ['.git', '.pictorial']) {
...
describe('suppressionNotice()', () => {
  it('starts with envelope and mentions /pictorial audit', () => {
    ...
    assert.match(text, /\/pictorial audit/);
  });
});
...
describe('renderCleanAck() / renderPendingAck()', () => {
  it('renderCleanAck stays short and ends with the steer line', () => {
    ...
    assert.match(text, /^\[pictorial@1\] Design hook scanned src\/App\.jsx\. No deterministic design-quality issues found\./);
    assert.match(text, /keep following the project design system and the pictorial skill guidance/);
  ...
  it('renderPendingAck quotes up to 3 known findings and counts the rest', () => {
    ...
    assert.match(text, /^\[pictorial@1\] Design hook scanned src\/SlopCard\.jsx\./);
...
describe('resolveHarness() / normalizeHookEvent()', () => {
  it('routes explicit env and Cursor conversation_id to cursor harness', () => {
    assert.equal(resolveHarness({ PICTORIAL_HOOK_HARNESS: 'cursor' }), 'cursor');
  ...
  it('routes a GitHub Copilot postToolUse event (toolName/toolArgs) to the github harness', () => {
    ...
    assert.equal(resolveHarness({ PICTORIAL_HOOK_HARNESS: 'github' }), 'github');
...
describe('expandScanTargets()', () => {
  ...
  it('does not follow imports from traversal-looking primary targets', () => {
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'pictorial-hook-outside-'));
    try {
...
describe('Cursor hook scripts', () => {
  ...
  it('preToolUse denies proposed writes with detector findings before they land', () => {
    ...
    const out = execFileSync(process.execPath, [path.join('skill', 'scripts', 'hook-before-edit.mjs')], {
      ...
      env: { ...process.env, PICTORIAL_HOOK_LOG: logPath },
      encoding: 'utf-8',
    });
  ...
  it('preToolUse delivers the stale-sidecar note within a 500-char budget (PR #508)', () => {
    ...
    const sidecarPath = path.join(cwd, '.pictorial', 'design.json');
...
`);
    ...
    const out = execFileSync(process.execPath, [path.join('skill', 'scripts', 'hook-before-edit.mjs')], {
      ...
      env: { ...process.env, PICTORIAL_HOOK_LOG: '' },
      encoding: 'utf-8',
    });
  ...
  it('preToolUse honors truthy PICTORIAL_HOOK_DISABLED values before stdin parsing', () => {
    const logPath = path.join(cwd, 'hook.ndjson');
    ...
    const out = execFileSync(process.execPath, [path.join('skill', 'scripts', 'hook-before-edit.mjs')], {
      ...
      env: {
        ...process.env,
        PICTORIAL_HOOK_DISABLED: 'true',
        PICTORIAL_HOOK_LOG: logPath,
      },
      encoding: 'utf-8',
    });
...
describe('runHook() — emission enrichment', () => {
  ...
  it('returns emission.kind fresh with findings on new hits', async () => {
    ...
    const r = await runHook({
      ...
      env: { PICTORIAL_HOOK_HARNESS: 'claude' },
      cwd,
...
describe('runStopHook()', () => {
  ...
  it('skips out-of-project files even when an older cache still lists them', async () => {
    ...
    const scratch = fs.mkdtempSync(path.join(os.tmpdir(), 'pictorial-scratch-'));
    try {
      const outside = path.join(scratch, 'landing.html');
  ...
  it('honors kill switches and the re-entrancy guard', async () => {
    const disabled = await runStopHook({
      ...
      env: { PICTORIAL_HOOK_DISABLED: '1' }, cwd,
    });
    ...
    const reentrant = await runStopHook({
      ...
      env: { PICTORIAL_HOOK_DEPTH: '1' }, cwd,
    });
