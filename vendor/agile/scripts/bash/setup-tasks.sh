...
if [[ ! -f "$IMPL_PLAN" ]]; then
    ...
    echo "Run $(format_agile_command plan "$REPO_ROOT") first to create the implementation plan." >&2
    exit 1
fi

if [[ ! -f "$FEATURE_SPEC" ]]; then
    echo "ERROR: spec.md not found in $FEATURE_DIR" >&2
    echo "Run $(format_agile_command specify "$REPO_ROOT") first to create the feature structure." >&2
    exit 1
fi
...
if TASKS_TEMPLATE_CONTENT=$(resolve_template_content "tasks-template" "$REPO_ROOT"; status=$?; printf x; exit "$status"); then
    ...
    echo "Template 'tasks-template' was not found in any supported location (overrides, presets, extensions, or shared core). Add an override at .agile/templates/overrides/tasks-template.md, or run 'agile init' / reinstall shared infra to restore the core .agile/templates/tasks-template.md template." >&2
    exit 1
fi
