...
Presets customize how Agile works — overriding templates, commands, and terminology without changing any tooling. They let you enforce organizational standards, adapt the workflow to your methodology, or localize the entire experience. Multiple presets can be stacked with priority ordering.
...
> **Note:** All preset commands require a project already initialized with `agile init`.
...
Adds a catalog to the project's `.agile/preset-catalogs.yml`.
...
1. **Environment variable** — `AGILE_PRESET_CATALOG_URL` overrides all catalogs
2. **Project config** — `.agile/preset-catalogs.yml`
3. **User config** — `~/.agile/preset-catalogs.yml`
4. **Built-in defaults** — official catalog + community catalog

Example `.agile/preset-catalogs.yml`:
...
flowchart TB
    subgraph stack [" "]
        direction TB
        A["⬆ Highest precedence<br/><br/>1. Project-local overrides<br/>.agile/templates/overrides/"]
        B["2. Presets — by priority<br/>.agile/presets/‹id›/"]
        C["3. Extensions — by priority<br/>.agile/extensions/‹id›/"]
        D["4. Agile core<br/>.agile/templates/<br/><br/>⬇ Lowest precedence"]
    end
...
| --------- | -------------- | ------------------------------------------ |
| Templates | `templates/`   | `.agile/templates/overrides/`            |
| Commands  | `commands/`    | `.agile/templates/overrides/`            |
| Scripts   | `scripts/`     | `.agile/templates/overrides/scripts/`    |
...
Most presets are independently created and maintained by their respective authors. The Agile maintainers do not review, audit, endorse, or support preset code. Review a preset's source code before installing and use at your own discretion. For issues with a specific preset, contact its author or file an issue on the preset's repository.
