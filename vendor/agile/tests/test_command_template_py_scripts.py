...
(interpreter-prefixed, path rewritten to the ``.agile`` tree).
...
def test_template_renders_python_invocation(name: str):
    ...
    assert re.search(
        r"python3 \.agile/scripts/python/\w+\.py(?: [\w-]+)*", result
...
def test_spaced_python_interpreter_uses_powershell_call_operator(monkeypatch):
    ...
    assert (
        ...
        ".agile/scripts/python/setup_plan.py --json"
    ) in result
...
def test_spaced_python_interpreter_invocation_runs_in_powershell(
    ...
    monkeypatch.setattr(
        "agile_cli.integrations.base.sys.executable", str(interpreter)
    )
    monkeypatch.setattr(
        "agile_cli.integrations.base.os", SimpleNamespace(name="nt")
    )
...
def test_install_shared_infra_copies_python_scripts(tmp_path):
    # --script py must install scripts/python/ into .agile/scripts/python/
    ...
    scripts_dir = tmp_path / ".agile" / "scripts"
