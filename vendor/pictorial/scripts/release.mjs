...
const COMPONENTS = {
  ...
  cli: {
    ...
    tweetHeader: (v) => `Pictorial CLI v${v} is out.`,
    tweetCta: 'npm i -g pictorial',
  },
  extension: {
    ...
    tweetHeader: (v) => `Pictorial browser extension v${v} is out.`,
    tweetCta: null,
  },
};

const REPO_URL = 'https://github.com/TheHalfMoon/wepld';
...
// The site (and its changelog) lives in the private pictorial-site repo;
...
const changelogCandidates = [
  ...
  path.join(repoRoot, '..', 'pictorial-site', 'site/pages/changelog.astro'),
];
...
if (component === 'skill' && !dryRun) {
  try {
    const res = await fetch('https://github.com/TheHalfMoon/wepld/api/version');
    ...
    console.log('⚠ could not reach github.com/TheHalfMoon/wepld/api/version to verify the served bundle');
  }
}
...
console.log(`\n--- Tweet (${tweet.length}/${TWEET_LIMIT} chars) for @pictorial_ai ---`);
console.log(tweet);
