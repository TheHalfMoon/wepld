...
``.factory/skills/agile-<name>/SKILL.md``. Agile installs into that
...
class DroidIntegration(SkillsIntegration):
    ...
    def post_process_skill_content(self, content: str) -> str:
        ...
        hyphenated ``/agile-<name>`` invocations, not dotted ``/agile.<name>``)
    ...
    def build_exec_args(
        ...
        *,
        ...
        Uses ``droid exec "<prompt>"`` for headless dispatch. Agile does
        not auto-apply any permission-bypass flag: operators who want to
        skip interactive confirmation can pass it through
        ``AGILE_INTEGRATION_DROID_EXTRA_ARGS`` (e.g.
        ``AGILE_INTEGRATION_DROID_EXTRA_ARGS="--skip-permissions-unsafe"``).
        ...
        ``AGILE_INTEGRATION_DROID_EXTRA_ARGS`` are appended after the
        canonical Agile flags so the canonical flags are guaranteed to
        ...
        # Operator-injected extra args are appended after Agile's
        # canonical --model / --output-format flags so the canonical
        # flags are guaranteed to be present in argv regardless of
        # whatever the operator passes via AGILE_INTEGRATION_DROID_EXTRA_ARGS.
