...
// boundaries; `.pictorial` is our own project marker.
const PROJECT_ROOT_MARKERS = ['.git', 'package.json', '.pictorial'];
...
function resolveDesignSidecarPath(cwd = process.cwd(), contextDir = cwd) {
  const candidates = [
    path.join(cwd, '.pictorial', 'design.json'),
...
//   - A directory carrying a project marker (.git / package.json / .pictorial)
