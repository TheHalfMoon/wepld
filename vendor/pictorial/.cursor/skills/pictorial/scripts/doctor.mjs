...
/**
 * Deep staleness pass over Pictorial's own project artifacts.
 *
...
function usage() {
  return [
    ...
    '',
    "Report drift between this project's Pictorial artifacts and what the",
    'installed version reads: PRODUCT.md, DESIGN.md and its sidecar,',
    '.pictorial/config.json, surface briefs, and the design hook.',
    '',
...
async function collect(cwd, targetOptions) {
  ...
  const findings = [
    ...
    ...checkHookInstallation({
      projectRoot,
      repoRoot: ctx.repoRoot,
      providerId: PICTORIAL_PROVIDER_ID,
    }),
...
function readProjectRootPatterns(repoRoot) {
  ...
  for (const name of ['config.json', 'config.local.json']) {
    try {
      const raw = JSON.parse(fs.readFileSync(path.join(repoRoot, '.pictorial', name), 'utf-8'));
...
function renderText(report, fixes) {
  ...
  lines.push(`Pictorial doctor: ${rel(report.projectRoot, process.cwd()) || '.'}`);
  ...
  if (fixes) {
    ...
    lines.push(`Run \`node doctor.mjs --fix\` to apply the automatic migrations, `
      + `or \`${PICTORIAL_COMMAND} doctor\` to work through all of them.`);
  }
...
if (invokedAsScript()) {
  cli().catch((err) => {
    process.stderr.write(`pictorial doctor failed: ${err?.message || err}\n`);
    process.exit(1);
  });
}
