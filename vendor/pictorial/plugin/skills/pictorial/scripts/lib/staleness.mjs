...
/**
 * Staleness detection for Pictorial's own project artifacts: PRODUCT.md,
 * DESIGN.md and its `.pictorial/design.json` sidecar, `.pictorial/config.json`,
 ...
 *   2. Schema drift. An artifact was written by an older Pictorial: fields it
...
const DIRECTION_WORK_PATHS = Object.freeze([
  path.join('.pictorial', 'surfaces'),
  path.join('.pictorial', 'mocks', 'decision'),
]);
 ...
 * pictorial-paths (which resolves the project root) and context.mjs (which
 * cannot import pictorial-paths without a cycle) share one definition of
 * where the retired locations are.
 */
export function designSidecarCandidatesFor(projectRoot, contextDir = projectRoot) {
  const candidates = [
    path.join(projectRoot, '.pictorial', 'design.json'),
 ...
 *
 * `sidecarCandidates` comes from pictorial-paths' resolver so this module
...
export function checkConfig({ projectRoot, repoRoot }) {
  ...
  for (const root of roots) {
    for (const name of ['config.json', 'config.local.json']) {
      const filePath = path.join(root, '.pictorial', name);
...
export function checkBuildPathUnset({ projectRoot, repoRoot, product }) {
  ...
  return [finding({
    ...
    artifact: 'config.json',
    filePath: '.pictorial/config.json',
    severity: 'mention',
    ...
    fix: 'Only when image generation exists in your tool surface, offer the choice once: '
      ...
      + 'Write the answer to `.pictorial/config.json` as `"buildPath": "comp"` or `"buildPath": "code"`, '
 ...
 */
export function checkProjectRoots({ patterns = [], candidates = [], configuredIn = '.pictorial/config.json' }) {
