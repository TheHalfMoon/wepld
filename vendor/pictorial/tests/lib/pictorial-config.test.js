...
} from '../../cli/lib/pictorial-config.mjs';

describe('cli/lib/pictorial-config', () => {
  let root;
  ...
  test('setHookConsent preserves unrelated keys in config.local.json', () => {
    mkdirSync(join(root, '.pictorial'), { recursive: true });
  ...
  test('writing consent gitignores config.local.json via .git/info/exclude', () => {
    ...
    expect(exclude).toContain('.pictorial/config.local.json');
    ...
    expect((again.match(/pictorial-config-ignore-start/g) || []).length).toBe(1);
  ...
  test('writeDetectionConfig local ignores do not create an implicit design-system override', () => {
    ...
    expect(readFileSync(join(root, '.git', 'info', 'exclude'), 'utf-8')).toContain('.pictorial/config.local.json');
  });
