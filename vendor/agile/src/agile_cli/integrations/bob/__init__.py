...
Bob 2.0 uses the ``.bob/skills/agile-<name>/SKILL.md`` layout by default.
...
def _warn_legacy_commands_deprecated() -> None:
    warnings.warn(
        ...
        "removed in a future Agile release. Omit --legacy-commands to use "
...
class _BobSkillsHelper(SkillsIntegration):
    """Default-mode helper: ``.bob/skills/agile-<name>/SKILL.md``.
...
class _BobMarkdownHelper(MarkdownIntegration):
    """Legacy-mode helper: ``.bob/commands/agile.<name>.md`` (Bob 1.x).
...
class BobIntegration(IntegrationBase):
    ...
    def is_skills_mode(
        ...
        3. Otherwise, when a *project_root* is supplied, the layout is inferred
           from **managed Agile artifacts** (see below).
        ...
        *does* call :meth:`setup` (see ``_migrate_commands.integration_upgrade``),
        ...
        inferred from managed Agile artifacts, not the mere presence of a
        ``.bob/skills/`` directory: a user may keep unrelated Bob 2 skills in
        ``.bob/skills/`` while their Agile commands still live in
        ``.bob/commands/agile.*.md``.  We therefore treat the project as
        legacy (command) mode only when managed Agile command files exist
        and no managed Agile skills (``agile-*`` skill dirs) do.  Passing
        ...
        if project_root is not None:
            ...
            has_managed_skills = any((bob_dir / "skills").glob("agile-*"))
            has_managed_commands = any((bob_dir / "commands").glob("agile.*.md"))
    ...
    def invoke_separator_for_mode(self, skills_enabled: bool) -> str:
        ...
        Skills projects render ``/agile-<cmd>``; legacy command projects
        render Bob 1.x ``/agile.<cmd>``.  Extension/preset registration
    ...
    def post_process_skill_content(self, content: str) -> str:
        ...
        ``/agile-*`` hook guidance that core Bob skills intentionally omit.
        """
