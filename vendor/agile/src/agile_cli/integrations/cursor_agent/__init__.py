...
Cursor Agent uses the ``.cursor/skills/agile-<name>/SKILL.md`` layout.
...
class CursorAgentIntegration(SkillsIntegration):
    ...
    config = {
        ...
        # IDE-first integration: ``agile init --integration cursor-agent`` must
        ...
        # ``shutil.which`` rather than as a hard ``agile init`` precheck.
        "requires_cli": False,
    }
    registrar_config = {
        "dir": ".cursor/skills",
    ...
    def build_exec_args(
        ...
        *,
        ...
        flag (which is reserved for the ``agile init`` precheck).
        ...
        * ``--force`` — auto-approve tool invocations (shell/write/MCP),
        ...
        ``specify workflow run agile --input integration=cursor-agent``
        ...
        files during ``/agile-*`` skill execution; without them the run
