...
function generateCounts(rootDir, skills, buildDir) {
  ...
  // of /pictorial. Count them from the command router table in SKILL.md.
  const pictorialSkill = skills.find(s => s.name === 'pictorial');
  let commandCount;
  if (pictorialSkill) {
    ...
    const routerMatches = pictorialSkill.body.match(/^\| `[^`]+` \|/gm);
...
/**
 * Narrow prose check for the pictorial skill source.
 *
...
`Pictorial. Design fluency for AI harnesses.
https://github.com/TheHalfMoon/wepld
...
async function build() {
  ...
  if (BUILD_OPTIONS.syncRootOutputs) {
    ...
    const deprecatedLocalSkills = [
      'frontend-design', 'teach-pictorial',
      'arrange', 'normalize', 'onboard', 'extract',
      // v3.0 consolidation: standalone skills -> /pictorial sub-commands
    ...
    const claudeSkillsSrc = path.join(DIST_DIR, 'claude-code', '.claude', 'skills', 'pictorial');
    if (fs.existsSync(claudeSkillsSrc)) {
      fs.mkdirSync(pluginSkillsDir, { recursive: true });
      copyDirSync(claudeSkillsSrc, path.join(pluginSkillsDir, 'pictorial'));
    }
