...
Bump rationale: Initial ratification of a concrete constitution for the brownfield
  Agile / wepld-agile codebase, derived from an exhaustive multi-pass analysis of
...
Templates reviewed for alignment:
  ✅ .agile/templates/plan-template.md — generic "Constitution Check" gate (line 39)
       remains valid; gates are now concretely populated by Principles I–V at plan time.
  ✅ .agile/templates/spec-template.md — no constitution-specific tokens; no change needed.
  ✅ .agile/templates/tasks-template.md — task categories (setup/foundational/story/polish)
       already accommodate testing + performance + UX tasks mandated here; no change needed.
  ✅ .github/agents/agile.*.agent.md — command guidance is agent-agnostic; no change needed.
...
Agile (the `wepld-agile` package and its bundled assets) is a local, offline-capable
...
are binding on all changes — including the `agile bundle` subcommand and any future
...
and one output grammar, then apply them to every group — including `agile bundle`.

### IV. Offline-First Performance & Resource Discipline

Agile is a local CLI; responsiveness, offline operability, and graceful degradation are the
performance contract.

- **`agile init` and core scaffolding MUST work fully offline** using bundled `core_pack`
...
- **Authority.** Principles I–V are binding gates. The `## Constitution Check` section of the
  plan template MUST be evaluated against these principles, and `/agile.analyze` treats
