...
from agile_cli._download_security import read_response_limited as _real_read_response_limited
from agile_cli._version import (
...
class TestUserStory1:
    def test_newer_available_prints_update_and_install_command(self):
        with patch("agile_cli._version._get_installed_version", return_value="0.7.4"), patch(
        ...
        assert "git+https://github.com/TheHalfMoon/wepld.git@v0.9.0" in output

    def test_up_to_date_prints_current_only(self):
        with patch("agile_cli._version._get_installed_version", return_value="0.9.0"), patch(
            "agile_cli.authentication.http.urllib.request.urlopen",
            return_value=mock_urlopen_response({"tag_name": "v0.9.0"}),
        ):
            result = runner.invoke(app, ["self", "check"])
    ...
    def test_dev_build_ahead_of_release_is_up_to_date(self):
        with patch("agile_cli._version._get_installed_version", return_value="0.7.5.dev0"), patch(
            "agile_cli.authentication.http.urllib.request.urlopen",
    ...
    def test_unknown_installed_still_prints_latest_and_reinstall(self):
        with patch("agile_cli._version._get_installed_version", return_value="unknown"), patch(
            "agile_cli.authentication.http.urllib.request.urlopen",
            return_value=mock_urlopen_response({"tag_name": "v0.7.4"}),
        ):
            result = runner.invoke(app, ["self", "check"])
    ...
    def test_unknown_installed_uses_placeholder_when_latest_tag_is_invalid(self):
        ...
        assert "git+https://github.com/TheHalfMoon/wepld.git@vX.Y.Z" in output
...
class TestBoundedRead:
    ...
    def test_response_body_is_bounded(self):
        ...
        ), patch("agile_cli._version.read_response_limited", side_effect=_spy):
            tag, reason = _fetch_latest_release_tag()
...
class TestUserStory2:
    ...
    def test_failure_prints_installed_plus_one_line_reason(
        ...
        with patch("agile_cli._version._get_installed_version", return_value="0.7.4"), patch(
            "agile_cli.authentication.http.urllib.request.urlopen", side_effect=side_effect
        ):
            result = runner.invoke(app, ["self", "check"])
        output = strip_ansi(result.output)
        ...
        if expected_reason == _RATE_LIMITED_REASON:
            ...
            assert "~/.agile/auth.json" in output
        else:
...
class TestUserStory3:
    ...
    def test_no_authorization_header_when_both_unset(self, monkeypatch):
        ...
        with patch("agile_cli.authentication.http.urllib.request.urlopen", side_effect=side_effect):
            _fetch_latest_release_tag()
        req = captured["request"]
